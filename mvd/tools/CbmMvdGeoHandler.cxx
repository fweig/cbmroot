...
void CbmMvdGeoHandler::Init(Bool_t isSimulation)
{
   ...
   if(!isSimulation)
   {
	  ...
	  else  LOG(info)<<"Using Mimosa style sensor";
...
if(fail)
    LOG(fatal)<<"MVD Geometry included, but pipe not found please check your setup";
}
//--------------------------------------------------------------------------
		...
		for(Int_t QuadNr = 0; QuadNr < 4; QuadNr++)
			...
			if(fSensorTyp == CbmMvdSensorTyp::MIMOSIS)
			    ...
			    fDetector->AddSensor(fSensorName, fSensorName, fnodeName, new CbmMvdMimosa26AHR, iStation, fVolId, 0.0, StatNr);
		       	iStation++;
			FillParameter();
                         LOG(debug1) << "found " << fSensorHolding + "/" + fSensorName <<  " number: " << fVolId << "  and added to MVD Detector";
			}
                      else break;
  ...
  if(fGeoTyp == FourStation || fGeoTyp == FourStationShift) {
	...
	for(Int_t SensNr = 0; SensNr < 50; SensNr++) {
	  ...
	  if(nodeFound) {
	    gGeoManager->cd(fnodeName);
	    fVolId = GetIDfromPath(fnodeName);
	    LOG(debug) << "found " << fnodeName << " number: " << iStation << " ID: " << fVolId << " and added to station map";
	    fStationMap[fVolId] = iStation;
	    iStation++;
