...
void CbmMvdGeoHandler::Init(Bool_t isSimulation) {
  ...
  if (!isSimulation) {
    if (fSensorTyp == CbmMvdSensorTyp::MIMOSIS)
      LOG(info) << "Using Mimosis style sensor";
    else
...
void CbmMvdGeoHandler::GetPipe() {
  ...
  for (Int_t iNode = 0; iNode < nodes->GetEntriesFast(); iNode++) {
    ...
    if (TString(node->GetName())
          .Contains(pipeName, TString::ECaseCompare::kIgnoreCase)) {
  ...
  if (fail) {
    ...
    for (Int_t iNode = 0; iNode < nodes->GetEntriesFast(); iNode++) {
      TGeoNode* node = (TGeoNode*) nodes->At(iNode);
      if (TString(node->GetName())
            .Contains("mvd", TString::ECaseCompare::kIgnoreCase)) {
        motherName = node->GetName();
        fMother    = "cave_1";
        LOG(debug) << "MvdGeoHandler found Mother: " << fMother;
        LOG(warn)
...
void CbmMvdGeoHandler::GetGeometryTyp() {
  if (gGeoManager->CheckPath(fMother + "/Beamtimeosetupoobgnum_0")) {
    ...
    fGeoTyp = beamtest;
               ...
               fMother + "/MVDoMistraloquero012oStationo150umodigi_0")) {
    LOG(debug) << "Found MVD with 3 Stations";
    fGeoTyp = ThreeStation;
  } else if (gGeoManager->CheckPath(fMother
  ...
  if (fGeoTyp == FourStation || fGeoTyp == FourStationShift) {
      ...
      for (Int_t QuadNr = 0; QuadNr < 4; QuadNr++) {
        ...
        for (Int_t Layer = 0; Layer < 2; Layer++) {
          ...
          for (Int_t SensNr = 0; SensNr < 50; SensNr++) {
            fSensorHolding = Form("/MVD-S%i-Q%i-L%i-C%02i-P0oPartAss_1",
                                  StatNr,
                                  QuadNr,
                                  Layer,
                                  SensNr);
            fSensorName =
              Form("MVD-S%i-Q%i-L%i-C%02i-P0", StatNr, QuadNr, Layer, SensNr);
            ...
            if (fVolId > -1)
              for (Int_t SegmentNr = 0; SegmentNr < 50; SegmentNr++) {
                ...
                fnodeName   = fMother + fDetectorName + fStationName
                            + fQuadrantName + fSectorName + fSensorHolding + "/"
                            + fSensorName + "_1";
                LOG(debug) << "looking for " << fnodeName;
                Bool_t nodeFound = gGeoManager->CheckPath(fnodeName.Data());
                if (nodeFound) {
                  fDetector->AddSensor(fSensorName,
                                       fSensorName,
                                       fnodeName,
                                       new CbmMvdMimosa26AHR,
                                       iStation,
                                       fVolId,
                                       0.0,
                                       StatNr);
    ...
    for (Int_t StatNr = 0; StatNr < 4; StatNr++) {
      ...
      for (Int_t QuadNr = 0; QuadNr < 4; QuadNr++) {
        ...
        for (Int_t SensNr = 0; SensNr < 50; SensNr++) {
          ...
          fnodeName = fMother + fDetectorName + fStationName + fQuadrantName
                      + fSensorHolding + "/" + fSensorName + "_1";
          ...
          if (nodeFound) {
            ...
            iStation++;
            FillParameter();
            LOG(debug1) << "found " << fSensorHolding + "/" + fSensorName
                        << " number: " << fVolId
    ...
    for (Int_t StatNr = 0; StatNr < 2; StatNr++) {
      ...
      for (Int_t Layer = 0; Layer < 2; Layer++) {
        for (Int_t SensNr = 0; SensNr < 50; SensNr++) {
          fQuadrantName =
            Form("/MVD-S%i-Q0-L%i-C%02i_1", StatNr, Layer, SensNr);
          fSensorHolding =
            Form("/MVD-S%i-Q0-L%i-C%02i-P0oPartAss_1", StatNr, Layer, SensNr);
      ...
      for (Int_t QuadNr = 0; QuadNr < 4; QuadNr++) {
        ...
        for (Int_t SensNr = 0; SensNr < 50; SensNr++) {
          ...
          if (nodeFound) {
            gGeoManager->cd(fnodeName);
            fVolId = GetIDfromPath(fnodeName);
            LOG(debug) << "found " << fnodeName << " number: " << iStation
                       << " ID: " << fVolId << " and added to station map";
            fStationMap[fVolId] = iStation;
            iStation++;
