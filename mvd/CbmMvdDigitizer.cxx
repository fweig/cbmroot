...
InitStatus CbmMvdDigitizer::Init() {
  ...
  cout << "-I- Initialising " << GetName() << " ...." << endl;
    ...
    if (! fInputPoints ) {
      LOG(error) << "No MvdPoint branch found. There was no MVD in the simulation. Switch this task off";
      return kERROR;
    }
	...
	fPileupManager = new CbmMvdPileupManager(fBgFileName,
						 fInputBranchName, fBgBufferSize);
	...
	fDeltaManager = new CbmMvdPileupManager(fDeltaFileName,
						fInputBranchName, fDeltaBufferSize);
