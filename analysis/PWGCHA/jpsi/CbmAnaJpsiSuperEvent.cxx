...
void CbmAnaJpsiSuperEvent::InitHist() {
  ...
  fHM->Create1<TH1D>("fh_se_bg_minv_reco",
                     "fh_se_bg_minv_reco;M_{ee} [GeV/c^{2}];particles/event",
                     4000,
                     0,
                     4.);
  fHM->Create1<TH1D>(
    "fh_se_bg_minv_chi2prim",
    "fh_se_bg_minv_chi2prim;M_{ee} [GeV/c^{2}];particles/event",
    4000,
    0,
    4.);
  fHM->Create1<TH1D>("fh_se_bg_minv_elid",
                     "fh_se_bg_minv_elid;M_{ee} [GeV/c^{2}];particles/event",
                     4000,
                     0,
                     4.);
  fHM->Create1<TH1D>("fh_se_bg_minv_ptcut",
                     "fh_se_bg_minv_ptcut;M_{ee} [GeV/c^{2}];particles/event",
                     4000,
                     0,
                     4.);

  fHM->Create1<TH1D>(
    "fh_se_event_number", "fh_se_event_number;a.u.;Number of events", 1, 0, 1.);

  fHM->Create1<TH1D>(
    "fh_se_bg_participants_minv_gg",
    "fh_se_bg_participants_minv_gg;M_{ee} [GeV/c^{2}];particles/event",
    4000,
    0,
    4.);
  fHM->Create1<TH1D>(
  ...
  fHM->Create1<TH1D>("fh_SE_PdgCode_of Others_BG",
                     "fh_SE_PdgCode_of Others_BG;PDGCode;Tracks per Event",
                     500,
                     -0.5,
                     499.5);
  ...
  fHM->Create1<TH1D>(
    ...
    "fh_se_bg_truematch_minv_ptCut;M_{ee} [GeV/c^{2}];particles/event",
    4000,
    0.,
    4.);
  fHM->Create1<TH1D>(
    "fh_se_bg_truematch_el_minv_ptCut",
    "fh_se_bg_truematch_el_minv_ptCut;M_{ee} [GeV/c^{2}];particles/event",
    4000,
    0.,
    4.);
  fHM->Create1<TH1D>(
    "fh_se_bg_truematch_notel_minv_ptCut",
    "fh_se_bg_truematch_notel_minv_ptCut;M_{ee} [GeV/c^{2}];particles/event",
    4000,
    0.,
    4.);
...
void CbmAnaJpsiSuperEvent::ReadCandidates() {
  ...
  for (UInt_t iFile = 0; iFile < fFileNames.size(); iFile++) {
    ...
    TClonesArray* candidates =
      (TClonesArray*) fd->FindObjectAny("JpsiCandidates");
    ...
    for (Int_t iEv = 0; iEv < nofEvents; iEv++) {
      ...
      for (Int_t iCand = 0; iCand < nofCandidates; iCand++) {
        CbmAnaJpsiCandidate* cand =
          (CbmAnaJpsiCandidate*) candidates->At(iCand);

        if (cand->fIsMcSignalElectron) continue;

        Bool_t isIdCut =
          (cand->fChi2Prim < fCuts.fChiPrimCut && cand->fIsElectron);
  ...
  for (Int_t iM = 0; iM < nofMinus; iM++) {
    ...
    for (Int_t iP = 0; iP < nofPlus; iP++) {
      ...
      CbmAnaJpsiKinematicParams pRec =
        CbmAnaJpsiKinematicParams::KinematicParamsWithCandidates(candM, candP);
      Bool_t isChi2Primary = (candM->fChi2Prim < fCuts.fChiPrimCut
                              && candP->fChi2Prim < fCuts.fChiPrimCut);
      Bool_t isEl          = (candM->fIsElectron && candP->fIsElectron);
      Bool_t isPtCut       = (candM->fMomentum.Perp() > fCuts.fPtCut
                        && candP->fMomentum.Perp() > fCuts.fPtCut);
      ...
      if (isChi2Primary && isEl)
        fHM->H1("fh_se_bg_minv_elid")->Fill(pRec.fMinv);
      if (isChi2Primary && isEl && isPtCut) {
        ...
        if (candM->fIsMcGammaElectron) {
          if (candP->fIsMcGammaElectron) {
            fHM->H1("fh_se_bg_participants_minv_gg")
              ->Fill(pRec.fMinv);  //gamma + gamma
          } else if (candP->fIsMcPi0Electron) {
            fHM->H1("fh_se_bg_participants_minv_gp")
              ->Fill(pRec.fMinv);  //gamma + Pi0
          } else {
            fHM->H1("fh_se_bg_participants_minv_go")
              ->Fill(pRec.fMinv);  //gamma + other
            fHM->H1("fh_SE_PdgCode_of Others_BG")
              ->Fill((double) candP->fMcPdg - 0.5);
          }
        } else if (candM->fIsMcPi0Electron) {
          if (candP->fIsMcGammaElectron) {
            fHM->H1("fh_se_bg_participants_minv_gp")
              ->Fill(pRec.fMinv);  //pi0 + gamma
          } else if (candP->fIsMcPi0Electron) {
            fHM->H1("fh_se_bg_participants_minv_pp")
              ->Fill(pRec.fMinv);  //pi0 + Pi0
          } else {
            fHM->H1("fh_se_bg_participants_minv_po")
              ->Fill(pRec.fMinv);  //pi0 + other
            fHM->H1("fh_SE_PdgCode_of Others_BG")
              ->Fill((double) candP->fMcPdg - 0.5);
          }
        } else {
          ...
          if (candP->fIsMcGammaElectron) {
            fHM->H1("fh_se_bg_participants_minv_go")
              ->Fill(pRec.fMinv);  //other + gamma
          } else if (candP->fIsMcPi0Electron) {
            fHM->H1("fh_se_bg_participants_minv_po")
              ->Fill(pRec.fMinv);  //other + Pi0
          } else {
            fHM->H1("fh_se_bg_participants_minv_oo")
              ->Fill(pRec.fMinv);  //other + other
            fHM->H1("fh_SE_PdgCode_of Others_BG")
              ->Fill((double) candP->fMcPdg - 0.5);
          }
        }
        ...
        if (isBG && isMismatch)
          fHM->H1("fh_se_bg_mismatch_minv_ptCut")->Fill(pRec.fMinv);
        if (isBG && !isMismatch) {
          fHM->H1("fh_se_bg_truematch_minv_ptCut")->Fill(pRec.fMinv);
          if (candP->fMcPdg == 11 && candM->fMcPdg == 11)
            fHM->H1("fh_se_bg_truematch_el_minv_ptCut")->Fill(pRec.fMinv);
          if (candP->fMcPdg != 11 || candM->fMcPdg != 11)
            fHM->H1("fh_se_bg_truematch_notel_minv_ptCut")->Fill(pRec.fMinv);
        }
      }

      if (isChi2Primary && isEl && pRec.fPt < 0.4) {
        fHM->H1("fh_se_bg_minv_diff_ptcuts_0")->Fill(pRec.fMinv);
      }
...
void CbmAnaJpsiSuperEvent::Draw(Option_t*) {
  ...
  DrawTextOnPad(
    CbmAnaJpsiHist::fAnaStepsLatex[kJpsiReco], 0.6, 0.89, 0.7, 0.99);
  c->cd(2);
  DrawH1(fHM->H1("fh_se_bg_minv_chi2prim"));
  DrawTextOnPad(
    CbmAnaJpsiHist::fAnaStepsLatex[kJpsiChi2Prim], 0.6, 0.89, 0.7, 0.99);
  c->cd(3);
  DrawH1(fHM->H1("fh_se_bg_minv_elid"));
  DrawTextOnPad(
    CbmAnaJpsiHist::fAnaStepsLatex[kJpsiElId], 0.6, 0.89, 0.7, 0.99);
  c->cd(4);
