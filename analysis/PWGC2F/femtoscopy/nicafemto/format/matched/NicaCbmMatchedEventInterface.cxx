...
NicaCbmEventInterface(mode),fMuchMatches(NULL),fRichMatches(NULL)
{
	...
	switch(fMode){
		case kSis100Electron:{
			fRichMatches = new NicaTrackClones("CbmTrackMatchNew","RichRingMatch","RICH");
		}break;
		case kSis100Hadron:
			break;
		case kSis100Muon:{
			fMuchMatches = new NicaTrackClones("CbmTrackMatchNew","MuchTrackMatch","MUCH");
		}break;
	}
}
...
void NicaCbmMatchedEventInterface::ConnectToTree() {
	...
	if(fRichMatches)
		fRichMatches->GetFromTree();
	if(fMuchMatches)
		fMuchMatches->GetFromTree();
}
...
void NicaCbmMatchedEventInterface::Register(Bool_t write) {
	...
	if(fMuchMatches)
		fMuchMatches->Register(write);
	if(fRichMatches)
		fRichMatches->Register(write);
}
...
void NicaCbmMatchedEventInterface::FillTrackInterface(NicaTrackInterface *track,
	...
	Track->SetLinkSts((CbmTrackMatchNew*)fStsMatches->UncheckedAt(glob->GetStsTrackIndex()));
	Track->SetLinkTof((CbmTrackMatchNew*)fTofMatches->UncheckedAt(glob->GetTofHitIndex()));
	Track->SetLinkTrd((CbmTrackMatchNew*)fTrdMatches->UncheckedAt(glob->GetTrdTrackIndex()));
	if(fMuchMatches){
		Track->SetLinkMuch((CbmTrackMatchNew*)fMuchMatches->UncheckedAt(glob->GetMuchTrackIndex()));
	}
	if(fRichMatches){
		Track->SetLinkRich((CbmTrackMatchNew*)fRichMatches->UncheckedAt(glob->GetRichRingIndex()));
	}
