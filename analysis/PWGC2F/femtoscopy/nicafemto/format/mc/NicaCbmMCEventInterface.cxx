...
void NicaCbmMCEventInterface::CopyData(NicaEventInterface* s) {
	FairMCEventHeader *header = (FairMCEventHeader*)((NicaCbmMCEventInterface*)s)->fEvent;
	TClonesArray *tracks = (TClonesArray*)((NicaCbmMCEventInterface*)s)->fCbmTracks;
	...
	for(int i =0;i<fCbmTracks->GetEntriesFast();i++){
		CbmMCTrack* from = (CbmMCTrack*)((NicaCbmMCEventInterface*)s)->fCbmTracks->UncheckedAt(i);
		CbmMCTrack *to = (CbmMCTrack*)fCbmTracks->UncheckedAt(i);
		*to = *from;
	}
}

void NicaCbmMCEventInterface::CopyAndCompress(NicaEventInterface* s, Int_t* map,
		Int_t map_size) {
	FairMCEventHeader *header = (FairMCEventHeader*)((NicaCbmMCEventInterface*)s)->fEvent;
	fEvent->SetRunID(header->GetRunID());
	fEvent->SetEventID(header->GetEventID());
	...
	fCbmTracks->Clear();
	for(int i =0;i<map_size;i++){
		CbmMCTrack* from = (CbmMCTrack*)((NicaCbmMCEventInterface*)s)->fCbmTracks->UncheckedAt(map[i]);
