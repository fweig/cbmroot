...
void NicaCbmEventInterface::FillTrackInterface(NicaTrackInterface *track,
	...
	cbm_track->SetStsTrack((CbmStsTrack*)fStsTracks->UncheckedAt(glob->GetStsTrackIndex()));
	cbm_track->SetTofHit((CbmTofHit*)fTofHits->UncheckedAt(glob->GetTofHitIndex()));
	cbm_track->SetTrdTrack((CbmTrdTrack*)fTrdTracks->UncheckedAt(glob->GetTrdTrackIndex()));
	switch(fMode){
		case kSis100Electron:{
			cbm_track->SetRichRing((CbmRichRing*)fRichRings->UncheckedAt(glob->GetRichRingIndex()));
		}break;
		case kSis100Hadron:
			break;
		case kSis100Muon:{
			cbm_track->SetMuchTrack((CbmMuchTrack*)fMuchTracks->UncheckedAt(glob->GetMuchTrackIndex()));
		}break;
	}
}
