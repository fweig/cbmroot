...
InitStatus CbmTofHitFinderQa::Init()
{
   ...
   if(NULL != fTofId)
     LOG(info)<<"CbmTofHitFinderQa::Init with GeoVersion "<<fGeoHandler->GetGeoVersion();
   else
   {
      switch(iGeoVersion)
      {
...
Bool_t   CbmTofHitFinderQa::RegisterInputs()
{
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the MCEventHeader object!!!";
      return kFALSE;
   }
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofPoint TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofPointsColl)
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the MCTrack TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fMcTracksColl)
   ...
   {
      LOG(WARNING)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofDigi TClonesArray!!!";
      LOG(WARNING)<<"                                  => Assuming that the CbmTofHitProducerNew is used!!!";
 //     return kFALSE;
   } // if( NULL == fTofDigisColl)
   fTofDigiMatchPointsColl   = (TClonesArray *) fManager->GetObject("TofDigiMatchPoints");
   if( NULL == fTofDigiMatchPointsColl)
   {
      LOG(WARNING)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofDigiMatchPoints TClonesArray!!!";
      LOG(WARNING)<<"                                  => Assuming that the CbmTofHitProducerNew is used!!!";
//      return kFALSE;
   } // if( NULL == fTofDigiMatchPointsColl)
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofHit TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofHitsColl)
   fTofDigiMatchColl   = (TClonesArray *) fManager->GetObject("TofDigiMatch");
   if( NULL == fTofDigiMatchColl)
   {
      LOG(WARNING)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofDigiMatch TClonesArray!!!";
      LOG(WARNING)<<"                                  => Assuming that the CbmTofHitProducerNew is used!!!";
 //     return kFALSE;
   } // if( NULL == fTofDigiMatchColl)
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => Could not get the TofHitMatch TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofDigiMatchPointsColl)
   ...
   {
      LOG(error)<<"CbmTofHitFinderQa::RegisterInputs => fTofDigisColl or fTofDigiMatchPointsColl or fTofDigiMatchColl present while the other is missing";
      LOG(error)<<"                                  => Cannot be the result of CbmTofHitProducerNew use!!!";
      return kFALSE;
   ...
   if( NULL != fRealTofPointsColl && NULL != fRealTofMatchColl )
   {
      ...
      LOG(info)<<"CbmTofHitFinderQa::RegisterInputs => Both fRealTofPointsColl & fRealTofMatchColl there, realistic mean TOF MC point used for QA";
   } // if( NULL != fRealTofPointsColl && NULL != fRealTofMatchColl )
...
Bool_t CbmTofHitFinderQa::SetWallPosZ( Double_t dWallPosCm)
{
   ...
   LOG(info)<<"CbmTofHitFinderQa::SetWallPosZ => Change histograms center on Z axis to "<<dWallPosCm<<" cm";
   return kTRUE;
}
...
*/
   ...
   fhNbHitsSingPntPerEvent = new TH1I("TofTests_NbHitsSingPntPerEvent", "Number of Single Point Hits per event; EventsIdx []; # [Hits]",
                                                   fuNbEventsForHitsNbPlots, -0.5, fuNbEventsForHitsNbPlots - 0.5 );
   fhNbHitsMultPntPerEvent = new TH1I("TofTests_NbHitsMultPntPerEvent", "Number of Multi Point Hits per event; EventsIdx []; # [Hits]",
                                                   fuNbEventsForHitsNbPlots, -0.5, fuNbEventsForHitsNbPlots - 0.5 );
   fhNbHitsSingTrkPerEvent = new TH1I("TofTests_NbHitsSingTrkPerEvent", "Number of Single Track Hits per event; EventsIdx []; # [Hits]",
                                                   fuNbEventsForHitsNbPlots, -0.5, fuNbEventsForHitsNbPlots - 0.5 );
   fhNbHitsMultTrkPerEvent = new TH1I("TofTests_NbHitsMultTrkPerEvent", "Number of Multi Track Hits per event; EventsIdx []; # [Hits]",
                                                   fuNbEventsForHitsNbPlots, -0.5, fuNbEventsForHitsNbPlots - 0.5 );
   // Nb different TOF digis in Hit
   fhNbDigisInHit      = new TH1I("TofTests_NbDigisInHit", "Number of Digis per Hit; # [Digis]", 100, -0.5, 100 - 0.5 );
   fhNbDigisInHitMapXY = new TProfile2D( "TofTests_NbDigisInHitMapXY",
...
Bool_t CbmTofHitFinderQa::FillHistos()
{
   ...
   if( kFALSE == fbHitProducerSource )
   {
      ...
      if( iNbTofDigis != iNbTofDigisMatch )
         LOG(fatal)<<"CbmTofHitFinderQa::FillHistos => Nb entries in TofDigiMatchPoints TClonesArray doe not match nb entries in TofDigi!!!";
