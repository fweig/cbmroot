...
CbmHadronAnalysis::CbmHadronAnalysis()
  : FairTask("HadronAnalysis"),
    ...
    fdVLenMin(5.),        // Ext Parameter: Min Lambda flight path length for accepting pair
    fdVLenMax(25.),       // Ext Parameter: Max Lambda flight path length for accepting pair
    fdDistTRD(10.),       // max accepted distance of Trd Hit from STS-TOF line 
    fdTRDHmulMin(0.),     // min associated Trd Hits to Track candidates
    fNMixedEvents(1)
{
  CreateHistogramms();
...
void CbmHadronAnalysis::CreateHistogramms()
{
  ...
  fa_LenMcDismom_glo = new TH2F("LenMcDismom_glo","GlobalTrack(all); momentum [GeV]; len MC pnt - dis [cm];",
                          100,0.,10.,400.,-LenDisMax,LenDisMax);
...
#include "TVector3.h"
  ...
  */
  ...
  if(iCandEv==0) { //initialize
    ...
    fhdEdxMul= new TH2F( Form("hdEdxMul"),Form("average energy loss vs TrdHitMul; TrdHitMul; dE []"),4,1,5,100,0.,5.E-5); 
    fhdEdxMulsec= new TH2F( Form("hdEdxMulsec"),
			    Form("av. energy loss vs TrdHitMul for secondaries; TrdHitMul; dE []"),4,1,5,100,0.,5.E-5);  
    fhDTRDprim = new TH1F( Form("hDTRDprim"), Form("TRD transverse matching distance (prim); d [cm]"), 100, 0., 2.*fdDistTRD); 
    fhDTRDsec  = new TH1F( Form("hDTRDsec"), Form("TRD transverse matching distance (sec); d [cm]"), 100, 0., 2.*fdDistTRD); 
  ...
  std::vector< std::vector<Int_t> > iTRD; iTRD.resize(nTofHits);
  ...
  if(nTrdHits > 0) {
	...
	Double_t dDtrans = TMath::Sqrt(TMath::Power(dXexp - pTrdHit->GetX(), 2) + TMath::Power(dYexp - pTrdHit->GetY(), 2));
  ...
  for (Int_t i=0; i<nTofHits; i++) {
    ...
    if( iStsMin[i][0]>-1 && iStsMin[i][1]>-1 ) {
      ...
      if(dD0 > fdD0ProtLim && (Double_t)iTRD[i].size() >= fdTRDHmulMin) { // secondary proton canditate, memorize relevant quantities
  ...
  for (Int_t i=0; i<nTofHits; i++) {
	...
	if( dTofDistMin[j] > fdDistPrimLim) {  // Sts hit not in the primary class
	  ...
	  for (Int_t k=0; k<nStsHits; k++) {
	    ...
	    Double_t dDist     = TMath::Sqrt(dDist2);
	    fhDperpS->Fill(dDist);
	    LOG(DEBUG) << "Sec Tof "<<i<<", Sts "<<j<<Form(" Sts2 %d -> dist %6.3f < %6.3f ? at z = %4.1f",
							   k, dDist, TMath::Min(dDistMin,fdDistSecLim2), sPos2Z);
	...
	if ((Double_t)iTRD[i].size() >= fdTRDHmulMin) {
	  ...
	  P[i].SetPxPyPzE(px,py,pz,E);	 
	  X[i].SetXYZT(pTofHit->GetX(),pTofHit->GetY(),pTofHit->GetZ(),pTofHit->GetTime());	 
	  LOG(DEBUG)<<"Init pion LV at ind "<<i<<" with beta = "<<bet<<", minv = "<< P[i].M()
		    <<", tof "<<X[i].T()
		    <<", TRDHmul "<<iTRD[i].size();
	  X0[i].SetXYZ(pSts2Hit->GetX(),pSts2Hit->GetY(),pSts2Hit->GetZ());
	  DX[i].SetXYZ(dDx,dDy,dDz);
