...
public:
  /*
	 ...
	 */
  static void CalculateAndSetTrackParamsToCandidate(CbmLmvmCandidate* cand,
    ...
    cand->fRapidity = 0.5
                      * TMath::Log((cand->fEnergy + cand->fMomentum.Z())
                                   / (cand->fEnergy - cand->fMomentum.Z()));
  }

  /*
	 * Armenteros - Podolansky plot
	 */
  static void CalculateArmPodParams(CbmLmvmCandidate* cand1,
    ...
    if (cand1->fCharge < 0.) {
      ...
      pln = (cand1->fMomentum.X() * spx + cand1->fMomentum.Y() * spy
             + cand1->fMomentum.Z() * spz)
            / sp;
      plp = (cand2->fMomentum.X() * spx + cand2->fMomentum.Y() * spy
             + cand2->fMomentum.Z() * spz)
            / sp;
    } else {
      pn = cand2->fMomentum.Mag();
      //pp = cand1->fMomentum.Mag();
      pln = (cand2->fMomentum.X() * spx + cand2->fMomentum.Y() * spy
             + cand2->fMomentum.Z() * spz)
            / sp;
      plp = (cand1->fMomentum.X() * spx + cand1->fMomentum.Y() * spy
             + cand1->fMomentum.Z() * spz)
            / sp;
    }
    if (pn == 0.0) return;
