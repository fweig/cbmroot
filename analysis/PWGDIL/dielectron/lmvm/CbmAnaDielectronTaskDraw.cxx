...
void CbmAnaDielectronTaskDraw::DrawHistFromFile(const string& fileName,
...
void CbmAnaDielectronTaskDraw::RebinMinvHist() {
  ...
  for (int i = 0; i < CbmLmvmHist::fNofAnaSteps; i++) {
    ...
    H1("fh_bg_truematch_el_minv_" + CbmLmvmHist::fAnaSteps[i])
      ->Rebin(2 * nRebin);
    H1("fh_bg_truematch_notel_minv_" + CbmLmvmHist::fAnaSteps[i])
      ->Rebin(2 * nRebin);
...
TH1D* CbmAnaDielectronTaskDraw::H1(const string& name) {
...
*/
TH2D* CbmAnaDielectronTaskDraw::H2(const string& name) {
...
void CbmAnaDielectronTaskDraw::DrawEfficiencyOnHist(TH1* h1,
  ...
  string effTxt = "";
  if (h2->GetEntries() != 0) {
    effTxt = Cbm::NumberToString<Double_t>(
      ((Double_t) h1->GetEntries() / h2->GetEntries() * 100.), 1);
  }
...
TH2D* CbmAnaDielectronTaskDraw::CreateSignificanceH2D(TH2D* signal,
  ...
  TH2D* hsig = new TH2D(
    name.c_str(), title.c_str(), nStepsX, xmin, xmax, nStepsY, ymin, ymax);
...
void CbmAnaDielectronTaskDraw::DrawPtYDistributionAll() {
  ...
  fHM->CreateCanvas("lmvm_pty_" + CbmLmvmHist::fAnaSteps[kAcc],
                    "lmvm_pty_" + CbmLmvmHist::fAnaSteps[kAcc],
                    800,
                    800);
  DrawPtYDistribution(kAcc, true);

  fHM->CreateCanvas("lmvm_pty_" + CbmLmvmHist::fAnaSteps[kPtCut],
                    "lmvm_pty_" + CbmLmvmHist::fAnaSteps[kPtCut],
                    800,
                    800);
...
void CbmAnaDielectronTaskDraw::DrawRapidityDistributionAll() {
  ...
  TCanvas* c = fHM->CreateCanvas(
    "lmvm_signal_rapidity", "lmvm_signal_rapidity", 750, 1000);
  c->Divide(3, 4);
  for (int step = 0; step < CbmLmvmHist::fNofAnaSteps; step++) {
    if (!fUseMvd && (step == kMvd1Cut || step == kMvd2Cut)) continue;
    c->cd(hi++);
    TH1D* proj =
      H2("fh_signal_pty_" + CbmLmvmHist::fAnaSteps[step])->ProjectionX();
    DrawH1(proj);
...
void CbmAnaDielectronTaskDraw::DrawPtYEfficiencyAll() {
  Int_t hi = 1;
  TCanvas* c =
    fHM->CreateCanvas("lmvm_pty_efficiency", "lmvm_pty_efficiency", 1000, 1000);
  c->Divide(3, 3);
  ...
  fHM->CreateCanvas(
    "lmvm_pty_efficiency_ptcut", "lmvm_pty_efficiency_ptcut", 600, 600);
  DrawPtYEfficiency(kPtCut, true);
}

void CbmAnaDielectronTaskDraw::DrawMomentumDistributionAll() {
  fHM->CreateCanvas("lmvm_signal_momentum_distribution",
                    "lmvm_signal_momentum_distribution",
                    600,
                    600);
...
void CbmAnaDielectronTaskDraw::DrawMotherPdg() {
  ...
  DrawH1({H1("fh_mc_mother_pdg"), H1("fh_acc_mother_pdg")},
         {"MC", "acc"},
         kLinear,
         kLog,
         true,
...
void CbmAnaDielectronTaskDraw::Draw1DSourceTypes(const string& hName,
                                                 bool doScale) {
  vector<TH1*> h;
...
void CbmAnaDielectronTaskDraw::Draw1DCut(const string& hName,
  ...
  TCanvas* c = fHM->CreateCanvas(
    ("lmvm_" + hName).c_str(), ("lmvm_" + hName).c_str(), w, h);
  ...
  if (fDrawSignificance) {
    ...
    TH1D* sign    = CreateSignificanceH1D(
      H1(sName), H1(bgName), hName + "_significance", sigOption);
    DrawH1(sign);
  }
}
...
void CbmAnaDielectronTaskDraw::DrawSourcesBgPairsEpEm(int step,
                                                      ...
                                                      bool drawAnaStep) {
  TH2D* h =
    (TH2D*) H2("fh_source_pairs_epem_" + CbmLmvmHist::fAnaSteps[step])->Clone();
...
void CbmAnaDielectronTaskDraw::DrawSourcesBgPairsAll() {
  Int_t hi    = 1;
  TCanvas* c1 = fHM->CreateCanvas("lmvm_bg_sources_pairs_epem_abs",
                                  "lmvm_bg_sources_pairs_epem_abs",
                                  900,
                                  900);
  c1->Divide(3, 3);
  ...
  hi          = 1;
  TCanvas* c2 = fHM->CreateCanvas("lmvm_bg_sources_pairs_epem_percent",
                                  "lmvm_bg_sources_pairs_epem_percent",
                                  900,
                                  900);
  c2->Divide(3, 3);
  for (int step = kReco; step < CbmLmvmHist::fNofAnaSteps; step++) {
  ...
  fHM->CreateCanvas(
    "lmvm_bg_sources_pairs_epem_abs_" + CbmLmvmHist::fAnaSteps[kPtCut],
    "lmvm_bg_sources_pairs_epem_abs_" + CbmLmvmHist::fAnaSteps[kPtCut],
    600,
    600);
  ...
  fHM->CreateCanvas(
    "lmvm_bg_sources_pairs_epem_percent_" + CbmLmvmHist::fAnaSteps[kPtCut],
    "lmvm_bg_sources_pairs_epem_percent_" + CbmLmvmHist::fAnaSteps[kPtCut],
    600,
    600);
  ...
  DrawBgSource2D("lmvm_source_pairs_2d",
                 "fh_source_pairs",
...
void CbmAnaDielectronTaskDraw::Draw2DCut(const string& hist,
                                         double cutCrossX,
                                         double cutCrossY) {
  TCanvas* c = fHM->CreateCanvas(
    ("lmvm_" + hist).c_str(), ("lmvm_" + hist).c_str(), 600, 900);
  c->Divide(2, 3);
  ...
  for (int i = 0; i < CbmLmvmHist::fNofSourceTypes; i++) {
    ...
    double nofPerEvent =
      H2(hist + "_" + CbmLmvmHist::fSourceTypes[i])->GetEntries()
      / (double) fNofEvents;
    cout << hist << "_" << CbmLmvmHist::fSourceTypes[i] << " = " << nofPerEvent
         << endl;
    DrawTextOnPad(
      (Cbm::NumberToString(nofPerEvent, 2) + "/ev."), 0.1, 0.9, 0.5, 0.99);
    DrawTextOnPad(CbmLmvmHist::fSourceTypesLatex[i], 0.6, 0.89, 0.7, 0.99);
    Draw2DCutTriangle(cutCrossX, cutCrossY);
    projX.push_back(
      H2(hist + "_" + CbmLmvmHist::fSourceTypes[i])->ProjectionX());
    projY.push_back(
      H2(hist + "_" + CbmLmvmHist::fSourceTypes[i])->ProjectionY());
  }
  ...
  fHM->CreateCanvas(("lmvm_" + hist + "_signal").c_str(),
                    ("lmvm_" + hist + "_signal").c_str(),
                    800,
                    800);
  DrawH2(H2(hist + "_" + CbmLmvmHist::fSourceTypes[kSignal]));
  Draw2DCutTriangle(cutCrossX, cutCrossY);
  fHM->CreateCanvas(("lmvm_" + hist + "_gamma").c_str(),
                    ("lmvm_" + hist + "_gamma").c_str(),
                    800,
                    800);
...
void CbmAnaDielectronTaskDraw::DrawGammaVertex() {
  ...
  TCanvas* c = fHM->CreateCanvas(
    "lmvm_vertex_el_gamma_mc", "lmvm_vertex_el_gamma_mc", 1200, 400);
  c->Divide(3, 1);
  c->cd(1);
  DrawH2(xz);
  xz->SetMinimum(1e-3);
  ...
  fHM->CreateCanvas(
    "lmvm_vertex_el_gamma_z_mc", "lmvm_vertex_el_gamma_z_mc", 600, 600);
  ...
  fHM->CreateCanvas(
    "lmvm_vertex_el_gamma_z_ptcut", "lmvm_vertex_el_gamma_z_ptcut", 600, 600);
  TH1D* zProjPtCut = (TH1D*) (H2("fh_vertex_el_gamma_xz_ptcut")
                                ->ProjectionX("fh_vertex_el_gamma_ptcut_pz")
                                ->Clone());
  ...
  TCanvas* cZoom = fHM->CreateCanvas("lmvm_vertex_el_gamma_mc_target_zoom",
  ...
  fHM->CreateCanvas(
    "lmvm_vertex_el_gamma_rz_mc", "lmvm_vertex_el_gamma_rz_mc", 600, 600);
...
void CbmAnaDielectronTaskDraw::Draw1DHistoForEachAnalysisStep(
  ...
  for (int step = 0; step < CbmLmvmHist::fNofAnaSteps; step++) {
    ...
    drOpt =
      "same";  // If the histogram is drawn, next histograms must be drawn with "same" option.
...
void CbmAnaDielectronTaskDraw::DrawMinvForEachAnalysisStep() {
  TCanvas* c1 = fHM->CreateCanvas("lmvm_minv_for_each_analysis_step_s_bg",
  ...
  fHM->CreateCanvas("lmvm_minv_for_each_analysis_step_gamma",
                    "lmvm_minv_for_each_analysis_step_gamma",
                    600,
                    600);
...
void CbmAnaDielectronTaskDraw::DrawMinvSandBg(int step) {
  ...
  DrawH1(
    {sbg, bg, s}, {"", "", ""}, kLinear, kLog, false, 0, 0, 0, 0, "Hist L");
  s->SetFillColor(kRed);
...
void CbmAnaDielectronTaskDraw::DrawMinvSandBgAll() {
  Int_t hi   = 1;
  TCanvas* c = fHM->CreateCanvas(
    "lmvm_minv_both_s_and_bg", "lmvm_minv_both_s_and_bg", 900, 900);
  c->Divide(3, 3);
  for (int step = kReco; step < CbmLmvmHist::fNofAnaSteps; step++) {
    if (!fUseMvd && (step == kMvd1Cut || step == kMvd2Cut)) continue;
    c->cd(hi++);
  ...
  fHM->CreateCanvas(
    "lmvm_minv_both_s_and_bg_ptcut", "lmvm_minv_both_s_and_bg_ptcut", 900, 900);
...
void CbmAnaDielectronTaskDraw::DrawMinvSource(int step, bool drawAnaStep) {
  ...
  for (int i = 0; i < CbmLmvmHist::fNofBgPairSources; i++) {
    ...
    legend->AddEntry(
      hists[i],
      (CbmLmvmHist::fBgPairSourceLatex[i] + "("
       + Cbm::NumberToString(100. * hists[i]->GetEntries() / nofBg, 1) + "%)")
        .c_str(),
      "f");
  }
...
void CbmAnaDielectronTaskDraw::DrawMinvSourceAll() {
  {
    Int_t hi = 1;
    TCanvas* c =
      fHM->CreateCanvas("lmvm_minv_source", "lmvm_minv_source", 900, 900);
    c->Divide(3, 3);
    for (int step = kReco; step < CbmLmvmHist::fNofAnaSteps; step++) {
      if (!fUseMvd && (step == kMvd1Cut || step == kMvd2Cut)) continue;
      c->cd(hi++);
    ...
    fHM->CreateCanvas("lmvm_minv_source_" + CbmLmvmHist::fAnaSteps[kPtCut],
                      "lmvm_minv_source_" + CbmLmvmHist::fAnaSteps[kPtCut],
                      600,
                      600);
    DrawMinvSource(kPtCut, false);

    fHM->CreateCanvas("lmvm_minv_source_" + CbmLmvmHist::fAnaSteps[kElId],
                      "lmvm_minv_source_" + CbmLmvmHist::fAnaSteps[kElId],
                      600,
                      600);
  ...
  // Draw mismatches and true matches minv
  {
    Int_t hi   = 1;
    TCanvas* c = fHM->CreateCanvas(
      "lmvm_minv_mismatches", "lmvm_minv_mismatches", 900, 900);
    c->Divide(3, 3);
    for (int step = kReco; step < CbmLmvmHist::fNofAnaSteps; step++) {
      if (!fUseMvd && (step == kMvd1Cut || step == kMvd2Cut)) continue;
      c->cd(hi++);
    ...
    double trueMatch =
      H1("fh_bg_truematch_minv_" + CbmLmvmHist::fAnaSteps[kPtCut])
        ->GetEntries();
    double trueMatchEl =
    ...
    double trueMatchNotEl =
      H1("fh_bg_truematch_notel_minv_" + CbmLmvmHist::fAnaSteps[kPtCut])
        ->GetEntries();
    double misMatch =
      H1("fh_bg_mismatch_minv_" + CbmLmvmHist::fAnaSteps[kPtCut])->GetEntries();
    double nofBg =
      H1("fh_bg_minv_" + CbmLmvmHist::fAnaSteps[kPtCut])->GetEntries();

    fHM->CreateCanvas("lmvm_minv_mismatches_" + CbmLmvmHist::fAnaSteps[kPtCut],
                      "lmvm_minv_mismatches_" + CbmLmvmHist::fAnaSteps[kPtCut],
                      700,
                      700);
    DrawH1(
      {H1("fh_bg_truematch_minv_" + CbmLmvmHist::fAnaSteps[kPtCut]),
...
void CbmAnaDielectronTaskDraw::DrawElPiMomHis() {
  ...
  DrawH1(
    {H1("fh_pi_mom_mc"),
     H1("fh_pi_mom_acc"),
     H1("fh_pi_mom_rec"),
     H1("fh_pi_mom_rec_only_sts"),
     H1("fh_pi_mom_rec_sts_rich_trd"),
     H1("fh_pi_mom_rec_sts_rich_trd_tof")},
    {"MC ("
       + Cbm::NumberToString(H1("fh_pi_mom_mc")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Acc ("
       + Cbm::NumberToString(H1("fh_pi_mom_acc")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec ("
       + Cbm::NumberToString(H1("fh_pi_mom_rec")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec only STS ("
       + Cbm::NumberToString(
         H1("fh_pi_mom_rec_only_sts")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec STS-RICH-TRD ("
       + Cbm::NumberToString(
         H1("fh_pi_mom_rec_sts_rich_trd")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec STS-RICH-TRD-TOF ("
       + Cbm::NumberToString(
         H1("fh_pi_mom_rec_sts_rich_trd_tof")->GetEntries() / fNofEvents, 2)
       + " per event)"},
    kLinear,
    kLog,
    true,
  ...
  DrawH1(
    {H1("fh_piprim_mom_mc"),
     H1("fh_piprim_mom_acc"),
     H1("fh_piprim_mom_rec"),
     H1("fh_piprim_mom_rec_only_sts"),
     H1("fh_piprim_mom_rec_sts_rich_trd"),
     H1("fh_piprim_mom_rec_sts_rich_trd_tof")},
    {"MC ("
       + Cbm::NumberToString(H1("fh_piprim_mom_mc")->GetEntries() / fNofEvents,
                             2)
       + " per event)",
     "Acc ("
       + Cbm::NumberToString(H1("fh_piprim_mom_acc")->GetEntries() / fNofEvents,
                             2)
       + " per event)",
     "Rec ("
       + Cbm::NumberToString(H1("fh_piprim_mom_rec")->GetEntries() / fNofEvents,
                             2)
       + " per event)",
     "Rec only STS ("
       + Cbm::NumberToString(
         H1("fh_piprim_mom_rec_only_sts")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec STS-RICH-TRD ("
       + Cbm::NumberToString(
         H1("fh_piprim_mom_rec_sts_rich_trd")->GetEntries() / fNofEvents, 2)
       + " per event)",
     "Rec STS-RICH-TRD-TOF ("
       + Cbm::NumberToString(
         H1("fh_piprim_mom_rec_sts_rich_trd_tof")->GetEntries() / fNofEvents, 2)
       + " per event)"},
    kLinear,
    kLog,
    true,
  ...
  DrawH1(h1);
  fHM->CreateCanvas(
    "lmvm_piprim_mom_notacc", "lmvm_piprim_mom_notacc", 800, 800);
  ...
  cout << "Number of primary pions minus at rapidity 2 = "
       << H1("fh_piprim_minus_rapidity_mc")
            ->GetBinContent(H1("fh_piprim_minus_rapidity_mc")->FindFixBin(2))
       << endl;
  ...
  cout << "Number of primary pions plus at rapidity 2 = "
       << H1("fh_piprim_plus_rapidity_mc")
            ->GetBinContent(H1("fh_piprim_plus_rapidity_mc")->FindFixBin(2))
       << endl;
  cout << "Number of primary pions plus at rapidity (1, 3) = "
       << H1("fh_piprim_plus_rapidity_mc")
            ->Integral(H1("fh_piprim_plus_rapidity_mc")->FindFixBin(1),
                       H1("fh_piprim_plus_rapidity_mc")->FindFixBin(3))
       << endl;
  ...
  fHM->CreateCanvas(
    "lmvm_piprim_plus_rapidity", "lmvm_piprim_plus_rapidity", 800, 800);
  DrawH1(H1("fh_piprim_plus_rapidity_mc"));
  fHM->CreateCanvas(
    "lmvm_piprim_minus_rapidity", "lmvm_piprim_minus_rapidity", 800, 800);
...
void CbmAnaDielectronTaskDraw::RemoveMvdCutBins() {
  for (int step = kMvd2Cut + 1 + 1; step < CbmLmvmHist::fNofAnaSteps + 1;
       step++) {
    H1("fh_nof_bg_tracks")
      ->SetBinContent(step - 2, H1("fh_nof_bg_tracks")->GetBinContent(step));
    H1("fh_nof_el_tracks")
      ->SetBinContent(step - 2, H1("fh_nof_el_tracks")->GetBinContent(step));

    H1("fh_nof_mismatches")
      ->SetBinContent(step - 2, H1("fh_nof_mismatches")->GetBinContent(step));
    H1("fh_nof_ghosts")
      ->SetBinContent(step - 2, H1("fh_nof_ghosts")->GetBinContent(step));

    int ny = H2("fh_source_tracks")->GetYaxis()->GetNbins();
    for (int y = 1; y <= ny; y++) {
      H2("fh_source_tracks")
        ->SetBinContent(
          step - 2, y, H2("fh_source_tracks")->GetBinContent(step, y));
    }
    ...
    for (int y = 1; y <= ny; y++) {
      H2("fh_source_pairs")
        ->SetBinContent(
          step - 2, y, H2("fh_source_pairs")->GetBinContent(step, y));
    }
  }
}

void CbmAnaDielectronTaskDraw::DrawBgSource2D(const string& canvasName,
  ...
  fHM->CreateCanvas(string(canvasName + "_abs").c_str(),
                    string(canvasName + "_abs").c_str(),
                    900,
                    600);
  ...
  fHM->CreateCanvas(string(canvasName + "_percent").c_str(),
                    string(canvasName + "_percent").c_str(),
                    900,
                    600);
...
void CbmAnaDielectronTaskDraw::DrawBgSourceTracks() {
  ...
  fHM->CreateCanvas(
    "fh_nof_topology_pairs_gamma", "fh_nof_topology_pairs_gamma", 600, 600);
  ...
  fHM->CreateCanvas(
    "fh_nof_topology_pairs_pi0", "fh_nof_topology_pairs_pi0", 600, 600);
...
void CbmAnaDielectronTaskDraw::DrawMismatchesAndGhosts() {
  ...
  TCanvas* c1 =
    fHM->CreateCanvas("lmvm_nof_mismatches", "lmvm_nof_mismatches", 600, 600);
  c1->Divide(2, 2);
  c1->cd(1);
...
void CbmAnaDielectronTaskDraw::DrawMinvPtAll() {
  Int_t hi   = 1;
  TCanvas* c = fHM->CreateCanvas(
    "lmvm_fh_signal_minv_pt", "lmvm_fh_signal_minv_pt", 750, 1000);
  c->Divide(3, 4);
  for (int step = 0; step < CbmLmvmHist::fNofAnaSteps; step++) {
    if (!fUseMvd && (step == kMvd1Cut || step == kMvd2Cut)) continue;
    c->cd(hi++);
...
void CbmAnaDielectronTaskDraw::DrawBgSourcesVsMomentum() {
  fHM->CreateCanvas(
    "lmvm_source_mom_mc_signal", "lmvm_source_mom_mc_signal", 600, 600);
  DrawH1(H1("fh_source_mom_mc_signal"));

  int hi = 1;
  TCanvas* c1 =
    fHM->CreateCanvas("lmvm_source_mom", "lmvm_source_mom", 900, 900);
  c1->Divide(3, 3);
  ...
  hi = 1;
  TCanvas* c2 =
    fHM->CreateCanvas("lmvm_opening_angle", "lmvm_opening_angle", 900, 900);
  c2->Divide(3, 3);
  for (Int_t step = kReco; step < CbmLmvmHist::fNofAnaSteps; step++) {
  ...
  fHM->CreateCanvas(
    "lmvm_opening_angle_ttcut", "lmvm_opening_angle_ttcut", 600, 600);
  Draw1DSourceTypes("fh_opening_angle_" + CbmLmvmHist::fAnaSteps[kTtCut],
                    false);
}
...
void CbmAnaDielectronTaskDraw::DrawMvdAndStsHist() {
  if (!fUseMvd) return;
  TCanvas* c1 =
    fHM->CreateCanvas("lmvm_nofhits_mvd_sts", "lmvm_nofhits_mvd_sts", 900, 450);
  c1->Divide(2, 1);
  c1->cd(1);
