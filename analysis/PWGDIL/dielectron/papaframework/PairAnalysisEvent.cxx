...
void PairAnalysisEvent::Init()
{
  ...
  // DEBUG stuff
  if(0) {
    fprintf(stderr, "check %s: has %d points in %p \n", "MVD",GetNumberOfPoints(ECbmModuleId::kMvd), GetPoints(ECbmModuleId::kMvd));
    fprintf(stderr, "check %s: has %d points in %p \n", "STS",GetNumberOfPoints(ECbmModuleId::kSts), GetPoints(ECbmModuleId::kSts));
    fprintf(stderr, "check %s: has %d points in %p \n", "RICH",GetNumberOfPoints(ECbmModuleId::kRich), GetPoints(ECbmModuleId::kRich));
    fprintf(stderr, "check %s: has %d points in %p \n", "TRD",GetNumberOfPoints(ECbmModuleId::kTrd), GetPoints(ECbmModuleId::kTrd));
    fprintf(stderr, "check %s: has %d points in %p \n", "TOF",GetNumberOfPoints(ECbmModuleId::kTof), GetPoints(ECbmModuleId::kTof));

    fprintf(stderr, "check %s: has %d hitMatches in %p \n", "MVD",GetNumberOfHitMatches(ECbmModuleId::kMvd), GetHitMatches(ECbmModuleId::kMvd));
    fprintf(stderr, "check %s: has %d hitMatches in %p \n", "STS",GetNumberOfHitMatches(ECbmModuleId::kSts), GetHitMatches(ECbmModuleId::kSts));
    fprintf(stderr, "check %s: has %d hitMatches in %p \n", "RICH",GetNumberOfHitMatches(ECbmModuleId::kRich), GetHitMatches(ECbmModuleId::kRich));
    fprintf(stderr, "check %s: has %d hitMatches in %p \n", "TRD",GetNumberOfHitMatches(ECbmModuleId::kTrd), GetHitMatches(ECbmModuleId::kTrd));
    fprintf(stderr, "check %s: has %d hitMatches in %p \n", "TOF",GetNumberOfHitMatches(ECbmModuleId::kTof), GetHitMatches(ECbmModuleId::kTof));

    fprintf(stderr, "check %s: has %d hits in %p \n", "MVD",GetNumberOfHits(ECbmModuleId::kMvd), GetHits(ECbmModuleId::kMvd));
    fprintf(stderr, "check %s: has %d hits in %p \n", "STS",GetNumberOfHits(ECbmModuleId::kSts), GetHits(ECbmModuleId::kSts));
    fprintf(stderr, "check %s: has %d hits in %p \n", "RICH",GetNumberOfHits(ECbmModuleId::kRich), GetHits(ECbmModuleId::kRich));
    fprintf(stderr, "check %s: has %d hits in %p \n", "TRD",GetNumberOfHits(ECbmModuleId::kTrd), GetHits(ECbmModuleId::kTrd));
    fprintf(stderr, "check %s: has %d hits in %p \n", "TOF",GetNumberOfHits(ECbmModuleId::kTof), GetHits(ECbmModuleId::kTof));
  }
  ...
  for (Int_t i=0; i<(fGlobalTracks?fGlobalTracks->GetEntriesFast():0); i++) {
    ...
    if(fTofHitMatches && tofHit)  tofMatch = static_cast<CbmMatch*>( fTofHitMatches->At(itof) );     //tofMatch = tofHit->GetMatch();
