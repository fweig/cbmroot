...
void CbmKresEtaMCAnalysis::Exec(int Event, double OpeningAngleCut, double GammaInvMassCut)
{
	...
	for (Int_t iTr = 0; iTr < ngTracks; iTr++){
		...
		double chi2 = 0;
		TVector3 refmomentum = CbmKresFunctions::FitToVertexAndGetChi(stsTrack, mcTrack->GetStartX(), mcTrack->GetStartY(), mcTrack->GetStartZ(), chi2);
	...
	EtaChargedPionsGammaAnalysis(Pion_Eta_refmomentum, Pion_Eta_MCtrack, Pion_Eta_Id, El_Photon_Eta_refmomentum, El_Photon_Eta_MCtrack, El_Photon_Eta_Id, fHistoList_eta_ppg);
	EtaPosNegNeutralPionsAnalysis(El_Photon_Pion_Eta_refmomentum, El_Photon_Pion_Eta_MCtrack, El_Photon_Pion_Eta_Id, Pion_Eta_refmomentum, Pion_Eta_MCtrack, Pion_Eta_Id, fHistoList_eta_ppp);


	EtaDoubleGammaAnalysis_plusBG(OpeningAngleCut, GammaInvMassCut, Event, All_El_refmomentum, All_El_MCtrack, All_El_Id, fHistoList_eta_gg);
	EtaChargedPionsGammaAnalysis_plusBG(OpeningAngleCut, GammaInvMassCut, Event, All_Pion_refmomentum, All_Pion_MCtrack, All_Pion_Id, All_El_refmomentum, All_El_MCtrack, All_El_Id, fHistoList_eta_ppg, fHistoList_eta_ppp);
...
Double_t CbmKresEtaMCAnalysis::CalculateOpeningAngleBetweenGammas_MC(CbmMCTrack* mctrack1, CbmMCTrack* mctrack2, CbmMCTrack* mctrack3, CbmMCTrack* mctrack4)
{
...
Double_t CbmKresEtaMCAnalysis::CalculateOpeningAngleBetweenGammas_Reco(TVector3 electron1, TVector3 electron2, TVector3 electron3, TVector3 electron4)
{
...
void CbmKresEtaMCAnalysis::EtaDoubleGammaAnalysis(vector<TVector3> RefMom, vector<CbmMCTrack*> MC,  vector<Int_t> Id, vector<TH1*> gg)
{
		...
		for(size_t j=i+1; j<MC.size(); j++) {
			for(size_t k=j+1; k<MC.size(); k++) {
				for(size_t l=k+1; l<MC.size(); l++) {
					...
					if (TMath::Abs(pdg1) != 11 || TMath::Abs(pdg2) != 11 || TMath::Abs(pdg3) != 11 || TMath::Abs(pdg4) != 11) continue;
					...
					if (mcId1 == mcId2 || mcId1 == mcId3 || mcId1 == mcId4 || mcId2 == mcId3 || mcId2 == mcId4 || mcId3 == mcId4) continue; // particle is not used twice
					...
					if (grandmotherId1 == grandmotherId2 && grandmotherId1 == grandmotherId3 && grandmotherId1 == grandmotherId4){
						...
						Double_t InvMass_reco = CbmKresFunctions::Invmass_4particles_RECO(RefMom.at(i), RefMom.at(j), RefMom.at(k), RefMom.at(l));
						cout << "Decay eta -> gamma gamma -> e+e- e+e- detected!\t\t mc mass: " << InvMass_true << "\t, reco mass: " << InvMass_reco << endl;
						...
						cout << "pdgs: " << mcGrandmotherPdg1 << "-->" << mcMotherPdg1 << "/" << mcMotherPdg2 << "/" << mcMotherPdg3 << "/" << mcMotherPdg4 << "-->" << pdg1 << "/" << pdg2 << "/" << pdg3 << "/" << pdg4 << endl;

						gg[0]->Fill(InvMass_true);
						gg[1]->Fill(InvMass_reco);

						cout << "\t \t mc true info: "  << endl;
						cout << "particle 1: \t" << MC.at(i)->GetPdgCode() << ";\t pt = " << MC.at(i)->GetPt() << ";\t X = " << MC.at(i)->GetStartX() << ";\t Y = " << MC.at(i)->GetStartY() << ";\t Z = " << MC.at(i)->GetStartZ() << ";\t E = " << MC.at(i)->GetEnergy() << endl;
						cout << "particle 2: \t" << MC.at(j)->GetPdgCode() << ";\t pt = " << MC.at(j)->GetPt() << ";\t X = " << MC.at(j)->GetStartX() << ";\t Y = " << MC.at(j)->GetStartY() << ";\t Z = " << MC.at(j)->GetStartZ() << ";\t E = " << MC.at(j)->GetEnergy() << endl;
						cout << "particle 3: \t" << MC.at(k)->GetPdgCode() << ";\t pt = " << MC.at(k)->GetPt() << ";\t X = " << MC.at(k)->GetStartX() << ";\t Y = " << MC.at(k)->GetStartY() << ";\t Z = " << MC.at(k)->GetStartZ() << ";\t E = " << MC.at(k)->GetEnergy() << endl;
						cout << "particle 4: \t" << MC.at(l)->GetPdgCode() << ";\t pt = " << MC.at(l)->GetPt() << ";\t X = " << MC.at(l)->GetStartX() << ";\t Y = " << MC.at(l)->GetStartY() << ";\t Z = " << MC.at(l)->GetStartZ() << ";\t E = " << MC.at(l)->GetEnergy() << endl;
						...
						if(motherId1 == motherId2) {
							...
							openingAngleBetweenGammasReco = CalculateOpeningAngleBetweenGammas_Reco(RefMom.at(i), RefMom.at(j), RefMom.at(k), RefMom.at(l));
						...
						if(motherId1 == motherId3) {
							...
							gg[3]->Fill(InvMass_realg1_refitted);
							gg[3]->Fill(InvMass_realg2_refitted);
							openingAngleBetweenGammasReco = CalculateOpeningAngleBetweenGammas_Reco(RefMom.at(i), RefMom.at(k), RefMom.at(j), RefMom.at(l));							
						}
						if(motherId1 == motherId4) {
							...
							gg[3]->Fill(InvMass_realg1_refitted);
							gg[3]->Fill(InvMass_realg2_refitted);
							openingAngleBetweenGammasReco = CalculateOpeningAngleBetweenGammas_Reco(RefMom.at(i), RefMom.at(l), RefMom.at(j), RefMom.at(k));
						}
						Double_t openingAngleBetweenGammas = CalculateOpeningAngleBetweenGammas_MC(MC.at(i), MC.at(j), MC.at(k), MC.at(l));
...
void CbmKresEtaMCAnalysis::EtaChargedPionsGammaAnalysis(vector<TVector3> RefMomPion, vector<CbmMCTrack*> MCPion, vector<Int_t> /*IdPion*/, 
                                                        vector<TVector3> RefMomEl,   vector<CbmMCTrack*> MCEl,   vector<Int_t> /*IdEl*/, vector<TH1*> ppg)
{
		...
		for(size_t j=i+1; j<MCEl.size(); j++) {
			...
			for(size_t k=0; k<MCPion.size(); k++) {
				for(size_t l=k+1; l<MCPion.size(); l++) {
					...
					Double_t InvMass_true = CbmKresFunctions::Invmass_4particles_MC(MCEl.at(i), MCEl.at(j), MCPion.at(k), MCPion.at(l));
					Double_t InvMass_reco = CbmKresFunctions::Invmass_2el_2pions_RECO(RefMomEl.at(i), RefMomEl.at(j), RefMomPion.at(k), RefMomPion.at(l));
					cout << "Decay eta -> p+p- gamma -> p+p- e+e- detected!\t\t mc mass: " << InvMass_true << "\t, reco mass: " << InvMass_reco << endl;
					cout << "motherids: " << motherId1 << "/" << motherId2 << "/" << motherId3 << "/" << motherId4 << endl;
					cout << "grandmotherid: " << grandmotherId1 << endl;
					cout << "pdgs: " << mcGrandmotherPdg1 << "-->" << mother1->GetPdgCode() << "/" << pdg3 << "/" << pdg4 << "-->" << pdg1 << "/" << pdg2 << "/" << pdg3 << "/" << pdg4 << endl;

					ppg[0]->Fill(InvMass_true);
					ppg[1]->Fill(InvMass_reco);

					cout << "\t \t mc true info: "  << endl;
					cout << "particle 1: \t" << MCEl.at(i)->GetPdgCode() << ";\t pt = " << MCEl.at(i)->GetPt() << ";\t X = " << MCEl.at(i)->GetStartX() << ";\t Y = " << MCEl.at(i)->GetStartY() << ";\t Z = " << MCEl.at(i)->GetStartZ() << ";\t E = " << MCEl.at(i)->GetEnergy() << endl;
					cout << "particle 2: \t" << MCEl.at(j)->GetPdgCode() << ";\t pt = " << MCEl.at(j)->GetPt() << ";\t X = " << MCEl.at(j)->GetStartX() << ";\t Y = " << MCEl.at(j)->GetStartY() << ";\t Z = " << MCEl.at(j)->GetStartZ() << ";\t E = " << MCEl.at(j)->GetEnergy() << endl;
					cout << "particle 3: \t" << MCPion.at(k)->GetPdgCode() << ";\t pt = " << MCPion.at(k)->GetPt() << ";\t X = " << MCPion.at(k)->GetStartX() << ";\t Y = " << MCPion.at(k)->GetStartY() << ";\t Z = " << MCPion.at(k)->GetStartZ() << ";\t E = " << MCPion.at(k)->GetEnergy() << endl;
					cout << "particle 4: \t" << MCPion.at(l)->GetPdgCode() << ";\t pt = " << MCPion.at(l)->GetPt() << ";\t X = " << MCPion.at(l)->GetStartX() << ";\t Y = " << MCPion.at(l)->GetStartY() << ";\t Z = " << MCPion.at(l)->GetStartZ() << ";\t E = " << MCPion.at(l)->GetEnergy() << endl;
...
void CbmKresEtaMCAnalysis::EtaPosNegNeutralPionsAnalysis(vector<TVector3> RefMomNeutral, vector<CbmMCTrack*> MCNeutral, vector<Int_t> IdNeutral, 
                                                         vector<TVector3> RefMomPion, vector<CbmMCTrack*> MCPion, vector<Int_t> /*IdPion*/, vector<TH1*> ppp)
{
					...
					if (grandmotherId1 == grandmotherId2 && grandmotherId1 == grandmotherId3 && grandmotherId1 == grandmotherId4){
						...
						for(size_t m=0; m<MCPion.size(); m++) {
							for(size_t s=m+1; s<MCPion.size(); s++) {
								...
								Double_t InvMass_six_true = CbmKresFunctions::Invmass_6particles_MC(MCNeutral.at(i), MCNeutral.at(j), MCNeutral.at(k), MCNeutral.at(l), MCPion.at(m), MCPion.at(s));
								Double_t InvMass_six_reco = CbmKresFunctions::Invmass_4el_2pions_RECO(RefMomNeutral.at(i), RefMomNeutral.at(j), RefMomNeutral.at(k), RefMomNeutral.at(l), RefMomPion.at(m), RefMomPion.at(s));
								cout << "Decay eta -> p+p- p0 -> p+p- e+e-e+e- detected!\t\t mc mass: " << InvMass_six_true << "\t, reco mass: " << InvMass_six_reco << endl;
								cout << "motherids for neutral: " << motherId1 << "/" << motherId2 << "/" << motherId3 << "/" << motherId4 << endl;
								cout << "grandmotherid: " << GrMother1->GetMotherId() << "/" <<  motherId5 << "/" << motherId6 << endl;
								cout << "pdgs: " << TopTrack->GetPdgCode() << "-->" << GrMother1->GetPdgCode() << "/" << pdg5 << "/" << pdg6 << endl;

								ppp[0]->Fill(InvMass_six_true);
								ppp[1]->Fill(InvMass_six_reco);

								cout << "\t \t mc true info: "  << endl;
								cout << "particle 1: \t" << MCNeutral.at(i)->GetPdgCode() << ";\t pt = " << MCNeutral.at(i)->GetPt() << ";\t X = " << MCNeutral.at(i)->GetStartX() << ";\t Y = " << MCNeutral.at(i)->GetStartY() << ";\t Z = " << MCNeutral.at(i)->GetStartZ() << ";\t E = " << MCNeutral.at(i)->GetEnergy() << endl;
								cout << "particle 2: \t" << MCNeutral.at(j)->GetPdgCode() << ";\t pt = " << MCNeutral.at(j)->GetPt() << ";\t X = " << MCNeutral.at(j)->GetStartX() << ";\t Y = " << MCNeutral.at(j)->GetStartY() << ";\t Z = " << MCNeutral.at(j)->GetStartZ() << ";\t E = " << MCNeutral.at(j)->GetEnergy() << endl;
								cout << "particle 3: \t" << MCNeutral.at(k)->GetPdgCode() << ";\t pt = " << MCNeutral.at(k)->GetPt() << ";\t X = " << MCNeutral.at(k)->GetStartX() << ";\t Y = " << MCNeutral.at(k)->GetStartY() << ";\t Z = " << MCNeutral.at(k)->GetStartZ() << ";\t E = " << MCNeutral.at(k)->GetEnergy() << endl;
								cout << "particle 4: \t" << MCNeutral.at(l)->GetPdgCode() << ";\t pt = " << MCNeutral.at(l)->GetPt() << ";\t X = " << MCNeutral.at(l)->GetStartX() << ";\t Y = " << MCNeutral.at(l)->GetStartY() << ";\t Z = " << MCNeutral.at(l)->GetStartZ() << ";\t E = " << MCNeutral.at(l)->GetEnergy() << endl;
								cout << "particle 5: \t" << MCPion.at(m)->GetPdgCode() << ";\t pt = " << MCPion.at(m)->GetPt() << ";\t X = " << MCPion.at(m)->GetStartX() << ";\t Y = " << MCPion.at(m)->GetStartY() << ";\t Z = " << MCPion.at(m)->GetStartZ() << ";\t E = " << MCPion.at(m)->GetEnergy() << endl;
								cout << "particle 6: \t" << MCPion.at(s)->GetPdgCode() << ";\t pt = " << MCPion.at(s)->GetPt() << ";\t X = " << MCPion.at(s)->GetStartX() << ";\t Y = " << MCPion.at(s)->GetStartY() << ";\t Z = " << MCPion.at(s)->GetStartZ() << ";\t E = " << MCPion.at(s)->GetEnergy() << endl;

								Double_t InvMass_true = CbmKresFunctions::Invmass_4particles_MC(MCNeutral.at(i), MCNeutral.at(j), MCNeutral.at(k), MCNeutral.at(l));
								Double_t InvMass_reco = CbmKresFunctions::Invmass_4particles_RECO(RefMomNeutral.at(i), RefMomNeutral.at(j), RefMomNeutral.at(k), RefMomNeutral.at(l));		
...
void CbmKresEtaMCAnalysis::EtaDoubleGammaAnalysis_plusBG(double OpeningAngleCut, double GammaInvMassCut, int Event, vector<TVector3> RefMom, vector<CbmMCTrack*> MC,  vector<Int_t> Id, vector<TH1*> gg)
{
	if (MC.size() < 4) return;
...
void CbmKresEtaMCAnalysis::EtaChargedPionsGammaAnalysis_plusBG(double OpeningAngleCut, double GammaInvMassCut, int Event, vector<TVector3> RefMomPion, vector<CbmMCTrack*> MCPion, vector<Int_t> /*IdPion*/, 
                                                               vector<TVector3> RefMomEl, vector<CbmMCTrack*> MCEl, vector<Int_t> IdEl, vector<TH1*> ppg, vector<TH1*> ppp)
{
	if (MCPion.size() < 2 || MCEl.size() < 2) return;
	...
	for(size_t k=0; k<MCPion.size(); k++) {
		...
		for(size_t l=k+1; l<MCPion.size(); l++) {
			...
			double Pions_angle_reco = CbmKresFunctions::CalculateOpeningAngleBetweenPions_Reco(RefMomPion.at(k), RefMomPion.at(l));
	...
	for (size_t gamma = 0; gamma < ECPGA_leptons_RefMom.size(); gamma++){
		...
		for(size_t pionpair = 0; pionpair < ECPGA_pions_RefMom.size(); pionpair++) {
			...
			if (mcTrack1->GetMotherId() == mcTrack2->GetMotherId() && mcTrack1->GetMotherId() != -1){
				...
				if (GammaTrack->GetPdgCode() == 22 && GammaTrack->GetMotherId() != -1 && mcTrack3->GetMotherId() == mcTrack4->GetMotherId() && 
	...
	////////    case with p+p-p0
	if(ECPGA_leptons_MC.size() < 2) return; 						// min 2 gammas to form pion is required in EtaPosNegNeutralPionsAnalysis_plusBG
...
//			Double_t InvMass_pi_true = CbmKresFunctions::Invmass_4particles_MC(mcTrack1, mcTrack2, mcTrack3, mcTrack4);
			...
			for(size_t k=0; k<MCPion.size(); k++) {
				for(size_t l=k+1; l<MCPion.size(); l++) {
					...
					double InvMass_six_true = CbmKresFunctions::Invmass_6particles_MC(mcTrack1, mcTrack2, mcTrack3, mcTrack4, MCPion.at(k), MCPion.at(l));
					double InvMass_six_reco = CbmKresFunctions::Invmass_4el_2pions_RECO(e11, e12, e21, e22, RefMomPion.at(k), RefMomPion.at(l));
					double Pions_angle_reco = CbmKresFunctions::CalculateOpeningAngleBetweenPions_Reco(RefMomPion.at(k), RefMomPion.at(l));
...
void CbmKresEtaMCAnalysis::Mixing_ppg()
{
	...
	cout << "Mixing for  eta->(pp)g  channel - nof lepton pairs = " << nof_leptons << ";  nof pion pairs = " << nof_pions << endl;
	for(Int_t a = 0; a < nof_leptons; a++) {
		for(Int_t b = 0; b < nof_pions; b++) {
			if (EMT_ppg_ee_Event[a] == EMT_ppg_pp_Event[b]) continue;		// to make sure that the photons are from two different events
...
void CbmKresEtaMCAnalysis::Mixing_three_body()
{
	...
	cout << "Mixing 3 bodies for  eta-> p p g  channel - nof photons = " << nof_photons << ";  nof +pions = " << nof_positive << ";  nof -pions = " << nof_negative << endl;
	
	for(Int_t p = 0; p < nof_positive; p++) {
		for(Int_t m = 0; m < nof_negative; m++) {
			if (EMT_ppg_positive_pion_Event[p] == EMT_ppg_negative_pion_Event[m]) continue;		// to make sure that the photons are from two different events
			...
			for(Int_t a = 0; a < nof_photons; a++) {
				if (EMT_ppg_ee_Event[a] == EMT_ppg_positive_pion_Event[p] || EMT_ppg_ee_Event[a] == EMT_ppg_negative_pion_Event[m]) continue;		// to make sure that the photons are from two different events
				TVector3 e11 = EMT_ppg_ee_pair_momenta[a][0];
				TVector3 e12 = EMT_ppg_ee_pair_momenta[a][1];
...
void CbmKresEtaMCAnalysis::InitHistograms()
{
	...
	InvMass_eta_gg_reffited = new TH1D("InvMass_eta_gg_reffited","InvMass_eta_gg_reffited; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_gg.push_back(InvMass_eta_gg_reffited);
	InvMassPhoton_eta_gg_mc = new TH1D("InvMassPhoton_eta_gg_mc","InvMassPhoton_eta_gg_mc; invariant mass in GeV/c^{2};#", 60, -0.01, 0.05);
	fHistoList_eta_gg.push_back(InvMassPhoton_eta_gg_mc);
	InvMassPhoton_eta_gg_reffited = new TH1D("InvMassPhoton_eta_gg_reffited","InvMassPhoton_eta_gg_reffited; invariant mass in GeV/c^{2};#", 60, -0.01, 0.05);
	fHistoList_eta_gg.push_back(InvMassPhoton_eta_gg_reffited);
	OpeningAnglePhoton_eta_gg_mc = new TH1D("OpeningAnglePhoton_eta_gg_mc","OpeningAnglePhoton_eta_gg_mc (between e+e- from #gamma); angle [deg];#", 100, -0.1, 9.9);
	fHistoList_eta_gg.push_back(OpeningAnglePhoton_eta_gg_mc);
	OpeningAnglePhoton_eta_gg_reffited = new TH1D("OpeningAnglePhoton_eta_gg_reffited","OpeningAnglePhoton_eta_gg_reffited (between e+e- from #gamma); angle [deg];#", 100, -0.1, 9.9);
	fHistoList_eta_gg.push_back(OpeningAnglePhoton_eta_gg_reffited);
	OpeningAngle_eta_gg_between_gg_mc = new TH1D("OpeningAngle_eta_gg_between_gg_mc","OpeningAngle_eta_gg_between_gg_mc (between #gamma#gamma from #eta); angle [deg];#", 900, -0.1, 89.9);
	fHistoList_eta_gg.push_back(OpeningAngle_eta_gg_between_gg_mc);
	OpeningAngle_eta_gg_between_gg_reffited = new TH1D("OpeningAngle_eta_gg_between_gg_reffited","OpeningAngle_eta_gg_between_gg_reffited (between #gamma#gamma from #eta); angle [deg];#", 900, -0.1, 89.9);
	fHistoList_eta_gg.push_back(OpeningAngle_eta_gg_between_gg_reffited);
	InvMass_eta_gg_allcombinations_mc = new TH1D("InvMass_eta_gg_allcombinations_mc","InvMass_eta_gg_allcombinations_mc; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_eta_gg.push_back(InvMass_eta_gg_allcombinations_mc);
	InvMass_eta_gg_allcombinations_reffited = new TH1D("InvMass_eta_gg_allcombinations_reffited","InvMass_eta_gg_allcombinations_reffited; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	InvMass_eta_gg_reco_aftercuts = new TH1D("InvMass_eta_gg_reco_aftercuts","InvMass_eta_gg_reco_aftercuts; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_gg.push_back(InvMass_eta_gg_reco_aftercuts);
	rap_vs_pt_eta_gg_reco_aftercuts = new TH2D("rap_vs_pt_eta_gg_reco_aftercuts", "rap_vs_pt_eta_gg_reco_aftercuts; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_eta_gg.push_back(rap_vs_pt_eta_gg_reco_aftercuts);
	rap_vs_pt_NOTeta_gg_reco_aftercuts = new TH2D("rap_vs_pt_NOTeta_gg_reco_aftercuts", "rap_vs_pt_NOTeta_gg_reco_aftercuts; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_eta_gg.push_back(rap_vs_pt_NOTeta_gg_reco_aftercuts);


	///////         eta -> p+ + p- + gamma -> p+p- e+e-
	InvMass_eta_ppg_mc = new TH1D("InvMass_eta_ppg_mc","InvMass_eta_ppg_mc; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_ppg.push_back(InvMass_eta_ppg_mc);
	InvMass_eta_ppg_reffited = new TH1D("InvMass_eta_ppg_reffited","InvMass_eta_ppg_reffited; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_ppg.push_back(InvMass_eta_ppg_reffited);
	InvMassPhoton_eta_ppg_mc = new TH1D("InvMassPhoton_eta_ppg_mc","InvMassPhoton_eta_ppg_mc; invariant mass in GeV/c^{2};#", 60, -0.01, 0.05);
	fHistoList_eta_ppg.push_back(InvMassPhoton_eta_ppg_mc);
	InvMassPhoton_eta_ppg_reffited = new TH1D("InvMassPhoton_eta_ppg_reffited","InvMassPhoton_eta_ppg_reffited; invariant mass in GeV/c^{2};#", 60, -0.01, 0.05);
	fHistoList_eta_ppg.push_back(InvMassPhoton_eta_ppg_reffited);
	OpeningAnglePhoton_eta_ppg_mc = new TH1D("OpeningAnglePhoton_eta_ppg_mc","OpeningAnglePhoton_eta_ppg_mc (between e+e- from #gamma); angle [deg];#", 100, -0.1, 9.9);
	fHistoList_eta_ppg.push_back(OpeningAnglePhoton_eta_ppg_mc);
	OpeningAnglePhoton_eta_ppg_reffited = new TH1D("OpeningAnglePhoton_eta_ppg_reffited","OpeningAnglePhoton_eta_ppg_reffited (between e+e- from #gamma); angle [deg];#", 100, -0.1, 9.9);
	fHistoList_eta_ppg.push_back(OpeningAnglePhoton_eta_ppg_reffited);
	InvMass_eta_ppg_allcombinations_mc = new TH1D("InvMass_eta_ppg_allcombinations_mc","InvMass_eta_ppg_allcombinations_mc; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_eta_ppg.push_back(InvMass_eta_ppg_allcombinations_mc);
	InvMass_eta_ppg_allcombinations_reffited = new TH1D("InvMass_eta_ppg_allcombinations_reffited","InvMass_eta_ppg_allcombinations_reffited; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	Pion_Pt_fromEta_reco = new TH1D("Pion_Pt_fromEta_reco","Pion_Pt_fromEta_reco; pions P_{t} in GeV/c;#", 500, 0.0, 5.0);
	fHistoList_eta_ppg.push_back(Pion_Pt_fromEta_reco);
	Pion_Pt_elsewhere_reco = new TH1D("Pion_Pt_elsewhere_reco","Pion_Pt_elsewhere_reco; pions P_{t} in GeV/c;#", 500, 0.0, 5.0);
	fHistoList_eta_ppg.push_back(Pion_Pt_elsewhere_reco);	
	OA_betweenPions_fromEta_mc = new TH1D("OA_betweenPions_fromEta_mc","OA_betweenPions_fromEta_mc (between #p^{+}#p^{-} from #eta); angle [deg];#", 900, -0.1, 89.9);
	fHistoList_eta_ppg.push_back(OA_betweenPions_fromEta_mc);
	OA_betweenPions_fromEta_reco = new TH1D("OA_betweenPions_fromEta_reco","OA_betweenPions_fromEta_reco (between #p^{+}#p^{-} from #eta); angle [deg];#", 900, -0.1, 89.9);
	fHistoList_eta_ppg.push_back(OA_betweenPions_fromEta_reco);
	OA_betweenPions_fromEta_reco_wrongcombinations = new TH1D("OA_betweenPions_fromEta_reco_wrongcombinations","OA_betweenPions_fromEta_reco_wrongcombinations (between #p^{+}#p^{-} from #eta); angle [deg];#", 900, -0.1, 89.9);
	...
	EMT_eta_three_body = new TH1D("EMT_eta_three_body","EMT_eta_three_body; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_eta_ppg.push_back(EMT_eta_three_body);
	InvMass_eta_ppg_reco_aftercuts = new TH1D("InvMass_eta_ppg_reco_aftercuts","InvMass_eta_ppg_reco_aftercuts; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_ppg.push_back(InvMass_eta_ppg_reco_aftercuts);
	rap_vs_pt_eta_ppg_reco_aftercuts = new TH2D("rap_vs_pt_eta_ppg_reco_aftercuts", "rap_vs_pt_eta_ppg_reco_aftercuts; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_eta_ppg.push_back(rap_vs_pt_eta_ppg_reco_aftercuts);
	rap_vs_pt_NOTeta_ppg_reco_aftercuts = new TH2D("rap_vs_pt_NOTeta_ppg_reco_aftercuts", "rap_vs_pt_NOTeta_ppg_reco_aftercuts; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_eta_ppg.push_back(rap_vs_pt_NOTeta_ppg_reco_aftercuts);



	///////         eta -> p+ + p- + p0 -> p+p- (g+g) --> p+p- (e+e-) (e+e-)
	InvMass_eta_ppp_mc = new TH1D("InvMass_eta_ppp_mc","InvMass_eta_ppp_mc; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_ppp.push_back(InvMass_eta_ppp_mc);
	InvMass_eta_ppp_reffited = new TH1D("InvMass_eta_ppp_reffited","InvMass_eta_ppp_reffited; invariant mass in GeV/c^{2};#", 50, 0.3, 0.8);
	fHistoList_eta_ppp.push_back(InvMass_eta_ppp_reffited);
	InvMass_eta_Npion_mc = new TH1D("InvMass_eta_Npion_mc","InvMass_eta_Npion_mc; invariant mass in GeV/c^{2};#", 30, 0.0, 0.3);
	fHistoList_eta_ppp.push_back(InvMass_eta_Npion_mc);
	InvMass_eta_Npion_reffited = new TH1D("InvMass_eta_Npion_reffited","InvMass_eta_Npion_reffited; invariant mass in GeV/c^{2};#", 30, 0., 0.3);
	fHistoList_eta_ppp.push_back(InvMass_eta_Npion_reffited);
	InvMass_eta_ppp_allcombinations_mc = new TH1D("InvMass_eta_ppp_allcombinations_mc","InvMass_eta_ppp_allcombinations_mc; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_eta_ppp.push_back(InvMass_eta_ppp_allcombinations_mc);
	InvMass_eta_ppp_allcombinations_reffited = new TH1D("InvMass_eta_ppp_allcombinations_reffited","InvMass_eta_ppp_allcombinations_reffited; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
