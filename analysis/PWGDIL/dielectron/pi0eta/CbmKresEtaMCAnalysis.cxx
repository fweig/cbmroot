...
void CbmKresEtaMCAnalysis::Init() {
  ...
  if (nullptr == ioman) {
    Fatal("CbmKresEtaMCAnalysis::Init", "RootManager not instantised!");
  }

  fMcTracks = (TClonesArray*) ioman->GetObject("MCTrack");
  if (nullptr == fMcTracks) {
    Fatal("CbmKresEtaMCAnalysis::Init", "No MCTrack array!");
  }

  fGlobalTracks = (TClonesArray*) ioman->GetObject("GlobalTrack");
  if (nullptr == fGlobalTracks) {
    Fatal("CbmKresEtaMCAnalysis::Init", "No GlobalTrack array!");
  }

  fStsTracks = (TClonesArray*) ioman->GetObject("StsTrack");
  if (nullptr == fStsTracks) {
    Fatal("CbmKresEtaMCAnalysis::Init", "No StsTrack array!");
  }

  fStsTrackMatches = (TClonesArray*) ioman->GetObject("StsTrackMatch");
  if (nullptr == fStsTrackMatches) {
    Fatal("CbmKresEtaMCAnalysis::Init", "No StsTrackMatch array!");
  }
}


void CbmKresEtaMCAnalysis::Exec(int Event,
  ...
  for (Int_t iTr = 0; iTr < ngTracks; iTr++) {
    ...
    if (TMath::Abs(pdg) == 11 && mcMotherTrack->GetPdgCode() == 22) {
      ...
      if (mcGrTrack->GetPdgCode() == 111) {
        ...
        CbmMCTrack* mcTopTrack =
          (CbmMCTrack*) fMcTracks->At(mcGrTrack->GetMotherId());
  ...
  EtaDoubleGammaAnalysis(El_Photon_Eta_refmomentum,
                         El_Photon_Eta_MCtrack,
                         El_Photon_Eta_Id,
                         fHistoList_eta_gg);
  EtaChargedPionsGammaAnalysis(Pion_Eta_refmomentum,
                               Pion_Eta_MCtrack,
                               Pion_Eta_Id,
                               El_Photon_Eta_refmomentum,
                               El_Photon_Eta_MCtrack,
                               El_Photon_Eta_Id,
                               fHistoList_eta_ppg);
  EtaPosNegNeutralPionsAnalysis(El_Photon_Pion_Eta_refmomentum,
                                El_Photon_Pion_Eta_MCtrack,
                                El_Photon_Pion_Eta_Id,
                                Pion_Eta_refmomentum,
                                Pion_Eta_MCtrack,
  ...
  EtaDoubleGammaAnalysis_plusBG(OpeningAngleCut,
                                GammaInvMassCut,
                                Event,
                                All_El_refmomentum,
                                All_El_MCtrack,
                                All_El_Id,
                                fHistoList_eta_gg);
  EtaChargedPionsGammaAnalysis_plusBG(OpeningAngleCut,
                                      GammaInvMassCut,
                                      Event,
                                      All_Pion_refmomentum,
                                      All_Pion_MCtrack,
...
Double_t CbmKresEtaMCAnalysis::CalculateOpeningAngleBetweenGammas_MC(
  CbmMCTrack* mctrack1,
  CbmMCTrack* mctrack2,
  CbmMCTrack* mctrack3,
  CbmMCTrack* mctrack4) {
...
Double_t CbmKresEtaMCAnalysis::CalculateOpeningAngleBetweenGammas_Reco(
  TVector3 electron1,
  TVector3 electron2,
  TVector3 electron3,
  TVector3 electron4) {
...
void CbmKresEtaMCAnalysis::EtaDoubleGammaAnalysis(vector<TVector3> RefMom,
    ...
    for (size_t j = i + 1; j < MC.size(); j++) {
      for (size_t k = j + 1; k < MC.size(); k++) {
        for (size_t l = k + 1; l < MC.size(); l++) {
          ...
          if (mcId1 == mcId2 || mcId1 == mcId3 || mcId1 == mcId4
              || mcId2 == mcId3 || mcId2 == mcId4 || mcId3 == mcId4)
          ...
          if (grandmotherId1 == grandmotherId2
            ...
            Double_t InvMass_true = CbmKresFunctions::Invmass_4particles_MC(
              MC.at(i), MC.at(j), MC.at(k), MC.at(l));
            Double_t InvMass_reco = CbmKresFunctions::Invmass_4particles_RECO(
              RefMom.at(i), RefMom.at(j), RefMom.at(k), RefMom.at(l));
            cout
            ...
            cout << "grandmotherid: " << grandmotherId1 << "/" << grandmotherId2
                 << "/" << grandmotherId3 << "/" << grandmotherId4 << endl;
            cout << "pdgs: " << mcGrandmotherPdg1 << "-->" << mcMotherPdg1
                 << "/" << mcMotherPdg2 << "/" << mcMotherPdg3 << "/"
                 << mcMotherPdg4 << "-->" << pdg1 << "/" << pdg2 << "/" << pdg3
                 << "/" << pdg4 << endl;
            ...
            cout << "particle 1: \t" << MC.at(i)->GetPdgCode()
                 << ";\t pt = " << MC.at(i)->GetPt()
                 << ";\t X = " << MC.at(i)->GetStartX()
                 << ";\t Y = " << MC.at(i)->GetStartY()
                 << ";\t Z = " << MC.at(i)->GetStartZ()
                 << ";\t E = " << MC.at(i)->GetEnergy() << endl;
            cout << "particle 2: \t" << MC.at(j)->GetPdgCode()
                 << ";\t pt = " << MC.at(j)->GetPt()
                 << ";\t X = " << MC.at(j)->GetStartX()
                 << ";\t Y = " << MC.at(j)->GetStartY()
                 << ";\t Z = " << MC.at(j)->GetStartZ()
                 << ";\t E = " << MC.at(j)->GetEnergy() << endl;
            cout << "particle 3: \t" << MC.at(k)->GetPdgCode()
                 << ";\t pt = " << MC.at(k)->GetPt()
                 << ";\t X = " << MC.at(k)->GetStartX()
                 << ";\t Y = " << MC.at(k)->GetStartY()
                 << ";\t Z = " << MC.at(k)->GetStartZ()
                 << ";\t E = " << MC.at(k)->GetEnergy() << endl;
            cout << "particle 4: \t" << MC.at(l)->GetPdgCode()
                 << ";\t pt = " << MC.at(l)->GetPt()
                 << ";\t X = " << MC.at(l)->GetStartX()
                 << ";\t Y = " << MC.at(l)->GetStartY()
                 << ";\t Z = " << MC.at(l)->GetStartZ()
            ...
            if (motherId1 == motherId2) {
              OpeningAngle1_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(i), MC.at(j));
              OpeningAngle2_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(k), MC.at(l));
              gg[4]->Fill(OpeningAngle1_mc);
              gg[4]->Fill(OpeningAngle2_mc);
              OpeningAngle1_refitted =
                CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(i),
                                                             RefMom.at(j));
              OpeningAngle2_refitted =
                CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(k),
                                                             RefMom.at(l));
              gg[5]->Fill(OpeningAngle1_refitted);
              gg[5]->Fill(OpeningAngle2_refitted);

              InvMass_realg1_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(i), MC.at(j));
              InvMass_realg2_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(k), MC.at(l));
              gg[2]->Fill(InvMass_realg1_mc);
              gg[2]->Fill(InvMass_realg2_mc);
              InvMass_realg1_refitted =
                CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(i),
                                                          RefMom.at(j));
              InvMass_realg2_refitted =
                CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(k),
                                                          RefMom.at(l));
            ...
            if (motherId1 == motherId3) {
              OpeningAngle1_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(i), MC.at(k));
              OpeningAngle2_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(j), MC.at(l));
              gg[4]->Fill(OpeningAngle1_mc);
              gg[4]->Fill(OpeningAngle2_mc);
              OpeningAngle1_refitted =
                CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(i),
                                                             RefMom.at(k));
              OpeningAngle2_refitted =
                CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(j),
                                                             RefMom.at(l));
              gg[5]->Fill(OpeningAngle1_refitted);
              gg[5]->Fill(OpeningAngle2_refitted);

              InvMass_realg1_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(i), MC.at(k));
              InvMass_realg2_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(j), MC.at(l));
              gg[2]->Fill(InvMass_realg1_mc);
              gg[2]->Fill(InvMass_realg2_mc);
              InvMass_realg1_refitted =
                CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(i),
                                                          RefMom.at(k));
              InvMass_realg2_refitted =
                CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(j),
                                                          RefMom.at(l));
              gg[3]->Fill(InvMass_realg1_refitted);
              gg[3]->Fill(InvMass_realg2_refitted);
              openingAngleBetweenGammasReco =
                CalculateOpeningAngleBetweenGammas_Reco(
                  RefMom.at(i), RefMom.at(k), RefMom.at(j), RefMom.at(l));
            }
            if (motherId1 == motherId4) {
              OpeningAngle1_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(i), MC.at(l));
              OpeningAngle2_mc =
                CbmKresFunctions::CalculateOpeningAngle_MC(MC.at(j), MC.at(k));
              gg[4]->Fill(OpeningAngle1_mc);
              gg[4]->Fill(OpeningAngle2_mc);
              OpeningAngle1_refitted =
                CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(i),
              ...
              InvMass_realg1_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(i), MC.at(l));
              InvMass_realg2_mc =
                CbmKresFunctions::Invmass_2particles_MC(MC.at(j), MC.at(k));
              gg[2]->Fill(InvMass_realg1_mc);
              gg[2]->Fill(InvMass_realg2_mc);
              InvMass_realg1_refitted =
                CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(i),
              ...
              openingAngleBetweenGammasReco =
                CalculateOpeningAngleBetweenGammas_Reco(
                  RefMom.at(i), RefMom.at(l), RefMom.at(j), RefMom.at(k));
            }
...
void CbmKresEtaMCAnalysis::EtaChargedPionsGammaAnalysis(
    ...
    for (size_t j = i + 1; j < MCEl.size(); j++) {
      ...
      for (size_t k = 0; k < MCPion.size(); k++) {
        for (size_t l = k + 1; l < MCPion.size(); l++) {
          ...
          Double_t InvMass_true = CbmKresFunctions::Invmass_4particles_MC(
            MCEl.at(i), MCEl.at(j), MCPion.at(k), MCPion.at(l));
          Double_t InvMass_reco = CbmKresFunctions::Invmass_2el_2pions_RECO(
            RefMomEl.at(i), RefMomEl.at(j), RefMomPion.at(k), RefMomPion.at(l));
          cout << "Decay eta -> p+p- gamma -> p+p- e+e- detected!\t\t mc mass: "
               << InvMass_true << "\t, reco mass: " << InvMass_reco << endl;
          cout << "motherids: " << motherId1 << "/" << motherId2 << "/"
               << motherId3 << "/" << motherId4 << endl;
          cout << "grandmotherid: " << grandmotherId1 << endl;
          cout << "pdgs: " << mcGrandmotherPdg1 << "-->"
               << mother1->GetPdgCode() << "/" << pdg3 << "/" << pdg4 << "-->"
          ...
          double InvMass_g_mc =
            CbmKresFunctions::Invmass_2particles_MC(MCEl.at(i), MCEl.at(j));
          double InvMass_g_reffited = CbmKresFunctions::Invmass_2particles_RECO(
            RefMomEl.at(i), RefMomEl.at(j));
          double OpeningAngle_g_mc =
            CbmKresFunctions::CalculateOpeningAngle_MC(MCEl.at(i), MCEl.at(j));
          double OpeningAngle_g_refitted =
            CbmKresFunctions::CalculateOpeningAngle_Reco(RefMomEl.at(i),
                                                         RefMomEl.at(j));
...
void CbmKresEtaMCAnalysis::EtaPosNegNeutralPionsAnalysis(
  vector<TVector3> RefMomNeutral,
      ...
      for (size_t k = j + 1; k < MCNeutral.size(); k++) {
        for (size_t l = k + 1; l < MCNeutral.size(); l++) {
          ...
          if (grandmotherId1 == grandmotherId2
            ...
            CbmMCTrack* TopTrack =
              (CbmMCTrack*) fMcTracks->At(GrMother1->GetMotherId());
            ...
            for (size_t m = 0; m < MCPion.size(); m++) {
              for (size_t s = m + 1; s < MCPion.size(); s++) {
                ...
                if (TMath::Abs(pdg5) != 211 || TMath::Abs(pdg6) != 211)
                  continue;
                ...
                Double_t InvMass_six_true =
                  CbmKresFunctions::Invmass_6particles_MC(MCNeutral.at(i),
                                                          MCNeutral.at(j),
                                                          MCNeutral.at(k),
                                                          MCNeutral.at(l),
                                                          MCPion.at(m),
                                                          MCPion.at(s));
                Double_t InvMass_six_reco =
                  CbmKresFunctions::Invmass_4el_2pions_RECO(RefMomNeutral.at(i),
                                                            RefMomNeutral.at(j),
                                                            RefMomNeutral.at(k),
                                                            RefMomNeutral.at(l),
                                                            RefMomPion.at(m),
                ...
                cout << "motherids for neutral: " << motherId1 << "/"
                     << motherId2 << "/" << motherId3 << "/" << motherId4
                     << endl;
                cout << "grandmotherid: " << GrMother1->GetMotherId() << "/"
                     << motherId5 << "/" << motherId6 << endl;
                cout << "pdgs: " << TopTrack->GetPdgCode() << "-->"
                     << GrMother1->GetPdgCode() << "/" << pdg5 << "/" << pdg6
                     << endl;
                ...
                cout << "\t \t mc true info: " << endl;
                cout << "particle 1: \t" << MCNeutral.at(i)->GetPdgCode()
                     << ";\t pt = " << MCNeutral.at(i)->GetPt()
                     << ";\t X = " << MCNeutral.at(i)->GetStartX()
                     << ";\t Y = " << MCNeutral.at(i)->GetStartY()
                     << ";\t Z = " << MCNeutral.at(i)->GetStartZ()
                     << ";\t E = " << MCNeutral.at(i)->GetEnergy() << endl;
                cout << "particle 2: \t" << MCNeutral.at(j)->GetPdgCode()
                ...
                cout << "particle 3: \t" << MCNeutral.at(k)->GetPdgCode()
                ...
                cout << "particle 4: \t" << MCNeutral.at(l)->GetPdgCode()
                ...
                Double_t InvMass_true =
                  CbmKresFunctions::Invmass_4particles_MC(MCNeutral.at(i),
                                                          MCNeutral.at(j),
                                                          MCNeutral.at(k),
                                                          MCNeutral.at(l));
...
void CbmKresEtaMCAnalysis::EtaDoubleGammaAnalysis_plusBG(
  double OpeningAngleCut,
  double GammaInvMassCut,
  int Event,
  ...
  for (size_t i = 0; i < MC.size(); i++) {
    for (size_t j = i + 1; j < MC.size(); j++) {
      ...
      double OpeningAngle_g_refitted =
        CbmKresFunctions::CalculateOpeningAngle_Reco(RefMom.at(i),
                                                     RefMom.at(j));
      double InvMass_g_refitted =
        CbmKresFunctions::Invmass_2particles_RECO(RefMom.at(i), RefMom.at(j));

      if (OpeningAngle_g_refitted > OpeningAngleCut
          || InvMass_g_refitted > GammaInvMassCut)
        continue;
  ...
  for (size_t gamma1 = 0; gamma1 < EDGA_RefMom.size(); gamma1++) {
    for (size_t gamma2 = gamma1 + 1; gamma2 < EDGA_RefMom.size(); gamma2++) {
      ...
      if (mcId1 == mcId2 || mcId1 == mcId3 || mcId1 == mcId4 || mcId2 == mcId3
          || mcId2 == mcId4 || mcId3 == mcId4)
        continue;  // particle is not used twice

      double openingAngleBetweenGammasReco =
        CalculateOpeningAngleBetweenGammas_Reco(e11, e12, e21, e22);
      if (openingAngleBetweenGammasReco < 10
          || openingAngleBetweenGammasReco > 40)
        continue;

      double InvMass_true = CbmKresFunctions::Invmass_4particles_MC(
        mcTrack1, mcTrack2, mcTrack3, mcTrack4);
      double InvMass_reco =
        CbmKresFunctions::Invmass_4particles_RECO(e11, e12, e21, e22);
      ...
      if ((mcTrack1->GetMotherId() == mcTrack2->GetMotherId()
        ...
        if (MothTrack1->GetPdgCode() == 22 && MothTrack2->GetPdgCode() == 22
            && MothTrack3->GetPdgCode() == 22 && MothTrack4->GetPdgCode() == 22
...
void CbmKresEtaMCAnalysis::EtaChargedPionsGammaAnalysis_plusBG(
  double OpeningAngleCut,
  double GammaInvMassCut,
  int Event,
  vector<TVector3> RefMomPion,
  ...
  for (size_t i = 0; i < MCEl.size(); i++) {
    for (size_t j = i + 1; j < MCEl.size(); j++) {
      ...
      double OpeningAngle_g_refitted =
        CbmKresFunctions::CalculateOpeningAngle_Reco(RefMomEl.at(i),
                                                     RefMomEl.at(j));
      double InvMass_g_refitted = CbmKresFunctions::Invmass_2particles_RECO(
        RefMomEl.at(i), RefMomEl.at(j));
  ...
  for (size_t k = 0; k < MCPion.size(); k++) {
    ...
    for (size_t l = k + 1; l < MCPion.size(); l++) {
      ...
      double Pions_angle_true =
        CbmKresFunctions::CalculateOpeningAngleBetweenPions_MC(MCPion.at(k),
                                                               MCPion.at(l));
      ...
      if (mcTrack3->GetMotherId() == mcTrack4->GetMotherId()
          && mcTrack4->GetMotherId() != -1) {
        CbmMCTrack* EtaTrack =
          (CbmMCTrack*) fMcTracks->At(mcTrack4->GetMotherId());
  ...
  if (ECPGA_leptons_MC.size() < 1)
    return;  // min 1 gamma to form eta is required
  for (size_t gamma = 0; gamma < ECPGA_leptons_RefMom.size(); gamma++) {
    if (ECPGA_pions_MC.size() < 1)
      continue;  // min 1 p+p- pair to form eta is required
    ...
    for (size_t pionpair = 0; pionpair < ECPGA_pions_RefMom.size();
         pionpair++) {
      ...
      Double_t InvMass_true = CbmKresFunctions::Invmass_4particles_MC(
        mcTrack1, mcTrack2, mcTrack3, mcTrack4);
      Double_t InvMass_reco =
        CbmKresFunctions::Invmass_2el_2pions_RECO(e11, e12, e21, e22);
      CbmLmvmKinematicParams params =
  ...
  for (size_t gamma1 = 0; gamma1 < ECPGA_leptons_RefMom.size(); gamma1++) {
    for (size_t gamma2 = gamma1 + 1; gamma2 < ECPGA_leptons_RefMom.size();
         gamma2++) {
      ...
      for (size_t k = 0; k < MCPion.size(); k++) {
        for (size_t l = k + 1; l < MCPion.size(); l++) {
          ...
          double InvMass_six_true = CbmKresFunctions::Invmass_6particles_MC(
            mcTrack1, mcTrack2, mcTrack3, mcTrack4, MCPion.at(k), MCPion.at(l));
          double InvMass_six_reco = CbmKresFunctions::Invmass_4el_2pions_RECO(
            e11, e12, e21, e22, RefMomPion.at(k), RefMomPion.at(l));
          double Pions_angle_reco =
            CbmKresFunctions::CalculateOpeningAngleBetweenPions_Reco(
              RefMomPion.at(k), RefMomPion.at(l));
...
void CbmKresEtaMCAnalysis::Mixing_gg() {
  ...
  for (Int_t a = 0; a < nof - 1; a++) {
    for (Int_t b = a + 1; b < nof; b++) {
      if (EMT_gg_Event[a] == EMT_gg_Event[b])
...
void CbmKresEtaMCAnalysis::Mixing_ppg() {
  ...
  cout << "Mixing for  eta->(pp)g  channel - nof lepton pairs = " << nof_leptons
...
void CbmKresEtaMCAnalysis::Mixing_three_body() {
  ...
  cout << "Mixing 3 bodies for  eta-> p p g  channel - nof photons = "
       << nof_photons << ";  nof +pions = " << nof_positive
  ...
  for (Int_t p = 0; p < nof_positive; p++) {
    for (Int_t m = 0; m < nof_negative; m++) {
      ...
      double Pions_angle_reco =
        CbmKresFunctions::CalculateOpeningAngleBetweenPions_Reco(e21, e22);
      if (Pions_angle_reco > 20) continue;
...
void CbmKresEtaMCAnalysis::InitHistograms() {
  ...
  InvMass_eta_gg_mc =
    new TH1D("InvMass_eta_gg_mc",
             "InvMass_eta_gg_mc; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_gg.push_back(InvMass_eta_gg_mc);
  InvMass_eta_gg_reffited =
    new TH1D("InvMass_eta_gg_reffited",
             "InvMass_eta_gg_reffited; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_gg.push_back(InvMass_eta_gg_reffited);
  InvMassPhoton_eta_gg_mc =
    new TH1D("InvMassPhoton_eta_gg_mc",
             "InvMassPhoton_eta_gg_mc; invariant mass in GeV/c^{2};#",
             60,
             -0.01,
             0.05);
  fHistoList_eta_gg.push_back(InvMassPhoton_eta_gg_mc);
  InvMassPhoton_eta_gg_reffited =
    new TH1D("InvMassPhoton_eta_gg_reffited",
             "InvMassPhoton_eta_gg_reffited; invariant mass in GeV/c^{2};#",
             60,
             -0.01,
             0.05);
  fHistoList_eta_gg.push_back(InvMassPhoton_eta_gg_reffited);
  OpeningAnglePhoton_eta_gg_mc = new TH1D(
  ...
  OpeningAnglePhoton_eta_gg_reffited =
    new TH1D("OpeningAnglePhoton_eta_gg_reffited",
  ...
  OpeningAngle_eta_gg_between_gg_mc =
    new TH1D("OpeningAngle_eta_gg_between_gg_mc",
  ...
  OpeningAngle_eta_gg_between_gg_reffited =
    new TH1D("OpeningAngle_eta_gg_between_gg_reffited",
  ...
  InvMass_eta_gg_allcombinations_mc =
    new TH1D("InvMass_eta_gg_allcombinations_mc",
             "InvMass_eta_gg_allcombinations_mc; invariant mass in GeV/c^{2};#",
             200,
             0.0,
             2.0);
  fHistoList_eta_gg.push_back(InvMass_eta_gg_allcombinations_mc);
  InvMass_eta_gg_allcombinations_reffited = new TH1D(
    "InvMass_eta_gg_allcombinations_reffited",
    "InvMass_eta_gg_allcombinations_reffited; invariant mass in GeV/c^{2};#",
    200,
    0.0,
    2.0);
  fHistoList_eta_gg.push_back(InvMass_eta_gg_allcombinations_reffited);
  EMT_eta_gg = new TH1D(
    "EMT_eta_gg", "EMT_eta_gg; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
  fHistoList_eta_gg.push_back(EMT_eta_gg);
  InvMass_eta_gg_reco_aftercuts =
    new TH1D("InvMass_eta_gg_reco_aftercuts",
             "InvMass_eta_gg_reco_aftercuts; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_gg.push_back(InvMass_eta_gg_reco_aftercuts);
  rap_vs_pt_eta_gg_reco_aftercuts =
    new TH2D("rap_vs_pt_eta_gg_reco_aftercuts",
             "rap_vs_pt_eta_gg_reco_aftercuts; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             5,
  ...
  rap_vs_pt_NOTeta_gg_reco_aftercuts =
    new TH2D("rap_vs_pt_NOTeta_gg_reco_aftercuts",
  ...
  InvMass_eta_ppg_mc =
    new TH1D("InvMass_eta_ppg_mc",
             "InvMass_eta_ppg_mc; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_ppg.push_back(InvMass_eta_ppg_mc);
  InvMass_eta_ppg_reffited =
    new TH1D("InvMass_eta_ppg_reffited",
             "InvMass_eta_ppg_reffited; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_ppg.push_back(InvMass_eta_ppg_reffited);
  InvMassPhoton_eta_ppg_mc =
    new TH1D("InvMassPhoton_eta_ppg_mc",
             "InvMassPhoton_eta_ppg_mc; invariant mass in GeV/c^{2};#",
             60,
             -0.01,
             0.05);
  fHistoList_eta_ppg.push_back(InvMassPhoton_eta_ppg_mc);
  InvMassPhoton_eta_ppg_reffited =
    new TH1D("InvMassPhoton_eta_ppg_reffited",
             "InvMassPhoton_eta_ppg_reffited; invariant mass in GeV/c^{2};#",
             60,
             -0.01,
             0.05);
  fHistoList_eta_ppg.push_back(InvMassPhoton_eta_ppg_reffited);
  OpeningAnglePhoton_eta_ppg_mc = new TH1D(
  ...
  OpeningAnglePhoton_eta_ppg_reffited =
    new TH1D("OpeningAnglePhoton_eta_ppg_reffited",
  ...
  InvMass_eta_ppg_allcombinations_mc = new TH1D(
  ...
  InvMass_eta_ppg_allcombinations_reffited = new TH1D(
    "InvMass_eta_ppg_allcombinations_reffited",
    "InvMass_eta_ppg_allcombinations_reffited; invariant mass in GeV/c^{2};#",
    200,
    0.0,
    2.0);
  fHistoList_eta_ppg.push_back(InvMass_eta_ppg_allcombinations_reffited);
  Pion_P_fromEta_reco = new TH1D("Pion_P_fromEta_reco",
                                 "Pion_P_fromEta_reco; pions P in GeV/c;#",
                                 500,
                                 0.0,
                                 5.0);
  fHistoList_eta_ppg.push_back(Pion_P_fromEta_reco);
  Pion_P_elsewhere_reco = new TH1D("Pion_P_elsewhere_reco",
                                   "Pion_P_elsewhere_reco; pions P in GeV/c;#",
                                   500,
                                   0.0,
                                   5.0);
  fHistoList_eta_ppg.push_back(Pion_P_elsewhere_reco);
  Pion_Pt_fromEta_reco =
    new TH1D("Pion_Pt_fromEta_reco",
             "Pion_Pt_fromEta_reco; pions P_{t} in GeV/c;#",
             500,
             0.0,
             5.0);
  fHistoList_eta_ppg.push_back(Pion_Pt_fromEta_reco);
  Pion_Pt_elsewhere_reco =
    new TH1D("Pion_Pt_elsewhere_reco",
             "Pion_Pt_elsewhere_reco; pions P_{t} in GeV/c;#",
             500,
             0.0,
             5.0);
  fHistoList_eta_ppg.push_back(Pion_Pt_elsewhere_reco);
  OA_betweenPions_fromEta_mc =
    new TH1D("OA_betweenPions_fromEta_mc",
  ...
  OA_betweenPions_fromEta_reco =
    new TH1D("OA_betweenPions_fromEta_reco",
  ...
  OA_betweenPions_fromEta_reco_wrongcombinations =
    new TH1D("OA_betweenPions_fromEta_reco_wrongcombinations",
  ...
  EMT_eta_ppg = new TH1D(
    "EMT_eta_ppg", "EMT_eta_ppg; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
  fHistoList_eta_ppg.push_back(EMT_eta_ppg);
  EMT_eta_three_body =
    new TH1D("EMT_eta_three_body",
             "EMT_eta_three_body; invariant mass in GeV/c^{2};#",
             200,
             0.0,
             2.0);
  fHistoList_eta_ppg.push_back(EMT_eta_three_body);
  InvMass_eta_ppg_reco_aftercuts =
    new TH1D("InvMass_eta_ppg_reco_aftercuts",
             "InvMass_eta_ppg_reco_aftercuts; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_ppg.push_back(InvMass_eta_ppg_reco_aftercuts);
  rap_vs_pt_eta_ppg_reco_aftercuts =
    new TH2D("rap_vs_pt_eta_ppg_reco_aftercuts",
             "rap_vs_pt_eta_ppg_reco_aftercuts; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             5,
  ...
  rap_vs_pt_NOTeta_ppg_reco_aftercuts =
    new TH2D("rap_vs_pt_NOTeta_ppg_reco_aftercuts",
  ...
  InvMass_eta_ppp_mc =
    new TH1D("InvMass_eta_ppp_mc",
             "InvMass_eta_ppp_mc; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_ppp.push_back(InvMass_eta_ppp_mc);
  InvMass_eta_ppp_reffited =
    new TH1D("InvMass_eta_ppp_reffited",
             "InvMass_eta_ppp_reffited; invariant mass in GeV/c^{2};#",
             50,
             0.3,
             0.8);
  fHistoList_eta_ppp.push_back(InvMass_eta_ppp_reffited);
  InvMass_eta_Npion_mc =
    new TH1D("InvMass_eta_Npion_mc",
             "InvMass_eta_Npion_mc; invariant mass in GeV/c^{2};#",
             30,
             0.0,
             0.3);
  fHistoList_eta_ppp.push_back(InvMass_eta_Npion_mc);
  InvMass_eta_Npion_reffited =
    new TH1D("InvMass_eta_Npion_reffited",
             "InvMass_eta_Npion_reffited; invariant mass in GeV/c^{2};#",
             30,
             0.,
             0.3);
  fHistoList_eta_ppp.push_back(InvMass_eta_Npion_reffited);
  InvMass_eta_ppp_allcombinations_mc = new TH1D(
  ...
  InvMass_eta_ppp_allcombinations_reffited = new TH1D(
    "InvMass_eta_ppp_allcombinations_reffited",
    "InvMass_eta_ppp_allcombinations_reffited; invariant mass in GeV/c^{2};#",
    200,
    0.0,
    2.0);
