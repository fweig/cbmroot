...
void CbmKresGammaCorrection::Init(std::vector< std::vector<double> >  & vect_all, std::vector< std::vector<double> >  & vect_two, std::vector< std::vector<double> >  & vect_onetwo, double  OA, double IM)
{
	InitHistograms();

	string Correction_path = string(gSystem->Getenv("VMCWORKDIR")) + Form("/analysis/conversion2/Correction_pi0_g_OA%i_IM%i_num2.root", (int)OA, (int)IM);		/// most recent files for conversion
	cout << "file is " << Correction_path << endl;
	TFile *fcorrection = new TFile(Correction_path.c_str());				// file with almost ?? Mio photons, homogeneously distributed over interested region
	// rapidity graphs
...
{
	Correction_factros_all = new TH2D("Correction_factros_all", "Correction_factros_all; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_factors.push_back(Correction_factros_all);
	Correction_factros_two = new TH2D("Correction_factros_two", "Correction_factros_two; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_factors.push_back(Correction_factros_two);
	Correction_factros_onetwo = new TH2D("Correction_factros_onetwo", "Correction_factros_onetwo; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
