...
void CbmKresEta::Exec(int fEventNumEta, double OpeningAngleCut, double GammaInvMassCut, int RealPID)
{
	...
	FindGammasTarget(fEventNumEta, OpeningAngleCut, GammaInvMassCut, RealPID, VMCtracks_minus_Target, VMCtracks_plus_Target, VStsTrack_minus_Target, VStsTrack_plus_Target, VMomenta_minus_Target, VMomenta_plus_Target, VRings_minus_Target, VRings_plus_Target, VStsIndex_minus_Target, VStsIndex_plus_Target, VRichRing_minus_Target, VRichRing_plus_Target, VMCIndex_minus_Target, VMCIndex_plus_Target);

	FindGammasOutside(fEventNumEta, OpeningAngleCut, GammaInvMassCut, RealPID, VMCtracks_minus_Outside, VMCtracks_plus_Outside, VStsTrack_minus_Outside, VStsTrack_plus_Outside, VRings_minus_Outside, VRings_plus_Outside, VStsIndex_minus_Outside, VStsIndex_plus_Outside, VRichRing_minus_Outside, VRichRing_plus_Outside, VMCIndex_minus_Outside, VMCIndex_plus_Outside);

	FindGammasBoth();

	FindEta("all", "Target", Gammas_all_Target, Gammas_stsIndex_all_Target, Gammas_MCIndex_all_Target, Gammas_MC_all_Target, fHistoList_Eta_all_Target, fHistoList_rap_pt_Eta_all_Target);
	FindEta("onetwo", "Target", Gammas_onetwo_Target, Gammas_stsIndex_onetwo_Target, Gammas_MCIndex_onetwo_Target, Gammas_MC_onetwo_Target, fHistoList_Eta_onetwo_Target, fHistoList_rap_pt_Eta_onetwo_Target);
	FindEta("two", "Target", Gammas_two_Target, Gammas_stsIndex_two_Target, Gammas_MCIndex_two_Target, Gammas_MC_two_Target, fHistoList_Eta_two_Target, fHistoList_rap_pt_Eta_two_Target);

	FindEta("all", "Outside", Gammas_all_Outside, Gammas_stsIndex_all_Outside, Gammas_MCIndex_all_Outside, Gammas_MC_all_Outside, fHistoList_Eta_all_Outside, fHistoList_rap_pt_Eta_all_Outside);
	FindEta("onetwo", "Outside", Gammas_onetwo_Outside, Gammas_stsIndex_onetwo_Outside, Gammas_MCIndex_onetwo_Outside, Gammas_MC_onetwo_Outside, fHistoList_Eta_onetwo_Outside, fHistoList_rap_pt_Eta_onetwo_Outside);
	FindEta("two", "Outside", Gammas_two_Outside, Gammas_stsIndex_two_Outside, Gammas_MCIndex_two_Outside, Gammas_MC_two_Outside, fHistoList_Eta_two_Outside, fHistoList_rap_pt_Eta_two_Outside);

	FindEta("all", "Both", Gammas_all_Both, Gammas_stsIndex_all_Both, Gammas_MCIndex_all_Both, Gammas_MC_all_Both, fHistoList_Eta_all_Both, fHistoList_rap_pt_Eta_all_Both);
	FindEta("onetwo", "Both", Gammas_onetwo_Both, Gammas_stsIndex_onetwo_Both, Gammas_MCIndex_onetwo_Both, Gammas_MC_onetwo_Both, fHistoList_Eta_onetwo_Both, fHistoList_rap_pt_Eta_onetwo_Both);
	FindEta("two", "Both", Gammas_two_Both, Gammas_stsIndex_two_Both, Gammas_MCIndex_two_Both, Gammas_MC_two_Both, fHistoList_Eta_two_Both, fHistoList_rap_pt_Eta_two_Both);

	int numformix = 500;
	if(fEventNumEta%numformix == 0) {
		EMT_eta_gg_Event_Both.insert( EMT_eta_gg_Event_Both.end(), EMT_eta_gg_Event_Outside.begin(), EMT_eta_gg_Event_Outside.end() );
		EMT_eta_gg_Event_Both.insert( EMT_eta_gg_Event_Both.end(), EMT_eta_gg_Event_Target.begin(), EMT_eta_gg_Event_Target.end() );
		EMT_eta_gg_pair_momenta_Both.insert( EMT_eta_gg_pair_momenta_Both.end(), EMT_eta_gg_pair_momenta_Outside.begin(), EMT_eta_gg_pair_momenta_Outside.end() );
		EMT_eta_gg_pair_momenta_Both.insert( EMT_eta_gg_pair_momenta_Both.end(), EMT_eta_gg_pair_momenta_Target.begin(), EMT_eta_gg_pair_momenta_Target.end() );
		EMT_eta_gg_NofRings_Both.insert( EMT_eta_gg_NofRings_Both.end(), EMT_eta_gg_NofRings_Outside.begin(), EMT_eta_gg_NofRings_Outside.end() );
		EMT_eta_gg_NofRings_Both.insert( EMT_eta_gg_NofRings_Both.end(), EMT_eta_gg_NofRings_Target.begin(), EMT_eta_gg_NofRings_Target.end() );
...
void CbmKresEta::SaveOutsideTracks(CbmMCTrack* mcTrack1, CbmStsTrack* stsTrack, double charge, int stsInd, int richInd, int stsMcTrackId, CbmRichRing* RING)
{
...
void CbmKresEta::SaveTargetTracks(CbmMCTrack* mcTrack1, CbmStsTrack* stsTrack, TVector3 refmom, double charge, int stsInd, int richInd, int stsMcTrackId, CbmRichRing* RING)
{
	int InRich = FindInRich(richInd, stsMcTrackId);
	if (charge < 0){
	...
	vector<Int_t>  MCIndex_minus, vector<Int_t>  MCIndex_plus)
{
	for (size_t i=0; i<Momenta_minus.size(); i++){
		for (size_t j=0; j<Momenta_plus.size(); j++){
			...
			int TruePair = 0;  //correctly reconstructed photon from eta meson:          0 - means wrong pair combination; 		1 - means correct pair
			if (part1MC->GetMotherId() == part2MC->GetMotherId() && part1MC->GetMotherId() != -1){
				...
				if (mcTrackmama->GetMotherId() != -1 && mcTrackmama->GetPdgCode() == 22){
					...
					if(mcTrackgrmama->GetPdgCode() == 221){
						TruePair = 1;
					}
				}
			}
	...
	vector<Int_t>  MCIndex_minus_Outside, vector<Int_t>  MCIndex_plus_Outside)
{
	for (size_t i=0; i<StsTrack_minus_Outside.size(); i++){
		for (size_t j=0; j<StsTrack_plus_Outside.size(); j++){
			...
			TVector3 part1 = CbmKresFunctions::FitToVertexAndGetChi(sts1, intersection.GetX(), intersection.GetY(), intersection.GetZ(), chi_electron);
			TVector3 part2 = CbmKresFunctions::FitToVertexAndGetChi(sts2, intersection.GetX(), intersection.GetY(), intersection.GetZ(), chi_positron);
...
void CbmKresEta::FindEta(TString /*mod*/, TString position, vector< vector<TVector3> > Gammas, vector< vector<int> > StsIndex, vector< vector<int> > MCIndex, vector< vector<CbmMCTrack*> > GammasMC, 
						 vector<TH1*> gg, vector<TH1*> rap_pt_separation)
{
	...
	for (size_t gamma1=0; gamma1<Gammas.size()-1; gamma1++){
		for (size_t gamma2=gamma1+1; gamma2<Gammas.size(); gamma2++){
			...
			if(STSmcId1 != STSmcId2 && STSmcId1 != STSmcId3 && STSmcId1 != STSmcId4 && STSmcId2 != STSmcId3 && STSmcId2 != STSmcId4 && STSmcId3 != STSmcId4){
				if( TMath::Abs(mcTrack1->GetPdgCode()) == 11 && TMath::Abs(mcTrack2->GetPdgCode()) == 11 && TMath::Abs(mcTrack3->GetPdgCode()) == 11 && TMath::Abs(mcTrack4->GetPdgCode()) == 11){
					if ((mcTrack1->GetMotherId() == mcTrack2->GetMotherId() && mcTrack3->GetMotherId() == mcTrack4->GetMotherId()) || 
						...
						(mcTrack1->GetMotherId() == mcTrack4->GetMotherId() && mcTrack2->GetMotherId() == mcTrack3->GetMotherId())  	){
						...
						if (MothTrack1->GetPdgCode() == 22 && MothTrack2->GetPdgCode() == 22 && MothTrack3->GetPdgCode() == 22 && MothTrack4->GetPdgCode() == 22 && MothTrack1->GetMotherId() != -1 
							&&  MothTrack1->GetMotherId() == MothTrack2->GetMotherId() && MothTrack1->GetMotherId() == MothTrack3->GetMotherId() && MothTrack1->GetMotherId() == MothTrack4->GetMotherId()){
...
// TARGET
{
	...
	for(Int_t a = 0; a < nof_Target-1; a++) {
		for(Int_t b = a+1; b < nof_Target; b++) {
			if (EMT_eta_gg_Event_Target[a] == EMT_eta_gg_Event_Target[b]) continue;		// to make sure that the photons are from two different events
			...
			if (EMT_eta_gg_NofRings_Target[a] == 2 && EMT_eta_gg_NofRings_Target[b] == 2) EMT_Eta_InvMass_two_Target->Fill(params.fMinv);
			if ( (EMT_eta_gg_NofRings_Target[a] == 1 || EMT_eta_gg_NofRings_Target[a] == 2) && (EMT_eta_gg_NofRings_Target[b] == 1 || EMT_eta_gg_NofRings_Target[b] == 2) ) EMT_Eta_InvMass_onetwo_Target->Fill(params.fMinv);
		}
	}
}
...
// OUTSIDE
{
	...
	for(Int_t a = 0; a < nof_Outside-1; a++) {
		for(Int_t b = a+1; b < nof_Outside; b++) {
			if(EMT_eta_gg_Event_Outside[a] == EMT_eta_gg_Event_Outside[b]) continue;		// to make sure that the photons are from two different events
			...
			if (EMT_eta_gg_NofRings_Outside[a] == 2 && EMT_eta_gg_NofRings_Outside[b] == 2) EMT_Eta_InvMass_two_Outside->Fill(params.fMinv);
			if ( (EMT_eta_gg_NofRings_Outside[a] == 1 || EMT_eta_gg_NofRings_Outside[a] == 2) && (EMT_eta_gg_NofRings_Outside[b] == 1 || EMT_eta_gg_NofRings_Outside[b] == 2) ) EMT_Eta_InvMass_onetwo_Outside->Fill(params.fMinv);
		}
	}
}

void CbmKresEta::Mixing_Both(vector<TH1*> rap_pt_separation_all, vector<TH1*> rap_pt_separation_onetwo, vector<TH1*> rap_pt_separation_two)
// BOTH
{
	...
	for(Int_t a = 0; a < nof_Both-1; a++) {
		for(Int_t b = a+1; b < nof_Both; b++) {
			if(EMT_eta_gg_Event_Both[a] == EMT_eta_gg_Event_Both[b]) continue;		// to make sure that the photons are from two different events
			...
			if (EMT_eta_gg_NofRings_Both[a] == 2 && EMT_eta_gg_NofRings_Both[b] == 2) EMT_Eta_InvMass_two_Both->Fill(params.fMinv);
			if ( (EMT_eta_gg_NofRings_Both[a] == 1 || EMT_eta_gg_NofRings_Both[a] == 2) && (EMT_eta_gg_NofRings_Both[b] == 1 || EMT_eta_gg_NofRings_Both[b] == 2) ) EMT_Eta_InvMass_onetwo_Both->Fill(params.fMinv);
			...
			if ( (EMT_eta_gg_NofRings_Both[a] == 1 || EMT_eta_gg_NofRings_Both[a] == 2) && (EMT_eta_gg_NofRings_Both[b] == 1 || EMT_eta_gg_NofRings_Both[b] == 2) ) {
				if (params.fRapidity > 1.2 && params.fRapidity <= 1.6){
...
{
	Particle_pull_px_Target = new TH1D("Particle_pull_px_Target","Target track, pull Px: mc - reco; P_{x}(mc) - P_{x}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_px_Target);
	Particle_pull_py_Target = new TH1D("Particle_pull_py_Target","Target track, pull Px: mc - reco; P_{y}(mc) - P_{y}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_py_Target);
	Particle_pull_pz_Target = new TH1D("Particle_pull_pz_Target","Target track, pull Px: mc - reco; P_{z}(mc) - P_{z}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_pz_Target);
	Particle_pull_px_Outside = new TH1D("Particle_pull_px_Outside","Outside track, pull Px: mc - reco; P_{x}(mc) - P_{x}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_px_Outside);
	Particle_pull_py_Outside = new TH1D("Particle_pull_py_Outside","Outside track, pull Px: mc - reco; P_{y}(mc) - P_{y}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_py_Outside);
	Particle_pull_pz_Outside = new TH1D("Particle_pull_pz_Outside","Outside track, pull Px: mc - reco; P_{z}(mc) - P_{z}(reco)", 100, -0.5, 0.5);
	fHistoList_Eta.push_back(Particle_pull_pz_Outside);
	Particle_pull_X_Outside = new TH1D("Particle_pull_X_Outside","Outside track, pull Px: mc - reco; X(mc) - X(reco)", 500, -5, 5);
	fHistoList_Eta.push_back(Particle_pull_X_Outside);
	Particle_pull_Y_Outside = new TH1D("Particle_pull_Y_Outside","Outside track, pull Px: mc - reco; Y(mc) - Y(reco)", 500, -5, 5);
	fHistoList_Eta.push_back(Particle_pull_Y_Outside);
	Particle_pull_Z_Outside = new TH1D("Particle_pull_Z_Outside","Outside track, pull Px: mc - reco; Z(mc) - Z(reco)", 500, -5, 5);
	fHistoList_Eta.push_back(Particle_pull_Z_Outside);

	/////////// histograms to check Cuts => zeroInRich_Target
	GammasInvMass_fromEta_beforeCuts_zeroInRich_Target = new TH1D("GammasInvMass_fromEta_beforeCuts_zeroInRich_Target","GammasInvMass_fromEta_beforeCuts_zeroInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasInvMass_fromEta_beforeCuts_zeroInRich_Target);
	GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Target = new TH1D("GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Target","GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Target);
	GammasMomentum_fromEta_beforeCuts_zeroInRich_Target = new TH1D("GammasMomentum_fromEta_beforeCuts_zeroInRich_Target","GammasMomentum_fromEta_beforeCuts_zeroInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasMomentum_fromEta_beforeCuts_zeroInRich_Target);
	GammasRapidity_fromEta_beforeCuts_zeroInRich_Target = new TH1D("GammasRapidity_fromEta_beforeCuts_zeroInRich_Target","GammasRapidity_fromEta_beforeCuts_zeroInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasRapidity_fromEta_beforeCuts_zeroInRich_Target);
	PlaneAngles_last_fromEta_zeroInRich_Target = new TH1D("PlaneAngles_last_fromEta_zeroInRich_Target","PlaneAngles_last_fromEta_zeroInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(PlaneAngles_last_fromEta_zeroInRich_Target);
	PlaneAngles_first_fromEta_zeroInRich_Target = new TH1D("PlaneAngles_first_fromEta_zeroInRich_Target","PlaneAngles_first_fromEta_zeroInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(PlaneAngles_first_fromEta_zeroInRich_Target);
	GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Target = new TH1D("GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Target","GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Target);
	GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Target = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Target","GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Target);
	GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Target = new TH1D("GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Target","GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Target);
	GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Target = new TH1D("GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Target","GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Target);
	PlaneAngles_last_wrongpairs_zeroInRich_Target = new TH1D("PlaneAngles_last_wrongpairs_zeroInRich_Target","PlaneAngles_last_wrongpairs_zeroInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Target.push_back(PlaneAngles_last_wrongpairs_zeroInRich_Target);
	PlaneAngles_first_wrongpairs_zeroInRich_Target = new TH1D("PlaneAngles_first_wrongpairs_zeroInRich_Target","PlaneAngles_first_wrongpairs_zeroInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_oneInRich_Target = new TH1D("GammasInvMass_fromEta_beforeCuts_oneInRich_Target","GammasInvMass_fromEta_beforeCuts_oneInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasInvMass_fromEta_beforeCuts_oneInRich_Target);
	GammasOpenAngle_fromEta_beforeCuts_oneInRich_Target = new TH1D("GammasOpenAngle_fromEta_beforeCuts_oneInRich_Target","GammasOpenAngle_fromEta_beforeCuts_oneInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasOpenAngle_fromEta_beforeCuts_oneInRich_Target);
	GammasMomentum_fromEta_beforeCuts_oneInRich_Target = new TH1D("GammasMomentum_fromEta_beforeCuts_oneInRich_Target","GammasMomentum_fromEta_beforeCuts_oneInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasMomentum_fromEta_beforeCuts_oneInRich_Target);
	GammasRapidity_fromEta_beforeCuts_oneInRich_Target = new TH1D("GammasRapidity_fromEta_beforeCuts_oneInRich_Target","GammasRapidity_fromEta_beforeCuts_oneInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasRapidity_fromEta_beforeCuts_oneInRich_Target);
	PlaneAngles_last_fromEta_oneInRich_Target = new TH1D("PlaneAngles_last_fromEta_oneInRich_Target","PlaneAngles_last_fromEta_oneInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(PlaneAngles_last_fromEta_oneInRich_Target);
	PlaneAngles_first_fromEta_oneInRich_Target = new TH1D("PlaneAngles_first_fromEta_oneInRich_Target","PlaneAngles_first_fromEta_oneInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(PlaneAngles_first_fromEta_oneInRich_Target);
	GammasInvMass_wrongpairs_beforeCuts_oneInRich_Target = new TH1D("GammasInvMass_wrongpairs_beforeCuts_oneInRich_Target","GammasInvMass_wrongpairs_beforeCuts_oneInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasInvMass_wrongpairs_beforeCuts_oneInRich_Target);
	GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Target = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Target","GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Target);
	GammasMomentum_wrongpairs_beforeCuts_oneInRich_Target = new TH1D("GammasMomentum_wrongpairs_beforeCuts_oneInRich_Target","GammasMomentum_wrongpairs_beforeCuts_oneInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasMomentum_wrongpairs_beforeCuts_oneInRich_Target);
	GammasRapidity_wrongpairs_beforeCuts_oneInRich_Target = new TH1D("GammasRapidity_wrongpairs_beforeCuts_oneInRich_Target","GammasRapidity_wrongpairs_beforeCuts_oneInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(GammasRapidity_wrongpairs_beforeCuts_oneInRich_Target);
	PlaneAngles_last_wrongpairs_oneInRich_Target = new TH1D("PlaneAngles_last_wrongpairs_oneInRich_Target","PlaneAngles_last_wrongpairs_oneInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Target.push_back(PlaneAngles_last_wrongpairs_oneInRich_Target);
	PlaneAngles_first_wrongpairs_oneInRich_Target = new TH1D("PlaneAngles_first_wrongpairs_oneInRich_Target","PlaneAngles_first_wrongpairs_oneInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_twoInRich_Target = new TH1D("GammasInvMass_fromEta_beforeCuts_twoInRich_Target","GammasInvMass_fromEta_beforeCuts_twoInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasInvMass_fromEta_beforeCuts_twoInRich_Target);
	GammasOpenAngle_fromEta_beforeCuts_twoInRich_Target = new TH1D("GammasOpenAngle_fromEta_beforeCuts_twoInRich_Target","GammasOpenAngle_fromEta_beforeCuts_twoInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasOpenAngle_fromEta_beforeCuts_twoInRich_Target);
	GammasMomentum_fromEta_beforeCuts_twoInRich_Target = new TH1D("GammasMomentum_fromEta_beforeCuts_twoInRich_Target","GammasMomentum_fromEta_beforeCuts_twoInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasMomentum_fromEta_beforeCuts_twoInRich_Target);
	GammasRapidity_fromEta_beforeCuts_twoInRich_Target = new TH1D("GammasRapidity_fromEta_beforeCuts_twoInRich_Target","GammasRapidity_fromEta_beforeCuts_twoInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasRapidity_fromEta_beforeCuts_twoInRich_Target);
	PlaneAngles_last_fromEta_twoInRich_Target = new TH1D("PlaneAngles_last_fromEta_twoInRich_Target","PlaneAngles_last_fromEta_twoInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(PlaneAngles_last_fromEta_twoInRich_Target);
	PlaneAngles_first_fromEta_twoInRich_Target = new TH1D("PlaneAngles_first_fromEta_twoInRich_Target","PlaneAngles_first_fromEta_twoInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(PlaneAngles_first_fromEta_twoInRich_Target);
	GammasInvMass_wrongpairs_beforeCuts_twoInRich_Target = new TH1D("GammasInvMass_wrongpairs_beforeCuts_twoInRich_Target","GammasInvMass_wrongpairs_beforeCuts_twoInRich_Target; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasInvMass_wrongpairs_beforeCuts_twoInRich_Target);
	GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Target = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Target","GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Target; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Target);
	GammasMomentum_wrongpairs_beforeCuts_twoInRich_Target = new TH1D("GammasMomentum_wrongpairs_beforeCuts_twoInRich_Target","GammasMomentum_wrongpairs_beforeCuts_twoInRich_Target; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasMomentum_wrongpairs_beforeCuts_twoInRich_Target);
	GammasRapidity_wrongpairs_beforeCuts_twoInRich_Target = new TH1D("GammasRapidity_wrongpairs_beforeCuts_twoInRich_Target","GammasRapidity_wrongpairs_beforeCuts_twoInRich_Target; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(GammasRapidity_wrongpairs_beforeCuts_twoInRich_Target);
	PlaneAngles_last_wrongpairs_twoInRich_Target = new TH1D("PlaneAngles_last_wrongpairs_twoInRich_Target","PlaneAngles_last_wrongpairs_twoInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Target.push_back(PlaneAngles_last_wrongpairs_twoInRich_Target);
	PlaneAngles_first_wrongpairs_twoInRich_Target = new TH1D("PlaneAngles_first_wrongpairs_twoInRich_Target","PlaneAngles_first_wrongpairs_twoInRich_Target; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_zeroInRich_Outside = new TH1D("GammasInvMass_fromEta_beforeCuts_zeroInRich_Outside","GammasInvMass_fromEta_beforeCuts_zeroInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasInvMass_fromEta_beforeCuts_zeroInRich_Outside);
	GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Outside = new TH1D("GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Outside","GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Outside);
	GammasMomentum_fromEta_beforeCuts_zeroInRich_Outside = new TH1D("GammasMomentum_fromEta_beforeCuts_zeroInRich_Outside","GammasMomentum_fromEta_beforeCuts_zeroInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasMomentum_fromEta_beforeCuts_zeroInRich_Outside);
	GammasRapidity_fromEta_beforeCuts_zeroInRich_Outside = new TH1D("GammasRapidity_fromEta_beforeCuts_zeroInRich_Outside","GammasRapidity_fromEta_beforeCuts_zeroInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasRapidity_fromEta_beforeCuts_zeroInRich_Outside);
	PlaneAngles_last_fromEta_zeroInRich_Outside = new TH1D("PlaneAngles_last_fromEta_zeroInRich_Outside","PlaneAngles_last_fromEta_zeroInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(PlaneAngles_last_fromEta_zeroInRich_Outside);
	PlaneAngles_first_fromEta_zeroInRich_Outside = new TH1D("PlaneAngles_first_fromEta_zeroInRich_Outside","PlaneAngles_first_fromEta_zeroInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(PlaneAngles_first_fromEta_zeroInRich_Outside);
	GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Outside = new TH1D("GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Outside","GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Outside);
	GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Outside = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Outside","GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Outside);
	GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Outside = new TH1D("GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Outside","GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Outside);
	GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Outside = new TH1D("GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Outside","GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Outside);
	PlaneAngles_last_wrongpairs_zeroInRich_Outside = new TH1D("PlaneAngles_last_wrongpairs_zeroInRich_Outside","PlaneAngles_last_wrongpairs_zeroInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Outside.push_back(PlaneAngles_last_wrongpairs_zeroInRich_Outside);
	PlaneAngles_first_wrongpairs_zeroInRich_Outside = new TH1D("PlaneAngles_first_wrongpairs_zeroInRich_Outside","PlaneAngles_first_wrongpairs_zeroInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_oneInRich_Outside = new TH1D("GammasInvMass_fromEta_beforeCuts_oneInRich_Outside","GammasInvMass_fromEta_beforeCuts_oneInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasInvMass_fromEta_beforeCuts_oneInRich_Outside);
	GammasOpenAngle_fromEta_beforeCuts_oneInRich_Outside = new TH1D("GammasOpenAngle_fromEta_beforeCuts_oneInRich_Outside","GammasOpenAngle_fromEta_beforeCuts_oneInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasOpenAngle_fromEta_beforeCuts_oneInRich_Outside);
	GammasMomentum_fromEta_beforeCuts_oneInRich_Outside = new TH1D("GammasMomentum_fromEta_beforeCuts_oneInRich_Outside","GammasMomentum_fromEta_beforeCuts_oneInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasMomentum_fromEta_beforeCuts_oneInRich_Outside);
	GammasRapidity_fromEta_beforeCuts_oneInRich_Outside = new TH1D("GammasRapidity_fromEta_beforeCuts_oneInRich_Outside","GammasRapidity_fromEta_beforeCuts_oneInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasRapidity_fromEta_beforeCuts_oneInRich_Outside);
	PlaneAngles_last_fromEta_oneInRich_Outside = new TH1D("PlaneAngles_last_fromEta_oneInRich_Outside","PlaneAngles_last_fromEta_oneInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(PlaneAngles_last_fromEta_oneInRich_Outside);
	PlaneAngles_first_fromEta_oneInRich_Outside = new TH1D("PlaneAngles_first_fromEta_oneInRich_Outside","PlaneAngles_first_fromEta_oneInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(PlaneAngles_first_fromEta_oneInRich_Outside);
	GammasInvMass_wrongpairs_beforeCuts_oneInRich_Outside = new TH1D("GammasInvMass_wrongpairs_beforeCuts_oneInRich_Outside","GammasInvMass_wrongpairs_beforeCuts_oneInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasInvMass_wrongpairs_beforeCuts_oneInRich_Outside);
	GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Outside = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Outside","GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Outside);
	GammasMomentum_wrongpairs_beforeCuts_oneInRich_Outside = new TH1D("GammasMomentum_wrongpairs_beforeCuts_oneInRich_Outside","GammasMomentum_wrongpairs_beforeCuts_oneInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasMomentum_wrongpairs_beforeCuts_oneInRich_Outside);
	GammasRapidity_wrongpairs_beforeCuts_oneInRich_Outside = new TH1D("GammasRapidity_wrongpairs_beforeCuts_oneInRich_Outside","GammasRapidity_wrongpairs_beforeCuts_oneInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(GammasRapidity_wrongpairs_beforeCuts_oneInRich_Outside);
	PlaneAngles_last_wrongpairs_oneInRich_Outside = new TH1D("PlaneAngles_last_wrongpairs_oneInRich_Outside","PlaneAngles_last_wrongpairs_oneInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Outside.push_back(PlaneAngles_last_wrongpairs_oneInRich_Outside);
	PlaneAngles_first_wrongpairs_oneInRich_Outside = new TH1D("PlaneAngles_first_wrongpairs_oneInRich_Outside","PlaneAngles_first_wrongpairs_oneInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_twoInRich_Outside = new TH1D("GammasInvMass_fromEta_beforeCuts_twoInRich_Outside","GammasInvMass_fromEta_beforeCuts_twoInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasInvMass_fromEta_beforeCuts_twoInRich_Outside);
	GammasOpenAngle_fromEta_beforeCuts_twoInRich_Outside = new TH1D("GammasOpenAngle_fromEta_beforeCuts_twoInRich_Outside","GammasOpenAngle_fromEta_beforeCuts_twoInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasOpenAngle_fromEta_beforeCuts_twoInRich_Outside);
	GammasMomentum_fromEta_beforeCuts_twoInRich_Outside = new TH1D("GammasMomentum_fromEta_beforeCuts_twoInRich_Outside","GammasMomentum_fromEta_beforeCuts_twoInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasMomentum_fromEta_beforeCuts_twoInRich_Outside);
	GammasRapidity_fromEta_beforeCuts_twoInRich_Outside = new TH1D("GammasRapidity_fromEta_beforeCuts_twoInRich_Outside","GammasRapidity_fromEta_beforeCuts_twoInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasRapidity_fromEta_beforeCuts_twoInRich_Outside);
	PlaneAngles_last_fromEta_twoInRich_Outside = new TH1D("PlaneAngles_last_fromEta_twoInRich_Outside","PlaneAngles_last_fromEta_twoInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(PlaneAngles_last_fromEta_twoInRich_Outside);
	PlaneAngles_first_fromEta_twoInRich_Outside = new TH1D("PlaneAngles_first_fromEta_twoInRich_Outside","PlaneAngles_first_fromEta_twoInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(PlaneAngles_first_fromEta_twoInRich_Outside);
	GammasInvMass_wrongpairs_beforeCuts_twoInRich_Outside = new TH1D("GammasInvMass_wrongpairs_beforeCuts_twoInRich_Outside","GammasInvMass_wrongpairs_beforeCuts_twoInRich_Outside; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasInvMass_wrongpairs_beforeCuts_twoInRich_Outside);
	GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Outside = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Outside","GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Outside; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Outside);
	GammasMomentum_wrongpairs_beforeCuts_twoInRich_Outside = new TH1D("GammasMomentum_wrongpairs_beforeCuts_twoInRich_Outside","GammasMomentum_wrongpairs_beforeCuts_twoInRich_Outside; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasMomentum_wrongpairs_beforeCuts_twoInRich_Outside);
	GammasRapidity_wrongpairs_beforeCuts_twoInRich_Outside = new TH1D("GammasRapidity_wrongpairs_beforeCuts_twoInRich_Outside","GammasRapidity_wrongpairs_beforeCuts_twoInRich_Outside; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(GammasRapidity_wrongpairs_beforeCuts_twoInRich_Outside);
	PlaneAngles_last_wrongpairs_twoInRich_Outside = new TH1D("PlaneAngles_last_wrongpairs_twoInRich_Outside","PlaneAngles_last_wrongpairs_twoInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Outside.push_back(PlaneAngles_last_wrongpairs_twoInRich_Outside);
	PlaneAngles_first_wrongpairs_twoInRich_Outside = new TH1D("PlaneAngles_first_wrongpairs_twoInRich_Outside","PlaneAngles_first_wrongpairs_twoInRich_Outside; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_zeroInRich_Both = new TH1D("GammasInvMass_fromEta_beforeCuts_zeroInRich_Both","GammasInvMass_fromEta_beforeCuts_zeroInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasInvMass_fromEta_beforeCuts_zeroInRich_Both);
	GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Both = new TH1D("GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Both","GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasOpenAngle_fromEta_beforeCuts_zeroInRich_Both);
	GammasMomentum_fromEta_beforeCuts_zeroInRich_Both = new TH1D("GammasMomentum_fromEta_beforeCuts_zeroInRich_Both","GammasMomentum_fromEta_beforeCuts_zeroInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasMomentum_fromEta_beforeCuts_zeroInRich_Both);
	GammasRapidity_fromEta_beforeCuts_zeroInRich_Both = new TH1D("GammasRapidity_fromEta_beforeCuts_zeroInRich_Both","GammasRapidity_fromEta_beforeCuts_zeroInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasRapidity_fromEta_beforeCuts_zeroInRich_Both);
	PlaneAngles_last_fromEta_zeroInRich_Both = new TH1D("PlaneAngles_last_fromEta_zeroInRich_Both","PlaneAngles_last_fromEta_zeroInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(PlaneAngles_last_fromEta_zeroInRich_Both);
	PlaneAngles_first_fromEta_zeroInRich_Both = new TH1D("PlaneAngles_first_fromEta_zeroInRich_Both","PlaneAngles_first_fromEta_zeroInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(PlaneAngles_first_fromEta_zeroInRich_Both);
	GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Both = new TH1D("GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Both","GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasInvMass_wrongpairs_beforeCuts_zeroInRich_Both);
	GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Both = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Both","GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasOpenAngle_wrongpairs_beforeCuts_zeroInRich_Both);
	GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Both = new TH1D("GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Both","GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasMomentum_wrongpairs_beforeCuts_zeroInRich_Both);
	GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Both = new TH1D("GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Both","GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(GammasRapidity_wrongpairs_beforeCuts_zeroInRich_Both);
	PlaneAngles_last_wrongpairs_zeroInRich_Both = new TH1D("PlaneAngles_last_wrongpairs_zeroInRich_Both","PlaneAngles_last_wrongpairs_zeroInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_zeroInRich_Both.push_back(PlaneAngles_last_wrongpairs_zeroInRich_Both);
	PlaneAngles_first_wrongpairs_zeroInRich_Both = new TH1D("PlaneAngles_first_wrongpairs_zeroInRich_Both","PlaneAngles_first_wrongpairs_zeroInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_oneInRich_Both = new TH1D("GammasInvMass_fromEta_beforeCuts_oneInRich_Both","GammasInvMass_fromEta_beforeCuts_oneInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasInvMass_fromEta_beforeCuts_oneInRich_Both);
	GammasOpenAngle_fromEta_beforeCuts_oneInRich_Both = new TH1D("GammasOpenAngle_fromEta_beforeCuts_oneInRich_Both","GammasOpenAngle_fromEta_beforeCuts_oneInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasOpenAngle_fromEta_beforeCuts_oneInRich_Both);
	GammasMomentum_fromEta_beforeCuts_oneInRich_Both = new TH1D("GammasMomentum_fromEta_beforeCuts_oneInRich_Both","GammasMomentum_fromEta_beforeCuts_oneInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasMomentum_fromEta_beforeCuts_oneInRich_Both);
	GammasRapidity_fromEta_beforeCuts_oneInRich_Both = new TH1D("GammasRapidity_fromEta_beforeCuts_oneInRich_Both","GammasRapidity_fromEta_beforeCuts_oneInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasRapidity_fromEta_beforeCuts_oneInRich_Both);
	PlaneAngles_last_fromEta_oneInRich_Both = new TH1D("PlaneAngles_last_fromEta_oneInRich_Both","PlaneAngles_last_fromEta_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_last_fromEta_oneInRich_Both);
	PlaneAngles_first_fromEta_oneInRich_Both = new TH1D("PlaneAngles_first_fromEta_oneInRich_Both","PlaneAngles_first_fromEta_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_first_fromEta_oneInRich_Both);
	GammasInvMass_wrongpairs_beforeCuts_oneInRich_Both = new TH1D("GammasInvMass_wrongpairs_beforeCuts_oneInRich_Both","GammasInvMass_wrongpairs_beforeCuts_oneInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasInvMass_wrongpairs_beforeCuts_oneInRich_Both);
	GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Both = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Both","GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasOpenAngle_wrongpairs_beforeCuts_oneInRich_Both);
	GammasMomentum_wrongpairs_beforeCuts_oneInRich_Both = new TH1D("GammasMomentum_wrongpairs_beforeCuts_oneInRich_Both","GammasMomentum_wrongpairs_beforeCuts_oneInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasMomentum_wrongpairs_beforeCuts_oneInRich_Both);
	GammasRapidity_wrongpairs_beforeCuts_oneInRich_Both = new TH1D("GammasRapidity_wrongpairs_beforeCuts_oneInRich_Both","GammasRapidity_wrongpairs_beforeCuts_oneInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(GammasRapidity_wrongpairs_beforeCuts_oneInRich_Both);
	PlaneAngles_last_wrongpairs_oneInRich_Both = new TH1D("PlaneAngles_last_wrongpairs_oneInRich_Both","PlaneAngles_last_wrongpairs_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_last_wrongpairs_oneInRich_Both);
	PlaneAngles_first_wrongpairs_oneInRich_Both = new TH1D("PlaneAngles_first_wrongpairs_oneInRich_Both","PlaneAngles_first_wrongpairs_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_first_wrongpairs_oneInRich_Both);
	PlaneAngles_last_fromEta_afterCuts_oneInRich_Both = new TH1D("PlaneAngles_last_fromEta_afterCuts_oneInRich_Both","PlaneAngles_last_fromEta_afterCuts_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_last_fromEta_afterCuts_oneInRich_Both);
	PlaneAngles_first_fromEta_afterCuts_oneInRich_Both = new TH1D("PlaneAngles_first_fromEta_afterCuts_oneInRich_Both","PlaneAngles_first_fromEta_afterCuts_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_first_fromEta_afterCuts_oneInRich_Both);	
	PlaneAngles_last_wrongpairs_afterCuts_oneInRich_Both = new TH1D("PlaneAngles_last_wrongpairs_afterCuts_oneInRich_Both","PlaneAngles_last_wrongpairs_afterCuts_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_oneInRich_Both.push_back(PlaneAngles_last_wrongpairs_afterCuts_oneInRich_Both);
	PlaneAngles_first_wrongpairs_afterCuts_oneInRich_Both = new TH1D("PlaneAngles_first_wrongpairs_afterCuts_oneInRich_Both","PlaneAngles_first_wrongpairs_afterCuts_oneInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	...
	GammasInvMass_fromEta_beforeCuts_twoInRich_Both = new TH1D("GammasInvMass_fromEta_beforeCuts_twoInRich_Both","GammasInvMass_fromEta_beforeCuts_twoInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasInvMass_fromEta_beforeCuts_twoInRich_Both);
	GammasOpenAngle_fromEta_beforeCuts_twoInRich_Both = new TH1D("GammasOpenAngle_fromEta_beforeCuts_twoInRich_Both","GammasOpenAngle_fromEta_beforeCuts_twoInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasOpenAngle_fromEta_beforeCuts_twoInRich_Both);
	GammasMomentum_fromEta_beforeCuts_twoInRich_Both = new TH1D("GammasMomentum_fromEta_beforeCuts_twoInRich_Both","GammasMomentum_fromEta_beforeCuts_twoInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasMomentum_fromEta_beforeCuts_twoInRich_Both);
	GammasRapidity_fromEta_beforeCuts_twoInRich_Both = new TH1D("GammasRapidity_fromEta_beforeCuts_twoInRich_Both","GammasRapidity_fromEta_beforeCuts_twoInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasRapidity_fromEta_beforeCuts_twoInRich_Both);
	PlaneAngles_last_fromEta_twoInRich_Both = new TH1D("PlaneAngles_last_fromEta_twoInRich_Both","PlaneAngles_last_fromEta_twoInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(PlaneAngles_last_fromEta_twoInRich_Both);
	PlaneAngles_first_fromEta_twoInRich_Both = new TH1D("PlaneAngles_first_fromEta_twoInRich_Both","PlaneAngles_first_fromEta_twoInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(PlaneAngles_first_fromEta_twoInRich_Both);
	GammasInvMass_wrongpairs_beforeCuts_twoInRich_Both = new TH1D("GammasInvMass_wrongpairs_beforeCuts_twoInRich_Both","GammasInvMass_wrongpairs_beforeCuts_twoInRich_Both; invariant mass in GeV/c^{2}", 510, -0.01, 0.5);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasInvMass_wrongpairs_beforeCuts_twoInRich_Both);
	GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Both = new TH1D("GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Both","GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Both; opening angle in degree", 300, -0.1, 29.9);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasOpenAngle_wrongpairs_beforeCuts_twoInRich_Both);
	GammasMomentum_wrongpairs_beforeCuts_twoInRich_Both = new TH1D("GammasMomentum_wrongpairs_beforeCuts_twoInRich_Both","GammasMomentum_wrongpairs_beforeCuts_twoInRich_Both; p in GeV/c", 400, 0, 4);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasMomentum_wrongpairs_beforeCuts_twoInRich_Both);
	GammasRapidity_wrongpairs_beforeCuts_twoInRich_Both = new TH1D("GammasRapidity_wrongpairs_beforeCuts_twoInRich_Both","GammasRapidity_wrongpairs_beforeCuts_twoInRich_Both; rapidity", 400, 0., 4.);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(GammasRapidity_wrongpairs_beforeCuts_twoInRich_Both);
	PlaneAngles_last_wrongpairs_twoInRich_Both = new TH1D("PlaneAngles_last_wrongpairs_twoInRich_Both","PlaneAngles_last_wrongpairs_twoInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	fHistoList_Eta_cuts_twoInRich_Both.push_back(PlaneAngles_last_wrongpairs_twoInRich_Both);
	PlaneAngles_first_wrongpairs_twoInRich_Both = new TH1D("PlaneAngles_first_wrongpairs_twoInRich_Both","PlaneAngles_first_wrongpairs_twoInRich_Both; #theta angle in degree;#", 720, -1., 179.);
	...
	AngleBetweenGammas_trueEta_before_cuts_all_Target = new TH1D("AngleBetweenGammas_trueEta_before_cuts_all_Target","AngleBetweenGammas_trueEta_before_cuts_all_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Target.push_back(AngleBetweenGammas_trueEta_before_cuts_all_Target);
	AngleBetweenGammas_wrongEta_before_cuts_all_Target = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_all_Target","AngleBetweenGammas_wrongEta_before_cuts_all_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Target.push_back(AngleBetweenGammas_wrongEta_before_cuts_all_Target);
	InvMass_GammaCombinations_all_Target = new TH1D("InvMass_GammaCombinations_all_Target","InvMass_GammaCombinations_all_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_all_Target = new TH2D("TrueEta_pt_vs_rap_all_Target","TrueEta_pt_vs_rap_all_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Target.push_back(TrueEta_pt_vs_rap_all_Target);
	WrongEta_pt_vs_rap_all_Target = new TH2D("WrongEta_pt_vs_rap_all_Target","WrongEta_pt_vs_rap_all_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Target.push_back(WrongEta_pt_vs_rap_all_Target);
	TrueEta_pt_vs_rap_est_all_Target = new TH2D("TrueEta_pt_vs_rap_est_all_Target","TrueEta_pt_vs_rap_est_all_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Target.push_back(TrueEta_pt_vs_rap_est_all_Target);
	WrongEta_pt_vs_rap_est_all_Target = new TH2D("WrongEta_pt_vs_rap_est_all_Target","WrongEta_pt_vs_rap_est_all_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Target.push_back(WrongEta_pt_vs_rap_est_all_Target);
	TrueEta_InvMass_after_cuts_all_Target = new TH1D("TrueEta_InvMass_after_cuts_all_Target","TrueEta_InvMass_after_cuts_all_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Target.push_back(TrueEta_InvMass_after_cuts_all_Target);
	WrongEta_InvMass_after_cuts_all_Target = new TH1D("WrongEta_InvMass_after_cuts_all_Target","WrongEta_InvMass_after_cuts_all_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Target.push_back(WrongEta_InvMass_after_cuts_all_Target);
	TrueEta_AngleBetweenGammas_after_cuts_all_Target = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_all_Target","TrueEta_AngleBetweenGammas_after_cuts_all_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Target.push_back(TrueEta_AngleBetweenGammas_after_cuts_all_Target);
	WrongEta_AngleBetweenGammas_after_cuts_all_Target = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_all_Target","WrongEta_AngleBetweenGammas_after_cuts_all_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Target.push_back(WrongEta_AngleBetweenGammas_after_cuts_all_Target);
	EMT_Eta_InvMass_all_Target = new TH1D("EMT_Eta_InvMass_all_Target","EMT_Eta_InvMass_all_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Target.push_back(EMT_Eta_InvMass_all_Target);
	//////// eta -> g + g    reconstruction case "onetwo" and "Target"
	AngleBetweenGammas_trueEta_before_cuts_onetwo_Target = new TH1D("AngleBetweenGammas_trueEta_before_cuts_onetwo_Target","AngleBetweenGammas_trueEta_before_cuts_onetwo_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Target.push_back(AngleBetweenGammas_trueEta_before_cuts_onetwo_Target);
	AngleBetweenGammas_wrongEta_before_cuts_onetwo_Target = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_onetwo_Target","AngleBetweenGammas_wrongEta_before_cuts_onetwo_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Target.push_back(AngleBetweenGammas_wrongEta_before_cuts_onetwo_Target);
	InvMass_GammaCombinations_onetwo_Target = new TH1D("InvMass_GammaCombinations_onetwo_Target","InvMass_GammaCombinations_onetwo_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Target.push_back(InvMass_GammaCombinations_onetwo_Target);
	Particles_PDG_onetwo_Target = new TH1D("Particles_PDG_onetwo_Target","Particles_PDG_onetwo_Target ; PDG", 2300, 0, 2300);
	fHistoList_Eta_onetwo_Target.push_back(Particles_PDG_onetwo_Target);
	TrueEta_pt_vs_rap_onetwo_Target = new TH2D("TrueEta_pt_vs_rap_onetwo_Target","TrueEta_pt_vs_rap_onetwo_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Target.push_back(TrueEta_pt_vs_rap_onetwo_Target);
	WrongEta_pt_vs_rap_onetwo_Target = new TH2D("WrongEta_pt_vs_rap_onetwo_Target","WrongEta_pt_vs_rap_onetwo_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Target.push_back(WrongEta_pt_vs_rap_onetwo_Target);
	TrueEta_pt_vs_rap_est_onetwo_Target = new TH2D("TrueEta_pt_vs_rap_est_onetwo_Target","TrueEta_pt_vs_rap_est_onetwo_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Target.push_back(TrueEta_pt_vs_rap_est_onetwo_Target);
	WrongEta_pt_vs_rap_est_onetwo_Target = new TH2D("WrongEta_pt_vs_rap_est_onetwo_Target","WrongEta_pt_vs_rap_est_onetwo_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Target.push_back(WrongEta_pt_vs_rap_est_onetwo_Target);
	TrueEta_InvMass_after_cuts_onetwo_Target = new TH1D("TrueEta_InvMass_after_cuts_onetwo_Target","TrueEta_InvMass_after_cuts_onetwo_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Target.push_back(TrueEta_InvMass_after_cuts_onetwo_Target);
	WrongEta_InvMass_after_cuts_onetwo_Target = new TH1D("WrongEta_InvMass_after_cuts_onetwo_Target","WrongEta_InvMass_after_cuts_onetwo_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Target.push_back(WrongEta_InvMass_after_cuts_onetwo_Target);
	TrueEta_AngleBetweenGammas_after_cuts_onetwo_Target = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_onetwo_Target","TrueEta_AngleBetweenGammas_after_cuts_onetwo_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Target.push_back(TrueEta_AngleBetweenGammas_after_cuts_onetwo_Target);
	WrongEta_AngleBetweenGammas_after_cuts_onetwo_Target = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_onetwo_Target","WrongEta_AngleBetweenGammas_after_cuts_onetwo_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Target.push_back(WrongEta_AngleBetweenGammas_after_cuts_onetwo_Target);
	EMT_Eta_InvMass_onetwo_Target = new TH1D("EMT_Eta_InvMass_onetwo_Target","EMT_Eta_InvMass_onetwo_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Target.push_back(EMT_Eta_InvMass_onetwo_Target);
	//////// eta -> g + g    reconstruction case "two" and "Target"
	AngleBetweenGammas_trueEta_before_cuts_two_Target = new TH1D("AngleBetweenGammas_trueEta_before_cuts_two_Target","AngleBetweenGammas_trueEta_before_cuts_two_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Target.push_back(AngleBetweenGammas_trueEta_before_cuts_two_Target);
	AngleBetweenGammas_wrongEta_before_cuts_two_Target = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_two_Target","AngleBetweenGammas_wrongEta_before_cuts_two_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Target.push_back(AngleBetweenGammas_wrongEta_before_cuts_two_Target);
	InvMass_GammaCombinations_two_Target = new TH1D("InvMass_GammaCombinations_two_Target","InvMass_GammaCombinations_two_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_two_Target = new TH2D("TrueEta_pt_vs_rap_two_Target","TrueEta_pt_vs_rap_two_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Target.push_back(TrueEta_pt_vs_rap_two_Target);
	WrongEta_pt_vs_rap_two_Target = new TH2D("WrongEta_pt_vs_rap_two_Target","WrongEta_pt_vs_rap_two_Target; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Target.push_back(WrongEta_pt_vs_rap_two_Target);
	TrueEta_pt_vs_rap_est_two_Target = new TH2D("TrueEta_pt_vs_rap_est_two_Target","TrueEta_pt_vs_rap_est_two_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Target.push_back(TrueEta_pt_vs_rap_est_two_Target);
	WrongEta_pt_vs_rap_est_two_Target = new TH2D("WrongEta_pt_vs_rap_est_two_Target","WrongEta_pt_vs_rap_est_two_Target; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Target.push_back(WrongEta_pt_vs_rap_est_two_Target);
	TrueEta_InvMass_after_cuts_two_Target = new TH1D("TrueEta_InvMass_after_cuts_two_Target","TrueEta_InvMass_after_cuts_two_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Target.push_back(TrueEta_InvMass_after_cuts_two_Target);
	WrongEta_InvMass_after_cuts_two_Target = new TH1D("WrongEta_InvMass_after_cuts_two_Target","WrongEta_InvMass_after_cuts_two_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Target.push_back(WrongEta_InvMass_after_cuts_two_Target);
	TrueEta_AngleBetweenGammas_after_cuts_two_Target = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_two_Target","TrueEta_AngleBetweenGammas_after_cuts_two_Target (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Target.push_back(TrueEta_AngleBetweenGammas_after_cuts_two_Target);
	WrongEta_AngleBetweenGammas_after_cuts_two_Target = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_two_Target","WrongEta_AngleBetweenGammas_after_cuts_two_Target (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Target.push_back(WrongEta_AngleBetweenGammas_after_cuts_two_Target);
	EMT_Eta_InvMass_two_Target = new TH1D("EMT_Eta_InvMass_two_Target","EMT_Eta_InvMass_two_Target; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	AngleBetweenGammas_trueEta_before_cuts_all_Outside = new TH1D("AngleBetweenGammas_trueEta_before_cuts_all_Outside","AngleBetweenGammas_trueEta_before_cuts_all_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Outside.push_back(AngleBetweenGammas_trueEta_before_cuts_all_Outside);
	AngleBetweenGammas_wrongEta_before_cuts_all_Outside = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_all_Outside","AngleBetweenGammas_wrongEta_before_cuts_all_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Outside.push_back(AngleBetweenGammas_wrongEta_before_cuts_all_Outside);
	InvMass_GammaCombinations_all_Outside = new TH1D("InvMass_GammaCombinations_all_Outside","InvMass_GammaCombinations_all_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_all_Outside = new TH2D("TrueEta_pt_vs_rap_all_Outside","TrueEta_pt_vs_rap_all_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Outside.push_back(TrueEta_pt_vs_rap_all_Outside);
	WrongEta_pt_vs_rap_all_Outside = new TH2D("WrongEta_pt_vs_rap_all_Outside","WrongEta_pt_vs_rap_all_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Outside.push_back(WrongEta_pt_vs_rap_all_Outside);
	TrueEta_pt_vs_rap_est_all_Outside = new TH2D("TrueEta_pt_vs_rap_est_all_Outside","TrueEta_pt_vs_rap_est_all_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Outside.push_back(TrueEta_pt_vs_rap_est_all_Outside);
	WrongEta_pt_vs_rap_est_all_Outside = new TH2D("WrongEta_pt_vs_rap_est_all_Outside","WrongEta_pt_vs_rap_est_all_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Outside.push_back(WrongEta_pt_vs_rap_est_all_Outside);
	TrueEta_InvMass_after_cuts_all_Outside = new TH1D("TrueEta_InvMass_after_cuts_all_Outside","TrueEta_InvMass_after_cuts_all_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Outside.push_back(TrueEta_InvMass_after_cuts_all_Outside);
	WrongEta_InvMass_after_cuts_all_Outside = new TH1D("WrongEta_InvMass_after_cuts_all_Outside","WrongEta_InvMass_after_cuts_all_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Outside.push_back(WrongEta_InvMass_after_cuts_all_Outside);
	TrueEta_AngleBetweenGammas_after_cuts_all_Outside = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_all_Outside","TrueEta_AngleBetweenGammas_after_cuts_all_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Outside.push_back(TrueEta_AngleBetweenGammas_after_cuts_all_Outside);
	WrongEta_AngleBetweenGammas_after_cuts_all_Outside = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_all_Outside","WrongEta_AngleBetweenGammas_after_cuts_all_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Outside.push_back(WrongEta_AngleBetweenGammas_after_cuts_all_Outside);
	EMT_Eta_InvMass_all_Outside = new TH1D("EMT_Eta_InvMass_all_Outside","EMT_Eta_InvMass_all_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Outside.push_back(EMT_Eta_InvMass_all_Outside);
	//////// eta -> g + g    reconstruction case "onetwo" and "Outside"
	AngleBetweenGammas_trueEta_before_cuts_onetwo_Outside = new TH1D("AngleBetweenGammas_trueEta_before_cuts_onetwo_Outside","AngleBetweenGammas_trueEta_before_cuts_onetwo_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Outside.push_back(AngleBetweenGammas_trueEta_before_cuts_onetwo_Outside);
	AngleBetweenGammas_wrongEta_before_cuts_onetwo_Outside = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_onetwo_Outside","AngleBetweenGammas_wrongEta_before_cuts_onetwo_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Outside.push_back(AngleBetweenGammas_wrongEta_before_cuts_onetwo_Outside);
	InvMass_GammaCombinations_onetwo_Outside = new TH1D("InvMass_GammaCombinations_onetwo_Outside","InvMass_GammaCombinations_onetwo_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Outside.push_back(InvMass_GammaCombinations_onetwo_Outside);
	Particles_PDG_onetwo_Outside = new TH1D("Particles_PDG_onetwo_Outside","Particles_PDG_onetwo_Outside ; PDG", 2300, 0, 2300);
	fHistoList_Eta_onetwo_Outside.push_back(Particles_PDG_onetwo_Outside);
	TrueEta_pt_vs_rap_onetwo_Outside = new TH2D("TrueEta_pt_vs_rap_onetwo_Outside","TrueEta_pt_vs_rap_onetwo_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Outside.push_back(TrueEta_pt_vs_rap_onetwo_Outside);
	WrongEta_pt_vs_rap_onetwo_Outside = new TH2D("WrongEta_pt_vs_rap_onetwo_Outside","WrongEta_pt_vs_rap_onetwo_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Outside.push_back(WrongEta_pt_vs_rap_onetwo_Outside);
	TrueEta_pt_vs_rap_est_onetwo_Outside = new TH2D("TrueEta_pt_vs_rap_est_onetwo_Outside","TrueEta_pt_vs_rap_est_onetwo_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Outside.push_back(TrueEta_pt_vs_rap_est_onetwo_Outside);
	WrongEta_pt_vs_rap_est_onetwo_Outside = new TH2D("WrongEta_pt_vs_rap_est_onetwo_Outside","WrongEta_pt_vs_rap_est_onetwo_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Outside.push_back(WrongEta_pt_vs_rap_est_onetwo_Outside);
	TrueEta_InvMass_after_cuts_onetwo_Outside = new TH1D("TrueEta_InvMass_after_cuts_onetwo_Outside","TrueEta_InvMass_after_cuts_onetwo_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Outside.push_back(TrueEta_InvMass_after_cuts_onetwo_Outside);
	WrongEta_InvMass_after_cuts_onetwo_Outside = new TH1D("WrongEta_InvMass_after_cuts_onetwo_Outside","WrongEta_InvMass_after_cuts_onetwo_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Outside.push_back(WrongEta_InvMass_after_cuts_onetwo_Outside);
	TrueEta_AngleBetweenGammas_after_cuts_onetwo_Outside = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_onetwo_Outside","TrueEta_AngleBetweenGammas_after_cuts_onetwo_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Outside.push_back(TrueEta_AngleBetweenGammas_after_cuts_onetwo_Outside);
	WrongEta_AngleBetweenGammas_after_cuts_onetwo_Outside = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_onetwo_Outside","WrongEta_AngleBetweenGammas_after_cuts_onetwo_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Outside.push_back(WrongEta_AngleBetweenGammas_after_cuts_onetwo_Outside);
	EMT_Eta_InvMass_onetwo_Outside = new TH1D("EMT_Eta_InvMass_onetwo_Outside","EMT_Eta_InvMass_onetwo_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Outside.push_back(EMT_Eta_InvMass_onetwo_Outside);
	//////// eta -> g + g    reconstruction case "two" and "Outside"
	AngleBetweenGammas_trueEta_before_cuts_two_Outside = new TH1D("AngleBetweenGammas_trueEta_before_cuts_two_Outside","AngleBetweenGammas_trueEta_before_cuts_two_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Outside.push_back(AngleBetweenGammas_trueEta_before_cuts_two_Outside);
	AngleBetweenGammas_wrongEta_before_cuts_two_Outside = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_two_Outside","AngleBetweenGammas_wrongEta_before_cuts_two_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Outside.push_back(AngleBetweenGammas_wrongEta_before_cuts_two_Outside);
	InvMass_GammaCombinations_two_Outside = new TH1D("InvMass_GammaCombinations_two_Outside","InvMass_GammaCombinations_two_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_two_Outside = new TH2D("TrueEta_pt_vs_rap_two_Outside","TrueEta_pt_vs_rap_two_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Outside.push_back(TrueEta_pt_vs_rap_two_Outside);
	WrongEta_pt_vs_rap_two_Outside = new TH2D("WrongEta_pt_vs_rap_two_Outside","WrongEta_pt_vs_rap_two_Outside; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Outside.push_back(WrongEta_pt_vs_rap_two_Outside);
	TrueEta_pt_vs_rap_est_two_Outside = new TH2D("TrueEta_pt_vs_rap_est_two_Outside","TrueEta_pt_vs_rap_est_two_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Outside.push_back(TrueEta_pt_vs_rap_est_two_Outside);
	WrongEta_pt_vs_rap_est_two_Outside = new TH2D("WrongEta_pt_vs_rap_est_two_Outside","WrongEta_pt_vs_rap_est_two_Outside; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Outside.push_back(WrongEta_pt_vs_rap_est_two_Outside);
	TrueEta_InvMass_after_cuts_two_Outside = new TH1D("TrueEta_InvMass_after_cuts_two_Outside","TrueEta_InvMass_after_cuts_two_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Outside.push_back(TrueEta_InvMass_after_cuts_two_Outside);
	WrongEta_InvMass_after_cuts_two_Outside = new TH1D("WrongEta_InvMass_after_cuts_two_Outside","WrongEta_InvMass_after_cuts_two_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Outside.push_back(WrongEta_InvMass_after_cuts_two_Outside);
	TrueEta_AngleBetweenGammas_after_cuts_two_Outside = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_two_Outside","TrueEta_AngleBetweenGammas_after_cuts_two_Outside (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Outside.push_back(TrueEta_AngleBetweenGammas_after_cuts_two_Outside);
	WrongEta_AngleBetweenGammas_after_cuts_two_Outside = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_two_Outside","WrongEta_AngleBetweenGammas_after_cuts_two_Outside (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Outside.push_back(WrongEta_AngleBetweenGammas_after_cuts_two_Outside);
	EMT_Eta_InvMass_two_Outside = new TH1D("EMT_Eta_InvMass_two_Outside","EMT_Eta_InvMass_two_Outside; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	AngleBetweenGammas_trueEta_before_cuts_all_Both = new TH1D("AngleBetweenGammas_trueEta_before_cuts_all_Both","AngleBetweenGammas_trueEta_before_cuts_all_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Both.push_back(AngleBetweenGammas_trueEta_before_cuts_all_Both);
	AngleBetweenGammas_wrongEta_before_cuts_all_Both = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_all_Both","AngleBetweenGammas_wrongEta_before_cuts_all_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Both.push_back(AngleBetweenGammas_wrongEta_before_cuts_all_Both);
	InvMass_GammaCombinations_all_Both = new TH1D("InvMass_GammaCombinations_all_Both","InvMass_GammaCombinations_all_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_all_Both = new TH2D("TrueEta_pt_vs_rap_all_Both","TrueEta_pt_vs_rap_all_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Both.push_back(TrueEta_pt_vs_rap_all_Both);
	WrongEta_pt_vs_rap_all_Both = new TH2D("WrongEta_pt_vs_rap_all_Both","WrongEta_pt_vs_rap_all_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_all_Both.push_back(WrongEta_pt_vs_rap_all_Both);
	TrueEta_pt_vs_rap_est_all_Both = new TH2D("TrueEta_pt_vs_rap_est_all_Both","TrueEta_pt_vs_rap_est_all_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Both.push_back(TrueEta_pt_vs_rap_est_all_Both);
	WrongEta_pt_vs_rap_est_all_Both = new TH2D("WrongEta_pt_vs_rap_est_all_Both","WrongEta_pt_vs_rap_est_all_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_all_Both.push_back(WrongEta_pt_vs_rap_est_all_Both);
	TrueEta_InvMass_after_cuts_all_Both = new TH1D("TrueEta_InvMass_after_cuts_all_Both","TrueEta_InvMass_after_cuts_all_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Both.push_back(TrueEta_InvMass_after_cuts_all_Both);
	WrongEta_InvMass_after_cuts_all_Both = new TH1D("WrongEta_InvMass_after_cuts_all_Both","WrongEta_InvMass_after_cuts_all_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_all_Both.push_back(WrongEta_InvMass_after_cuts_all_Both);
	TrueEta_AngleBetweenGammas_after_cuts_all_Both = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_all_Both","TrueEta_AngleBetweenGammas_after_cuts_all_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Both.push_back(TrueEta_AngleBetweenGammas_after_cuts_all_Both);
	WrongEta_AngleBetweenGammas_after_cuts_all_Both = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_all_Both","WrongEta_AngleBetweenGammas_after_cuts_all_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_all_Both.push_back(WrongEta_AngleBetweenGammas_after_cuts_all_Both);
	EMT_Eta_InvMass_all_Both = new TH1D("EMT_Eta_InvMass_all_Both","EMT_Eta_InvMass_all_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	multi_InvMass_Eta_all_Both_1 = new TH1D("multi_InvMass_Eta_all_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_1);
	multi_InvMass_Eta_all_Both_2 = new TH1D("multi_InvMass_Eta_all_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_2);
	multi_InvMass_Eta_all_Both_3 = new TH1D("multi_InvMass_Eta_all_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_3);
	multi_InvMass_Eta_all_Both_4 = new TH1D("multi_InvMass_Eta_all_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_4);
	multi_InvMass_Eta_all_Both_5 = new TH1D("multi_InvMass_Eta_all_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_5);
	multi_InvMass_Eta_all_Both_6 = new TH1D("multi_InvMass_Eta_all_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_6);
	multi_InvMass_Eta_all_Both_7 = new TH1D("multi_InvMass_Eta_all_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_7);
	multi_InvMass_Eta_all_Both_8 = new TH1D("multi_InvMass_Eta_all_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_8);
	multi_InvMass_Eta_all_Both_9 = new TH1D("multi_InvMass_Eta_all_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_9);
	multi_InvMass_Eta_all_Both_10 = new TH1D("multi_InvMass_Eta_all_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_10);
	multi_InvMass_Eta_all_Both_11 = new TH1D("multi_InvMass_Eta_all_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_11);
	multi_InvMass_Eta_all_Both_12 = new TH1D("multi_InvMass_Eta_all_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_12);
	multi_InvMass_Eta_all_Both_13 = new TH1D("multi_InvMass_Eta_all_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_13);
	multi_InvMass_Eta_all_Both_14 = new TH1D("multi_InvMass_Eta_all_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_14);
	multi_InvMass_Eta_all_Both_15 = new TH1D("multi_InvMass_Eta_all_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_15);
	multi_InvMass_Eta_all_Both_16 = new TH1D("multi_InvMass_Eta_all_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_16);
	multi_InvMass_Eta_all_Both_17 = new TH1D("multi_InvMass_Eta_all_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_InvMass_Eta_all_Both_17);
	multi_EMT_Eta_all_Both_1 = new TH1D("multi_EMT_Eta_all_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_1);
	multi_EMT_Eta_all_Both_2 = new TH1D("multi_EMT_Eta_all_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_2);
	multi_EMT_Eta_all_Both_3 = new TH1D("multi_EMT_Eta_all_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_3);
	multi_EMT_Eta_all_Both_4 = new TH1D("multi_EMT_Eta_all_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_4);
	multi_EMT_Eta_all_Both_5 = new TH1D("multi_EMT_Eta_all_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_5);
	multi_EMT_Eta_all_Both_6 = new TH1D("multi_EMT_Eta_all_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_6);
	multi_EMT_Eta_all_Both_7 = new TH1D("multi_EMT_Eta_all_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_7);
	multi_EMT_Eta_all_Both_8 = new TH1D("multi_EMT_Eta_all_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_8);
	multi_EMT_Eta_all_Both_9 = new TH1D("multi_EMT_Eta_all_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_9);
	multi_EMT_Eta_all_Both_10 = new TH1D("multi_EMT_Eta_all_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_10);
	multi_EMT_Eta_all_Both_11 = new TH1D("multi_EMT_Eta_all_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_11);
	multi_EMT_Eta_all_Both_12 = new TH1D("multi_EMT_Eta_all_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_12);
	multi_EMT_Eta_all_Both_13 = new TH1D("multi_EMT_Eta_all_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_13);
	multi_EMT_Eta_all_Both_14 = new TH1D("multi_EMT_Eta_all_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_14);
	multi_EMT_Eta_all_Both_15 = new TH1D("multi_EMT_Eta_all_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_15);
	multi_EMT_Eta_all_Both_16 = new TH1D("multi_EMT_Eta_all_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_16);
	multi_EMT_Eta_all_Both_17 = new TH1D("multi_EMT_Eta_all_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_all_Both.push_back(multi_EMT_Eta_all_Both_17);



	//////// eta -> g + g    reconstruction case "onetwo" and "Both"
	AngleBetweenGammas_trueEta_before_cuts_onetwo_Both = new TH1D("AngleBetweenGammas_trueEta_before_cuts_onetwo_Both","AngleBetweenGammas_trueEta_before_cuts_onetwo_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Both.push_back(AngleBetweenGammas_trueEta_before_cuts_onetwo_Both);
	AngleBetweenGammas_wrongEta_before_cuts_onetwo_Both = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_onetwo_Both","AngleBetweenGammas_wrongEta_before_cuts_onetwo_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Both.push_back(AngleBetweenGammas_wrongEta_before_cuts_onetwo_Both);
	InvMass_GammaCombinations_onetwo_Both = new TH1D("InvMass_GammaCombinations_onetwo_Both","InvMass_GammaCombinations_onetwo_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_onetwo_Both = new TH2D("TrueEta_pt_vs_rap_onetwo_Both","TrueEta_pt_vs_rap_onetwo_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Both.push_back(TrueEta_pt_vs_rap_onetwo_Both);
	WrongEta_pt_vs_rap_onetwo_Both = new TH2D("WrongEta_pt_vs_rap_onetwo_Both","WrongEta_pt_vs_rap_onetwo_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_onetwo_Both.push_back(WrongEta_pt_vs_rap_onetwo_Both);
	TrueEta_pt_vs_rap_est_onetwo_Both = new TH2D("TrueEta_pt_vs_rap_est_onetwo_Both","TrueEta_pt_vs_rap_est_onetwo_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Both.push_back(TrueEta_pt_vs_rap_est_onetwo_Both);
	WrongEta_pt_vs_rap_est_onetwo_Both = new TH2D("WrongEta_pt_vs_rap_est_onetwo_Both","WrongEta_pt_vs_rap_est_onetwo_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_onetwo_Both.push_back(WrongEta_pt_vs_rap_est_onetwo_Both);
	TrueEta_InvMass_after_cuts_onetwo_Both = new TH1D("TrueEta_InvMass_after_cuts_onetwo_Both","TrueEta_InvMass_after_cuts_onetwo_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Both.push_back(TrueEta_InvMass_after_cuts_onetwo_Both);
	WrongEta_InvMass_after_cuts_onetwo_Both = new TH1D("WrongEta_InvMass_after_cuts_onetwo_Both","WrongEta_InvMass_after_cuts_onetwo_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_onetwo_Both.push_back(WrongEta_InvMass_after_cuts_onetwo_Both);
	TrueEta_AngleBetweenGammas_after_cuts_onetwo_Both = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_onetwo_Both","TrueEta_AngleBetweenGammas_after_cuts_onetwo_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Both.push_back(TrueEta_AngleBetweenGammas_after_cuts_onetwo_Both);
	WrongEta_AngleBetweenGammas_after_cuts_onetwo_Both = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_onetwo_Both","WrongEta_AngleBetweenGammas_after_cuts_onetwo_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_onetwo_Both.push_back(WrongEta_AngleBetweenGammas_after_cuts_onetwo_Both);
	EMT_Eta_InvMass_onetwo_Both = new TH1D("EMT_Eta_InvMass_onetwo_Both","EMT_Eta_InvMass_onetwo_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	multi_InvMass_Eta_onetwo_Both_1 = new TH1D("multi_InvMass_Eta_onetwo_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_1);
	multi_InvMass_Eta_onetwo_Both_2 = new TH1D("multi_InvMass_Eta_onetwo_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_2);
	multi_InvMass_Eta_onetwo_Both_3 = new TH1D("multi_InvMass_Eta_onetwo_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_3);
	multi_InvMass_Eta_onetwo_Both_4 = new TH1D("multi_InvMass_Eta_onetwo_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_4);
	multi_InvMass_Eta_onetwo_Both_5 = new TH1D("multi_InvMass_Eta_onetwo_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_5);
	multi_InvMass_Eta_onetwo_Both_6 = new TH1D("multi_InvMass_Eta_onetwo_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_6);
	multi_InvMass_Eta_onetwo_Both_7 = new TH1D("multi_InvMass_Eta_onetwo_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_7);
	multi_InvMass_Eta_onetwo_Both_8 = new TH1D("multi_InvMass_Eta_onetwo_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_8);
	multi_InvMass_Eta_onetwo_Both_9 = new TH1D("multi_InvMass_Eta_onetwo_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_9);
	multi_InvMass_Eta_onetwo_Both_10 = new TH1D("multi_InvMass_Eta_onetwo_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_10);
	multi_InvMass_Eta_onetwo_Both_11 = new TH1D("multi_InvMass_Eta_onetwo_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_11);
	multi_InvMass_Eta_onetwo_Both_12 = new TH1D("multi_InvMass_Eta_onetwo_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_12);
	multi_InvMass_Eta_onetwo_Both_13 = new TH1D("multi_InvMass_Eta_onetwo_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_13);
	multi_InvMass_Eta_onetwo_Both_14 = new TH1D("multi_InvMass_Eta_onetwo_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_14);
	multi_InvMass_Eta_onetwo_Both_15 = new TH1D("multi_InvMass_Eta_onetwo_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_15);
	multi_InvMass_Eta_onetwo_Both_16 = new TH1D("multi_InvMass_Eta_onetwo_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_16);
	multi_InvMass_Eta_onetwo_Both_17 = new TH1D("multi_InvMass_Eta_onetwo_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_InvMass_Eta_onetwo_Both_17);
	multi_EMT_Eta_onetwo_Both_1 = new TH1D("multi_EMT_Eta_onetwo_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_1);
	multi_EMT_Eta_onetwo_Both_2 = new TH1D("multi_EMT_Eta_onetwo_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_2);
	multi_EMT_Eta_onetwo_Both_3 = new TH1D("multi_EMT_Eta_onetwo_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_3);
	multi_EMT_Eta_onetwo_Both_4 = new TH1D("multi_EMT_Eta_onetwo_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_4);
	multi_EMT_Eta_onetwo_Both_5 = new TH1D("multi_EMT_Eta_onetwo_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_5);
	multi_EMT_Eta_onetwo_Both_6 = new TH1D("multi_EMT_Eta_onetwo_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_6);
	multi_EMT_Eta_onetwo_Both_7 = new TH1D("multi_EMT_Eta_onetwo_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_7);
	multi_EMT_Eta_onetwo_Both_8 = new TH1D("multi_EMT_Eta_onetwo_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_8);
	multi_EMT_Eta_onetwo_Both_9 = new TH1D("multi_EMT_Eta_onetwo_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_9);
	multi_EMT_Eta_onetwo_Both_10 = new TH1D("multi_EMT_Eta_onetwo_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_10);
	multi_EMT_Eta_onetwo_Both_11 = new TH1D("multi_EMT_Eta_onetwo_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_11);
	multi_EMT_Eta_onetwo_Both_12 = new TH1D("multi_EMT_Eta_onetwo_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_12);
	multi_EMT_Eta_onetwo_Both_13 = new TH1D("multi_EMT_Eta_onetwo_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_13);
	multi_EMT_Eta_onetwo_Both_14 = new TH1D("multi_EMT_Eta_onetwo_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_14);
	multi_EMT_Eta_onetwo_Both_15 = new TH1D("multi_EMT_Eta_onetwo_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_15);
	multi_EMT_Eta_onetwo_Both_16 = new TH1D("multi_EMT_Eta_onetwo_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_16);
	multi_EMT_Eta_onetwo_Both_17 = new TH1D("multi_EMT_Eta_onetwo_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_onetwo_Both.push_back(multi_EMT_Eta_onetwo_Both_17);



	//////// eta -> g + g    reconstruction case "two" and "Both"
	AngleBetweenGammas_trueEta_before_cuts_two_Both = new TH1D("AngleBetweenGammas_trueEta_before_cuts_two_Both","AngleBetweenGammas_trueEta_before_cuts_two_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Both.push_back(AngleBetweenGammas_trueEta_before_cuts_two_Both);
	AngleBetweenGammas_wrongEta_before_cuts_two_Both = new TH1D("AngleBetweenGammas_wrongEta_before_cuts_two_Both","AngleBetweenGammas_wrongEta_before_cuts_two_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Both.push_back(AngleBetweenGammas_wrongEta_before_cuts_two_Both);
	InvMass_GammaCombinations_two_Both = new TH1D("InvMass_GammaCombinations_two_Both","InvMass_GammaCombinations_two_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	TrueEta_pt_vs_rap_two_Both = new TH2D("TrueEta_pt_vs_rap_two_Both","TrueEta_pt_vs_rap_two_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Both.push_back(TrueEta_pt_vs_rap_two_Both);
	WrongEta_pt_vs_rap_two_Both = new TH2D("WrongEta_pt_vs_rap_two_Both","WrongEta_pt_vs_rap_two_Both; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_Eta_two_Both.push_back(WrongEta_pt_vs_rap_two_Both);
	TrueEta_pt_vs_rap_est_two_Both = new TH2D("TrueEta_pt_vs_rap_est_two_Both","TrueEta_pt_vs_rap_est_two_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Both.push_back(TrueEta_pt_vs_rap_est_two_Both);
	WrongEta_pt_vs_rap_est_two_Both = new TH2D("WrongEta_pt_vs_rap_est_two_Both","WrongEta_pt_vs_rap_est_two_Both; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 5, 0., 2.);
	fHistoList_Eta_two_Both.push_back(WrongEta_pt_vs_rap_est_two_Both);
	TrueEta_InvMass_after_cuts_two_Both = new TH1D("TrueEta_InvMass_after_cuts_two_Both","TrueEta_InvMass_after_cuts_two_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Both.push_back(TrueEta_InvMass_after_cuts_two_Both);
	WrongEta_InvMass_after_cuts_two_Both = new TH1D("WrongEta_InvMass_after_cuts_two_Both","WrongEta_InvMass_after_cuts_two_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_Eta_two_Both.push_back(WrongEta_InvMass_after_cuts_two_Both);
	TrueEta_AngleBetweenGammas_after_cuts_two_Both = new TH1D("TrueEta_AngleBetweenGammas_after_cuts_two_Both","TrueEta_AngleBetweenGammas_after_cuts_two_Both (between #gamma#gamma from #eta); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Both.push_back(TrueEta_AngleBetweenGammas_after_cuts_two_Both);
	WrongEta_AngleBetweenGammas_after_cuts_two_Both = new TH1D("WrongEta_AngleBetweenGammas_after_cuts_two_Both","WrongEta_AngleBetweenGammas_after_cuts_two_Both (between #gamma#gamma); angle [deg]", 900, -0.1, 89.9);
	fHistoList_Eta_two_Both.push_back(WrongEta_AngleBetweenGammas_after_cuts_two_Both);
	EMT_Eta_InvMass_two_Both = new TH1D("EMT_Eta_InvMass_two_Both","EMT_Eta_InvMass_two_Both; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	...
	multi_InvMass_Eta_two_Both_1 = new TH1D("multi_InvMass_Eta_two_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_1);
	multi_InvMass_Eta_two_Both_2 = new TH1D("multi_InvMass_Eta_two_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_2);
	multi_InvMass_Eta_two_Both_3 = new TH1D("multi_InvMass_Eta_two_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_3);
	multi_InvMass_Eta_two_Both_4 = new TH1D("multi_InvMass_Eta_two_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_4);
	multi_InvMass_Eta_two_Both_5 = new TH1D("multi_InvMass_Eta_two_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_5);
	multi_InvMass_Eta_two_Both_6 = new TH1D("multi_InvMass_Eta_two_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_6);
	multi_InvMass_Eta_two_Both_7 = new TH1D("multi_InvMass_Eta_two_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_7);
	multi_InvMass_Eta_two_Both_8 = new TH1D("multi_InvMass_Eta_two_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_8);
	multi_InvMass_Eta_two_Both_9 = new TH1D("multi_InvMass_Eta_two_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_9);
	multi_InvMass_Eta_two_Both_10 = new TH1D("multi_InvMass_Eta_two_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_10);
	multi_InvMass_Eta_two_Both_11 = new TH1D("multi_InvMass_Eta_two_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_11);
	multi_InvMass_Eta_two_Both_12 = new TH1D("multi_InvMass_Eta_two_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_12);
	multi_InvMass_Eta_two_Both_13 = new TH1D("multi_InvMass_Eta_two_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_13);
	multi_InvMass_Eta_two_Both_14 = new TH1D("multi_InvMass_Eta_two_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_14);
	multi_InvMass_Eta_two_Both_15 = new TH1D("multi_InvMass_Eta_two_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_15);
	multi_InvMass_Eta_two_Both_16 = new TH1D("multi_InvMass_Eta_two_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_16);
	multi_InvMass_Eta_two_Both_17 = new TH1D("multi_InvMass_Eta_two_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_InvMass_Eta_two_Both_17);
	multi_EMT_Eta_two_Both_1 = new TH1D("multi_EMT_Eta_two_Both_1","rapidity = (1.2-1.6)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_1);
	multi_EMT_Eta_two_Both_2 = new TH1D("multi_EMT_Eta_two_Both_2","rapidity = (1.2-1.6)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_2);
	multi_EMT_Eta_two_Both_3 = new TH1D("multi_EMT_Eta_two_Both_3","rapidity = (1.2-1.6)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_3);
	multi_EMT_Eta_two_Both_4 = new TH1D("multi_EMT_Eta_two_Both_4","rapidity = (1.2-1.6)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_4);
	multi_EMT_Eta_two_Both_5 = new TH1D("multi_EMT_Eta_two_Both_5","rapidity = (1.6-2.0)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_5);
	multi_EMT_Eta_two_Both_6 = new TH1D("multi_EMT_Eta_two_Both_6","rapidity = (1.6-2.0)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_6);
	multi_EMT_Eta_two_Both_7 = new TH1D("multi_EMT_Eta_two_Both_7","rapidity = (1.6-2.0)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_7);
	multi_EMT_Eta_two_Both_8 = new TH1D("multi_EMT_Eta_two_Both_8","rapidity = (1.6-2.0)      P_{t} = (1.2-2.0 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_8);
	multi_EMT_Eta_two_Both_9 = new TH1D("multi_EMT_Eta_two_Both_9","rapidity = (2.0-2.4)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_9);
	multi_EMT_Eta_two_Both_10 = new TH1D("multi_EMT_Eta_two_Both_10","rapidity = (2.0-2.4)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_10);
	multi_EMT_Eta_two_Both_11 = new TH1D("multi_EMT_Eta_two_Both_11","rapidity = (2.0-2.4)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_11);
	multi_EMT_Eta_two_Both_12 = new TH1D("multi_EMT_Eta_two_Both_12","rapidity = (2.0-2.4)      P_{t} = (1.2-1.6 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_12);
	multi_EMT_Eta_two_Both_13 = new TH1D("multi_EMT_Eta_two_Both_13","rapidity = (2.4-2.8)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_13);
	multi_EMT_Eta_two_Both_14 = new TH1D("multi_EMT_Eta_two_Both_14","rapidity = (2.4-2.8)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_14);
	multi_EMT_Eta_two_Both_15 = new TH1D("multi_EMT_Eta_two_Both_15","rapidity = (2.4-2.8)      P_{t} = (0.8-1.2 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_15);
	multi_EMT_Eta_two_Both_16 = new TH1D("multi_EMT_Eta_two_Both_16","rapidity = (2.8-3.2)      P_{t} = (0.0-0.4 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
	fHistoList_rap_pt_Eta_two_Both.push_back(multi_EMT_Eta_two_Both_16);
	multi_EMT_Eta_two_Both_17 = new TH1D("multi_EMT_Eta_two_Both_17","rapidity = (2.8-3.2)      P_{t} = (0.4-0.8 GeV/c^{2}) ; invariant mass in GeV/c^{2};#", 200, 0.0, 2.0);
