...
void CbmKresConversionGeneral::Exec(int /*fEventNumGen*/)
{
	...
	for ( Int_t iTr = 0; iTr < ngTracks; iTr++ ){
		...
		CbmTrackMatchNew * stsMatch = (CbmTrackMatchNew*) fStsTrackMatches->At(stsInd);			/// match of reconstructed track to Monte Carlo
		...
		CbmRichRing * richRing = (CbmRichRing*) fRichRings->At(richInd);
		if ( richRing == nullptr ) continue;
		CbmTrackMatchNew * richMatch = (CbmTrackMatchNew*)fRichRingMatches->At(richInd);			/// match of reconstructed ring to Monte Carlo
		if ( richMatch == nullptr ) continue;
		if ( richMatch->GetNofLinks() <= 0 ) continue;
		int richMcTrackId = richMatch->GetMatchedLink().GetIndex();
		if ( richMcTrackId < 0 ) continue;
		...
		if ( ringHit.GetAaxis() > fMinAaxis && ringHit.GetAaxis() < fMaxAaxis && ringHit.GetBaxis() > fMinBaxis && ringHit.GetAaxis() < fMaxBaxis ){				//// only good rings
			if ( mcTrackRICH->GetPdgCode() == -11 ) Distance_positron->Fill(richRing->GetCenterX(), richRing->GetCenterY(), CbmRichUtil::GetRingTrackDistance(iTr));
			if ( mcTrackRICH->GetPdgCode() == 11 ) Distance_electron->Fill(richRing->GetCenterX(), richRing->GetCenterY(), CbmRichUtil::GetRingTrackDistance(iTr));
			...
			for ( int iH = 0; iH < ringHit.GetNofHits(); iH++ ){
				...
				Double_t sign = sqrt((xmin-xc)*(xmin-xc)+(ymin-yc)*(ymin-yc)) - sqrt((xh-xc)*(xh-xc)+(yh-yc)*(yh-yc));
				double dr = Lmin;
				if ( sign < 0 ){
					dr = -Lmin;
				}
...
{
	//***** photons analysis 
	MC_All_photons_Pt = new TH1D ("MC_All_photons_Pt", "Monte Carlo,   all #gamma,   p_{t} distribution; p_{t} in GeV/c", 200, 0, 2);
	fHistoList_MC.push_back(MC_All_photons_Pt);

	MC_Not_Direct_photons_Pt = new TH1D ("MC_Not_Direct_photons_Pt", "Monte Carlo,   #gamma from decays,   p_{t} distribution; p_{t} in GeV/c", 200, 0, 2);
	fHistoList_MC.push_back(MC_Not_Direct_photons_Pt);

	MC_Direct_photons_Pt = new TH1D ("MC_Direct_photons_Pt", "Monte Carlo,   direct #gamma,   p_{t} distribution; p_{t} in GeV/c", 200, 0, 2);
	fHistoList_MC.push_back(MC_Direct_photons_Pt);

	MC_All_photons_P = new TH1D ("MC_All_photons_P", "Monte Carlo,   all #gamma,   p distribution; p in GeV/c", 1000, 0, 10);
	fHistoList_MC.push_back(MC_All_photons_P);

	MC_Not_Direct_photons_P = new TH1D ("MC_Not_Direct_photons_P", "Monte Carlo,   #gamma from decays,   p distribution; p in GeV/c", 1000, 0, 10);
	fHistoList_MC.push_back(MC_Not_Direct_photons_P);

	MC_Direct_photons_P = new TH1D ("MC_Direct_photons_P", "Monte Carlo,   direct #gamma,   p distribution; p in GeV/c", 1000, 0, 10);
	fHistoList_MC.push_back(MC_Direct_photons_P);

	MC_Direct_photons_Pt_vs_rap = new TH2D("MC_Direct_photons_Pt_vs_rap", "Monte Carlo,   direct #gamma,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 90, -2., 7.,  60, -1., 5.);
	fHistoList_MC.push_back(MC_Direct_photons_Pt_vs_rap);

	MC_Direct_photons_Pt_vs_rap_est = new TH2D("MC_Direct_photons_Pt_vs_rap_est", "Monte Carlo,   direct #gamma,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	...
	MC_Not_Direct_photons_theta = new TH1D("MC_Not_Direct_photons_theta", "Monte Carlo,   #gamma from decays,   #theta distribution; emission angle #theta in deg;Entries", 180., 0., 180.);
	fHistoList_MC.push_back(MC_Not_Direct_photons_theta);

	MC_Not_Direct_photons_theta_vs_rap = new TH2D("MC_Not_Direct_photons_theta_vs_rap", "Monte Carlo,   #gamma from decays,   #theta vs rapidity distribution; emission angle #theta in deg; rapidity y", 180., 0., 180., 270, -2., 7.);
	fHistoList_MC.push_back(MC_Not_Direct_photons_theta_vs_rap);

	MC_Direct_photons_theta = new TH1D("MC_Direct_photons_theta", "Monte Carlo,   direct #gamma,   #theta distribution; emission angle #theta in deg;Entries", 180., 0., 180.);
	fHistoList_MC.push_back(MC_Direct_photons_theta);

	MC_Direct_photons_theta_vs_rap = new TH2D("MC_Direct_photons_theta_vs_rap", "Monte Carlo,   direct #gamma,   #theta vs rapidity distribution; emission angle #theta in deg; rapidity y", 180., 0., 180., 270, -2., 7.);
	fHistoList_MC.push_back(MC_Direct_photons_theta_vs_rap);

	MC_electrons_Pt_vs_rap_est = new TH2D("MC_electrons_Pt_vs_rap_est", "Monte Carlo,   direct e,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_MC.push_back(MC_electrons_Pt_vs_rap_est);

	MC_Reconstructed_electrons_Pt_vs_rap_est = new TH2D("MC_Reconstructed_electrons_Pt_vs_rap_est", "Monte Carlo,   reconstructed e,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_MC.push_back(MC_Reconstructed_electrons_Pt_vs_rap_est);

	MC_omega_Pt_vs_rap_est = new TH2D("MC_omega_Pt_vs_rap_est", "Monte Carlo,   #omega,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_MC.push_back(MC_omega_Pt_vs_rap_est);



	//***** pi0 analysis
	MC_pi0_Pt = new TH1D("MC_pi0_Pt", "Monte Carlo,   #pi^{0},   p_{t} distribution;p_{t} in GeV/c; Entries", 200., 0., 10.);
	fHistoList_MC.push_back(MC_pi0_Pt);

	MC_pi0_Pt_est = new TH1D("MC_pi0_Pt_est", "Monte Carlo,   #pi^{0},   p_{t} distribution;p_{t} in GeV/c; Entries", 20., 0., 2.);
	fHistoList_MC.push_back(MC_pi0_Pt_est);

	MC_pi0_Pt_vs_rap = new TH2D("MC_pi0_Pt_vs_rap", "Monte Carlo,   #pi^{0},   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_MC.push_back(MC_pi0_Pt_vs_rap);

	MC_pi0_Pt_vs_rap_primary = new TH2D("MC_pi0_Pt_vs_rap_primary", "Monte Carlo,   primary #pi^{0},   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 90, -2., 7.,  60, -1., 5.);
	fHistoList_MC.push_back(MC_pi0_Pt_vs_rap_primary);

	Pi0_pt_vs_rap_est = new TH2D("Pi0_pt_vs_rap_est", "Pi0_pt_vs_rap_est; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_MC.push_back(Pi0_pt_vs_rap_est); 

	Pi0_pt_vs_rap_est_primary = new TH2D("Pi0_pt_vs_rap_est_primary", "Pi0_pt_vs_rap_est_primary; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 40, 0., 4.);
	fHistoList_MC.push_back(Pi0_pt_vs_rap_est_primary); 

	MC_pi0_theta = new TH1D("MC_pi0_theta", "Monte Carlo,   #pi^{0},   #theta distribution; emission angle #theta in deg;Entries", 180., 0., 180.);
	fHistoList_MC.push_back(MC_pi0_theta);

	MC_pi0_phi = new TH1D("MC_pi0_phi", "Monte Carlo,   #pi^{0},   #phi distribution; emission angle #phi in deg; Entries", 180., -180., 180.);
	fHistoList_MC.push_back(MC_pi0_phi);

	MC_pi0_Rapidity = new TH1D("MC_pi0_Rapidity", "Monte Carlo,   #pi^{0},   #rapidity distribution; emission angle #phi in deg; Entries", 90, -2., 7.);
	fHistoList_MC.push_back(MC_pi0_Rapidity);

	MC_pi0_theta_vs_rap = new TH2D("MC_pi0_theta_vs_rap", "Monte Carlo,   #pi^{0},   #theta vs rapidity distribution; emission angle #theta in deg; rapidity y", 180., 0., 180., 270, -2., 7.);
	fHistoList_MC.push_back(MC_pi0_theta_vs_rap);

	MC_leptons_conversion_ZY = new TH2D("MC_leptons_conversion_ZY","Start vertices of leptons coming from #gamma; z[cm]; y[cm]", 800, -1, 400, 880, -220, 220);
	fHistoList_MC.push_back(MC_leptons_conversion_ZY);
	MC_leptons_conversion_XY = new TH2D("MC_leptons_conversion_XY","Start vertices of leptons coming from #gamma; x[cm]; y[cm]", 1200, -300, 300, 880, -220, 220);
	fHistoList_MC.push_back(MC_leptons_conversion_XY);
	MC_leptons_conversion_XZ = new TH2D("MC_leptons_conversion_XZ","Start vertices of leptons coming from #gamma; x[cm]; z[cm]", 1200, -300, 300, 800, -1, 400);
	fHistoList_MC.push_back(MC_leptons_conversion_XZ);

	MC_leptons_from_pi0_start_vertex = new TH2D("MC_leptons_from_pi0_start_vertex","Start vertices of leptons coming from #pi^{0}; z[cm]; y[cm]", 200, -1, 100, 200, -50, 50);
	...
	MC_eta_Pt_vs_rap = new TH2D("MC_eta_Pt_vs_rap", "Monte Carlo,   #eta,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);
	fHistoList_MC.push_back(MC_eta_Pt_vs_rap);

	MC_eta_Pt_vs_rap_primary = new TH2D("MC_eta_Pt_vs_rap_primary", "Monte Carlo,   primary #eta,   p_{t} vs rapidity distribution; rapidity y; p_{t} in GeV/c ", 90, -2., 7.,  60, -1., 5.);
	fHistoList_MC.push_back(MC_eta_Pt_vs_rap_primary);

	MC_eta_theta = new TH1D("MC_eta_theta", "Monte Carlo,   #eta,   #theta distribution; emission angle #theta in deg; Entries", 180., 0., 180.);
	fHistoList_MC.push_back(MC_eta_theta);

	MC_eta_theta_vs_rap = new TH2D("MC_eta_theta_vs_rap", "Monte Carlo,   #eta,   #theta vs rapidity distribution; emission angle #theta in deg; rapidity y", 180., 0., 180., 270, -2., 7.);
	fHistoList_MC.push_back(MC_eta_theta_vs_rap);

	//***** RICH analysis: A, B, dR, points, hits
	ForChristian_P_vs_R = new TH2D("ForChristian_P_vs_R", "ForChristian_P_vs_R; P [GeV]; R [cm]; Nof", 100, 0, 10, 100, 0, 8);
	...
	AllPoints3D = new TH3D("AllPoints3D", "AllPoints3D; X [cm]; Y [cm]; Z [cm]", 300, -150, 150, 120, 120, 240, 100, 160, 260);
	...
	NumberOfRings_electrons = new TH2D("NumberOfRings_electrons", "Number of rings from electrons; X [cm]; Y [cm]; Nof", 60, -150, 150, 20, 120, 220);
	fHistoList.push_back(NumberOfRings_electrons);

	AllHits_electrons = new TH2D("AllHits_electrons", "Hits from electrons after unfolding; X [cm]; Y [cm]; Nof", 300, -150, 150, 120, 120, 240);
	...
	dR2d_electrons  = new TProfile2D("dR2d_electrons","dR for electrons; X [cm]; Y [cm]", 60, -150, 150, 20, 120, 220, 0, 1);
	fHistoList.push_back(dR2d_electrons);

	Distance_electron = new TProfile2D("Distance_electron","Distance between projected track and center of the ring for electrons; X [cm];Y [cm]; Nof", 60, -150, 150, 20, 120, 220);
	fHistoList.push_back(Distance_electron);

	Distance_positron = new TProfile2D("Distance_positron","Distance between projected track and center of the ring for positrons; X [cm];Y [cm]; Nof", 60, -150, 150, 20, 120, 220);
	...
	AllPointsPerPixel = new TH2D("AllPointsPerPixel", "AllPointsPerPixel; X [cm]; Y [cm]; Nof", 350, -105, 105, 142, 125, 210);
	...
	HitsPerPmtFullPlane = new TH1D("HitsPerPmtFullPlane", "Number of hits per PMT. Distribution for all PMTs; Nof photons", 50, -0.5, 49.5);
	fHistoList.push_back(HitsPerPmtFullPlane);
	HitsPerPmtFullMiddle = new TH1D("HitsPerPmtFullMiddle", "Number of hits per PMT. Distribution for all PMT in y={125#;155}, x={-105#;105}; Nof photons", 50, -0.5, 49.5);
