			...
			for(size_t j=i+1; j<Electrons.size(); j++) {
				for(size_t k=j+1; k<Electrons.size(); k++) {
					for(size_t l=k+1; l<Electrons.size(); l++) {
						...
						if (grandmotherId1 == grandmotherId2 && grandmotherId1 == grandmotherId3 && grandmotherId1 == grandmotherId4 && GrTrack->GetPdgCode() == 221){
							EtaConversion++;
							cout << "Decay eta -> gamma gamma -> e+e- e+e- detected!\t\t mc mass: " << GrTrack->GetMass() << endl;
