...
void CbmKresConversionKF::Exec(int fEventNumKF, double OpeningAngleCut, double GammaInvMassCut, int RealPID)
{
	...
	FindPi0("All", GammasAll, GammasAllStsIndex, GammasAllMC, GammasAllZ, fPi0InvMassRecoKF_All, fPi0_pt_vs_rap_All, fPi0_pt_vs_rap_est_All, fPi0InvMassRecoKF_All_target, fPi0InvMassRecoKF_All_mvd, fPi0InvMassRecoKF_All_sts, fPi0InvMassRecoKF_All_outside, MultiplicityGamma_All, MultiplicityChargedParticles_All, fHistoList_bg_all);

	FindPi0("Zero", GammasZero, GammasZeroStsIndex, GammasZeroMC, GammasZeroZ, fPi0InvMassRecoKF_Zero, fPi0_pt_vs_rap_Zero, fPi0_pt_vs_rap_est_Zero, fPi0InvMassRecoKF_Zero_target, fPi0InvMassRecoKF_Zero_mvd, fPi0InvMassRecoKF_Zero_sts, fPi0InvMassRecoKF_Zero_outside, MultiplicityGamma_Zero, MultiplicityChargedParticles_Zero, fHistoList_bg_zero);

	FindPi0("One", GammasOne, GammasOneStsIndex, GammasOneMC, GammasOneZ, fPi0InvMassRecoKF_One, fPi0_pt_vs_rap_One, fPi0_pt_vs_rap_est_One, fPi0InvMassRecoKF_One_target, fPi0InvMassRecoKF_One_mvd, fPi0InvMassRecoKF_One_sts, fPi0InvMassRecoKF_One_outside, MultiplicityGamma_One, MultiplicityChargedParticles_One, fHistoList_bg_one);

	FindPi0("Two", GammasTwo, GammasTwoStsIndex, GammasTwoMC, GammasTwoZ, fPi0InvMassRecoKF_Two, fPi0_pt_vs_rap_Two, fPi0_pt_vs_rap_est_Two, fPi0InvMassRecoKF_Two_target, fPi0InvMassRecoKF_Two_mvd, fPi0InvMassRecoKF_Two_sts, fPi0InvMassRecoKF_Two_outside, MultiplicityGamma_Two, MultiplicityChargedParticles_Two, fHistoList_bg_two);
...
void CbmKresConversionKF::FindGammas(vector<KFParticle> allgammas, vector<KFParticle> particlevector, int Event, double AngleCut,  double InvMassCut, int RealPID)
{
	for (size_t tt=0; tt<allgammas.size(); tt++){
		...
		TVector3 refmomentum0 = CbmKresFunctions::FitToVertex(stsTrack0, allgammas[tt].GetX(), allgammas[tt].GetY(), allgammas[tt].GetZ());
		TVector3 refmomentum1 = CbmKresFunctions::FitToVertex(stsTrack1, allgammas[tt].GetX(), allgammas[tt].GetY(), allgammas[tt].GetZ());
		...
		if (richcheck_0 == 0 && proj->GetX() > -115 && proj->GetX() < 115 && ((proj->GetY() < -120 && proj->GetY() > -200) || (proj->GetY() > 120 && proj->GetY() < 200)) ) continue; 
		FairTrackParam* proj2 = (FairTrackParam*) fRichProjections->At(grDaughter1.at(0));
		if (richcheck_1 == 0 && proj2->GetX() > -115 && proj2->GetX() < 115 && ((proj2->GetY() < -120 && proj2->GetY() > -200) || (proj2->GetY() > 120 && proj2->GetY() < 200)) ) continue; 
		...
		Double_t opening_angle_refitted = CbmKresFunctions::CalculateOpeningAngle_Reco(frefmomentum.at(0), frefmomentum.at(1));
		...
		if ( TMath::Abs(mcTrack0->GetPdgCode()) == 11 && (mcTrack0->GetPdgCode()+mcTrack1->GetPdgCode()) == 0 && (mcTrack0->GetMotherId() == mcTrack1->GetMotherId()) && mcTrack0->GetMotherId() != -1 ){
			CbmMCTrack* mcTrackMother = (CbmMCTrack*) fMcTracks->At(mcTrack0->GetMotherId());
			if (mcTrackMother != nullptr && mcTrackMother->GetPdgCode() == 22 && mcTrackMother->GetMotherId() != -1){ // electrons/positrons from gamma
...
void CbmKresConversionKF::FindPi0(TString /*mod*/, vector< vector<TVector3> > Gammas, vector< vector<int> > StsIndex, vector< vector<CbmMCTrack*> > GammasMC, 
	vector<Double_t> GammasZ, TH1D* Pi0InvMassRecoKF, TH2D* Pi0_pt_vs_rap, TH2D* Pi0_pt_vs_rap_est, TH1D* Pi0InvMassRecoKF_target, TH1D* Pi0InvMassRecoKF_mvd, 
	TH1D* Pi0InvMassRecoKF_sts, TH1D* Pi0InvMassRecoKF_outside, TH2D* MultiplicityGamma, TH2D* MultiplicityChargedParticles, vector<TH1*> BGCases)

{
	...
	for (size_t gamma1=0; gamma1<Gammas.size()-1; gamma1++){
		for (size_t gamma2=gamma1+1; gamma2<Gammas.size(); gamma2++){
			...
			if(StsIndex[gamma1][0] == StsIndex[gamma2][0] || StsIndex[gamma1][0] == StsIndex[gamma2][1] || StsIndex[gamma1][1] == StsIndex[gamma2][0] || StsIndex[gamma1][1] == StsIndex[gamma2][1]) continue; // particles not used twice --> different
...
// combines photons from two different events, taken from each time N events
{
	...
	for(Int_t a = 0; a < nof-1; a++) {
		for(Int_t b = a+1; b < nof; b++) {
			...
			if ( (EMT_NofRings[a] == 1 || EMT_NofRings[a] == 2) && (EMT_NofRings[b] == 1 || EMT_NofRings[b] == 2) ) fEMT_InvMass_OneTwo->Fill(params.fMinv);
			...
			if (EMT_Z[a] < 4 && EMT_Z[b] < 4){
				...
				if ( (EMT_NofRings[a] == 1 || EMT_NofRings[a] == 2) && (EMT_NofRings[b] == 1 || EMT_NofRings[b] == 2) ) fEMT_InvMass_OneTwo_target->Fill(params.fMinv);
			}
			if (EMT_Z[a] > 4 && EMT_Z[b] > 4){
				...
				if ( (EMT_NofRings[a] == 1 || EMT_NofRings[a] == 2) && (EMT_NofRings[b] == 1 || EMT_NofRings[b] == 2) ) fEMT_InvMass_OneTwo_outside->Fill(params.fMinv);
			}
		}
	}
}
...
void CbmKresConversionKF::InitHistograms()
{
	...
	CheckForCuts_InvMass_MC = new TH1D("CheckForCuts_InvMass_MC","CheckForCuts_InvMass_MC; invariant mass in GeV/c^{2};#", 510, -0.01, 0.5);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_MC);
	CheckForCuts_InvMass_Reco = new TH1D("CheckForCuts_InvMass_Reco","CheckForCuts_InvMass_Reco; invariant mass in GeV/c^{2};#", 510, -0.01, 0.5);
	...
	CheckForCuts_OA_Reco = new TH1D("CheckForCuts_OA_Reco","CheckForCuts_OA_Reco; #theta angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_Reco);
	CheckForCuts_InvMass_MC_from_one_pi0 = new TH1D("CheckForCuts_InvMass_MC_from_one_pi0","CheckForCuts_InvMass_MC_from_one_pi0; invariant mass in GeV/c^{2};#", 510, -0.01, 0.5);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_MC_from_one_pi0);
	CheckForCuts_InvMass_Reco_from_one_pi0 = new TH1D("CheckForCuts_InvMass_Reco_from_one_pi0","CheckForCuts_InvMass_Reco_from_one_pi0; invariant mass in GeV/c^{2};#", 510, -0.01, 0.5);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_Reco_from_one_pi0);
	CheckForCuts_OA_MC_from_one_pi0 = new TH1D("CheckForCuts_OA_MC_from_one_pi0","CheckForCuts_OA_MC_from_one_pi0; angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_MC_from_one_pi0);
	CheckForCuts_OA_Reco_from_one_pi0 = new TH1D("CheckForCuts_OA_Reco_from_one_pi0","CheckForCuts_OA_Reco_from_one_pi0; angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_Reco_from_one_pi0);
	CheckForCuts_z_vs_InvMass_MC_from_one_pi0 = new TH2D("CheckForCuts_z_vs_InvMass_MC_from_one_pi0", "CheckForCuts_z_vs_InvMass_MC_from_one_pi0; Z [cm]; invariant mass in GeV/c^{2}; Nof", 1000, -1, 99, 310, -0.01, 0.3);
	fHistoList_CheckForCuts.push_back(CheckForCuts_z_vs_InvMass_MC_from_one_pi0);
	CheckForCuts_z_vs_InvMass_Reco_from_one_pi0 = new TH2D("CheckForCuts_z_vs_InvMass_Reco_from_one_pi0","CheckForCuts_z_vs_InvMass_Reco_from_one_pi0; Z [cm]; invariant mass in GeV/c^{2}; Nof", 1000, -1, 99, 310, -0.01, 0.3);
	fHistoList_CheckForCuts.push_back(CheckForCuts_z_vs_InvMass_Reco_from_one_pi0);
	CheckForCuts_z_vs_OA_MC_from_one_pi0 = new TH2D("CheckForCuts_z_vs_OA_MC_from_one_pi0","CheckForCuts_z_vs_OA_MC_from_one_pi0; Z [cm]; #theta in deg; Nof", 1000, -1, 99, 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_z_vs_OA_MC_from_one_pi0);
	CheckForCuts_z_vs_OA_Reco_from_one_pi0 = new TH2D("CheckForCuts_z_vs_OA_Reco_from_one_pi0","CheckForCuts_z_vs_OA_Reco_from_one_pi0; Z [cm]; #theta in deg; Nof", 1000, -1, 99, 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_z_vs_OA_Reco_from_one_pi0);
	CheckForCuts_InvMass_Reco_from_one_pi0_less4cm = new TH1D("CheckForCuts_InvMass_Reco_from_one_pi0_less4cm","CheckForCuts_InvMass_Reco_from_one_pi0_less4cm; invariant mass in GeV/c^{2};#", 310, -0.01, 0.3);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_Reco_from_one_pi0_less4cm);
	CheckForCuts_OA_Reco_from_one_pi0_less4cm = new TH1D("CheckForCuts_OA_Reco_from_one_pi0_less4cm","CheckForCuts_OA_Reco_from_one_pi0_less4cm; angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_Reco_from_one_pi0_less4cm);
	CheckForCuts_InvMass_Reco_from_one_pi0_4cm_21cm = new TH1D("CheckForCuts_InvMass_Reco_from_one_pi0_4cm_21cm","CheckForCuts_InvMass_Reco_from_one_pi0_4cm_21cm; invariant mass in GeV/c^{2};#", 310, -0.01, 0.3);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_Reco_from_one_pi0_4cm_21cm);
	CheckForCuts_OA_Reco_from_one_pi0_4cm_21cm = new TH1D("CheckForCuts_OA_Reco_from_one_pi0_4cm_21cm","CheckForCuts_OA_Reco_from_one_pi0_4cm_21cm; angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_Reco_from_one_pi0_4cm_21cm);
	CheckForCuts_InvMass_Reco_from_one_pi0_more21cm = new TH1D("CheckForCuts_InvMass_Reco_from_one_pi0_more21cm","CheckForCuts_InvMass_Reco_from_one_pi0_more21cm; invariant mass in GeV/c^{2};#", 310, -0.01, 0.3);
	fHistoList_CheckForCuts.push_back(CheckForCuts_InvMass_Reco_from_one_pi0_more21cm);
	CheckForCuts_OA_Reco_from_one_pi0_more21cm = new TH1D("CheckForCuts_OA_Reco_from_one_pi0_more21cm","CheckForCuts_OA_Reco_from_one_pi0_more21cm; angle [deg];#", 300, -0.1, 29.9);
	fHistoList_CheckForCuts.push_back(CheckForCuts_OA_Reco_from_one_pi0_more21cm);



	// 0-2 => All
	fGammaInvMassReco_All = new TH1D("fGammaInvMassReco_All","fGammaInvMassReco_All; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_All.push_back(fGammaInvMassReco_All);
	fGammaOpeningAngleReco_All = new TH1D("fGammaOpeningAngleReco_All","fGammaOpeningAngleReco_All; angle [deg];#", 200, -0.1, 19.9);
	...
	fPi0InvMassRecoKF_All = new TH1D("fPi0InvMassRecoKF_All","fPi0InvMassRecoKF_All; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	fPi0_pt_vs_rap_All = new TH2D("fPi0_pt_vs_rap_All", "fPi0_pt_vs_rap_All; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);  
	fHistoList_All.push_back(fPi0_pt_vs_rap_All);
	fPi0_pt_vs_rap_est_All = new TH2D("fPi0_pt_vs_rap_est_All", "fPi0_pt_vs_rap_est_All; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 10, 0., 4.);
	fHistoList_All.push_back(fPi0_pt_vs_rap_est_All); 
	// 0-2 => All  target  
	fGammaInvMassReco_All_target = new TH1D("fGammaInvMassReco_All_target","fGammaInvMassReco_All_target; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_All_target.push_back(fGammaInvMassReco_All_target);
	fGammaOpeningAngleReco_All_target = new TH1D("fGammaOpeningAngleReco_All_target","fGammaOpeningAngleReco_All_target; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_All_target.push_back(fGammaOpeningAngleReco_All_target);
	fPi0InvMassRecoKF_All_target = new TH1D("fPi0InvMassRecoKF_All_target","fPi0InvMassRecoKF_All_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_All_target.push_back(fPi0InvMassRecoKF_All_target);
	fEMT_InvMass_All_target = new TH1D("fEMT_InvMass_All_target","fEMT_InvMass_All_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_All_target.push_back(fEMT_InvMass_All_target);
	// 0-2 => All  mvd  
	fGammaInvMassReco_All_mvd = new TH1D("fGammaInvMassReco_All_mvd","fGammaInvMassReco_All_mvd; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_All_mvd.push_back(fGammaInvMassReco_All_mvd);
	fGammaOpeningAngleReco_All_mvd = new TH1D("fGammaOpeningAngleReco_All_mvd","fGammaOpeningAngleReco_All_mvd; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_All_mvd.push_back(fGammaOpeningAngleReco_All_mvd);
	fPi0InvMassRecoKF_All_mvd = new TH1D("fPi0InvMassRecoKF_All_mvd","fPi0InvMassRecoKF_All_mvd; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_All_mvd.push_back(fPi0InvMassRecoKF_All_mvd);
	// 0-2 => All  sts  
	fGammaInvMassReco_All_sts = new TH1D("fGammaInvMassReco_All_sts","fGammaInvMassReco_All_sts; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_All_sts.push_back(fGammaInvMassReco_All_sts);
	fGammaOpeningAngleReco_All_sts = new TH1D("fGammaOpeningAngleReco_All_sts","fGammaOpeningAngleReco_All_sts; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_All_sts.push_back(fGammaOpeningAngleReco_All_sts);
	fPi0InvMassRecoKF_All_sts = new TH1D("fPi0InvMassRecoKF_All_sts","fPi0InvMassRecoKF_All_sts; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_All_sts.push_back(fPi0InvMassRecoKF_All_sts);
	// 0-2 => All  outside the target (mvd+sts+pipe)
	fGammaInvMassReco_All_outside = new TH1D("fGammaInvMassReco_All_outside","fGammaInvMassReco_All_outside; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_All_outside.push_back(fGammaInvMassReco_All_outside);
	fGammaOpeningAngleReco_All_outside = new TH1D("fGammaOpeningAngleReco_All_outside","fGammaOpeningAngleReco_All_outside; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_All_outside.push_back(fGammaOpeningAngleReco_All_outside);
	fPi0InvMassRecoKF_All_outside = new TH1D("fPi0InvMassRecoKF_All_outside","fPi0InvMassRecoKF_All_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_All_outside.push_back(fPi0InvMassRecoKF_All_outside);
	fEMT_InvMass_All_outside = new TH1D("fEMT_InvMass_All_outside","fEMT_InvMass_All_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	// 0 => Zero
	fGammaInvMassReco_Zero = new TH1D("fGammaInvMassReco_Zero","fGammaInvMassReco_Zero; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Zero.push_back(fGammaInvMassReco_Zero);
	fGammaOpeningAngleReco_Zero = new TH1D("fGammaOpeningAngleReco_Zero","fGammaOpeningAngleReco_Zero; angle [deg];#", 200, -0.1, 19.9);
	...
	fPi0InvMassRecoKF_Zero = new TH1D("fPi0InvMassRecoKF_Zero","fPi0InvMassRecoKF_Zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	fPi0_pt_vs_rap_Zero = new TH2D("fPi0_pt_vs_rap_Zero", "fPi0_pt_vs_rap_Zero; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);  
	fHistoList_Zero.push_back(fPi0_pt_vs_rap_Zero);
	fPi0_pt_vs_rap_est_Zero = new TH2D("fPi0_pt_vs_rap_est_Zero", "fPi0_pt_vs_rap_est_Zero; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 10, 0., 4.);
	fHistoList_Zero.push_back(fPi0_pt_vs_rap_est_Zero);
	// 0 => Zero  target  
	fGammaInvMassReco_Zero_target = new TH1D("fGammaInvMassReco_Zero_target","fGammaInvMassReco_Zero_target; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Zero_target.push_back(fGammaInvMassReco_Zero_target);
	fGammaOpeningAngleReco_Zero_target = new TH1D("fGammaOpeningAngleReco_Zero_target","fGammaOpeningAngleReco_Zero_target; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Zero_target.push_back(fGammaOpeningAngleReco_Zero_target);
	fPi0InvMassRecoKF_Zero_target = new TH1D("fPi0InvMassRecoKF_Zero_target","fPi0InvMassRecoKF_Zero_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_target.push_back(fPi0InvMassRecoKF_Zero_target);
	fEMT_InvMass_Zero_target = new TH1D("fEMT_InvMass_Zero_target","fEMT_InvMass_Zero_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_target.push_back(fEMT_InvMass_Zero_target);
	// 0 => Zero  mvd  
	fGammaInvMassReco_Zero_mvd = new TH1D("fGammaInvMassReco_Zero_mvd","fGammaInvMassReco_Zero_mvd; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Zero_mvd.push_back(fGammaInvMassReco_Zero_mvd);
	fGammaOpeningAngleReco_Zero_mvd = new TH1D("fGammaOpeningAngleReco_Zero_mvd","fGammaOpeningAngleReco_Zero_mvd; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Zero_mvd.push_back(fGammaOpeningAngleReco_Zero_mvd);
	fPi0InvMassRecoKF_Zero_mvd = new TH1D("fPi0InvMassRecoKF_Zero_mvd","fPi0InvMassRecoKF_Zero_mvd; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_mvd.push_back(fPi0InvMassRecoKF_Zero_mvd);
	// 0 => Zero  sts  
	fGammaInvMassReco_Zero_sts = new TH1D("fGammaInvMassReco_Zero_sts","fGammaInvMassReco_Zero_sts; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Zero_sts.push_back(fGammaInvMassReco_Zero_sts);
	fGammaOpeningAngleReco_Zero_sts = new TH1D("fGammaOpeningAngleReco_Zero_sts","fGammaOpeningAngleReco_Zero_sts; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Zero_sts.push_back(fGammaOpeningAngleReco_Zero_sts);
	fPi0InvMassRecoKF_Zero_sts = new TH1D("fPi0InvMassRecoKF_Zero_sts","fPi0InvMassRecoKF_Zero_sts; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_sts.push_back(fPi0InvMassRecoKF_Zero_sts);
	// 0 => Zero  outside the target (mvd+sts+pipe)
	fGammaInvMassReco_Zero_outside = new TH1D("fGammaInvMassReco_Zero_outside","fGammaInvMassReco_Zero_outside; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Zero_outside.push_back(fGammaInvMassReco_Zero_outside);
	fGammaOpeningAngleReco_Zero_outside = new TH1D("fGammaOpeningAngleReco_Zero_outside","fGammaOpeningAngleReco_Zero_outside; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Zero_outside.push_back(fGammaOpeningAngleReco_Zero_outside);
	fPi0InvMassRecoKF_Zero_outside = new TH1D("fPi0InvMassRecoKF_Zero_outside","fPi0InvMassRecoKF_Zero_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_outside.push_back(fPi0InvMassRecoKF_Zero_outside);
	fEMT_InvMass_Zero_outside = new TH1D("fEMT_InvMass_Zero_outside","fEMT_InvMass_Zero_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Zero_outside.push_back(fEMT_InvMass_Zero_outside);



	// 1 => One
	fGammaInvMassReco_One = new TH1D("fGammaInvMassReco_One","fGammaInvMassReco_One; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_One.push_back(fGammaInvMassReco_One);
	fGammaOpeningAngleReco_One = new TH1D("fGammaOpeningAngleReco_One","fGammaOpeningAngleReco_One; angle [deg];#", 200, -0.1, 19.9);
	...
	fPi0InvMassRecoKF_One = new TH1D("fPi0InvMassRecoKF_One","fPi0InvMassRecoKF_One; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	fPi0_pt_vs_rap_One = new TH2D("fPi0_pt_vs_rap_One", "fPi0_pt_vs_rap_One; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);  
	fHistoList_One.push_back(fPi0_pt_vs_rap_One);
	fPi0_pt_vs_rap_est_One = new TH2D("fPi0_pt_vs_rap_est_One", "fPi0_pt_vs_rap_est_One; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 10, 0., 4.);
	fHistoList_One.push_back(fPi0_pt_vs_rap_est_One);
	// 1 => One  target  
	fGammaInvMassReco_One_target = new TH1D("fGammaInvMassReco_One_target","fGammaInvMassReco_One_target; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_One_target.push_back(fGammaInvMassReco_One_target);
	fGammaOpeningAngleReco_One_target = new TH1D("fGammaOpeningAngleReco_One_target","fGammaOpeningAngleReco_One_target; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_One_target.push_back(fGammaOpeningAngleReco_One_target);
	fPi0InvMassRecoKF_One_target = new TH1D("fPi0InvMassRecoKF_One_target","fPi0InvMassRecoKF_One_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_target.push_back(fPi0InvMassRecoKF_One_target);
	fEMT_InvMass_One_target = new TH1D("fEMT_InvMass_One_target","fEMT_InvMass_One_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_target.push_back(fEMT_InvMass_One_target);
	// 1 => One  mvd  
	fGammaInvMassReco_One_mvd = new TH1D("fGammaInvMassReco_One_mvd","fGammaInvMassReco_One_mvd; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_One_mvd.push_back(fGammaInvMassReco_One_mvd);
	fGammaOpeningAngleReco_One_mvd = new TH1D("fGammaOpeningAngleReco_One_mvd","fGammaOpeningAngleReco_One_mvd; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_One_mvd.push_back(fGammaOpeningAngleReco_One_mvd);
	fPi0InvMassRecoKF_One_mvd = new TH1D("fPi0InvMassRecoKF_One_mvd","fPi0InvMassRecoKF_One_mvd; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_mvd.push_back(fPi0InvMassRecoKF_One_mvd);
	// 1 => One  sts  
	fGammaInvMassReco_One_sts = new TH1D("fGammaInvMassReco_One_sts","fGammaInvMassReco_One_sts; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_One_sts.push_back(fGammaInvMassReco_One_sts);
	fGammaOpeningAngleReco_One_sts = new TH1D("fGammaOpeningAngleReco_One_sts","fGammaOpeningAngleReco_One_sts; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_One_sts.push_back(fGammaOpeningAngleReco_One_sts);
	fPi0InvMassRecoKF_One_sts = new TH1D("fPi0InvMassRecoKF_One_sts","fPi0InvMassRecoKF_One_sts; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_sts.push_back(fPi0InvMassRecoKF_One_sts);
	// 1 => One  outside the target (mvd+sts+pipe)
	fGammaInvMassReco_One_outside = new TH1D("fGammaInvMassReco_One_outside","fGammaInvMassReco_One_outside; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_One_outside.push_back(fGammaInvMassReco_One_outside);
	fGammaOpeningAngleReco_One_outside = new TH1D("fGammaOpeningAngleReco_One_outside","fGammaOpeningAngleReco_One_outside; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_One_outside.push_back(fGammaOpeningAngleReco_One_outside);
	fPi0InvMassRecoKF_One_outside = new TH1D("fPi0InvMassRecoKF_One_outside","fPi0InvMassRecoKF_One_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_outside.push_back(fPi0InvMassRecoKF_One_outside);	
	fEMT_InvMass_One_outside = new TH1D("fEMT_InvMass_One_outside","fEMT_InvMass_One_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_One_outside.push_back(fEMT_InvMass_One_outside);



	// 2 => Two
	fGammaInvMassReco_Two = new TH1D("fGammaInvMassReco_Two","fGammaInvMassReco_Two; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Two.push_back(fGammaInvMassReco_Two);
	fGammaOpeningAngleReco_Two = new TH1D("fGammaOpeningAngleReco_Two","fGammaOpeningAngleReco_Two; angle [deg];#", 200, -0.1, 19.9);
	...
	fPi0InvMassRecoKF_Two = new TH1D("fPi0InvMassRecoKF_Two","fPi0InvMassRecoKF_Two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	fPi0_pt_vs_rap_Two = new TH2D("fPi0_pt_vs_rap_Two", "fPi0_pt_vs_rap_Two; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);  
	fHistoList_Two.push_back(fPi0_pt_vs_rap_Two);
	fPi0_pt_vs_rap_est_Two = new TH2D("fPi0_pt_vs_rap_est_Two", "fPi0_pt_vs_rap_est_Two; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 10, 0., 4.);
	fHistoList_Two.push_back(fPi0_pt_vs_rap_est_Two);
	// 2 => Two  target  
	fGammaInvMassReco_Two_target = new TH1D("fGammaInvMassReco_Two_target","fGammaInvMassReco_Two_target; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Two_target.push_back(fGammaInvMassReco_Two_target);
	fGammaOpeningAngleReco_Two_target = new TH1D("fGammaOpeningAngleReco_Two_target","fGammaOpeningAngleReco_Two_target; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Two_target.push_back(fGammaOpeningAngleReco_Two_target);
	fPi0InvMassRecoKF_Two_target = new TH1D("fPi0InvMassRecoKF_Two_target","fPi0InvMassRecoKF_Two_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_target.push_back(fPi0InvMassRecoKF_Two_target);
	fEMT_InvMass_Two_target = new TH1D("fEMT_InvMass_Two_target","fEMT_InvMass_Two_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_target.push_back(fEMT_InvMass_Two_target);
	// 2 => Two  mvd  
	fGammaInvMassReco_Two_mvd = new TH1D("fGammaInvMassReco_Two_mvd","fGammaInvMassReco_Two_mvd; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Two_mvd.push_back(fGammaInvMassReco_Two_mvd);
	fGammaOpeningAngleReco_Two_mvd = new TH1D("fGammaOpeningAngleReco_Two_mvd","fGammaOpeningAngleReco_Two_mvd; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Two_mvd.push_back(fGammaOpeningAngleReco_Two_mvd);
	fPi0InvMassRecoKF_Two_mvd = new TH1D("fPi0InvMassRecoKF_Two_mvd","fPi0InvMassRecoKF_Two_mvd; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_mvd.push_back(fPi0InvMassRecoKF_Two_mvd);
	// 2 => Two  sts  
	fGammaInvMassReco_Two_sts = new TH1D("fGammaInvMassReco_Two_sts","fGammaInvMassReco_Two_sts; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Two_sts.push_back(fGammaInvMassReco_Two_sts);
	fGammaOpeningAngleReco_Two_sts = new TH1D("fGammaOpeningAngleReco_Two_sts","fGammaOpeningAngleReco_Two_sts; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Two_sts.push_back(fGammaOpeningAngleReco_Two_sts);
	fPi0InvMassRecoKF_Two_sts = new TH1D("fPi0InvMassRecoKF_Two_sts","fPi0InvMassRecoKF_Two_sts; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_sts.push_back(fPi0InvMassRecoKF_Two_sts);
	// 2 => Two  outside the target (mvd+sts+pipe)
	fGammaInvMassReco_Two_outside = new TH1D("fGammaInvMassReco_Two_outside","fGammaInvMassReco_Two_outside; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_Two_outside.push_back(fGammaInvMassReco_Two_outside);
	fGammaOpeningAngleReco_Two_outside = new TH1D("fGammaOpeningAngleReco_Two_outside","fGammaOpeningAngleReco_Two_outside; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_Two_outside.push_back(fGammaOpeningAngleReco_Two_outside);
	fPi0InvMassRecoKF_Two_outside = new TH1D("fPi0InvMassRecoKF_Two_outside","fPi0InvMassRecoKF_Two_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_outside.push_back(fPi0InvMassRecoKF_Two_outside);
	fEMT_InvMass_Two_outside = new TH1D("fEMT_InvMass_Two_outside","fEMT_InvMass_Two_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_Two_outside.push_back(fEMT_InvMass_Two_outside);



	// 1-2 => OneTwo
	fGammaInvMassReco_OneTwo = new TH1D("fGammaInvMassReco_OneTwo","fGammaInvMassReco_OneTwo; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_OneTwo.push_back(fGammaInvMassReco_OneTwo);
	fGammaOpeningAngleReco_OneTwo = new TH1D("fGammaOpeningAngleReco_OneTwo","fGammaOpeningAngleReco_OneTwo; angle [deg];#", 200, -0.1, 19.9);
	...
	fPi0InvMassRecoKF_OneTwo = new TH1D("fPi0InvMassRecoKF_OneTwo","fPi0InvMassRecoKF_OneTwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo.push_back(fPi0InvMassRecoKF_OneTwo);
	fEMT_InvMass_OneTwo = new TH1D("fEMT_InvMass_OneTwo","fEMT_InvMass_OneTwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo.push_back(fEMT_InvMass_OneTwo);
	fPi0_pt_vs_rap_OneTwo = new TH2D("fPi0_pt_vs_rap_OneTwo", "fPi0_pt_vs_rap_OneTwo; rapidity y; p_{t} in GeV/c ", 90, -2., 7., 60, -1., 5.);  
	fHistoList_OneTwo.push_back(fPi0_pt_vs_rap_OneTwo);
	fPi0_pt_vs_rap_est_OneTwo = new TH2D("fPi0_pt_vs_rap_est_OneTwo", "fPi0_pt_vs_rap_est_OneTwo; rapidity y; p_{t} in GeV/c ", 10, 0., 4., 10, 0., 4.);
	fHistoList_OneTwo.push_back(fPi0_pt_vs_rap_est_OneTwo);
	// 1-2 => OneTwo  target  
	fGammaInvMassReco_OneTwo_target = new TH1D("fGammaInvMassReco_OneTwo_target","fGammaInvMassReco_OneTwo_target; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_OneTwo_target.push_back(fGammaInvMassReco_OneTwo_target);
	fGammaOpeningAngleReco_OneTwo_target = new TH1D("fGammaOpeningAngleReco_OneTwo_target","fGammaOpeningAngleReco_OneTwo_target; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_OneTwo_target.push_back(fGammaOpeningAngleReco_OneTwo_target);
	fPi0InvMassRecoKF_OneTwo_target = new TH1D("fPi0InvMassRecoKF_OneTwo_target","fPi0InvMassRecoKF_OneTwo_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo_target.push_back(fPi0InvMassRecoKF_OneTwo_target);
	fEMT_InvMass_OneTwo_target = new TH1D("fEMT_InvMass_OneTwo_target","fEMT_InvMass_OneTwo_target; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo_target.push_back(fEMT_InvMass_OneTwo_target);
	// 1-2 => OneTwo  mvd  
	fGammaInvMassReco_OneTwo_mvd = new TH1D("fGammaInvMassReco_OneTwo_mvd","fGammaInvMassReco_OneTwo_mvd; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_OneTwo_mvd.push_back(fGammaInvMassReco_OneTwo_mvd);
	fGammaOpeningAngleReco_OneTwo_mvd = new TH1D("fGammaOpeningAngleReco_OneTwo_mvd","fGammaOpeningAngleReco_OneTwo_mvd; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_OneTwo_mvd.push_back(fGammaOpeningAngleReco_OneTwo_mvd);
	fPi0InvMassRecoKF_OneTwo_mvd = new TH1D("fPi0InvMassRecoKF_OneTwo_mvd","fPi0InvMassRecoKF_OneTwo_mvd; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo_mvd.push_back(fPi0InvMassRecoKF_OneTwo_mvd);
	// 1-2 => OneTwo  sts  
	fGammaInvMassReco_OneTwo_sts = new TH1D("fGammaInvMassReco_OneTwo_sts","fGammaInvMassReco_OneTwo_sts; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_OneTwo_sts.push_back(fGammaInvMassReco_OneTwo_sts);
	fGammaOpeningAngleReco_OneTwo_sts = new TH1D("fGammaOpeningAngleReco_OneTwo_sts","fGammaOpeningAngleReco_OneTwo_sts; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_OneTwo_sts.push_back(fGammaOpeningAngleReco_OneTwo_sts);
	fPi0InvMassRecoKF_OneTwo_sts = new TH1D("fPi0InvMassRecoKF_OneTwo_sts","fPi0InvMassRecoKF_OneTwo_sts; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo_sts.push_back(fPi0InvMassRecoKF_OneTwo_sts);
	// 1-2 => OneTwo  outside the target (mvd+sts+pipe)
	fGammaInvMassReco_OneTwo_outside = new TH1D("fGammaInvMassReco_OneTwo_outside","fGammaInvMassReco_OneTwo_outside; invariant mass in GeV/c^{2};#", 110, -0.01, 0.1);
	fHistoList_OneTwo_outside.push_back(fGammaInvMassReco_OneTwo_outside);
	fGammaOpeningAngleReco_OneTwo_outside = new TH1D("fGammaOpeningAngleReco_OneTwo_outside","fGammaOpeningAngleReco_OneTwo_outside; angle [deg];#", 200, -0.1, 19.9);
	fHistoList_OneTwo_outside.push_back(fGammaOpeningAngleReco_OneTwo_outside);
	fPi0InvMassRecoKF_OneTwo_outside = new TH1D("fPi0InvMassRecoKF_OneTwo_outside","fPi0InvMassRecoKF_OneTwo_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_OneTwo_outside.push_back(fPi0InvMassRecoKF_OneTwo_outside);
	fEMT_InvMass_OneTwo_outside = new TH1D("fEMT_InvMass_OneTwo_outside","fEMT_InvMass_OneTwo_outside; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	MultiplicityGamma_All = new TH2D("MultiplicityGamma_All","MultiplicityGamma_All; Nof gammas in event; invariant mass in GeV/c^{2};#", 400, 0, 30, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityGamma_All);
	MultiplicityGamma_Zero = new TH2D("MultiplicityGamma_Zero","MultiplicityGamma_Zero; Nof gammas in event; invariant mass in GeV/c^{2};#", 400, 0, 30, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityGamma_Zero);
	MultiplicityGamma_One = new TH2D("MultiplicityGamma_One","MultiplicityGamma_One; Nof gammas in event; invariant mass in GeV/c^{2};#", 400, 0, 30, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityGamma_One);
	MultiplicityGamma_Two = new TH2D("MultiplicityGamma_Two","MultiplicityGamma_Two; Nof gammas in event; invariant mass in GeV/c^{2};#", 400, 0, 30, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityGamma_Two);
	MultiplicityGamma_OneTwo = new TH2D("MultiplicityGamma_OneTwo","MultiplicityGamma_OneTwo; Nof gammas in event; invariant mass in GeV/c^{2};#", 400, 0, 30, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityGamma_OneTwo);

	MultiplicityChargedParticles_All = new TH2D("MultiplicityChargedParticles_All","MultiplicityChargedParticles_All; Nof charged particles in event; invariant mass in GeV/c^{2};#", 1000, 0, 1000, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityChargedParticles_All);
	MultiplicityChargedParticles_Zero = new TH2D("MultiplicityChargedParticles_Zero","MultiplicityChargedParticles_Zero; Nof charged particles in event; invariant mass in GeV/c^{2};#", 1000, 0, 1000, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityChargedParticles_Zero);	
	MultiplicityChargedParticles_One = new TH2D("MultiplicityChargedParticles_One","MultiplicityChargedParticles_One; Nof charged particles in event; invariant mass in GeV/c^{2};#", 1000, 0, 1000, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityChargedParticles_One);	
	MultiplicityChargedParticles_Two = new TH2D("MultiplicityChargedParticles_Two","MultiplicityChargedParticles_Two; Nof charged particles in event; invariant mass in GeV/c^{2};#", 1000, 0, 1000, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityChargedParticles_Two);	
	MultiplicityChargedParticles_OneTwo = new TH2D("MultiplicityChargedParticles_OneTwo","MultiplicityChargedParticles_OneTwo; Nof charged particles in event; invariant mass in GeV/c^{2};#", 1000, 0, 1000, 1000, 0, 2.0);
	fHistoList_multiplicity.push_back(MultiplicityChargedParticles_OneTwo);



	// Multi EMT
	EMTMulti_InvMass_All_m1 = new TH1D("EMTMulti_InvMass_All_m1","EMTMulti_InvMass_All_m1; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m1);
	EMTMulti_InvMass_All_m2 = new TH1D("EMTMulti_InvMass_All_m2","EMTMulti_InvMass_All_m2; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m2);
	EMTMulti_InvMass_All_m3 = new TH1D("EMTMulti_InvMass_All_m3","EMTMulti_InvMass_All_m3; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m3);
	EMTMulti_InvMass_All_m4 = new TH1D("EMTMulti_InvMass_All_m4","EMTMulti_InvMass_All_m4; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m4);
	EMTMulti_InvMass_All_m5 = new TH1D("EMTMulti_InvMass_All_m5","EMTMulti_InvMass_All_m5; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m5);
	EMTMulti_InvMass_All_m6 = new TH1D("EMTMulti_InvMass_All_m6","EMTMulti_InvMass_All_m6; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m6);
	EMTMulti_InvMass_All_m7 = new TH1D("EMTMulti_InvMass_All_m7","EMTMulti_InvMass_All_m7; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_All.push_back(EMTMulti_InvMass_All_m7);

	EMTMulti_InvMass_Zero_m1 = new TH1D("EMTMulti_InvMass_Zero_m1","EMTMulti_InvMass_Zero_m1; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m1);
	EMTMulti_InvMass_Zero_m2 = new TH1D("EMTMulti_InvMass_Zero_m2","EMTMulti_InvMass_Zero_m2; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m2);
	EMTMulti_InvMass_Zero_m3 = new TH1D("EMTMulti_InvMass_Zero_m3","EMTMulti_InvMass_Zero_m3; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m3);
	EMTMulti_InvMass_Zero_m4 = new TH1D("EMTMulti_InvMass_Zero_m4","EMTMulti_InvMass_Zero_m4; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m4);
	EMTMulti_InvMass_Zero_m5 = new TH1D("EMTMulti_InvMass_Zero_m5","EMTMulti_InvMass_Zero_m5; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m5);
	EMTMulti_InvMass_Zero_m6 = new TH1D("EMTMulti_InvMass_Zero_m6","EMTMulti_InvMass_Zero_m6; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m6);
	EMTMulti_InvMass_Zero_m7 = new TH1D("EMTMulti_InvMass_Zero_m7","EMTMulti_InvMass_Zero_m7; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Zero.push_back(EMTMulti_InvMass_Zero_m7);

	EMTMulti_InvMass_One_m1 = new TH1D("EMTMulti_InvMass_One_m1","EMTMulti_InvMass_One_m1; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m1);
	EMTMulti_InvMass_One_m2 = new TH1D("EMTMulti_InvMass_One_m2","EMTMulti_InvMass_One_m2; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m2);
	EMTMulti_InvMass_One_m3 = new TH1D("EMTMulti_InvMass_One_m3","EMTMulti_InvMass_One_m3; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m3);
	EMTMulti_InvMass_One_m4 = new TH1D("EMTMulti_InvMass_One_m4","EMTMulti_InvMass_One_m4; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m4);
	EMTMulti_InvMass_One_m5 = new TH1D("EMTMulti_InvMass_One_m5","EMTMulti_InvMass_One_m5; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m5);
	EMTMulti_InvMass_One_m6 = new TH1D("EMTMulti_InvMass_One_m6","EMTMulti_InvMass_One_m6; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m6);
	EMTMulti_InvMass_One_m7 = new TH1D("EMTMulti_InvMass_One_m7","EMTMulti_InvMass_One_m7; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_One.push_back(EMTMulti_InvMass_One_m7);

	EMTMulti_InvMass_Two_m1 = new TH1D("EMTMulti_InvMass_Two_m1","EMTMulti_InvMass_Two_m1; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m1);
	EMTMulti_InvMass_Two_m2 = new TH1D("EMTMulti_InvMass_Two_m2","EMTMulti_InvMass_Two_m2; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m2);
	EMTMulti_InvMass_Two_m3 = new TH1D("EMTMulti_InvMass_Two_m3","EMTMulti_InvMass_Two_m3; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m3);
	EMTMulti_InvMass_Two_m4 = new TH1D("EMTMulti_InvMass_Two_m4","EMTMulti_InvMass_Two_m4; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m4);
	EMTMulti_InvMass_Two_m5 = new TH1D("EMTMulti_InvMass_Two_m5","EMTMulti_InvMass_Two_m5; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m5);
	EMTMulti_InvMass_Two_m6 = new TH1D("EMTMulti_InvMass_Two_m6","EMTMulti_InvMass_Two_m6; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m6);
	EMTMulti_InvMass_Two_m7 = new TH1D("EMTMulti_InvMass_Two_m7","EMTMulti_InvMass_Two_m7; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_Two.push_back(EMTMulti_InvMass_Two_m7);

	EMTMulti_InvMass_OneTwo_m1 = new TH1D("EMTMulti_InvMass_OneTwo_m1","EMTMulti_InvMass_OneTwo_m1; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m1);
	EMTMulti_InvMass_OneTwo_m2 = new TH1D("EMTMulti_InvMass_OneTwo_m2","EMTMulti_InvMass_OneTwo_m2; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m2);
	EMTMulti_InvMass_OneTwo_m3 = new TH1D("EMTMulti_InvMass_OneTwo_m3","EMTMulti_InvMass_OneTwo_m3; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m3);
	EMTMulti_InvMass_OneTwo_m4 = new TH1D("EMTMulti_InvMass_OneTwo_m4","EMTMulti_InvMass_OneTwo_m4; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m4);
	EMTMulti_InvMass_OneTwo_m5 = new TH1D("EMTMulti_InvMass_OneTwo_m5","EMTMulti_InvMass_OneTwo_m5; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m5);
	EMTMulti_InvMass_OneTwo_m6 = new TH1D("EMTMulti_InvMass_OneTwo_m6","EMTMulti_InvMass_OneTwo_m6; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_multiplicity_OneTwo.push_back(EMTMulti_InvMass_OneTwo_m6);
	EMTMulti_InvMass_OneTwo_m7 = new TH1D("EMTMulti_InvMass_OneTwo_m7","EMTMulti_InvMass_OneTwo_m7; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8_mothedPDG_all = new TH1D("sameMIDcase8_mothedPDG_all","sameMIDcase8_mothedPDG_all; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_all.push_back(sameMIDcase8_mothedPDG_all);
	PdgCase8NonEComeFromTarget_all = new TH1D("PdgCase8NonEComeFromTarget_all","PdgCase8NonEComeFromTarget_all; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_all.push_back(PdgCase8NonEComeFromTarget_all);
	PdgCase8NonE_NOT_FromTarget_all = new TH1D("PdgCase8NonE_NOT_FromTarget_all","PdgCase8NonE_NOT_FromTarget_all; Id ;#", 5000, -2500, 2500);
	...
	Case8ElFromDalitz_all = new TH1D("Case8ElFromDalitz_all","Case8ElFromDalitz_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_all.push_back(Case8ElFromDalitz_all);
	Case8NonElFrom_pn_all = new TH1D("Case8NonElFrom_pn_all","Case8NonElFrom_pn_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_all.push_back(Case8NonElFrom_pn_all);
	Case8NonElFrom_eta_all = new TH1D("Case8NonElFrom_eta_all","Case8NonElFrom_eta_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_all.push_back(Case8NonElFrom_eta_all);
	Case8NonElFrom_kaon_all = new TH1D("Case8NonElFrom_kaon_all","Case8NonElFrom_kaon_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8NonEMotherPdg_all = new TH1D("sameMIDcase8NonEMotherPdg_all","sameMIDcase8NonEMotherPdg_all; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_all.push_back(sameMIDcase8NonEMotherPdg_all);
	sameMIDcase8NonEMotherIM_all = new TH1D("sameMIDcase8NonEMotherIM_all","sameMIDcase8NonEMotherIM_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_all.push_back(sameMIDcase8NonEMotherIM_all);
	sameMIDcase8NonEPdgFromTarget_all = new TH1D("sameMIDcase8NonEPdgFromTarget_all","sameMIDcase8NonEPdgFromTarget_all; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_all.push_back(sameMIDcase8NonEPdgFromTarget_all);
	sameMIDcase8NonEComeFromTargetIM_all = new TH1D("sameMIDcase8NonEComeFromTargetIM_all","sameMIDcase8NonEComeFromTargetIM_all; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_all.push_back(sameMIDcase8NonEComeFromTargetIM_all);
	sameMIDcase8NonEComeFromTargetP_all = new TH1D("sameMIDcase8NonEComeFromTargetP_all","sameMIDcase8NonEComeFromTargetP_all; P in GeV/c^{2} ;#", 200, 0, 10);
	fHistoList_bg_all.push_back(sameMIDcase8NonEComeFromTargetP_all);
	sameMIDcase8NonEComeFromTargetPt_all = new TH1D("sameMIDcase8NonEComeFromTargetPt_all","sameMIDcase8NonEComeFromTargetPt_all; Pt in GeV/c^{2} ;#", 200, 0, 10);
	...
	sameGRIDcase8_zero = new TH1D("sameGRIDcase8_zero","sameGRIDcase8_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8_mothedPDG_zero = new TH1D("sameMIDcase8_mothedPDG_zero","sameMIDcase8_mothedPDG_zero; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_zero.push_back(sameMIDcase8_mothedPDG_zero);
	PdgCase8NonEComeFromTarget_zero = new TH1D("PdgCase8NonEComeFromTarget_zero","PdgCase8NonEComeFromTarget_zero; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_zero.push_back(PdgCase8NonEComeFromTarget_zero);
	PdgCase8NonE_NOT_FromTarget_zero = new TH1D("PdgCase8NonE_NOT_FromTarget_zero","PdgCase8NonE_NOT_FromTarget_zero; Id ;#", 5000, -2500, 2500);
	...
	Case8ElFromDalitz_zero = new TH1D("Case8ElFromDalitz_zero","Case8ElFromDalitz_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_zero.push_back(Case8ElFromDalitz_zero);
	Case8NonElFrom_pn_zero = new TH1D("Case8NonElFrom_pn_zero","Case8NonElFrom_pn_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_zero.push_back(Case8NonElFrom_pn_zero);
	Case8NonElFrom_eta_zero = new TH1D("Case8NonElFrom_eta_zero","Case8NonElFrom_eta_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_zero.push_back(Case8NonElFrom_eta_zero);
	Case8NonElFrom_kaon_zero = new TH1D("Case8NonElFrom_kaon_zero","Case8NonElFrom_kaon_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8NonEMotherPdg_zero = new TH1D("sameMIDcase8NonEMotherPdg_zero","sameMIDcase8NonEMotherPdg_zero; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_zero.push_back(sameMIDcase8NonEMotherPdg_zero);
	sameMIDcase8NonEMotherIM_zero = new TH1D("sameMIDcase8NonEMotherIM_zero","sameMIDcase8NonEMotherIM_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_zero.push_back(sameMIDcase8NonEMotherIM_zero);
	sameMIDcase8NonEPdgFromTarget_zero = new TH1D("sameMIDcase8NonEPdgFromTarget_zero","sameMIDcase8NonEPdgFromTarget_zero; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_zero.push_back(sameMIDcase8NonEPdgFromTarget_zero);
	sameMIDcase8NonEComeFromTargetIM_zero = new TH1D("sameMIDcase8NonEComeFromTargetIM_zero","sameMIDcase8NonEComeFromTargetIM_zero; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_zero.push_back(sameMIDcase8NonEComeFromTargetIM_zero);
	sameMIDcase8NonEComeFromTargetP_zero = new TH1D("sameMIDcase8NonEComeFromTargetP_zero","sameMIDcase8NonEComeFromTargetP_zero; P in GeV/c^{2} ;#", 200, 0, 10);
	fHistoList_bg_zero.push_back(sameMIDcase8NonEComeFromTargetP_zero);
	sameMIDcase8NonEComeFromTargetPt_zero = new TH1D("sameMIDcase8NonEComeFromTargetPt_zero","sameMIDcase8NonEComeFromTargetPt_zero; Pt in GeV/c^{2} ;#", 200, 0, 10);
	...
	sameMIDcase8_mothedPDG_one = new TH1D("sameMIDcase8_mothedPDG_one","sameMIDcase8_mothedPDG_one; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_one.push_back(sameMIDcase8_mothedPDG_one);
	PdgCase8NonEComeFromTarget_one = new TH1D("PdgCase8NonEComeFromTarget_one","PdgCase8NonEComeFromTarget_one; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_one.push_back(PdgCase8NonEComeFromTarget_one);
	PdgCase8NonE_NOT_FromTarget_one = new TH1D("PdgCase8NonE_NOT_FromTarget_one","PdgCase8NonE_NOT_FromTarget_one; Id ;#", 5000, -2500, 2500);
	...
	Case8ElFromDalitz_one = new TH1D("Case8ElFromDalitz_one","Case8ElFromDalitz_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_one.push_back(Case8ElFromDalitz_one);
	Case8NonElFrom_pn_one = new TH1D("Case8NonElFrom_pn_one","Case8NonElFrom_pn_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_one.push_back(Case8NonElFrom_pn_one);
	Case8NonElFrom_eta_one = new TH1D("Case8NonElFrom_eta_one","Case8NonElFrom_eta_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_one.push_back(Case8NonElFrom_eta_one);
	Case8NonElFrom_kaon_one = new TH1D("Case8NonElFrom_kaon_one","Case8NonElFrom_kaon_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8NonEMotherPdg_one = new TH1D("sameMIDcase8NonEMotherPdg_one","sameMIDcase8NonEMotherPdg_one; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_one.push_back(sameMIDcase8NonEMotherPdg_one);
	sameMIDcase8NonEMotherIM_one = new TH1D("sameMIDcase8NonEMotherIM_one","sameMIDcase8NonEMotherIM_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_one.push_back(sameMIDcase8NonEMotherIM_one);
	sameMIDcase8NonEPdgFromTarget_one = new TH1D("sameMIDcase8NonEPdgFromTarget_one","sameMIDcase8NonEPdgFromTarget_one; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_one.push_back(sameMIDcase8NonEPdgFromTarget_one);
	sameMIDcase8NonEComeFromTargetIM_one = new TH1D("sameMIDcase8NonEComeFromTargetIM_one","sameMIDcase8NonEComeFromTargetIM_one; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_one.push_back(sameMIDcase8NonEComeFromTargetIM_one);
	sameMIDcase8NonEComeFromTargetP_one = new TH1D("sameMIDcase8NonEComeFromTargetP_one","sameMIDcase8NonEComeFromTargetP_one; P in GeV/c^{2} ;#", 200, 0, 10);
	fHistoList_bg_one.push_back(sameMIDcase8NonEComeFromTargetP_one);
	sameMIDcase8NonEComeFromTargetPt_one = new TH1D("sameMIDcase8NonEComeFromTargetPt_one","sameMIDcase8NonEComeFromTargetPt_one; Pt in GeV/c^{2} ;#", 200, 0, 10);
	...
	sameMIDcase8_mothedPDG_two = new TH1D("sameMIDcase8_mothedPDG_two","sameMIDcase8_mothedPDG_two; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_two.push_back(sameMIDcase8_mothedPDG_two);
	PdgCase8NonEComeFromTarget_two = new TH1D("PdgCase8NonEComeFromTarget_two","PdgCase8NonEComeFromTarget_two; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_two.push_back(PdgCase8NonEComeFromTarget_two);
	PdgCase8NonE_NOT_FromTarget_two = new TH1D("PdgCase8NonE_NOT_FromTarget_two","PdgCase8NonE_NOT_FromTarget_two; Id ;#", 5000, -2500, 2500);
	...
	Case8ElFromDalitz_two = new TH1D("Case8ElFromDalitz_two","Case8ElFromDalitz_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_two.push_back(Case8ElFromDalitz_two);
	Case8NonElFrom_pn_two = new TH1D("Case8NonElFrom_pn_two","Case8NonElFrom_pn_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_two.push_back(Case8NonElFrom_pn_two);
	Case8NonElFrom_eta_two = new TH1D("Case8NonElFrom_eta_two","Case8NonElFrom_eta_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_two.push_back(Case8NonElFrom_eta_two);
	Case8NonElFrom_kaon_two = new TH1D("Case8NonElFrom_kaon_two","Case8NonElFrom_kaon_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8NonEMotherPdg_two = new TH1D("sameMIDcase8NonEMotherPdg_two","sameMIDcase8NonEMotherPdg_two; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_two.push_back(sameMIDcase8NonEMotherPdg_two);
	sameMIDcase8NonEMotherIM_two = new TH1D("sameMIDcase8NonEMotherIM_two","sameMIDcase8NonEMotherIM_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_two.push_back(sameMIDcase8NonEMotherIM_two);
	sameMIDcase8NonEPdgFromTarget_two = new TH1D("sameMIDcase8NonEPdgFromTarget_two","sameMIDcase8NonEPdgFromTarget_two; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_two.push_back(sameMIDcase8NonEPdgFromTarget_two);
	sameMIDcase8NonEComeFromTargetIM_two = new TH1D("sameMIDcase8NonEComeFromTargetIM_two","sameMIDcase8NonEComeFromTargetIM_two; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_two.push_back(sameMIDcase8NonEComeFromTargetIM_two);
	sameMIDcase8NonEComeFromTargetP_two = new TH1D("sameMIDcase8NonEComeFromTargetP_two","sameMIDcase8NonEComeFromTargetP_two; P in GeV/c^{2} ;#", 200, 0, 10);
	fHistoList_bg_two.push_back(sameMIDcase8NonEComeFromTargetP_two);
	sameMIDcase8NonEComeFromTargetPt_two = new TH1D("sameMIDcase8NonEComeFromTargetPt_two","sameMIDcase8NonEComeFromTargetPt_two; Pt in GeV/c^{2} ;#", 200, 0, 10);
	...
	sameMIDcase8_onetwo = new TH1D("sameMIDcase8_onetwo","sameMIDcase8_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0); 
	fHistoList_bg_onetwo.push_back(sameMIDcase8_onetwo);
	sameGRIDcase8_onetwo = new TH1D("sameGRIDcase8_onetwo","sameGRIDcase8_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	...
	sameMIDcase8_mothedPDG_onetwo = new TH1D("sameMIDcase8_mothedPDG_onetwo","sameMIDcase8_mothedPDG_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(sameMIDcase8_mothedPDG_onetwo);
	PdgCase8NonEComeFromTarget_onetwo = new TH1D("PdgCase8NonEComeFromTarget_onetwo","PdgCase8NonEComeFromTarget_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(PdgCase8NonEComeFromTarget_onetwo);
	PdgCase8NonE_NOT_FromTarget_onetwo = new TH1D("PdgCase8NonE_NOT_FromTarget_onetwo","PdgCase8NonE_NOT_FromTarget_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(PdgCase8NonE_NOT_FromTarget_onetwo);
	PdgCase8motherNonE_onetwo = new TH1D("PdgCase8motherNonE_onetwo","PdgCase8motherNonE_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(PdgCase8motherNonE_onetwo);
	Case8ElFromDalitz_onetwo = new TH1D("Case8ElFromDalitz_onetwo","Case8ElFromDalitz_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(Case8ElFromDalitz_onetwo);
	Case8NonElFrom_pn_onetwo = new TH1D("Case8NonElFrom_pn_onetwo","Case8NonElFrom_pn_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(Case8NonElFrom_pn_onetwo);
	Case8NonElFrom_eta_onetwo = new TH1D("Case8NonElFrom_eta_onetwo","Case8NonElFrom_eta_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(Case8NonElFrom_eta_onetwo);
	Case8NonElFrom_kaon_onetwo = new TH1D("Case8NonElFrom_kaon_onetwo","Case8NonElFrom_kaon_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(Case8NonElFrom_kaon_onetwo);
	sameMIDcase8NonEPdg_onetwo = new TH1D("sameMIDcase8NonEPdg_onetwo","sameMIDcase8NonEPdg_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEPdg_onetwo);
	sameMIDcase8NonEMotherPdg_onetwo = new TH1D("sameMIDcase8NonEMotherPdg_onetwo","sameMIDcase8NonEMotherPdg_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEMotherPdg_onetwo);
	sameMIDcase8NonEMotherIM_onetwo = new TH1D("sameMIDcase8NonEMotherIM_onetwo","sameMIDcase8NonEMotherIM_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEMotherIM_onetwo);
	sameMIDcase8NonEPdgFromTarget_onetwo = new TH1D("sameMIDcase8NonEPdgFromTarget_onetwo","sameMIDcase8NonEPdgFromTarget_onetwo; Id ;#", 5000, -2500, 2500);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEPdgFromTarget_onetwo);
	sameMIDcase8NonEComeFromTargetIM_onetwo = new TH1D("sameMIDcase8NonEComeFromTargetIM_onetwo","sameMIDcase8NonEComeFromTargetIM_onetwo; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEComeFromTargetIM_onetwo);
	sameMIDcase8NonEComeFromTargetP_onetwo = new TH1D("sameMIDcase8NonEComeFromTargetP_onetwo","sameMIDcase8NonEComeFromTargetP_onetwo; P in GeV/c^{2} ;#", 200, 0, 10);
	fHistoList_bg_onetwo.push_back(sameMIDcase8NonEComeFromTargetP_onetwo);
	sameMIDcase8NonEComeFromTargetPt_onetwo = new TH1D("sameMIDcase8NonEComeFromTargetPt_onetwo","sameMIDcase8NonEComeFromTargetPt_onetwo; Pt in GeV/c^{2} ;#", 200, 0, 10);
