...
void CbmKresConversionManual::Init() {
  ...
  if (nullptr == ioman) {
    Fatal("CbmKresConversionManual::Init", "RootManager not instantised!");
  }

  fMcTracks = (TClonesArray*) ioman->GetObject("MCTrack");
  if (nullptr == fMcTracks) {
    Fatal("CbmKresConversionManual::Init", "No MCTrack array!");
  }
  ...
  if (nullptr == fPrimVertex) {
    LOG(fatal) << "CbmKresConversionManual::Init  No PrimaryVertex array!";
  }

  fGlobalTracks = (TClonesArray*) ioman->GetObject("GlobalTrack");
  if (nullptr == fGlobalTracks) {
    Fatal("CbmKresConversionManual::Init", "No GlobalTrack array!");
  }

  fStsTracks = (TClonesArray*) ioman->GetObject("StsTrack");
  if (nullptr == fStsTracks) {
    Fatal("CbmKresConversionManual::Init", "No StsTrack array!");
  }

  fStsTrackMatches = (TClonesArray*) ioman->GetObject("StsTrackMatch");
  if (nullptr == fStsTrackMatches) {
    Fatal("CbmKresConversionManual::Init", "No StsTrackMatch array!");
  }

  fRichProjections = (TClonesArray*) ioman->GetObject("RichProjection");
  if (nullptr == fRichProjections) {
    Fatal("CbmKresConversionManual::Init", "No RichProjection array!");
  }

  fRichRings = (TClonesArray*) ioman->GetObject("RichRing");
  if (nullptr == fRichRings) {
    Fatal("CbmKresConversionManual::Init", "No RichRing array!");
  }

  fRichRingMatches = (TClonesArray*) ioman->GetObject("RichRingMatch");
  if (nullptr == fRichRingMatches) {
    Fatal("CbmKresConversionManual::Init", "No RichRingMatch array!");
  }

  fRichHits = (TClonesArray*) ioman->GetObject("RichHit");
  if (nullptr == fRichHits) {
    Fatal("CbmKresConversionManual::Init", "No RichHit array!");
  }

  fArrayMvdHit = (TClonesArray*) ioman->GetObject("MvdHit");
  if (nullptr == fArrayMvdHit) {
    Fatal("CbmKresConversionManual::Init", "No MvdHit array!");
  }

  fArrayStsHit = (TClonesArray*) ioman->GetObject("StsHit");
  if (nullptr == fArrayStsHit) {
    Fatal("CbmKresConversionManual::Init", "No StsHit array!");
  }
...
void CbmKresConversionManual::Exec(int fEventNumMan,
  ...
  for (Int_t i = 0; i < ngTracks; i++) {
    ...
    if (richInd > -1) {
      ...
      Pdg_vs_Distance->Fill(TMath::Abs(mcTrack->GetPdgCode()),
                            CbmRichUtil::GetRingTrackDistance(i));
      if (TMath::Abs(mcTrack->GetPdgCode()) == 11)
        P_vs_Distance->Fill(mcTrack->GetP(),
                            CbmRichUtil::GetRingTrackDistance(i));
    }
  ...
  if (fEventNumMan % numformix == 0) {
    EMT_man_Event_Both.insert(EMT_man_Event_Both.end(),
                              EMT_man_Event_Outside.begin(),
                              EMT_man_Event_Outside.end());
    EMT_man_Event_Both.insert(EMT_man_Event_Both.end(),
                              EMT_man_Event_Target.begin(),
                              EMT_man_Event_Target.end());
    EMT_man_pair_momenta_Both.insert(EMT_man_pair_momenta_Both.end(),
                                     EMT_man_pair_momenta_Outside.begin(),
                                     EMT_man_pair_momenta_Outside.end());
    EMT_man_pair_momenta_Both.insert(EMT_man_pair_momenta_Both.end(),
                                     EMT_man_pair_momenta_Target.begin(),
                                     EMT_man_pair_momenta_Target.end());
    EMT_man_NofRings_Both.insert(EMT_man_NofRings_Both.end(),
                                 EMT_man_NofRings_Outside.begin(),
                                 EMT_man_NofRings_Outside.end());
    EMT_man_NofRings_Both.insert(EMT_man_NofRings_Both.end(),
                                 EMT_man_NofRings_Target.begin(),
...
void CbmKresConversionManual::FindGammasTarget(
  int EventNumMan,
  double AngleCut,
  double InvMassCut,
  int RealPID,
  ...
  for (size_t i = 0; i < Momenta_minus.size(); i++) {
    for (size_t j = 0; j < Momenta_plus.size(); j++) {
      ...
      double AnnValue = 999;
      if (UseAnn == 1) {
        AnnValue = fAnnSelection->DoSelect(InvmassReco,
                                           OpeningAngle,
                                           PlaneAngle_last,
                                           fKFVertex.GetRefZ(),
                                           part1,
...
void CbmKresConversionManual::FindGammasBoth() {
  Gammas_all_Both.insert(Gammas_all_Both.end(),
                         Gammas_all_Outside.begin(),
                         Gammas_all_Outside.end());
  Gammas_all_Both.insert(
    Gammas_all_Both.end(), Gammas_all_Target.begin(), Gammas_all_Target.end());
  Gammas_zero_Both.insert(Gammas_zero_Both.end(),
                          Gammas_zero_Outside.begin(),
                          Gammas_zero_Outside.end());
  Gammas_zero_Both.insert(Gammas_zero_Both.end(),
                          Gammas_zero_Target.begin(),
                          Gammas_zero_Target.end());
  Gammas_one_Both.insert(Gammas_one_Both.end(),
                         Gammas_one_Outside.begin(),
                         Gammas_one_Outside.end());
  Gammas_one_Both.insert(
    Gammas_one_Both.end(), Gammas_one_Target.begin(), Gammas_one_Target.end());
  Gammas_two_Both.insert(Gammas_two_Both.end(),
                         Gammas_two_Outside.begin(),
                         Gammas_two_Outside.end());
  Gammas_two_Both.insert(
    Gammas_two_Both.end(), Gammas_two_Target.begin(), Gammas_two_Target.end());
  Gammas_onetwo_Both.insert(Gammas_onetwo_Both.end(),
                            Gammas_onetwo_Outside.begin(),
                            Gammas_onetwo_Outside.end());
  Gammas_onetwo_Both.insert(Gammas_onetwo_Both.end(),
                            Gammas_onetwo_Target.begin(),
                            Gammas_onetwo_Target.end());

  Gammas_stsIndex_all_Both.insert(Gammas_stsIndex_all_Both.end(),
                                  Gammas_stsIndex_all_Outside.begin(),
                                  Gammas_stsIndex_all_Outside.end());
  Gammas_stsIndex_all_Both.insert(Gammas_stsIndex_all_Both.end(),
                                  Gammas_stsIndex_all_Target.begin(),
                                  Gammas_stsIndex_all_Target.end());
  Gammas_stsIndex_zero_Both.insert(Gammas_stsIndex_zero_Both.end(),
                                   Gammas_stsIndex_zero_Outside.begin(),
                                   Gammas_stsIndex_zero_Outside.end());
  Gammas_stsIndex_zero_Both.insert(Gammas_stsIndex_zero_Both.end(),
                                   Gammas_stsIndex_zero_Target.begin(),
                                   Gammas_stsIndex_zero_Target.end());
  Gammas_stsIndex_one_Both.insert(Gammas_stsIndex_one_Both.end(),
                                  Gammas_stsIndex_one_Outside.begin(),
                                  Gammas_stsIndex_one_Outside.end());
  Gammas_stsIndex_one_Both.insert(Gammas_stsIndex_one_Both.end(),
                                  Gammas_stsIndex_one_Target.begin(),
                                  Gammas_stsIndex_one_Target.end());
  Gammas_stsIndex_two_Both.insert(Gammas_stsIndex_two_Both.end(),
                                  Gammas_stsIndex_two_Outside.begin(),
                                  Gammas_stsIndex_two_Outside.end());
  Gammas_stsIndex_two_Both.insert(Gammas_stsIndex_two_Both.end(),
                                  Gammas_stsIndex_two_Target.begin(),
                                  Gammas_stsIndex_two_Target.end());
  Gammas_stsIndex_onetwo_Both.insert(Gammas_stsIndex_onetwo_Both.end(),
                                     Gammas_stsIndex_onetwo_Outside.begin(),
                                     Gammas_stsIndex_onetwo_Outside.end());
  Gammas_stsIndex_onetwo_Both.insert(Gammas_stsIndex_onetwo_Both.end(),
                                     Gammas_stsIndex_onetwo_Target.begin(),
                                     Gammas_stsIndex_onetwo_Target.end());

  Gammas_MCIndex_all_Both.insert(Gammas_MCIndex_all_Both.end(),
                                 Gammas_MCIndex_all_Outside.begin(),
                                 Gammas_MCIndex_all_Outside.end());
  Gammas_MCIndex_all_Both.insert(Gammas_MCIndex_all_Both.end(),
                                 Gammas_MCIndex_all_Target.begin(),
                                 Gammas_MCIndex_all_Target.end());
  Gammas_MCIndex_zero_Both.insert(Gammas_MCIndex_zero_Both.end(),
                                  Gammas_MCIndex_zero_Outside.begin(),
                                  Gammas_MCIndex_zero_Outside.end());
  Gammas_MCIndex_zero_Both.insert(Gammas_MCIndex_zero_Both.end(),
                                  Gammas_MCIndex_zero_Target.begin(),
                                  Gammas_MCIndex_zero_Target.end());
  Gammas_MCIndex_one_Both.insert(Gammas_MCIndex_one_Both.end(),
                                 Gammas_MCIndex_one_Outside.begin(),
                                 Gammas_MCIndex_one_Outside.end());
  Gammas_MCIndex_one_Both.insert(Gammas_MCIndex_one_Both.end(),
                                 Gammas_MCIndex_one_Target.begin(),
                                 Gammas_MCIndex_one_Target.end());
  Gammas_MCIndex_two_Both.insert(Gammas_MCIndex_two_Both.end(),
                                 Gammas_MCIndex_two_Outside.begin(),
                                 Gammas_MCIndex_two_Outside.end());
  Gammas_MCIndex_two_Both.insert(Gammas_MCIndex_two_Both.end(),
                                 Gammas_MCIndex_two_Target.begin(),
                                 Gammas_MCIndex_two_Target.end());
  Gammas_MCIndex_onetwo_Both.insert(Gammas_MCIndex_onetwo_Both.end(),
                                    Gammas_MCIndex_onetwo_Outside.begin(),
                                    Gammas_MCIndex_onetwo_Outside.end());
  Gammas_MCIndex_onetwo_Both.insert(Gammas_MCIndex_onetwo_Both.end(),
                                    Gammas_MCIndex_onetwo_Target.begin(),
                                    Gammas_MCIndex_onetwo_Target.end());

  Gammas_MC_all_Both.insert(Gammas_MC_all_Both.end(),
                            Gammas_MC_all_Outside.begin(),
                            Gammas_MC_all_Outside.end());
  Gammas_MC_all_Both.insert(Gammas_MC_all_Both.end(),
                            Gammas_MC_all_Target.begin(),
                            Gammas_MC_all_Target.end());
  Gammas_MC_zero_Both.insert(Gammas_MC_zero_Both.end(),
                             Gammas_MC_zero_Outside.begin(),
                             Gammas_MC_zero_Outside.end());
  Gammas_MC_zero_Both.insert(Gammas_MC_zero_Both.end(),
                             Gammas_MC_zero_Target.begin(),
                             Gammas_MC_zero_Target.end());
  Gammas_MC_one_Both.insert(Gammas_MC_one_Both.end(),
                            Gammas_MC_one_Outside.begin(),
                            Gammas_MC_one_Outside.end());
  Gammas_MC_one_Both.insert(Gammas_MC_one_Both.end(),
                            Gammas_MC_one_Target.begin(),
                            Gammas_MC_one_Target.end());
  Gammas_MC_two_Both.insert(Gammas_MC_two_Both.end(),
                            Gammas_MC_two_Outside.begin(),
                            Gammas_MC_two_Outside.end());
  Gammas_MC_two_Both.insert(Gammas_MC_two_Both.end(),
                            Gammas_MC_two_Target.begin(),
                            Gammas_MC_two_Target.end());
  Gammas_MC_onetwo_Both.insert(Gammas_MC_onetwo_Both.end(),
                               Gammas_MC_onetwo_Outside.begin(),
                               Gammas_MC_onetwo_Outside.end());
  Gammas_MC_onetwo_Both.insert(Gammas_MC_onetwo_Both.end(),
                               Gammas_MC_onetwo_Target.begin(),
...
void CbmKresConversionManual::FindGammasBothWithAdditionalCuts() {
  ...
  Gamma_WAC.insert(
    Gamma_WAC.end(), Gammas_two_Target.begin(), Gammas_two_Target.end());
  Gamma_WAC.insert(
    Gamma_WAC.end(), Gammas_all_Outside.begin(), Gammas_all_Outside.end());
  Gammas_stsIndex_WAC.insert(Gammas_stsIndex_WAC.end(),
                             Gammas_stsIndex_two_Target.begin(),
                             Gammas_stsIndex_two_Target.end());
  Gammas_stsIndex_WAC.insert(Gammas_stsIndex_WAC.end(),
                             Gammas_stsIndex_all_Outside.begin(),
                             Gammas_stsIndex_all_Outside.end());
  Gammas_MCIndex_WAC.insert(Gammas_MCIndex_WAC.end(),
                            Gammas_MCIndex_two_Target.begin(),
                            Gammas_MCIndex_two_Target.end());
  Gammas_MCIndex_WAC.insert(Gammas_MCIndex_WAC.end(),
                            Gammas_MCIndex_all_Outside.begin(),
                            Gammas_MCIndex_all_Outside.end());
  Gammas_MC_WAC.insert(Gammas_MC_WAC.end(),
                       Gammas_MC_two_Target.begin(),
                       Gammas_MC_two_Target.end());
  Gammas_MC_WAC.insert(Gammas_MC_WAC.end(),
                       Gammas_MC_all_Outside.begin(),
  ...
  for (size_t gamma1 = 0; gamma1 < Gammas.size() - 1; gamma1++) {
    for (size_t gamma2 = gamma1 + 1; gamma2 < Gammas.size(); gamma2++) {
      ...
      if (mod == "OneTwo" && position == "Both") {
        ...
        // only pt
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          pt_onetwo_1->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          pt_onetwo_2->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          pt_onetwo_3->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          pt_onetwo_4->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          pt_onetwo_5->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          pt_onetwo_6->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          pt_onetwo_7->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          pt_onetwo_8->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          pt_onetwo_9->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          pt_onetwo_10->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          pt_onetwo_11->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          pt_onetwo_12->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          pt_onetwo_13->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          pt_onetwo_14->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          pt_onetwo_15->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          pt_onetwo_16->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          pt_onetwo_17->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          pt_onetwo_18->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          pt_onetwo_19->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          pt_onetwo_20->Fill(params.fMinv);

        if (params.fRapidity > 1.2 && params.fRapidity <= 1.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_1->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_2->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_3->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_4->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_5->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_6->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_7->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_8->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_9->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_10->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_11->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_12->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_13->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_14->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_15->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_16->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_17->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_18->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_19->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_20->Fill(params.fMinv);
        }
        if (params.fRapidity > 1.6 && params.fRapidity <= 2.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_21->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_22->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_23->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_24->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_25->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_26->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_27->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_28->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_29->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_30->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_31->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_32->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_33->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_34->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_35->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_36->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_37->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_38->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_39->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_40->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.0 && params.fRapidity <= 2.4) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_41->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_42->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_43->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_44->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_45->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_46->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_47->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_48->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_49->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_50->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_51->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_52->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_53->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_54->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_55->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_56->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_57->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_58->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_59->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_60->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.4 && params.fRapidity <= 2.8) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_61->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_62->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_63->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_64->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_65->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_66->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_67->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_68->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_69->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_70->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_71->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_72->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_73->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_74->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_75->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_76->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.8 && params.fRapidity <= 3.2) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_81->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_82->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_83->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_84->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_85->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_86->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_87->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_88->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_89->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_90->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_91->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_92->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.2 && params.fRapidity <= 3.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_101->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_102->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_103->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_104->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_105->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_106->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_107->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_108->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.6 && params.fRapidity <= 4.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_111->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_112->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_113->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_114->Fill(params.fMinv);
        }
      }
      ...
      if (mod == "All" && position == "Both") {
        ...
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          pt_all_10->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          pt_all_11->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          pt_all_12->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          pt_all_13->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          pt_all_14->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          pt_all_15->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          pt_all_16->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          pt_all_17->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          pt_all_18->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          pt_all_19->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          pt_all_20->Fill(params.fMinv);

        if (params.fRapidity > 1.2 && params.fRapidity <= 1.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_1->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_2->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_3->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_4->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_5->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_6->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_7->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_8->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_all_9->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_all_10->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_all_11->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_all_12->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_all_13->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_all_14->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_all_15->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_all_16->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_all_17->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_all_18->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_all_19->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_all_20->Fill(params.fMinv);
        }
        if (params.fRapidity > 1.6 && params.fRapidity <= 2.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_21->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_22->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_23->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_24->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_25->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_26->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_27->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_28->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_all_29->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_all_30->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_all_31->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_all_32->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_all_33->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_all_34->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_all_35->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_all_36->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_all_37->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_all_38->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_all_39->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_all_40->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.0 && params.fRapidity <= 2.4) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_41->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_42->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_43->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_44->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_45->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_46->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_47->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_48->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_all_49->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_all_50->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_all_51->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_all_52->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_all_53->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_all_54->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_all_55->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_all_56->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_all_57->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_all_58->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_all_59->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_all_60->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.4 && params.fRapidity <= 2.8) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_61->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_62->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_63->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_64->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_65->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_66->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_67->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_68->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_all_69->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_all_70->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_all_71->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_all_72->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_all_73->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_all_74->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_all_75->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_all_76->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.8 && params.fRapidity <= 3.2) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_81->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_82->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_83->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_84->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_85->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_86->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_87->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_88->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_all_89->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_all_90->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_all_91->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_all_92->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.2 && params.fRapidity <= 3.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_101->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_102->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_103->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_104->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_all_105->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_all_106->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_all_107->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_all_108->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.6 && params.fRapidity <= 4.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_all_111->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_all_112->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_all_113->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_all_114->Fill(params.fMinv);
        }
      }
      ...
      if (STSmcId1 == STSmcId2 || STSmcId1 == STSmcId3 || STSmcId1 == STSmcId4
          || STSmcId2 == STSmcId3 || STSmcId2 == STSmcId4
          || STSmcId3 == STSmcId4)
      ...
      if (mcMotherPdg1 == 22 && mcMotherPdg2 == 22 && mcMotherPdg3 == 111
        ...
        if (mod == "OneTwo" && position == "Both")
          Pi0_pt_est_onetwo_Both->Fill(params.fPt);
        if (mod == "All" && position == "Both")
          Pi0_pt_est_all_Both->Fill(params.fPt);
  ...
  for (Int_t a = 0; a < nof_Target - 1; a++) {
    for (Int_t b = a + 1; b < nof_Target; b++) {
      ...
      if ((EMT_man_NofRings_Target[a] == 1 || EMT_man_NofRings_Target[a] == 2)
          && (EMT_man_NofRings_Target[b] == 1
              || EMT_man_NofRings_Target[b] == 2))
  ...
  for (Int_t a = 0; a < nof_Outside - 1; a++) {
    for (Int_t b = a + 1; b < nof_Outside; b++) {
      ...
      if ((EMT_man_NofRings_Outside[a] == 1 || EMT_man_NofRings_Outside[a] == 2)
          && (EMT_man_NofRings_Outside[b] == 1
              || EMT_man_NofRings_Outside[b] == 2))
...
// BOTH
{
  ...
  for (Int_t a = 0; a < nof_Both - 1; a++) {
    for (Int_t b = a + 1; b < nof_Both; b++) {
      ...
      if (EMT_man_NofRings_Both[a] == 0 && EMT_man_NofRings_Both[b] == 0)
        EMT_InvMass_zero_Both->Fill(params.fMinv);
      if (EMT_man_NofRings_Both[a] == 1 && EMT_man_NofRings_Both[b] == 1)
        EMT_InvMass_one_Both->Fill(params.fMinv);
      if (EMT_man_NofRings_Both[a] == 2 && EMT_man_NofRings_Both[b] == 2)
        EMT_InvMass_two_Both->Fill(params.fMinv);
      ...
      if ((EMT_man_NofRings_Both[a] == 1 || EMT_man_NofRings_Both[a] == 2)
        ...
        if (params.fRapidity > 1.2 && params.fRapidity <= 1.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_1->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_2->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_3->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_4->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_5->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_6->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_7->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_8->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_mixing_9->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_mixing_10->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_mixing_11->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_mixing_12->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_mixing_13->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_mixing_14->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_mixing_15->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_mixing_16->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_mixing_17->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_mixing_18->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_mixing_19->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_mixing_20->Fill(params.fMinv);
        }
        if (params.fRapidity > 1.6 && params.fRapidity <= 2.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_21->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_22->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_23->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_24->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_25->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_26->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_27->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_28->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_mixing_29->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_mixing_30->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_mixing_31->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_mixing_32->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_mixing_33->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_mixing_34->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_mixing_35->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_mixing_36->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_mixing_37->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_mixing_38->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_mixing_39->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_mixing_40->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.0 && params.fRapidity <= 2.4) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_41->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_42->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_43->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_44->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_45->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_46->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_47->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_48->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_mixing_49->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_mixing_50->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_mixing_51->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_mixing_52->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_mixing_53->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_mixing_54->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_mixing_55->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_mixing_56->Fill(params.fMinv);
          if (params.fPt > 1.6 && params.fPt <= 1.7)
            rap_vs_Pt_InM_mixing_57->Fill(params.fMinv);
          if (params.fPt > 1.7 && params.fPt <= 1.8)
            rap_vs_Pt_InM_mixing_58->Fill(params.fMinv);
          if (params.fPt > 1.8 && params.fPt <= 1.9)
            rap_vs_Pt_InM_mixing_59->Fill(params.fMinv);
          if (params.fPt > 1.9 && params.fPt <= 2.0)
            rap_vs_Pt_InM_mixing_60->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.4 && params.fRapidity <= 2.8) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_61->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_62->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_63->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_64->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_65->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_66->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_67->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_68->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_mixing_69->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_mixing_70->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_mixing_71->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_mixing_72->Fill(params.fMinv);
          if (params.fPt > 1.2 && params.fPt <= 1.3)
            rap_vs_Pt_InM_mixing_73->Fill(params.fMinv);
          if (params.fPt > 1.3 && params.fPt <= 1.4)
            rap_vs_Pt_InM_mixing_74->Fill(params.fMinv);
          if (params.fPt > 1.4 && params.fPt <= 1.5)
            rap_vs_Pt_InM_mixing_75->Fill(params.fMinv);
          if (params.fPt > 1.5 && params.fPt <= 1.6)
            rap_vs_Pt_InM_mixing_76->Fill(params.fMinv);
        }
        if (params.fRapidity > 2.8 && params.fRapidity <= 3.2) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_81->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_82->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_83->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_84->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_85->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_86->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_87->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_88->Fill(params.fMinv);
          if (params.fPt > 0.8 && params.fPt <= 0.9)
            rap_vs_Pt_InM_mixing_89->Fill(params.fMinv);
          if (params.fPt > 0.9 && params.fPt <= 1.0)
            rap_vs_Pt_InM_mixing_90->Fill(params.fMinv);
          if (params.fPt > 1.0 && params.fPt <= 1.1)
            rap_vs_Pt_InM_mixing_91->Fill(params.fMinv);
          if (params.fPt > 1.1 && params.fPt <= 1.2)
            rap_vs_Pt_InM_mixing_92->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.2 && params.fRapidity <= 3.6) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_101->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_102->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_103->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_104->Fill(params.fMinv);
          if (params.fPt > 0.4 && params.fPt <= 0.5)
            rap_vs_Pt_InM_mixing_105->Fill(params.fMinv);
          if (params.fPt > 0.5 && params.fPt <= 0.6)
            rap_vs_Pt_InM_mixing_106->Fill(params.fMinv);
          if (params.fPt > 0.6 && params.fPt <= 0.7)
            rap_vs_Pt_InM_mixing_107->Fill(params.fMinv);
          if (params.fPt > 0.7 && params.fPt <= 0.8)
            rap_vs_Pt_InM_mixing_108->Fill(params.fMinv);
        }
        if (params.fRapidity > 3.6 && params.fRapidity <= 4.0) {
          if (params.fPt > 0.0 && params.fPt <= 0.1)
            rap_vs_Pt_InM_mixing_111->Fill(params.fMinv);
          if (params.fPt > 0.1 && params.fPt <= 0.2)
            rap_vs_Pt_InM_mixing_112->Fill(params.fMinv);
          if (params.fPt > 0.2 && params.fPt <= 0.3)
            rap_vs_Pt_InM_mixing_113->Fill(params.fMinv);
          if (params.fPt > 0.3 && params.fPt <= 0.4)
            rap_vs_Pt_InM_mixing_114->Fill(params.fMinv);
        }
      }
      ...
      if (params.fRapidity > 1.2 && params.fRapidity <= 1.6) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_1->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_2->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_3->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_4->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_5->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_6->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_7->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_8->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          rap_vs_Pt_InM_all_mixing_9->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          rap_vs_Pt_InM_all_mixing_10->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          rap_vs_Pt_InM_all_mixing_11->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          rap_vs_Pt_InM_all_mixing_12->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          rap_vs_Pt_InM_all_mixing_13->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          rap_vs_Pt_InM_all_mixing_14->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          rap_vs_Pt_InM_all_mixing_15->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          rap_vs_Pt_InM_all_mixing_16->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          rap_vs_Pt_InM_all_mixing_17->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          rap_vs_Pt_InM_all_mixing_18->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          rap_vs_Pt_InM_all_mixing_19->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          rap_vs_Pt_InM_all_mixing_20->Fill(params.fMinv);
      }
      if (params.fRapidity > 1.6 && params.fRapidity <= 2.0) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_21->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_22->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_23->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_24->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_25->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_26->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_27->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_28->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          rap_vs_Pt_InM_all_mixing_29->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          rap_vs_Pt_InM_all_mixing_30->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          rap_vs_Pt_InM_all_mixing_31->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          rap_vs_Pt_InM_all_mixing_32->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          rap_vs_Pt_InM_all_mixing_33->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          rap_vs_Pt_InM_all_mixing_34->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          rap_vs_Pt_InM_all_mixing_35->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          rap_vs_Pt_InM_all_mixing_36->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          rap_vs_Pt_InM_all_mixing_37->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          rap_vs_Pt_InM_all_mixing_38->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          rap_vs_Pt_InM_all_mixing_39->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          rap_vs_Pt_InM_all_mixing_40->Fill(params.fMinv);
      }
      if (params.fRapidity > 2.0 && params.fRapidity <= 2.4) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_41->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_42->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_43->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_44->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_45->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_46->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_47->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_48->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          rap_vs_Pt_InM_all_mixing_49->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          rap_vs_Pt_InM_all_mixing_50->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          rap_vs_Pt_InM_all_mixing_51->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          rap_vs_Pt_InM_all_mixing_52->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          rap_vs_Pt_InM_all_mixing_53->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          rap_vs_Pt_InM_all_mixing_54->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          rap_vs_Pt_InM_all_mixing_55->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          rap_vs_Pt_InM_all_mixing_56->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          rap_vs_Pt_InM_all_mixing_57->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          rap_vs_Pt_InM_all_mixing_58->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          rap_vs_Pt_InM_all_mixing_59->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          rap_vs_Pt_InM_all_mixing_60->Fill(params.fMinv);
      }
      if (params.fRapidity > 2.4 && params.fRapidity <= 2.8) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_61->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_62->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_63->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_64->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_65->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_66->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_67->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_68->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          rap_vs_Pt_InM_all_mixing_69->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          rap_vs_Pt_InM_all_mixing_70->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          rap_vs_Pt_InM_all_mixing_71->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          rap_vs_Pt_InM_all_mixing_72->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          rap_vs_Pt_InM_all_mixing_73->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          rap_vs_Pt_InM_all_mixing_74->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          rap_vs_Pt_InM_all_mixing_75->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          rap_vs_Pt_InM_all_mixing_76->Fill(params.fMinv);
      }
      if (params.fRapidity > 2.8 && params.fRapidity <= 3.2) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_81->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_82->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_83->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_84->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_85->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_86->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_87->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_88->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          rap_vs_Pt_InM_all_mixing_89->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          rap_vs_Pt_InM_all_mixing_90->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          rap_vs_Pt_InM_all_mixing_91->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          rap_vs_Pt_InM_all_mixing_92->Fill(params.fMinv);
      }
      if (params.fRapidity > 3.2 && params.fRapidity <= 3.6) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_101->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_102->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_103->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_104->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          rap_vs_Pt_InM_all_mixing_105->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          rap_vs_Pt_InM_all_mixing_106->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          rap_vs_Pt_InM_all_mixing_107->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          rap_vs_Pt_InM_all_mixing_108->Fill(params.fMinv);
      }
      if (params.fRapidity > 3.6 && params.fRapidity <= 4.0) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          rap_vs_Pt_InM_all_mixing_111->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          rap_vs_Pt_InM_all_mixing_112->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          rap_vs_Pt_InM_all_mixing_113->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          rap_vs_Pt_InM_all_mixing_114->Fill(params.fMinv);
      }
      ...
      if ((EMT_man_NofRings_Both[a] == 1 || EMT_man_NofRings_Both[a] == 2)
          && (EMT_man_NofRings_Both[b] == 1 || EMT_man_NofRings_Both[b] == 2)) {
        if (params.fPt > 0.0 && params.fPt <= 0.1)
          pt_onetwo_mixing_1->Fill(params.fMinv);
        if (params.fPt > 0.1 && params.fPt <= 0.2)
          pt_onetwo_mixing_2->Fill(params.fMinv);
        if (params.fPt > 0.2 && params.fPt <= 0.3)
          pt_onetwo_mixing_3->Fill(params.fMinv);
        if (params.fPt > 0.3 && params.fPt <= 0.4)
          pt_onetwo_mixing_4->Fill(params.fMinv);
        if (params.fPt > 0.4 && params.fPt <= 0.5)
          pt_onetwo_mixing_5->Fill(params.fMinv);
        if (params.fPt > 0.5 && params.fPt <= 0.6)
          pt_onetwo_mixing_6->Fill(params.fMinv);
        if (params.fPt > 0.6 && params.fPt <= 0.7)
          pt_onetwo_mixing_7->Fill(params.fMinv);
        if (params.fPt > 0.7 && params.fPt <= 0.8)
          pt_onetwo_mixing_8->Fill(params.fMinv);
        if (params.fPt > 0.8 && params.fPt <= 0.9)
          pt_onetwo_mixing_9->Fill(params.fMinv);
        if (params.fPt > 0.9 && params.fPt <= 1.0)
          pt_onetwo_mixing_10->Fill(params.fMinv);
        if (params.fPt > 1.0 && params.fPt <= 1.1)
          pt_onetwo_mixing_11->Fill(params.fMinv);
        if (params.fPt > 1.1 && params.fPt <= 1.2)
          pt_onetwo_mixing_12->Fill(params.fMinv);
        if (params.fPt > 1.2 && params.fPt <= 1.3)
          pt_onetwo_mixing_13->Fill(params.fMinv);
        if (params.fPt > 1.3 && params.fPt <= 1.4)
          pt_onetwo_mixing_14->Fill(params.fMinv);
        if (params.fPt > 1.4 && params.fPt <= 1.5)
          pt_onetwo_mixing_15->Fill(params.fMinv);
        if (params.fPt > 1.5 && params.fPt <= 1.6)
          pt_onetwo_mixing_16->Fill(params.fMinv);
        if (params.fPt > 1.6 && params.fPt <= 1.7)
          pt_onetwo_mixing_17->Fill(params.fMinv);
        if (params.fPt > 1.7 && params.fPt <= 1.8)
          pt_onetwo_mixing_18->Fill(params.fMinv);
        if (params.fPt > 1.8 && params.fPt <= 1.9)
          pt_onetwo_mixing_19->Fill(params.fMinv);
        if (params.fPt > 1.9 && params.fPt <= 2.0)
          pt_onetwo_mixing_20->Fill(params.fMinv);
      }

      if (params.fPt > 0.0 && params.fPt <= 0.1)
        pt_all_mixing_1->Fill(params.fMinv);
      if (params.fPt > 0.1 && params.fPt <= 0.2)
        pt_all_mixing_2->Fill(params.fMinv);
      if (params.fPt > 0.2 && params.fPt <= 0.3)
        pt_all_mixing_3->Fill(params.fMinv);
      if (params.fPt > 0.3 && params.fPt <= 0.4)
        pt_all_mixing_4->Fill(params.fMinv);
      if (params.fPt > 0.4 && params.fPt <= 0.5)
        pt_all_mixing_5->Fill(params.fMinv);
      if (params.fPt > 0.5 && params.fPt <= 0.6)
        pt_all_mixing_6->Fill(params.fMinv);
      if (params.fPt > 0.6 && params.fPt <= 0.7)
        pt_all_mixing_7->Fill(params.fMinv);
      if (params.fPt > 0.7 && params.fPt <= 0.8)
        pt_all_mixing_8->Fill(params.fMinv);
      if (params.fPt > 0.8 && params.fPt <= 0.9)
        pt_all_mixing_9->Fill(params.fMinv);
      if (params.fPt > 0.9 && params.fPt <= 1.0)
        pt_all_mixing_10->Fill(params.fMinv);
      if (params.fPt > 1.0 && params.fPt <= 1.1)
        pt_all_mixing_11->Fill(params.fMinv);
      if (params.fPt > 1.1 && params.fPt <= 1.2)
        pt_all_mixing_12->Fill(params.fMinv);
      if (params.fPt > 1.2 && params.fPt <= 1.3)
        pt_all_mixing_13->Fill(params.fMinv);
      if (params.fPt > 1.3 && params.fPt <= 1.4)
        pt_all_mixing_14->Fill(params.fMinv);
      if (params.fPt > 1.4 && params.fPt <= 1.5)
        pt_all_mixing_15->Fill(params.fMinv);
      if (params.fPt > 1.5 && params.fPt <= 1.6)
        pt_all_mixing_16->Fill(params.fMinv);
      if (params.fPt > 1.6 && params.fPt <= 1.7)
        pt_all_mixing_17->Fill(params.fMinv);
      if (params.fPt > 1.7 && params.fPt <= 1.8)
        pt_all_mixing_18->Fill(params.fMinv);
      if (params.fPt > 1.8 && params.fPt <= 1.9)
        pt_all_mixing_19->Fill(params.fMinv);
      if (params.fPt > 1.9 && params.fPt <= 2.0)
        pt_all_mixing_20->Fill(params.fMinv);
    }
  }
}
...
int CbmKresConversionManual::CheckIfElectron(CbmRichRing* ring,
                                             double momentum) {
  int identified = 0;
...
double CbmKresConversionManual::CalculatePlaneAngle_last(CbmStsTrack* Sts_1,
                                                         CbmStsTrack* Sts_2) {
...
double CbmKresConversionManual::CalculatePlaneAngle_first(CbmStsTrack* Sts_1,
                                                          CbmStsTrack* Sts_2) {
  double FinalAngle = 400;
  Int_t hits1sts    = Sts_1->GetNofStsHits();
  Int_t hits2sts    = Sts_2->GetNofStsHits();
...
void CbmKresConversionManual::InitHistograms() {
  ...
  InvMass_vs_OA_candidates_Both =
    new TH2D("InvMass_vs_OA_candidates_Both",
  ...
  InvMass_vs_OA_fromPi0_Both =
    new TH2D("InvMass_vs_OA_fromPi0_Both",
  ...
  GammasInvMass_candidates_Both =
    new TH1D("GammasInvMass_candidates_Both",
             "GammasInvMass_candidates_Both; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Both.push_back(GammasInvMass_candidates_Both);
  GammasOA_candidates_Both =
    new TH1D("GammasOA_candidates_Both",
             "GammasOA_candidates_Both; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Both.push_back(GammasOA_candidates_Both);
  GammasInvMass_fromPi0_Both =
    new TH1D("GammasInvMass_fromPi0_Both",
             "GammasInvMass_fromPi0_Both; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Both.push_back(GammasInvMass_fromPi0_Both);
  GammasOA_fromPi0_Both =
    new TH1D("GammasOA_fromPi0_Both",
             "GammasOA_fromPi0_Both; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Both.push_back(GammasOA_fromPi0_Both);
  PlaneAngles_last_candidates_Both =
    new TH1D("PlaneAngles_last_candidates_Both",
             "PlaneAngles_last_candidates_Both; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Both.push_back(PlaneAngles_last_candidates_Both);
  PlaneAngles_last_fromPi0_Both =
    new TH1D("PlaneAngles_last_fromPi0_Both",
             "PlaneAngles_last_fromPi0_Both; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Both.push_back(PlaneAngles_last_fromPi0_Both);
  PlaneAngles_first_candidates_Both =
    new TH1D("PlaneAngles_first_candidates_Both",
             "PlaneAngles_first_candidates_Both; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Both.push_back(PlaneAngles_first_candidates_Both);
  PlaneAngles_first_fromPi0_Both =
    new TH1D("PlaneAngles_first_fromPi0_Both",
             "PlaneAngles_first_fromPi0_Both; #theta angle in degree;#",
             720,
             -1.,
             179.);
  ...
  InvMass_vs_OA_candidates_Target =
    new TH2D("InvMass_vs_OA_candidates_Target",
  ...
  InvMass_vs_OA_fromPi0_Target =
    new TH2D("InvMass_vs_OA_fromPi0_Target",
  ...
  GammasInvMass_candidates_Target =
    new TH1D("GammasInvMass_candidates_Target",
             "GammasInvMass_candidates_Target; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Target.push_back(GammasInvMass_candidates_Target);
  GammasOA_candidates_Target =
    new TH1D("GammasOA_candidates_Target",
             "GammasOA_candidates_Target; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Target.push_back(GammasOA_candidates_Target);
  GammasInvMass_fromPi0_Target =
    new TH1D("GammasInvMass_fromPi0_Target",
             "GammasInvMass_fromPi0_Target; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Target.push_back(GammasInvMass_fromPi0_Target);
  GammasOA_fromPi0_Target =
    new TH1D("GammasOA_fromPi0_Target",
             "GammasOA_fromPi0_Target; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Target.push_back(GammasOA_fromPi0_Target);
  PlaneAngles_last_candidates_Target =
    new TH1D("PlaneAngles_last_candidates_Target",
             "PlaneAngles_last_candidates_Target; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Target.push_back(PlaneAngles_last_candidates_Target);
  PlaneAngles_last_fromPi0_Target =
    new TH1D("PlaneAngles_last_fromPi0_Target",
             "PlaneAngles_last_fromPi0_Target; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Target.push_back(PlaneAngles_last_fromPi0_Target);
  PlaneAngles_first_candidates_Target =
    new TH1D("PlaneAngles_first_candidates_Target",
  ...
  PlaneAngles_first_fromPi0_Target =
    new TH1D("PlaneAngles_first_fromPi0_Target",
             "PlaneAngles_first_fromPi0_Target; #theta angle in degree;#",
             720,
             -1.,
             179.);
  ...
  InvMass_vs_OA_candidates_Outside =
    new TH2D("InvMass_vs_OA_candidates_Outside",
  ...
  InvMass_vs_OA_fromPi0_Outside =
    new TH2D("InvMass_vs_OA_fromPi0_Outside",
  ...
  GammasInvMass_candidates_Outside =
    new TH1D("GammasInvMass_candidates_Outside",
             "GammasInvMass_candidates_Outside; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Outside.push_back(GammasInvMass_candidates_Outside);
  GammasOA_candidates_Outside =
    new TH1D("GammasOA_candidates_Outside",
             "GammasOA_candidates_Outside; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Outside.push_back(GammasOA_candidates_Outside);
  GammasInvMass_fromPi0_Outside =
    new TH1D("GammasInvMass_fromPi0_Outside",
             "GammasInvMass_fromPi0_Outside; invariant mass in GeV/c^{2};#",
             510,
             -0.01,
             0.5);
  fHistoList_man_cuts_Outside.push_back(GammasInvMass_fromPi0_Outside);
  GammasOA_fromPi0_Outside =
    new TH1D("GammasOA_fromPi0_Outside",
             "GammasOA_fromPi0_Outside; opening angle in degree;#",
             300,
             -0.1,
             29.9);
  fHistoList_man_cuts_Outside.push_back(GammasOA_fromPi0_Outside);
  PlaneAngles_last_candidates_Outside =
    new TH1D("PlaneAngles_last_candidates_Outside",
  ...
  PlaneAngles_last_fromPi0_Outside =
    new TH1D("PlaneAngles_last_fromPi0_Outside",
             "PlaneAngles_last_fromPi0_Outside; #theta angle in degree;#",
             720,
             -1.,
             179.);
  fHistoList_man_cuts_Outside.push_back(PlaneAngles_last_fromPi0_Outside);
  PlaneAngles_first_candidates_Outside =
    new TH1D("PlaneAngles_first_candidates_Outside",
  ...
  PlaneAngles_first_fromPi0_Outside =
    new TH1D("PlaneAngles_first_fromPi0_Outside",
             "PlaneAngles_first_fromPi0_Outside; #theta angle in degree;#",
             720,
             -1.,
             179.);
  ...
  GammaInvMassReco_all_Target =
    new TH1D("GammaInvMassReco_all_Target",
             "GammaInvMassReco_all_Target; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_all_Target.push_back(GammaInvMassReco_all_Target);
  GammaOpeningAngleReco_all_Target =
    new TH1D("GammaOpeningAngleReco_all_Target",
             "GammaOpeningAngleReco_all_Target; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_all_Target.push_back(GammaOpeningAngleReco_all_Target);
  Pdg_all_Target =
    new TH1D("Pdg_all_Target", "Pdg_all_Target; Id;#", 800, 0, 400);
  fHistoList_man_all_Target.push_back(Pdg_all_Target);
  P_reco_all_Target = new TH1D(
    "P_reco_all_Target", "P_reco_all_Target; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_all_Target.push_back(P_reco_all_Target);
  Pt_reco_all_Target = new TH1D("Pt_reco_all_Target",
                                "Pt_reco_all_Target; P_{t} in GeV/c^{2};#",
                                300,
                                0,
                                3);
  fHistoList_man_all_Target.push_back(Pt_reco_all_Target);
  Pi0InvMassReco_all_Target =
    new TH1D("Pi0InvMassReco_all_Target",
             "Pi0InvMassReco_all_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Target.push_back(Pi0InvMassReco_all_Target);
  EMT_InvMass_all_Target =
    new TH1D("EMT_InvMass_all_Target",
             "EMT_InvMass_all_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Target.push_back(EMT_InvMass_all_Target);
  Pi0_pt_vs_rap_all_Target =
    new TH2D("Pi0_pt_vs_rap_all_Target",
             "Pi0_pt_vs_rap_all_Target; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_all_Target =
    new TH2D("Pi0_pt_vs_rap_est_all_Target",
             "Pi0_pt_vs_rap_est_all_Target; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_all_Target =
    new TH1D("DalitzPi0_all_Target",
             "DalitzPi0_all_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Target.push_back(DalitzPi0_all_Target);
  PhotonsPi0_all_Target =
    new TH1D("PhotonsPi0_all_Target",
             "PhotonsPi0_all_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_zero_Target =
    new TH1D("GammaInvMassReco_zero_Target",
             "GammaInvMassReco_zero_Target; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_zero_Target.push_back(GammaInvMassReco_zero_Target);
  GammaOpeningAngleReco_zero_Target =
    new TH1D("GammaOpeningAngleReco_zero_Target",
             "GammaOpeningAngleReco_zero_Target; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_zero_Target.push_back(GammaOpeningAngleReco_zero_Target);
  Pdg_zero_Target =
    new TH1D("Pdg_zero_Target", "Pdg_zero_Target; Id;#", 800, 0, 400);
  fHistoList_man_zero_Target.push_back(Pdg_zero_Target);
  P_reco_zero_Target = new TH1D(
    "P_reco_zero_Target", "P_reco_zero_Target; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_zero_Target.push_back(P_reco_zero_Target);
  Pt_reco_zero_Target = new TH1D("Pt_reco_zero_Target",
                                 "Pt_reco_zero_Target; P_{t} in GeV/c^{2};#",
                                 300,
                                 0,
                                 3);
  fHistoList_man_zero_Target.push_back(Pt_reco_zero_Target);
  Pi0InvMassReco_zero_Target =
    new TH1D("Pi0InvMassReco_zero_Target",
             "Pi0InvMassReco_zero_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Target.push_back(Pi0InvMassReco_zero_Target);
  EMT_InvMass_zero_Target =
    new TH1D("EMT_InvMass_zero_Target",
             "EMT_InvMass_zero_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Target.push_back(EMT_InvMass_zero_Target);
  Pi0_pt_vs_rap_zero_Target =
    new TH2D("Pi0_pt_vs_rap_zero_Target",
             "Pi0_pt_vs_rap_zero_Target; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_zero_Target =
    new TH2D("Pi0_pt_vs_rap_est_zero_Target",
             "Pi0_pt_vs_rap_est_zero_Target; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_zero_Target =
    new TH1D("DalitzPi0_zero_Target",
             "DalitzPi0_zero_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Target.push_back(DalitzPi0_zero_Target);
  PhotonsPi0_zero_Target =
    new TH1D("PhotonsPi0_zero_Target",
             "PhotonsPi0_zero_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_one_Target =
    new TH1D("GammaInvMassReco_one_Target",
             "GammaInvMassReco_one_Target; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_one_Target.push_back(GammaInvMassReco_one_Target);
  GammaOpeningAngleReco_one_Target =
    new TH1D("GammaOpeningAngleReco_one_Target",
             "GammaOpeningAngleReco_one_Target; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_one_Target.push_back(GammaOpeningAngleReco_one_Target);
  Pdg_one_Target =
    new TH1D("Pdg_one_Target", "Pdg_one_Target; Id;#", 800, 0, 400);
  fHistoList_man_one_Target.push_back(Pdg_one_Target);
  P_reco_one_Target = new TH1D(
    "P_reco_one_Target", "P_reco_one_Target; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_one_Target.push_back(P_reco_one_Target);
  Pt_reco_one_Target = new TH1D("Pt_reco_one_Target",
                                "Pt_reco_one_Target; P_{t} in GeV/c^{2};#",
                                300,
                                0,
                                3);
  fHistoList_man_one_Target.push_back(Pt_reco_one_Target);
  Pi0InvMassReco_one_Target =
    new TH1D("Pi0InvMassReco_one_Target",
             "Pi0InvMassReco_one_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Target.push_back(Pi0InvMassReco_one_Target);
  EMT_InvMass_one_Target =
    new TH1D("EMT_InvMass_one_Target",
             "EMT_InvMass_one_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Target.push_back(EMT_InvMass_one_Target);
  Pi0_pt_vs_rap_one_Target =
    new TH2D("Pi0_pt_vs_rap_one_Target",
             "Pi0_pt_vs_rap_one_Target; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_one_Target =
    new TH2D("Pi0_pt_vs_rap_est_one_Target",
             "Pi0_pt_vs_rap_est_one_Target; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_one_Target =
    new TH1D("DalitzPi0_one_Target",
             "DalitzPi0_one_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Target.push_back(DalitzPi0_one_Target);
  PhotonsPi0_one_Target =
    new TH1D("PhotonsPi0_one_Target",
             "PhotonsPi0_one_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_two_Target =
    new TH1D("GammaInvMassReco_two_Target",
             "GammaInvMassReco_two_Target; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_two_Target.push_back(GammaInvMassReco_two_Target);
  GammaOpeningAngleReco_two_Target =
    new TH1D("GammaOpeningAngleReco_two_Target",
             "GammaOpeningAngleReco_two_Target; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_two_Target.push_back(GammaOpeningAngleReco_two_Target);
  Pdg_two_Target =
    new TH1D("Pdg_two_Target", "Pdg_two_Target; Id;#", 800, 0, 400);
  fHistoList_man_two_Target.push_back(Pdg_two_Target);
  P_reco_two_Target = new TH1D(
    "P_reco_two_Target", "P_reco_two_Target; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_two_Target.push_back(P_reco_two_Target);
  Pt_reco_two_Target = new TH1D("Pt_reco_two_Target",
                                "Pt_reco_two_Target; P_{t} in GeV/c^{2};#",
                                300,
                                0,
                                3);
  fHistoList_man_two_Target.push_back(Pt_reco_two_Target);
  Pi0InvMassReco_two_Target =
    new TH1D("Pi0InvMassReco_two_Target",
             "Pi0InvMassReco_two_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Target.push_back(Pi0InvMassReco_two_Target);
  EMT_InvMass_two_Target =
    new TH1D("EMT_InvMass_two_Target",
             "EMT_InvMass_two_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Target.push_back(EMT_InvMass_two_Target);
  Pi0_pt_vs_rap_two_Target =
    new TH2D("Pi0_pt_vs_rap_two_Target",
             "Pi0_pt_vs_rap_two_Target; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_two_Target =
    new TH2D("Pi0_pt_vs_rap_est_two_Target",
             "Pi0_pt_vs_rap_est_two_Target; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_two_Target =
    new TH1D("DalitzPi0_two_Target",
             "DalitzPi0_two_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Target.push_back(DalitzPi0_two_Target);
  PhotonsPi0_two_Target =
    new TH1D("PhotonsPi0_two_Target",
             "PhotonsPi0_two_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_onetwo_Target =
    new TH1D("GammaInvMassReco_onetwo_Target",
             "GammaInvMassReco_onetwo_Target; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_onetwo_Target.push_back(GammaInvMassReco_onetwo_Target);
  GammaOpeningAngleReco_onetwo_Target =
    new TH1D("GammaOpeningAngleReco_onetwo_Target",
             "GammaOpeningAngleReco_onetwo_Target; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_onetwo_Target.push_back(GammaOpeningAngleReco_onetwo_Target);
  Pdg_onetwo_Target =
    new TH1D("Pdg_onetwo_Target", "Pdg_onetwo_Target; Id;#", 800, 0, 400);
  fHistoList_man_onetwo_Target.push_back(Pdg_onetwo_Target);
  P_reco_onetwo_Target = new TH1D("P_reco_onetwo_Target",
                                  "P_reco_onetwo_Target; P in GeV/c^{2};#",
                                  600,
                                  0,
                                  6);
  fHistoList_man_onetwo_Target.push_back(P_reco_onetwo_Target);
  Pt_reco_onetwo_Target =
    new TH1D("Pt_reco_onetwo_Target",
             "Pt_reco_onetwo_Target; P_{t} in GeV/c^{2};#",
             300,
             0,
             3);
  fHistoList_man_onetwo_Target.push_back(Pt_reco_onetwo_Target);
  Pi0InvMassReco_onetwo_Target =
    new TH1D("Pi0InvMassReco_onetwo_Target",
             "Pi0InvMassReco_onetwo_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Target.push_back(Pi0InvMassReco_onetwo_Target);
  EMT_InvMass_onetwo_Target =
    new TH1D("EMT_InvMass_onetwo_Target",
             "EMT_InvMass_onetwo_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Target.push_back(EMT_InvMass_onetwo_Target);
  Pi0_pt_vs_rap_onetwo_Target =
    new TH2D("Pi0_pt_vs_rap_onetwo_Target",
             "Pi0_pt_vs_rap_onetwo_Target; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_onetwo_Target =
    new TH2D("Pi0_pt_vs_rap_est_onetwo_Target",
             "Pi0_pt_vs_rap_est_onetwo_Target; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_onetwo_Target =
    new TH1D("DalitzPi0_onetwo_Target",
             "DalitzPi0_onetwo_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Target.push_back(DalitzPi0_onetwo_Target);
  PhotonsPi0_onetwo_Target =
    new TH1D("PhotonsPi0_onetwo_Target",
             "PhotonsPi0_onetwo_Target; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_all_Outside =
    new TH1D("GammaInvMassReco_all_Outside",
             "GammaInvMassReco_all_Outside; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_all_Outside.push_back(GammaInvMassReco_all_Outside);
  GammaOpeningAngleReco_all_Outside =
    new TH1D("GammaOpeningAngleReco_all_Outside",
             "GammaOpeningAngleReco_all_Outside; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_all_Outside.push_back(GammaOpeningAngleReco_all_Outside);
  Pdg_all_Outside =
    new TH1D("Pdg_all_Outside", "Pdg_all_Outside; Id;#", 800, 0, 400);
  fHistoList_man_all_Outside.push_back(Pdg_all_Outside);
  P_reco_all_Outside = new TH1D(
    "P_reco_all_Outside", "P_reco_all_Outside; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_all_Outside.push_back(P_reco_all_Outside);
  Pt_reco_all_Outside = new TH1D("Pt_reco_all_Outside",
                                 "Pt_reco_all_Outside; P_{t} in GeV/c^{2};#",
                                 300,
                                 0,
                                 3);
  fHistoList_man_all_Outside.push_back(Pt_reco_all_Outside);
  Pi0InvMassReco_all_Outside =
    new TH1D("Pi0InvMassReco_all_Outside",
             "Pi0InvMassReco_all_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Outside.push_back(Pi0InvMassReco_all_Outside);
  EMT_InvMass_all_Outside =
    new TH1D("EMT_InvMass_all_Outside",
             "EMT_InvMass_all_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Outside.push_back(EMT_InvMass_all_Outside);
  Pi0_pt_vs_rap_all_Outside =
    new TH2D("Pi0_pt_vs_rap_all_Outside",
             "Pi0_pt_vs_rap_all_Outside; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_all_Outside =
    new TH2D("Pi0_pt_vs_rap_est_all_Outside",
             "Pi0_pt_vs_rap_est_all_Outside; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_all_Outside =
    new TH1D("DalitzPi0_all_Outside",
             "DalitzPi0_all_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Outside.push_back(DalitzPi0_all_Outside);
  PhotonsPi0_all_Outside =
    new TH1D("PhotonsPi0_all_Outside",
             "PhotonsPi0_all_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_zero_Outside =
    new TH1D("GammaInvMassReco_zero_Outside",
             "GammaInvMassReco_zero_Outside; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_zero_Outside.push_back(GammaInvMassReco_zero_Outside);
  GammaOpeningAngleReco_zero_Outside =
    new TH1D("GammaOpeningAngleReco_zero_Outside",
             "GammaOpeningAngleReco_zero_Outside; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_zero_Outside.push_back(GammaOpeningAngleReco_zero_Outside);
  Pdg_zero_Outside =
    new TH1D("Pdg_zero_Outside", "Pdg_zero_Outside; Id;#", 800, 0, 400);
  fHistoList_man_zero_Outside.push_back(Pdg_zero_Outside);
  P_reco_zero_Outside = new TH1D(
    "P_reco_zero_Outside", "P_reco_zero_Outside; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_zero_Outside.push_back(P_reco_zero_Outside);
  Pt_reco_zero_Outside = new TH1D("Pt_reco_zero_Outside",
                                  "Pt_reco_zero_Outside; P_{t} in GeV/c^{2};#",
                                  300,
                                  0,
                                  3);
  fHistoList_man_zero_Outside.push_back(Pt_reco_zero_Outside);
  Pi0InvMassReco_zero_Outside =
    new TH1D("Pi0InvMassReco_zero_Outside",
             "Pi0InvMassReco_zero_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Outside.push_back(Pi0InvMassReco_zero_Outside);
  EMT_InvMass_zero_Outside =
    new TH1D("EMT_InvMass_zero_Outside",
             "EMT_InvMass_zero_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Outside.push_back(EMT_InvMass_zero_Outside);
  Pi0_pt_vs_rap_zero_Outside =
    new TH2D("Pi0_pt_vs_rap_zero_Outside",
             "Pi0_pt_vs_rap_zero_Outside; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_zero_Outside =
    new TH2D("Pi0_pt_vs_rap_est_zero_Outside",
             "Pi0_pt_vs_rap_est_zero_Outside; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_zero_Outside =
    new TH1D("DalitzPi0_zero_Outside",
             "DalitzPi0_zero_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Outside.push_back(DalitzPi0_zero_Outside);
  PhotonsPi0_zero_Outside =
    new TH1D("PhotonsPi0_zero_Outside",
             "PhotonsPi0_zero_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_one_Outside =
    new TH1D("GammaInvMassReco_one_Outside",
             "GammaInvMassReco_one_Outside; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_one_Outside.push_back(GammaInvMassReco_one_Outside);
  GammaOpeningAngleReco_one_Outside =
    new TH1D("GammaOpeningAngleReco_one_Outside",
             "GammaOpeningAngleReco_one_Outside; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_one_Outside.push_back(GammaOpeningAngleReco_one_Outside);
  Pdg_one_Outside =
    new TH1D("Pdg_one_Outside", "Pdg_one_Outside; Id;#", 800, 0, 400);
  fHistoList_man_one_Outside.push_back(Pdg_one_Outside);
  P_reco_one_Outside = new TH1D(
    "P_reco_one_Outside", "P_reco_one_Outside; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_one_Outside.push_back(P_reco_one_Outside);
  Pt_reco_one_Outside = new TH1D("Pt_reco_one_Outside",
                                 "Pt_reco_one_Outside; P_{t} in GeV/c^{2};#",
                                 300,
                                 0,
                                 3);
  fHistoList_man_one_Outside.push_back(Pt_reco_one_Outside);
  Pi0InvMassReco_one_Outside =
    new TH1D("Pi0InvMassReco_one_Outside",
             "Pi0InvMassReco_one_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Outside.push_back(Pi0InvMassReco_one_Outside);
  EMT_InvMass_one_Outside =
    new TH1D("EMT_InvMass_one_Outside",
             "EMT_InvMass_one_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Outside.push_back(EMT_InvMass_one_Outside);
  Pi0_pt_vs_rap_one_Outside =
    new TH2D("Pi0_pt_vs_rap_one_Outside",
             "Pi0_pt_vs_rap_one_Outside; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_one_Outside =
    new TH2D("Pi0_pt_vs_rap_est_one_Outside",
             "Pi0_pt_vs_rap_est_one_Outside; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_one_Outside =
    new TH1D("DalitzPi0_one_Outside",
             "DalitzPi0_one_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Outside.push_back(DalitzPi0_one_Outside);
  PhotonsPi0_one_Outside =
    new TH1D("PhotonsPi0_one_Outside",
             "PhotonsPi0_one_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_two_Outside =
    new TH1D("GammaInvMassReco_two_Outside",
             "GammaInvMassReco_two_Outside; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_two_Outside.push_back(GammaInvMassReco_two_Outside);
  GammaOpeningAngleReco_two_Outside =
    new TH1D("GammaOpeningAngleReco_two_Outside",
             "GammaOpeningAngleReco_two_Outside; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_two_Outside.push_back(GammaOpeningAngleReco_two_Outside);
  Pdg_two_Outside =
    new TH1D("Pdg_two_Outside", "Pdg_two_Outside; Id;#", 800, 0, 400);
  fHistoList_man_two_Outside.push_back(Pdg_two_Outside);
  P_reco_two_Outside = new TH1D(
    "P_reco_two_Outside", "P_reco_two_Outside; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_two_Outside.push_back(P_reco_two_Outside);
  Pt_reco_two_Outside = new TH1D("Pt_reco_two_Outside",
                                 "Pt_reco_two_Outside; P_{t} in GeV/c^{2};#",
                                 300,
                                 0,
                                 3);
  fHistoList_man_two_Outside.push_back(Pt_reco_two_Outside);
  Pi0InvMassReco_two_Outside =
    new TH1D("Pi0InvMassReco_two_Outside",
             "Pi0InvMassReco_two_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Outside.push_back(Pi0InvMassReco_two_Outside);
  EMT_InvMass_two_Outside =
    new TH1D("EMT_InvMass_two_Outside",
             "EMT_InvMass_two_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Outside.push_back(EMT_InvMass_two_Outside);
  Pi0_pt_vs_rap_two_Outside =
    new TH2D("Pi0_pt_vs_rap_two_Outside",
             "Pi0_pt_vs_rap_two_Outside; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_two_Outside =
    new TH2D("Pi0_pt_vs_rap_est_two_Outside",
             "Pi0_pt_vs_rap_est_two_Outside; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_two_Outside =
    new TH1D("DalitzPi0_two_Outside",
             "DalitzPi0_two_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Outside.push_back(DalitzPi0_two_Outside);
  PhotonsPi0_two_Outside =
    new TH1D("PhotonsPi0_two_Outside",
             "PhotonsPi0_two_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_onetwo_Outside =
    new TH1D("GammaInvMassReco_onetwo_Outside",
             "GammaInvMassReco_onetwo_Outside; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_onetwo_Outside.push_back(GammaInvMassReco_onetwo_Outside);
  GammaOpeningAngleReco_onetwo_Outside =
    new TH1D("GammaOpeningAngleReco_onetwo_Outside",
             "GammaOpeningAngleReco_onetwo_Outside; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_onetwo_Outside.push_back(GammaOpeningAngleReco_onetwo_Outside);
  Pdg_onetwo_Outside =
    new TH1D("Pdg_onetwo_Outside", "Pdg_onetwo_Outside; Id;#", 800, 0, 400);
  fHistoList_man_onetwo_Outside.push_back(Pdg_onetwo_Outside);
  P_reco_onetwo_Outside = new TH1D("P_reco_onetwo_Outside",
                                   "P_reco_onetwo_Outside; P in GeV/c^{2};#",
                                   600,
                                   0,
                                   6);
  fHistoList_man_onetwo_Outside.push_back(P_reco_onetwo_Outside);
  Pt_reco_onetwo_Outside =
    new TH1D("Pt_reco_onetwo_Outside",
             "Pt_reco_onetwo_Outside; P_{t} in GeV/c^{2};#",
             300,
             0,
             3);
  fHistoList_man_onetwo_Outside.push_back(Pt_reco_onetwo_Outside);
  Pi0InvMassReco_onetwo_Outside =
    new TH1D("Pi0InvMassReco_onetwo_Outside",
             "Pi0InvMassReco_onetwo_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Outside.push_back(Pi0InvMassReco_onetwo_Outside);
  EMT_InvMass_onetwo_Outside =
    new TH1D("EMT_InvMass_onetwo_Outside",
             "EMT_InvMass_onetwo_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Outside.push_back(EMT_InvMass_onetwo_Outside);
  Pi0_pt_vs_rap_onetwo_Outside =
    new TH2D("Pi0_pt_vs_rap_onetwo_Outside",
             "Pi0_pt_vs_rap_onetwo_Outside; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_onetwo_Outside =
    new TH2D("Pi0_pt_vs_rap_est_onetwo_Outside",
             "Pi0_pt_vs_rap_est_onetwo_Outside; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_onetwo_Outside =
    new TH1D("DalitzPi0_onetwo_Outside",
             "DalitzPi0_onetwo_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Outside.push_back(DalitzPi0_onetwo_Outside);
  PhotonsPi0_onetwo_Outside =
    new TH1D("PhotonsPi0_onetwo_Outside",
             "PhotonsPi0_onetwo_Outside; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_all_Both =
    new TH1D("GammaInvMassReco_all_Both",
             "GammaInvMassReco_all_Both; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_all_Both.push_back(GammaInvMassReco_all_Both);
  GammaOpeningAngleReco_all_Both =
    new TH1D("GammaOpeningAngleReco_all_Both",
             "GammaOpeningAngleReco_all_Both; angle [deg];#",
             200,
             -0.1,
             19.9);
  ...
  P_reco_all_Both =
    new TH1D("P_reco_all_Both", "P_reco_all_Both; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_all_Both.push_back(P_reco_all_Both);
  Pt_reco_all_Both = new TH1D(
    "Pt_reco_all_Both", "Pt_reco_all_Both; P_{t} in GeV/c^{2};#", 300, 0, 3);
  fHistoList_man_all_Both.push_back(Pt_reco_all_Both);
  Pi0InvMassReco_all_Both =
    new TH1D("Pi0InvMassReco_all_Both",
             "Pi0InvMassReco_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Both.push_back(Pi0InvMassReco_all_Both);
  EMT_InvMass_all_Both =
    new TH1D("EMT_InvMass_all_Both",
             "EMT_InvMass_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Both.push_back(EMT_InvMass_all_Both);
  Pi0_pt_vs_rap_all_Both =
    new TH2D("Pi0_pt_vs_rap_all_Both",
             "Pi0_pt_vs_rap_all_Both; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_all_Both =
    new TH2D("Pi0_pt_vs_rap_est_all_Both",
             "Pi0_pt_vs_rap_est_all_Both; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_all_Both =
    new TH1D("DalitzPi0_all_Both",
             "DalitzPi0_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_all_Both.push_back(DalitzPi0_all_Both);
  PhotonsPi0_all_Both =
    new TH1D("PhotonsPi0_all_Both",
             "PhotonsPi0_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_zero_Both =
    new TH1D("GammaInvMassReco_zero_Both",
             "GammaInvMassReco_zero_Both; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_zero_Both.push_back(GammaInvMassReco_zero_Both);
  GammaOpeningAngleReco_zero_Both =
    new TH1D("GammaOpeningAngleReco_zero_Both",
             "GammaOpeningAngleReco_zero_Both; angle [deg];#",
             200,
             -0.1,
             19.9);
  ...
  P_reco_zero_Both = new TH1D(
    "P_reco_zero_Both", "P_reco_zero_Both; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_zero_Both.push_back(P_reco_zero_Both);
  Pt_reco_zero_Both = new TH1D(
    "Pt_reco_zero_Both", "Pt_reco_zero_Both; P_{t} in GeV/c^{2};#", 300, 0, 3);
  fHistoList_man_zero_Both.push_back(Pt_reco_zero_Both);
  Pi0InvMassReco_zero_Both =
    new TH1D("Pi0InvMassReco_zero_Both",
             "Pi0InvMassReco_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Both.push_back(Pi0InvMassReco_zero_Both);
  EMT_InvMass_zero_Both =
    new TH1D("EMT_InvMass_zero_Both",
             "EMT_InvMass_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Both.push_back(EMT_InvMass_zero_Both);
  Pi0_pt_vs_rap_zero_Both =
    new TH2D("Pi0_pt_vs_rap_zero_Both",
  ...
  Pi0_pt_vs_rap_est_zero_Both =
    new TH2D("Pi0_pt_vs_rap_est_zero_Both",
             "Pi0_pt_vs_rap_est_zero_Both; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_zero_Both =
    new TH1D("DalitzPi0_zero_Both",
             "DalitzPi0_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_zero_Both.push_back(DalitzPi0_zero_Both);
  PhotonsPi0_zero_Both =
    new TH1D("PhotonsPi0_zero_Both",
             "PhotonsPi0_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_one_Both =
    new TH1D("GammaInvMassReco_one_Both",
             "GammaInvMassReco_one_Both; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_one_Both.push_back(GammaInvMassReco_one_Both);
  GammaOpeningAngleReco_one_Both =
    new TH1D("GammaOpeningAngleReco_one_Both",
             "GammaOpeningAngleReco_one_Both; angle [deg];#",
             200,
             -0.1,
             19.9);
  ...
  P_reco_one_Both =
    new TH1D("P_reco_one_Both", "P_reco_one_Both; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_one_Both.push_back(P_reco_one_Both);
  Pt_reco_one_Both = new TH1D(
    "Pt_reco_one_Both", "Pt_reco_one_Both; P_{t} in GeV/c^{2};#", 300, 0, 3);
  fHistoList_man_one_Both.push_back(Pt_reco_one_Both);
  Pi0InvMassReco_one_Both =
    new TH1D("Pi0InvMassReco_one_Both",
             "Pi0InvMassReco_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Both.push_back(Pi0InvMassReco_one_Both);
  EMT_InvMass_one_Both =
    new TH1D("EMT_InvMass_one_Both",
             "EMT_InvMass_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Both.push_back(EMT_InvMass_one_Both);
  Pi0_pt_vs_rap_one_Both =
    new TH2D("Pi0_pt_vs_rap_one_Both",
             "Pi0_pt_vs_rap_one_Both; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_one_Both =
    new TH2D("Pi0_pt_vs_rap_est_one_Both",
             "Pi0_pt_vs_rap_est_one_Both; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_one_Both =
    new TH1D("DalitzPi0_one_Both",
             "DalitzPi0_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_one_Both.push_back(DalitzPi0_one_Both);
  PhotonsPi0_one_Both =
    new TH1D("PhotonsPi0_one_Both",
             "PhotonsPi0_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_two_Both =
    new TH1D("GammaInvMassReco_two_Both",
             "GammaInvMassReco_two_Both; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_two_Both.push_back(GammaInvMassReco_two_Both);
  GammaOpeningAngleReco_two_Both =
    new TH1D("GammaOpeningAngleReco_two_Both",
             "GammaOpeningAngleReco_two_Both; angle [deg];#",
             200,
             -0.1,
             19.9);
  ...
  P_reco_two_Both =
    new TH1D("P_reco_two_Both", "P_reco_two_Both; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_two_Both.push_back(P_reco_two_Both);
  Pt_reco_two_Both = new TH1D(
    "Pt_reco_two_Both", "Pt_reco_two_Both; P_{t} in GeV/c^{2};#", 300, 0, 3);
  fHistoList_man_two_Both.push_back(Pt_reco_two_Both);
  Pi0InvMassReco_two_Both =
    new TH1D("Pi0InvMassReco_two_Both",
             "Pi0InvMassReco_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Both.push_back(Pi0InvMassReco_two_Both);
  EMT_InvMass_two_Both =
    new TH1D("EMT_InvMass_two_Both",
             "EMT_InvMass_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Both.push_back(EMT_InvMass_two_Both);
  Pi0_pt_vs_rap_two_Both =
    new TH2D("Pi0_pt_vs_rap_two_Both",
             "Pi0_pt_vs_rap_two_Both; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_two_Both =
    new TH2D("Pi0_pt_vs_rap_est_two_Both",
             "Pi0_pt_vs_rap_est_two_Both; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_two_Both =
    new TH1D("DalitzPi0_two_Both",
             "DalitzPi0_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_two_Both.push_back(DalitzPi0_two_Both);
  PhotonsPi0_two_Both =
    new TH1D("PhotonsPi0_two_Both",
             "PhotonsPi0_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  GammaInvMassReco_onetwo_Both =
    new TH1D("GammaInvMassReco_onetwo_Both",
             "GammaInvMassReco_onetwo_Both; invariant mass in GeV/c^{2};#",
             110,
             -0.01,
             0.1);
  fHistoList_man_onetwo_Both.push_back(GammaInvMassReco_onetwo_Both);
  GammaOpeningAngleReco_onetwo_Both =
    new TH1D("GammaOpeningAngleReco_onetwo_Both",
             "GammaOpeningAngleReco_onetwo_Both; angle [deg];#",
             200,
             -0.1,
             19.9);
  fHistoList_man_onetwo_Both.push_back(GammaOpeningAngleReco_onetwo_Both);
  Pdg_onetwo_Both =
    new TH1D("Pdg_onetwo_Both", "Pdg_onetwo_Both; Id;#", 800, 0, 400);
  fHistoList_man_onetwo_Both.push_back(Pdg_onetwo_Both);
  P_reco_onetwo_Both = new TH1D(
    "P_reco_onetwo_Both", "P_reco_onetwo_Both; P in GeV/c^{2};#", 600, 0, 6);
  fHistoList_man_onetwo_Both.push_back(P_reco_onetwo_Both);
  Pt_reco_onetwo_Both = new TH1D("Pt_reco_onetwo_Both",
                                 "Pt_reco_onetwo_Both; P_{t} in GeV/c^{2};#",
                                 300,
                                 0,
                                 3);
  fHistoList_man_onetwo_Both.push_back(Pt_reco_onetwo_Both);
  Pi0InvMassReco_onetwo_Both =
    new TH1D("Pi0InvMassReco_onetwo_Both",
             "Pi0InvMassReco_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Both.push_back(Pi0InvMassReco_onetwo_Both);
  EMT_InvMass_onetwo_Both =
    new TH1D("EMT_InvMass_onetwo_Both",
             "EMT_InvMass_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Both.push_back(EMT_InvMass_onetwo_Both);
  Pi0_pt_vs_rap_onetwo_Both =
    new TH2D("Pi0_pt_vs_rap_onetwo_Both",
             "Pi0_pt_vs_rap_onetwo_Both; rapidity y; p_{t} in GeV/c ",
             90,
             -2.,
             7.,
             60,
  ...
  Pi0_pt_vs_rap_est_onetwo_Both =
    new TH2D("Pi0_pt_vs_rap_est_onetwo_Both",
             "Pi0_pt_vs_rap_est_onetwo_Both; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_onetwo_Both =
    new TH1D("DalitzPi0_onetwo_Both",
             "DalitzPi0_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_onetwo_Both.push_back(DalitzPi0_onetwo_Both);
  PhotonsPi0_onetwo_Both =
    new TH1D("PhotonsPi0_onetwo_Both",
             "PhotonsPi0_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  // Both
  Pdg_vs_Distance = new TH2D("Pdg_vs_Distance",
                             "Pdg_vs_Distance; pdg; distance in cm",
                             2500,
                             0,
                             2499,
                             500,
  ...
  // Multiplicity Target
  MultiplicityGamma_all_Target =
    new TH2D("MultiplicityGamma_all_Target",
  ...
  MultiplicityGamma_zero_Target =
    new TH2D("MultiplicityGamma_zero_Target",
  ...
  MultiplicityGamma_one_Target =
    new TH2D("MultiplicityGamma_one_Target",
  ...
  MultiplicityGamma_two_Target =
    new TH2D("MultiplicityGamma_two_Target",
  ...
  MultiplicityGamma_onetwo_Target =
    new TH2D("MultiplicityGamma_onetwo_Target",
  ...
  MultiplicityChargedParticles_all_Target =
    new TH2D("MultiplicityChargedParticles_all_Target",
             ...
             0,
             2.0);
  fHistoList_multiplicity_man_Target.push_back(
    MultiplicityChargedParticles_all_Target);
  MultiplicityChargedParticles_zero_Target =
    new TH2D("MultiplicityChargedParticles_zero_Target",
  ...
  fHistoList_multiplicity_man_Target.push_back(
    MultiplicityChargedParticles_zero_Target);
  MultiplicityChargedParticles_one_Target =
    new TH2D("MultiplicityChargedParticles_one_Target",
  ...
  fHistoList_multiplicity_man_Target.push_back(
    MultiplicityChargedParticles_one_Target);
  MultiplicityChargedParticles_two_Target =
    new TH2D("MultiplicityChargedParticles_two_Target",
  ...
  fHistoList_multiplicity_man_Target.push_back(
    MultiplicityChargedParticles_two_Target);
  MultiplicityChargedParticles_onetwo_Target =
    new TH2D("MultiplicityChargedParticles_onetwo_Target",
  ...
  MultiplicityGamma_all_Outside =
    new TH2D("MultiplicityGamma_all_Outside",
  ...
  MultiplicityGamma_zero_Outside =
    new TH2D("MultiplicityGamma_zero_Outside",
  ...
  MultiplicityGamma_one_Outside =
    new TH2D("MultiplicityGamma_one_Outside",
  ...
  MultiplicityGamma_two_Outside =
    new TH2D("MultiplicityGamma_two_Outside",
  ...
  MultiplicityGamma_onetwo_Outside =
    new TH2D("MultiplicityGamma_onetwo_Outside",
             ...
             0,
             2.0);
  fHistoList_multiplicity_man_Outside.push_back(
    MultiplicityGamma_onetwo_Outside);

  MultiplicityChargedParticles_all_Outside =
    new TH2D("MultiplicityChargedParticles_all_Outside",
  ...
  fHistoList_multiplicity_man_Outside.push_back(
    MultiplicityChargedParticles_zero_Outside);
  MultiplicityChargedParticles_one_Outside =
    new TH2D("MultiplicityChargedParticles_one_Outside",
  ...
  fHistoList_multiplicity_man_Outside.push_back(
    MultiplicityChargedParticles_one_Outside);
  MultiplicityChargedParticles_two_Outside =
    new TH2D("MultiplicityChargedParticles_two_Outside",
  ...
  fHistoList_multiplicity_man_Outside.push_back(
    MultiplicityChargedParticles_two_Outside);
  MultiplicityChargedParticles_onetwo_Outside =
    new TH2D("MultiplicityChargedParticles_onetwo_Outside",
  ...
  // Multiplicity Both
  MultiplicityGamma_all_Both =
    new TH2D("MultiplicityGamma_all_Both",
  ...
  MultiplicityGamma_zero_Both =
    new TH2D("MultiplicityGamma_zero_Both",
  ...
  MultiplicityGamma_one_Both =
    new TH2D("MultiplicityGamma_one_Both",
  ...
  MultiplicityGamma_two_Both =
    new TH2D("MultiplicityGamma_two_Both",
  ...
  MultiplicityGamma_onetwo_Both =
    new TH2D("MultiplicityGamma_onetwo_Both",
  ...
  MultiplicityChargedParticles_all_Both =
    new TH2D("MultiplicityChargedParticles_all_Both",
             ...
             0,
             2.0);
  fHistoList_multiplicity_man_Both.push_back(
    MultiplicityChargedParticles_all_Both);
  MultiplicityChargedParticles_zero_Both =
    new TH2D("MultiplicityChargedParticles_zero_Both",
  ...
  fHistoList_multiplicity_man_Both.push_back(
    MultiplicityChargedParticles_zero_Both);
  MultiplicityChargedParticles_one_Both =
    new TH2D("MultiplicityChargedParticles_one_Both",
  ...
  fHistoList_multiplicity_man_Both.push_back(
    MultiplicityChargedParticles_one_Both);
  MultiplicityChargedParticles_two_Both =
    new TH2D("MultiplicityChargedParticles_two_Both",
  ...
  fHistoList_multiplicity_man_Both.push_back(
    MultiplicityChargedParticles_two_Both);
  MultiplicityChargedParticles_onetwo_Both =
    new TH2D("MultiplicityChargedParticles_onetwo_Both",
  ...
  rap_vs_Pt_InM_mixing_1 =
    new TH1D("rap_vs_Pt_InM_mixing_1",
  ...
  rap_vs_Pt_InM_mixing_2 =
    new TH1D("rap_vs_Pt_InM_mixing_2",
  ...
  rap_vs_Pt_InM_mixing_3 =
    new TH1D("rap_vs_Pt_InM_mixing_3",
  ...
  rap_vs_Pt_InM_mixing_4 =
    new TH1D("rap_vs_Pt_InM_mixing_4",
  ...
  rap_vs_Pt_InM_mixing_5 =
    new TH1D("rap_vs_Pt_InM_mixing_5",
  ...
  rap_vs_Pt_InM_mixing_6 =
    new TH1D("rap_vs_Pt_InM_mixing_6",
  ...
  rap_vs_Pt_InM_mixing_7 =
    new TH1D("rap_vs_Pt_InM_mixing_7",
  ...
  rap_vs_Pt_InM_mixing_8 =
    new TH1D("rap_vs_Pt_InM_mixing_8",
  ...
  rap_vs_Pt_InM_mixing_9 =
    new TH1D("rap_vs_Pt_InM_mixing_9",
  ...
  rap_vs_Pt_InM_mixing_10 =
    new TH1D("rap_vs_Pt_InM_mixing_10",
  ...
  rap_vs_Pt_InM_mixing_11 =
    new TH1D("rap_vs_Pt_InM_mixing_11",
  ...
  rap_vs_Pt_InM_mixing_12 =
    new TH1D("rap_vs_Pt_InM_mixing_12",
  ...
  rap_vs_Pt_InM_mixing_13 =
    new TH1D("rap_vs_Pt_InM_mixing_13",
  ...
  rap_vs_Pt_InM_mixing_14 =
    new TH1D("rap_vs_Pt_InM_mixing_14",
  ...
  rap_vs_Pt_InM_mixing_15 =
    new TH1D("rap_vs_Pt_InM_mixing_15",
  ...
  rap_vs_Pt_InM_mixing_16 =
    new TH1D("rap_vs_Pt_InM_mixing_16",
  ...
  rap_vs_Pt_InM_mixing_17 =
    new TH1D("rap_vs_Pt_InM_mixing_17",
  ...
  rap_vs_Pt_InM_mixing_18 =
    new TH1D("rap_vs_Pt_InM_mixing_18",
  ...
  rap_vs_Pt_InM_mixing_19 =
    new TH1D("rap_vs_Pt_InM_mixing_19",
  ...
  rap_vs_Pt_InM_mixing_20 =
    new TH1D("rap_vs_Pt_InM_mixing_20",
  ...
  rap_vs_Pt_InM_mixing_21 =
    new TH1D("rap_vs_Pt_InM_mixing_21",
  ...
  rap_vs_Pt_InM_mixing_22 =
    new TH1D("rap_vs_Pt_InM_mixing_22",
  ...
  rap_vs_Pt_InM_mixing_23 =
    new TH1D("rap_vs_Pt_InM_mixing_23",
  ...
  rap_vs_Pt_InM_mixing_24 =
    new TH1D("rap_vs_Pt_InM_mixing_24",
  ...
  rap_vs_Pt_InM_mixing_25 =
    new TH1D("rap_vs_Pt_InM_mixing_25",
  ...
  rap_vs_Pt_InM_mixing_26 =
    new TH1D("rap_vs_Pt_InM_mixing_26",
  ...
  rap_vs_Pt_InM_mixing_27 =
    new TH1D("rap_vs_Pt_InM_mixing_27",
  ...
  rap_vs_Pt_InM_mixing_28 =
    new TH1D("rap_vs_Pt_InM_mixing_28",
  ...
  rap_vs_Pt_InM_mixing_29 =
    new TH1D("rap_vs_Pt_InM_mixing_29",
  ...
  rap_vs_Pt_InM_mixing_30 =
    new TH1D("rap_vs_Pt_InM_mixing_30",
  ...
  rap_vs_Pt_InM_mixing_31 =
    new TH1D("rap_vs_Pt_InM_mixing_31",
  ...
  rap_vs_Pt_InM_mixing_32 =
    new TH1D("rap_vs_Pt_InM_mixing_32",
  ...
  rap_vs_Pt_InM_mixing_33 =
    new TH1D("rap_vs_Pt_InM_mixing_33",
  ...
  rap_vs_Pt_InM_mixing_34 =
    new TH1D("rap_vs_Pt_InM_mixing_34",
  ...
  rap_vs_Pt_InM_mixing_35 =
    new TH1D("rap_vs_Pt_InM_mixing_35",
  ...
  rap_vs_Pt_InM_mixing_36 =
    new TH1D("rap_vs_Pt_InM_mixing_36",
  ...
  rap_vs_Pt_InM_mixing_37 =
    new TH1D("rap_vs_Pt_InM_mixing_37",
  ...
  rap_vs_Pt_InM_mixing_38 =
    new TH1D("rap_vs_Pt_InM_mixing_38",
  ...
  rap_vs_Pt_InM_mixing_39 =
    new TH1D("rap_vs_Pt_InM_mixing_39",
  ...
  rap_vs_Pt_InM_mixing_40 =
    new TH1D("rap_vs_Pt_InM_mixing_40",
  ...
  rap_vs_Pt_InM_mixing_41 =
    new TH1D("rap_vs_Pt_InM_mixing_41",
  ...
  rap_vs_Pt_InM_mixing_42 =
    new TH1D("rap_vs_Pt_InM_mixing_42",
  ...
  rap_vs_Pt_InM_mixing_43 =
    new TH1D("rap_vs_Pt_InM_mixing_43",
  ...
  rap_vs_Pt_InM_mixing_44 =
    new TH1D("rap_vs_Pt_InM_mixing_44",
  ...
  rap_vs_Pt_InM_mixing_45 =
    new TH1D("rap_vs_Pt_InM_mixing_45",
  ...
  rap_vs_Pt_InM_mixing_46 =
    new TH1D("rap_vs_Pt_InM_mixing_46",
  ...
  rap_vs_Pt_InM_mixing_47 =
    new TH1D("rap_vs_Pt_InM_mixing_47",
  ...
  rap_vs_Pt_InM_mixing_48 =
    new TH1D("rap_vs_Pt_InM_mixing_48",
  ...
  rap_vs_Pt_InM_mixing_49 =
    new TH1D("rap_vs_Pt_InM_mixing_49",
  ...
  rap_vs_Pt_InM_mixing_50 =
    new TH1D("rap_vs_Pt_InM_mixing_50",
  ...
  rap_vs_Pt_InM_mixing_51 =
    new TH1D("rap_vs_Pt_InM_mixing_51",
  ...
  rap_vs_Pt_InM_mixing_52 =
    new TH1D("rap_vs_Pt_InM_mixing_52",
  ...
  rap_vs_Pt_InM_mixing_53 =
    new TH1D("rap_vs_Pt_InM_mixing_53",
  ...
  rap_vs_Pt_InM_mixing_54 =
    new TH1D("rap_vs_Pt_InM_mixing_54",
  ...
  rap_vs_Pt_InM_mixing_55 =
    new TH1D("rap_vs_Pt_InM_mixing_55",
  ...
  rap_vs_Pt_InM_mixing_56 =
    new TH1D("rap_vs_Pt_InM_mixing_56",
  ...
  rap_vs_Pt_InM_mixing_57 =
    new TH1D("rap_vs_Pt_InM_mixing_57",
  ...
  rap_vs_Pt_InM_mixing_58 =
    new TH1D("rap_vs_Pt_InM_mixing_58",
  ...
  rap_vs_Pt_InM_mixing_59 =
    new TH1D("rap_vs_Pt_InM_mixing_59",
  ...
  rap_vs_Pt_InM_mixing_60 =
    new TH1D("rap_vs_Pt_InM_mixing_60",
  ...
  rap_vs_Pt_InM_mixing_61 =
    new TH1D("rap_vs_Pt_InM_mixing_61",
  ...
  rap_vs_Pt_InM_mixing_62 =
    new TH1D("rap_vs_Pt_InM_mixing_62",
  ...
  rap_vs_Pt_InM_mixing_63 =
    new TH1D("rap_vs_Pt_InM_mixing_63",
  ...
  rap_vs_Pt_InM_mixing_64 =
    new TH1D("rap_vs_Pt_InM_mixing_64",
  ...
  rap_vs_Pt_InM_mixing_65 =
    new TH1D("rap_vs_Pt_InM_mixing_65",
  ...
  rap_vs_Pt_InM_mixing_66 =
    new TH1D("rap_vs_Pt_InM_mixing_66",
  ...
  rap_vs_Pt_InM_mixing_67 =
    new TH1D("rap_vs_Pt_InM_mixing_67",
  ...
  rap_vs_Pt_InM_mixing_68 =
    new TH1D("rap_vs_Pt_InM_mixing_68",
  ...
  rap_vs_Pt_InM_mixing_69 =
    new TH1D("rap_vs_Pt_InM_mixing_69",
  ...
  rap_vs_Pt_InM_mixing_70 =
    new TH1D("rap_vs_Pt_InM_mixing_70",
  ...
  rap_vs_Pt_InM_mixing_71 =
    new TH1D("rap_vs_Pt_InM_mixing_71",
  ...
  rap_vs_Pt_InM_mixing_72 =
    new TH1D("rap_vs_Pt_InM_mixing_72",
  ...
  rap_vs_Pt_InM_mixing_73 =
    new TH1D("rap_vs_Pt_InM_mixing_73",
  ...
  rap_vs_Pt_InM_mixing_74 =
    new TH1D("rap_vs_Pt_InM_mixing_74",
  ...
  rap_vs_Pt_InM_mixing_75 =
    new TH1D("rap_vs_Pt_InM_mixing_75",
  ...
  rap_vs_Pt_InM_mixing_76 =
    new TH1D("rap_vs_Pt_InM_mixing_76",
  ...
  rap_vs_Pt_InM_mixing_81 =
    new TH1D("rap_vs_Pt_InM_mixing_81",
  ...
  rap_vs_Pt_InM_mixing_82 =
    new TH1D("rap_vs_Pt_InM_mixing_82",
  ...
  rap_vs_Pt_InM_mixing_83 =
    new TH1D("rap_vs_Pt_InM_mixing_83",
  ...
  rap_vs_Pt_InM_mixing_84 =
    new TH1D("rap_vs_Pt_InM_mixing_84",
  ...
  rap_vs_Pt_InM_mixing_85 =
    new TH1D("rap_vs_Pt_InM_mixing_85",
  ...
  rap_vs_Pt_InM_mixing_86 =
    new TH1D("rap_vs_Pt_InM_mixing_86",
  ...
  rap_vs_Pt_InM_mixing_87 =
    new TH1D("rap_vs_Pt_InM_mixing_87",
  ...
  rap_vs_Pt_InM_mixing_88 =
    new TH1D("rap_vs_Pt_InM_mixing_88",
  ...
  rap_vs_Pt_InM_mixing_89 =
    new TH1D("rap_vs_Pt_InM_mixing_89",
  ...
  rap_vs_Pt_InM_mixing_90 =
    new TH1D("rap_vs_Pt_InM_mixing_90",
  ...
  rap_vs_Pt_InM_mixing_91 =
    new TH1D("rap_vs_Pt_InM_mixing_91",
  ...
  rap_vs_Pt_InM_mixing_92 =
    new TH1D("rap_vs_Pt_InM_mixing_92",
  ...
  rap_vs_Pt_InM_mixing_101 =
    new TH1D("rap_vs_Pt_InM_mixing_101",
  ...
  rap_vs_Pt_InM_mixing_102 =
    new TH1D("rap_vs_Pt_InM_mixing_102",
  ...
  rap_vs_Pt_InM_mixing_103 =
    new TH1D("rap_vs_Pt_InM_mixing_103",
  ...
  rap_vs_Pt_InM_mixing_104 =
    new TH1D("rap_vs_Pt_InM_mixing_104",
  ...
  rap_vs_Pt_InM_mixing_105 =
    new TH1D("rap_vs_Pt_InM_mixing_105",
  ...
  rap_vs_Pt_InM_mixing_106 =
    new TH1D("rap_vs_Pt_InM_mixing_106",
  ...
  rap_vs_Pt_InM_mixing_107 =
    new TH1D("rap_vs_Pt_InM_mixing_107",
  ...
  rap_vs_Pt_InM_mixing_108 =
    new TH1D("rap_vs_Pt_InM_mixing_108",
  ...
  rap_vs_Pt_InM_mixing_111 =
    new TH1D("rap_vs_Pt_InM_mixing_111",
  ...
  rap_vs_Pt_InM_mixing_112 =
    new TH1D("rap_vs_Pt_InM_mixing_112",
  ...
  rap_vs_Pt_InM_mixing_113 =
    new TH1D("rap_vs_Pt_InM_mixing_113",
  ...
  rap_vs_Pt_InM_mixing_114 =
    new TH1D("rap_vs_Pt_InM_mixing_114",
  ...
  rap_vs_Pt_InM_all_mixing_1 =
    new TH1D("rap_vs_Pt_InM_all_mixing_1",
             "rapidity = (1.2-1.6)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_2 =
    new TH1D("rap_vs_Pt_InM_all_mixing_2",
             "rapidity = (1.2-1.6)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_3 =
    new TH1D("rap_vs_Pt_InM_all_mixing_3",
             "rapidity = (1.2-1.6)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_4 =
    new TH1D("rap_vs_Pt_InM_all_mixing_4",
             "rapidity = (1.2-1.6)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_5 =
    new TH1D("rap_vs_Pt_InM_all_mixing_5",
             "rapidity = (1.2-1.6)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_6 =
    new TH1D("rap_vs_Pt_InM_all_mixing_6",
             "rapidity = (1.2-1.6)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_7 =
    new TH1D("rap_vs_Pt_InM_all_mixing_7",
             "rapidity = (1.2-1.6)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_8 =
    new TH1D("rap_vs_Pt_InM_all_mixing_8",
             "rapidity = (1.2-1.6)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_9 =
    new TH1D("rap_vs_Pt_InM_all_mixing_9",
             "rapidity = (1.2-1.6)      P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_10 =
    new TH1D("rap_vs_Pt_InM_all_mixing_10",
             "rapidity = (1.2-1.6)      P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_11 =
    new TH1D("rap_vs_Pt_InM_all_mixing_11",
             "rapidity = (1.2-1.6)      P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_12 =
    new TH1D("rap_vs_Pt_InM_all_mixing_12",
             "rapidity = (1.2-1.6)      P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_13 =
    new TH1D("rap_vs_Pt_InM_all_mixing_13",
             "rapidity = (1.2-1.6)      P_{t} = (1.2-1.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_14 =
    new TH1D("rap_vs_Pt_InM_all_mixing_14",
             "rapidity = (1.2-1.6)      P_{t} = (1.3-1.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_15 =
    new TH1D("rap_vs_Pt_InM_all_mixing_15",
             "rapidity = (1.2-1.6)      P_{t} = (1.4-1.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_16 =
    new TH1D("rap_vs_Pt_InM_all_mixing_16",
             "rapidity = (1.2-1.6)      P_{t} = (1.5-1.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_17 =
    new TH1D("rap_vs_Pt_InM_all_mixing_17",
             "rapidity = (1.2-1.6)      P_{t} = (1.6-1.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_18 =
    new TH1D("rap_vs_Pt_InM_all_mixing_18",
             "rapidity = (1.2-1.6)      P_{t} = (1.7-1.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_19 =
    new TH1D("rap_vs_Pt_InM_all_mixing_19",
             "rapidity = (1.2-1.6)      P_{t} = (1.8-1.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_20 =
    new TH1D("rap_vs_Pt_InM_all_mixing_20",
             "rapidity = (1.2-1.6)      P_{t} = (1.9-2.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_21 =
    new TH1D("rap_vs_Pt_InM_all_mixing_21",
             "rapidity = (1.6-2.0)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_22 =
    new TH1D("rap_vs_Pt_InM_all_mixing_22",
             "rapidity = (1.6-2.0)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_23 =
    new TH1D("rap_vs_Pt_InM_all_mixing_23",
             "rapidity = (1.6-2.0)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_24 =
    new TH1D("rap_vs_Pt_InM_all_mixing_24",
             "rapidity = (1.6-2.0)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_25 =
    new TH1D("rap_vs_Pt_InM_all_mixing_25",
             "rapidity = (1.6-2.0)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_26 =
    new TH1D("rap_vs_Pt_InM_all_mixing_26",
             "rapidity = (1.6-2.0)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_27 =
    new TH1D("rap_vs_Pt_InM_all_mixing_27",
             "rapidity = (1.6-2.0)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_28 =
    new TH1D("rap_vs_Pt_InM_all_mixing_28",
             "rapidity = (1.6-2.0)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_29 =
    new TH1D("rap_vs_Pt_InM_all_mixing_29",
             "rapidity = (1.6-2.0)      P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_30 =
    new TH1D("rap_vs_Pt_InM_all_mixing_30",
             "rapidity = (1.6-2.0)      P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_31 =
    new TH1D("rap_vs_Pt_InM_all_mixing_31",
             "rapidity = (1.6-2.0)      P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_32 =
    new TH1D("rap_vs_Pt_InM_all_mixing_32",
             "rapidity = (1.6-2.0)      P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_33 =
    new TH1D("rap_vs_Pt_InM_all_mixing_33",
             "rapidity = (1.6-2.0)      P_{t} = (1.2-1.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_34 =
    new TH1D("rap_vs_Pt_InM_all_mixing_34",
             "rapidity = (1.6-2.0)      P_{t} = (1.3-1.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_35 =
    new TH1D("rap_vs_Pt_InM_all_mixing_35",
             "rapidity = (1.6-2.0)      P_{t} = (1.4-1.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_36 =
    new TH1D("rap_vs_Pt_InM_all_mixing_36",
             "rapidity = (1.6-2.0)      P_{t} = (1.5-1.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_37 =
    new TH1D("rap_vs_Pt_InM_all_mixing_37",
             "rapidity = (1.6-2.0)      P_{t} = (1.6-1.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_38 =
    new TH1D("rap_vs_Pt_InM_all_mixing_38",
             "rapidity = (1.6-2.0)      P_{t} = (1.7-1.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_39 =
    new TH1D("rap_vs_Pt_InM_all_mixing_39",
             "rapidity = (1.6-2.0)      P_{t} = (1.8-1.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_40 =
    new TH1D("rap_vs_Pt_InM_all_mixing_40",
             "rapidity = (1.6-2.0)      P_{t} = (1.9-2.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_41 =
    new TH1D("rap_vs_Pt_InM_all_mixing_41",
             "rapidity = (2.0-2.4)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_42 =
    new TH1D("rap_vs_Pt_InM_all_mixing_42",
             "rapidity = (2.0-2.4)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_43 =
    new TH1D("rap_vs_Pt_InM_all_mixing_43",
             "rapidity = (2.0-2.4)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_44 =
    new TH1D("rap_vs_Pt_InM_all_mixing_44",
             "rapidity = (2.0-2.4)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_45 =
    new TH1D("rap_vs_Pt_InM_all_mixing_45",
             "rapidity = (2.0-2.4)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_46 =
    new TH1D("rap_vs_Pt_InM_all_mixing_46",
             "rapidity = (2.0-2.4)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_47 =
    new TH1D("rap_vs_Pt_InM_all_mixing_47",
             "rapidity = (2.0-2.4)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_48 =
    new TH1D("rap_vs_Pt_InM_all_mixing_48",
             "rapidity = (2.0-2.4)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_49 =
    new TH1D("rap_vs_Pt_InM_all_mixing_49",
             "rapidity = (2.0-2.4)      P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_50 =
    new TH1D("rap_vs_Pt_InM_all_mixing_50",
             "rapidity = (2.0-2.4)      P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_51 =
    new TH1D("rap_vs_Pt_InM_all_mixing_51",
             "rapidity = (2.0-2.4)      P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_52 =
    new TH1D("rap_vs_Pt_InM_all_mixing_52",
             "rapidity = (2.0-2.4)      P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_53 =
    new TH1D("rap_vs_Pt_InM_all_mixing_53",
             "rapidity = (2.0-2.4)      P_{t} = (1.2-1.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_54 =
    new TH1D("rap_vs_Pt_InM_all_mixing_54",
             "rapidity = (2.0-2.4)      P_{t} = (1.3-1.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_55 =
    new TH1D("rap_vs_Pt_InM_all_mixing_55",
             "rapidity = (2.0-2.4)      P_{t} = (1.4-1.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_56 =
    new TH1D("rap_vs_Pt_InM_all_mixing_56",
             "rapidity = (2.0-2.4)      P_{t} = (1.5-1.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_57 =
    new TH1D("rap_vs_Pt_InM_all_mixing_57",
             "rapidity = (2.0-2.4)      P_{t} = (1.6-1.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_58 =
    new TH1D("rap_vs_Pt_InM_all_mixing_58",
             "rapidity = (2.0-2.4)      P_{t} = (1.7-1.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_59 =
    new TH1D("rap_vs_Pt_InM_all_mixing_59",
             "rapidity = (2.0-2.4)      P_{t} = (1.8-1.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_60 =
    new TH1D("rap_vs_Pt_InM_all_mixing_60",
             "rapidity = (2.0-2.4)      P_{t} = (1.9-2.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_61 =
    new TH1D("rap_vs_Pt_InM_all_mixing_61",
             "rapidity = (2.4-2.8)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_62 =
    new TH1D("rap_vs_Pt_InM_all_mixing_62",
             "rapidity = (2.4-2.8)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_63 =
    new TH1D("rap_vs_Pt_InM_all_mixing_63",
             "rapidity = (2.4-2.8)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_64 =
    new TH1D("rap_vs_Pt_InM_all_mixing_64",
             "rapidity = (2.4-2.8)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_65 =
    new TH1D("rap_vs_Pt_InM_all_mixing_65",
             "rapidity = (2.4-2.8)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_66 =
    new TH1D("rap_vs_Pt_InM_all_mixing_66",
             "rapidity = (2.4-2.8)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_67 =
    new TH1D("rap_vs_Pt_InM_all_mixing_67",
             "rapidity = (2.4-2.8)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_68 =
    new TH1D("rap_vs_Pt_InM_all_mixing_68",
             "rapidity = (2.4-2.8)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_69 =
    new TH1D("rap_vs_Pt_InM_all_mixing_69",
             "rapidity = (2.4-2.8)      P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_70 =
    new TH1D("rap_vs_Pt_InM_all_mixing_70",
             "rapidity = (2.4-2.8)      P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_71 =
    new TH1D("rap_vs_Pt_InM_all_mixing_71",
             "rapidity = (2.4-2.8)      P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_72 =
    new TH1D("rap_vs_Pt_InM_all_mixing_72",
             "rapidity = (2.4-2.8)      P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_73 =
    new TH1D("rap_vs_Pt_InM_all_mixing_73",
             "rapidity = (2.4-2.8)      P_{t} = (1.2-1.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_74 =
    new TH1D("rap_vs_Pt_InM_all_mixing_74",
             "rapidity = (2.4-2.8)      P_{t} = (1.3-1.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_75 =
    new TH1D("rap_vs_Pt_InM_all_mixing_75",
             "rapidity = (2.4-2.8)      P_{t} = (1.4-1.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_76 =
    new TH1D("rap_vs_Pt_InM_all_mixing_76",
             "rapidity = (2.4-2.8)      P_{t} = (1.5-1.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_81 =
    new TH1D("rap_vs_Pt_InM_all_mixing_81",
             "rapidity = (2.8-3.2)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_82 =
    new TH1D("rap_vs_Pt_InM_all_mixing_82",
             "rapidity = (2.8-3.2)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_83 =
    new TH1D("rap_vs_Pt_InM_all_mixing_83",
             "rapidity = (2.8-3.2)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_84 =
    new TH1D("rap_vs_Pt_InM_all_mixing_84",
             "rapidity = (2.8-3.2)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_85 =
    new TH1D("rap_vs_Pt_InM_all_mixing_85",
             "rapidity = (2.8-3.2)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_86 =
    new TH1D("rap_vs_Pt_InM_all_mixing_86",
             "rapidity = (2.8-3.2)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_87 =
    new TH1D("rap_vs_Pt_InM_all_mixing_87",
             "rapidity = (2.8-3.2)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_88 =
    new TH1D("rap_vs_Pt_InM_all_mixing_88",
             "rapidity = (2.8-3.2)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_89 =
    new TH1D("rap_vs_Pt_InM_all_mixing_89",
             "rapidity = (2.8-3.2)      P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_90 =
    new TH1D("rap_vs_Pt_InM_all_mixing_90",
             "rapidity = (2.8-3.2)      P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_91 =
    new TH1D("rap_vs_Pt_InM_all_mixing_91",
             "rapidity = (2.8-3.2)      P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_92 =
    new TH1D("rap_vs_Pt_InM_all_mixing_92",
             "rapidity = (2.8-3.2)      P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_101 =
    new TH1D("rap_vs_Pt_InM_all_mixing_101",
             "rapidity = (3.2-3.6)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_102 =
    new TH1D("rap_vs_Pt_InM_all_mixing_102",
             "rapidity = (3.2-3.6)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_103 =
    new TH1D("rap_vs_Pt_InM_all_mixing_103",
             "rapidity = (3.2-3.6)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_104 =
    new TH1D("rap_vs_Pt_InM_all_mixing_104",
             "rapidity = (3.2-3.6)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_105 =
    new TH1D("rap_vs_Pt_InM_all_mixing_105",
             "rapidity = (3.2-3.6)      P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_106 =
    new TH1D("rap_vs_Pt_InM_all_mixing_106",
             "rapidity = (3.2-3.6)      P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_107 =
    new TH1D("rap_vs_Pt_InM_all_mixing_107",
             "rapidity = (3.2-3.6)      P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_108 =
    new TH1D("rap_vs_Pt_InM_all_mixing_108",
             "rapidity = (3.2-3.6)      P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_111 =
    new TH1D("rap_vs_Pt_InM_all_mixing_111",
             "rapidity = (3.6-4.0)      P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_112 =
    new TH1D("rap_vs_Pt_InM_all_mixing_112",
             "rapidity = (3.6-4.0)      P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_113 =
    new TH1D("rap_vs_Pt_InM_all_mixing_113",
             "rapidity = (3.6-4.0)      P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  rap_vs_Pt_InM_all_mixing_114 =
    new TH1D("rap_vs_Pt_InM_all_mixing_114",
             "rapidity = (3.6-4.0)      P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant "
             "mass in GeV/c^{2};#",
             1000,
             0,
  ...
  Pi0_pt_est_onetwo_Both =
    new TH1D("Pi0_pt_est_onetwo_Both",
             "P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             20,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(Pi0_pt_est_onetwo_Both);
  pt_onetwo_1 =
    new TH1D("pt_onetwo_1",
             "P_{t} = (0.0-0.1 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  pt_onetwo_2 =
    new TH1D("pt_onetwo_2",
             "P_{t} = (0.1-0.2 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_2);
  pt_onetwo_3 =
    new TH1D("pt_onetwo_3",
             "P_{t} = (0.2-0.3 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_3);
  pt_onetwo_4 =
    new TH1D("pt_onetwo_4",
             "P_{t} = (0.3-0.4 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_4);
  pt_onetwo_5 =
    new TH1D("pt_onetwo_5",
             "P_{t} = (0.4-0.5 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_5);
  pt_onetwo_6 =
    new TH1D("pt_onetwo_6",
             "P_{t} = (0.5-0.6 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_6);
  pt_onetwo_7 =
    new TH1D("pt_onetwo_7",
             "P_{t} = (0.6-0.7 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_7);
  pt_onetwo_8 =
    new TH1D("pt_onetwo_8",
             "P_{t} = (0.7-0.8 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_8);
  pt_onetwo_9 =
    new TH1D("pt_onetwo_9",
             "P_{t} = (0.8-0.9 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_9);
  pt_onetwo_10 =
    new TH1D("pt_onetwo_10",
             "P_{t} = (0.9-1.0 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_10);
  pt_onetwo_11 =
    new TH1D("pt_onetwo_11",
             "P_{t} = (1.0-1.1 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_11);
  pt_onetwo_12 =
    new TH1D("pt_onetwo_12",
             "P_{t} = (1.1-1.2 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_12);
  pt_onetwo_13 =
    new TH1D("pt_onetwo_13",
             "P_{t} = (1.2-1.3 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_13);
  pt_onetwo_14 =
    new TH1D("pt_onetwo_14",
             "P_{t} = (1.3-1.4 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_14);
  pt_onetwo_15 =
    new TH1D("pt_onetwo_15",
             "P_{t} = (1.4-1.5 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_15);
  pt_onetwo_16 =
    new TH1D("pt_onetwo_16",
             "P_{t} = (1.5-1.6 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_16);
  pt_onetwo_17 =
    new TH1D("pt_onetwo_17",
             "P_{t} = (1.6-1.7 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_17);
  pt_onetwo_18 =
    new TH1D("pt_onetwo_18",
             "P_{t} = (1.7-1.8 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_18);
  pt_onetwo_19 =
    new TH1D("pt_onetwo_19",
             "P_{t} = (1.8-1.9 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_pt_onetwo.push_back(pt_onetwo_19);
  pt_onetwo_20 =
    new TH1D("pt_onetwo_20",
             "P_{t} = (1.9-2.0 GeV/c^{2}) ;invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  ...
  // BG cases
  //Both all
  BG1_InM_all_Both = new TH1D("BG1_InM_all_Both",
                              "BG1_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG1_InM_all_Both);
  BG2_InM_all_Both = new TH1D("BG2_InM_all_Both",
                              "BG2_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG2_InM_all_Both);
  BG3_InM_all_Both = new TH1D("BG3_InM_all_Both",
                              "BG3_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG3_InM_all_Both);
  BG4_InM_all_Both = new TH1D("BG4_InM_all_Both",
                              "BG4_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG4_InM_all_Both);
  BG5_InM_all_Both = new TH1D("BG5_InM_all_Both",
                              "BG5_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG5_InM_all_Both);
  BG6_InM_all_Both = new TH1D("BG6_InM_all_Both",
                              "BG6_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG6_InM_all_Both);
  BG7_InM_all_Both = new TH1D("BG7_InM_all_Both",
                              "BG7_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG7_InM_all_Both);
  BG8_InM_all_Both = new TH1D("BG8_InM_all_Both",
                              "BG8_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG8_InM_all_Both);
  BG9_InM_all_Both = new TH1D("BG9_InM_all_Both",
                              "BG9_InM_all_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_all_Both.push_back(BG9_InM_all_Both);
  BG10_InM_all_Both =
    new TH1D("BG10_InM_all_Both",
             "BG10_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(BG10_InM_all_Both);
  PdgCase8_InM_all_Both = new TH1D(
    "PdgCase8_InM_all_Both", "PdgCase8_InM_all_Both; Id ;#", 5000, -2500, 2500);
  fHistoList_bg_InM_all_Both.push_back(PdgCase8_InM_all_Both);
  PdgCase8mothers_InM_all_Both = new TH1D("PdgCase8mothers_InM_all_Both",
                                          "PdgCase8mothers_InM_all_Both; Id ;#",
                                          5000,
                                          -2500,
                                          2500);
  fHistoList_bg_InM_all_Both.push_back(PdgCase8mothers_InM_all_Both);
  sameMIDcase8_InM_all_Both =
    new TH1D("sameMIDcase8_InM_all_Both",
             "sameMIDcase8_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(sameMIDcase8_InM_all_Both);
  sameGRIDcase8_InM_all_Both =
    new TH1D("sameGRIDcase8_InM_all_Both",
             "sameGRIDcase8_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(sameGRIDcase8_InM_all_Both);
  Case1ZYPos_InM_all_Both = new TH2D("Case1ZYPos_InM_all_Both",
                                     "Case1ZYPos_InM_all_Both; z[cm]; y[cm]",
                                     400,
                                     -1,
                                     200,
                                     200,
  ...
  sameMIDcase8_mothedPDG_InM_all_Both =
    new TH1D("sameMIDcase8_mothedPDG_InM_all_Both",
             "sameMIDcase8_mothedPDG_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(sameMIDcase8_mothedPDG_InM_all_Both);
  PdgCase8NonEComeFromTarget_InM_all_Both =
    new TH1D("PdgCase8NonEComeFromTarget_InM_all_Both",
             "PdgCase8NonEComeFromTarget_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(PdgCase8NonEComeFromTarget_InM_all_Both);
  PdgCase8NonE_NOT_FromTarget_InM_all_Both =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_all_Both",
             "PdgCase8NonE_NOT_FromTarget_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(
    PdgCase8NonE_NOT_FromTarget_InM_all_Both);
  PdgCase8motherNonE_InM_all_Both =
    new TH1D("PdgCase8motherNonE_InM_all_Both",
             "PdgCase8motherNonE_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(PdgCase8motherNonE_InM_all_Both);
  Case8ElFromDalitz_InM_all_Both =
    new TH1D("Case8ElFromDalitz_InM_all_Both",
             "Case8ElFromDalitz_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(Case8ElFromDalitz_InM_all_Both);
  Case8NonElFrom_pn_InM_all_Both =
    new TH1D("Case8NonElFrom_pn_InM_all_Both",
             "Case8NonElFrom_pn_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(Case8NonElFrom_pn_InM_all_Both);
  Case8NonElFrom_eta_InM_all_Both =
    new TH1D("Case8NonElFrom_eta_InM_all_Both",
             "Case8NonElFrom_eta_InM_all_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_all_Both.push_back(Case8NonElFrom_eta_InM_all_Both);
  Case8NonElFrom_kaon_InM_all_Both =
    new TH1D("Case8NonElFrom_kaon_InM_all_Both",
  ...
  sameMIDcase8NonEPdg_InM_all_Both =
    new TH1D("sameMIDcase8NonEPdg_InM_all_Both",
             "sameMIDcase8NonEPdg_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(sameMIDcase8NonEPdg_InM_all_Both);
  sameMIDcase8NonEMotherPdg_InM_all_Both =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_all_Both",
             "sameMIDcase8NonEMotherPdg_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(sameMIDcase8NonEMotherPdg_InM_all_Both);
  sameMIDcase8NonEMotherIM_InM_all_Both = new TH1D(
    "sameMIDcase8NonEMotherIM_InM_all_Both",
    "sameMIDcase8NonEMotherIM_InM_all_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_all_Both.push_back(sameMIDcase8NonEMotherIM_InM_all_Both);
  sameMIDcase8NonEPdgFromTarget_InM_all_Both =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_all_Both",
             "sameMIDcase8NonEPdgFromTarget_InM_all_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_all_Both.push_back(
  ...
  sameMIDcase8NonEComeFromTargetP_InM_all_Both =
    new TH1D("sameMIDcase8NonEComeFromTargetP_InM_all_Both",
             "sameMIDcase8NonEComeFromTargetP_InM_all_Both; P in GeV/c^{2} ;#",
             200,
             0,
             10);
  fHistoList_bg_InM_all_Both.push_back(
    sameMIDcase8NonEComeFromTargetP_InM_all_Both);
  sameMIDcase8NonEComeFromTargetPt_InM_all_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetPt_InM_all_Both",
    "sameMIDcase8NonEComeFromTargetPt_InM_all_Both; Pt in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_all_Both.push_back(
  ...
  //Both zero
  BG1_InM_zero_Both =
    new TH1D("BG1_InM_zero_Both",
             "BG1_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG1_InM_zero_Both);
  BG2_InM_zero_Both =
    new TH1D("BG2_InM_zero_Both",
             "BG2_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG2_InM_zero_Both);
  BG3_InM_zero_Both =
    new TH1D("BG3_InM_zero_Both",
             "BG3_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG3_InM_zero_Both);
  BG4_InM_zero_Both =
    new TH1D("BG4_InM_zero_Both",
             "BG4_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG4_InM_zero_Both);
  BG5_InM_zero_Both =
    new TH1D("BG5_InM_zero_Both",
             "BG5_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG5_InM_zero_Both);
  BG6_InM_zero_Both =
    new TH1D("BG6_InM_zero_Both",
             "BG6_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG6_InM_zero_Both);
  BG7_InM_zero_Both =
    new TH1D("BG7_InM_zero_Both",
             "BG7_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG7_InM_zero_Both);
  BG8_InM_zero_Both =
    new TH1D("BG8_InM_zero_Both",
             "BG8_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG8_InM_zero_Both);
  BG9_InM_zero_Both =
    new TH1D("BG9_InM_zero_Both",
             "BG9_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG9_InM_zero_Both);
  BG10_InM_zero_Both =
    new TH1D("BG10_InM_zero_Both",
             "BG10_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(BG10_InM_zero_Both);
  PdgCase8_InM_zero_Both = new TH1D("PdgCase8_InM_zero_Both",
                                    "PdgCase8_InM_zero_Both; Id ;#",
                                    5000,
                                    -2500,
                                    2500);
  fHistoList_bg_InM_zero_Both.push_back(PdgCase8_InM_zero_Both);
  PdgCase8mothers_InM_zero_Both =
    new TH1D("PdgCase8mothers_InM_zero_Both",
             "PdgCase8mothers_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(PdgCase8mothers_InM_zero_Both);
  sameMIDcase8_InM_zero_Both =
    new TH1D("sameMIDcase8_InM_zero_Both",
             "sameMIDcase8_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(sameMIDcase8_InM_zero_Both);
  sameGRIDcase8_InM_zero_Both =
    new TH1D("sameGRIDcase8_InM_zero_Both",
             "sameGRIDcase8_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(sameGRIDcase8_InM_zero_Both);
  Case1ZYPos_InM_zero_Both = new TH2D("Case1ZYPos_InM_zero_Both",
                                      "Case1ZYPos_InM_zero_Both; z[cm]; y[cm]",
                                      400,
                                      -1,
                                      200,
                                      200,
  ...
  sameMIDcase8_mothedPDG_InM_zero_Both =
    new TH1D("sameMIDcase8_mothedPDG_InM_zero_Both",
             "sameMIDcase8_mothedPDG_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(sameMIDcase8_mothedPDG_InM_zero_Both);
  PdgCase8NonEComeFromTarget_InM_zero_Both =
    new TH1D("PdgCase8NonEComeFromTarget_InM_zero_Both",
             "PdgCase8NonEComeFromTarget_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(
    PdgCase8NonEComeFromTarget_InM_zero_Both);
  PdgCase8NonE_NOT_FromTarget_InM_zero_Both =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_zero_Both",
             "PdgCase8NonE_NOT_FromTarget_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(
  ...
  PdgCase8motherNonE_InM_zero_Both =
    new TH1D("PdgCase8motherNonE_InM_zero_Both",
             "PdgCase8motherNonE_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(PdgCase8motherNonE_InM_zero_Both);
  Case8ElFromDalitz_InM_zero_Both =
    new TH1D("Case8ElFromDalitz_InM_zero_Both",
             "Case8ElFromDalitz_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(Case8ElFromDalitz_InM_zero_Both);
  Case8NonElFrom_pn_InM_zero_Both =
    new TH1D("Case8NonElFrom_pn_InM_zero_Both",
             "Case8NonElFrom_pn_InM_zero_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_zero_Both.push_back(Case8NonElFrom_pn_InM_zero_Both);
  Case8NonElFrom_eta_InM_zero_Both =
    new TH1D("Case8NonElFrom_eta_InM_zero_Both",
  ...
  Case8NonElFrom_kaon_InM_zero_Both =
    new TH1D("Case8NonElFrom_kaon_InM_zero_Both",
  ...
  sameMIDcase8NonEPdg_InM_zero_Both =
    new TH1D("sameMIDcase8NonEPdg_InM_zero_Both",
             "sameMIDcase8NonEPdg_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(sameMIDcase8NonEPdg_InM_zero_Both);
  sameMIDcase8NonEMotherPdg_InM_zero_Both =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_zero_Both",
             "sameMIDcase8NonEMotherPdg_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(
    sameMIDcase8NonEMotherPdg_InM_zero_Both);
  sameMIDcase8NonEMotherIM_InM_zero_Both = new TH1D(
    "sameMIDcase8NonEMotherIM_InM_zero_Both",
    "sameMIDcase8NonEMotherIM_InM_zero_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_zero_Both.push_back(sameMIDcase8NonEMotherIM_InM_zero_Both);
  sameMIDcase8NonEPdgFromTarget_InM_zero_Both =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_zero_Both",
             "sameMIDcase8NonEPdgFromTarget_InM_zero_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_zero_Both.push_back(
  ...
  sameMIDcase8NonEComeFromTargetP_InM_zero_Both =
    new TH1D("sameMIDcase8NonEComeFromTargetP_InM_zero_Both",
             "sameMIDcase8NonEComeFromTargetP_InM_zero_Both; P in GeV/c^{2} ;#",
             200,
             0,
             10);
  fHistoList_bg_InM_zero_Both.push_back(
    sameMIDcase8NonEComeFromTargetP_InM_zero_Both);
  sameMIDcase8NonEComeFromTargetPt_InM_zero_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetPt_InM_zero_Both",
    "sameMIDcase8NonEComeFromTargetPt_InM_zero_Both; Pt in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_zero_Both.push_back(
    sameMIDcase8NonEComeFromTargetPt_InM_zero_Both);
  //Both one
  BG1_InM_one_Both = new TH1D("BG1_InM_one_Both",
                              "BG1_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG1_InM_one_Both);
  BG2_InM_one_Both = new TH1D("BG2_InM_one_Both",
                              "BG2_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG2_InM_one_Both);
  BG3_InM_one_Both = new TH1D("BG3_InM_one_Both",
                              "BG3_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG3_InM_one_Both);
  BG4_InM_one_Both = new TH1D("BG4_InM_one_Both",
                              "BG4_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG4_InM_one_Both);
  BG5_InM_one_Both = new TH1D("BG5_InM_one_Both",
                              "BG5_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG5_InM_one_Both);
  BG6_InM_one_Both = new TH1D("BG6_InM_one_Both",
                              "BG6_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG6_InM_one_Both);
  BG7_InM_one_Both = new TH1D("BG7_InM_one_Both",
                              "BG7_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG7_InM_one_Both);
  BG8_InM_one_Both = new TH1D("BG8_InM_one_Both",
                              "BG8_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG8_InM_one_Both);
  BG9_InM_one_Both = new TH1D("BG9_InM_one_Both",
                              "BG9_InM_one_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_one_Both.push_back(BG9_InM_one_Both);
  BG10_InM_one_Both =
    new TH1D("BG10_InM_one_Both",
             "BG10_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(BG10_InM_one_Both);
  PdgCase8_InM_one_Both = new TH1D(
    "PdgCase8_InM_one_Both", "PdgCase8_InM_one_Both; Id ;#", 5000, -2500, 2500);
  fHistoList_bg_InM_one_Both.push_back(PdgCase8_InM_one_Both);
  PdgCase8mothers_InM_one_Both = new TH1D("PdgCase8mothers_InM_one_Both",
                                          "PdgCase8mothers_InM_one_Both; Id ;#",
                                          5000,
                                          -2500,
                                          2500);
  fHistoList_bg_InM_one_Both.push_back(PdgCase8mothers_InM_one_Both);
  sameMIDcase8_InM_one_Both =
    new TH1D("sameMIDcase8_InM_one_Both",
             "sameMIDcase8_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(sameMIDcase8_InM_one_Both);
  sameGRIDcase8_InM_one_Both =
    new TH1D("sameGRIDcase8_InM_one_Both",
             "sameGRIDcase8_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(sameGRIDcase8_InM_one_Both);
  Case1ZYPos_InM_one_Both = new TH2D("Case1ZYPos_InM_one_Both",
                                     "Case1ZYPos_InM_one_Both; z[cm]; y[cm]",
                                     400,
                                     -1,
                                     200,
                                     200,
  ...
  sameMIDcase8_mothedPDG_InM_one_Both =
    new TH1D("sameMIDcase8_mothedPDG_InM_one_Both",
             "sameMIDcase8_mothedPDG_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(sameMIDcase8_mothedPDG_InM_one_Both);
  PdgCase8NonEComeFromTarget_InM_one_Both =
    new TH1D("PdgCase8NonEComeFromTarget_InM_one_Both",
             "PdgCase8NonEComeFromTarget_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(PdgCase8NonEComeFromTarget_InM_one_Both);
  PdgCase8NonE_NOT_FromTarget_InM_one_Both =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_one_Both",
             "PdgCase8NonE_NOT_FromTarget_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(
    PdgCase8NonE_NOT_FromTarget_InM_one_Both);
  PdgCase8motherNonE_InM_one_Both =
    new TH1D("PdgCase8motherNonE_InM_one_Both",
             "PdgCase8motherNonE_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(PdgCase8motherNonE_InM_one_Both);
  Case8ElFromDalitz_InM_one_Both =
    new TH1D("Case8ElFromDalitz_InM_one_Both",
             "Case8ElFromDalitz_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(Case8ElFromDalitz_InM_one_Both);
  Case8NonElFrom_pn_InM_one_Both =
    new TH1D("Case8NonElFrom_pn_InM_one_Both",
             "Case8NonElFrom_pn_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(Case8NonElFrom_pn_InM_one_Both);
  Case8NonElFrom_eta_InM_one_Both =
    new TH1D("Case8NonElFrom_eta_InM_one_Both",
             "Case8NonElFrom_eta_InM_one_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_one_Both.push_back(Case8NonElFrom_eta_InM_one_Both);
  Case8NonElFrom_kaon_InM_one_Both =
    new TH1D("Case8NonElFrom_kaon_InM_one_Both",
  ...
  sameMIDcase8NonEPdg_InM_one_Both =
    new TH1D("sameMIDcase8NonEPdg_InM_one_Both",
             "sameMIDcase8NonEPdg_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(sameMIDcase8NonEPdg_InM_one_Both);
  sameMIDcase8NonEMotherPdg_InM_one_Both =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_one_Both",
             "sameMIDcase8NonEMotherPdg_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(sameMIDcase8NonEMotherPdg_InM_one_Both);
  sameMIDcase8NonEMotherIM_InM_one_Both = new TH1D(
    "sameMIDcase8NonEMotherIM_InM_one_Both",
    "sameMIDcase8NonEMotherIM_InM_one_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_one_Both.push_back(sameMIDcase8NonEMotherIM_InM_one_Both);
  sameMIDcase8NonEPdgFromTarget_InM_one_Both =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_one_Both",
             "sameMIDcase8NonEPdgFromTarget_InM_one_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_one_Both.push_back(
  ...
  sameMIDcase8NonEComeFromTargetP_InM_one_Both =
    new TH1D("sameMIDcase8NonEComeFromTargetP_InM_one_Both",
             "sameMIDcase8NonEComeFromTargetP_InM_one_Both; P in GeV/c^{2} ;#",
             200,
             0,
             10);
  fHistoList_bg_InM_one_Both.push_back(
    sameMIDcase8NonEComeFromTargetP_InM_one_Both);
  sameMIDcase8NonEComeFromTargetPt_InM_one_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetPt_InM_one_Both",
    "sameMIDcase8NonEComeFromTargetPt_InM_one_Both; Pt in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_one_Both.push_back(
    sameMIDcase8NonEComeFromTargetPt_InM_one_Both);
  //Both two
  BG1_InM_two_Both = new TH1D("BG1_InM_two_Both",
                              "BG1_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG1_InM_two_Both);
  BG2_InM_two_Both = new TH1D("BG2_InM_two_Both",
                              "BG2_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG2_InM_two_Both);
  BG3_InM_two_Both = new TH1D("BG3_InM_two_Both",
                              "BG3_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG3_InM_two_Both);
  BG4_InM_two_Both = new TH1D("BG4_InM_two_Both",
                              "BG4_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG4_InM_two_Both);
  BG5_InM_two_Both = new TH1D("BG5_InM_two_Both",
                              "BG5_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG5_InM_two_Both);
  BG6_InM_two_Both = new TH1D("BG6_InM_two_Both",
                              "BG6_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG6_InM_two_Both);
  BG7_InM_two_Both = new TH1D("BG7_InM_two_Both",
                              "BG7_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG7_InM_two_Both);
  BG8_InM_two_Both = new TH1D("BG8_InM_two_Both",
                              "BG8_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG8_InM_two_Both);
  BG9_InM_two_Both = new TH1D("BG9_InM_two_Both",
                              "BG9_InM_two_Both; invariant mass in GeV/c^{2};#",
                              1000,
                              0,
                              2.0);
  fHistoList_bg_InM_two_Both.push_back(BG9_InM_two_Both);
  BG10_InM_two_Both =
    new TH1D("BG10_InM_two_Both",
             "BG10_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(BG10_InM_two_Both);
  PdgCase8_InM_two_Both = new TH1D(
    "PdgCase8_InM_two_Both", "PdgCase8_InM_two_Both; Id ;#", 5000, -2500, 2500);
  fHistoList_bg_InM_two_Both.push_back(PdgCase8_InM_two_Both);
  PdgCase8mothers_InM_two_Both = new TH1D("PdgCase8mothers_InM_two_Both",
                                          "PdgCase8mothers_InM_two_Both; Id ;#",
                                          5000,
                                          -2500,
                                          2500);
  fHistoList_bg_InM_two_Both.push_back(PdgCase8mothers_InM_two_Both);
  sameMIDcase8_InM_two_Both =
    new TH1D("sameMIDcase8_InM_two_Both",
             "sameMIDcase8_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(sameMIDcase8_InM_two_Both);
  sameGRIDcase8_InM_two_Both =
    new TH1D("sameGRIDcase8_InM_two_Both",
             "sameGRIDcase8_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(sameGRIDcase8_InM_two_Both);
  Case1ZYPos_InM_two_Both = new TH2D("Case1ZYPos_InM_two_Both",
                                     "Case1ZYPos_InM_two_Both; z[cm]; y[cm]",
                                     400,
                                     -1,
                                     200,
                                     200,
  ...
  sameMIDcase8_mothedPDG_InM_two_Both =
    new TH1D("sameMIDcase8_mothedPDG_InM_two_Both",
             "sameMIDcase8_mothedPDG_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(sameMIDcase8_mothedPDG_InM_two_Both);
  PdgCase8NonEComeFromTarget_InM_two_Both =
    new TH1D("PdgCase8NonEComeFromTarget_InM_two_Both",
             "PdgCase8NonEComeFromTarget_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(PdgCase8NonEComeFromTarget_InM_two_Both);
  PdgCase8NonE_NOT_FromTarget_InM_two_Both =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_two_Both",
             "PdgCase8NonE_NOT_FromTarget_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(
    PdgCase8NonE_NOT_FromTarget_InM_two_Both);
  PdgCase8motherNonE_InM_two_Both =
    new TH1D("PdgCase8motherNonE_InM_two_Both",
             "PdgCase8motherNonE_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(PdgCase8motherNonE_InM_two_Both);
  Case8ElFromDalitz_InM_two_Both =
    new TH1D("Case8ElFromDalitz_InM_two_Both",
             "Case8ElFromDalitz_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(Case8ElFromDalitz_InM_two_Both);
  Case8NonElFrom_pn_InM_two_Both =
    new TH1D("Case8NonElFrom_pn_InM_two_Both",
             "Case8NonElFrom_pn_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(Case8NonElFrom_pn_InM_two_Both);
  Case8NonElFrom_eta_InM_two_Both =
    new TH1D("Case8NonElFrom_eta_InM_two_Both",
             "Case8NonElFrom_eta_InM_two_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_two_Both.push_back(Case8NonElFrom_eta_InM_two_Both);
  Case8NonElFrom_kaon_InM_two_Both =
    new TH1D("Case8NonElFrom_kaon_InM_two_Both",
  ...
  sameMIDcase8NonEPdg_InM_two_Both =
    new TH1D("sameMIDcase8NonEPdg_InM_two_Both",
             "sameMIDcase8NonEPdg_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(sameMIDcase8NonEPdg_InM_two_Both);
  sameMIDcase8NonEMotherPdg_InM_two_Both =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_two_Both",
             "sameMIDcase8NonEMotherPdg_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(sameMIDcase8NonEMotherPdg_InM_two_Both);
  sameMIDcase8NonEMotherIM_InM_two_Both = new TH1D(
    "sameMIDcase8NonEMotherIM_InM_two_Both",
    "sameMIDcase8NonEMotherIM_InM_two_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_two_Both.push_back(sameMIDcase8NonEMotherIM_InM_two_Both);
  sameMIDcase8NonEPdgFromTarget_InM_two_Both =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_two_Both",
             "sameMIDcase8NonEPdgFromTarget_InM_two_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_two_Both.push_back(
  ...
  sameMIDcase8NonEComeFromTargetP_InM_two_Both =
    new TH1D("sameMIDcase8NonEComeFromTargetP_InM_two_Both",
             "sameMIDcase8NonEComeFromTargetP_InM_two_Both; P in GeV/c^{2} ;#",
             200,
             0,
             10);
  fHistoList_bg_InM_two_Both.push_back(
    sameMIDcase8NonEComeFromTargetP_InM_two_Both);
  sameMIDcase8NonEComeFromTargetPt_InM_two_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetPt_InM_two_Both",
    "sameMIDcase8NonEComeFromTargetPt_InM_two_Both; Pt in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_two_Both.push_back(
  ...
  //Both onetwo
  BG1_InM_onetwo_Both =
    new TH1D("BG1_InM_onetwo_Both",
             "BG1_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG1_InM_onetwo_Both);
  BG2_InM_onetwo_Both =
    new TH1D("BG2_InM_onetwo_Both",
             "BG2_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG2_InM_onetwo_Both);
  BG3_InM_onetwo_Both =
    new TH1D("BG3_InM_onetwo_Both",
             "BG3_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG3_InM_onetwo_Both);
  BG4_InM_onetwo_Both =
    new TH1D("BG4_InM_onetwo_Both",
             "BG4_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG4_InM_onetwo_Both);
  BG5_InM_onetwo_Both =
    new TH1D("BG5_InM_onetwo_Both",
             "BG5_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG5_InM_onetwo_Both);
  BG6_InM_onetwo_Both =
    new TH1D("BG6_InM_onetwo_Both",
             "BG6_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG6_InM_onetwo_Both);
  BG7_InM_onetwo_Both =
    new TH1D("BG7_InM_onetwo_Both",
             "BG7_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG7_InM_onetwo_Both);
  BG8_InM_onetwo_Both =
    new TH1D("BG8_InM_onetwo_Both",
             "BG8_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG8_InM_onetwo_Both);
  BG9_InM_onetwo_Both =
    new TH1D("BG9_InM_onetwo_Both",
             "BG9_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG9_InM_onetwo_Both);
  BG10_InM_onetwo_Both =
    new TH1D("BG10_InM_onetwo_Both",
             "BG10_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(BG10_InM_onetwo_Both);
  PdgCase8_InM_onetwo_Both = new TH1D("PdgCase8_InM_onetwo_Both",
                                      "PdgCase8_InM_onetwo_Both; Id ;#",
                                      5000,
                                      -2500,
                                      2500);
  fHistoList_bg_InM_onetwo_Both.push_back(PdgCase8_InM_onetwo_Both);
  PdgCase8mothers_InM_onetwo_Both =
    new TH1D("PdgCase8mothers_InM_onetwo_Both",
             "PdgCase8mothers_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(PdgCase8mothers_InM_onetwo_Both);
  sameMIDcase8_InM_onetwo_Both =
    new TH1D("sameMIDcase8_InM_onetwo_Both",
             "sameMIDcase8_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(sameMIDcase8_InM_onetwo_Both);
  sameGRIDcase8_InM_onetwo_Both =
    new TH1D("sameGRIDcase8_InM_onetwo_Both",
             "sameGRIDcase8_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(sameGRIDcase8_InM_onetwo_Both);
  Case1ZYPos_InM_onetwo_Both =
    new TH2D("Case1ZYPos_InM_onetwo_Both",
             "Case1ZYPos_InM_onetwo_Both; z[cm]; y[cm]",
             400,
             -1,
             200,
             200,
  ...
  sameMIDcase8_mothedPDG_InM_onetwo_Both =
    new TH1D("sameMIDcase8_mothedPDG_InM_onetwo_Both",
             "sameMIDcase8_mothedPDG_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(
    sameMIDcase8_mothedPDG_InM_onetwo_Both);
  PdgCase8NonEComeFromTarget_InM_onetwo_Both =
    new TH1D("PdgCase8NonEComeFromTarget_InM_onetwo_Both",
             "PdgCase8NonEComeFromTarget_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(
  ...
  PdgCase8NonE_NOT_FromTarget_InM_onetwo_Both =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_onetwo_Both",
             "PdgCase8NonE_NOT_FromTarget_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(
  ...
  PdgCase8motherNonE_InM_onetwo_Both =
    new TH1D("PdgCase8motherNonE_InM_onetwo_Both",
             "PdgCase8motherNonE_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(PdgCase8motherNonE_InM_onetwo_Both);
  Case8ElFromDalitz_InM_onetwo_Both =
    new TH1D("Case8ElFromDalitz_InM_onetwo_Both",
  ...
  Case8NonElFrom_pn_InM_onetwo_Both =
    new TH1D("Case8NonElFrom_pn_InM_onetwo_Both",
  ...
  Case8NonElFrom_eta_InM_onetwo_Both = new TH1D(
  ...
  Case8NonElFrom_kaon_InM_onetwo_Both = new TH1D(
    "Case8NonElFrom_kaon_InM_onetwo_Both",
    "Case8NonElFrom_kaon_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(Case8NonElFrom_kaon_InM_onetwo_Both);
  sameMIDcase8NonEPdg_InM_onetwo_Both =
    new TH1D("sameMIDcase8NonEPdg_InM_onetwo_Both",
             "sameMIDcase8NonEPdg_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(sameMIDcase8NonEPdg_InM_onetwo_Both);
  sameMIDcase8NonEMotherPdg_InM_onetwo_Both =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_onetwo_Both",
             "sameMIDcase8NonEMotherPdg_InM_onetwo_Both; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_onetwo_Both.push_back(
    sameMIDcase8NonEMotherPdg_InM_onetwo_Both);
  sameMIDcase8NonEMotherIM_InM_onetwo_Both = new TH1D(
    "sameMIDcase8NonEMotherIM_InM_onetwo_Both",
    "sameMIDcase8NonEMotherIM_InM_onetwo_Both; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_onetwo_Both.push_back(
  ...
  sameMIDcase8NonEPdgFromTarget_InM_onetwo_Both =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_onetwo_Both",
  ...
  fHistoList_bg_InM_onetwo_Both.push_back(
    sameMIDcase8NonEComeFromTargetIM_InM_onetwo_Both);
  sameMIDcase8NonEComeFromTargetP_InM_onetwo_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetP_InM_onetwo_Both",
    "sameMIDcase8NonEComeFromTargetP_InM_onetwo_Both; P in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_onetwo_Both.push_back(
    sameMIDcase8NonEComeFromTargetP_InM_onetwo_Both);
  sameMIDcase8NonEComeFromTargetPt_InM_onetwo_Both = new TH1D(
    "sameMIDcase8NonEComeFromTargetPt_InM_onetwo_Both",
    "sameMIDcase8NonEComeFromTargetPt_InM_onetwo_Both; Pt in GeV/c^{2} ;#",
    200,
    0,
    10);
  fHistoList_bg_InM_onetwo_Both.push_back(
  ...
  AnnTruePairs =
    new TH1D("AnnTruePairs", "AnnTruePairs; Ann value ;#", 100, -1.2, 1.2);
  fHistoList_man_Both.push_back(AnnTruePairs);
  AnnFalsePairs =
    new TH1D("AnnFalsePairs", "AnnFalsePairs; Ann value ;#", 100, -1.2, 1.2);
  fHistoList_man_Both.push_back(AnnFalsePairs);
  AnnTruePairs_AfterCuts = new TH1D(
    "AnnTruePairs_AfterCuts", "AnnTruePairs; Ann value ;#", 100, -1.2, 1.2);
  fHistoList_man_Both.push_back(AnnTruePairs_AfterCuts);
  AnnFalsePairs_AfterCuts = new TH1D("AnnFalsePairs_AfterCuts",
                                     "AnnFalsePairs_AfterCuts; Ann value ;#",
                                     100,
                                     -1.2,
                                     1.2);
  ...
  // WAC
  Pi0InvMassReco_WAC =
    new TH1D("Pi0InvMassReco_WAC",
             "Pi0InvMassReco_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_man_WAC.push_back(Pi0InvMassReco_WAC);
  EMT_InvMass_WAC = new TH1D("EMT_InvMass_WAC",
                             "EMT_InvMass_WAC; invariant mass in GeV/c^{2};#",
                             1000,
                             0,
                             2.0);
  fHistoList_man_WAC.push_back(EMT_InvMass_WAC);
  Pi0_pt_vs_rap_WAC = new TH2D("Pi0_pt_vs_rap_WAC",
                               "Pi0_pt_vs_rap_WAC; rapidity y; p_{t} in GeV/c ",
                               90,
                               -2.,
                               7.,
                               60,
  ...
  Pi0_pt_vs_rap_est_WAC =
    new TH2D("Pi0_pt_vs_rap_est_WAC",
             "Pi0_pt_vs_rap_est_WAC; rapidity y; p_{t} in GeV/c ",
             10,
             0.,
             4.,
             40,
  ...
  DalitzPi0_WAC = new TH1D("DalitzPi0_WAC",
                           "DalitzPi0_WAC; invariant mass in GeV/c^{2};#",
                           1000,
                           0,
                           2.0);
  fHistoList_man_WAC.push_back(DalitzPi0_WAC);
  PhotonsPi0_WAC = new TH1D("PhotonsPi0_WAC",
                            "PhotonsPi0_WAC; invariant mass in GeV/c^{2};#",
                            1000,
                            0,
                            2.0);
  fHistoList_man_WAC.push_back(PhotonsPi0_WAC);
  MultiplicityGamma_WAC = new TH2D(
    "MultiplicityGamma_WAC",
    "MultiplicityGamma_WAC; Nof gammas in event; invariant mass in GeV/c^{2};#",
    400,
    0,
    30,
    1000,
  ...
  MultiplicityChargedParticles_WAC =
    new TH2D("MultiplicityChargedParticles_WAC",
  ...
  BG1_InM_WAC = new TH1D(
    "BG1_InM_WAC", "BG1_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG1_InM_WAC);
  BG2_InM_WAC = new TH1D(
    "BG2_InM_WAC", "BG2_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG2_InM_WAC);
  BG3_InM_WAC = new TH1D(
    "BG3_InM_WAC", "BG3_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG3_InM_WAC);
  BG4_InM_WAC = new TH1D(
    "BG4_InM_WAC", "BG4_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG4_InM_WAC);
  BG5_InM_WAC = new TH1D(
    "BG5_InM_WAC", "BG5_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG5_InM_WAC);
  BG6_InM_WAC = new TH1D(
    "BG6_InM_WAC", "BG6_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG6_InM_WAC);
  BG7_InM_WAC = new TH1D(
    "BG7_InM_WAC", "BG7_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG7_InM_WAC);
  BG8_InM_WAC = new TH1D(
    "BG8_InM_WAC", "BG8_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG8_InM_WAC);
  BG9_InM_WAC = new TH1D(
    "BG9_InM_WAC", "BG9_InM_WAC; invariant mass in GeV/c^{2};#", 1000, 0, 2.0);
  fHistoList_bg_InM_WAC.push_back(BG9_InM_WAC);
  BG10_InM_WAC = new TH1D("BG10_InM_WAC",
                          "BG10_InM_WAC; invariant mass in GeV/c^{2};#",
                          1000,
                          0,
                          2.0);
  fHistoList_bg_InM_WAC.push_back(BG10_InM_WAC);
  PdgCase8_InM_WAC =
    new TH1D("PdgCase8_InM_WAC", "PdgCase8_InM_WAC; Id ;#", 5000, -2500, 2500);
  fHistoList_bg_InM_WAC.push_back(PdgCase8_InM_WAC);
  PdgCase8mothers_InM_WAC = new TH1D("PdgCase8mothers_InM_WAC",
                                     "PdgCase8mothers_InM_WAC; Id ;#",
                                     5000,
                                     -2500,
                                     2500);
  fHistoList_bg_InM_WAC.push_back(PdgCase8mothers_InM_WAC);
  sameMIDcase8_InM_WAC =
    new TH1D("sameMIDcase8_InM_WAC",
             "sameMIDcase8_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8_InM_WAC);
  sameGRIDcase8_InM_WAC =
    new TH1D("sameGRIDcase8_InM_WAC",
             "sameGRIDcase8_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(sameGRIDcase8_InM_WAC);
  Case1ZYPos_InM_WAC = new TH2D("Case1ZYPos_InM_WAC",
                                "Case1ZYPos_InM_WAC; z[cm]; y[cm]",
                                400,
                                -1,
                                200,
                                200,
  ...
  sameMIDcase8_mothedPDG_InM_WAC =
    new TH1D("sameMIDcase8_mothedPDG_InM_WAC",
             "sameMIDcase8_mothedPDG_InM_WAC; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8_mothedPDG_InM_WAC);
  PdgCase8NonEComeFromTarget_InM_WAC =
    new TH1D("PdgCase8NonEComeFromTarget_InM_WAC",
             "PdgCase8NonEComeFromTarget_InM_WAC; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_WAC.push_back(PdgCase8NonEComeFromTarget_InM_WAC);
  PdgCase8NonE_NOT_FromTarget_InM_WAC =
    new TH1D("PdgCase8NonE_NOT_FromTarget_InM_WAC",
             "PdgCase8NonE_NOT_FromTarget_InM_WAC; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_WAC.push_back(PdgCase8NonE_NOT_FromTarget_InM_WAC);
  PdgCase8motherNonE_InM_WAC = new TH1D("PdgCase8motherNonE_InM_WAC",
                                        "PdgCase8motherNonE_InM_WAC; Id ;#",
                                        5000,
                                        -2500,
                                        2500);
  fHistoList_bg_InM_WAC.push_back(PdgCase8motherNonE_InM_WAC);
  Case8ElFromDalitz_InM_WAC =
    new TH1D("Case8ElFromDalitz_InM_WAC",
             "Case8ElFromDalitz_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(Case8ElFromDalitz_InM_WAC);
  Case8NonElFrom_pn_InM_WAC =
    new TH1D("Case8NonElFrom_pn_InM_WAC",
             "Case8NonElFrom_pn_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(Case8NonElFrom_pn_InM_WAC);
  Case8NonElFrom_eta_InM_WAC =
    new TH1D("Case8NonElFrom_eta_InM_WAC",
             "Case8NonElFrom_eta_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(Case8NonElFrom_eta_InM_WAC);
  Case8NonElFrom_kaon_InM_WAC =
    new TH1D("Case8NonElFrom_kaon_InM_WAC",
             "Case8NonElFrom_kaon_InM_WAC; invariant mass in GeV/c^{2};#",
             1000,
             0,
             2.0);
  fHistoList_bg_InM_WAC.push_back(Case8NonElFrom_kaon_InM_WAC);
  sameMIDcase8NonEPdg_InM_WAC = new TH1D("sameMIDcase8NonEPdg_InM_WAC",
                                         "sameMIDcase8NonEPdg_InM_WAC; Id ;#",
                                         5000,
                                         -2500,
                                         2500);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8NonEPdg_InM_WAC);
  sameMIDcase8NonEMotherPdg_InM_WAC =
    new TH1D("sameMIDcase8NonEMotherPdg_InM_WAC",
             "sameMIDcase8NonEMotherPdg_InM_WAC; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8NonEMotherPdg_InM_WAC);
  sameMIDcase8NonEMotherIM_InM_WAC =
    new TH1D("sameMIDcase8NonEMotherIM_InM_WAC",
  ...
  sameMIDcase8NonEPdgFromTarget_InM_WAC =
    new TH1D("sameMIDcase8NonEPdgFromTarget_InM_WAC",
             "sameMIDcase8NonEPdgFromTarget_InM_WAC; Id ;#",
             5000,
             -2500,
             2500);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8NonEPdgFromTarget_InM_WAC);
  sameMIDcase8NonEComeFromTargetIM_InM_WAC = new TH1D(
    "sameMIDcase8NonEComeFromTargetIM_InM_WAC",
    "sameMIDcase8NonEComeFromTargetIM_InM_WAC; invariant mass in GeV/c^{2};#",
    1000,
    0,
    2.0);
  fHistoList_bg_InM_WAC.push_back(sameMIDcase8NonEComeFromTargetIM_InM_WAC);
  sameMIDcase8NonEComeFromTargetP_InM_WAC =
    new TH1D("sameMIDcase8NonEComeFromTargetP_InM_WAC",
  ...
  sameMIDcase8NonEComeFromTargetPt_InM_WAC =
    new TH1D("sameMIDcase8NonEComeFromTargetPt_InM_WAC",
  ...
  Chi2_for_Primary = new TH1D(
    "Chi2_for_Primary", "Chi2_for_Primary; #chi^{2};entries", 200, 0, 20);
  fHistoList_manual.push_back(Chi2_for_Primary);
  Chi2_for_Secondary = new TH1D(
    "Chi2_for_Secondary", "Chi2_for_Secondary; #chi^{2};entries", 200, 0, 20);
