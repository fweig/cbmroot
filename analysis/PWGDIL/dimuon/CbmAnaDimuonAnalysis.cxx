...
//----------------- STS MC matching
		    ...
		    muMn_reco.Nmuch=kTRUE;
		      ...
		      muMn_reco.Ntrd=kTRUE;
	              ...
	              if(mass > (p0min+p1min*momentum+p2min*momentum*momentum) && 
                         mass < (p0max+p1max*momentum+p2max*momentum*momentum))muMn_reco.Ntof=kTRUE;
		    }
		  }
		}
	      }
		  ...
		  muPl_reco.Nsts=kTRUE;
		    ...
		    muPl_reco.Nmuch=kTRUE;
		      ...
		      muPl_reco.Ntrd=kTRUE;
	              ...
	              if(mass > (p0min+p1min*momentum+p2min*momentum*momentum) && 
                         mass < (p0max+p1max*momentum+p2max*momentum*momentum))muPl_reco.Ntof=kTRUE;
		    }
		  }
		}
	      }
 ...
 if(chi2vertex<=fChi2VertexCut){
   ...
   if(nStsHits>=fNofStsCut  && chi2sts<=fChi2StsCut){
	 ...
	 if(mass > (p0min+p1min*momentum+p2min*momentum*momentum) && 
         mass < (p0max+p1max*momentum+p2max*momentum*momentum))BgSup[5]->Fill(momentum);
       }
     }
   }
 }
...
void CbmAnaDimuonAnalysis::Finish(){
  ...
  dir1b->cd(); 
  for(int j=0; j<4; j++)effReco_P[j][1]->Write(); 
  for(int j=0; j<4; j++)effReco_Theta[j][1]->Write(); 
  for(int j=0; j<5; j++)eff4pi_P[j][1]->Write(); 
  for(int j=0; j<5; j++)eff4pi_Theta[j][1]->Write();
  ...
  dir2b->cd(); 
  for(int j=0; j<4; j++)effReco_P[j][2]->Write(); 
  for(int j=0; j<4; j++)effReco_Theta[j][2]->Write(); 
  for(int j=0; j<5; j++)eff4pi_P[j][2]->Write(); 
  for(int j=0; j<5; j++)eff4pi_Theta[j][2]->Write();
  ...
  dir3b->cd(); 
  for(int j=0; j<4; j++)effReco_P[j][0]->Write(); 
  for(int j=0; j<4; j++)effReco_Theta[j][0]->Write(); 
  for(int j=0; j<5; j++)eff4pi_P[j][0]->Write(); 
  for(int j=0; j<5; j++)eff4pi_Theta[j][0]->Write();   
  ff->Close();  
  //  fPlutoFile->Close();
  }else{
  TFile* f = new TFile("sup_histo.root","recreate");  
  for(int j=0; j<6; j++)BgSup[j]->Write(); 
  f->Close();
  }
