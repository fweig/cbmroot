...
Bool_t CbmEventGenerator::GenerateEvent(FairGenericStack* stack) {
  ...
  LOG(info) << GetName() << ": Event ID " << fEvent->GetEventID()
  		<< ", tracks " << fNTracks << ", vertex (" << fVertex.X() << ", "
			<< fVertex.Y() << ", " << fVertex.Z() << ") cm";
...
void CbmEventGenerator::MakeVertex() {
	...
	if ( fForceVertexInTarget && fTarget ) MakeVertexInTarget();
	else MakeVertexInFocalPlane();
...
void CbmEventGenerator::MakeVertexInTarget() {
	...
	while ( ! isInTarget ) {
		...
		if ( nSamples > 1000. ) LOG(fatal) << GetName() << ": Aborting after "
				<< nSamples << " beam samplings. Adjust beam and target.";
...
void CbmEventGenerator::Print(Option_t*) const {
	...
	LOG(info) << "Vertex smearing along beam "
			<< ( fSmearVertexZ ? "ON" : "OFF");
	if ( fEventPlane ) LOG(info) << "Random event plane angle between "
			<< fPhiMin << " and " << fPhiMax << " rad";
	else LOG(info) << "Fixed event plane angle = 0";
	LOG(info) << "Number of generators = "
			<< fGenList->GetEntries();
...
// -----   Set beam position   ---------------------------------------------
void CbmEventGenerator::SetBeamPosition(Double_t meanX, Double_t meanY,
