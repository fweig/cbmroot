...
void CbmTransport::ConfigureVMC() {
  ...
  if ( fEngine == kGeant3 ) {
    ...
    TG4RunConfiguration* runConfig
        = new TG4RunConfiguration(runConfigSettings[0],runConfigSettings[1],
                                  runConfigSettings[2]);
    vmc = new TGeant4("TGeant4", "C++ Interface to Geant4", runConfig);
    fGeant4Settings->Init(vmc);
  } //? Geant4

  else LOG(fatal) << GetName() << ": unknown transport engine!";
...
void CbmTransport::ForceUserDecays() {
  ...
  if ( (! fDecayModes.empty()) && fEngine == kGeant4 ) LOG(fatal) << GetName()
...
void CbmTransport::Run(Int_t nEvents) {
  ...
  if ( fOutFileName.IsNull() ) LOG(fatal) << GetName()
  		<< ": No output file specified!";
  if ( fParFileName.IsNull() ) LOG(fatal) << GetName()
  ...
  else LOG(warn) << GetName() << ": No target defined!";
...
void CbmTransport::SetTarget(const char* medium, Double_t thickness,
                             Double_t diameter, Double_t x, Double_t y,
