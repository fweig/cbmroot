...
namespace {  //anonymous namespace with helpers
  std::map<Int_t, ECbmModuleId> dbToCbmModuleIdMap = {
    {0, ECbmModuleId::kCave},
    {1, ECbmModuleId::kMagnet},
    {2, ECbmModuleId::kPipe},
    {4, ECbmModuleId::kMvd},
    {8, ECbmModuleId::kSts},
    {16, ECbmModuleId::kRich},
    {32, ECbmModuleId::kTrd},
    {64, ECbmModuleId::kTof},
    {128, ECbmModuleId::kPsd},
    {256, ECbmModuleId::kPlatform},
    {512, ECbmModuleId::kMuch},
    {1024, ECbmModuleId::kHodo},
    {2048, ECbmModuleId::kTarget},
    {4096, ECbmModuleId::kShield},
  };
  ...
  std::string LocalDbPath() {
    std::string path =
      std::string(gSystem->Getenv("VMCWORKDIR")) + "/geometry/db/local.db";
...
std::vector<std::string> CbmGeoSetupDbProvider::GetSetupTags() {
  ...
  std::string query =
    std::string("select stag,revision from setup order by revision desc");
  TSQLResult* resQ = db.Query(query.c_str());
  if (resQ == 0 || resQ->GetRowCount() == 0) return {};
...
std::vector<std::string> CbmGeoSetupDbProvider::GetFieldTags() {
  return ListTable("field", "tag");
};

std::vector<std::string> CbmGeoSetupDbProvider::GetMediaTags() {
  return ListTable("material", "matag");
};
...
CbmGeoSetup CbmGeoSetupDbProvider::GetSetupByTag(std::string setupTag,
  ...
  std::string query = "select s.idsd, s.stag, s.sdate, s.desc, "
                      ...
                      "where s.stag='"
                      + setupTag + "' "
                      + (revision.empty() ? "" : " and s.revision=" + revision)
...
CbmGeoSetupModule CbmGeoSetupDbProvider::GetModuleByTag(ECbmModuleId moduleId,
                                                        std::string tag) {
  TSQLiteServer db(LocalDbPath().c_str());
  ...
  if (resQ->GetRowCount() > 1)
    LOG(fatal) << "Geometry DB: found more than one entry for moduleId "
               << moduleId << " and tag " << tag;
  ...
  Double_t rot[9] = {std::atof(row->GetField(6)),
                     std::atof(row->GetField(7)),
                     std::atof(row->GetField(8)),
                     std::atof(row->GetField(9)),
                     std::atof(row->GetField(10)),
                     std::atof(row->GetField(11)),
                     std::atof(row->GetField(12)),
                     std::atof(row->GetField(13)),
                     std::atof(row->GetField(14))};
  module.GetMatrix().SetRotation(rot);
  Double_t trans[3] = {std::atof(row->GetField(15)),
                       std::atof(row->GetField(16)),
                       std::atof(row->GetField(17))};
  module.GetMatrix().SetTranslation(trans);
  Double_t scale[3] = {std::atof(row->GetField(18)),
                       std::atof(row->GetField(19)),
                       std::atof(row->GetField(20))};
...
CbmGeoSetupField CbmGeoSetupDbProvider::GetFieldByTag(std::string tag) {
  ...
  std::string query =
    std::string("select fi.idfi, fi.tag, fi.date, fi.desc, fi.author, "
  ...
  if (resQ->GetRowCount() > 1)
    LOG(fatal) << "Geometry DB: found more than one field with tag " << tag;
  ...
  field.GetMatrix().SetTranslation(std::atof(row->GetField(6)),
                                   std::atof(row->GetField(7)),
                                   std::atof(row->GetField(8)));
...
CbmGeoSetupMedia CbmGeoSetupDbProvider::GetMediaByTag(std::string tag) {
  ...
  if (resQ->GetRowCount() > 1)
    LOG(fatal) << "Geometry DB: found more than one material with tag " << tag;
...
void CbmGeoSetupDbProvider::LoadSetup(std::string setupTag,
                                      std::string revision) {
  ...
  for (auto module : moduleTags) {
    for (auto moduleId : moduleMap) {
      if (static_cast<int>(moduleId.first) == module.first) {
        moduleMap.at(dbToCbmModuleIdMap[module.first]) =
