...
private:
  //Entries properties
  const char* param_double_name
    [PDATABASE_MAX_DOUBLE_PARAM];  //the parameter name matching the ID
  ...
  const char* param_double_descr
    [PDATABASE_MAX_DOUBLE_PARAM];  //the parameter long description
  ...
  Bool_t
  CheckEntry(Int_t key);  //check if key with unique name already existing
...
public:
  ...
  Int_t
  MakeParamString(const char* paramname,
  ...
  Int_t
  GetParamDouble(const char* paramname);  //get "paramname" from double database
  Int_t
  GetParamString(const char* paramname);  //get "paramname" from string database
  Int_t GetParamInt(const char* paramname,
  ...
  Bool_t GetParamInt(Int_t pkey1, Int_t value1, Int_t pkey2, Int_t** result) {
    //	cout << pkey1 << ":" << value1<< ":" << pkey2<< ":" << endl;
    return GetParamInt(GetEntryInt(pkey1, value1), pkey2, result);
  };
  Bool_t
  GetParamDouble(Int_t pkey1, Int_t value1, Int_t pkey2, Double_t** result) {
    return GetParamDouble(GetEntryInt(pkey1, value1), pkey2, result);
  };
  Bool_t
  GetParamString(Int_t pkey1, Int_t value1, Int_t pkey2, const char** result) {
  ...
  Bool_t
  SetParamDouble(const char* name, const char* paramname, Double_t result) {
  ...
  Bool_t
  SetParamTObj(const char* name, const char* paramname, TObject* result) {
    Int_t key = GetEntry(name);
    if (key < 0) return kFALSE;
  ...
  //listings
  Int_t AddListEntry(const char* name,
                     const char* count,
                     const char* link,
                     const char* newname);
  Bool_t MakeListIterator(Int_t key,
                          const char* count,
                          const char* link,
                          Int_t* listkey);
  ...
  Bool_t
  ListEntries(Int_t key = -1, Int_t option = 0, const char* pattern = nullptr);
