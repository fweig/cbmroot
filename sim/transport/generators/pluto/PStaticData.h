...
public:
  ...
  bool GetFreezeOut(void) { return freeze; }
  ...
  void SetParticleBaryon(const char* id,
                         Int_t num = 1);  // baryon number by name
  ...
  /** TODO: (VF) The first method is commented out in the implementation file
     ** for unknown reasons. The second and third method reference the first one.
     ** Seems not to be used by us (CBM):
