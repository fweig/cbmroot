...
int PStaticData::GetSecondaryKey(int key, int defkey)
{
  ...
  while (makeDataBase()->MakeListIterator(key, nalias_param, lalias_param, &listkey)) {
    ...
    if (makeDataBase()->GetParamInt(listkey, defkey_param, &dummy)) {
      if (*dummy == defkey) return listkey;
    }
  }
  return -1;
}
...
int PStaticData::AddParticle(int pid, const char* name, double mass)
{
  ...
  ii = new int(0);
  if (!base->SetParamInt(pkey, "meson", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "baryon", ii)) return -1;
  ii = new int(0);  //Meson by default: No limitation in LMass
  if (!base->SetParamInt(pkey, "lepton", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "charge", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "spin", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "parity", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "ispin", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "pythiakf", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "widx", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "tdepth", ii)) return -1;
  ii = new int(0);
  if (!base->SetParamInt(pkey, "hdepth", ii)) return -1;
  return pid;
}
...
int PStaticData::GetParticleIDbyKF(const int kf)
{
  ...
  if (!makeDataBase()->GetParamInt(key, pid_param, &id)) return 0;
  return *id;
}
...
int PStaticData::GetDecayNProductsByKey(const int& key)
{
  ...
  if (makeDataBase()->GetParamInt(key, d5_param, &i_result)) return 5;
  if (makeDataBase()->GetParamInt(key, d4_param, &i_result)) return 4;
  if (makeDataBase()->GetParamInt(key, d3_param, &i_result)) return 3;
  if (makeDataBase()->GetParamInt(key, d2_param, &i_result)) return 2;
  if (makeDataBase()->GetParamInt(key, d1_param, &i_result)) return 1;
  return 0;
};
...
int PStaticData::GetDecayNProducts(const char* id)
{
  ...
  if (makeDataBase()->GetParamInt((char*) id, "d5", &i_result)) return 5;
  if (makeDataBase()->GetParamInt((char*) id, "d4", &i_result)) return 4;
  if (makeDataBase()->GetParamInt((char*) id, "d3", &i_result)) return 3;
  if (makeDataBase()->GetParamInt((char*) id, "d2", &i_result)) return 2;
  if (makeDataBase()->GetParamInt((char*) id, "d1", &i_result)) return 1;
  return 0;
}
...
int PStaticData::GetDecayIdxByKey(int key)
{
  ...
  Int_t* pos;
  if (!makeDataBase()->GetParamInt(key, didx_param, &pos)) return -1;
  return *pos;
}
...
void PStaticData::GetDecayModeByKey(int idx, int* id)
{
  ...
  if (makeDataBase()->GetParamInt(idx, d4_param, &d4_key)) found = 4;
  if (makeDataBase()->GetParamInt(idx, d5_param, &d5_key)) found = 5;
  if (makeDataBase()->GetParamInt(idx, d6_param, &d6_key)) found = 6;
  if (makeDataBase()->GetParamInt(idx, d7_param, &d7_key)) found = 7;
