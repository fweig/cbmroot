...
Bool_t PDataBase::CheckEntry(Int_t key) {
  ...
  Int_t pp = 0;
  if (strings[key][pp] == nullptr) return kFALSE;
  return kTRUE;
};
...
Bool_t PDataBase ::GetParamDouble(Int_t key,
  ...
  if (doubles[key][pp] == nullptr) return kFALSE;
...
Bool_t PDataBase ::GetParamInt(Int_t key,
  ...
  if (ints[key][pp] == nullptr) return kFALSE;
...
Bool_t PDataBase ::GetParamInt(Int_t key, Int_t pkey, Int_t** result) {
  ...
  if (pkey < 0) return kFALSE;
  if (ints[key][pkey] == nullptr) return kFALSE;
...
PDataBase ::GetParamTObj(Int_t key, const char* paramname, TObject** result) {
  ...
  if (tobjs[key][pp] == nullptr) return kFALSE;
...
Int_t PDataBase::AddListEntry(const char* name,
  ...
  if (key < 0) {
    ...
    //Fatal("AddListEntry","Unable to get entry %s",name);
    return -1;
  }
  ...
  if (!GetParamInt(key, count, &i_count)) {
    ...
    GetParamInt(lastkey1, link, &i_link);
    i_newlink = new Int_t(*i_link);
    SetParamInt(targetkey, link, i_newlink);
    //set new entry
    i_link = new Int_t(targetkey);
    SetParamInt(lastkey1, link, i_link);
  }
  return targetkey;
};
  ...
  for (int run = 0; run < 2; run++) {
    ...
    for (int i = start; i <= end; i++) {
      ...
      for (int pat = 0; pat < array->GetEntriesFast(); pat++) {
        ...
        //		 else if (invert && checkline) valid_key[i]++;
  ...
  for (int r = 0; r < 10000; r++)
    for (int i = 0; i < 50; i++) {
      ...
      if (!GetParamInt(GetEntry(strings[i][0]), pid_param, &i_result)) {
        ...
        cout << "PStaticData::ID: " << i << " not found" << endl;
      }
      mypid += *i_result;
      //	    cout << strings[i][0] << " is " << *i_result << endl;
    }
  cout << timer.RealTime() << endl;
  cout << mypid << endl;
}
