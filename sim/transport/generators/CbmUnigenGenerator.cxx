  ...
  fIonMap()
{
  ...
  if ( mode == kRotateFixed ) LOG(fatal) << GetName() << ": When choosing "
      << "mode kRotateFixed, a rotation angle must be specified!";
  if ( fileName[0] == '\0') return;
  if ( ! Init() ) LOG(fatal) << GetName()
...
void CbmUnigenGenerator::AddPrimary(FairPrimaryGenerator* primGen,
  ...
  primGen->AddTrack(pdgCode, momentum.Px(), momentum.Py(), momentum.Pz(),
                    0., 0., 0.);
  fNofPrimaries++;
}
// --------------------------------------------------------------------------
...
Bool_t CbmUnigenGenerator::Init() {
  ...
  switch (fMode) {
    case kStandard:
      ss << " standard; rotate event plane to zero"; break;
    case kRotateFixed:
      ss << " rotate event plane by fixed angle " << fPhi; break;
    case kNoRotation:
      ss << " no event plane rotation"; break;
    case kReuseEvents:
      ss << " re-use events if necessary; random event plane angle"; break;
...
void CbmUnigenGenerator::ProcessIon(FairPrimaryGenerator* primGen,
  ...
  if ( GetIonLambdas(pdgCode) ) {
    ...
    LOG(warn) << GetName() << ": Replacing hypernucleus (PDG "
        << pdgCode << ") by PDG " << ionPdg;
  } //? Lambdas in ion
...
Bool_t CbmUnigenGenerator::ReadEvent(FairPrimaryGenerator* primGen) {
  ...
  for (Int_t iPart = 0; iPart < fEvent->GetNpa(); iPart++) {
    ...
    else ProcessIon(primGen, particle->GetPdg(), momentum);
  ...
  // Info to screen
  LOG(info) << GetName() << ": Event ID " << eventId << ", particles "
      << nPart << ", primaries " << fNofPrimaries << ", b = " << impact
      << " fm, phi (source) = " << phi1 << " rad , phi (generated) = "
      << phi2 << " rad";
...
Int_t CbmUnigenGenerator::RegisterIons() {
    ...
    for (Int_t iParticle = 0; iParticle < fEvent->GetNpa(); ++iParticle) {
      ...
      if (pdgCode > 1e9) {  // ion
        ...
        if (fIonMap.find(ionName) == fIonMap.end()) {  // new ion
          ...
          nIons++;
          LOG(debug) << GetName() << ": Adding new ion " << ionName
              << ", PDG " << pdgCode << " (mass " << mass << ", charge "
              << charge << ", nLambda " << nLambda << ") from event "
              << iEvent << " at index " << iParticle;
        } //? new ion
  ...
  for (const auto &entry : fIonMap) run->AddNewIon(entry.second);
