...
CbmTrdModuleSimR::CbmTrdModuleSimR(Int_t mod, Int_t ly, Int_t rot)
  : CbmTrdModuleSim(mod, ly, rot),
    ...
    fAddCrosstalk(true), //for pulsed mode
...
void CbmTrdModuleSimR::ProcessPulseBuffer(Int_t address, Bool_t FNcall, Bool_t MultiCall=false, Bool_t down=true, Bool_t up=true){
  ...
  for(size_t i=0;i<fPulseBuffer[address].first.size();i++){
    ...
    if(fPulseBuffer[address].first[i] <= fClipLevel && fPulseBuffer[address].first[i] >= -12.)  temp.push_back((Int_t) fPulseBuffer[address].first[i]);
  ...
  if(!CbmTrdDigitizer::IsTimeBased())       corr=CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC); //correction for EB case is done later, due to the event time 0 and the unsigned data member for the time in the digi
  ...
  if(fDebug){    
    ...
    if(trigger == 1 && !MultiCall){
      ...
      if(fLinkQA[address].size() == 2 && fLinkQA[address][1]["Event"] != fLinkQA[address][0]["Event"]){
      	...
      	fQA->Fill("Links QA time",(digi->GetCharge() - fMCQA[address]) / fMCQA[address], fLinkQA[address][1]["Time"] - fLinkQA[address][0]["Time"]);
  	...
  	fQA->Fill("Links QA time FN",(digi->GetCharge() - fMCQA[address]) / fMCQA[address], fLinkQA[address][1]["Time"] - fLinkQA[address][0]["Time"]);
  ...
  if(!FNcall && fPrintPulse)  std::cout<<"main call    charge: "<<digi->GetCharge()<<"   col : " << col<<"   lay : " << CbmTrdAddress::GetLayerId(address)<<"   trigger: " << trigger<<"    time: " << digi->GetTime()<<std::endl;
  if(FNcall && fPrintPulse)   std::cout<<"FN call     charge: "<<digi->GetCharge()<<"   col : " << col<<"   lay : " << CbmTrdAddress::GetLayerId(address)<< "   trigger: " << trigger<<"    time: " << digi->GetTime()<<std::endl;
...
//_______________________________________________________________________________
void CbmTrdModuleSimR::AddDigitoPulseBuffer(Int_t address, Double_t reldrift,Double_t charge, Double_t /*chargeTR*/, Double_t time, Int_t /*trigger*/, Int_t /*epoints*/, Int_t /*ipoint*/)
{
  ...
  else for(auto ini=0;ini<3;ini++)  {std::vector<Int_t> v;fMCBuffer[address].push_back(v);}    
  ...
  if(fTimeBuffer[address] > 0) {
    ...
    Bool_t found=false;
    for(Int_t links=0;links<fPulseBuffer[address].second->GetNofLinks();links++) if(fPulseBuffer[address].second->GetLink(links).GetIndex() == fPointId) found=true;
    if(!found)  {
...
std::vector<Double_t> CbmTrdModuleSimR::MakePulse(Double_t charge,std::vector<Double_t> pulse, Int_t address){
  ...
  for(Int_t i=0;i<32;i++){
    if(i < fPresamples) {sample[i]=0.;continue;}
    if(fTimeShift)     sample[i]=fCalibration*charge*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
    if(!fTimeShift)    sample[i]=fCalibration*charge*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
...
void CbmTrdModuleSimR::AddToPulse(Int_t address,Double_t charge,Double_t reldrift,std::vector<Double_t> pulse){
  ...
  for(Int_t i=0;i<32;i++){
    if(i < fPresamples+startbin) {pulse[i]=temppulse[i];continue;}
  ...
  if(comptrigger==0 && trigger==1){
    for(Int_t i=0;i<32;i++) {
      if(i < fPresamples && startbin >= fPresamples) {pulse[i]=temppulse[i+startbin-fPresamples];continue;}
      if(i < fPresamples && startbin < fPresamples)  {pulse[i]=temppulse[i+startbin];continue;}
      Int_t addtime=i-fPresamples;
      Int_t shift=startbin+i;
      if(fTimeShift){
  	if(shift<32)    pulse[i]=temppulse[shift]+fCalibration*charge*1e6*CalcResponse(addtime*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
  	else            pulse[i]=fCalibration*charge*1e6*CalcResponse(addtime*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
  	if(pulse[i] > fClipLevel && fClipping) pulse[i]=fClipLevel-1;  //clipping
      }
      if(!fTimeShift){
  	if(shift<32)    pulse[i]=temppulse[shift]+fCalibration*charge*1e6*CalcResponse(addtime*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
  	else            pulse[i]=fCalibration*charge*1e6*CalcResponse(addtime*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
  	if(pulse[i] > fClipLevel && fClipping) pulse[i]=fClipLevel-1;  //clipping
      }
    }
    fTimeBuffer[address]=fCurrentTime;    
...
Bool_t CbmTrdModuleSimR::CheckMulti(Int_t address,std::vector<Double_t> pulse){
  ...
  if(trigger==2){
    ...
    Int_t ncols= fDigiPar->GetNofColumns();
    //    Double_t timeshift = gRandom->Uniform(0.,CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));      
    Double_t timeshift = ((Int_t) (fMultiBuffer[address].second*10) % (Int_t) (CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)*10)) / 10; 
...
//    Double_t dt=TMath::Abs(fMultiBuffer[address].second - fTimeBuffer[address]);
    ...
    for(Int_t i=0;i<32;i++) {
      if(i < fPresamples) {pulse[i]=temppulse[shift+i-fPresamples];continue;}
      if(shift+i-fPresamples < 32){
      	if(fTimeShift)    pulse[i]=temppulse[shift+i-fPresamples]+fCalibration*fMultiBuffer[address].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
      	if(!fTimeShift)   pulse[i]=temppulse[shift+i-fPresamples]+fCalibration*fMultiBuffer[address].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
      }
      else{
      	if(fTimeShift)    pulse[i]=fCalibration*fMultiBuffer[address].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
      	if(!fTimeShift)   pulse[i]=fCalibration*fMultiBuffer[address].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
      }
    ...
    while(FNtrigger==1  && FNaddress!=0){
      ...
      FNpulse=fPulseBuffer[FNaddress].first;
      for(Int_t i=0;i<32;i++) {
	if(i < fPresamples && temphigh[FNaddress].size() > 0.) {pulse[i]=temphigh[FNaddress][shift+i-fPresamples];continue;}
	if(fTimeShift){
	  if((size_t) shift+i-fPresamples<temphigh[FNaddress].size())             FNpulse[i]=temphigh[FNaddress][shift+i-fPresamples]+fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
	  else                                                           FNpulse[i]=fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
	  if(FNpulse[i] > fClipLevel && fClipping) FNpulse[i]=fClipLevel-1;  //clipping
	}
	if(!fTimeShift){
	  if((size_t) shift+i-fPresamples<temphigh[FNaddress].size())             FNpulse[i]=temphigh[FNaddress][shift+i-fPresamples]+fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
	  else                                                           FNpulse[i]=fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
	  if(FNpulse[i] > fClipLevel && fClipping) FNpulse[i]=fClipLevel-1;  //clipping
	}	
	...
	// }	
      ...
      if(!fMCBuffer[FNaddress].empty())      for(UInt_t links=0;links<fMCBuffer[FNaddress][0].size();links++)  FNdigiMatch->AddLink(CbmLink(fMultiBuffer[FNaddress].first, fMCBuffer[FNaddress][0][links], fMCBuffer[FNaddress][1][links],  fMCBuffer[FNaddress][2][links]));
    ...
    while(FNtrigger==1 && FNaddress!=0){
      ...
      FNpulse=fPulseBuffer[FNaddress].first;
      for(Int_t i=0;i<32;i++) {
	if(i < fPresamples && templow[FNaddress].size() > 0.) {pulse[i]=templow[FNaddress][shift+i-fPresamples];continue;}
	if(fTimeShift){	  
	  if((size_t) shift+i-fPresamples<templow[FNaddress].size())             FNpulse[i]=templow[FNaddress][shift+i-fPresamples]+fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
	  else                                                          FNpulse[i]=fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+timeshift);
	  if(FNpulse[i] > fClipLevel && fClipping) FNpulse[i]=fClipLevel-1;  //clipping
	}
	if(!fTimeShift){
	  if((size_t) shift+i-fPresamples<templow[FNaddress].size())             FNpulse[i]=templow[FNaddress][shift+i-fPresamples]+fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
	  else                                                          FNpulse[i]=fCalibration*fMultiBuffer[FNaddress].first*1e6*CalcResponse((i-fPresamples)*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
	  if(FNpulse[i] > fClipLevel && fClipping) FNpulse[i]=fClipLevel-1;  //clipping
	}
      }
    ...
    CbmMatch* digiMatch = new CbmMatch();
    if(!fMCBuffer[address].empty())    for(UInt_t links=0;links<fMCBuffer[address][0].size();links++)  digiMatch->AddLink(CbmLink(fMultiBuffer[address].first, fMCBuffer[address][0][links], fMCBuffer[address][1][links],  fMCBuffer[address][2][links]));
    fPulseBuffer[address].second = digiMatch;
    if(fDebug){
      std::vector<std::map<TString,Int_t>> vecLink;
      std::map<TString,Int_t>  LinkQA;    
...
Double_t CbmTrdModuleSimR::CalcResponse(Double_t t)
{
  ...
  else                    return (t/fTau)*(t/fTau)*TMath::Exp(-(t/fTau));
...
Bool_t CbmTrdModuleSimR::MakeDigi(CbmTrdPoint *point, Double_t time, Bool_t TR)
{
  ...
  if(fPulseSwitch && !fMessageConverter->GetSetter()){    
    ...
    for(Int_t i=frecostart;i<=frecostop;i++) recomask.push_back(i);
  ...
  for (Int_t i = 0; i < 3; i++)  fQAPosition[i]=point_in[i];
  for (Int_t i = 0; i < 3; i++)  fQAPos_out[i]=point_out[i];
  
  fCurrentTime -= 1e9 * (point_in[2] / 100) / 2.99e8; // subtract time of flight to the detector layer
  ...
  if(fDistributionMode == 4){
    Double_t dist_gas = TMath::Sqrt(cluster_delta[0]*cluster_delta[0]+cluster_delta[1]*cluster_delta[1]+cluster_delta[2]*cluster_delta[2]);
  ...
  if(!fPulseSwitch){
    for (Int_t ipoints = 0; ipoints < epoints; ipoints++){
      ...
      if(!dist) break;
      if(fDistributionMode == 4)  for(Int_t i=0;i<3; i++) cluster_pos[i]=steps.second[i+ipoints*3];
  ...
  if(fPulseSwitch){
	...
	for (Int_t i = 0; i < 3; i++)  printf("  (%i) | in: %7.3f + delta: %7.3f * cluster: %i/%i = cluster_pos: %7.3f out: %7.3f g_in:%f g_out:%f\n",i,local_point_in[i],cluster_delta[i],ipoints,(Int_t)nCluster,cluster_pos[i],local_point_out[i],point_in[i],point_out[i]);
      ...
      Double_t Drift_x = TMath::Abs(double(int(cluster_pos[0]*1000000) % int(fDigiPar->GetAnodeWireSpacing()*100))/100);
      ...
      if(TMath::Abs(AddDrifttime(Drift_x,cluster_pos[2])) < driftcomp && TMath::Abs(AddDrifttime(Drift_x,cluster_pos[2])) > 0.)     {driftcomp=TMath::Abs(AddDrifttime(Drift_x,cluster_pos[2]));start=ipoints;}
    }
...
void CbmTrdModuleSimR::ScanPadPlane(const Double_t* local_point, Double_t reldrift,Double_t clusterELoss, Double_t clusterELossTR,Int_t epoints,Int_t ipoint)
{
  ...
  if (sectorId < 0 && columnId < 0 && rowId < 0) {
	...
	if(!CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch && print)            {
	  ...
	  std::cout<<" time: " << fCurrentTime<< "  col: "  << iCol<< "  row: " << iRow - rowId<< "  secrow: " << secRow<< "   charge: " << ch*1e6<< " 1 " << std::endl;
	}
	if(CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch)             AddDigitoBuffer(address, ch, tr, fCurrentTime, Int_t (1));
	if(fPulseSwitch)                                                {if(fDebug)   fQA->Fill("E self MC",ch*epoints*1e6);AddDigitoPulseBuffer(address, reldrift,ch, tr, fCurrentTime, Int_t (1),epoints,ipoint);}
	...
	while(!lowerend){
	  ...
	  if(ch>=(fMinimumChargeTH/epoints) && !CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch && print)                   {
	    std::cout<<" time: " << fCurrentTime<< "  col: "  << iCol-collow<< "  row: " << iRow - rowId<< "  secrow: " << secRow<<"   charge: " << ch*1e6<< " 1 " << std::endl;
	    AddDigi( address, ch, tr, fCurrentTime, Int_t (1));collow++;
	  }
	  if(ch<(fMinimumChargeTH/epoints) && !CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch && print)                    {
	    std::cout<<" time: " << fCurrentTime<< "  col: "  << iCol-collow<< "  row: " << iRow - rowId<< "  secrow: " << secRow<< "   charge: " << ch*1e6<< " 0 " << std::endl;  
	  ...
	  if(ch>=(fMinimumChargeTH/epoints) && fPulseSwitch && !print)           {
	    ...
	    AddDigitoPulseBuffer(address, reldrift,ch, tr, fCurrentTime, Int_t (1),epoints,ipoint);collow++;} 
	  if(ch<(fMinimumChargeTH/epoints) && fPulseSwitch && !print)            {
	    if(fDebug)   fQA->Fill("E FN MC",ch*epoints*1e6);
	    AddDigitoPulseBuffer(address, reldrift,ch, tr, fCurrentTime, Int_t (2),epoints,ipoint);lowerend=true;}
	...
	while(!upperend){
	  ...
	  if(ch>=(fMinimumChargeTH/epoints) && !CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch && print)                   {
	    std::cout<<" time: " << fCurrentTime<< "  col: "  << iCol+colhigh<< "  row: " << iRow - rowId<< "  secrow: " << secRow<< "   charge: " << ch*1e6<< " 1 " << std::endl;  
	    AddDigi( address, ch, tr, fCurrentTime, Int_t (1));colhigh++;
	  }
	  if(ch<(fMinimumChargeTH/epoints) && !CbmTrdDigitizer::IsTimeBased() && !fPulseSwitch && print)                    {
	    std::cout<<" time: " << fCurrentTime<< "  col: "  << iCol+colhigh<< "  row: " << iRow - rowId<< "  secrow: " << secRow<< "   charge: " << ch*1e6<< " 0 " << std::endl;  
	  ...
	  if(ch<(fMinimumChargeTH/epoints) && fPulseSwitch && !print)            {
	    if(fDebug)   fQA->Fill("E FN MC",ch*epoints*1e6);
	    if(ipoint == epoints-1 && epoints > 1)    AddDigitoPulseBuffer(address, reldrift,ch, tr, fCurrentTime, Int_t (2),epoints,ipoint);
	    if(ipoint != epoints-1 && epoints > 1)    AddDigitoPulseBuffer(address, reldrift,ch, tr, fCurrentTime, Int_t (2),epoints,ipoint);
 ...
 */  
  ...
  if(!fDigiPar){
    LOG(warn) << GetName() << "::SetAsicPar : No Digi params for module "<< fModAddress <<". Try calling first CbmTrdModSim::SetDigiPar.";
   return;
  }
        ...
        if ((rowId % gRow[iFebGroup]) == 0){
          if ((c % gCol[iFebGroup]) == 0){
            ...
            if (local_point[0] > 2*fDx){
              LOG(error) << "CbmTrdModuleSimR::SetAsicPar: asic position x=" << local_point[0] << " is out of bounds [0," << 2*fDx<< "]!";
              fDigiPar->Print("all");
            }
            if (local_point[1] > 2*fDy){     
              LOG(error) << "CbmTrdModuleSimR::SetAsicPar: asic position y=" << local_point[1] << " is out of bounds [0," << 2*fDy<< "]!";
              fDigiPar->Print("all");
            }
            for (Int_t ir = rowId; ir < rowId + gRow[iFebGroup]; ir++) {
              for (Int_t ic = c; ic < c + gCol[iFebGroup]; ic++) {
                if (ir >= fDigiPar->GetNofRows() )     LOG(error) << "CbmTrdModuleSimR::SetAsicPar: ir " << ir << " is out of bounds!";
                if (ic >= fDigiPar->GetNofColumns() )  LOG(error) << "CbmTrdModuleSimR::SetAsicPar: ic " << ic << " is out of bounds!";
    ...
    for (Int_t r = 0; r < nRow; r++){
      for (Int_t c = 0; c < nCol; c++){
        ...
        if (fAsicPar->GetAsicAddress(channelAddress) == -1)
          LOG(error) << "CbmTrdModuleSimR::SetAsicPar: Channel address:" << channelAddress << " is not or multible initialized in module " << fModAddress << "(ID:" << CbmTrdAddress::GetModuleId(fModAddress) << ")" << "(s:" << s << ", r:" << r << ", c:" << c << ")";
      }
    }
  }
}
...
{ 
  ...
  fTau = tau;
  Double_t sum=0;
  for(auto i=frecostart;i<=frecostop;i++) sum+=fCalibration*CalcResponse(i*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
  fEReco     = sum;
}
...
Double_t CbmTrdModuleSimR::AddNoise(Double_t charge){
  ...
  if (fSigma_noise_keV > 0.0 && CbmTrdDigitizer::AddNoise() && !fPulseSwitch){
    Double_t noise = gRandom->Gaus(0, fSigma_noise_keV * 1e-6 );// keV->GeV // add only once per digi and event noise !!!
...
Int_t CbmTrdModuleSimR::AddCrosstalk(Double_t address,Int_t i, Int_t sec,Int_t row,Int_t col,Int_t ncols){
  ...
  if (fAddCrosstalk && fPulseSwitch){
    Int_t FNaddress=0;
    if(col>=1)           FNaddress = CbmTrdAddress::GetAddress(CbmTrdAddress::GetLayerId(address), CbmTrdAddress::GetModuleId(fModAddress), sec, row, col-1);
    if(fPulseBuffer[FNaddress].first.size() >= (size_t) i+1)   cross += fPulseBuffer[FNaddress].first[i] * fCrosstalkLevel;
...
Double_t CbmTrdModuleSimR::GetStep(Double_t dist,Int_t roll)
{
  ...
  std::pair<Double_t,Double_t> bethe[12] = {
    std::make_pair(1.5,1.5),std::make_pair(2,1.1),std::make_pair(3,1.025),std::make_pair(4,1),std::make_pair(10,1.1),
    std::make_pair(20,1.2),std::make_pair(100,1.5),std::make_pair(200,1.6),std::make_pair(300,1.65),std::make_pair(400,1.675),
    std::make_pair(500,1.7),std::make_pair(1000,1.725)
  };
...
std::pair<Int_t,std::vector<Double_t>> CbmTrdModuleSimR::GetTotalSteps(Double_t In[3],Double_t Out[3],Double_t dist)
{
  ...
  Double_t delta[3];
  Double_t s=0;
  for (Int_t i = 0; i < 3; i++)  delta[i] = (Out[i] - In[i]);
  ...
  for(Int_t i = 1; i<steps ; i++){
    ...
    if(prob >= roll)  {
      ...
      for (Int_t n = 0; n < 3; n++)    pos[n] = lastpos[n] +  move * delta[n];
      for (Int_t n = 0; n < 3; n++)    lastpos[n] = pos[n];
      //      Double_t r = TMath::Sqrt((In[0]-pos[0])*(In[0]-pos[0])+(In[1]-pos[1])*(In[1]-pos[1]));
      if( TMath::Abs(pos[0]) <  fDigiPar->GetSizeX() && TMath::Abs(pos[1]) <  fDigiPar->GetSizeY() && (pos[2]) <  Out[2])  {posvec.push_back(pos[0]);posvec.push_back(pos[1]);posvec.push_back(pos[2]);pointcount++;i=1;  roll = gRandom->Integer(100);  continue;}
    }
    if( TMath::Abs(pos[0]) < fDigiPar->GetSizeX() && TMath::Abs(pos[1]) <  fDigiPar->GetSizeY() && (pos[2]) <  Out[2])  {continue;}
    break;
  }
