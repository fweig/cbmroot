...
//      nofLatticeHits++;
  ...
  if(trackLength>0.) trackLength = TMath::Sqrt(trackLength);
    ...
    LOG(warn) << GetName() << "::MakeDigi: NULL track length for"
    " dEdx("<<std::setprecision(5)<<ELossdEdX*1e6<<") keV ";
    return kFALSE;
  }
  if(txy>0.) txy = TMath::Sqrt(txy);
  else{
    LOG(warn) << GetName() << "::MakeDigi: NULL xy track length projection for"
    " dEdx("<<std::setprecision(5)<<ELossdEdX*1e6<<") keV ";
    return kFALSE;
  }
  ...
  if(TMath::Abs(lout[0]-pos[0])>1.e-3){
    LOG(warn) << GetName() <<"::MakeDigi: Along wire coordinate error : x_sim="
      <<std::setprecision(5)<<lout[0]<<" x_calc="<<std::setprecision(5)<<pos[0];
  }
  if(TMath::Abs(ELossdEdX-e)>1.e-3){
    LOG(warn) << GetName() <<"::MakeDigi: dEdx partition to anode wires error : E[keV] = "
      <<std::setprecision(5)<<ELossdEdX*1e6<<" Sum(Ei)[keV]="<<std::setprecision(5)<<e*1e6;
  }
...
*/
  ...
  if(!fTriangleBinning->SetOrigin(dx, dy)){
    LOG(warn) << "CbmTrdModuleSimT::ScanPadPlane: Hit outside integration limits ["
  ...
  if(VERBOSE) {
    ...
    for(Int_t ir(nr); ir--; ){
      printf("      r[%d] ", ir);
      for(Int_t ic(0); ic<nc; ic++) printf("%6.4f/%6.4f ", 1.e2*array[ic][ir][0], 1.e2*array[ic][ir][1]);
      printf("\n");
    }
  }
 ...
 */  
  ...
  if(VERBOSE) printf("CbmTrdModuleSimT::FlushBuffer(%llu) FASP start[%llu] end[%llu] closeTS[%c]\n", time, fFASP->GetStartTime(), fFASP->GetEndTime(), (closeTS?'y':'n'));
  ...
  for(; it!=fBuffer.end(); it++){
    ...
    if(asicAddress<0){
      LOG(warn) << GetName() << "::FlushBuffer: FASP Calibration for ro_ch " << localAddress << " in module " << fModAddress <<" missing.";
    } else {
      LOG(debug) << GetName() << "::FlushBuffer: Found FASP "<< asicAddress%1000 <<" for ro_ch " << localAddress << " in module " << fModAddress;
...
//       chFasp[1] = fasp->GetChannel(localAddress, 1);
  ...
  while(it!=fBuffer.end()){
    ...
    while(iv != fBuffer[localAddress].end()){
      ...
      if(col<0){
        ...
        if(VERBOSE) printf("CbmTrdModuleSimT::FlushBuffer : request ly[%d] mod[%d] sec[%d] srow[%d] col[%d]\n", fLayerId, CbmTrdAddress::GetModuleId(fModAddress), sec, srow, col);
      ...
      digiMatch = iv->second;
      fDigitizer->SendData(digi, digiMatch); n++;
      iv=fBuffer[localAddress].erase(iv); // remove from saved buffer
    }
  ...
  if(VERBOSE) printf("CbmTrdModuleSimT::FlushBuffer : write %d digis from %duns to %duns. Digits still in buffer %d\n", n, TMath::Nint(timeMin),TMath::Nint(timeMax), nDigiLeft);
