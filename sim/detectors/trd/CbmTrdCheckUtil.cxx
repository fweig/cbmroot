...
void CbmTrdCheckUtil::CreateHist(std::string name,
  ...
  if (f2D[name]) return;
  if (ybins == 0)
    f1D[name] = new TH1D(name.data(), name.data(), xbins, xlow, xhigh);
  else
    f2D[name] = new TH2D(
      name.data(), name.data(), xbins, xlow, xhigh, ybins, ylow, yhigh);
}
...
void CbmTrdCheckUtil::CreateProfile(std::string name,
  ...
  if (ybins == 0)
    ...
    fProfile2D[name] = new TProfile2D(
      name.data(), name.data(), xbins, xlow, xhigh, ybins, ylow, yhigh);
}
...
void CbmTrdCheckUtil::CreateProfile3D(std::string name,
  ...
  fProfile3D[name] = new TProfile3D(name.data(),
                                    name.data(),
                                    xbins,
                                    xlow,
                                    xhigh,
