...
void CbmTrdRawToDigiR::SetPars(Int_t mode,Double_t cal,Double_t tau,std::vector<Int_t> mask){
  ...
  //default
  if(mode==0){
    for(UInt_t i=0;i<=mask.size();i++) fSampleMask.push_back(mask[i]);
    fCalibration = cal;
    fTau = tau;
    Double_t sum=0;
    for(UInt_t i=0;i<mask.size();i++) sum+=fCalibration*CalcResponse(mask[i]*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
    fEReco     = sum;
    FillLookUps();
  }
  if(mode==1){
    for(Int_t i=fMaxBin;i<=fMaxBin;i++) fSampleMask.push_back(i);
    fCalibration = cal;
    fTau = tau;
    fLookUp = 0;
    Double_t sum=0;
    for(UInt_t i=0;i<fSampleMask.size();i++) sum+=fCalibration*CalcResponse(fSampleMask[i]*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
    fEReco     = sum;
    auto start = high_resolution_clock::now(); 
    FillLookUps();
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start);
    if(fDebug)   std::cout<<std::endl<<std::endl<<"filling took: "<<duration.count()<<"  microseconds "<<std::endl<<fEReco<<std::endl<<std::endl<<std::endl;
  }
  if(mode==2){
    Double_t sum=0;
    for(UInt_t i=0;i<fSampleMask.size();i++) sum+=fCalibration*CalcResponse(fSampleMask[i]*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC));
...
void CbmTrdRawToDigiR::FillLookUps(std::string write){
  ...
  if(fLookUp == 1){
    for(Int_t shift=0.;shift<CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC);shift++){      
      Double_t sum=0;
      for(Int_t i=fMinBin;i<=fMaxBin;i++) sum+=fCalibration*CalcResponse(fSampleMask[i]*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+shift);
      fEReco     = sum;
	...
	while(true){
	  ...
	  if(!(fElookupAsym[max][a-extrapolate-count][b-extrapolate] > 0)) {
	    fElookupAsym[max][a-extrapolate-count][b-extrapolate]=fElookupAsym[max][a-extrapolate][b-extrapolate];
	    if(fDebug && max == 200)  fQA->Fill("Asym",a-extrapolate-count,b-extrapolate,fElookupAsym[max][a][b]);
	    count++;
	  }
	  count=1;
	  if(!(fElookupAsym[max][a-extrapolate+count][b-extrapolate] > 0)) {
  ...
  if(fLookUp == 2){
	...
	if(!(fElookupAsym[max][a][b] > 0)) fElookupAsym[max][a][b]=shift;
	sum =0.;
	for(UInt_t i=0;i<fSampleMask.size();i++)	sum+=energy * fCalibration*CalcResponse(fSampleMask[i]*CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)+shift);
...
void CbmTrdRawToDigiR::ReadMaps(std::string file){
  if(fLookUp == 3){
	...
	if(fDebug)  fQA->Fill("Read Small",h->GetXaxis()->GetBinCenter(x),h->GetYaxis()->GetBinCenter(y),h->GetBinContent(x,y));
      }
    }
	...
	if(fDebug)  fQA->Fill("Read Asym",h->GetXaxis()->GetBinCenter(x),h->GetYaxis()->GetBinCenter(y),h->GetBinContent(x,y));
      }
    }
    f.Close();
  }
...
CbmTrdDigi* CbmTrdRawToDigiR::MakeDigi(std::vector<Int_t> samples, Int_t channel, Int_t uniqueModuleId, ULong64_t time, Bool_t FN){
  ...
  if(fReadFile == ""){
    ...
    if(fLookUp == 4 && FN)   {
      digicharge=fElookup[samples[fMaxBin]];
    }
...
Float_t CbmTrdRawToDigiR::GetTimeShift(std::vector<Int_t> samples){
  ...
  if(fReadFile == ""){
    if(fLookUp == 1)   {
      shift=fElookupAsym[samples[fMaxBin]][samples[fMinBin]][samples[fHighBin]];
    }
    else{
      return 0.;
    }
    ...
    if(fLookUp == 3)   {
      shift=fElookupA[samples[fMinBin]][samples[fMaxBin]];
    }
    if(fLookUp == 4)   {
      shift=fElookupA[samples[fMinBin]][samples[fMaxBin]];
    }
...
Double_t CbmTrdRawToDigiR::GetCharge(std::vector<Int_t> samples, Int_t shift){
  ...
  if(fReadFile == ""){
    if(fLookUp == 1)   {      
      ...
      else           charge=fElookupSmall[fElookupAsym[samples[fMaxBin]][samples[fMinBin]][samples[fHighBin]]][samples[fMaxBin]];
    }
    if(fLookUp == 2)   {      
      Int_t samplesum=0;
      for(size_t i=0;i<fSampleMask.size();i++){
    ...
    if(fLookUp == 3)   {      
      if(shift > -1) charge=fElookupSmall[shift][samples[fMaxBin]];
      else           charge=fElookupSmall[fElookupA[samples[fMinBin]][samples[fMaxBin]]][samples[fMaxBin]];
    }
    if(fLookUp == 4)   {      
      if(shift > -1) charge=fElookupSmall[shift][samples[fMaxBin]];
      else           charge=fElookupSmall[fElookupA[samples[fMinBin]][samples[fMaxBin]]][samples[fMaxBin]];
