...
void CbmStsDigitize::Exec(Option_t* /*opt*/) {
  ...
  LOG(info) << left << setw(15) << GetName() << "["
            ...
            << " Points: processed " << fNofPointsProc
            << ", ignored " << fNofPointsIgno
...
void CbmStsDigitize::Finish() {
  ...
  // ---  In time-based mode: process the remaining signals in the buffers
    ...
    for ( auto& entry : fModules ) entry.second->ProcessAnalogBuffer(-1.);
...
void CbmStsDigitize::SetParContainers()
{
  ...
  fParSim = static_cast<CbmStsParSim*>
    (rtdb->getContainer("CbmStsParSim"));
  fParSetModule = static_cast<CbmStsParSetModule*>
    (rtdb->getContainer("CbmStsParSetModule"));
  fParSetSensor = static_cast<CbmStsParSetSensor*>
    (rtdb->getContainer("CbmStsParSetSensor"));
...
UInt_t CbmStsDigitize::InitSensors() {
  ...
  for (Int_t iSensor = 0; iSensor < fSetup->GetNofSensors(); iSensor++) {
    ...
    auto result = fSensors.insert( std::make_pair( sensAddress,
      fSensorFactory->CreateSensor(sensorPar) ) );
...
void CbmStsDigitize::ProcessAnalogBuffers(Double_t readoutTime) {
  ...
  for (auto& it : fModules) it.second->ProcessAnalogBuffer(readoutTime);
...
void CbmStsDigitize::SetGlobalDefaults() {
  ...
  fUserParAsic = new CbmStsParAsic(nAdc, dynRange, threshold, timeResol,
                                      deadTime, noiseRms, znr);
  ...
  fUserParCond = new CbmStsParSensorCond(vFd, vBias, temperature,
...
void CbmStsDigitize::SetGlobalAsicParams(UInt_t nAdc,
  ...
  assert( nAdc > 0 );
  if (fUserParAsic) delete fUserParAsic;
  fUserParAsic = new CbmStsParAsic(nAdc, dynRange, threshold,
                                      timeResolution, deadTime,
                                      noise, zeroNoiseRate);
...
void CbmStsDigitize::SetProcesses(CbmStsELoss eLossModel,
  ...
  fParSim->SetProcesses(eLossModel, useLorentzShift,
                        useDiffusion, useCrossTalk);
}
// -------------------------------------------------------------------------
