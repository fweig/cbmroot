...
void CbmStsSimModule::AddSignal(UShort_t channel, Double_t time,
...
void CbmStsSimModule::Digitize(UShort_t channel, CbmStsSignal* signal) {
  ...
  if ( fDigitizer ) fDigitizer->CreateDigi(address, channel, dTime, adc,
                                           signal->GetMatch());
...
Int_t CbmStsSimModule::ProcessAnalogBuffer(Double_t readoutTime) {
  ...
  for(auto& chanIt: fAnalogBuffer) {
    ...
    if ( ! (chanIt.second).empty() ) {
      ...
      Double_t timeLimit = readoutTime - 5. * asic.GetTimeResol()
              - asic.GetDeadTime();
