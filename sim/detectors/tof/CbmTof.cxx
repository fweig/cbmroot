...
void CbmTof::Initialize() {
  ...
  if (fbOnePointPerTrack) {
    for (auto itInactiveCounter = fInactiveCounters.cbegin();
         itInactiveCounter != fInactiveCounters.cend();) {
      if (fActiveCounters.find(*itInactiveCounter) != fActiveCounters.end()) {
        ...
        // sensitivity.
        CbmTofDetectorInfo tCounterInfo(ECbmModuleId::kTof,
                                        std::get<0>(*itInactiveCounter),
                                        std::get<1>(*itInactiveCounter),
                                        std::get<2>(*itInactiveCounter),
                                        0,
                                        0);
        fInactiveCounterIDs.emplace(
          fGeoHandler->GetDetIdPointer()->SetDetectorInfo(tCounterInfo));
...
void CbmTof::FinishEvent() {
  ...
  if (fbOnePointPerTrack) {
    ...
    for (Int_t iPoint = 0; iPoint < fTofCollection->GetEntriesFast();
         iPoint++) {
      CbmTofPoint* tCurrentPoint =
        dynamic_cast<CbmTofPoint*>(fTofCollection->At(iPoint));
      ...
      if (1 == tCurrentPoint->GetNLinks()) {
        ...
        tLinkToTrack.SetLink(
          0, fOutputTreeEntry, tLinkToTrack.GetType(), tLinkToTrack.GetIndex());
  ...
  Double_t dGlobalTargetCoordinates[3] = {
    tEventHeader->GetX(), tEventHeader->GetY(), tEventHeader->GetZ()};
  Double_t dGlobalTargetCoordinates1[3] = {
    tEventHeader->GetX(), tEventHeader->GetY(), tEventHeader->GetZ() + 1.};
  ...
  for (auto const& CounterInBeam : fCountersInBeam) {
    ...
    CbmTofDetectorInfo tCounterInfo(
      ECbmModuleId::kTof, iModuleType, iModuleIndex, iCounterIndex, 0, 0);
    iUniqueCounterId =
      fGeoHandler->GetDetIdPointer()->SetDetectorInfo(tCounterInfo);
    ...
    tCurrentNode->GetMatrix()->MasterToLocal(dGlobalTargetCoordinates,
                                             dLocalTargetCoordinates);
    tCurrentNode->GetMatrix()->MasterToLocal(dGlobalTargetCoordinates1,
                                             dLocalTargetCoordinates1);

    // Calculate the intersection point between the beam line and the counter plane in the local c.s.
    dLocalCounterCoordinates[0] =
      dLocalTargetCoordinates[0]
    ...
    dLocalCounterCoordinates[1] =
      dLocalTargetCoordinates[1]
      - dLocalTargetCoordinates[2]
    ...
    if (tCurrentNode->GetShape()->Contains(dLocalCounterCoordinates)) {
      ...
      tCurrentNode->GetMatrix()->LocalToMaster(dLocalCounterCoordinates,
                                               dGlobalCounterCoordinates);
      ...
      Double_t dCounterTargetDistance =
        dGlobalCounterCoordinates[2] - dGlobalTargetCoordinates[2];

      if (0. < dBeamMomentumLab) {
        Double_t dBeamVelocityLab =
          dBeamMomentumLab
          / TMath::Sqrt(TMath::Power(dBeamMomentumLab, 2.)
      ...
      tBeamPoint =
        new ((*fTofCollection)[fTofCollection->GetEntriesFast()]) CbmTofPoint();
      tBeamPoint->SetDetectorID(iUniqueCounterId);
      tBeamPoint->SetEventID(
        fOutputTreeEntry);  // set to the exact same value in 'CbmMCPointBuffer::Fill'
  ...
  if (fbOnePointPerTrack) {
    ...
    if (fbProcessAnyTrack || 0 != gMC->TrackCharge() || 0 == gMC->TrackPid()) {
      ...
      if (!fbIsNewTrack) {
        ...
        for (Int_t iPoint = fTofCollection->GetEntriesFast() - 1; iPoint >= 0;
             iPoint--) {
          tCounterPoint =
            dynamic_cast<CbmTofPoint*>(fTofCollection->At(iPoint));

          if (tCounterPoint->GetDetectorID() == iCounterID
              && tCounterPoint->GetTrackID() == iTrackID) {
      ...
      if (gMC->IsTrackEntering()) {
        ...
        gMC->TrackMomentum(
          dTrackMomentumX, dTrackMomentumY, dTrackMomentumZ, dTrackEnergy);
        ...
        if (bCounterPointExists) {
          ...
          tCounterPoint->SetEnergyLoss(tCounterPoint->GetEnergyLoss()
                                       + dTrackEnergyDeposit);
          ...
          tCounterPoint =
            new ((*fTofCollection)[fTofCollection->GetEntriesFast()])
              CbmTofPoint();
          tCounterPoint->SetTrackID(iTrackID);
          tCounterPoint->SetDetectorID(iCounterID);
          tCounterPoint->SetEventID(
            fOutputTreeEntry);  // set to the exact same value in 'CbmMCPointBuffer::Fill'
    ...
    if (((0 == gMC->GetStack()->GetCurrentTrack()->GetPdgCode())
...
void CbmTof::Register() {
  FairRootManager::Instance()->Register(
    "TofPoint", "Tof", fTofCollection, kTRUE);
}
// -------------------------------------------------------------------------
...
void CbmTof::CreateInBeamNodes() {
  for (auto& CounterInBeam : fCountersInBeam) {
    (CounterInBeam.second).second =
      new TGeoPhysicalNode(((CounterInBeam.second).first).Data());
  }
}
// -------------------------------------------------------------------------
...
CbmTofPoint* CbmTof::AddHit(Int_t trackID,
  ...
  return new (clref[size])
    CbmTofPoint(trackID, detID, pos, mom, time, length, eLoss);
}
// -------------------------------------------------------------------------
