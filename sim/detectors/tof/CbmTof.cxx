  ...
  if( fbOnePointPerTrack )
  {
    for(auto itInactiveCounter = fInactiveCounters.cbegin(); itInactiveCounter != fInactiveCounters.cend(); )
    {
      ...
      else
      {
        ...
        // sensitivity. 
        CbmTofDetectorInfo tCounterInfo(ECbmModuleId::kTof, std::get<0>(*itInactiveCounter), std::get<1>(*itInactiveCounter), std::get<2>(*itInactiveCounter), 0, 0);
...
{   
  if ( Cbm::GeometryUtils::IsNewGeometryFile(fgeoName) ) {
    LOG(info) << "Importing TOF geometry from ROOT file "
              << fgeoName.Data();
    Cbm::GeometryUtils::ImportRootGeometry(fgeoName, this, fCombiTrans);
  } else {
    LOG(info) << "Constructing TOF geometry from ROOT file "
              << fgeoName.Data();
