...
InitStatus CbmTofMergeMcPoints::Init()
{
   ...
   if(NULL != fTofId) 
     LOG(info)<<"CbmTofMergeMcPoints::Init with GeoVersion "<<fGeoHandler->GetGeoVersion();
   else
   {
      switch(iGeoVersion)
      {
...
void CbmTofMergeMcPoints::ReadAndCreateDataBranches()
{
   ...
   if (NULL == ioman)
      LOG(fatal) << "CbmTofMergeMcPoints::ReadAndCreateDataBranches() NULL FairRootManager." ;
   
   // TOF
   fMcTracksColl  = (TClonesArray *) ioman->GetObject("MCTrack");
   if( NULL == fMcTracksColl)
   {
      LOG(fatal)<<"CbmTofMergeMcPoints::ReadAndCreateDataBranches => Could not get the MCTrack TClonesArray!!!";
   } // if( NULL == fMcTracksColl)
   
   fTofPointsColl = (TClonesArray *) ioman->GetObject("TofPoint");
   if( NULL == fTofPointsColl)
   {
      LOG(fatal)<<"CbmTofMergeMcPoints::ReadAndCreateDataBranches => Could not get the TofPoint TClonesArray!!!";
   } // if( NULL == fTofPointsColl)
