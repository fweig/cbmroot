...
Bool_t   CbmTofDigitize::RegisterInputs()
{
   ...
   {
      LOG(error)<<"CbmTofDigitize::RegisterInputs => Could not get the TofPoint TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofPointsColl)
   ...
   {
      LOG(error)<<"CbmTofDigitize::RegisterInputs => Could not get the MCTrack TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fMcTracksColl)
...
Bool_t   CbmTofDigitize::LoadBeamtimeValues()
{
   ...
   if(kFALSE == fDigiBdfPar->LoadBeamtimeHistos()){
     LOG(fatal)<<"CbmTofDigitize::LoadBeamtimeValues: Cluster properties from testbeam could not be loaded! ";
     return kFALSE;
   }
         ...
         switch( fDigiBdfPar->GetClusterRadiusModel() )
         {
            case 0:
            {
               ...
               LOG(debug)<<"CbmTofDigitize::LoadBeamtimeValues => SM type "<<iSmType
                          <<" Mean Cluster Size "<<(fh1ClusterSizeProb[iSmType]->GetMean());
               break;
            }
            case 1:
            {
         ...
         for( Int_t iSm = 0; iSm < iNbSm; iSm++ )
		        ...
		        if(fbMonitorHistos)
			{
                        ...
                        if (0 == fStorDigiMatch[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide].size()){
                           LOG(error)<<Form(" cannot add digiMatch for (%d,%d,%d,%d,%d) at pos  %d",
                              iSmType,iSm,iRpc,iCh,iSide,fiNbDigis);
                           break;
                        }
			...
			if( iNbDigis > 1) {
			  LOG(debug)<<Form("Now apply deadtime %f for %d digis, digi0=%d for adress 0x%08x ",
					   fDigiBdfPar->GetDeadtime(), iNbDigis,iDigi0,fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi0].first->GetAddress())
				    ;
			  ...
			  for( Int_t iDigi = 1; iDigi < iNbDigis; iDigi++) {
			    ...
			    if( ( fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi].first->GetTime()
				 -fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi0].first->GetTime() )< fdDeadtime )
			    {    // Store Link with weight 0 to have a trace of MC tracks firing the channel
                                 // but whose Digi is not propagated to next stage
			      digiMatch=fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi0].second;
			      Int_t nL=fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi].second->GetNofLinks();
			      if (nL != 1) LOG(fatal)<<"Invalid number of Links " << nL ; 
			      
			      CbmLink L0 = fStorDigi[iSmType][iSm*iNbRpc + iRpc][iNbSides*iCh+iSide][iDigi].second->GetLink(0);
         ...
         if( 0 == iChType )
         {
            ...
            else
            {
               ...
               LOG(error)<<"CbmTofDigitize::GenerateClusterRadius => Test "
                     ...
                     <<" Type "<<iChType<<" Type "<<(Int_t)iChType;
               return kFALSE;
...
//         continue;
	 ...
	 LOG(debug1)<<Form(" TofDigitizerBDF:: (%3d,%3d,%3d,%3d) - node at (%6.1f,%6.1f,%6.1f) : 0x%p Pos(%6.1f,%6.1f,%6.1f)",
...
//                                          );
         ...
         for( Int_t iStripInd = iChannel - iNbStripsSideA;
	       ...
	       if(fGeoHandler->GetGeoVersion() < k14a)
		     ...
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, valB %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
...
//                                          );
         ...
         for( Int_t iStripInd = iChannel - iNbStripsSideA;
	       ...
	       if(fGeoHandler->GetGeoVersion() < k14a)
		     ...
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, valC %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
              } // if Side A above threshold
               if( fDigiBdfPar->GetFeeThreshold() <
		     ...
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, valE %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
               } // if Side B above threshold
            } // if( 0 <= iStripInd && iStripInd < iNbCh )
   ...
   //         continue;
	 ...
	 LOG(error)<<Form("CbmTofDigitize => det ID 0x%08x",iDetId)
      ...
      LOG(debug2)<<"CbmTofDigitize::DigitizeFlatDisc => Check Point "<<iPntInd
            ...
            <<" Type "<<iChType<<" Orient. "<<fDigiBdfPar->GetChanOrient( iSmType, iRpc );
...
//         continue;
	...
	continue;
      ...
      TGeoNode *fNode=        // prepare global->local trafo
         gGeoManager->FindNode(fChannelInfo->GetX(),fChannelInfo->GetY(),fChannelInfo->GetZ());
	 LOG(debug1)<<Form(" TofDigitizerBDF:: (%3d,%3d,%3d,%3d) - node at (%6.1f,%6.1f,%6.1f) : %p Pos(%6.1f,%6.1f,%6.1f)",
			  iSmType,iSM,iRpc,iChannel,
			  fChannelInfo->GetX(),fChannelInfo->GetY(),fChannelInfo->GetZ(),fNode,
			  vPntPos.X(), vPntPos.Y(), vPntPos.Z())
		   ;
      ...
      Double_t poipos_local[3];
      gGeoManager->MasterToLocal(poipos, poipos_local);
      LOG(debug1)<<Form(" TofDigitizerBDF:: DetId 0x%08x, ChanId 0x%08x, local pos  (%5.1f,%5.1f,%5.1f) refpos  (%5.1f,%5.1f,%5.1f) ",
...
//         continue;
      ...
      if( 0 == iChType)
      {
		 ...
		 //                        - ( poipos_local[1] - fChannelInfo->GetY() )
	       ...
	       //			   tofDigi->GetMatch()->AddLink(1., iPntInd);
               ...
               fStorDigi[iSmType][iSM*iNbRpc + iRpc][2*iChannel].push_back( data );
	       fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel].push_back( iPntInd );
	       LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, MCp %d, MCt %d, TB %6.2f, TotB %6.1f",
         ...
         else
         {
		  ...
		  fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iChannel].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): MCpi %zu, valE %d, MCti %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
      ...
      if( 0 == iChType)
      {
	      ...
	      continue;
            ...
            if( dClustCharge  + 0.0000001 < dChargeSideCh )
            {
               LOG(error)<<"CbmTofDigitize::DigitizeFlatDisc => Side Charge "
                         <<dChargeSideCh<<" "<<dClustCharge<<" "<<dClustArea;
               LOG(error)<<"CbmTofDigitize::DigitizeFlatDisc => Check Point "<<iPntInd
	    ...
	    LOG(debug1)<<Form(" TofDigitizerBDF:: chrg %7.1f, times %5.1f,%5.1f, pos  %5.1f,%5.1f,%5.1f ",dChargeCentral,
	    ...
	    LOG(debug1)<<Form(" TofDigitizerBDF:: chrg %7.1f, gain %7.1f, thr %7.1f times %5.1f,%5.1f ",dChargeCentral,
		  ...
		  LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, MCpA %d, MCtt %d",
		  ...
		  LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, MCpB %d, MCtB %d",
				     iSmType,iSM,iRpc,iChanInd,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChanInd+1].size(),iPntInd,iTrackID)
			    ;
         ...
         else
         {
		  ...
		  fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iChanInd].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, MCpP %d, MCtP %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
            } // for( Int_t iChanInd = iMinChanInd + 1; iChanInd < iMaxChanInd; iChanInd++ )
            ...
            if( kTRUE == bCheckOtherRow )
		     ...
		     //					 tofDigi->GetMatch()->AddLink(1., iPntInd);
                     ...
                     fStorDigi[iSmType][iSM*iNbRpc + iRpc][iChanInd].push_back( data );
		     fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iChanInd].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, MCpX %d, MCtX %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
...
//         continue;
	 ...
	 LOG(debug1)<<Form(" TofDigitizerBDF:: (%3d,%3d,%3d,%3d) - node at (%6.1f,%6.1f,%6.1f) : 0x%p Pos(%6.1f,%6.1f,%6.1f)",
      ...
      if( 0 == iChType)
      {
	    ...
	    //			tofDigi->GetMatch()->AddLink(1., iPntInd);
            ...
            fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val1 %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel+1].size(),iPntInd,iTrackID)
			      ;
	    ...
	    //			tofDigi->GetMatch()->AddLink(1., iPntInd);
            ...
            fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iChannel].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val2 %d, MCt %d",
         ...
         else
         {
	       ...
	       //			   tofDigi->GetMatch()->AddLink(1., iPntInd);
               ...
               fStorDigi[iSmType][iSM*iNbRpc + iRpc][iChannel].push_back( data );
	       fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iChannel].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val3 %d, MCt %d",
      ...
      if( 0 == iChType)
      {
         ...
         if( 0 < iChannel)
         {
		  ...
		  fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val4 %d, MCt %d",
         ...
         if( iChannel < iNbCh - 1)
         {
		  ...
		  //				   tofDigi->GetMatch()->AddLink(1., iPntInd);
                  ...
                  fStorDigi[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd+1].push_back( data1 );
		  fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd+1].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val5 %d, MCt %d",
		  ...
		  //				   tofDigi->GetMatch()->AddLink(1., iPntInd);
                  ...
                  fStorDigi[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd].push_back( data2 );
		  fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val6 %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd].size(),iPntInd,iTrackID)
			      ;
 ...
 //           Bool_t   bCheckOtherRow = kFALSE; // -> Comment to remove warning because set but never used
            ...
            if( iRow*iNbCh/2.0 < iChannel )
            {
		     ...
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val7 %d, MCt %d",
				     iSmType,iSM,iRpc,iChannel,fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][2*iSideChInd].size(),iPntInd,iTrackID)
			      ;
            ...
            if( iChannel < (1+iRow)*iNbCh/2.0 - 1)
            {
		     ...
		     //					  tofDigi->GetMatch()->AddLink(1., iPntInd);
                     ...
                     fStorDigi[iSmType][iSM*iNbRpc + iRpc][iSideChInd].push_back( data );
		     fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iSideChInd].push_back( iPntInd );
		     LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val8 %d, MCt %d",
...
//               bCheckOtherRow = kTRUE; // -> Comment to remove warning because set but never used
			...
			//						 tofDigi->GetMatch()->AddLink(1., iPntInd);
                        ...
                        fStorDigi[iSmType][iSM*iNbRpc + iRpc][iChanInd].push_back( data );
			fStorDigiMatch[iSmType][iSM*iNbRpc + iRpc][iChanInd].push_back( iPntInd );
			LOG(debug1)<<Form("Digimatch (%d,%d,%d,%d): size %zu, val9 %d, MCt %d",
