...
void CbmRich::Initialize()
{
    ...
    LOG(info) << "CbmRich::Initialize() Transport engine:" << std::string(fRun->GetName());
    LOG(info) << "CbmRich::Initialize() fIsGeant4:" << (fIsGeant4? "true" : "false");
}
                            ...
                            FairVolume* vol)
{
    ...
    if (volName.Contains("pmt_pixel")){
        if (gMC->IsTrackEntering()){
            ...
            if ( pdgCode == 50000050 && isPhotonReflected(tMom.P())) { // Cherenkovs only
...
Bool_t CbmRich::isPhotonReflected(Double_t photonEnergy)
{
    ...
    static std::vector<Double_t> enrg = {
        6.1997,5.9045,5.6361,5.391,5.1664,4.9598,4.769,4.5924,4.4284,4.2757,4.1331,3.9998,3.8748,3.7574,
        3.6469,3.5427,3.4443,3.3512,3.263,3.1793,3.0998,3.0242,2.9522,2.8836,2.818,2.7554,2.6955,2.6382,
        2.5832,2.5305,2.4799,2.4313,2.3845,2.3395,2.2962,2.2544,2.2142,2.1753,2.1378,2.1016,2.0666,2.0327,
        1.9999,1.9682,1.9374,1.9076,1.8787,1.8507,1.8234,1.797,1.7713,1.7464,1.7221,1.6985,1.6756,1.6533,
        1.6315,1.6103,1.5897,1.5695,1.5499 };


    static std::vector<Double_t> eff = {
        0.22529,0.1862,0.15901, 0.14713, 0.13963, 0.13898, 0.13762, 0.13622, 0.13868, 0.13951, 0.14031, 
        0.14073, 0.1409,  0.13977, 0.14205, 0.14072, 0.1396,  0.13826, 0.13665, 0.13513, 0.13463, 0.13287, 
        0.13182, 0.13084, 0.12824, 0.12601, 0.12622, 0.12681, 0.12193, 20.12011, 0.12109, 0.11908, 0.11526, 
        0.11364, 0.11385, 0.12015, 0.11935, 0.11712, 0.1208,  0.12021, 0.11909, 0.11783, 0.12257, 0.1215,  
        0.12199, 0.12494, 0.13101, 0.12089, 0.12284, 0.12569, 0.13136, 0.13307, 0.13705, 0.13844, 0.13753,0.14416, 
   ...
   // Double_t eff = interpolator->Eval(1.e9 * photonEnergy);
    ...
    if (phEnrgEv >= enrg[0]) {
        ...
        effCalc = eff[i0] + (phEnrgEv - enrg[i0]) * (eff[i1] - eff[i0]) / (enrg[i1] - enrg[i0]);
...
void CbmRich::ConstructGeometry()
{
    ...
    if ( fileName.EndsWith(".root") ) {
      if ( Cbm::GeometryUtils::IsNewGeometryFile(fgeoName) ) {
        LOG(info) << "Importing RICH geometry from ROOT file "
                  << fgeoName.Data();
        ...
        LOG(info) << "Constructing RICH geometry from ROOT file "
                  << fgeoName.Data();
        ...
        LOG(fatal) << "Geometry format of RICH file " << fileName.Data() << " not supported. Only ROOT and GDML formats are supported.";
    }
}
