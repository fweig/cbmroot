...
Bool_t CbmRich::CheckIfSensitive(std::string name) {
  ...
  if (volName.Contains("pmt_pixel") || volName.Contains("sens_plane"))
    return kTRUE;
  // mirrors
...
void CbmRich::Register() {
  FairRootManager::Instance()->Register(
    "RichPoint", "Rich", fRichPoints, kTRUE);
  FairRootManager::Instance()->Register(
    "RefPlanePoint", "RichRefPlane", fRichRefPlanePoints, kTRUE);
  FairRootManager::Instance()->Register(
    "RichMirrorPoint", "RichMirror", fRichMirrorPoints, kFALSE);
}
...
void CbmRich::ConstructGeometry() {
  ...
  if (fileName.EndsWith(".root")) {
    ...
    LOG(info) << "Constructing RICH geometry from GDML  file "
              << fileName.Data();
...
void CbmRich::ConstructGdmlGeometry(TGeoMatrix* geoMatrix) {
  ...
  ExpandNodeForGdml(gGeoManager->GetTopVolume()->GetNode(
    gGeoManager->GetTopVolume()->GetNdaughters() - 1));

  for (Int_t k = maxInd + 1; k < newMaxInd + 1; k++) {
    TGeoMedium* medToDel =
      (TGeoMedium*) (gGeoManager->GetListOfMedia()->At(maxInd + 1));
    LOG(debug) << "    removing media " << medToDel->GetName() << " with id "
               << medToDel->GetId() << " (k=" << k << ")";
...
CbmRichPoint* CbmRich::AddHit(Int_t trackID,
  ...
  return new (clref[size])
    CbmRichPoint(trackID, detID, pos, mom, time, length, eLoss);
}
