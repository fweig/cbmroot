...
void CbmMuchDigitizeGem::Exec(Option_t*) {
  ...
  if (fGenerateElectronicsNoise) {
    ...
    LOG(info) << "+ " << setw(20) << GetName() << ": Generated  " << nNoise
	      ...
	      << fCurrentEventTime << " ns and " 
              << fNofNoiseTot << " total noise generated till now.";
    LOG(debug3) << "+ " << setw(20) << GetName() << ": Generated  " << fNofNoiseSignals
		<< " noise signals for this time slice from t = " << fPreviousEventTime << " ns to "  << fCurrentEventTime << "ns";
...
Int_t CbmMuchDigitizeGem::GenerateNoise(Double_t t1, Double_t t2){
  LOG(debug) << "+ " << setw(20) << GetName()<< ": Previous event time " << t1 << " Current event time " << t2 << " ns.";
  if( t1 > t2 ) {  LOG(debug) << "+ " << ": Previous event time " << t1 << " is greater than Current event time " << t2 << ". No electronics noise signal generated for "<< fCurrentEvent << " event." ;
  return 0;
  };
...
void CbmMuchDigitizeGem::AddNoiseSignal(CbmMuchPad* pad, Double_t time,Double_t charge) {
  ...
  fNofNoiseSignals++;
  LOG(debug3)<< "+ " << setw(20) << GetName() <<": Registered a Noise CbmMuchSignal into the CbmMuchReadoutBuffer. Number of Noise Signal generated "<< fNofNoiseSignals;
}
//====================End of Noise part=================//
...
void CbmMuchDigitizeGem::ReadAndRegister(Long_t eventTime){
  ...
  LOG(debug) << GetName() << ": Number of Signals read out from Buffer "<< ReadOutSignal << " and SignalList contain " << SignalList.size() << " entries.";
  ...
  for (std::vector<CbmMuchSignal*>::iterator LoopOver= SignalList.begin(); LoopOver != SignalList.end(); LoopOver++)
	...
	LOG(debug2)<< GetName() << ": New digi: sector = "<< CbmMuchAddress::GetSectorIndex(digi->GetAddress()) <<" channel= " << CbmMuchAddress::GetChannelIndex(digi->GetAddress());
...
//cout<<"fQThreshold "<<fQThreshold<<" fqmax "<<fQMax<<endl;
  ...
  if( signal->GetCharge() < 0 ) return (NULL); //Before type casting signed int into unsigned int need to check for -ve value otherwise after casing -ve int value will become a large +ve value.
  else if((unsigned int)signal->GetCharge() < fQThreshold) return (NULL);
...
*/
  ...
  if (module->GetDetectorType()==3 ||module->GetDetectorType()==4 ) {
    ...
    if(!sFirst){
      LOG(debug) << GetName() << ": Not Processing MCPoint " << iPoint <<" because it is on the module " << module3 <<"  but not the first sector. "<< sFirst;
      return 1;
    }
    CbmMuchSectorRadial* sLast  = (CbmMuchSectorRadial*) module3->GetSectorByIndex(module3->GetNSectors()-1); //Last sector

    if(!sLast){
      LOG(debug) << GetName() << ": Not Processing MCPoint " << iPoint <<" because it is not the last sector of module."<< module3;
      return 1;
    }
	...
	if(!Status)LOG(debug) << GetName() << ": Processing MCPoint " << iPoint <<" in which Primary Electron : "<<i<< " not contributed charge. ";
        continue;
      }  
	...
	if (!Status) LOG(debug3) << GetName() << ": Processing MCPoint " << iPoint
				 <<" in which Primary Electron : "<<i<< " not contributed charge. ";
      }
	...
	if(!Status)LOG(debug3) << GetName() << ": Processing MCPoint " << iPoint <<" in which Primary Electron : "<<i<< " not contributed charge. ";
...
Double_t CbmMuchDigitizeGem::GetNPrimaryElectronsPerCm(const CbmMuchPoint* point, int detectortype){
  ...
  */
  ...
  Double_t n;
  Double_t mpvRpc = 50.0; // For RPC MPV and sigma is taken to produce final landau in accordance with experimental value
  ...
  if(detectortype == 4) /// RPC
  {
  	...
  	while (n > 5e4) n = gRandom->Landau(mpvRpc,sigmaRpc); // restrict Landau tail to increase performance
   	return n; 
...
Bool_t CbmMuchDigitizeGem::BufferSignals(Int_t iPoint,Double_t time, Double_t driftTime){
  ...
  if(!fAddressCharge.size()) { LOG(debug2) << "Buffering MC Point " << iPoint
					   << " but fAddressCharge size is " << fAddressCharge.size() << "so nothing to Buffer for this MCPoint.";
    return kFALSE;
  }
  ...
  LOG(debug2) << GetName() << ": For MC Point " << iPoint
	      << " buffered " << fAddressCharge.size() << " CbmMuchSignal into the CbmReadoutBuffer.";
  return kTRUE;
