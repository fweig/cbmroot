...
ClassImp(CbmBlastWave)
  ...
  CbmBlastWave::CbmBlastWave(Int_t recoLevel,
                             Int_t /*iVerbose*/,
                             TString Mode,
                             Int_t PDG,
                             TString pname,
  ...
  , fYmaxv()
  , paramGlobal(0)
  , paramGlobalInterval(0)
  , param2GlobalInterval(0)
  , paramLocal(0)
  , paramLocalInterval(0)
  , totalLocal(0)
  , totalGlobal(0)
...

{
  ...
  histodndy =
    new TH1F("dN/dy", "Rapidity distribution", 40, -2. * ycm, 2. * ycm);
  histodndy->SetXTitle("y");
  histodndy->SetYTitle("dN/dy");

  histodndymodel = new TH1F(
    "dN/dy model", "Rapidity distribution from model", 40, -2. * ycm, 2. * ycm);
  histodndymodel->SetXTitle("y");
  histodndymodel->SetYTitle("dN/dy");

  histoeta = new TH1F("etamax", "Event-by-event etamax", 50, 0., ycm);
  ...
  histomult =
    new TH1F("Multiplicity", "Event-by-event multiplicity", 1000, 0.5, 1000.5);
  ...
  if (fRecoLevel == -1 || fRecoLevel > 10)
    model = new BlastWave(fMass, fPDGID, false, -2. * ycm, 2. * ycm, ycm, 0.);
  else
    model = new BlastWave(fMass, fPDGID, true, -2. * ycm, 2. * ycm, ycm, 0.);
  modelmc = new BlastWave(fMass, fPDGID, true, -2. * ycm, 2. * ycm, ycm, 0.);

  if (fRecoLevel == -1 || fRecoLevel > 10)
    modelL = new BlastWaveLongitudinal(
      fMass, fPDGID, false, -2. * ycm, 2. * ycm, ycm, Tlong);
  else
    modelL = new BlastWaveLongitudinal(
      fMass, fPDGID, true, -2. * ycm, 2. * ycm, ycm, Tlong);
  modelmcL = new BlastWaveLongitudinal(
    fMass, fPDGID, false, -2. * ycm, 2. * ycm, ycm, Tlong);
...
void CbmBlastWave::AddRapidityInterval(double ymin, double ymax) {
  ...
  if (fRecoLevel == -1 || fRecoLevel > 10)
    modelsY.push_back(new BlastWave(fMass, fPDGID, false, ymin, ymax, ycm, 0.));
  else
    modelsY.push_back(new BlastWave(fMass, fPDGID, true, ymin, ymax, ycm, 0.));
}
...
void CbmBlastWave::Exec() {
  ...
  if (fRecoLevel != -1 && fTopoReconstructor != NULL
      && fTopoReconstructor->GetTracks()[2].PDG().size() == 0)
    return;
  ...
  if (events % fEventStats == 0) {
    ...
    //std::cout << "T = " << T << "\n";
    histomult->SetBinContent(totalEvents,
                             totalLocal / static_cast<double>(fEventStats));
    histomult->SetBinError(
      totalEvents, 0. * sqrt(totalLocal) / static_cast<double>(fEventStats));
    double s1 = y2Local;
    double s2 = y4Local;
    int Numb  = totalLocal;
    double erretamax =
      modelL->detady2(y2Local / totalLocal)
      * TMath::Sqrt((s2 - s1 * s1 / Numb) / (Numb - 1.) / Numb);
    histomultmodel->SetBinContent(
      totalEvents,
      modelL->GetA(totalLocal / static_cast<double>(fEventStats), etamax));
    histomultmodel->SetBinError(
      totalEvents,
      modelL->GetAerror(totalLocal / static_cast<double>(fEventStats),
                        etamax,
                        0. * TMath::Sqrt(totalLocal)
                          / static_cast<double>(fEventStats),
                        erretamax));
...
void CbmBlastWave::Finish() {
  ...
  std::cout << "eta = " << params.eta.value << "  "
            << "Error = " << params.eta.error << "\n";

  double etamax    = modelL->Geteta(y2Global / totalGlobal);
  double s1        = y2Global;
  double s2        = y4Global;
  int Numb         = totalGlobal;
  double erretamax = modelL->detady2(y2Global / totalGlobal)
                     * TMath::Sqrt((s2 - s1 * s1 / Numb) / (Numb - 1.) / Numb);
  //std::cout << "<y2> = " << y2Global / totalGlobal << " <y2>2 = " << modelL->y2Av(etamax) << " <y2>3 = " << modelmcL->y2Av2(etamax) << "\n";
  std::cout << "etamax = " << etamax << "  "
            << "Error = " << erretamax << "\n";

  //double y2av = 0., y2norm = 0.;
  for (int n = 1; n < histodndymodel2->GetNbinsX(); n++) {
    histodndymodel2->SetBinContent(
      n,
      modelL->GetA(totalGlobal / static_cast<double>(totalEvents), etamax)
        * modelmcL->fy(histodndymodel2->GetXaxis()->GetBinCenter(n), etamax));
  }

  std::cout << "<" << name << "> = "
            << modelL->GetA(totalGlobal / static_cast<double>(totalEvents),
                            etamax)
            << " error = "
            << modelL->GetAerror(totalGlobal / static_cast<double>(totalEvents),
                                 etamax,
                                 TMath::Sqrt(totalGlobal)
