...
void CbmL1TrackMerger::Init() {
  ...
  fArrayTrdTrackM =
    L1_DYNAMIC_CAST<TClonesArray*>(rootMgr->GetObject("TrdTrackMatch"));
...
Int_t CbmL1TrackMerger::MergeSimple(TClonesArray* stsTracks,
  ...
  for (Int_t iTrdTrack = 0; iTrdTrack < trdTracks->GetEntriesFast();
       iTrdTrack++) {
...
Int_t CbmL1TrackMerger::MergeImPlane(TClonesArray* stsTracks,
  ...
  // Loop over STS tracks
  for (Int_t iStsTrack = 0; iStsTrack < stsTracks->GetEntriesFast();
       iStsTrack++) {
    ...
    for (Int_t iTrdTrack = 0; iTrdTrack < trdTracks->GetEntriesFast();
      ...
      if (NULL == trdTrack) continue;
      trdTrackM =
        L1_DYNAMIC_CAST<CbmTrackMatch*>(fArrayTrdTrackM->At(iTrdTrack));
      ...
      // Get chi2
      chi2XY = GetChi2XY(
        kfTrack, const_cast<FairTrackParam*>(trdTrack->GetParamFirst()));
      // Choose the closest
      if (chi2XY < minChi2XY) {
        minChi2XY      = chi2XY;
...
Double_t CbmL1TrackMerger::GetChi2XY(CbmKFTrack& kfTrack,
  ...
  Double_t chi2 = 0.5 * (dx * dx * c0 - 2 * dx * dy * c1 + dy * dy * c2)
                  / (c0 * c2 - c1 * c1);
