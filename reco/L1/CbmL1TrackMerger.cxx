...
Int_t CbmL1TrackMerger::DoMerge(TClonesArray* stsTracks, TClonesArray* trdTracks, TClonesArray* glbTracks)
{
  ...
  if (1 == fMethod) { return MergeSimple(stsTracks, trdTracks, glbTracks); }
  else if (2 == fMethod) {
...
Int_t CbmL1TrackMerger::MergeImPlane(TClonesArray* stsTracks, TClonesArray* trdTracks, TClonesArray* glbTracks)
{
  ...
  for (Int_t iStsTrack = 0; iStsTrack < stsTracks->GetEntriesFast(); iStsTrack++) {
    ...
    for (Int_t iTrdTrack = 0; iTrdTrack < trdTracks->GetEntriesFast(); iTrdTrack++) {
      ...
      if (stsTrackM->GetMCTrackId() == trdTrackM->GetMCTrackId()) { fh_dx_true->Fill(dx, theta); }
      else {
      ...
      // Fill histogramms
      if (stsTrackM->GetMCTrackId() == trdTrackM->GetMCTrackId()) { fh_dy_true->Fill(dy, theta); }
      else {
      ...
      // Fill histogramms
      if (stsTrackM->GetMCTrackId() == trdTrackM->GetMCTrackId()) { fh_dtx_true->Fill(dtx, theta); }
      else {
        fh_dtx_false->Fill(dtx, theta);
      }

      // Cut on dtx
      if (theta < 10) { dtx_cut = 0.001; }
      else if (theta < 15) {
        dtx_cut = 0.002;
      }
      else if (theta < 20) {
      ...
      // Fill histogramms
      if (stsTrackM->GetMCTrackId() == trdTrackM->GetMCTrackId()) { fh_dty_true->Fill(dty, theta); }
      else {
