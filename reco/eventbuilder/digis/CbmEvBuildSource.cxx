...
//  Int_t n=0;
  ...
  if (par!=fEv)
  {
    LOG(fatal) << "CbmEvBuildSource::ReadEvent(" << par << "): Can't jump to arbitrary event but 0. Current event is " << fEv;
    return 3;
  }
...
Int_t CbmEvBuildSource::GetNextTimeSlice()
{
  ...
  cout << fEvHeader << endl;
  LOG(info) << "CbmBuildEventsSimple:	Sts digis in slice " << fSlice->GetDataSize(kSts) << ". Slice start: " << fSlice->GetStartTime();
  fNDigis=0;
