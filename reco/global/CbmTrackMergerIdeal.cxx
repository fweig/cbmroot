...
// -----   Default constructor   -------------------------------------------
CbmTrackMergerIdeal::CbmTrackMergerIdeal() : CbmTrackMerger(), fStsMatch(NULL), fTrdMatch(NULL) { fVerbose = 1; }
// -------------------------------------------------------------------------
...
Int_t CbmTrackMergerIdeal::DoMerge(TClonesArray* stsTracks, TClonesArray* trdTracks, TClonesArray* glbTracks)
{
  ...
  if (stsTracks && fStsMatch) {
    ...
    if (nSts != fStsMatch->GetEntriesFast()) {
      cout << "-E- CbmTrackMergerIdeal::DoMerge: "
           << "Unequal array sizes of StsTrack and StsTrackMatch! (" << nSts << ", " << fStsMatch->GetEntriesFast()
           << endl;
      nSts = 0;
    }
  }
  ...
  if (trdTracks && fTrdMatch) {
    ...
    if (nTrd != fTrdMatch->GetEntriesFast()) {
      cout << "-E- CbmTrackMergerIdeal::DoMerge: "
           << "Unequal array sizes of TrdTrack and TrdTrackMatch! (" << nTrd << ", " << fTrdMatch->GetEntriesFast()
           << endl;
      nTrd = 0;
    }
  }
  ...
  if (fVerbose > 1)
    cout << endl << "-I- CbmTrackMergerIdeal: " << nSts << " StsTracks, " << nTrd << " TrdTracks" << endl;
  ...
  for (Int_t iTrd = 0; iTrd < nTrd; iTrd++) {
    ...
    if (mcTrdMap.find(iMC) != mcTrdMap.end()) {
      cout << "-W- CbmTrackMergerIdeal::DoMerge: "
           << "MCTrack " << iMC << " matched to TrdTrack " << mcTrdMap[iMC] << " and " << iTrd << endl;
      continue;
    }
    mcTrdMap[iMC] = iTrd;
  }
  ...
  for (Int_t iSts = 0; iSts < nSts; iSts++) {
    ...
    CbmGlobalTrack* gTrack = new ((*glbTracks)[nGlb++]) CbmGlobalTrack();
  ...
  if (fVerbose > 1) {
    cout << "-I- CbmTrackMergerIdeal: " << nGlb << " GlobalTracks created from StsTracks" << endl;
    cout << "-I- CbmTrackMergerIdeal: " << nMerged << " StsTracks merged with TrdTracks" << endl;
  }
  ...
  if (fVerbose > 1) {
    cout << "-I- CbmTrackMergerIdeal: " << nTrdOnly << " GlobalTracks created from TrdTracks" << endl;
    cout << "-I- CbmTrackMergerIdeal: Total " << nGlb << " GlobalTracks created" << endl;
  }
