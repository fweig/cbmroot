...
CbmTrackMergerIdeal::CbmTrackMergerIdeal()
  : CbmTrackMerger(), fStsMatch(NULL), fTrdMatch(NULL) {
  fVerbose = 1;
}
// -------------------------------------------------------------------------
...
Int_t CbmTrackMergerIdeal::DoMerge(TClonesArray* stsTracks,
  ...
  if (fVerbose > 1)
    cout << endl
         << "-I- CbmTrackMergerIdeal: " << nSts << " StsTracks, " << nTrd
         << " TrdTracks" << endl;
  ...
  for (Int_t iTrd = 0; iTrd < nTrd; iTrd++) {
    ...
    if (mcTrdMap.find(iMC) != mcTrdMap.end()) {
      cout << "-W- CbmTrackMergerIdeal::DoMerge: "
           << "MCTrack " << iMC << " matched to TrdTrack " << mcTrdMap[iMC]
           << " and " << iTrd << endl;
      continue;
    }
    mcTrdMap[iMC] = iTrd;
  ...
  if (fVerbose > 1) {
    ...
    cout << "-I- CbmTrackMergerIdeal: Total " << nGlb << " GlobalTracks created"
         << endl;
  }
