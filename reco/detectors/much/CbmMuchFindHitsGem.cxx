...
InitStatus CbmMuchFindHitsGem::Init() {
  ...
  if (! fEvents) 
    fEvents = dynamic_cast<TClonesArray*>(FairRootManager::Instance()->GetObject("CbmEvent")); 
...
void CbmMuchFindHitsGem::ExecClusteringPeaks(CbmMuchCluster* cluster,Int_t iCluster, CbmEvent* event) {
  ...
  if (nDigis<=2)  { CreateHits(cluster,iCluster, event); return; }
...
void CbmMuchFindHitsGem::CreateHits(CbmMuchCluster* cluster, Int_t iCluster, CbmEvent* event) {
  ...
  for (Int_t i=0;i<nDigis;i++) {
    ...
    Double_t rpcHitTimeError = 2.3;  	// Hit time error for RPC = 2.3 ns, as residual dist width is 2.3. This is made to make pull dist width ~1 
	...
	else t = digi->GetTime(); // Not correcting Drift Time for mCBM data
	dt  = gemHitTimeError;
      }
