...
UInt_t CbmRecoSts::CreateModules() {
  ...
  for (Int_t iModule = 0; iModule < fSetup->GetNofModules(); iModule++) {
    ...
    // --- Module parameters
    const CbmStsParModule& modPar = ( fUserParModule ?
        *fUserParModule : fParSetModule->GetParModule(moduleAddress) );

    // --- Sensor parameters
    const CbmStsParSensor& sensPar = ( fUserParSensor ?
        *fUserParSensor : fParSetSensor->GetParSensor(sensorAddress) );

    // --- Sensor conditions
    const CbmStsParSensorCond& sensCond = ( fUserParSensorCond ?
        *fUserParSensorCond : fParSetCond->GetParSensor(sensorAddress));
    ...
    // --- Create reco module
    CbmStsRecoModule* recoModule = new CbmStsRecoModule(setupModule,
                                                        modPar, sensPar,
                                                        lorentzF, lorentzB);
...
void CbmRecoSts::Finish() {
  ...
  LOG(info) << "Time Reset       : " << fixed << setprecision(1)
          << setw(6) << 1000. * fTime1 << " ms (" << setprecision(1)
          << setw(4) << 100. * fTime1 / fTimeTot << " %)";
  LOG(info) << "Time Distribute  : " << fixed << setprecision(1)
          << setw(6) << 1000. * fTime2 << " ms (" << setprecision(1)
          << 100. * fTime2 / fTimeTot << " %)";
  LOG(info) << "Time Reconstruct : " << fixed << setprecision(1)
          << setw(6) << 1000. * fTime3 << " ms (" << setprecision(1)
          << setw(4) << 100. * fTime3 / fTimeTot << " %)";
  LOG(info) << "Time Output      : " << fixed << setprecision(1)
          << setw(6) << 1000. * fTime4 << " ms (" << setprecision(1)
          << setw(4) << 100. * fTime4 / fTimeTot << " %)";
  LOG(info) << "=====================================";
...
InitStatus CbmRecoSts::Init()
{
  ...
  if ( ! fDigiManager->IsPresent(ECbmModuleId::kSts) ) LOG(fatal) << GetName()
...
void CbmRecoSts::ProcessData(CbmEvent* event) {
  ...
  if ( event ) {
    LOG(info) << setw(20) << left << GetName() << "[" << fixed
        << setprecision(4) << realTime << " s] : Event "
        << right << setw(6) << event->GetNumber() << ", digis: " << nDigis
        << ", ignored: " << nDigisIgnored << ", clusters: " << nClusters
        << ", hits " << nHits;
  } //? event mode
  else {
    LOG(info) << setw(20) << left << GetName() << "[" << fixed
        << setprecision(4) << realTime << " s] : TSlice "
        << right << setw(6) << fNofTimeslices << ", digis: " << nDigis
        << ", ignored: " << nDigisIgnored << ", clusters: " << nClusters
        << ", hits " << nHits;
  }
...
void CbmRecoSts::SetParContainers()
{
  ...
  fParSim = dynamic_cast<CbmStsParSim*>
  (db->getContainer("CbmStsParSim"));
  fParSetModule = dynamic_cast<CbmStsParSetModule*>
  (db->getContainer("CbmStsParSetModule"));
  fParSetSensor = dynamic_cast<CbmStsParSetSensor*>
  (db->getContainer("CbmStsParSetSensor"));
