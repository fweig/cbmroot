...
void CbmStsRecoModule::AddDigiToQueue(const CbmStsDigi* digi,
                                      Int_t digiIndex) {
  fLock.lock();
  Int_t moduleAddress = CbmStsAddress::GetMotherAddress(digi->GetAddress(),
                                                        kStsModule);
  assert( moduleAddress == fSetupModule->GetAddress() );
  assert ( digi->GetChannel() < 2 * fNofStrips );
  if ( digi->GetChannel() < fNofStrips ) fDigisF.push_back({digi, digiIndex});
  else fDigisB.push_back({digi, digiIndex});
  fLock.unlock();
}
// -------------------------------------------------------------------------
...
void CbmStsRecoModule::Reconstruct() {
  ...
  fClusterFinder->Exec(fDigisF, fClustersF, fSetupModule->GetAddress(),
                       fNofStrips, 0, fTimeCutDigiSig,
                       fTimeCutDigiAbs, fConnectEdgeFront, fParModule);
  fClusterFinder->Exec(fDigisB, fClustersB, fSetupModule->GetAddress(),
                       fNofStrips, fNofStrips, fTimeCutDigiSig,
                       fTimeCutDigiAbs, fConnectEdgeBack, fParModule);

  // --- Perform cluster analysis
  for ( auto& cluster : fClustersF ) fClusterAna->Exec(cluster, fParModule);
  for ( auto& cluster : fClustersB ) fClusterAna->Exec(cluster, fParModule);

  // --- Sort clusters by time
  std::sort(fClustersF.begin(), fClustersF.end(),
            [](CbmStsCluster& cluster1, CbmStsCluster& cluster2) {
    return (cluster1.GetTime() < cluster2.GetTime());
  });
  std::sort(fClustersB.begin(), fClustersB.end(),
            [](CbmStsCluster& cluster1, CbmStsCluster& cluster2) {
    return (cluster1.GetTime() < cluster2.GetTime());
  });
  ...
  // --- Perform hit finding
  assert(fHitFinder);
  fHitFinder->Exec(fClustersF, fClustersB, fHits,
                   fSetupModule->GetAddress(),
                   fTimeCutClusterSig, fTimeCutClusterAbs,
                   fDyActive, fNofStrips, fStripPitch,
                   fStereoFront, fStereoBack,
