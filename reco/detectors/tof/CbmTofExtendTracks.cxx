...
InitStatus CbmTofExtendTracks::Init() {
  ...
  if (!fStsHitArrayIn) {
    LOG(fatal) << "-W- CbmTofExtendTracks::Init: No StsHit array!";
    return kERROR;
  }
  ...
  if (!fMuchHitArrayIn) {
    LOG(fatal) << "-W- CbmTofExtendTracks::Init: No MuchHit array!";
    return kERROR;
  }
  ...
  if (!fRichHitArrayIn) {
    LOG(fatal) << "-W- CbmTofExtendTracks::Init: No RichHit array!";
    return kERROR;
  }
...
Bool_t CbmTofExtendTracks::UpdateCalHistos() {
  while (fiCorMode>0) {
	...
	LOG(info) << "UpdateCalHist on level "<<iLev
			  <<" from src "<<fiCorSrc<<" in mode "<<iCorMode;
    switch (iCorMode) {
      case 0: { // T
        TH2* hCorDT=NULL;
      	if(fiCorSrc%10 == 0 ) hCorDT=fhTrkStationDT[iLev];
      	else                  hCorDT=fhTrkPullDT[iLev];
          if ( NULL != hCorDT ) {
              ...
              if(dLim > 0.) {              
                ...
                LOG(warn) << "Update hExt_Toff Ind " << ix << ": Old "
                          << fhExt_Toff->GetBinContent(ix + 1)  
  					      << ", FitMean "
                          << dFMean << " => " << dVal
						  << ", width "<< fhExt_Tsig->GetBinContent(ix + 1);
              ...
              if(dLim > 0.) {              
                ...
                LOG(warn) << "Update hExt_Xoff Ind " << ix << ": Old "
                          << fhExt_Xoff->GetBinContent(ix + 1)  
  					      << ", FitMean "
                          << dFMean << " => " << dVal
						  << ", width "<< fhExt_Xsig->GetBinContent(ix + 1);						  
              }
            } else {
    ...
    case 2: { // Y
              ...
              if(dLim > 0.) {
                ...
                LOG(warn) << "Update hExt_Yoff Ind " << ix << ": Old "
                          << fhExt_Yoff->GetBinContent(ix + 1)  
  					      << ", FitMean "
                          << dFMean << " => " << dVal
						  << ", width "<< fhExt_Ysig->GetBinContent(ix + 1);
              }
            } else {
      ...
      break;
      default:
    	LOG(info)<<"Correction Mode not implemented";
    } // switch end
  }
  return kTRUE;
}
...
void CbmTofExtendTracks::Exec(Option_t* opt) {
  ...
  if (!fEventsColl) {
    ...
    LOG(info) << "ExtExec TS " << fiTS
    		  << " with " << fEventsColl->GetEntriesFast() << " evts";
    fiTS++;
...
void CbmTofExtendTracks::ExecExtend(Option_t* /*opt*/, CbmEvent* tEvent) {
  ...
  const Double_t STATION_TOF_ZWIDTH  = 1.;
  const Double_t STATION_STS_ZWIDTH  = 0.5;
    ...
    for (Int_t iHit = 0; iHit < tEvent->GetNofData(ECbmDataType::kMuchPixelHit);
      ...
      if (fiEvent < NDefSetup) {
        ...
        if (itMapStationZ != fMapStationZ.end()) {
          ...
          LOG(info) << Form("Proc ev %d, MuchIn St %d, H %d, MpHit: ",
          		fiEvent, iAllSt, iH) << tHit->ToString();          
  ...
  */	  
  ...
  Int_t iNTrks=tEvent->GetNofData(ECbmDataType::kTofTrack);
  if( iNTrks<0 ) iNTrks=0;
  fvTrkCalHits.clear();
...
void CbmTofExtendTracks::CreateHistograms() {
  ...
  for (; itMapStationZ != fMapStationZ.end(); ++itMapStationZ) {
    ...
    LOG(info)<< Form(" station %2d, z %3d, Id %4d ",
    		iStNum, itMapStationZ->first,itMapStationZ->second);
  }  
  UInt_t NSt = fMapStationZ.size();
  ...
  for(Int_t iLev=0; iLev<NLev; iLev++){
    fhExt_TrkSizVel[iLev] = 
      new TH2F(Form("hExt_TrkSizVel%d",iLev),";TrkSize;v (cm/ns)",15,0,15,100,0.,50.);
    fhExt_TrkSizChiSq[iLev] = 
      new TH2F(Form("hExt_TrkSizChiSq%d",iLev),";TrkSize;#chi^2",15,0,15,50,0.,5.);
  
    fhTrkStationDX[iLev] = new TH2F(Form("hTrkStationDX%d",iLev),
                            ...
                            -10.,
                            10.);
    fhTrkStationDY[iLev] = new TH2F(Form("hTrkStationDY%d",iLev),
                            ...
                            -10.,
                            10.);  
    fhTrkStationDZ[iLev] = new TH2F(Form("hTrkStationDZ%d",iLev),
                            ...
                            -20.,
                            20.);
    fhTrkStationDT[iLev] = new TH2F(Form("hTrkStationDT%d",iLev),
                            ...
                            -50.,
                            50.);
    fhTrkStationNHits[iLev] = new TH2F(Form("hTrkStationNHits%d",iLev),
  ...
  if (fiStationUT > -1) {
	...
	itMapStationZ = fMapStationZ.begin();
	for (Int_t iSt=0; iSt<fiStationUT; iSt++) itMapStationZ++;
	dSUT_z=itMapStationZ->first;
	LOG(info)<<"Create SUT histos for station "
			 << fiStationUT << " at distance "<<dSUT_z;
	...
	fhExtSutXY_Missed =
	  new TH2F("hExtSutXY_Missed",
	             ...
	             dSUT_dy);	
	fhExtSutXY_DX =
	  new TH3F("hExtSutXY_DX",
	             Form("StationUnderTest %d #DeltaX ; X (cm); Y (cm); #DeltaX (cm)",fiStationUT),
	             dNbinX,
	             -dSUT_dx,
	             dSUT_dx,
	             dNbinY,
				 ...
				 10.	);		
	fhExtSutXY_DY =
	  new TH3F("hExtSutXY_DY",
	             Form("StationUnderTest %d #DeltaY ; X (cm); Y (cm); #DeltaY (cm)",fiStationUT),
	             dNbinX,
	             -dSUT_dx,
	             dSUT_dx,
	             dNbinY,
				 ...
				 10.	);		
	fhExtSutXY_DT =
	  new TH3F("hExtSutXY_DT",
	             Form("StationUnderTest %d #DeltaT ; X (cm); Y (cm); #DeltaT (ns)",fiStationUT),
	             dNbinX,
	             -dSUT_dx,
	             dSUT_dx,
	             dNbinY,
...
void CbmTofExtendTracks::FillHistograms(CbmEvent* tEvent) {
  ...
  Int_t NTrkTof=tEvent->GetNofData(ECbmDataType::kTofTrack);
  ...
  for (Int_t iTr = 0; iTr < NTrkTof; iTr++) {
    ...
    for (UInt_t iSt = 0; iSt < fvMuchHitIndex.size(); iSt++) {
      for (UInt_t iHit = 0; iHit < fvMuchHitIndex[iSt].size(); iHit++) {
        ...
        LOG(info) << Form("Proc ev %d, Trk %d, St %d, iLe%d MtHit: ",
        		fiEvent, iTr, iTrSt, iLev) << tHit->ToString();
        ...
        LOG(info)<<"Got MuchCorI Tr "<<iTr
				...
				<<", dy " << dDY;
        */
        LOG(info) << Form("Proc ev %d, Trk %d, St %d, iLe%d, DX %6.3f, dY %6.3f, MtHit: ",
        		fiEvent, iTr, iTrSt, iLev, dDX, dDY) << tHit->ToString();
  ...
  for(UInt_t iTr=0; iTr<NTrk; iTr++){
	...
	for (UInt_t iH=0; iH<fvTrkCalHits[iTr].size(); iH++){
	  if ( fMapStationZ[(Int_t)(fvTrkCalHits[iTr][iH]->GetZ())]/100
		== Int_t(ECbmModuleId::kTof) ) NTofHits++;
	}
...
//	  LOG(info)<<"CheckII with "<<iReqStations<<","<<iHit;
	...
	fhExt_TrkSizChiSq[iLev]->Fill(
	  (Double_t) fvTrkCalHits[iTr].size(),fvTrkPar[iTr]->GetChiSq());
			  
	fhExt_TrkSizVel[iLev]->Fill(
	  (Double_t) fvTrkCalHits[iTr].size(), 1./(fvTrkPar[iTr]->GetTt()));
	...
	for (UInt_t iHit=0; iHit<fvTrkCalHits[iTr].size(); iHit++) {
      ...
      LOG(info) << Form("Proc ev %d, Trk %d, St %d, Lev%d, DX %6.3f, dY %6.3f, MtHit: ",
      		fiEvent, iTr, iTrSt, 0, dDX, dDY)<< tHit->ToString();
	...
	for (UInt_t iSt=0; iSt<NSt; iSt++) {
	  for (UInt_t iHit=0; iHit<fvAllHitPointer[iSt].size(); iHit++) {
	    ...
	    LOG(info)<<"Got AllHit for Tr "<<iTr
			                <<", St "<<iSt
							<<", StZ "<<fMapStationZ[(Int_t)(tHit->GetZ())]%100
	                		<<", Hit "<< iHit <<", poi "<< tHit;
	    LOG(info)<<"tHit: "<<tHit->ToString();
	    */
	    assert((Int_t)iSt == fMapStationZ[(Int_t)(tHit->GetZ())]%100);
        ...
        LOG(info) << Form("Proc ev %d, Trk %d, St %d, Lev%d, DX %6.3f, dY %6.3f, MtHit: ",
        		fiEvent, iTr, iSt, iLev, dDX, dDY)<< tHit->ToString();
...
void CbmTofExtendTracks::Line3Dfit(std::vector<CbmPixelHit*> pHits, 
  ...
  LOG(debug) << "Line3Dfit result("<<pHits.size()<<") " 
		    << gMinuit->fCstatu << " : X0 " 
		    << Form (" %7.4f, TX %7.4f, Y0 %7.4f, TY %7.4f, Chi2DoF %7.4f ",
		    		dRes[0], dRes[1], dRes[2], dRes[3],fMinuit.GetChi2DoF());
...
void CbmTofExtendTracks::TrkAddStation(Int_t iStation) {
  ...
  for (UInt_t iTr=0; iTr<NTr; iTr++){
	for (UInt_t iHit=0; iHit<fvAllHitPointer[iStation].size(); iHit++) {
	  ...
	  LOG(debug)<<" Hit "<<iHit<<": X "<<tHit->GetX()
			   <<", Y "<<tHit->GetY()
			   <<", Z "<<tHit->GetZ()
			   <<", T "<<tHit->GetTime();
	  Double_t MatchChi2=
	    TMath::Power((tHit->GetX()+fvXoff[iStation]
					 - GetFitX(tHit->GetZ(),fvTrkPar[iTr]))/tSIGX,2) +
		TMath::Power((tHit->GetY()+fvYoff[iStation]
					 - GetFitY(tHit->GetZ(),fvTrkPar[iTr]))/tSIGY,2);
		TMath::Power((tHit->GetTime()+fvToff[iStation]
				     - GetFitT(tHit->GetZ(),fvTrkPar[iTr]))/tSIGT,2);
	  MatchChi2 /= 3.;
	  ...
	  if( MatchChi2 < fdChi2Max ) {
	    ...
	    LOG(debug)<<"New Match "<<iMatch<<" stored as candidate "
	    		<<iCand<<" for Tr "<<iTr<<" with hit "<<iHit;
	  }
  ...
  for (Int_t iCand=0; iCand<iMatch; iCand++){
	if( MatchHit[iCand]>-1) {
	  ...
	  LOG(info)<<"Add hit "<<MatchHit[iCand]
			   <<", station "<<fMapStationZ[(Int_t)(tHit->GetZ())]%100
	  ...
	  */
	  ...
	  for(Int_t i=iCand+1; i<iMatch; i++) {
	    if( MatchHit[i]==MatchHit[iCand] ||
	        MatchTrk[i]==MatchTrk[iCand] ) MatchHit[i]=-1;
	  }
