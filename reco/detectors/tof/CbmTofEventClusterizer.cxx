...
Bool_t   CbmTofEventClusterizer::RegisterInputs()
{
   ...
   if( NULL == fManager)
   {
      LOG(error)<<"CbmTofEventClusterizer::RegisterInputs => Could not find FairRootManager!!!";
      return kFALSE;
   ...
   */
   ...
   if ( ! fDigiMan->IsPresent(ECbmModuleId::kTof) ) {
     ...
     if( NULL == fTrbHeader)   {
       LOG(info)<<"CbmTofEventClusterizer::RegisterInputs => Could not get TofTrbHeader Object";
     }
...
Bool_t   CbmTofEventClusterizer::InitParameters()
{
   ...
   if( 0 == fDigiPar )
   {
      LOG(error)<<"CbmTofEventClusterizer::InitParameters => Could not obtain the CbmTofDigiPar ";
      return kFALSE; 
   }
   ...
   if( 0 == fDigiBdfPar )
   {
      LOG(error)<<"CbmTofEventClusterizer::InitParameters => Could not obtain the CbmTofDigiBdfPar ";
      return kFALSE; 
   }
		...
		*/
                ...
                if(NULL != htempWalk0 && NULL != htempWalk1 ) { // reinitialize Walk array 
                  ...
                  if(htempWalk0->GetNbinsX() != nbClWalkBinX) 
                    LOG(error)<<"CbmTofEventClusterizer::InitCalibParameter: Inconsistent Walk histograms"
                              ;
...
Bool_t   CbmTofEventClusterizer::LoadGeometry()
{
   ...
   for(Int_t iDetIndx=0; iDetIndx<iNbDet; iDetIndx++){
     ...
     for (Int_t iCh = 0; iCh<iNbChan*2; iCh++)  fvTimeLastDigi[iDetIndx][iCh]=0.;
           ...
           for( Int_t iRpc = 0; iRpc < iNbRpc; iRpc++ )
            {
               ...
               Int_t iNbChan = fDigiBdfPar->GetNbChan( iSmType, iRpc );
	       if(iNbChan == 0) {
               LOG(warning)<<"CbmTofEventClusterizer::LoadGeometry: StoreDigi without channels "
...
Bool_t   CbmTofEventClusterizer::CreateHistos()
{
   TDirectory * oldir = gDirectory; // <= To prevent histos from being sucked in by the param file of the TRootManager!
   gROOT->cd(); // <= To prevent histos from being sucked in by the param file of the TRootManager !
   fhClustBuildTime = new TH1I( "TofEventClus_ClustBuildTime", "Time needed to build clusters in each event; Time [s]",4000, 0.0, 4.0 );
...
/*   TH2* hTemp = 0;*/
   ...
   for(Int_t iDetIndx=0; iDetIndx<iNbDet; iDetIndx++){
	 ...
	 if(NULL == fChannelInfo)
           LOG(warning)<<Form("missing ChannelInfo for ch %d addr 0x%08x",iCh,iCCellId);
       }
       ...
       fhRpcDigiMul[iDetIndx] =  new TH2F(
          Form("cl_SmT%01d_sm%03d_rpc%03d_DigiMul", iSmType, iSmId, iRpcId ),
          Form("Digi Multiplicity of Rpc #%03d in Sm %03d of type %d; strip #; digi mul", iRpcId, iSmId, iSmType ),
            fDigiBdfPar->GetNbChan(iSmType,iRpcId),0,fDigiBdfPar->GetNbChan(iSmType,iRpcId),
	   15,0,15.);
       ...
       fhRpcDigiDTLD[iDetIndx] =  new TH2F(
          ...
          Form("Time distance to last digi of Rpc #%03d in Sm %03d of type %d; channel; t_{digi} - t_{previous digi} (s)", iRpcId, iSmId, iSmType ),
   ...
   for(Int_t iDetIndx=0; iDetIndx<iNbDet; iDetIndx++){
	  ...
	  fDigiBdfPar->GetNbChan(iSmType,iRpcId)+2,0.,fDigiBdfPar->GetNbChan(iSmType,iRpcId)+2);
       ...
       fhTRpcCluTofOff[iDetIndx][iSel] =  new TH2F( 
          Form("cl_SmT%01d_sm%03d_rpc%03d_Sel%02d_TofOff", iSmType, iSmId, iRpcId, iSel ),
          Form("Clu TimeDeviation of Rpc #%03d in Sm %03d of type %d under Selector %02d; Strip []; TOff [ns]", iRpcId, iSmId, iSmType, iSel ),
          fDigiBdfPar->GetNbChan(iSmType,iRpcId),0,fDigiBdfPar->GetNbChan(iSmType,iRpcId),
          999, -TSumMax*4.,TSumMax*4.); 
  ...
  if(0<iNSel){                  // check software triggers
   ...
   for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
   {
     ...
     if(iHitInd == 0) {
       ...
       if ( iDetMul > 5 && fviClusterMul[fiBeamRefType][fiBeamRefSm][fiBeamRefDet]>0) { // FIXME: hardwired constants
...
//            CbmTofDigi *pDig0 = (CbmTofDigi*) (fTofCalDigisColl->At(iDigInd0));
   ...
   for (Int_t iSel=0; iSel<iNSel; iSel++){
	 ...
	 if(fIdMode==0 && fDutRpc>-1) {iR0=fDutRpc; iRl=fDutRpc+1;}
	 ...
	 if(fIdMode==0 && fSelRpc>-1) {iR0=fSelRpc; iRl=fSelRpc+1;}
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iRefMul += fviClusterMul[fSelId][fSelSm][iRpc];
	   ...
	   LOG(debug1)<<"CbmTofEventClusterizer::FillHistos(): Found selector 1, BeamRef at" << pBeamRef;
             dTTrig[iSel]=dDoubleMax;
             for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
             {
              pHit = (CbmTofHit*) fTofHitsColl->At( iHitInd );
       ...
       default :
       LOG(info)<<"CbmTofEventClusterizer::FillHistos: selection not implemented "<<iSel;
         ;
     }  // switch end 
     if(fTRefMode>10){
       dTTrig[iSel]=dTRef;
...
//         CbmTofDigi *pDig1 = (CbmTofDigi*) (fTofCalDigisColl->At(iDigInd1));
         ...
         if(iCh0 != iCh1 || iS0==iS1){
           LOG(fatal)<<Form(" MT2 for Tofhit %d in iDetIndx %d, Ch %d from %3.0f strips: ",iHitInd,iDetIndx,iCh,dNstrips)
     ...
     if(TMath::Abs(hitpos_local[1])< (fhRpcCluPosition[iDetIndx]->GetYaxis()->GetXmax())){
		...
		*/
		 ...
		 fhTRpcCluTofOff[iDetIndx][iSel]->Fill((Double_t)iCh,
						       pHit->GetTime()-dTTrig[iSel]);  // valid for beam experiments
...
//     fhRpcDTLastHits_CluSize[iDetIndx]->Write();
     ...
     LOG(debug)<<"Write triggered Histos for Det Ind "<<iDetIndx<<Form(", UID 0x%08x",fDigiBdfPar->GetDetUId(iDetIndx));
	 ...
	 if (Ymax > 0.) {
	   ...
	   if ( TMath::Abs(dTOffmax)  > 0.3*htempTOff_py->GetXaxis()->GetXmax()) { 
	     LOG(debug) << "Use Maximum of TOff in ch " << iCh << " of histo " <<   htempTOff-> GetName()
		       << ": " << dTOffmax << ", " << htempTOff_py->GetXaxis()->GetXmax()
		       <<" instead of " <<  htempTOff_pfx->GetBinContent(iCh+1)
		        ;
...
//           htempTOff     = fhRpcCluDelMatTOff[iDetIndx]; // -> Comment to remove warning because set but never used
	 ...
	 {
	 Int_t iCalSel=0;
	 htempPos_pfx  = fhTRpcCluPosition[iDetIndx][iCalSel]->ProfileX("_pfx",1,fhTRpcCluPosition[iDetIndx][iCalSel]->GetNbinsY());
         htempTOff_pfx = fhTRpcCluTOff[iDetIndx][iCalSel]->ProfileX("_pfx",1,fhTRpcCluTofOff[iDetIndx][iCalSel]->GetNbinsY(),"s");
         htempTOff_px  = fhTRpcCluTofOff[iDetIndx][iCalSel]->ProjectionX("_px",1,fhTRpcCluTofOff[iDetIndx][iCalSel]->GetNbinsY());
	 }
	 break;
...
//         h2tmp1->Write();
	  ...
	  //if(5==iSmType || 8==iSmType || 2==iSmType) iWalkUpd=0; // keep both sides consistent for diamonds and pads 
	   ...
	   Int_t iTry=3;
           while(iTry-- > 0 &&  h1tmp0->GetBinContent(iWx+1)!=fvCPWalk[iSmType][iSm*iNbRpc+iRpc][iCh][0][iWx]) {    
	     h1tmp0->SetBinContent(iWx+1,fvCPWalk[iSmType][iSm*iNbRpc+iRpc][iCh][0][iWx]);
	   }
	   if (iTry==0)
	     LOG(error)<<"writing not successful for "<<h1tmp0->GetName()<<", attempts left: "<<iTry<<", iWx "<<iWx
		       <<", got "<< h1tmp0->GetBinContent(iWx+1)
		       <<", expected "<<fvCPWalk[iSmType][iSm*iNbRpc+iRpc][iCh][0][iWx]
		       ;    
	   iTry=3;
           while(iTry-- > 0 &&  h1tmp1->GetBinContent(iWx+1)!=fvCPWalk[iSmType][iSm*iNbRpc+iRpc][iCh][1][iWx]) {  
	     h1tmp1->SetBinContent(iWx+1,fvCPWalk[iSmType][iSm*iNbRpc+iRpc][iCh][1][iWx]);
	   }
	   if (iTry==0)
	     LOG(error)<<"writing not successful for "<<h1tmp1->GetName()<<", attempts left: "<<iTry<<", iWx "<<iWx
...
//          htmp1->Write();
	 ...
	 if(TMath::Abs(dTOffmax-TMean) > 2.*TMean) {
	   TMean=dTOffmax;
	   LOG(debug) << "Use peak position for TOff of TSR "<<iSmType<<iSm<<iRpc<<", B= "<<iB<<": "<<TMean;
	 }
         Double_t TWidth=((TProfile *)hAvTOff_pfx)->GetBinError(iB+1);
         Double_t dTYOff=YMean/fDigiBdfPar->GetSigVel(iSmType,iSm,iRpc) ;

	 if (fiBeamRefAddr == iUniqueId ) TMean=0.; // don't shift reference counter
         LOG(debug)<<Form("<ICor> Correct TSR %d%d%d by TMean=%8.2f, TYOff=%8.2f, TWidth=%8.2f, ",iSmType,iSm,iRpc,TMean,dTYOff,TWidth);
	 ...
	 Double_t TWMean=0.; // weighted mean of all channels BeamRef counter channels
	  ...
	  if(htempPos_py->GetEntries() > fdYFitMin  && fPosYMaxScal < -1.1) { //disabled
		...
		if(TMath::Abs(ff->GetParameter(3)-YMean)<0.5*fChannelInfo->GetSizey()){
		  ...
		  LOG(info) << "FBad YBox "<<htempPos_py->GetEntries()<<" entries in "<<iSmType<<iSm<<iRpc<<iCh
			<<", chi2 "<<ff->GetChisquare()
			<< Form(", striplen (%5.2f), %4.2f: %7.2f +/- %5.2f, pos res %5.2f +/- %5.2f at y_cen = %5.2f +/- %5.2f",
				fChannelInfo->GetSizey(),dVscal,
				2.*ff->GetParameter(1),2.*ff->GetParError(1),
	  ...
	  if (fiBeamRefAddr == iUniqueId ) {  // don't shift reference counter
		...
		if( 0 != ((TH1 *)htempTOff_px)->GetBinContent(iRefCh+1)) {
		  ...
		  TWMean+= ((TProfile *)htempTOff_pfx)->GetBinContent(iRefCh+1) * ((TH1 *)htempTOff_px)->GetBinContent(iRefCh+1);
		}
	      }
	      if(dW >0.) TWMean /= dW;
	      else           TWMean=0.;
...
/*            Double_t Ymax=hbin->GetMaximum();*/
	...
	const Double_t TMaxDev=1.; //FIXME, const value in code
        ...
        {
	  if ( TMath::Abs(TOff0_mean - fvCPTOff[iSmType][iSm*iNbRpc+iRpc][iCh][0])>TMaxDev )  fvCPTOff[iSmType][iSm*iNbRpc+iRpc][iCh][0]=TOff0_mean;
	  if ( TMath::Abs(TOff1_mean - fvCPTOff[iSmType][iSm*iNbRpc+iRpc][iCh][1])>TMaxDev )  fvCPTOff[iSmType][iSm*iNbRpc+iRpc][iCh][1]=TOff1_mean;
	}
...
//     fhSmCluTOff[iS]->Write();
     ...
     for (Int_t iPar=0; iPar<4; iPar++) fhSmCluFpar[iS][iPar]->Write();
...
Bool_t   CbmTofEventClusterizer::BuildClusters()
{
  ...
  if( kTRUE )
  {
    for( Int_t iDigInd = 0; iDigInd < iNbTofDigi; iDigInd++ )
    {
      //CbmTofDigi *pDigi = (CbmTofDigi*) fTofDigisColl->At( iDigInd );
      CbmTofDigi *pDigi = &(fTofDigiVec.at( iDigInd ));
      LOG(debug)<<iDigInd<<" "<<pDigi<<Form(" Address : 0x%08x ",pDigi->GetAddress())
                     <<" SmT " << pDigi->GetType()
                     <<" Sm " << pDigi->GetSm()
                     <<" Rpc "<< pDigi->GetRpc()
                     <<" Ch " << pDigi->GetChannel()
        ...
        if(fvTimeLastDigi[iDetIndx][iDigiCh] > 0) {
          if( fdStartAna10s > 0.) {
            ...
            if ( dTimeAna10s < fdSpillDuration)
              fhRpcDigiDTLD[iDetIndx]->Fill( iDigiCh,  (pDigi->GetTime()-fvTimeLastDigi[iDetIndx][iDigiCh])/1.E9 );
          }
        }
            ...
            if(iDigI3 == iNbTofDigi) // same side neighbour did not fire
            {
              ...
              switch(iCorMode){
                ...
                case 1: // shift found hit
                  LOG(debug2) << Form("shift channel %d%d%d%d%d and  ",(Int_t)pDigi->GetType(),(Int_t)pDigi->GetSm(),(Int_t)pDigi->GetRpc(),(Int_t)pDigi->GetChannel(),(Int_t)pDigi->GetSide())
                  ...
                  LOG(debug2) << Form("resultchannel %d%d%d%d%d and  ",(Int_t)pDigi->GetType(),(Int_t)pDigi->GetSm(),(Int_t)pDigi->GetRpc(),(Int_t)pDigi->GetChannel(),(Int_t)pDigi->GetSide())
				     << Form(" %d%d%d%d%d ",(Int_t)pDigi2->GetType(),(Int_t)pDigi2->GetSm(),(Int_t)pDigi2->GetRpc(),(Int_t)pDigi2->GetChannel(),(Int_t)pDigi2->GetSide())
				     ;
                  break;
      ...
      if( pDigi2Min !=NULL ){
        CbmTofDetectorInfo xDetInfo(ECbmModuleId::kTof, pDigi->GetType(), pDigi->GetSm(), pDigi->GetRpc(), 0, pDigi->GetChannel());
        Int_t iChId = fTofId->SetDetectorInfo( xDetInfo );
        fChannelInfo = fDigiPar->GetCell( iChId );
        if(NULL == fChannelInfo){
  ...
  if( kTRUE == fDigiBdfPar->UseExpandedDigi() )
  {
    ...
    for(Int_t iDetIndx=0; iDetIndx<iNbDet; iDetIndx++){
      ...
      for(Int_t iStrip=0; iStrip<iNbStrips; iStrip++)  {
        Int_t iDigiMul =  fStorDigiExp[iSmType] [iSm*fDigiBdfPar->GetNbRpc(iSmType) + iRpc] [iStrip].size() ;
        ...
        if( iDigiMul > 0 ) {
          ...
          if(iDigiMul==1) {
            ...
            if(iStrip > 0)
              if ( fStorDigiExp[iSmType] [iSm*fDigiBdfPar->GetNbRpc(iSmType ) + iRpc] [iStrip-1].size() > 1)  {
                fhRpcDigiStatus[iDetIndx] ->Fill(iStrip,1);
                if( TMath::Abs(
                    fStorDigiExp[iSmType] [iSm*fDigiBdfPar->GetNbRpc( iSmType ) + iRpc] [iStrip][0]->GetTime()
                    -fStorDigiExp[iSmType] [iSm*fDigiBdfPar->GetNbRpc( iSmType ) + iRpc] [iStrip-1][0]->GetTime()) < fMaxTimeDist )
...
Bool_t CbmTofEventClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
  ...
  LOG(debug1) << Form("Inspect channel TSRC %d%d%d%d at time %f, pos %f, size ",iSmType,iSm,iRpc,iCh,dLastTime,dLastPosY)
  ...
  if( 1 < fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size() )
  {
    ...
    for(size_t i1=0; i1<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()-1; i1++)
    {
      ...
      {
	LOG(debug1)<<"check digi pair "<<i1<<","<<i2<<" with size "<<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size();
  ...
  */
  ...
  for (UInt_t i=0; i<vDigiIndRef.size();i++){
    LOG(debug1)<<" "<<vDigiIndRef.at(i)<<"(M"<<fviClusterMul[iSmType][iSm][iRpc]<<")";
  }
  LOG(debug1)  ;
