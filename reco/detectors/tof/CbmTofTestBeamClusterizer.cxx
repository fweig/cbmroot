...
/************************************************************************************/
CbmTofTestBeamClusterizer::CbmTofTestBeamClusterizer() : CbmTofTestBeamClusterizer("TestbeamClusterizer", 0, 0)
{
...
Bool_t CbmTofTestBeamClusterizer::RegisterOutputs()
{
  ...
  rootMgr->Register(tHitDigiMatchBranchName, "Tof", fTofDigiMatchColl,
                    fbWriteHitsInOut && IsOutputBranchPersistent(tHitDigiMatchBranchName));
...
Bool_t CbmTofTestBeamClusterizer::InitParameters()
{
  ...
  LOG(info) << "CbmTofTestBeamClusterizer::InitParameter: currently " << fDigiPar->GetNrOfModules() << " digi cells ";
...
{
  LOG(info) << "CbmTofTestBeamClusterizer::LoadGeometry starting for  " << fDigiBdfPar->GetNbDet()
...
Bool_t CbmTofTestBeamClusterizer::CreateHistos()
{
  ...
  fhClustBuildTime =
    new TH1I("TofTestBeamClus_ClustBuildTime", "Time needed to build clusters in each event; Time [s]", 4000, 0.0, 4.0);
...
Bool_t CbmTofTestBeamClusterizer::FillHistos()
{
  ...
  if (0 < iNbTofHits) {
    ...
    if (0 < iNSel) {  // check software triggers
      ...
      LOG(debug) << "CbmTofTestBeamClusterizer::FillHistos: Muls: " << fviClusterMul[fDutId][fDutSm][fDutRpc] << ", "
      ...
      LOG(debug) << "CbmTofTestBeamClusterizer::FillHistos: BRefMul: " << iBeamRefMul << ", " << iBeamAddRefMul;
        ...
        switch (iSel) {
          case 0:  //  Detector under Test (Dut) && Diamonds,BeamRef
            ...
            LOG(debug) << "CbmTofTestBeamClusterizer::FillHistos(): selector 0: DutMul "
                       << fviClusterMul[fDutId][fDutSm][fDutRpc] << ", " << iDutMul << ", BRefMul " << iBeamRefMul
                       << " TRef: " << dTRef << ", BeamAddRefMul " << iBeamAddRefMul << ", " << fiBeamAddRefMul;
    ...
    for (Int_t iSmType = 0; iSmType < fDigiBdfPar->GetNbSmTypes(); iSmType++) {
      for (Int_t iRpc = 0; iRpc < fDigiBdfPar->GetNbRpc(iSmType); iRpc++) {
        ...
        for (UInt_t uCluster = 0; uCluster < fviClusterSize[iSmType][iRpc].size(); uCluster++) {
          LOG(debug2) << "CbmTofTestBeamClusterizer::FillHistos:  " << Form(" %3d %3d %3d ", iSmType, iRpc, uCluster);
         ...
         */
          ...
          for (Int_t iCh = 0; iCh < iNbCh; iCh++)  // update Offset and Gain
          {
            ...
            if (htempPos_py->GetEntries() > fdYFitMin && fPosYMaxScal < -1.1) {  //disabled
              LOG(debug1) << Form("Determine YMean in %s of channel %d by fit to %d entries", htempPos->GetName(), iCh,
                                  (Int_t) htempPos_py->GetEntries());
            ...
            if (htempTOff_px->GetBinContent(iCh + 1) > WalkNHmin) {
              fvCPTOff[iSmType][iSm * iNbRpc + iRpc][iCh][0] += -dTYOff + TMean;
              fvCPTOff[iSmType][iSm * iNbRpc + iRpc][iCh][1] += +dTYOff + TMean;
              LOG(debug) << Form("Calib: TSRC %d%d%d%d, hits %6.0f, dTY  %8.3f, TM %8.3f -> new "
                                 "Off %8.3f,%8.3f ",
...
Bool_t CbmTofTestBeamClusterizer::BuildClusters()
{
  ...
  if (kTRUE) {
    for (Int_t iDigInd = 0; iDigInd < iNbTofDigi; iDigInd++) {
      ...
      LOG(debug1) << iDigInd << " " << pDigi << Form(" Address : 0x%08x ", pDigi->GetAddress()) << " SmT "
                  << pDigi->GetType() << " Sm " << pDigi->GetSm() << " Rpc " << pDigi->GetRpc() << " Ch "
                  << pDigi->GetChannel() << " S " << pDigi->GetSide() << " : " << pDigi->ToString()
        //         <<" Time "<<pDigi->GetTime()
        //         <<" Tot " <<pDigi->GetTot()
        ;
    ...
    for (Int_t iDigInd = 0; iDigInd < iNbTofDigi; iDigInd++) {
      ...
      if (fDigiBdfPar->GetNbSmTypes() > pDigi->GetType()  // prevent crash due to misconfiguration
        ...
        if (0) {  //pDigi->GetType()==7 && pDigi->GetSm()==0){
          LOG(info)
            << "CbmTofTestBeamClusterizer::BuildClusters: CalDigi " << iDigIndCal << ",  T " << pCalDigi->GetType()
...
Bool_t CbmTofTestBeamClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX,
