...
CbmTofTestBeamClusterizer::CbmTofTestBeamClusterizer()
  : CbmTofTestBeamClusterizer("TestbeamClusterizer",0,0)
{
...
Bool_t   CbmTofTestBeamClusterizer::RegisterInputs()
{
   ...
   if( NULL == fManager)
   {
      LOG(error)<<"CbmTofTestBeamClusterizer::RegisterInputs => Could not find FairRootManager!!!";
      return kFALSE;
   ...
   */
   ...
   if( NULL == fTofDigisColl)
   {
      LOG(error)<<"CbmTofTestBeamClusterizer::RegisterInputs => Could not get the CbmTofDigi TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofDigisColl)
   ...
   if( NULL == fTrbHeader)
   {
      LOG(info)<<"CbmTofTestBeamClusterizer::RegisterInputs => Could not get the TofTrbHeader Object";
   }
...
Bool_t   CbmTofTestBeamClusterizer::InitParameters()
{
   ...
   if( 0 == fDigiPar )
   {
      LOG(error)<<"CbmTofTestBeamClusterizer::InitParameters => Could not obtain the CbmTofDigiPar ";
      return kFALSE; 
   }
   ...
   if( 0 == fDigiBdfPar )
   {
      LOG(error)<<"CbmTofTestBeamClusterizer::InitParameters => Could not obtain the CbmTofDigiBdfPar ";
      return kFALSE; 
   }
...
/*   TH2* hTemp = 0;*/
   ...
   for (Int_t iS=0; iS< fDigiBdfPar->GetNbSmTypes(); iS++){
     ...
     for(Int_t iSM = 0; iSM < fDigiBdfPar->GetNbSm(iS); iSM++)
     {
       ...
       {
         LOG(info)<<Form("Channel info for module type %d obtained from counter: %d-%d-%d", iS, iS, iSM, 0);
         break;
       }
     }
   ...
   for (Int_t iSel=0; iSel<iNSel; iSel++){
	 ...
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iDutMul += fviClusterMul[fDutId][fDutSm][iRpc];
         ...
         if(   iDutMul>0  && iDutMul < fiCluMulMax ) {                           
             LOG(debug1)<<"CbmTofTestBeamClusterizer::FillHistos(): Found selector 0";
	 ...
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iRefMul += fviClusterMul[fSelId][fSelSm][iRpc];
         ...
         if(   iRefMul>0  && iRefMul < fiCluMulMax  ) {                        
             LOG(debug1)<<"CbmTofTestBeamClusterizer::FillHistos(): Found selector 1";
             dTTrig[iSel]=dDoubleMax;
             for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
             {
              pHit = (CbmTofHit*) fTofHitsColl->At( iHitInd );
       ...
       default :
       LOG(info)<<"CbmTofTestBeamClusterizer::FillHistos: selection not implemented "<<iSel;
         ;
     }  // switch end 
     if(fTRefMode>10){
       dTTrig[iSel]=dTRef;
...
void CbmTofTestBeamClusterizer::fit_ybox(TH1 *h1, Double_t ysize, Double_t* fpar=NULL)
{
...
Bool_t CbmTofTestBeamClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
