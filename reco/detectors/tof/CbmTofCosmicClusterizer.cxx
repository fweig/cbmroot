...
CbmTofCosmicClusterizer::CbmTofCosmicClusterizer()
  : CbmTofCosmicClusterizer("CosmicClusterizer",0,0)
{
...
Bool_t   CbmTofCosmicClusterizer::RegisterInputs()
{
   ...
   if( NULL == fManager)
   {
      LOG(error)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not find FairRootManager!!!";
      return kFALSE;
   ...
   */
   ...
   if( NULL == fTofDigisColl)
   {
      LOG(error)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the CbmTofDigi TClonesArray!!!";
      return kFALSE;
   } // if( NULL == fTofDigisColl)
   ...
   if( NULL == fTrbHeader)
   {
      LOG(info)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the TofTrbHeader Object";
   }
...
Bool_t   CbmTofCosmicClusterizer::InitParameters()
{
   ...
   if( k14a > iGeoVersion )
   {
      LOG(error)<<"CbmTofCosmicClusterizer::InitParameters => Only compatible with geometries after v14a !!!"
                ;
      return kFALSE;
   }
   ...
   if( 0 == fDigiPar )
   {
      LOG(error)<<"CbmTofCosmicClusterizer::InitParameters => Could not obtain the CbmTofDigiPar ";
      return kFALSE; 
   }
   ...
   if( 0 == fDigiBdfPar )
   {
      LOG(error)<<"CbmTofCosmicClusterizer::InitParameters => Could not obtain the CbmTofDigiBdfPar ";
      return kFALSE; 
   }

   rtdb->initContainers(  ana->GetRunId() );

   LOG(info)<<"CbmTofCosmicClusterizer::InitParameter: currently " 
   ...
   LOG(info)<<"<I>  Sel2Type = "<<fSel2Id<<", Sm "<<fSel2Sm<<", Det "<<fSel2Rpc
	    <<", MulMax "<<fSel2MulMax
            ;
            ...
            for( Int_t iCh = 0; iCh < iNbCh; iCh++ )
              {
                ...
                if(NULL != htempWalk0 && NULL != htempWalk1 ) { // reinitialize Walk array 
                  ...
                  if(htempWalk0->GetNbinsX() != nbClWalkBinX) 
                    LOG(error)<<"CbmTofCosmicClusterizer::InitCalibParameter: Inconsistent Walk histograms"
                              ;
...
{
   LOG(info)<<"CbmTofCosmicClusterizer::LoadGeometry starting for  "
           ...
           for( Int_t iRpc = 0; iRpc < iNbRpc; iRpc++ )
            {
               ...
               Int_t iNbChan = fDigiBdfPar->GetNbChan( iSmType, iRpc );
	       if(iNbChan == 0) {
               LOG(warning)<<"CbmTofCosmicClusterizer::LoadGeometry: StoreDigi without channels "
...
Bool_t   CbmTofCosmicClusterizer::CreateHistos()
{
   TDirectory * oldir = gDirectory; // <= To prevent histos from being sucked in by the param file of the TRootManager!
   gROOT->cd(); // <= To prevent histos from being sucked in by the param file of the TRootManager !
   fhClustBuildTime = new TH1I( "TofCosmicClus_ClustBuildTime", "Time needed to build clusters in each event; Time [s]",4000, 0.0, 4.0 );
   ...
   for(Int_t iDetIndx=0; iDetIndx<iNbDet; iDetIndx++){
	  ...
	  fDigiBdfPar->GetNbChan(iSmType,iRpcId),0.,fDigiBdfPar->GetNbChan(iSmType,iRpcId));
       ...
       fhTRpcCluDelTof[iDetIndx][iSel] = new TH2F( 
          ...
          nbClDelTofBinX,0.,fdDelTofMax,nbClDelTofBinY,-TSumMax,TSumMax);  // x-axis is distance of hits 
...
Bool_t   CbmTofCosmicClusterizer::FillHistos()
{
 ...
 // plot remaining time difference to previous hits 
   ...
   LOG(debug) <<"CbmTofCosmicClusterizer::FillHistos: Sel2Mul: "
              <<iSel2Mul
              ;
   ...
   for (Int_t iSel=0; iSel<iNSel; iSel++){
	 ...
	 LOG(debug)<<"selector 1: RefMul "
       ...
       default :
       LOG(info)<<"CbmTofCosmicClusterizer::FillHistos: selection not implemented "<<iSel;
         ;
     }  // switch end 
     if(fTRefMode>10){
	 ...
	 //if(fviClusterMul[fSel2Id][fSel2Sm][fSel2Rpc] > 0 && fviClusterMul[fSel2Id][fSel2Sm][fSel2Rpc] < fiCluMulMax)
	     ...
	     Double_t dSEl2dYdz=(pHit->GetY()-pTrig[iSel]->GetY())/(pHit->GetZ()-pTrig[iSel]->GetZ());
             ...
             if (TMath::Sqrt(TMath::Power(pHit->GetX()-dzscal*pTrig[iSel]->GetX(),2.)
                            +TMath::Power(pHit->GetY()-dzscal*pTrig[iSel]->GetY(),2.))<fdCaldXdYMax * dMatXYScal )
             {
               BSel[iSel]=kTRUE;
 ...
 for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
 {
   ...
   if(TMath::Abs(hitpos_local[1])>fChannelInfo->GetSizey()*fPosYMaxScal) continue;
	 ...
	 // Fill event selected (triggered) histograms 
	   ...
	   //// look for geometrical match  with selector hit
	     ...
	     if ( !fEnableMatchPosScaling && dSel2dXdYMin[iSel]<1.E300)
		    ...
		    > 0.5*fdCaldXdYMax) continue;  // FIXME: refine position selection cut in cosmic measurement
		 ...
		 // Double_t dAvTot=0.5*(pDig0->GetTot()+pDig1->GetTot());   (VF) not used
               ...
               LOG(debug)<<Form("TRpcCluWalk for Ev %d, Link %d(%d), Sel %d, TSR %d%d%d, Ch %d,%d, S %d,%d Tcor %8.3f, LocT %6.1f, W-ent: %6.0f,%6.0f",
			 fiNevtBuild, iLink,(Int_t)digiMatch->GetNofLinks(),iSel,iSmType,iSm,iRpc,
				iCh0, iCh1, iS0, iS1, dTcor[iSel], hitpos_local[1]/fDigiBdfPar->GetSigVel(iSmType,iSm,iRpc),
		 ...
		 LOG(error) << Form(" Inconsistent walk histograms -> debugging necessary ... for %d, %d, %d, %d, %d, %d, %d ",
	       ...
	       LOG(debug)<<Form("TRpcCluWalk values side %d: %5.2f, %7.3f, %6.3f,    side %d:  %5.2f, %7.3f, %6.3f",
		 ...
		 if( iDetId != (pTrig[iSel]->GetAddress() & DetMask) ){ // transform matched hit-pair back into detector frame
		   ...
		   fhRpcCluDelMatTOff[iDetIndx]->Fill((Double_t)iCh,(pHit->GetTime()-dTTrig[iSel])-dTTcor[iSel]);
             ...
             LOG(error)<<"CbmTofCosmicClusterizer::FillHistos: invalid digi index "<<iDetIndx
	 ...
	 dDist=TMath::Sqrt( TMath::Power(pHit->GetX()-pRef->GetX(),2.)
			   +TMath::Power(pHit->GetY()-pRef->GetY(),2.)
	 ...
	 dDelTof *= dZsgn;   // store timing correction
	   ...
	   // fill Time Offset histograms with DelTof correction 
           fhTRpcCluTOff[iDetIndx][iSel]->Fill((Double_t)iCh,(pHit->GetTime()-dTTrig[iSel])-dTTcor[iSel]); //  DEC2017 modified ! 
	   if(fvLastHits[iSmType][iSm][iRpc][iCh].size()>1){     // check for previous hits in memory time interval 
	     std::list<CbmTofHit *>::iterator itL=fvLastHits[iSmType][iSm][iRpc][iCh].end(); 
	     itL--; 
   ...
   for( Int_t iSmType = 0; iSmType < fDigiBdfPar->GetNbSmTypes(); iSmType++ ){
      for( Int_t iRpc = 0; iRpc < fDigiBdfPar->GetNbRpc( iSmType); iRpc++ )
      {
         ...
         {
            LOG(debug2)<<"CbmTofCosmicClusterizer::FillHistos:  "
...
Bool_t   CbmTofCosmicClusterizer::BuildClusters()
{
  ...
  LOG(debug)<<"CbmTofCosmicClusterizer::BuildClusters from "
		...
		if(dTDif<dTDifMin){
		 ...
		 if(iDigI3 == iNbTofDigi) // same side neighbour did not fire 
		 {
		   ...
		   case 2:  // insert missing hits
		     ...
		     LOG(debug) << Form("InsertDigi  TSRCS %d%d%d%d%d and  ",(Int_t)pDigiN->GetType(),(Int_t)pDigiN->GetSm(),(Int_t)pDigiN->GetRpc(),(Int_t)pDigiN->GetChannel(),(Int_t)pDigiN->GetSide()) 
				 ;
		     ...
		     LOG(debug) << Form("InsertDigi2 TSRCS %d%d%d%d%d and  ",(Int_t)pDigiN2->GetType(),(Int_t)pDigiN2->GetSm(),(Int_t)pDigiN2->GetRpc(),(Int_t)pDigiN2->GetChannel(),(Int_t)pDigiN2->GetSide()) 
				 ;
		     break;
		   }
		 }
   ...
   if( kTRUE == fDigiBdfPar->UseExpandedDigi() )
   {
	 ...
	 if(fbPs2Ns) {
	   ...
	   if(iWx < nbClWalkBinX -1) {    // linear interpolation to next bin
         ...
         if(0) {//pDigi->GetType()==7 && pDigi->GetSm()==0){
          LOG(info)<<"CbmTofCosmicClusterizer::BuildClusters: CalDigi "
                    <<iDigIndCal<<",  T "
                    <<pCalDigi->GetType()<<", Sm "
                    <<pCalDigi->GetSm()<<", R "
                    <<pCalDigi->GetRpc()<<", Ch "
	...
	LOG(debug)<<"CbmTofCosmicClusterizer::BuildClusters: Sort "<<fTofCalDigisColl->GetEntries()<<" calibrated digis "
                   ;
      if(iNbTofDigi>1){
...
//                  dLastPosX = 0.0; // -> Comment to remove warning because set but never used
				...
				break;
                              ...
                              if(NULL == fChannelInfo){
				LOG(error)<<"CbmTofCosmicClusterizer::BuildClusters: no geometry info! "
				...
				break;
                              ...
                              if(5==iSmType && dTimeDif !=0.) { 
                                ...
                                LOG(debug)<<"CbmTofCosmicClusterizer::BuildClusters: Diamond hit in "
...
Bool_t CbmTofCosmicClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
