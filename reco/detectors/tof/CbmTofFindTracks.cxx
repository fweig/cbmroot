   ...
   case 4 : // correct mean deviation from fit (Pull), extract width for x direction
    {
	 ...
	 dVal -= htmp1D->GetBinContent(ix+1);
	 if(dVal<-3.) dVal=-3.; // limit maximal shift in X, for larger values, change geometry file
	 if(dVal>3.)  dVal=3.;
	 //if( fRpcAddr[ix] != fiBeamCounter )  // don't correct beam counter position
...
void CbmTofFindTracks::ExecFind(Option_t* /*opt*/)
{
  ...
  for (Int_t iHit=0; iHit<fTofHitArray->GetEntries(); iHit++){
   ...
   Double_t dSIGX=GetSigX(iDetId);
    ...
    LOG(debug) << Form("Exec found Hit %02d, addr 0x%08x, sta %02d, HM %02d, X %6.2f(%3.2f) Y %6.2f(%3.2f)  Z %6.2f(%3.2f)  T %6.2f(%3.2f) (%6.2f)",
		       iHit,pHit->GetAddress(),GetStationOfAddr(iDetId),fStationHMul[GetStationOfAddr(iDetId)],
		       pHit->GetX(),pHit->GetDx(),pHit->GetY(),pHit->GetDy(),pHit->GetZ(),pHit->GetDz(),pHit->GetTime(),pHit->GetTimeError(),dTcor)
	       ; 
  }
...
void CbmTofFindTracks::CreateHistograms(){
  ...
  for (Int_t iSt=0; iSt<fNTofStations; iSt++){
    ...
    vhPullTB[iSt]=new TH1F( Form("hPullTB_Station_%d",iSt),
			    ...
			    59, -1.25*fT0MAX, 1.25*fT0MAX);
    const Double_t TOTmax=50.;
    vhResidualTBWalk[iSt]=new TH2F( Form("hResidualTBWalk_Station_%d",iSt),
			    Form("hResidualTBWalk_Station_%d;  #DeltaT (ns)",iSt),
			    TOTmax,0.,TOTmax, 59, -1.25*fT0MAX, 1.25*fT0MAX);
    vhResidualYWalk[iSt]=new TH2F( Form("hResidualYWalk_Station_%d",iSt),
			    Form("hResidualYWalk_Station_%d;  #DeltaT (ns)",iSt),
			    TOTmax,0.,TOTmax,59, -DY0MAX, DY0MAX); 
    Double_t XSIZ=16.; 
    Int_t Nbins=32.;
    ...
    vhXY_DX[iSt] = new TH3F( Form("hXY_DX_%d",iSt),
			     ...
			     NbinsX, -XSIZ, XSIZ, Nbins, -XSIZ, XSIZ, Nbins, -2., 2.); 
...
//      Int_t iSt0 = GetStationOfAddr(iDetId0);                       // Station of 1. Hit (FU) not used
	...
	*/
	...
	Double_t dDTB= fTrackletTools->GetTdif(pTrk, fMapStationRpcId[iSt], pHit);    // ignore pHit in calc of reference
	...
	for (Int_t iSt=0; iSt<fNTofStations; iSt++){
	  ...
	  if ( FairLogger::GetLogger()->IsLogNeeded(fair::Severity::debug) ) {
		...
		if(iChMiss>-1 && iChMiss<iNbCh) 
		{
		  ...
		  LOG(debug)<<Form("Geo consistency check 0x%08x at St%d, z=%7.2f,%7.2f: iChTrafo %d, Miss %d , xloc %6.2f, dx %4.2f",
				   iAddr,iSt,zPos,zPosMiss,iChTrafo, iChMiss,hitpos_localD[0],fChannelInfoD->GetSizex());
		  fChannelInfo=fChannelInfoMiss;
		  iAddr=iAddrMiss;
		}
		else zPosMiss=zPos;
		LOG(debug) << Form ( "Predicted hit in Last Station 0x%08x at local pos x %6.2f, y %6.2f, z   %6.2f, cell %p",
				     iAddr, hitpos_localD[0],hitpos_localD[1], zPos, fChannelInfoD ) ;
		LOG(debug) << Form ( "Measured  hit in Last Station 0x%08x at local pos x %6.2f, y %6.2f, z   %6.2f, cell %p",
				     pHit->GetAddress(), hitpos_local[0],hitpos_local[1], 
                                     pHit->GetZ(), fChannelInfo) ;
	      }		  
	    }
	  }
	}
	...
	if( pTrk->GetNofHits() == fNReqStations-1 ) { // one hit missing
	  for (Int_t iSt=0; iSt<fNTofStations; iSt++){
		...
		if(iChMiss>-1 && iChMiss<iNbCh) 
		{
		  ...
		  LOG(debug)<<Form("Geo consistency check 0x%08x at St%d, z=%7.2f,%7.2f: iChTrafo %d, Miss %d , xloc %6.2f, dx %4.2f",
				   iAddr,iSt,zPos,zPosMiss,iChTrafo, iChMiss,hitpos_local[0],fChannelInfo->GetSizex());
		  fChannelInfo=fChannelInfoMiss;
		  iAddr=iAddrMiss;
		}
		else zPosMiss=zPos;
...
void CbmTofFindTracks::SetStation (Int_t iVal, Int_t iModType, Int_t iModId, Int_t iRpcId)
{
  Int_t iCenterCh=0;
  if(NULL != fDigiBdfPar) 
    iCenterCh=TMath::Floor((fDigiBdfPar->GetNbChan( iModType, iRpcId )-1)/2);
