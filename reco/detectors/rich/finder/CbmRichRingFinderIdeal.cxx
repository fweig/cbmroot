...
void CbmRichRingFinderIdeal::Init()
{
    ...
    if (mcManager == nullptr) LOG(fatal) << "CbmRichRingFinderIdeal::Init() NULL MCDataManager.";

    fDigiMan = CbmDigiManager::Instance();
    fDigiMan->Init();
    if ( ! fDigiMan->IsPresent(ECbmModuleId::kRich) ) { Fatal("CbmRichHitProducer::Init","No RichDigi array!"); }
    if ( ! fDigiMan->IsMatchPresent(ECbmModuleId::kRich) ) { Fatal("CbmRichHitProducer::Init","No RichMatchDigi array!"); }

    fMcTracks = mcManager->InitBranch("MCTrack");
    if (NULL == fMcTracks) {LOG(fatal) << "CbmRichRingFinderIdeal::Init No MCTrack!";}

    fRichPoints = mcManager->InitBranch("RichPoint");
    if (NULL == fRichPoints) {LOG(fatal) << "CbmRichRingFinderIdeal::Init No RichPoint!";}
    
    fEventList = (CbmMCEventList*) ioman->GetObject("MCEventList.");
    if (NULL == fEventList) {LOG(fatal) << "CbmRichRingFinderIdeal::Init No MCEventList!";}
}
                                     ...
                                     TClonesArray* ringArray)
{
    ...
    for (Int_t iHit = 0; iHit < nofRichHits; iHit++) {
        ...
        vector<pair<Int_t, Int_t>> motherIds = CbmMatchRecoToMC::GetMcTrackMotherIdsForRichHit(fDigiMan, richHit, fRichPoints, fMcTracks, eventId);
