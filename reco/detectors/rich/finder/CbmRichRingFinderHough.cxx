      ...
      TClonesArray* rRingArray)
{
	...
	if (NULL == rHitArray) {
	    LOG(error) << "-E- CbmRichRingFinderHough::DoFind: Hit array missing!"<< rHitArray;
		return -1;
	}
	...
	LOG(info) << "CbmRichRingFinderHough. Number of found rings "<< rRingArray->GetEntriesFast();

	LOG(info) << "time1:"<< dt1 << " time2:" << dt2 << " time3:" << dt3 << " total:" << dt1 + dt2 + dt3;
