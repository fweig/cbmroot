...
void CbmRichRecoTbQa::InitHistograms() {
  ...
  fHM->Create1<TH1D>("fhNofRichDigisPerTS",
                     "fhNofRichDigisPerTS;RICH digis per time slice;Yield",
                     100,
                     -1,
                     -1.);
  fHM->Create1<TH1D>("fhNofRichHitsPerTS",
                     "fhNofRichHitsPerTS;RICH hits per time slice;Yield",
                     100,
                     -1.,
                     -1.);
  fHM->Create1<TH1D>("fhNofRichRingsPerTS",
                     "fhNofRichRingsPerTS;RICH rings per time slice;Yield",
                     100,
                     -1.,
                     -1.);

  fHM->Create1<TH1D>("fhRichDigiTimeLog",
                     "fhRichDigiTimeLog;RICH digi time [ns];Yield",
                     nBinsLog,
                     minLog,
                     maxLog);

  fHM->Create1<TH1D>("fhRichRingTimeLog",
                     "fhRichRingTimeLog;RICH ring time [ns];Yield",
                     nBinsLog,
                     minLog,
                     maxLog);

  fHM->Create1<TH1D>("fhRichPointTime",
                     "fhRichPointTime;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimeChPhoton",
                     "fhRichPointTimeChPhoton;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimeNotChPhoton",
                     "fhRichPointTimeNotChPhoton;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimePrimEl",
                     "fhRichPointTimePrimEl;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimeSecEl",
                     "fhRichPointTimeSecEl;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimeOther",
                     "fhRichPointTimeOther;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);
  fHM->Create1<TH1D>("fhRichPointTimePion",
                     "fhRichPointTimePion;RICH MC point time [ns];Yield",
                     100,
                     0.,
                     100.);

  fHM->Create1<TH1D>("fhStsPointTime",
                     "fhStsPointTime;STS MC point time [ns];Yield",
                     400,
                     0.,
                     400.);
  ...
  for (int iEv = -1; iEv < fNofLogEvents; iEv++) {
    ...
    fHM->Create1<TH1D>(
      richDigiStr, richDigiStr + ";Time [ns];Yield", nBinsLog, minLog, maxLog);

    string richRingStr = "fhRichRingTimeLog_" + to_string(iEv);
    fHM->Create1<TH1D>(
      richRingStr, richRingStr + ";Time [ns];Yield", nBinsLog, minLog, maxLog);

    string richStr = "fhRichPointTimeLog_" + to_string(iEv);
    fHM->Create1<TH1D>(
      richStr, richStr + ";Time [ns];Yield", nBinsLog, minLog, maxLog);

    string stsStr = "fhStsPointTimeLog_" + to_string(iEv);
    fHM->Create1<TH1D>(
      stsStr, stsStr + ";Time [ns];Yield", nBinsLog, minLog, maxLog);
  }

  fHM->Create1<TH1D>("fhTimeBetweenEvents",
                     "fhTimeBetweenEvents;Time between events [ns];Yield",
                     100,
                     0.,
                     500.);
  ...
  fHM->Create1<TH1D>(
    "fhMomRefElAcc", "fhMomRefElAcc;P [GeV/c];Yield", 10, 0., 10.);
  fHM->Create1<TH1D>(
    "fhMomRefElRec", "fhMomRefElRec;P [GeV/c];Yield", 10, 0., 10.);


  fHM->Create1<TH1D>(
    "fhNofHitsElAcc", "fhNofHitsElAcc;Nof hits in ring;Yield", 20, -.5, 39.5);
  fHM->Create1<TH1D>(
    "fhNofHitsElRec", "fhNofHitsElRec;Nof hits in ring;Yield", 20, -.5, 39.5);
  fHM->Create1<TH1D>("fhNofHitsRefElAcc",
                     "fhNofHitsRefElAcc;Nof hits in ring;Yield",
                     20,
                     -.5,
                     39.5);
  fHM->Create1<TH1D>("fhNofHitsRefElRec",
                     "fhNofHitsRefElRec;Nof hits in ring;Yield",
                     20,
                     -.5,
                     39.5);

  fHM->Create1<TH1D>(
    "fhEventMultElAcc", "fhEventMultElAcc;Nof MC tracks;Yield", 10, 0., 2000);
  fHM->Create1<TH1D>(
    "fhEventMultElRec", "fhEventMultElRec;Nof MC tracks;Yield", 10, 0, 2000);
}
...
void CbmRichRecoTbQa::Exec(Option_t* /*option*/) {
  ...
  cout << "CbmRichRecoTbQa, time slice:" << fTimeSliceNum
  ...
  for (int iEv = 0; fEventList->GetEventTime(iEv, fileId) > 0.; iEv++) {
    double dT = (iEv == 0) ? fEventList->GetEventTime(iEv, fileId)
                           : fEventList->GetEventTime(iEv, fileId)
                               - fEventList->GetEventTime(iEv - 1, fileId);
...
Int_t CbmRichRecoTbQa::GetNofPrimaryMcTracks(Int_t iEv) {
  ...
  for (Int_t j = 0; j < nofMcTracks; j++) {
    const CbmMCTrack* track =
      static_cast<CbmMCTrack*>(fMCTracks->Get(fileId, iEv, j));
...
void CbmRichRecoTbQa::RingRecoEfficiency() {
  ...
  for (Int_t iHit = 0; iHit < nofRichHits; iHit++) {
    ...
    vector<pair<Int_t, Int_t>> motherIds =
      CbmMatchRecoToMC::GetMcTrackMotherIdsForRichHit(
        fDigiMan, hit, fRichPoints, fMCTracks, -1);
  ...
  for (Int_t iRing = 0; iRing < nofRings; iRing++) {
    ...
    const CbmMCTrack* track = static_cast<CbmMCTrack*>(
      fMCTracks->Get(fileId,
                     richRingMatch->GetMatchedLink().GetEntry(),
                     richRingMatch->GetMatchedLink().GetIndex()));
    Int_t nofPrimMcTracks =
      GetNofPrimaryMcTracks(richRingMatch->GetMatchedLink().GetEntry());
    pair<Int_t, Int_t> val =
      std::make_pair(richRingMatch->GetMatchedLink().GetEntry(),
                     richRingMatch->GetMatchedLink().GetIndex());
    Int_t nofRichHitsInRing = nofHitsInRing[val];
    Double_t mom            = track->GetP();
    Bool_t isClone =
      (std::find(fRecRings.begin(), fRecRings.end(), val) != fRecRings.end());
    if (nofRichHitsInRing >= 7 && isRichOk && IsMcPrimaryElectron(track)
        && !isClone) {
...
Bool_t
CbmRichRecoTbQa::IsCherenkovPhotonFromPrimaryElectron(const CbmRichPoint* point,
                                                      Int_t fileId,
                                                      Int_t eventId) {
  if (point == nullptr) return false;
  Int_t trackId = point->GetTrackID();
  ...
  if (mcTrack == nullptr || TMath::Abs(mcTrack->GetPdgCode()) != 50000050)
    return false;

  Int_t motherId = mcTrack->GetMotherId();
  if (motherId < 0) return false;
  CbmMCTrack* mcMotherTrack =
    (CbmMCTrack*) fMCTracks->Get(fileId, eventId, motherId);
...
void CbmRichRecoTbQa::DrawHist() {
  ...
  {
    fHM->CreateCanvas(
      "rich_tb_fhTimeBetweenEvents", "rich_tb_fhTimeBetweenEvents", 800, 800);
    DrawH1(fHM->H1("fhTimeBetweenEvents"), kLinear, kLog);
    Double_t min = 0.;
    Double_t max = 10.;
    Double_t partIntegral =
      fHM->H1("fhTimeBetweenEvents")
        ->Integral(fHM->H1("fhTimeBetweenEvents")->FindBin(min),
                   fHM->H1("fhTimeBetweenEvents")->FindBin(max));
    Double_t allIntegral = fHM->H1("fhTimeBetweenEvents")->Integral();
    Double_t ratio       = 100. * partIntegral / allIntegral;
    cout << ratio << "% of all time between events are in range (" << min
         << " ," << max << ") ns" << endl;
  }
  ...
  {
    fHM->CreateCanvas(
      "rich_tb_fhRichPointTime", "rich_tb_fhRichPointTime", 800, 800);
    DrawH1(fHM->H1("fhRichPointTime"), kLinear, kLog);
  }

  {
    TCanvas* c = fHM->CreateCanvas("rich_tb_fhRichPointTimeSources",
                                   "rich_tb_fhRichPointTimeSources",
                                   1600,
                                   800);
    c->Divide(2, 1);
    c->cd(1);
    DrawH1({fHM->H1("fhRichPointTimeChPhoton"),
            fHM->H1("fhRichPointTimeNotChPhoton")},
           {"Ch. Photons", "Not Ch. Photons"},
           kLinear,
           kLog);

    c->cd(2);
    DrawH1({fHM->H1("fhRichPointTimeSecEl"),
            fHM->H1("fhRichPointTimePrimEl"),
            fHM->H1("fhRichPointTimePion"),
            fHM->H1("fhRichPointTimeOther")},
           {"e^{#pm}_{sec}", "e^{#pm}_{prim}", "#pi^{#pm}", "Other"},
           kLinear,
           kLog);
  }
  ...
  {
    fHM->CreateCanvas(
      "rich_tb_fhStsPointTime", "rich_tb_fhStsPointTime", 800, 800);
    DrawH1(fHM->H1("fhStsPointTime"), kLinear, kLog);
  }

  {
    fHM->CreateCanvas(
      "rich_tb_fhRichPointTimeLog", "rich_tb_fhRichPointTimeLog", 1600, 600);
    DrawTimeLog("fhRichPointTimeLog", fNofLogEvents);
  }

  {
    fHM->CreateCanvas(
      "rich_tb_fhStsPointTimeLog", "rich_tb_fhStsPointTimeLog", 1600, 600);
    DrawTimeLog("fhStsPointTimeLog", fNofLogEvents);
  }

  {
    fHM->CreateCanvas("rich_tb_reco_fhRichDigiTimeLogAll",
  ...
  {
    fHM->CreateCanvas("rich_tb_reco_fhRichDigiTimeLog",
  ...
  {
    fHM->CreateCanvas("rich_tb_reco_fhRichRingTimeLogAll",
  ...
  {
    fHM->CreateCanvas("rich_tb_reco_timelog_hits_rings",
                      "rich_tb_reco_timelog_hits_rings",
                      1600,
                      600);
    DrawH1({fHM->H1("fhRichDigiTimeLog"), fHM->H1("fhRichRingTimeLog")},
           {"Hits", "Rings"});
    gPad->SetLeftMargin(0.07);
    gPad->SetRightMargin(0.05);
  }
  ...
  {
    fHM->CreateCanvas("rich_tb_reco_fhRichRingTimeLog",
  ...
  {
    TCanvas* c = fHM->CreateCanvas("rich_tb_reco_fhRichObectsPerTimeSlice",
...

  {
    TCanvas* c = fHM->CreateCanvas(
      "rich_tb_reco_acc_rec", "rich_tb_reco_acc_rec", 1600, 600);
    c->Divide(2, 1);
    c->cd(1);
    ...
    Double_t effRefMom =
      100. * (Double_t) nofRefRecMom / (Double_t) nofRefAccMom;
    ...
    Double_t effNofHits =
      100. * (Double_t) nofRecNofHits / (Double_t) nofAccNofHits;
    Double_t effRefNofHits =
      100. * (Double_t) nofRefRecNofHits / (Double_t) nofRefAccNofHits;
    ...
    TH1D* hRefEffMom =
      Cbm::DivideH1(fHM->H1("fhMomRefElRec"), fHM->H1("fhMomRefElAcc"));
    ...
    fHM->CreateCanvas(
      "rich_tb_reco_eff_nofHits", "rich_tb_reco_eff_nofHits", 800, 800);
    TH1D* hEffHits =
      Cbm::DivideH1(fHM->H1("fhNofHitsElRec"), fHM->H1("fhNofHitsElAcc"));
    TH1D* hRefEffHits =
      Cbm::DivideH1(fHM->H1("fhNofHitsRefElRec"), fHM->H1("fhNofHitsRefElAcc"));
    DrawH1(
      {hEffHits, hRefEffHits},
    ...
    fHM->CreateCanvas(
      "rich_tb_reco_eff_eventMult", "rich_tb_reco_eff_eventMult", 800, 800);
    TH1D* hEffEventMult =
      Cbm::DivideH1(fHM->H1("fhEventMultElRec"), fHM->H1("fhEventMultElAcc"));
...
void CbmRichRecoTbQa::DrawTimeLog(const string& hMainName,
  ...
  for (int iEv = startInd; iEv < nofLogEvents; iEv++) {
    ...
    string option =
      ((iEv == 0 && !withNoise) || (iEv == -1 && withNoise)) ? "" : "same";
    int ind   = iEv % 6;
    int color = (ind == 0)
                  ? kBlue
                  : (ind == 1)
