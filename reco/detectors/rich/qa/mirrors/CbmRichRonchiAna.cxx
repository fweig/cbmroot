...
// constant values
fPi(3.141592654),
fRadiusMirror(3000000), // in microns   !!! MIGHT HAVE TO BE CHANGED TO DISTANCE_MIRROR-POINTSOURCE IF IT IS NOT IN THE CENTER OF CURVATURE
fEdgeLengthCCD(13300),  // in microns
fCcdPixelSize(13),      // in microns
fPitchGrid(200),        // in microns
fImageWidth(1024),      // in pixels

// values to be measured first
fDistRulingCCD(21200),                              // in microns
fDistMirrorCCD(3118500),                            // in microns
fDistMirrorRuling(fDistMirrorCCD-fDistRulingCCD),   // in microns
fOffsetCCDOptAxisX(0),//(7500),                           // in microns
fOffsetCCDOptAxisY(-58000),                         // in microns
fOffsetLEDOpticalAxisX(0),//(14500), //(13000),                      // in microns
fOffsetLEDOpticalAxisY(57000),                      // in microns
fCenterCcdX(0),                                     // in pixels
...
void CbmRichRonchiAna::Run()
{
    ...
    TH2D* hSmoothLinesH = new TH2D("hSmoothLinesH", "hSmoothLinesH;X [pixel];Y [pixel];Intensity", width, -.5, width - 0.5, height, -0.5, height - 0.5);
    TH2D* hLineSearchH = new TH2D("hLineSearchH", "hLineSearchH;X [pixel];Y [pixel];Intensity", width, -.5, width - 0.5, height, -0.5, height - 0.5);
    ...
    TH2D* hSmoothLinesV = new TH2D("hSmoothLinesV", "hSmoothLinesV;X [pixel];Y [pixel];Intensity", width, -.5, width - 0.5, height, -0.5, height - 0.5);
    TH2D* hLineSearchV = new TH2D("hLineSearchV", "hLineSearchV;X [pixel];Y [pixel];Intensity", width, -.5, width - 0.5, height, -0.5, height - 0.5);
...
void CbmRichRonchiAna::DoMeanIntensityY(vector<vector<double> >& data)
{
    ...
    double threshold = 6500;     // threshold for average intensity to delete areas that are too dark (and so don't belong to mirror image)
    ...
    for (int x = 0; x < nX; x++) {
        for (int y = 0; y < nY; y++) {
            double total = 0.;
            double weightSum = 0.;
            for (int xW = -halfAvWindow; xW <= halfAvWindow; xW++) {    // scanning window around current pixel to calculate average intensity
                for (int yW = -halfAvWindow; yW <= halfAvWindow; yW++) {

                    int xWAbs = std::abs(xW);
                    int yWAbs = std::abs(yW);

                    double weightX = (xWAbs < (int)weightsX.size() )? weightsX[xWAbs] : weightsX[weightsX.size() - 1];   // if corresponding pixel is farther away than number of weights, take smallest ...
                    double weightY = (yWAbs < (int)weightsY.size() )? weightsY[yWAbs] : weightsY[weightsY.size() - 1];   // ... weight value
            ...
            if (dataNew[x][y] <= threshold) dataNew[x][y] = 0.;    // deleting pixels that are too dark; to yield only data for mirror and not background
        }
    }
    data = dataNew;
}
...
void CbmRichRonchiAna::DoPeakFinderY(vector<vector<double> >& data)
{
    ...
    for (int x = 0; x < nX; x++) {
        for (int y = halfWindow; y < nY - halfWindow; y++){
            bool isPeak = (data[x][y] > 0. && data[x][y] >= data[x][y - 1]) && (data[x][y] >= data[x][y + 1]); // comparing current pixel with direct neighbours in y direction
            if (!isPeak) continue;
            ...
            bool hasNeighbourPeak = (dataNew[x][y - 2] > 0. || dataNew[x][y - 1] > 0. || dataNew[x][y + 1] > 0. || dataNew[x][y + 2] > 0.);

            if (isBiggest && isPeak && !hasNeighbourPeak) dataNew[x][y] = data[x][y];   // set value if is the biggest of 'halfWindow' neighbours and has no neighboured peak
        }
    }
    data = dataNew;
}
...
void CbmRichRonchiAna::DoSmoothLines(vector<vector<double> >& data)
{
    ...
    for (int x = meanHalfLength; x < nX-meanHalfLength; x++) {  // scanning whole image (except small border)
        for (int y = meanHalfHeight; y < nY-meanHalfHeight; y++) {
            ...
            double sumY = 0.;
            int counter = 0;
            for (int x2 = -meanHalfLength; x2 <= meanHalfLength; x2++) {    // scanning small window around current pixel and summing up positions in y of current and neighboured pixels
            ...
            if (data[x][y] > 0. && dataNew[x][y] == 0.) {   // if this line pixel is not filled yet in 'dataNew'
                ...
                for (int x1 = x + 1; x1 < nX; x1++) {   // drawing line in x; values are not intensity but line index
                    ...
                    for (int y1 = -halfWindowY; y1 <= halfWindowY; y1++) {  
                        if (data[x1][curY + y1] > 0. && dataNew[x1][curY + y1] == 0.){  // if pixel of line in next column is found ...
                    ...
                    if (!isFound){  // to prevent indexing lines with wrong index (e.g. if gap is too big and next found line would not be continuation of current line)
                        missingCounterTotal++;
                        missingCounter++;
                    } 
                    else {
...
// finding intersections
vector<CbmRichRonchiIntersectionData> CbmRichRonchiAna::DoIntersection(vector<vector<double> >& dataH, const vector<vector<double> >& dataV)
{
    int nX = dataV.size();
    int nY = dataV[0].size();
    ...
    for (int x = 0; x < nX; x++) {
        for (int y = 0; y < nY; y++) {
            if (dataH[x][y] > 0. && dataV[x][y] > 0.) { // filling vector with data if both vertical and horizontal image have data greater ZERO here
                CbmRichRonchiIntersectionData data;
                data.fPixelX = x;       
                data.fPixelY = y;
                data.fLineY = dataH[x][y];  // 'data[x][y]' contains the line index now, not intensity (see prev. function 'DoLineSearch')
    ...
    for (size_t i1 = 0; i1 < intersections.size(); i1++) {
        for (size_t i2 = i1 + 1; i2 < intersections.size(); i2++) {   
            double dX = intersections[i1].fPixelX - intersections[i2].fPixelX;  // distances in x and y of current observed intersections
...
//                        << intersections[i2].fPixelX <<  " " << intersections[i2].fPixelY << " " << dist << endl;
    ...
    for (it = removeSet.begin(); it != removeSet.end(); ++it) {
        swap(intersections[*it], intersections[intersections.size() - 1]);   // move one of both double counted intersections to the end of vector and remove these afterwards
...
vector<vector<double>> CbmRichRonchiAna::DoSuperpose(const vector<vector<double> >& dataH, const vector<vector<double> >& dataV)
{
    int nX = dataV.size();
    int nY = dataV[0].size();
...
void CbmRichRonchiAna::DoOrderLines(vector<CbmRichRonchiIntersectionData>& intersections, const string& option)
{
    ...
    for (auto const &curIntr : intersections){   
        int lineInd = (option == "x") ? curIntr.fLineX : curIntr.fLineY;    // fLineX/Y is the line index, set in 'DoLineSearch'
        linesMap[lineInd].fMeanPrimary += (option == "x") ? curIntr.fPixelX : curIntr.fPixelY;      // fPixelX/Y are the x/y-values, set in 'DoIntersection')
    ...
    for (auto &kv : linesMap) {     
        kv.second.fMeanPrimary = (double)kv.second.fMeanPrimary / kv.second.fNofPoints;     // calculating mean values of x and y positions to enable sorting
...
void CbmRichRonchiAna::UpdateIntersectionLineInd(vector<CbmRichRonchiIntersectionData> &intersections, CbmRichRonchiLineData *line1, 
...
void CbmRichRonchiAna::DoLocalNormal(vector<CbmRichRonchiIntersectionData> &data)
{
    ...
    for (size_t i = 0; i < data.size(); i++) {
        ...
        data[i].fRulingV.SetXYZ((data[i].fOrderedLineX - centerLineX) * fPitchGrid, (data[i].fOrderedLineY - centerLineY) * fPitchGrid, fDistRulingCCD);

        // slopes in X and Y direction of reflected beam
        double sRefX = -(data[i].fRulingV.X() - data[i].fCcdV.X()) / fDistRulingCCD; // 'minus' because Ruling is behind CoC
        ...
        double mirrorCenterDist = sqrt(pow(data[i].fMirrorV.X()+fOffsetCCDOptAxisX, 2) + pow(data[i].fMirrorV.Y()+fOffsetCCDOptAxisY, 2));   // distance from mirrors center in microns
        double sagitta = fRadiusMirror - sqrt(pow(fRadiusMirror, 2) - pow(mirrorCenterDist, 2));       // to calculate cathetus length
        double cathetus = fRadiusMirror - sagitta;   // if point source is in the center of curvature !!
        
        double angleIncX = atan(data[i].fMirrorV.X() / cathetus);  // if point source is in the center of curvature (CoC), otherwise add offset in Z direction !!
        double angleIncY = atan((data[i].fMirrorV.Y() - sqrt(fOffsetLEDOpticalAxisY * fOffsetLEDOpticalAxisY) - sqrt(fOffsetCCDOptAxisY * fOffsetCCDOptAxisY)) / cathetus);
...
void CbmRichRonchiAna::DoSphere(vector<CbmRichRonchiIntersectionData> &data)
{
    ...
    for (int iX = xMin; iX <= xMax; iX++) {
        int iPX = GetMinIndexForLineX(iX, data);    // 'GetMinIndex..' to get index of 'data' for current intersection
...
{
    for (size_t i = 0; i < data.size(); i++) {
        if (data[i].fOrderedLineX == lineX && data[i].fOrderedLineY == lineY) return i;     // returns 'data' index of currently investigated intersection
    }
    return -1;
}
...
void CbmRichRonchiAna::DoDeviation(vector<CbmRichRonchiIntersectionData> &data)
{
    ...
    TH2D* hMirrorHeight = new TH2D("hMirrorDeviation", "hMirrorDeviation;index X;index Y;Deviation [mum]", 60, -0.5, 59.5, 60, -0.5, 59.5);
...
void CbmRichRonchiAna::DrawXYMum(vector<CbmRichRonchiIntersectionData> &data)
{
    ...
    TH2D *hMirrorXY = new TH2D("hMirrorXY", "hMirrorXY;Mirror_X [mum];Mirror_Y [mum];", 1, -250000, 250000, 1, -250000, 250000);
...
void CbmRichRonchiAna::DrawXZProjection(vector<CbmRichRonchiIntersectionData> &data, int orderedLineY, double scale)
{
    ...
    TH2D *hZX = new TH2D(histName.c_str(), (histName+";Z [mum];X [mum];").c_str(), 100, -0.02* scale * fDistMirrorRuling, 1.05 * scale * fDistMirrorRuling, 100, 
            scale * -250000, scale * 250000);
    DrawH2(hZX);
    gPad->SetRightMargin(0.10);
    for (size_t i = 0; i < data.size(); i++){
        ...
        TLine *mirrorLine = new TLine(data[i].fRulingV.Z(), data[i].fRulingV.X(), data[i].fMirrorV.Z(), data[i].fMirrorV.X());
        mirrorLine->Draw();
...
void CbmRichRonchiAna::DrawMirrorSegments(vector<CbmRichRonchiIntersectionData> &data, int orderedLineX, int orderedLineY)
{
    ...
    TH2D *hZX = new TH2D(histNameZX.c_str(), (histNameZX+";Z [mum];X [mum];").c_str(), 100, fDistMirrorRuling - hSize, fDistMirrorRuling + hSize, 100, -1.*hSize, hSize);
    c->cd(2);
    DrawH2(hZX);
    gPad->SetRightMargin(0.10);
    for (size_t i = 0; i < data.size(); i++){
...
void CbmRichRonchiAna::DrawMirrorSegmentsSphere(vector<CbmRichRonchiIntersectionData> &data, int orderedLineX, int orderedLineY)
{
    ...
    TH2D *hZX = new TH2D(histNameZX.c_str(), (histNameZX + ";Z [mum];X(Y) [mum];").c_str(), 100, data[0].fBRSph.Z() - 0.25*hSize, data[0].fBRSph.Z() + hSize, 
                        100, -1. * 250000, 250000);
    c->cd(2);
    DrawH2(hZX);
    gPad->SetRightMargin(0.10);
...
void CbmRichRonchiAna::DrawOneMirrorSegment(const TVector3& tl, const TVector3& tr, const TVector3& bl, const TVector3& br, int color)
{
