...
void CbmRichGeoTest::ProcessMc()
{
    ...
    for (Int_t iE = 0; iE < nofEvents; iE++) {
        ...
        for(Int_t iT = 0; iT < nofMcTracks; iT++){
            ...
            Bool_t isMcPrimaryElectron = (pdg == 11 && motherId == -1) || (mcTrack->GetGeantProcessId() == kPPrimary && pdg == 11);
...
void CbmRichGeoTest::DrawHist()
{
    ...
    
    {
		...
		c->Divide(3,1);
		c->cd(1);
		DrawH1( {(TH1D*)fHM->H1("fhNofHitsAll")->Clone(), (TH1D*)fHM->H1("fhNofHitsCircleFit")->Clone(), (TH1D*)fHM->H1("fhNofHitsEllipseFit")->Clone()},
	...
	TH1D* pxPiEff = Cbm::DivideH1((TH1D*)fHM->H1("fhAccMomPi")->Clone(), (TH1D*)fHM->H1("fhMcMomPi")->Clone(), "", 100., "Geometrical acceptance [%]");
...

    {
        ...
        c->Divide(2, 1);
        c->cd(1);
        DrawH1({fHM->H1("fhPhotonEPlaneZ+"), fHM->H1("fhPhotonEPlaneZ-"), fHM->H1("fhPhotonEPmtPoint"), fHM->H1("fhPhotonEPmtHit")},
            {"Sens plane Z+", "Sens plane Z-", "PMT point", "PMT hit"});
        c->cd(2);
        DrawH1({fHM->H1("fhLambdaPlaneZ+"), fHM->H1("fhLambdaPlaneZ-"), fHM->H1("fhLambdaPmtPoint"), fHM->H1("fhLambdaPmtHit")},
            {"Sens plane Z+", "Sens plane Z-", "PMT Point", "PMT hit"});
    }
}
