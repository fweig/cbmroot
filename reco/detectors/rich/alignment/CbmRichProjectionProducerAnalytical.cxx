														   ...
														   const string volumeName)
{
	...
	for (std::map<string, std::pair<Double_t, Double_t> >::iterator it=mirrorCorrectionParamMap.begin(); it!=mirrorCorrectionParamMap.end(); ++it) {
		if ( it->first == mirrorID ) {
			cout << "Map's first key: " << it->first << " and pair: " << it->second.first << ", " << it->second.second << endl;
...
string CbmRichProjectionProducerAnalytical::GetMirrorID(
				 	 	 	 	 	 	 	 	 	 	const string volumeName)
{
