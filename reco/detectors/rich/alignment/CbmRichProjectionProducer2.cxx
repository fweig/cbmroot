...
Double_t*
CbmRichProjectionProducer2::ProjectionProducer(FairTrackParam* trackParam) {
  ...
  vector<Double_t> ptM(3), ptC(3), ptCNew(3), momR1(3), ptR1(3), normalPMT(3),
    ptR2Mirr(3), ptR2Center(3), ptPMirr(3), ptPR2(3), ptTileCenter(3);
  ...
  TString mirrorIntersection1 = CbmRichNavigationUtil2::FindIntersection(
    trackParam, mirrorPoint, "mirror_tile_type", navi);
  ptM.at(0) = mirrorPoint.x();
  ptM.at(1) = mirrorPoint.y();
  ptM.at(2) = mirrorPoint.z();
  cout << "mirrorIntersection1: " << mirrorIntersection1 << endl;
  cout << "Mirror point coordinates = {" << mirrorPoint.x() << ", "
       << mirrorPoint.y() << ", " << mirrorPoint.z() << "}" << endl;
  TString mirrorIntersection =
  ...
  if (mirrorIntersection1) {
    ...
    cout << "Sphere center coordinates of the aligned mirror tile, ideal = {"
    ...
    cout << "FairTrackParam Point coordinates = {" << ptR1.at(0) << ", "
         << ptR1.at(1) << ", " << ptR1.at(2) << "}" << endl;
    cout << "And FairTrackParam Point direction cosines = {" << ptR1.at(3)
         << ", " << ptR1.at(4) << ", " << ptR1.at(5) << "}" << endl;
    cout << "Mirror Point coordinates = {" << ptM.at(0) << ", " << ptM.at(1)
         << ", " << ptM.at(2) << "}" << endl;

    ComputeR2(
      ptR2CenterUnCorr, ptR2MirrUnCorr, ptM, ptC, ptR1, navi, "Uncorrected");
    ComputeR2(ptR2Center, ptR2Mirr, ptM, ptC, ptR1, navi, "Corrected");
    ComputeR2(
      ptR2CenterIdeal, ptR2MirrIdeal, ptM, ptCIdeal, ptR1, navi, "Uncorrected");

    ComputeP(
      ptPMirrUnCorr, ptPR2UnCorr, normalPMT, ptM, ptR2MirrUnCorr, constantePMT);
    ComputeP(ptPMirr, ptPR2, normalPMT, ptM, ptR2Mirr, constantePMT);
    ComputeP(
      ptPMirrIdeal, ptPR2Ideal, normalPMT, ptM, ptR2MirrIdeal, constantePMT);
    ...
    cout << "New mirror points coordinates = {" << outPosUnCorr.x() << ", "
         << outPosUnCorr.y() << ", " << outPosUnCorr.z() << "}" << endl;
    ...
    cout << endl
         ...
         << outPosIdeal.y() << ", " << outPosIdeal.z() << "}" << endl
         << endl;
  } else {
 ...
 */
  for (Int_t iPmt = 0; iPmt < NofPMTPoints; iPmt++) {
    ...
    cout << "a[0] = " << a[0] << ", a[1] = " << a[1] << " et a[2] = " << a[2]
         << endl;
    break;
  }
  for (Int_t iPmt = 0; iPmt < NofPMTPoints; iPmt++) {
    ...
    if (TMath::Sqrt(TMath::Power(a[0] - pmtPoint->GetX(), 2)
      ...
      cout << "b[0] = " << b[0] << ", b[1] = " << b[1] << " et b[2] = " << b[2]
           << endl;
      break;
    }
  }
