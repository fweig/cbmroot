...
void CbmRichMirrorSortingAlignment::Exec(Option_t* Option) {
  ...
  if (fRichRings->GetEntriesFast() != 0) {
    ...
    for (Int_t iGlobalTrack = 0; iGlobalTrack < fGlobalTracks->GetEntriesFast();
         iGlobalTrack++) {
      CbmRichMirror* mirrorObject =
        new CbmRichMirror();  // Create CbmRichMirror object, to be later filled.
                              // ----- PART 1 ----- //
      ...
      fCopFit->DoFit(&ringL);
      //fTauFit->DoFit(&ringL);
      mirrorObject->setRingLight(
        ringL);  // Fill Cbm Rich Ring Light inside mirrorObject.
      cout << "ring Center Coo: " << ringL.GetCenterX() << ", "
           << ringL.GetCenterY() << endl;
      mcTrack->GetMomentum(momentum);
      mirrorObject->setMomentum(
        momentum);  // Fill track momentum inside mirrorObject.
      ...
      if (trackMotherId == -1) {
        if (fMirrorPoints->GetEntriesFast() > 0) {
          ...
          cout << "mirrPoint: {" << mirrPoint->GetX() << ", "
               << mirrPoint->GetY() << ", " << mirrPoint->GetZ() << "}" << endl;

          mirrNode = gGeoManager->FindNode(ptM.at(0), ptM.at(1), ptM.at(2));
          cout << "Mirror node name: " << mirrNode->GetName()
               << " and full path " << gGeoManager->GetPath() << endl;
          string str1 = gGeoManager->GetPath(), str2 = "mirror_tile_",
                 str3 = "";
          cout << "str1 before CbmRichNavigationUtil::FindIntersection: "
               << str1 << endl;
          TVector3 crossP;
          str1 = CbmRichNavigationUtil::FindIntersection(
            point, crossP, "mirror_tile_");
          cout << "str1 after CbmRichNavigationUtil::FindIntersection: " << str1
               << endl;
      ...
      cout << "Key str: " << mirrorObject->getMirrorId() << endl
           << "Mirror map: " << fMirrorMap[mirrorObject->getMirrorId()].size()
           << endl
           << endl;
      //mirrNode->Clear();
      //gGeoManager->Clear();
    ...
    if (curMirrorId != "" && it->second.size() > fThreshold) {
      ...
      for (int i = 0; i < it->second.size(); i++) {
        ...
        for (Int_t iH = 0; iH < ringL.GetNofHits(); iH++) {
          ...
          phi = TMath::ATan2(hit.fY - ringL.GetCenterY(),
                             hit.fX - ringL.GetCenterX());

          // ----- Theta_Ch and Theta_0 calculations ----- //
          thetaCh = sqrt(TMath::Power(projHit[0] - hit.fX, 2)
                         + TMath::Power(projHit[1] - hit.fY, 2));
          theta0  = sqrt(TMath::Power(ringL.GetCenterX() - hit.fX, 2)
                        + TMath::Power(ringL.GetCenterY() - hit.fY, 2));
...
void CbmRichMirrorSortingAlignment::DrawFitAndExtractAngles(
  ...
  std::map<string, TH2D*> histoMap) {
  Int_t thresh = 3;
  Double_t p1 = 0, p2 = 0, p3 = 0, chi2 = 0, focalLength = 150., q = 0., A = 0.,
           alpha = 0., mis_x = 0., mis_y = 0.;
  // mis_x && mis_y corresponds respect. to rotation angles around the Y and X axes.
  // !!! BEWARE: AXES INDEXES ARE SWITCHED !!!

  for (std::map<string, TH2D*>::iterator it = histoMap.begin();
       it != histoMap.end();
       ++it) {
    if (it->first != "") {
      ...
      p1       = fit->GetParameter("Delta_phi"),
      p2 = fit->GetParameter("Delta_lambda"), p3 = fit->GetParameter("Offset"),
      ...
      mis_x = TMath::ATan(fit->GetParameter(1) / focalLength) * 0.5
              * TMath::Power(10, 3);
      mis_y = TMath::ATan(fit->GetParameter(0) / focalLength) * 0.5
              * TMath::Power(10, 3);
      cout << "Horizontal displacement = " << mis_x
           << " [mrad] and vertical displacement = " << mis_y << " [mrad]."
           << endl
           << endl;
      ...
      Cbm::SaveCanvasAsImage(
        can, string(fOutputDir.Data() + fStudyName), "png");
...
void CbmRichMirrorSortingAlignment::Finish() {
  ...
  TString str_correction =
    fOutputDir + "/correction_table/correction_param_array.txt";
  ...
  for (std::map<string, vector<Double_t>>::iterator it = anglesMap.begin();
       it != anglesMap.end();
       ++it) {
    string mirrorId = it->first;
    cout << "curMirrorId: " << mirrorId << endl;
    vector<Double_t> misAngles = it->second;
    cout << "mirror correction parameters infos: {" << misAngles[0] << ", "
         << misAngles[1] << "}" << endl;
  ...
  TString str_angles =
    fOutputDir + "/correction_table/reconstructed_angles_array.txt";
  ofstream anglesFile;
  ...
  for (std::map<string, vector<Double_t>>::iterator it = anglesMap.begin();
    ...
    cout << "curMirrorId: " << mirrorId << endl;
    vector<Double_t> misAngles = it->second;
    cout << "mirror reconstructed angles infos: {" << misAngles[2] << ", "
         << misAngles[3] << "}" << endl;
