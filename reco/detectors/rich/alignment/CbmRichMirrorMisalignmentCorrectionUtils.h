...
//	   virtual void Init() { }
	   ...
	   void Init(const string &s) {
		   ...
		   else { LOG(info) << "CbmRichProjectionProducerAnalytical::Init() No correction table."; }
	   }
	   ...
	   void ReadAndStoreMirrorCorrectionParamMap(std::map<string, std::pair<Double_t, Double_t> > &mirrorCorrectionParamMap) {
			...
			if (corrFile.is_open())
			{
				LOG(info) << "CbmRichProjectionProducerUtils::CheckMirrorCorrectionParameterFile() Correction table used at: " << fPathToMirrorMisalignmentCorrectionParameterFile;
