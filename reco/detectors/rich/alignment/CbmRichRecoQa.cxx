...
void CbmRichRecoQa::InitHistograms()
{
    ...
    for (Int_t i = 0; i < 4; i++){
		...
		fHM->Create3<TH3D>("fhRingTrackDistVsXYTruematch"+s, "fhRingTrackDistVsXYTruematch"+s+";X [cm];Y [cm];Ring-track distance [cm]", 240, xMin, xMax, 416, -200., 200., 400, 0., 5.);
		...
		fHM->Create3<TH3D>("fhRingTrackDistDiffXRingVsXYTruematchUp"+s, "fhRingTrackDistDiffXRingVsXYTruematchUp"+s+";X [cm];Y [cm];X Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, 110, 200, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffYRingVsXYTruematchUp"+s, "fhRingTrackDistDiffYRingVsXYTruematchUp"+s+";X [cm];Y [cm];Y Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, 110, 200, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffXTrackVsXYTruematchUp"+s, "fhRingTrackDistDiffXTrackVsXYTruematchUp"+s+";X [cm];Y [cm];X Track-ring distance [cm]", nbBinsX, xMin, xMax, nbBinsY, 110, 200, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffYTrackVsXYTruematchUp"+s, "fhRingTrackDistDiffYTrackVsXYTruematchUp"+s+";X [cm];Y [cm];Y Track-ring distance [cm]", nbBinsX, xMin, xMax, nbBinsY, 110, 200, 200, -range, range);

		fHM->Create3<TH3D>("fhRingTrackDistDiffXRingVsXYTruematchDown"+s, "fhRingTrackDistDiffXRingVsXYTruematchDown"+s+";X [cm];Y [cm];X Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, -200, -110, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffYRingVsXYTruematchDown"+s, "fhRingTrackDistDiffYRingVsXYTruematchDown"+s+";X [cm];Y [cm];Y Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, -200, -110, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffXTrackVsXYTruematchDown"+s, "fhRingTrackDistDiffXTrackVsXYTruematchDown"+s+";X [cm];Y [cm];X Track-ring distance [cm]", nbBinsX, xMin, xMax, nbBinsY, -200, -110, 200, -range, range);
		fHM->Create3<TH3D>("fhRingTrackDistDiffYTrackVsXYTruematchDown"+s, "fhRingTrackDistDiffYTrackVsXYTruematchDown"+s+";X [cm];Y [cm];Y Track-ring distance [cm]", nbBinsX, xMin, xMax, nbBinsY, -200, -110, 200, -range, range);

		fHM->Create3<TH3D>("fhRingTrackDistVsXYHalfUpTruematch"+s, "fhRingTrackDistVsXYHalfUpTruematch"+s+";X [cm];Y [cm];Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, 110., 200., 100, 0., range);
		fHM->Create3<TH3D>("fhRingTrackDistVsXYHalfDownTruematch"+s, "fhRingTrackDistVsXYHalfDownTruematch"+s+";X [cm];Y [cm];Ring-track distance [cm]", nbBinsX, xMin, xMax, nbBinsY, -200., -110., 100, 0., range);
    }
	...
	fHM->Create2<TH2D>("fhRingTrackDistVsMomTruematchElId", "fhRingTrackDistVsMomTruematchElId;P [GeV/c];Ring-track distance [cm];Yield (a.u.)", 40, 0., 10., 120, 0., 3.);
	fHM->Create3<TH3D>("fhRingTrackDistDiffXAfterEl", "fhRingTrackDistDiffXAfterEl;X [cm];Y [cm];X Ring-track distance [cm]", 60, xMin, xMax, 104, 110, 200, 200, -range_2, range_2);
	fHM->Create3<TH3D>("fhRingTrackDistDiffYAfterEl", "fhRingTrackDistDiffYAfterEl;X [cm];Y [cm];Y Ring-track distance [cm]", 60, xMin, xMax, 104, 110, 200, 200, -range_2, range_2);
...
void CbmRichRecoQa::DrawHist()
{
    ...
    {
		TCanvas* c = fHM->CreateCanvas("fh_ring_track_distance_truematch_elid", "fh_ring_track_distance_truematch_elid", 1400, 1400);
		c->Divide(2,2);
		c->cd(1);
...
//						kLinear, kLog, true, 0.45, 0.2, 0.9, 0.35);
		DrawH1({py}, {string("before ElId (" + this->GetMeanRmsOverflowString(py) + ")")}, kLinear, kLog, false, 0.45, 0.2, 0.9, 0.35);
		c->cd(4);
		DrawH1({pyElId}, {string("after ElId (" + this->GetMeanRmsOverflowString(pyElId) + ")")}, kLinear, kLog, false, 0.45, 0.2, 0.9, 0.35);
	}

    Double_t range = 2.5;
    {
    	TCanvas* c = fHM->CreateCanvas("fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_after_el", "fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_after_el", 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
...
{
    {
    	TCanvas* c = fHM->CreateCanvas("fh_ring_track_distance_vs_xy_truematch_half_"+s, "fh_ring_track_distance_vs_xy_truematch_half_"+s, 1800, 600);
    	c->Divide(2, 1);
    ...
    {
    	TCanvas* c = fHM->CreateCanvas("fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_projection_"+s, "fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_projection_"+s, 1800, 600);
    	c->Divide(2, 1);
    	TH3D *h1 = (TH3D*) fHM->H3("fhRingTrackDistVsXYTruematch"+s)->Clone();
    	c->cd(1);
    ...
    Double_t range = 1.;
    {
    	TCanvas* c = fHM->CreateCanvas("fh_ring_track_distance_diff_x_y_ring_vs_xy_truematch_up_"+s, "fh_ring_track_distance_diff_x_y_ring_vs_xy_truematch_up_"+s, 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
    ...
    {
    	TCanvas* c = fHM->CreateCanvas("fh_ring_track_distance_diff_x_y_ring_vs_xy_truematch_down_"+s, "fh_ring_track_distance_diff_x_y_ring_vs_xy_truematch_down_"+s, 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
...
*/
    {
    	TCanvas* c = fHM->CreateCanvas("fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_up_"+s, "fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_up_"+s, 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
    ...
    {
    	TCanvas* c = fHM->CreateCanvas("fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_down_"+s, "fh_track_ring_distance_diff_x_y_track_vs_xy_truematch_down_"+s, 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
    ...
    {
    	TCanvas* c = fHM->CreateCanvas("fh_track_ring_distance_vs_xy_truematch_halves_"+s, "fh_track_ring_distance_vs_xy_truematch_halves_"+s, 1800, 600);
    	c->Divide(2,1);
    	c->cd(1);
