...
Int_t CbmTrdModuleRecR::FindClusters()
{
  ...
  deque<tuple<Int_t, Bool_t, const CbmTrdDigi*>>::                   iterator mainit; //subiterator for the deques in each module; searches for main-trigger to then add the neighbors
  deque<tuple<Int_t, Bool_t, const CbmTrdDigi*>>::                   iterator FNit; //last iterator to find the FN digis which correspond to the main trigger or the adjacent main triggers
  deque<tuple<Int_t, Bool_t, const CbmTrdDigi*>>::                   iterator start; //marker to erase already processed entries from the map to reduce the complexity of the algorithm
  deque<tuple<Int_t, Bool_t, const CbmTrdDigi*>>::                   iterator stop; //marker to erase already processed entries from the map to reduce the complexity of the algorithm
...
//      Int_t digiId =           get<0>(*mainit);
  ...
  for (mainit=fDigiMap.begin() ; mainit != fDigiMap.end(); mainit++) {
      ...
      Double_t CoGbot=0.;
      Double_t CoGrow=0.; 
      tuple<const CbmTrdDigi*,const CbmTrdDigi*,const CbmTrdDigi*> topdigi;// used to store the necassary digis for the CoG calculation without the need to revisit those digis
...
void CbmTrdModuleRecR::addClusters(deque<std::pair<Int_t,  const CbmTrdDigi*>> cluster)
{
  ...
  for (std::deque<std::pair<Int_t, const CbmTrdDigi*>>::iterator iDigi = cluster.begin(); iDigi != cluster.end(); iDigi++) {
...
CbmTrdHit* CbmTrdModuleRecR::MakeHit(Int_t clusterId, const CbmTrdCluster* /*cluster*/, std::vector<const CbmTrdDigi*> *digis)
{
  ...
  if(EB){
    if(errorclass == 0)       xVar = 0.0258725;
    else if(errorclass == 1)  xVar = 0.0267693;  
    else if(errorclass == 2)  xVar = 0.0344325;
    else if(errorclass == 3)  xVar = 0.0260322;
    else if(errorclass == 4)  xVar = 0.040115;  

    if(errorclass == 0)       yVar = 0.024549;
    else if(errorclass == 1)  yVar = 0.025957;  
    else if(errorclass == 2)  yVar = 0.0250713;
    else if(errorclass == 3)  yVar = 0.0302682;
    else if(errorclass == 4)  yVar = 0.0291146;  
  }
  else{
    if(EBP) time -= 50; //due to the event time of 0 in the EB mode and the ULong in the the digi time
    if(errorclass == 0)       xVar = 0.0426313;
    else if(errorclass == 1)  xVar = 0.0426206;  
    else if(errorclass == 2)  xVar = 0.0636962;
    else if(errorclass == 3)  xVar = 0.038981;
    else if(errorclass == 4)  xVar = 0.0723851;  

    if(errorclass == 0)       yVar = 0.0401438;
    else if(errorclass == 1)  yVar = 0.0407502;  
    else if(errorclass == 2)  yVar = 0.0397242;
    else if(errorclass == 3)  yVar = 0.0519485;
    else if(errorclass == 4)  yVar = 0.0504586;  
  }
  ...
  if(!EB){//preliminary correction for angle dependence in the position reconsutrction
  ...
  return new ((*fHits)[nofHits]) CbmTrdHit(fModAddress, global, cluster_pad_dposV, 0, clusterId,totalCharge/1e6,time,Double_t(CbmTrdDigi::Clk(CbmTrdDigi::kSPADIC)));
