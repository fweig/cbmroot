...
Bool_t CbmTrdSetTracksPidLike::ReadData()
{
  ...
  if(fMCinput) { /// mc pid method
    if(fMomDep){
      std::vector<TString> histnames{"MC_electron_p_eloss","MC_pion_p_eloss","MC_kaon_p_eloss","MC_proton_p_eloss","MC_muon_p_eloss"};
	...
	for(Int_t x=1;x<=h[i]->GetNbinsX();x++){
	  Double_t sum=0.;
	  for(Int_t y=1;y<=h[i]->GetNbinsY();y++)        sum += h[i]->GetBinContent(x,y);
	  for(Int_t y=1;y<=h[i]->GetNbinsY();y++)        if(sum > 0)   h[i]->SetBinContent(x,y,h[i]->GetBinContent(x,y)/sum   );
	}
    ...
    if(!fMomDep){
      std::vector<TString> histnames{"MC_electron_eloss","MC_pion_eloss","MC_kaon_eloss","MC_proton_eloss","MC_muon_eloss"};
      inArr = new TObjArray(histnames.size());
      for(size_t i =0; i < histnames.size() ; i++)  {
	h[i] = (TH2D*) histFile->Get(histnames[i]);
	if(!h[i]) continue;
    ...
    if(fMomDep){
      std::vector<TString> histnames{"ELE_electron_p_eloss","PIO_pion_p_eloss","ELE_kaon_p_eloss","ELE_proton_p_eloss","ELE_muon_p_eloss"};
      inArr = new TObjArray(histnames.size());
      for(size_t i =0; i < histnames.size() ; i++)  {
	h[i] = (TH2D*) histFile->Get(histnames[i]);
	h[i]->SetNameTitle(histnames[i],histnames[i]);
    ...
    if(!fMomDep){
      std::vector<TString> histnames{"ELE_electron_eloss","PIO_pion_eloss","ELE_kaon_eloss","ELE_proton_eloss","ELE_muon_eloss"};
      inArr = new TObjArray(histnames.size());
      for(size_t i =0; i < histnames.size() ; i++)  {
	h[i] = (TH2D*) histFile->Get(histnames[i]);
	if(!h[i]) continue;
  ...
  for(Int_t i=0; i<inArr->GetEntriesFast(); i++) {
    ...
    else if(name.Contains("pion"))                            particle = CbmTrdSetTracksPidLike::kPion;
