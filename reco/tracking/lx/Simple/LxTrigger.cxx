...
bool LxFinder::Trigger() {
  ...
  for (list<LxTrack*>::iterator i = caSpace.tracks.begin();
    ...
    for (; j != caSpace.tracks.end(); ++j) {
      ...
      scaltype firstParticleCharge =
        firstRay->tx - firstPoint->x / firstPoint->z;
      scaltype secondParticleCharge =
        secondRay->tx - secondPoint->x / secondPoint->z;
  ...
  for (list<LxTrack*>::iterator i = caSpace.tracks.begin();
    ...
    string particleName(
      mcTrack ? TDatabasePDG::Instance()->GetParticle(mcTrack->pdg)->GetName()
              : "ghost");

    if (!particleName.empty()
        && (particleName.back() == '+' || particleName.back() == '-'))
      particleName.pop_back();

    map<string, unsigned int>::iterator j =
      particlesCountAll.find(particleName);
    ...
    triggeringAllTracksVertices->Fill(
      mcTrack->z);  // Save arbitrary track info.

    if (recoTrack->distanceOk)
      triggeringDistTracksVertices->Fill(
        mcTrack->z);  // Save track fulfilling distance criterion info.

    if (recoTrack->oppCharged)
      triggeringSignTracksVertices->Fill(
        mcTrack->z);  // Save track fulfilling signs criterion info.

    if (recoTrack->triggering)
      triggeringTrigTracksVertices->Fill(
        mcTrack
          ->z);  // Save track fulfilling the both criteria above (triggering track) info.
  }
