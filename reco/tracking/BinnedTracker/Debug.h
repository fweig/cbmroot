...
struct CbmBinnedDebug {
  ...
  CbmBinnedDebug()
    ...
    fTrdClusterArray =
      static_cast<TClonesArray*>(fIoman->GetObject("TrdCluster"));
    fTrdDigiMatchArray =
      static_cast<TClonesArray*>(fIoman->GetObject("TrdDigiMatch"));
  ...
  void Exec() {
    ...
    for (Int_t i = 0; i < nofStsHits; ++i) {
      const CbmStsHit* stsHit =
        static_cast<const CbmStsHit*>(fStsHitArray->At(i));
      Int_t stationNumber =
      ...
      const CbmStsCluster* frontCluster = static_cast<const CbmStsCluster*>(
        fStsClusterArray->At(frontClusterInd));
      ...
      for (Int_t j = 0; j < nofFrontDigis; ++j) {
        ...
        const CbmStsDigi* stsDigi =
          static_cast<const CbmStsDigi*>(fStsDigiArray->At(stsDigiInd));
        ...
        for (Int_t k = 0; k < nofLinks; ++k) {
          ...
          const CbmStsPoint* stsPoint = static_cast<const CbmStsPoint*>(
            fStsPointArray->Get(0, eventId, mcPointId));
      ...
      const CbmStsCluster* backCluster =
        static_cast<const CbmStsCluster*>(fStsClusterArray->At(backClusterInd));
      ...
      for (Int_t j = 0; j < nofDigis; ++j) {
        ...
        for (Int_t k = 0; k < nofLinks; ++k) {
          ...
          const CbmTrdPoint* trdPoint = static_cast<const CbmTrdPoint*>(
            fTrdPointArray->Get(0, eventId, mcPointId));
    ...
    for (int i = 0; i < nofMCTracks; ++i) {
      ...
      const CbmMCTrack* track =
        static_cast<const CbmMCTrack*>(fMCTrackArray->Get(0, fEventNumber, i));
  ...
  bool TrackHasHitSts(int stNo, Int_t trackInd, Int_t hitInd) {
    return fMCTracks[trackInd].stsHits[stNo].find(hitInd)
           != fMCTracks[trackInd].stsHits[stNo].end();
  }

  bool TrackHasHitTrd(int stNo, Int_t trackInd, Int_t hitInd) {
    return fMCTracks[trackInd].trdHits[stNo].find(hitInd)
