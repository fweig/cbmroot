...
CbmBinnedGeoReader::CbmBinnedGeoReader(FairRootManager*, CbmBinnedTracker* tracker) : fNavigator(0),
...
void CbmBinnedGeoReader::SearchStation(ECbmModuleId stationType, TGeoNode* node, CbmBinnedHitReader* hitReader, list<const char*>::const_iterator stationPath,
   list<const char*>::const_iterator stationPathEnd, const std::list<const char*>& geoPath, bool is4d)
{
   if (stationPath == stationPathEnd)
   {
      ...
      if (is4d)
      {
         CbmBinned4DStation* station4d = new CbmBinned4DStation(stationType, front, back, gNofZbins, gNofYbins, gNofXbins, gNofTbins);
         station = station4d;
      }
      else
      {
...
void CbmBinnedGeoReader::ReadSts()
{
   ...
   SearchStation(ECbmModuleId::kSts, gGeoManager->GetCurrentNode(), CbmBinnedHitReader::Instance(readerName), stationPath.begin(), stationPath.end(), geoPath);
...
void CbmBinnedGeoReader::ReadRich()
{
   ...
   SearchStation(ECbmModuleId::kRich, gGeoManager->GetCurrentNode(), CbmBinnedHitReader::Instance(readerName), stationPath.begin(), stationPath.end(), geoPath);
}
...
void CbmBinnedGeoReader::ReadMuch()
{
   ...
   SearchStation(ECbmModuleId::kMuch, gGeoManager->GetCurrentNode(), CbmBinnedHitReader::Instance(readerName), stationPath.begin(), stationPath.end(), geoPath);
...
void CbmBinnedGeoReader::ReadTrd()
{
   ...
   SearchStation(ECbmModuleId::kTrd, gGeoManager->GetCurrentNode(), CbmBinnedHitReader::Instance(readerName), stationPath.begin(), stationPath.end(), geoPath);
...
void CbmBinnedGeoReader::ReadTof()
{
   ...
   SearchStation(ECbmModuleId::kTof, gGeoManager->GetCurrentNode(), CbmBinnedHitReader::Instance(readerName), stationPath.begin(), stationPath.end(), geoPath, true);
}
