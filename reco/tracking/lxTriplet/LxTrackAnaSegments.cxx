...
LxTrackAnaSegments::LxTrackAnaSegments(LxTrackAnaTriplet& o)
  : owner(o), stationsInAlgo(LXSTATIONS), useBgr(false) {}
...
void LxTrackAnaSegments::Init() {
  ...
  for (Int_t i = 0; i < LXSTATIONS; ++i) {
    ...
    if (i > 0) {
      ...
      sprintf(
        title, "Tx break between triplets on stations %d and %d", i - 1, i);
      ...
      sprintf(
        title, "Ty break between triplets on stations %d and %d", i - 1, i);
...
static bool GetPoints2(LxSimpleTrack* track,
  ...
  for (; i >= 0; --i) {
    for (; j >= 0; --j) {
      if (!track->muchPoints[i][j].empty())
        points[i][j].insert(points[i][j].end(),
                            track->muchPoints[i][j].begin(),
                            track->muchPoints[i][j].end());
      else {
        if (0 != track->parent)
...
void LxTrackAnaSegments::BuildStatistics() {
  ...
  for (vector<LxSimpleTrack*>::iterator i = tracks.begin(); i != tracks.end();
    ...
    for (Int_t j = 0; j < stationsInAlgo; ++j) {
      ...
      for (list<LxSimplePoint>::iterator k = points[j][1].begin();
           k != points[j][1].end();
           ++k) {
        LxSimplePoint p1 = *k;
        ...
        for (list<LxSimplePoint>::iterator l = points[j][2].begin();
             l != points[j][2].end();
             ++l) {
          LxSimplePoint p2 = *l;
          ...
          if (abs(rX - p2.x) <= cutCoeff * xDispRight[j]
              && abs(rY - p2.y) <= cutCoeff * yDispRight[j]) {
            angleOk = true;
            break;
          }
        }  // l
    ...
    for (list<LxSimplePoint>::iterator j = points[0][1].begin();
         j != points[0][1].end();
         ++j) {
      LxSimplePoint p01 = *j;
      Double_t txEst    = p01.x / p01.z;

      for (list<LxSimplePoint>::iterator k = points[1][1].begin();
           k != points[1][1].end();
           ++k) {
        LxSimplePoint p11 = *k;
        ...
        for (list<LxSimplePoint>::iterator l = points[0][0].begin();
             l != points[0][0].end();
             ++l) {
          LxSimplePoint p00 = *l;

          for (list<LxSimplePoint>::iterator m = points[0][2].begin();
               m != points[0][2].end();
               ++m) {
            LxSimplePoint p02 = *m;
  ...
  cout << "LxTrackAnaSegments::BuildStatistics(): triggered: " << triggerEvents
       << " times" << endl;
}
...
void LxTrackAnaSegments::StatForTrack(LxSimpleTrack* track) {
  ...
  for (Int_t i = 0; i < LXSTATIONS; ++i) {
    ...
    if (i > 0) {
      ...
      muchOutStationXDispByVertex[i - 1]->Fill(
        p1.x - p2.x + txVertex * deltaZ /* / scatCoeff*/);
      muchOutStationYDispByVertex[i - 1]->Fill(
        p1.y - p2.y + tyVertex * deltaZ /* / scatCoeff*/);
