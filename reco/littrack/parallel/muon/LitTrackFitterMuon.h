 ...
 */
    template<class T>
    inline void LitTrackFitterMuon(LitTrack<T>& track,
      ...
      for (unsigned char isg = 0; isg < layout.GetNofStationGroups(); isg++) {
        ...
        field.Set(
          v1, absorber.fieldSliceFront.Z, v2, absorber.fieldSliceBack.Z);
        LitRK4Extrapolation(par, absorber.Z, field);
        LitAddMaterial(par, absorber.material);

        //Approximate the field between the absorbers
        const LitSubstationMuon<T>& ss1 =
          stationGroup.stations[0].substations[0];
        const LitSubstationMuon<T>& ss2 =
          stationGroup.stations[1].substations[0];
        ...
        for (unsigned char ist = 0; ist < stationGroup.GetNofStations();
             ist++) {
          ...
          for (unsigned char iss = 0; iss < station.GetNofSubstations();
               iss++) {
