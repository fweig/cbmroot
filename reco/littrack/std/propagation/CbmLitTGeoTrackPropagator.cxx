...
CbmLitTGeoTrackPropagator::CbmLitTGeoTrackPropagator(TrackExtrapolatorPtr extrapolator) : fExtrapolator(extrapolator)
{
...
LitStatus CbmLitTGeoTrackPropagator::Propagate(const CbmLitTrackParam* parIn, CbmLitTrackParam* parOut, litfloat zOut,
                                               int pdg, std::vector<litfloat>* F, litfloat* length)
{
  *parOut = *parIn;
  return Propagate(parOut, zOut, pdg, F, length);
}

LitStatus CbmLitTGeoTrackPropagator::Propagate(CbmLitTrackParam* par, litfloat zOut, int pdg, std::vector<litfloat>* F,
                                               litfloat* length)
...
void CbmLitTGeoTrackPropagator::UpdateF(std::vector<litfloat>& F, const std::vector<litfloat>& newF)
{
...
bool CbmLitTGeoTrackPropagator::IsParCorrect(const CbmLitTrackParam* par)
{
  ...
  if (std::abs(par->GetTx()) > maxSlope || std::abs(par->GetTy()) > maxSlope || std::abs(par->GetTx()) < minSlope
      || std::abs(par->GetTy()) < minSlope || std::abs(par->GetQp()) > maxQp) {
    return false;
  }

  if (std::isnan(par->GetX()) || std::isnan(par->GetY()) || std::isnan(par->GetTx()) || std::isnan(par->GetTy())
      || std::isnan(par->GetQp())) {
    return false;
  }
