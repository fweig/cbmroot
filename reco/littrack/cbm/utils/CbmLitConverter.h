...
public:
                                            ...
                                            CbmLitPixelHit* litHit)
    {
        ...
        litHit->SetT(hit->GetTime());  ///correction removed from here. The time for CbmMuchPixelHit is corrected now in CbmMuchFindHitsGem class
                                                 ...
                                                 TClonesArray* tofTracks)
    {
        ...
        for (Int_t i = 0; i < nofStsTracks; i++) {
            ...
            if (event)
                event->AddData(ECbmDataType::kGlobalTrack, iTrack);
        }
        ...
        for (size_t iTrack = 0; iTrack < litTracks.size(); iTrack++) {
            ...
            if (isCreateTrdTrack) {
                ...
                if (event)
                    event->AddData(ECbmDataType::kTrdTrack, trdTrackNo);
            ...
            if (isCreateMuchTrack) {
                ...
                if (event)
                    event->AddData(ECbmDataType::kMuchTrack, muchTrackNo);
        ...
        for (size_t iTrack = 0; iTrack < litTofTracks.size(); iTrack++) {
            ...
            if (event)
                event->AddData(ECbmDataType::kTofHit, tofHitId);
        }
    }
