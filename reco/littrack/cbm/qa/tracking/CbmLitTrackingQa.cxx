...
//   //
   ...
   if (fDet.GetDet(ECbmModuleId::kSts)) CreateH1("hng_NofGhosts_Sts_Nh", "Number of hits", "Yield", nofBinsPoints, minNofPoints, maxNofPoints);
   if (fDet.GetDet(ECbmModuleId::kTrd)) CreateH1("hng_NofGhosts_Trd_Nh", "Number of hits", "Yield", nofBinsPoints, minNofPoints, maxNofPoints);
   if (fDet.GetDet(ECbmModuleId::kMuch)) CreateH1("hng_NofGhosts_Much_Nh", "Number of hits", "Yield", nofBinsPoints, minNofPoints, maxNofPoints);
   if (fDet.GetDet(ECbmModuleId::kRich)) {
   ...
   if (fDet.GetDet(ECbmModuleId::kSts)) CreateH1("hno_NofObjects_StsTracks", "Tracks per event", "Yield", nofBinsC, 1., maxXC);
   if (fDet.GetDet(ECbmModuleId::kTrd)) CreateH1("hno_NofObjects_TrdTracks", "Tracks per event", "Yield", nofBinsC, 1., maxXC);
   if (fDet.GetDet(ECbmModuleId::kMuch)) CreateH1("hno_NofObjects_MuchTracks", "Tracks per event", "Yield", nofBinsC, 1., maxXC);
   if (fDet.GetDet(ECbmModuleId::kRich)) {
...
void CbmLitTrackingQa::ProcessGlobalTracks()
{
   ...
   for(Int_t iTrack = 0; iTrack < nofGlobalTracks; iTrack++) {
      ...
      if (isRichOk)
         rtDistance = CbmRichUtil::GetRingTrackDistance(iTrack);
      ...
      if (isMuchOk) { muchMCId = { muchTrackMatch->GetMatchedLink().GetEntry(), muchTrackMatch->GetMatchedLink().GetIndex() }; }
      if (isTofOk) {
         ...
         for (vector<CbmLink>::const_iterator tofMCIt = tofMCLinks.begin(); tofMCIt != tofMCLinks.end(); ++tofMCIt)
         {
             ...
             if (tofPoint != NULL)
                 tofMCIds.push_back(pair<Int_t, Int_t> (tofMCIt->GetEntry(), tofPoint->GetTrackID()));
         }
      }
      if (isRichOk) { richMCId = { richRingMatch->GetMatchedLink().GetEntry(), richRingMatch->GetMatchedLink().GetIndex() }; }
...
void CbmLitTrackingQa::ProcessRichRings()
{
   ...
   for(Int_t iRing = 0; iRing < nofRings; iRing++) {
     ...
     pair<Int_t, Int_t> richMCId = { isRichOk ? richRingMatch->GetMatchedLink().GetEntry() : -1, isRichOk ? richRingMatch->GetMatchedLink().GetIndex() : -1 };//GetMCTrackId() : -1;
...
void CbmLitTrackingQa::FillTrackQualityHistograms(
   ...
   ECbmModuleId detId)
{
   string detName = (detId == ECbmModuleId::kSts) ? "Sts" : (detId == ECbmModuleId::kTrd) ? "Trd" : (detId == ECbmModuleId::kMuch) ? "Much" : (detId == ECbmModuleId::kRich) ? "Rich" : "";
   assert(detName != "");
...
void CbmLitTrackingQa::ProcessMcTracks(Int_t iEvent)
{
    ...
    for (Int_t iMCTrack = 0; iMCTrack < nofMcTracks; ++iMCTrack) {
        ...
        // Check local tracks
        Bool_t stsConsecutive = (fUseConsecutivePointsInSts) ? litMCTrack.GetNofConsecutivePoints(ECbmModuleId::kSts) >= fMinNofPointsSts : true;
      ...
      const vector<Double_t>& par)
{
   ...
   Bool_t recOk = (histTypeName == "hte") ? (mcMap.find(make_pair(mcEventId, mcId)) != mcMap.end() && accOk) : (ElectronId(mcEventId, mcId, mcMap, effName) && accOk);
   Int_t nofParams = par.size();
   assert(nofParams < 3 && nofParams > 0);
   if (nofParams == 1) {
      if (accOk) { fHM->H1(accHistName)->Fill(par[0]); }
      ...
      const string& effName)
{
   ...
   TVector3 mom;
   mcTrack->GetMomentum(mom);
   Bool_t isRichElectron = (fDet.GetDet(ECbmModuleId::kRich) && (effName.find("Rich") != string::npos)) ? CbmLitGlobalElectronId::GetInstance().IsRichElectron(globalTrackIndex, mom.Mag()) : true;
   Bool_t isTrdElectron = (fDet.GetDet(ECbmModuleId::kTrd) && (effName.find("Trd") != string::npos)) ? CbmLitGlobalElectronId::GetInstance().IsRichElectron(globalTrackIndex, mom.Mag()) : true;
   Bool_t isTofElectron = (fDet.GetDet(ECbmModuleId::kTof) && (effName.find("Tof") != string::npos)) ? CbmLitGlobalElectronId::GetInstance().IsRichElectron(globalTrackIndex, mom.Mag()) : true;
...
void CbmLitTrackingQa::PionSuppression()
{
   ...
   for (Int_t iGT = 0; iGT < nofGlobalTracks; iGT++) {
      ...
      if (std::abs(pdgSts) == 211) {
         Bool_t isRichElectron = (fDet.GetDet(ECbmModuleId::kRich)) ? CbmLitGlobalElectronId::GetInstance().IsRichElectron(iGT, p) : true;
         Bool_t isTrdElectron = (fDet.GetDet(ECbmModuleId::kTrd)) ? CbmLitGlobalElectronId::GetInstance().IsTrdElectron(iGT, p) : true;
         Bool_t isTofElectron = (fDet.GetDet(ECbmModuleId::kTof)) ? CbmLitGlobalElectronId::GetInstance().IsTofElectron(iGT, p) : true;
