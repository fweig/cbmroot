/* Copyright (C) 2011-2014 UGiessen/JINR-LIT, Giessen/Dubna
...
string DefaultRowNameFormatter(const string& name) { return name; }

string EventNoRowNameFormatter(const string& name) { return Split(name, '_')[1]; }

string NofObjectsRowNameFormatter(const string& name) { return Split(name, '_')[2]; }
...
string NofGhostsRowNameFormatter(const string& name) { return Split(name, '_')[2]; }

string EventNoCellFormatter(const TH1* hist, Int_t /* nofEvents */)
{
  return NumberToString<Double_t>(hist->GetEntries());
}

string NofObjectsCellFormatter(const TH1* hist, Int_t /* nofEvents */)
{
  return NumberToString<Double_t>(hist->GetMean());
}

string TrackHitsCellFormatter(const TH1* hist, Int_t /* nofEvents */)
{
  return NumberToString<Double_t>(hist->GetMean());
}
...
CbmLitTrackingQaStudyReport::CbmLitTrackingQaStudyReport() : CbmStudyReport()
{
...
void CbmLitTrackingQaStudyReport::Create()
{
  ...
  Out() << PrintTable("Number of all, true and fake hits in tracks and rings", "hth_.+_TrackHits_.*",
                      TrackHitsRowNameFormatter, TrackHitsCellFormatter);
...
string CbmLitTrackingQaStudyReport::PrintTable(const string& tableName, const string& pattern,
...
string CbmLitTrackingQaStudyReport::PrintEfficiencyTable(const string& tableName, const string& pattern) const
{
  Int_t nofStudies = HM().size();
...
void CbmLitTrackingQaStudyReport::DrawEfficiency(const string& canvasName, const string& histName)
{
...
void CbmLitTrackingQaStudyReport::DrawAccAndRec(const string& canvasName, const string& histName)
{
  TCanvas* canvas = CreateCanvas(canvasName.c_str(), canvasName.c_str(), 600, 500);
  canvas->SetGrid();
  canvas->cd();
...
void CbmLitTrackingQaStudyReport::DivideHistos(TH1* histo1, TH1* histo2, TH1* histo3, Double_t scale)
{
  histo1->Sumw2();
  histo2->Sumw2();
  histo3->Sumw2();
...
void CbmLitTrackingQaStudyReport::DrawMeanEfficiencyLines(const vector<TH1*>& histos,
