/* Copyright (C) 2011-2018 UGiessen/JINR-LIT, Giessen/Dubna
...
CbmLitTrackingQaReport::CbmLitTrackingQaReport() : CbmSimulationReport(), fGlobalTrackVariants()
{
...
string CbmLitTrackingQaReport::PrintTrackingEfficiency(Bool_t includeRich, Bool_t isPidEfficiency) const
{
...
string CbmLitTrackingQaReport::PrintPionSuppression() const
{
  ...
  for (Int_t iHist = 0; iHist < nofHistos; iHist++) {
    ...
    string pionSupStr   = NumberToString<Double_t>(pionSup) + " (" + NumberToString<Double_t>(recPions) + "/"
...
void CbmLitTrackingQaReport::DrawEfficiencyHistos()
{
  ...
  for (UInt_t i = 0; i < fGlobalTrackVariants.size(); i++) {
    ...
    DrawEfficiency("tracking_qa_global_tracking_efficiency_all_" + variant + "_p",
                   "hte_Sts.*_" + variant + "_All_Eff_p", DefaultEfficiencyLabelFormatter);
    DrawEfficiency("tracking_qa_global_tracking_efficiency_muon_" + variant + "_p",
                   "hte_Sts.*_" + variant + "_Muon_Eff_p", DefaultEfficiencyLabelFormatter);
    DrawEfficiency("tracking_qa_global_tracking_efficiency_electron_" + variant + "_p",
                   "hte_Sts.*_" + variant + "_Electron_Eff_p", DefaultEfficiencyLabelFormatter);
    DrawEfficiency("tracking_qa_pid_efficiency_electron_" + variant + "_p",
                   "hpe_((?!Sts_))Sts.*_" + variant + "_Electron_Eff_p", ElectronIdEfficiencyLabelFormatter);
    ...
    DrawEfficiency("tracking_qa_global_tracking_efficiency_" + variant + "_angle",
                   "hte_Sts.*_" + variant + "_All_Eff_Angle", DefaultEfficiencyLabelFormatter);
    if (variant.find("Rich") != string::npos) {
      DrawEfficiency("tracking_qa_global_tracking_efficiency_electron_" + variant + "_RingXc",
                     "hte_Sts.*_" + variant + "_Electron_Eff_RingXc", DefaultEfficiencyLabelFormatter);
      DrawEfficiency("tracking_qa_global_tracking_efficiency_electron_" + variant + "_RingYc",
                     "hte_Sts.*_" + variant + "_Electron_Eff_RingYc", DefaultEfficiencyLabelFormatter);
    }
  }
  ...
  for (UInt_t i = 0; i < localTrackVariants.size(); i++) {
    ...
    string re =
      (variant == "Sts") ? "hte_Sts_Sts_(All|Muon|Electron)_Eff" : "hte_" + variant + "_.*_(All|Muon|Electron)_Eff";
  ...
  DrawEfficiency("tracking_qa_local_tracking_efficiency_Rich_RingNh", "hte_Rich_Rich_Electron_Eff_RingNh",
                 DefaultEfficiencyLabelFormatter);
  DrawEfficiency("tracking_qa_local_tracking_efficiency_Rich_BoA", "hte_Rich_Rich_Electron_Eff_BoA",
                 DefaultEfficiencyLabelFormatter);
  DrawEfficiency("tracking_qa_local_tracking_efficiency_Rich_RingXc", "hte_Rich_Rich_Electron_Eff_RingXc",
                 DefaultEfficiencyLabelFormatter);
  DrawEfficiency("tracking_qa_local_tracking_efficiency_Rich_RingYc", "hte_Rich_Rich_Electron_Eff_RingYc",
                 DefaultEfficiencyLabelFormatter);
  ...
  for (UInt_t i = 0; i < accRecTracks.size(); i++) {
    ...
    string re = (variant == "Sts") ? "hte_Sts_Sts_(All|Muon|Electron)_(Acc|Rec)_Np"
    ...
    re = (variant == "Sts") ? "hte_Sts_Sts_(Muon|Electron)_(Acc|Rec)_p"
                            : "hte_" + variant + "_.*_(Muon|Electron)_(Acc|Rec)_p";
    DrawAccAndRec("tracking_qa_local_acc_and_rec_" + variant + "_ElectronMuon_p", re);
  }

  //
  DrawPionSuppression("tracking_qa_pion_suppression_wo_Rich_p", "hps_((?!Rich)).*All_PionSup_p",
                      DefaultPionSuppressionLabelFormatter);
  DrawPionSuppression("tracking_qa_pion_suppression_with_rich_p", "hps_Rich.*All_PionSup_p",
                      DefaultPionSuppressionLabelFormatter);
...
void CbmLitTrackingQaReport::DrawEfficiency(const string& canvasName, const string& histNamePattern,
...
void CbmLitTrackingQaReport::DrawPionSuppression(const string& canvasName, const string& histNamePattern,
                                                 string (*labelFormatter)(const string&, Double_t))
{
  vector<TH1*> histos = HM()->H1Vector(histNamePattern);
  if (histos.size() == 0) return;
...
void CbmLitTrackingQaReport::DrawMeanEfficiencyLines(const vector<TH1*>& histos, const vector<Double_t>& efficiencies)
{
...
void CbmLitTrackingQaReport::DrawAccAndRec(const string& canvasName, const string& histNamePattern)
{
  vector<TH1*> histos = HM()->H1Vector(histNamePattern);
  if (histos.size() == 0) return;
...
void CbmLitTrackingQaReport::DrawYPt(const string& canvasName, const string& effHistName, Bool_t drawOnlyEfficiency)
{
...
void CbmLitTrackingQaReport::DrawHitsHistos(const string& canvasName, const string& hist)
{
  if (!(HM()->Exists(hist + "_All") && HM()->Exists(hist + "_True") && HM()->Exists(hist + "_Fake")
        && HM()->Exists(hist + "_TrueOverAll") && HM()->Exists(hist + "_FakeOverAll")))
    return;
  ...
  DrawH1(list_of(hAll)(hTrue)(hFake),
         list_of("all: " + NumberToString<Double_t>(hAll->GetMean(), 1))(
           "true: " + NumberToString<Double_t>(hTrue->GetMean(), 1))("fake: "
                                                                     + NumberToString<Double_t>(hFake->GetMean(), 1)),
...
void CbmLitTrackingQaReport::DivideHistos(TH1* histo1, TH1* histo2, TH1* histo3, Double_t scale)
{
  histo1->Sumw2();
  histo2->Sumw2();
  histo3->Sumw2();
