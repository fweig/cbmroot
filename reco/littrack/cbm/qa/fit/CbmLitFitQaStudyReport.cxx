...
CbmLitFitQaStudyReport::CbmLitFitQaStudyReport() : CbmStudyReport() {
...
void CbmLitFitQaStudyReport::DrawTrackParamsAtVertex() {
  ...
  TCanvas* canvas2 = CreateCanvas("fit_study_qa_momentum_projection",
  ...
  for (UInt_t iStudy = 0; iStudy < nofStudies; iStudy++) {
    histos1[iStudy] = HM()[iStudy]
                        ->H2("htf_MomRes_Mom")
                        ->ProjectionY(string("htf_MomRes_Mom_ProjectionY_"
                                             + NumberToString<Int_t>(iStudy))
                                        .c_str());
  ...
  for (UInt_t iStudy = 0; iStudy < nofStudies; iStudy++) {
    histos1[iStudy]->GetFunction("gaus")->SetLineColor(
      histos1[iStudy]->GetLineColor());
  }
  gPad->SetGridx(true);
  gPad->SetGridy(true);

  TCanvas* canvas3 = CreateCanvas("fit_study_qa_momentum_momres_mom_sigma",
  ...
  for (UInt_t iStudy = 0; iStudy < nofStudies; iStudy++) {
    ...
    histos2[iStudy]->SetName(
      string("htf_MomRes_Mom_2_" + NumberToString<Int_t>(iStudy)).c_str());
  ...
  TCanvas* canvas4 = CreateCanvas("fit_study_qa_momentum_momres_mom_rms",
  ...
  for (UInt_t iStudy = 0; iStudy < nofStudies; iStudy++) {
    ...
    histos3[iStudy] = hMomres->ProjectionX(
      string("htf_MomRes_Mom_ProjectionX_" + NumberToString<Int_t>(iStudy))
        .c_str());
  ...
  TCanvas* canvas5 = CreateCanvas(
    "fit_study_qa_chi_primary", "fit_study_qa_chi_primary", 600, 600);
