                                  ...
                                  Int_t trackId)
{
    ...
    if (nofMvdHits > 0) { // first track parameters in MVD
        ...
        if (mcTrack.GetNofPointsAtStation(ECbmModuleId::kMvd, firstStation) > 0) {
            ...
            FillResidualsAndPulls(firstParam, &firstPoint, "htf_Sts_FirstParam_", nofMvdHits + nofStsHits, ECbmModuleId::kMvd);
        }
        ...
        if (mcTrack.GetNofPointsAtStation(ECbmModuleId::kSts, firstStation) > 0) {
            ...
            FillResidualsAndPulls(firstParam, &firstPoint, "htf_Sts_FirstParam_", nofMvdHits + nofStsHits, ECbmModuleId::kSts);
        }
    }
                                        ...
                                        ECbmModuleId detId)
{
    ...
    if (sigmaX < 0) fHM->H2(histName + "WrongCov_X")->Fill(mcP, wrongPar); else fHM->H2(histName + "Pull_X")->Fill(mcP, resX / sigmaX);
    if (sigmaY < 0) fHM->H2(histName + "WrongCov_Y")->Fill(mcP, wrongPar); else fHM->H2(histName + "Pull_Y")->Fill(mcP, resY / sigmaY);
    if (sigmaTx < 0) fHM->H2(histName + "WrongCov_Tx")->Fill(mcP, wrongPar); else fHM->H2(histName + "Pull_Tx")->Fill(mcP, resTx / sigmaTx);
    if (sigmaTy < 0) fHM->H2(histName + "WrongCov_Ty")->Fill(mcP, wrongPar); else fHM->H2(histName + "Pull_Ty")->Fill(mcP, resTy / sigmaTy);
    if (sigmaQp < 0) fHM->H2(histName + "WrongCov_Qp")->Fill(mcP, wrongPar); else fHM->H2(histName + "Pull_Qp")->Fill(mcP, resQp / sigmaQp);
}
    ...
    for (Int_t i = 0; i < 2; i++) {
        ...
        for (Int_t iCat = 0; iCat < 3; iCat++) {
            for (Int_t iPar = 0; iPar < 5; iPar++) {
                ...
                fHM->Add(histName, new TH2F(histName.c_str(), string(histName + ";" + momentumAxisTitle + +";" + xTitles[histId] + ";Counter").c_str(),
                                            nofMomentumBins, momentumMin, momentumMax,
