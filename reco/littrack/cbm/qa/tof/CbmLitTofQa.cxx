...
void CbmLitTofQa::FillTrackCategoriesAndAcceptanceFunctions() {
  vector<string> tmp = list_of("All")("Positive")("Negative")("Primary")(
    "Secondary")("Electron")("Muon")("Proton")("AntiProton")("Pion")(
    "PionPlus")("PionMinus")("Kaon")("KaonPlus")("KaonMinus");
  ...
  fTrackAcceptanceFunctions["Positive"] =
    CbmLitAcceptanceFunction::PositiveTrackAcceptanceFunction;
  fTrackAcceptanceFunctions["Negative"] =
  ...
  fTrackAcceptanceFunctions["AntiProton"] =
...
void CbmLitTofQa::CreateHistograms() {
  ...
  for (Int_t iCat = 0; iCat < nofTrackCategories; iCat++) {
    ...
    fHM->Add(name,
             new TH2F(name.c_str(),
                      string(name + ";P [GeV/c];M^{2} [(GeV/c)^{2}]").c_str(),
                      fPRangeBins,
                      fPRangeMin,
                      fPRangeMax,
                      400,
    ...
    fHM->Add(
      name,
      new TH1F(
        name.c_str(), string(name + ";Distance [cm]").c_str(), 200, 0., 50.));
    name = "hmp_TofTrack_" + fTrackCategories[iCat] + "_NormDistance";
    fHM->Add(name,
             new TH1F(name.c_str(),
                      string(name + ";Normalized distance").c_str(),
                      200,
                      0.,
                      50.));
    ...
    fHM->Add(
      name,
      new TH1F(
        name.c_str(), string(name + ";Length [cm]").c_str(), 1200, 0., 1200.));
    name = "hmp_TofTrack_" + fTrackCategories[iCat] + "_NofHitsPerGlobalTrack";
    fHM->Add(name,
             new TH1F(name.c_str(),
                      string(name + ";number of hits per global track").c_str(),
                      11,
                      -0.5,
                      10.5));
  }
  ...
  fHM->Add(name,
           new TH1F(name.c_str(),
                    string(name + ";dt [ps];Counter").c_str(),
                    1000,
                    -500.,
                    500.));
  ...
  fHM->Add(name,
           new TH1F(name.c_str(),
                    string(name + ";Time [ns];Counter").c_str(),
                    2000,
                    0.,
                    36.));
  ...
  fHM->Add(name,
           new TH1F(name.c_str(),
                    string(name + ";Number of tracks;Counter").c_str(),
                    100,
                    0.,
                    100.));
...
void CbmLitTofQa::ProcessMC(Int_t iEvent) {
  ...
  for (Int_t iHit = 0; iHit < nofHits; iHit++) {
    ...
    const CbmTofPoint* tofPoint = static_cast<const CbmTofPoint*>(
      fTofPoints->Get(0, tofPointEventNo, tofPointIndex));
    fMCTrackIdForTofHits.insert(
      make_pair(tofPointEventNo, tofPoint->GetTrackID()));
  }

  Int_t nofPoints = fTofPoints->Size(0, iEvent);
  for (Int_t iPoint = 0; iPoint < nofPoints; iPoint++) {
    const CbmTofPoint* tofPoint =
      static_cast<const CbmTofPoint*>(fTofPoints->Get(0, iEvent, iPoint));
    fMCTrackIdForTofPoints.insert(
      pair<Int_t, Int_t>(iEvent, tofPoint->GetTrackID()));
  }
}
...
void CbmLitTofQa::ProcessGlobalTracks() {
  ...
  for (Int_t iTrack = 0; iTrack < nofGlobalTracks; iTrack++) {
    ...
    const CbmTofHit* tofHit =
      static_cast<const CbmTofHit*>(fTofHits->At(tofId));
    ...
    const CbmTofPoint* tofPoint = static_cast<const CbmTofPoint*>(
      fTofPoints->Get(0, tofMCEventId, tofMCPointId));
    ...
    CbmKFParticleInterface::ExtrapolateTrackToPV(
      stsTrack, fPrimVertex, &vtxTrack, chiSqPrimary);

    Bool_t chiSqPrimaryOk = chiSqPrimary < 3.;

    //      Double_t ctCorrection = -0.007;
    Double_t ctCorrection = 0.0;
    Double_t ctReco       = 0.299792458 * tofHit->GetTime()
                      + ctCorrection;  // ToF time in ns -> transfrom to ct in m
    Double_t ctMC =
      0.299792458
      * tofPoint->GetTime();  // mc time in ns -> transfrom to ct in m
    Double_t trackLengthReco = globalTrack->GetLength() / 100.;  //global length
    // Double_t trackLengthMC = tofPoint->GetLength() / 100.; //mc length
    Double_t preco =
      (vtxTrack.GetQp() != 0) ? std::abs(1. / vtxTrack.GetQp()) : 0;
    Double_t t      = (trackLengthReco != 0) ? (ctReco / trackLengthReco) : 0;
    Double_t m2reco = preco * preco * (t * t - 1);
    Double_t radialPos =
      sqrt(tofHit->GetX() * tofHit->GetX() + tofHit->GetY() * tofHit->GetY());

    if (chiSqPrimaryOk && radialPos < 50.) {
      nofTracksForTimeZero++;
      Double_t beta = 1.;
      if (m2reco > 0.6)
        beta = preco / sqrt(preco * preco + 0.93827231 * 0.93827231);
      if (m2reco <= 0.6)
        beta = preco / sqrt(preco * preco + 0.1395679 * 0.1395679);
      timeZeroReco +=
        tofHit->GetTime() - trackLengthReco / (beta * 0.299792458);
      timeZeroMC +=
        tofPoint->GetTime() - trackLengthReco / (beta * 0.299792458);
    ...
    for (Int_t iCat = 0; iCat < nofTrackCategories; iCat++) {
      ...
      LitTrackAcceptanceFunction function =
        fTrackAcceptanceFunctions.find(category)->second;
      ...
      Bool_t accTofOk =
        fMCTrackIdForTofHits.find(make_pair(tofMCEventId, stsMCTrackId))
        != fMCTrackIdForTofHits.end();

      if (categoryOk && chiSqPrimaryOk) {
        fHM->H1("hmp_Tof_Reco_" + category + "_m2p")->Fill(preco, m2reco);
        if (accTofOk)
          fHM->H1("hmp_Tof_RecoAccTof_" + category + "_m2p")
            ->Fill(preco, m2reco);

        if (stsMCTrackId == tofMCTrackId) {
          fHM->H1("hmp_Tof_RecoMCID_" + category + "_m2p")->Fill(preco, m2reco);
          if (accTofOk)
            fHM->H1("hmp_Tof_RecoMCIDAccTof_" + category + "_m2p")
              ->Fill(preco, m2reco);
        }
      }
    }
...
void CbmLitTofQa::ProcessTofHits() {
  ...
  for (Int_t iHit = 0; iHit < nofTofHits; iHit++) {
    ...
    fHM->H1("hmp_Tof_dTime")
      ->Fill(1000 * (tofPoint->GetTime() - tofHit->GetTime()));
  }
}
...
void CbmLitTofQa::ProcessTofTracks() {
  ...
  for (Int_t iTrack = 0; iTrack < nofTofTracks; iTrack++) {
    const CbmTofTrack* tofTrack =
      static_cast<const CbmTofTrack*>(fTofTracks->At(iTrack));
  ...
  for (Int_t iTrack = 0; iTrack < nofTofTracks; iTrack++) {
    const CbmTofTrack* tofTrack =
      static_cast<const CbmTofTrack*>(fTofTracks->At(iTrack));
    const CbmTofHit* tofHit =
      static_cast<const CbmTofHit*>(fTofHits->At(tofTrack->GetTofHitIndex()));
    CbmMatch* tofHitMatch =
      static_cast<CbmMatch*>(fTofHitsMatches->At(tofTrack->GetTofHitIndex()));
    if (tofHitMatch == NULL) { continue; }
    Int_t tofMCPointId          = tofHitMatch->GetMatchedLink().GetIndex();
    Int_t tofMCEventId          = tofHitMatch->GetMatchedLink().GetEntry();
    const FairMCPoint* tofPoint = static_cast<const FairMCPoint*>(
      fTofPoints->Get(0, tofMCEventId, tofMCPointId));
    Int_t tofMCTrackId = tofPoint->GetTrackID();
    ...
    for (Int_t iCat = 0; iCat < nofTrackCategories; iCat++) {
      ...
      if (categoryOk) {
        ...
        fHM->H1("hmp_TofTrack_" + category + "_NormDistance")
          ->Fill(tofTrack->GetDistance());
        fHM->H1("hmp_TofTrack_" + category + "_Length")
          ->Fill(tofTrack->GetTrackLength());
...
void CbmLitTofQa::FitHistograms() {
  ...
  TH1* meanHist =
    (TH1*) gDirectory->Get("hmp_Tof_RecoMCID_Pion_m2p_1");  // mean
  TH1* sigmaHist =
    (TH1*) gDirectory->Get("hmp_Tof_RecoMCID_Pion_m2p_2");  // sigma
