...
CbmLitTofQaReport::CbmLitTofQaReport() : CbmSimulationReport() {
...
void CbmLitTofQaReport::Draw() {
  ...
  DrawH2ByPattern(
    "hmp_Tof_RecoMCIDAccTof_.+_m2p", kLinear, kLinear, kLog, "colz");
...
void CbmLitTofQaReport::FitHistograms() {
  vector<string> categories =
    list_of("Electron")("Muon")("Pion")("Proton")("Kaon");
  ...
  for (Int_t iCat = 0; iCat < nofCategories; iCat++) {
    ...
    TCanvas* canvas =
      CreateCanvas(canvasName.c_str(), canvasName.c_str(), 1200, 1000);
    canvas->cd(1);
    ...
    hist->FitSlicesY();
    TH1* meanHist =
      (TH1*) gDirectory->Get(string(histName + "_1").c_str());  // mean
    TH1* sigmaHist =
      (TH1*) gDirectory->Get(string(histName + "_2").c_str());  // sigma
    Int_t nofBins   = meanHist->GetNbinsX();
    TGraph* upGraph = new TGraph(nofBins);
    upGraph->GetXaxis()->SetRangeUser(meanHist->GetXaxis()->GetXmin(),
                                      meanHist->GetXaxis()->GetXmax());
    TGraph* downGraph = new TGraph(nofBins);
    downGraph->GetXaxis()->SetRangeUser(meanHist->GetXaxis()->GetXmin(),
                                        meanHist->GetXaxis()->GetXmax());
    TGraph* meanGraph = new TGraph(nofBins);
    meanGraph->GetXaxis()->SetRangeUser(meanHist->GetXaxis()->GetXmin(),
                                        meanHist->GetXaxis()->GetXmax());
...
void CbmLitTofQaReport::FitFunction(TGraph* graph) {
  ...
  std::cout << "Function: " << p0 << "*x^4" << string((p1 > 0) ? "+" : "") << p1
            << "*x^3" << string((p2 > 0) ? "+" : "") << p2 << "*x^2"
            << string((p3 > 0) ? "+" : "") << p3 << "*x"
