    ...
    Option_t* /*opt*/)
{
   ...
   // STS: hit->track
   MatchStsTracks(fMvdHitMatches, fStsHitMatches, fMvdPoints,
                  fStsPoints, fStsTracks, fStsTrackMatches);
...
void CbmMatchRecoToMC::MatchClusters(ECbmModuleId systemId, const TClonesArray* clusters,
																		 TClonesArray* clusterMatches) {
      ...
      TClonesArray* trackMatches)
{
   ...
   for (Int_t iTrack = 0; iTrack < nofTracks; iTrack++) {
      ...
      for (Int_t iHit = 0; iHit < nofHits; iHit++) {
	if ((track->GetHitType(iHit) == kMUCHPIXELHIT) || (track->GetHitType(iHit) == kMUCHSTRAWHIT && hitMatches == fMuchPixelHitMatches)) continue; //AZ
    ...
    TClonesArray* ringMatches)
{
    ...
    for (Int_t iRing = 0; iRing < nRings; iRing++) {
        ...
        for (Int_t iHit=0; iHit < nofHits; iHit++) {
            ...
            vector<pair<Int_t, Int_t> > motherIds = GetMcTrackMotherIdsForRichHit(fDigiManager, hit, richMcPoints, mcTracks, fEventNumber);
