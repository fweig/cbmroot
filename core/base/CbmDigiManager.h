...
class CbmDigiManager
{
  ...
  public:
     ...
     **/
    template <class Digi> const Digi* Get(Int_t index) const {
    		...
    		ECbmModuleId system = Digi::GetSystem();
                ...
                } catch( const boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<boost::bad_any_cast> >& ) {
                  LOG( fatal ) << "Failed boost any_cast in Digimanager::Get for a digi of type "
                               << Digi::GetClassName();
  ...
  private:
    ...
    template <class Digi> void SetBranch();
