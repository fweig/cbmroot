...
void CbmStsParSetSensorCond::Init() {
  ...
  for ( auto& it : fParams ) it.second.Init();
...
UInt_t CbmStsParSetSensorCond::ReadParams(const char* fileName) {
  ...
  while ( kTRUE ) {  // read one line per sensor
    ...
    line >> sName >> address >> vDep >> vBias >> temperature
    ...
    if ( fParams.count(address) ) {
      LOG(fatal) << GetName() << ": Multiple occurence of address "
          << address << " in input file";
      continue;
    }

    // Add sensor conditions to set
    fParams[address].SetParams(vDep, vBias, temperature,
                               cCoupling, cInterstrip);
    nSensors++;
  } //# input lines
...
void CbmStsParSetSensorCond::SetGlobalPar(Double_t vFd,
  ...
  if ( fGlobal ) LOG(warn) << GetName()
      << ": Overwriting current global settings!";
  fGlobalParams.SetParams(vFd, vBias, temperature,
                          cCoupling, cInterstrip);
  fGlobal = kTRUE;
}
// -------------------------------------------------------------------------
...
std::string CbmStsParSetSensorCond::ToString() {
  ...
  std::stringstream ss;
  if ( ! IsSet() ) ss << "Empty";
  else if ( fGlobal ) ss << "(Global) " << fGlobalParams.ToString();
  else ss << "Conditions for " << fParams.size() << " sensors";
  return ss.str();
}
// -------------------------------------------------------------------------
