...
public:
    ...
    void PrintCylindrical() {
        ...
        for(it_type iterator = fPmtMap.begin(); iterator != fPmtMap.end(); iterator++) {
            ...
            LOG(info) << "PMT position in (x,y,z) [cm]: " << iterator->second.fX << "  " << iterator->second.fY << "  " << iterator->second.fZ;
            LOG(info) << "PMT plane position in (x,y,z) [cm]: " << iterator->second.fPlaneX << "  " << iterator->second.fPlaneY << "  " << iterator->second.fPlaneZ;
    ...
    void PrintMirror() {
        LOG(info) << "Refractive index for lowest photon energies: "<< fNRefrac << ", (n-1)*10000: " << (fNRefrac-1.0)*10000.0;
...
public:
    ...
    std::map<std::string, CbmRichRecGeoParPmt> fPmtMap; // PMT parameter map for CbmRichGeometryTypeCylindrical, string is geo path to PMT block
