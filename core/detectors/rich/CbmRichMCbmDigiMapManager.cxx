...
void CbmRichMCbmDigiMapManager::Init() {
  ...
  if (pmtVolume == nullptr)
    pmtVolume = gGeoManager->FindVolumeFast(
      "pmt_vol_0");  // check for mRICH  // or pmt_vol_1
  if (pmtVolume != nullptr) {
  ...
  while ((curNode = geoIterator())) {
    ...
    if (TString(curNode->GetVolume()->GetName()).Contains(pixelNameStr)) {
      ...
      Int_t DiRICH_Add = ((7 & 0xF) << 12) + ((x & 0xF) << 8) + ((y & 0xF) << 4)
                         + (posAtPmt & 0xF);
...
CbmRichPixelData*
CbmRichMCbmDigiMapManager::GetPixelDataByAddress(Int_t address) {
...
vector<Int_t>
CbmRichMCbmDigiMapManager::GetDirectNeighbourPixels(Int_t /*address*/) {
  std::vector<Int_t> v;
...
vector<Int_t>
CbmRichMCbmDigiMapManager::GetDiagonalNeighbourPixels(Int_t /*address*/) {
  std::vector<Int_t> v;
