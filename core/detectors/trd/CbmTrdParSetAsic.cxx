...
Bool_t CbmTrdParSetAsic::getParams(FairParamList* l) {
  ...
  for (Int_t i = 0; i < fNrOfModules; i++) {
    fModPar[moduleId[i]] = new CbmTrdParSetAsic(
      GetName(),
      Form("%s for Module %d", GetTitle(), moduleId[i]) /*, GetContext()*/);
    // only for FASP
    if (9 == typeAsic[i]) {
      Int_t sizePerFasp = 1 + NFASPCH * 4;
      Int_t maxValues   = maxNrAsics * (sizePerFasp);
      TArrayI values(maxValues);
      if (!l->fill(Form("FaspInfo - Module %d", moduleId[i]), &values))
        continue;
      ...
      if (!l->fill(Form("SpadicInfo - Module %d", moduleId[i]), &values))
        continue;
      for (Int_t iasic = 0; iasic < nAsic[i]; iasic++) {
        ...
        asic->SetComponentId(
          static_cast<CbmTrdParSpadic*>(asic)->CreateComponentId(
            values[offset + 1],
            values[offset + 2],
            values[offset + 3],
            values[offset + 4]));
...
void CbmTrdParSetAsic::putParams(FairParamList* l) {
  ...
  for (std::map<Int_t, CbmTrdParSetAsic*>::iterator imod = fModPar.begin();
       imod != fModPar.end();
       imod++) {
  ...
  for (Int_t i = 0; i < fNrOfModules; i++) {
    ...
    if (mod->fModuleMap.begin()->second->IsA() == CbmTrdParSpadic::Class()) {
      ...
      for (auto iModuleIt : mod->fModuleMap) {
        ...
        if ((((CbmTrdParSpadic*) iModuleIt.second)->GetNchannels())
            != NSPADICCH) {
          LOG(fatal) << "Number of channels found "
                     << ((CbmTrdParSpadic*) iModuleIt.second)->GetNchannels()
                     << " is differnt from the expected " << NSPADICCH;
        }

        Int_t iAsicChannel(0);
        for (auto channelAddressIt :
      ...
      for (auto iModuleIt : mod->fModuleMap) {
        ...
        for (Int_t ich(0); ich < nchannels; ich += 2) {
          for (Int_t ipair(0); ipair < 2; ipair++) {
            ...
            const CbmTrdParFaspChannel* ch =
              fasp->GetChannel(faspAddress, ipair);
            asicInfo[offset + 1 + ich + ipair] = faspAddress;
            asicInfo[offset + 1 + (1 * NFASPCH) + ich + ipair] =
              ch->GetPileUpTime();
            asicInfo[offset + 1 + (2 * NFASPCH) + ich + ipair] =
              ch->GetThreshold();
            asicInfo[offset + 1 + (3 * NFASPCH) + ich + ipair] =
              ch->GetMinDelaySignal();
          }
        }
        iAsicNr++;
      }
...
void CbmTrdParSetAsic::SetAsicPar(Int_t address, CbmTrdParAsic* p) {
  ...
  if (it != fModuleMap.end()) {
    LOG(warn) << GetName() << "::SetAsicPar : The ASIC @ " << address
