...
void CbmTrdGas::Init()
{
   ...
   if (nmixt != 3) {
      LOG(error) << "CbmTrdGas::Init: This is not a mixture composed out of three different elements.";
   ...
   if (!(foundCarbon && foundOxygen)) {
      LOG(error) << "CbmTrdGas::Init: This gas mixture has no CO2 admixture \n"
         << "CbmTrdGas::Init: If you want to use this mixture you have to change \n"
   ...
   if (elem[noblegas] == 54) {
      LOG(info) << "CbmTrdGas::Init: Percent  (Xe) : " << (fPercentNobleGas * 100);
   }
...
void CbmTrdGas::SetFileName()
{
   ...
   if (fDetType == 0) {
      ...
      LOG(error) << "CbmTrdGas::SetFileName: Stop execution of program due to initialization error.";
