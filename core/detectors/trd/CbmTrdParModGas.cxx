...
//  Int_t val;
  ...
  for(Int_t ie(0); ie<so->GetEntries(); ie+=2){
    ...
    if(name.EqualTo("Module")) fModuleId = ((TObjString*)(*so)[ie+1])->String().Atoi();
      ...
      else if(gas.EqualTo("Xe")) SetNobleGasType(0);
      else {
...
Float_t CbmTrdParModGas::EkevFC(Float_t ekev) const
{
  ...
  if(VERBOSE) printf("        ua[V]=%d gain[%5.2e] wi[eV]=%5.2f :: E[keV]=%6.3f E[fC]=%6.2f\n", fUa, gain, wi, ekev, efC);
  return efC;
 ...
 */  
  switch(shell){
    ...
    case 'M': return 2;
    default:
      LOG(warn) << GetName() <<"::GetShellId: Atomic shell : "<<shell<<" not defined for gas "<<(GetNobleGasType()==2?"Ar":"Xe");
      return -1;
  }
}
...
Float_t CbmTrdParModGas::GetBindingEnergy(const Char_t shell, Bool_t main) const
{
  ...
  if(!main) return fgkBindingEnergy[gasId][shellId];
    ...
    if(shellId<NSHELLS-1) return fgkBindingEnergy[gasId][shellId+1];
      ...
      LOG(warn) << GetName() <<"::GetBindingEnergy: Request atomic shell : "<<shellId+1<<" not defined for gas "<<(gasId?"Ar":"Xe");
      return 0;
    }
  }
  return 0;
...
Char_t CbmTrdParModGas::GetPEshell(Float_t Ex) const
{
  ...
  LOG(debug) << GetName() <<"::GetPEshell: Ex[keV] "<<Ex<<" less than highes atomic shell binding energy : "<<fgkBindingEnergy[gasId][NSHELLS-1]<<" for gas "<<(gasId?"Ar":"Xe");
  return 0;
}
...
//  Double_t dhh(fGasThick);
  ...
  if(VERBOSE) printf("ScanDriftTime :: Try : [%7.4f %7.4f] => [%7.4f %7.4f] dzdy[%5.2f] dy[%5.2f]\n", y0, z0, y1, z1, dzdy, dy);
  while(y1<-dwh-1e-3){
    y0+=dw;y1+=dw;
  }
  while(y1>dwh+1.e-3){
