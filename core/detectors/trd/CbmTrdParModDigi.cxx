...
*/
  if (nofSectors > 1) {
    ...
    if (sectorSizeX.GetAt(0) == 2 * sizex) { //substructure only in y-direction
      ...
      LOG(warn) << GetName() << "::CbmTrdParModDigi - detect different information on module size x : geometry=" << std::setprecision(5) << 2 * sizex << " CbmTrdPads.h="<<sectorSizeX.GetAt(0);
      beginy = 0;
      endy = 2 * sizey;
      summed_sectsize = 0;
      for (Int_t i = 0; i < fNofSectors; i++) {
...
*/
  ...
  for(Int_t isec(0); isec<fNofSectors; isec++) 
    printf(" [c(%3d) r(%2d)]x[w(%4.2f) h(%5.2f)] area[%5.2fx%5.2f] limits x[%5.2f %5.2f] y[%5.2f %5.2f]\n                ", 
           GetNofColumnsInSector(isec), GetNofRowsInSector(isec), fPadSizeX.At(isec), fPadSizeY.At(isec),
          fSectorSizeX[isec], fSectorSizeY[isec], fSectorBeginX[isec], fSectorEndX[isec], fSectorBeginY[isec], fSectorEndY[isec]); printf("\n");
}
...
*/  
  ...
  if (/*fAnodeWireOffset > 0.0 && */fAnodeWireSpacing > 0.0) {  // wires must be defined defined
    ...
    if ( fabs(local_point[1]) - fSizeY > 1.e-3 )
      LOG(error) << "CbmTrdParModDigi::ProjectPositionToNextAnodeWire - local point must be within gas volume, y=" << std::setprecision(5) << local_point[1] << " module size "<<fSizeY;
    ...
    if ( fabs(local_point[1]) > fSizeY - fAnodeWireOffset )
      LOG(error) << "CbmTrdParModDigi::ProjectPositionToNextAnodeWire - local point projected outside anode wire plane, from " 
                  << std::setprecision(5) << ypos << " to "
                  << std::setprecision(5) << local_point[1] << " - last anode wire at "
                  << std::setprecision(5) << fSizeY - fAnodeWireOffset;

  } else {
    LOG(error) << "CbmTrdParModDigi::ProjectPositionToNextAnodeWire: fAnodeWireOffset and fAnodeWireSpacing not set. ProjectPositionToNextAnodeWire can not be used.";
  }
}

//___________________________________________________________________________
Int_t CbmTrdParModDigi::GetSector(
...
*/
  ...
  LOG(error) << "CbmTrdParModDigi::GetSector: Could not find local point ["<<local_point[0]<<" "<<local_point[1]<<"] in any of the sectors";
  Print("all");
    ...
    for (Int_t iSector = 0; iSector < fNofSectors; iSector++) {  // for each sector
      ...
      if ( npady < nofRows + secRows ) {  // if we are in the sector containing the pad
        ...
        if ( (rowId < 0) || (rowId > GetNofRowsInSector(iSector)-1) )
          LOG(fatal) << "CbmTrdParModDigi::GetModuleRow rowId "   << rowId     << " of " << GetNofRowsInSector(iSector)-1
...
//___________________________________________________________________________
Int_t CbmTrdParModDigi::GetModuleRow(
          Int_t& sectorId,
          Int_t& rowId) const
{
  ...
  if ( (sectorId < 0) || (sectorId > GetNofSectors()-1) )
    LOG(fatal) << "CbmTrdParModDigi::GetModuleRow sectorId " << sectorId << " of " << GetNofSectors()-1
    //<< " in moduleAddress " << fModuleAddress 
    << " is out of bounds!";

  // check limits
  if ( (rowId < 0) || (rowId > GetNofRowsInSector(sectorId)-1) )
    LOG(fatal) << "CbmTrdParModDigi::GetModuleRow rowId "   << rowId     << " of " << GetNofRowsInSector(sectorId)-1
    //<< " in moduleAddress " << fModuleAddress 
    << " is out of bounds!";
...
Int_t CbmTrdParModDigi::GetSectorRow(Int_t growId, Int_t& rowId) const
{
  ...
  if ( (growId < 0) || (growId >= GetNofRows()) )
    LOG(fatal) << "CbmTrdParModDigi::GetSectorRow rowId "   << growId     << " of " << GetNofRows()
    //<< " in moduleAddress " << fModuleAddress 
    << " is out of bounds!";
...
//___________________________________________________________________________
Bool_t CbmTrdParModDigi::GetPadInfo(
        ...
        Int_t& rowId) const
{
  ...
  if ( fabs(local_point[0]) > fSizeX ){
    LOG(error) << "CbmTrdParModDigi::GetPadInfo - local point x must be within gas volume, x=" 
      << std::setprecision(8) << local_point[0] << " fSizeX="
      << std::setprecision(8) << fSizeX;
    return kFALSE;
  }
  if ( fabs(local_point[1]) > fSizeY ){
    LOG(error) << "CbmTrdParModDigi::GetPadInfo - local point y must be within gas volume, y=" 
      << std::setprecision(8) << local_point[1] << " fSizeY="
      << std::setprecision(8) << fSizeY;
    return kFALSE;
  }
...
//___________________________________________________________________________
void CbmTrdParModDigi::GetPadInfo(
...
*/
  ...
  if ( gLogger->IsLogNeeded(fair::Severity::debug2) ){
    ...
    LOG(debug2) << "sec2   x: " << std::setprecision(5) << fSectorBeginX.GetAt(2)
          << " y: " << std::setprecision(5) << fSectorBeginY.GetAt(2);
    LOG(debug2) << "sec1   x: " << std::setprecision(5) << fSectorBeginX.GetAt(1)
          << " y: " << std::setprecision(5) << fSectorBeginY.GetAt(1);
    LOG(debug2) << "sec0   x: " << std::setprecision(5) << fSectorBeginX.GetAt(0)
          << " y: " << std::setprecision(5) << fSectorBeginY.GetAt(0);
...
//___________________________________________________________________________
void CbmTrdParModDigi::TransformToLocalCorner(
            const Double_t* local_point,
            Double_t& posX,
            Double_t& posY) const
{
...
//___________________________________________________________________________
void CbmTrdParModDigi::TransformToLocalSector(
            const Double_t* local_point,
            Double_t& posX,
            Double_t& posY) const
{
...
//___________________________________________________________________________
void CbmTrdParModDigi::TransformToLocalPad(
          const Double_t* local_point,
          Double_t& posX,
          Double_t& posY) const
{
...
//___________________________________________________________________________
void CbmTrdParModDigi::TransformHitError(
...
//   }
  ...
  // check limits
  if ( (sectorId < 0) || (sectorId > GetNofSectors()-1) )
    LOG(fatal) << "CbmTrdParModDigi::GetPosition sectorId " << sectorId << " of " << GetNofSectors()-1                 //<< " in moduleAddress " << moduleAddress 
    << " is out of bounds!";

  // check limits
  if ( (columnId < 0) || (columnId > GetNofColumnsInSector(sectorId)-1) )
    LOG(fatal) << "CbmTrdParModDigi::GetPosition columnId " << columnId << " of " << GetNofColumnsInSector(sectorId)-1 
    //<< " in moduleAddress " << moduleAddress 
    << " is out of bounds!";

  // check limits
  if ( (rowId < 0) || (rowId > GetNofRowsInSector(sectorId)-1) )
    LOG(fatal) << "CbmTrdParModDigi::GetPosition rowId "   << rowId     << " of " << GetNofRowsInSector(sectorId)-1    
    //<< " in moduleAddress " << moduleAddress 
    << " is out of bounds!";
  ...
  if ( (local_point[0]< 0) || (local_point[0] > 2 * fSizeX) )
    LOG(fatal) << "CbmTrdParModDigi::GetPosition local_point[0]=" << local_point[0] 
    //<< " in moduleAddress " << moduleAddress 
    << " is out of bounds!";
  // check limits
  if ( (local_point[1]< 0) || (local_point[1] > 2 * fSizeY) )
    LOG(fatal) << "CbmTrdParModDigi::GetPosition local_point[1]=" << local_point[1] 
    //<< " in moduleAddress " << moduleAddress 
    << " is out of bounds!";
