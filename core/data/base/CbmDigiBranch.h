 ...
 **/
template <class Digi> class CbmDigiBranch : public CbmDigiBranchBase
{
	...
	public:
		...
		virtual Bool_t ConnectToTree() {
			...
			fDigiVector =
					frm->InitObjectAs<std::vector<Digi> const *>(fName.Data());
		...
		virtual std::string ToString() const {
		   ...
		   else if ( fDigiArray ) ss << "TClonesArray";
		   else ss << "not connected";
		   ss << "), match branch " << fName + "Match (";
		   if ( fMatchVector ) ss << "vector";
		   else if ( fMatchArray ) ss << "TClonesArray";
		   else ss << "not connected";
		   ss << ")";
		   return ss.str();
		}
