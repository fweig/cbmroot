...
// -----   Default constructor
CbmStsHit::CbmStsHit()
  : CbmPixelHit(), fFrontClusterId(-1), fBackClusterId(-1), fDu(-1.), fDv(-1.) {
  SetTime(-1.);
  SetTimeError(-1.);
}
...
CbmStsHit::CbmStsHit(Int_t address,
                     const TVector3& pos,
                     const TVector3& dpos,
                     Double_t dxy,
...
string CbmStsHit::ToString() const {
  ...
  ss << "StsHit: address " << GetAddress() << " | time " << GetTime() << " +- "
     << GetTimeError() << " | Position (" << std::setprecision(6) << GetX()
     << ", " << GetY() << ", " << GetZ() << ") cm | Error (" << GetDx() << ", "
     << GetDy() << ", " << GetDz() << ") cm | Cluster (" << fFrontClusterId
     << ", " << fBackClusterId << ")";
  return ss.str();
}
