...
TEST(_GTestCbmPixelHit, CheckDefaultConstructor) {
  CbmPixelHit test;
  {
    ...
    comparePixelHitDataMembers(
      test, kPIXELHIT, 0., 0., -1, -1, nullptr, -1., -1., 0., 0., 0., 0., 0.);
  }
}
...
TEST(_GTestCbmPixelHit, CheckStandardConstructorWithTime) {
  CbmPixelHit test {-1, 0., 0., 0., 0., 0., 0., 0., -1, -2., -2.};
  {
    ...
    comparePixelHitDataMembers(
      test, kPIXELHIT, 0., 0., -1, -1, nullptr, -2., -2., 0., 0., 0., 0., 0.);
  }
}
...
TEST(_GTestCbmPixelHit, CheckSetters) {
  ...
  test.SetX(-11.);
  {
    ...
    comparePixelHitDataMembers(
      test, kPIXELHIT, 0., 0., -1, -1, nullptr, -1., -1., -11., 0., 0., 0., 0.);
  }

  test.SetDx(-12.);
  {
    SCOPED_TRACE("CheckSetters: SetDx");
    comparePixelHitDataMembers(test,
                               kPIXELHIT,
                               0.,
                               0.,
                               -1,
...
TEST(_GTestCbmPixelHit, CheckToString) {
  CbmPixelHit test;
  EXPECT_STREQ(
    "CbmPixelHit: address=-1 pos=(0,0,0) err=(0,0,0) dxy=0 refId=-1\n",
    test.ToString().c_str());
}
