...
TEST(_GTestCbmTrdPoint , CheckDefaultConstructor)
{
  ...
  compareTrdPointDataMembers(test, -1, -1, TVector3(0., 0., 0.), TVector3(0., 0., 0.), 
                             TVector3(0., 0., 0.), TVector3(0., 0., 0.), 0., 0., 0., 0);

  CbmTrdPoint* test1 = new CbmTrdPoint();

  compareTrdPointDataMembers(*test1, -1, -1, TVector3(0., 0., 0.), TVector3(0., 0., 0.), 
                             TVector3(0., 0., 0.), TVector3(0., 0., 0.), 0., 0., 0., 0);
...
TEST(_GTestCbmTrdPoint , CheckStandardConstructor)
{
  ...
  compareTrdPointDataMembers(test, trackid, detid, posin, momin, posout, momout, 
                             tof, length, eloss, eventid);

  CbmTrdPoint* test1 = new CbmTrdPoint(trackid, detid, posin, momin, posout, momout, tof, length, eloss);

  compareTrdPointDataMembers(*test1, trackid, detid, posin, momin, posout, momout, 
                             tof, length, eloss, eventid);
}
...
TEST(_GTestCbmTrdPoint , CheckCopyConstructor)
{
  ...
  compareTrdPointDataMembers(test2, trackid, detid, posin, momin, posout, momout, 
                             tof, length, eloss, eventid);
...
TEST(_GTestCbmTrdPoint, CheckPrint)
{
  ...
  EXPECT_STREQ("[INFO] TRD point for track 34 in detector 45\n[INFO]     Position In (-2.1, 6.1, 23) cm\n[INFO]     Momentum In (2.5, 4.5, 78) GeV\n[INFO]     Position Out (2.1, -6.1, -23) cm\n[INFO]     Momentum Out (-2.5, -4.5, -78) GeV\n[INFO]     Time 34.56 ns,  Length 1.2 cm,  Energy loss 456700 keV\n",
               output.c_str());
}
