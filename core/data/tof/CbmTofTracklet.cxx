...
Double_t CbmTofTracklet::GetZ0x() {
  Double_t dZ0 = 0.;
  if (fTrackPar.GetTx() != 0.)
    dZ0 = -fTrackPar.GetX() / fTrackPar.GetTx() + fTrackPar.GetZ();
  return dZ0;
}

Double_t CbmTofTracklet::GetZ0y() {
  Double_t dZ0 = 0.;
  if (fTrackPar.GetTy() != 0.)
    dZ0 = -fTrackPar.GetY() / fTrackPar.GetTy() + fTrackPar.GetZ();
  return dZ0;
}

Double_t CbmTofTracklet::GetR0() {
  Double_t dR0 =
    TMath::Sqrt(TMath::Power(GetFitX(0.), 2) + TMath::Power(GetFitY(0.), 2));
  return dR0;
}
...
Double_t CbmTofTracklet::GetTex(CbmTofHit* pHit) {
  ...
  */
  ...
  LOG(debug) << Form(
    "GetTex T0 %7.3f, Z %7.1f, DZ %5.1f, Sign %2.0f, Tt %7.4f => Tex %7.3f, ",
    fhit[0].GetTime(),
    dZ,
    dZ - fhit[0].GetZ(),
    dSign,
    fTt,
    dTex) << fTrackPar.ToString();
  return dTex;
}
...
Double_t CbmTofTracklet::UpdateT0() {  //returns estimated time at R=0
    ...
    if (fTofDet[iHit] >= 0) {  // exclude faked hits
      if (nValidHits == 0) {
        ...
        dDist1 = fhit[iHit1].GetZ()
                 * TMath::Sqrt(1. + fTrackPar.GetTx() * fTrackPar.GetTx()
                               + fTrackPar.GetTy() * fTrackPar.GetTy());
      }
  ...
  */
  ...
  fT0 = (RRsum * tsum - Rsum * Rtsum)
        / det_cov_mat;  // Best Guess for time at origin
  fTt = (-Rsum * tsum + esum * Rtsum)
        / det_cov_mat;  // Best guess for inverted velocity
  fT0Err = TMath::Sqrt(
    RRsum / det_cov_mat);  // sqrt of (0,0) in Covariance matrix -> error on fT0
  fTtErr = TMath::Sqrt(
    esum / det_cov_mat);  // sqrt of (1,1) in Covariance Matrix -> error on fTt
  fT0TtCov =
    -Rsum / det_cov_mat;  // (0,1)=(1,0) in Covariance Matrix -> cov(fT0,fTt)
...
Double_t CbmTofTracklet::UpdateTt() {
  ...
  for (UInt_t iHL = 0; iHL < fhit.size() - 1; iHL++) {
    ...
    for (UInt_t iHH = iHL + 1; iHH < fhit.size(); iHH++) {
      ...
      {
        dTt +=
          (fhit[iHH].GetTime() - fhit[iHL].GetTime())
          / (fhit[iHH].GetR()
             - fhit[iHL]
                 .GetR());  // only valid for tracks from nominal vertex (0,0,0)
        iNt++;
      }
    }
  }
...
Double_t CbmTofTracklet::GetXdif(Int_t iDetId, CbmTofHit* pHit) {
  ...
  if (1) {
    for (UInt_t iHL = 0; iHL < fhit.size() - 1; iHL++) {
      if (iDetId == fTofDet[iHL] || 0 == fTofDet[iHL])
        continue;  // exclude faked hits
      for (UInt_t iHH = iHL + 1; iHH < fhit.size(); iHH++) {
        if (iDetId == fTofDet[iHH] || 0 == fTofDet[iHH])
          continue;  // exclude faked hits
        //dTt+=(fhit[iHH].GetTime()-fhit[iHL].GetTime())/(fhit[iHH].GetR()-fhit[iHL].GetR()); // for projective geometries only !!!
        dTx += (fhit[iHH].GetX() - fhit[iHL].GetX())
               / (fhit[iHH].GetZ() - fhit[iHL].GetZ());
        iNref++;
      }
    }
    dTx /= iNref;
  ...
  if (1) {
    for (UInt_t iHL = 0; iHL < fhit.size() - 1; iHL++) {
      ...
      for (UInt_t iHH = iHL + 1; iHH < fhit.size(); iHH++) {
        if (iDetId == fTofDet[iHH] || 0 == fTofDet[iHH])
          continue;  // exclude faked hits
        dTy += (fhit[iHH].GetY() - fhit[iHL].GetY())
               / (fhit[iHH].GetZ() - fhit[iHL].GetZ());
        iNref++;
      }
    }
  ...
  if (1) {
    for (UInt_t iHL = 0; iHL < fhit.size() - 1; iHL++) {
      ...
      for (UInt_t iHH = iHL + 1; iHH < fhit.size(); iHH++) {
        ...
        //dTt+=(fhit[iHH].GetTime()-fhit[iHL].GetTime())/(fhit[iHH].GetR()-fhit[iHL].GetR()); // for projective geometries only !!!
        Double_t dDist =
          TMath::Sqrt(TMath::Power((fhit[iHH].GetX() - fhit[iHL].GetX()), 2)
...
const Double_t*
CbmTofTracklet::GetPoint(Int_t n) {  // interface to event display: CbmTracks
  fP[0] = fhit[n].GetX();
  fP[1] = fhit[n].GetY();
  fP[2] = fhit[n].GetZ();
  fP[3] = fhit[n].GetTime();
...
const Double_t*
CbmTofTracklet::GetFitPoint(Int_t n) {  // interface to event display: CbmTracks
...
void CbmTofTracklet::PrintInfo() {
  LOG(info) << Form(
    "TrklInfo: Nhits %d, Tt %6.3f, stations: ", GetNofHits(), GetTt());
  LOG(info);
  for (Int_t iH = 0; iH < GetNofHits(); iH++) {
    LOG(info) << Form(
      "  Hit %2d: Ind %5d, det 0x%08x, addr 0x%08x, chi %6.3f, ae[]= %6.4f",
      iH,
      fTofHit[iH],
      fTofDet[iH],
      fhit[iH].GetAddress(),
      fMatChi[iH],
      fhit[iH].GetTimeError());
  }
}

Double_t CbmTofTracklet::Dist3D(CbmTofHit* pHit0, CbmTofHit* pHit1) {
  Double_t dDist =
    TMath::Sqrt(TMath::Power((pHit0->GetX() - pHit1->GetX()), 2)
    ...
    for (UInt_t iHL = 0; iHL < iNhit - 1; iHL++) {
      for (UInt_t iHH = iHL + 1; iHH < iNhit; iHH++) {
        dTt += (fhit[iHH].GetTime() - fhit[iHL].GetTime())
               / (fhit[iHH].GetR() - fhit[iHL].GetR());
        iNt++;
      }
    }
  }
  if (iNt == 0) {
    LOG(debug) << " CbmTofTracklet::GetRefVel: No valid hit pair ";
  } else {
