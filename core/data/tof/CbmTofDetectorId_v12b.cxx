...
CbmTofDetectorId_v12b::CbmTofDetectorId_v12b()
  ...
  modulemask =
    ((maskarray[0] << shiftarray[0]) | (maskarray[1] << shiftarray[1])
     | (maskarray[2] << shiftarray[2]) | (maskarray[3] << shiftarray[3])
     | (0 << shiftarray[4]) | (maskarray[5] << shiftarray[5]));
}

CbmTofDetectorInfo
CbmTofDetectorId_v12b::GetDetectorInfo(const Int_t detectorId) {
  for (Int_t i = 0; i < array_length; i++) {
...
Int_t CbmTofDetectorId_v12b::SetDetectorInfo(const CbmTofDetectorInfo detInfo) {
  return ((detInfo.fDetectorSystem << shiftarray[0])
          ...
          | (detInfo.fGap << shiftarray[4]) | (detInfo.fCell << shiftarray[5]));
}
