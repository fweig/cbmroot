...
// -------          Constructor  ----------------
CbmTrdRawMessageSpadic::CbmTrdRawMessageSpadic( std::uint8_t channelId, std::uint8_t elinkId, std::uint8_t crobId, std::uint16_t criId, std::uint8_t hitType, std::uint8_t nrSamples, bool multiHit,	std::uint64_t fullTime, std::vector<std::int16_t> samples)
...
void CbmTrdRawMessageSpadic::SetSample(std::int16_t value, std::uint8_t pos)
{
	...
	if ((std::uint8_t)(pos + 1) > fSamples.size())
	{
		fSamples.resize(pos + 1);
	}
	fSamples[pos] = value;
