 ...
 */
  ...
  if(!fNCols){  // first digi 
    ...
    else if(terminator>0) SetProfileStop();
...
void CbmTrdCluster::ReInit(Int_t address, Int_t row, Int_t time)
{
  ...
  SetNRows(row); SetProfileStart(0); SetProfileStop(0);
  if(time>=0xffff)
...
Bool_t CbmTrdCluster::Merge(CbmTrdCluster *second)
{
  ...
  if(second->fStartCh+second->fNCols == fStartCh &&
     !second->HasOpenStop() && !HasOpenStart()){
//     cout<<"Merge before with "<<second->ToString();
  ...
  if(fStartCh+fNCols == second->fStartCh &&
     !HasOpenStop() && !second->HasOpenStart()) {
//     cout<<"Merge after  with "<<second->ToString();
  ...
  // special care for FASP clusters which can be merged also on pairing neighboring
  if(HasTrianglePads()){
    if((fStartCh+fNCols-1 == second->fStartCh) &&
      HasOpenStop() && second->HasOpenStart()) { // need to merge digi
      fNCols  += second->fNCols-1;
      fStartTime = TMath::Min(fStartTime, second->fStartTime);      
      AddDigis(second->GetDigis());
...
string CbmTrdCluster::ToString() const
{
  ...
  ss<<"CbmTrdCluster: mod="<<GetAddress()<<" t0="<<fStartTime<<" R="<<(Int_t)GetRow()<<" "<<(HasTrianglePads()?"T":"R")<<"Chs=";
  ss<<(HasOpenStart()?"/":"|");
