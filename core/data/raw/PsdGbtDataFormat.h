...
namespace PsdData {
   ...
   struct PsdEventHeaderAB{
      ...
      void printout() {
         printf("magic word AB: %u; hits number: %u; microslice: %llu\n", uMagicWordAB, uHitsNumber, static_cast<long long unsigned int>(ulMicroSlice));
      }
   ...
   struct PsdHitHeader{
      ...
      void printout() {
         printf("waveform points: %u; hit channel: %u; signal charge: %u; zero level: %u\n", uWfmPoints, uHitChannel, uSignalCharge, uZeroLevel);
      }
