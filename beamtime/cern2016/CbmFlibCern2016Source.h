...
class CbmFlibCern2016Source : public FairSource
{
  public:
    ...
    void AddUnpacker(CbmTSUnpack* unpacker, Int_t flibId, ECbmModuleId detId)
    { 
      fUnpackers.insert ( std::pair<Int_t,CbmTSUnpack*>(flibId,unpacker) ); 
      fDetectorSystemMap.insert ( std::pair<ECbmModuleId,Int_t>(detId,flibId) ); 
    }    
  ...
  private:
    ...
    std::map<ECbmModuleId, Int_t> fDetectorSystemMap; //! Map detector system id to flib system id 
    ...
    ClassDef(CbmFlibCern2016Source, 2)
};
