...
bool get4v1x::Message::convertFromOld(void* src)
{
   data = 0;
...
uint64_t get4v1x::Message::getMsgFullTime(uint32_t epoch) const
{
   switch (getMessageType()) {
      case MSG_HIT:
...
double get4v1x::Message::getMsgFullTimeD(uint32_t epoch) const
{
   switch (getMessageType()) {
      case MSG_EPOCH2:
         return FullTimeStamp2(getEpoch2Number(), 0) / 20. + 512.;
...
uint64_t get4v1x::Message::CalcDistance(uint64_t start, uint64_t stop)
{
   if (start>stop) {
...
double get4v1x::Message::CalcDistanceD(double start, double stop)
{
   if (start>stop) {
      stop += 0x3FFFFFFFFFFFLLU;
      if (start>stop) {
...
void get4v1x::Message::printDataCout(unsigned kind, uint32_t epoch) const
{
...
void get4v1x::Message::printDataLog(unsigned kind, uint32_t epoch) const
{
 ...
 * \li get4v1x::msg_print_Prefix (1) - ROC number and message type
 * \li get4v1x::msg_print_Data   (2) - print all message specific data fields
 * \li get4v1x::msg_print_Hex    (4) - print data as hex dump
 * \li get4v1x::msg_print_Human  (8) - print in human readable format
 *
...
//void get4v1x::Message::printData(std::ostream& os, unsigned kind, uint32_t epoch) const
void get4v1x::Message::printData(unsigned outType, unsigned kind, uint32_t epoch) const
{
   char buf[256];
   // Use a TString in case of FairLogger as the log level is decided only later
   TString sLogBuff = "";
...
//      os << buf;
      if( msg_print_Cout == outType)
         std::cout << buf;
      else sLogBuff = TString::Format("%s", buf );
   }
...
//      uint8_t* arr = static_cast<uint8_t*> ( &data );
      ...
      switch (getMessageType()) {
        case MSG_NOP:
         ...
         case MSG_SYS: {
            ...
            switch (getSysMesType()) {
              ...
              case SYSMSG_GET4_EVENT:
              {
                 ...
                 uint32_t get4_24b_er_chan = getGet4V10R24ErrorChan();
                 uint32_t get4_24b_er_edge = getGet4V10R24ErrorEdge();
                 uint32_t get4_24b_er_unus = getGet4V10R24ErrorUnused();
                 uint32_t get4_24b_er_chip = getGet4V10R24ErrorChip();
                 uint32_t get4_24b_er_code = getGet4V10R24ErrorData();

                 snprintf(sysbuf, sizeof(sysbuf),
                   "Get4:0x%02x Ch:0x%01x Edge:%01x Unused:%04x ErrCode:0x%02x - GET4 V1 Error Event",
                   get4_24b_er_chip, get4_24b_er_chan, get4_24b_er_edge, get4_24b_er_unus, get4_24b_er_code);

                 /*
                 // For csv files output
                 snprintf(sysbuf, sizeof(sysbuf),
                       "Get4 V1 24 bits, chip %02d, type:0x10 => ;Error ;;;;;;;;;;;;;;%02x ;Channel; %1d ;Edge; %1d;Unused; %4x;",
                        get4_24b_er_chip, get4_24b_er_code, get4_24b_er_chan, get4_24b_er_edge, get4_24b_er_unus );
                       */
                 break;
              } //
              ...
              {
                 // 32 bits readout mode message
                 uint32_t get4_32b_chip = getGet4V10R32ChipId();
                 uint32_t get4_32b_type    = getGet4V10R32MessageType();
                 ...
                 switch(get4_32b_type)
                 {
                    case GET4_32B_EPOCH: // epoch message
                    {
                        ...
                        snprintf(sysbuf, sizeof(sysbuf),
                                "Get4 V1 32 bits, chip %02d, type:0x%02x => Epoch 0x%06x Sync: %1d",
                                get4_32b_chip, get4_32b_type, get4_32b_ep_epoch, get4_32b_ep_sync );

                        /*
                        snprintf(sysbuf, sizeof(sysbuf),
                                "Get4 V1 32 bits, type:0x%02x => ;chip;%02d;Epoch; %08d; Sync:; %1d;",
                                get4_32b_type, get4_32b_chip, get4_32b_ep_epoch, get4_32b_ep_sync );
                                */
                        break;
                    }
                    case GET4_32B_SLCM: // slow control message
                    {
                        ...
                        uint32_t get4_32b_sl_edge = getGet4V10R32SlEdge();
                        uint32_t get4_32b_sl_chan = getGet4V10R32SlChan();

                        snprintf(sysbuf, sizeof(sysbuf),
                           "Get4 V1 32 bits, chip %02d, type:0x%02x => Slow control Chan:%01d Edge:%01d Type:%01x Data:0x%06x",
                           get4_32b_chip, get4_32b_type, get4_32b_sl_chan, get4_32b_sl_edge,
                           get4_32b_sl_type, get4_32b_sl_data );

                        /*
                        // For csv files output
                        snprintf(sysbuf, sizeof(sysbuf),
                           "Get4 V1 32 bits, type:0x%02x => ;chip;%02d;Slow control ;;;;;;;;;;;;;;Chan:;%01d ;Edge:;%01d ;Type:;%01x ;Data:;0x%06x;",
                           get4_32b_type, get4_32b_chip, get4_32b_sl_chan, get4_32b_sl_edge,
                           get4_32b_sl_type, get4_32b_sl_data );
                        */
                        break;
                    }
                    case GET4_32B_ERROR: // error event
                    {
                        ...
                        uint32_t get4_32b_er_chan = getGet4V10R32ErrorChan();
                        uint32_t get4_32b_er_edge = getGet4V10R32ErrorEdge();

                        snprintf(sysbuf, sizeof(sysbuf),
                          "Get4 V1 32 bits, chip %02d, type:0x%02x => Error 0x%02x Channel %1d Edge %1d Unused:%04x",
                          get4_32b_chip, get4_32b_type, get4_32b_er_code, get4_32b_er_chan,
                          get4_32b_er_edge, get4_32b_er_unus );

                        /*
                        // For csv files output
                        snprintf(sysbuf, sizeof(sysbuf),
                          "Get4 V1 32 bits, type:0x%02x => ;chip;%02d;Error ;;;;;;;;;;;;;;%02x ;Channel; %1d ;Edge; %1d;Unused; %4x;",
                          get4_32b_type, get4_32b_chip, get4_32b_er_code, get4_32b_er_chan,
                          get4_32b_er_edge, get4_32b_er_unus );
                                */
                        break;
                    }
                    case GET4_32B_DATA: // data event
                    {
                        ...
                        uint32_t get4_32b_dat_ts   = getGet4V10R32HitTs();
                        uint32_t get4_32b_dat_chan = getGet4V10R32HitChan();
                        uint32_t get4_32b_dat_dll  = getGet4V10R32HitDllFlag();

                        snprintf(sysbuf, sizeof(sysbuf),
                          "Get4 V1 32 bits, chip %02d, type:0x%02x => Data Dll %1d Channel %1d Ts:0x%03x Ft:0x%02x Tot:0x%02x",
                          get4_32b_chip, get4_32b_type, get4_32b_dat_dll, get4_32b_dat_chan,
                          get4_32b_dat_ts, get4_32b_dat_ft, get4_32b_dat_tot);

                        /*
                        // For csv files output
                        snprintf(sysbuf, sizeof(sysbuf),
                          "Get4 V1 32 bits, type:0x%02x => ;chip;%02d;Data ;;;;Dll; %1d ;Channel; %1d ;Ts:;%04d ;Ft:;%03d ;Tot:;%03d;",
                          get4_32b_type, get4_32b_chip, get4_32b_dat_dll, get4_32b_dat_chan,
                          get4_32b_dat_ts, get4_32b_dat_ft, get4_32b_dat_tot);
                          */
                        break;
                    }
                    default:
                     snprintf(sysbuf, sizeof(sysbuf),
                                "Get4 V1 32 bits, unknown type:0x%02x ", get4_32b_type );
                    break;
                 } // switch(get4_32b_type)
                 //snprintf(sysbuf, sizeof(sysbuf),
                 //   "Get4 V1 32 bits, full data:0x%08x %08x ", getFieldBE(32,  32), getFieldBE(0,  32) );
         ...
         case MSG_GET4_SLC:
         {
            // 32b GET4 slow control message, new "true" ROC support

            snprintf(buf, sizeof(buf),
               "Get4 V1 32 bits Slow control in \"new true\" readout mode "
               "=> not supported, check CbmGet4v1xToolsDef files" );
            break;
         }
         case MSG_GET4_32B:
         {
            ...
            snprintf(buf, sizeof(buf),
               "Get4 V1 32 bits Data in \"new true\" readout mode "
               "=> not supported, check CbmGet4v1xToolsDef files" );
...
uint32_t get4v1x::Message::RawSize(int fmt)
{
   switch (fmt) {
...
bool get4v1x::Message::assign(void* src, int fmt)
{
   switch (fmt) {
      case formatEth1:
         convertFromOld(src);
...
bool get4v1x::Message::copyto(void* tgt, int fmt)
{
   switch (fmt) {
      case formatEth1:
         convertToOld(tgt);
