...
CbmTSUnpackSpadic11OnlineMonitor::CbmTSUnpackSpadic11OnlineMonitor(Bool_t highPerformance)
  : CbmTSUnpack(),
...
Bool_t CbmTSUnpackSpadic11OnlineMonitor::DoUnpack(const fles::Timeslice& ts, size_t component)
{
  ...
  return kTRUE;
	...
	if (it == groupToExpMap.end()) {
      ...
      {
	if(SpadicChannel > 31 || groupId > 1) LOG(debug) << "CbmTSUnpackSpadic11OnlineMonitor::                     ChId " << SpadicChannel << "  GroupId: " << groupId;
	...
	if (SpadicChannel < 0 || SpadicChannel > 31){
	  if (SpadicChannel !=-1) LOG(debug) << "CbmTSUnpackSpadic11OnlineMonitor::GetChannelOnPadPlane ChId " << SpadicChannel << "  GroupId: " << groupId;
	  return -1;
	} else {
