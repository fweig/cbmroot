...
CbmTrdRawBeamProfile::CbmTrdRawBeamProfile()
  : FairTask("CbmTrdRawBeamProfile"),
    ...
    fTrbBuffer()
{
  LOG(debug) << "Default Constructor of CbmTrdRawBeamProfile";
}
...
CbmTrdRawBeamProfile::~CbmTrdRawBeamProfile()
{
  ...
  fTimeBuffer.clear();
    ...
    LOG(debug) << "Destructor of CbmTrdRawBeamProfile";
}

  // ----  Initialisation  ----------------------------------------------
  void CbmTrdRawBeamProfile::SetParContainers()
  {
    LOG(debug) << "SetParContainers of CbmTrdRawBeamProfile";
  ...
  {
    LOG(debug) << "Initilization of CbmTrdRawBeamProfile";
...
//      for (secondTimeIt ; secondTimeIt != (MSIt->second).end(); secondTimeIt++){
  ...
  {
    LOG(info) << "CbmTrdRawBeamProfile::Clusterizer              :  SuperEpoch: " << fSuperEpoch << "  Epoch: " << fEpoch;
    /*
  ...
  {
    if (raw == NULL)
      LOG(error) << "CbmTrdRawBeamProfile::GetSectorID: CbmSpadicRawMessage == NULL";
    return 0;
  }
  Int_t CbmTrdRawBeamProfile::GetRowID(CbmSpadicRawMessage* raw)
  {
    if (raw == NULL)
      LOG(error) << "CbmTrdRawBeamProfile::GetRowID: CbmSpadicRawMessage == NULL";
    Int_t chID = raw->GetChannelID();
    Int_t sourceA = raw->GetSourceAddress();
    switch (sourceA) {
    case (SpadicBaseAddress+0):  // first spadic
  ...
  {
    if (raw == NULL)
      LOG(error) << "CbmTrdRawBeamProfile::GetColumnID: CbmSpadicRawMessage == NULL";
    Int_t chID = raw->GetChannelID();
    Int_t sourceA = raw->GetSourceAddress();
    switch (sourceA) {
    case (SpadicBaseAddress+0):  // first spadic
  ...
  Int_t CbmTrdRawBeamProfile::GetChannelOnPadPlane(Int_t SpadicChannel)
  {
    ...
    if (SpadicChannel < 0 || SpadicChannel > 31){
      LOG(error) << "CbmTrdRawBeamProfile::GetChannelOnPadPlane ChId " << SpadicChannel;
      return -1;
    } else {
...
Int_t CbmTrdRawBeamProfile::GetCombiID(CbmSpadicRawMessage* raw)
{
  ...
  void CbmTrdRawBeamProfile::Finish()
  {
    Clusterizer();
    TimeClustering2015CernSPS();
    LOG(debug) << "Finish of CbmTrdRawBeamProfile";
    // Write to file
    fHM->WriteToFile();
    // Update Histos and Canvases

    LOG(info) << "CbmTrdRawBeamProfile::Finish Container:            " << fContainerCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Spadic Messages:      " << fSpadicMessageCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Nxyter Messages:      " << fNxyterMessageCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Trb Messages:         " << fTrbMessageCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Infos:                " << fInfoCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Hits:                 " << fHitCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Multihits:            " << fMultiHitCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Errors:               " << fErrorCounter << " (stoptype != 0 && != 3)";
    LOG(info) << "CbmTrdRawBeamProfile::Finish LostHits:             " << fLostHitCounter << " (buffer full)";
    LOG(info) << "CbmTrdRawBeamProfile::Finish Identical Messages:   " << fDoubleCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Fragemented Signals:  " << fFragmentedCounter;
    LOG(info) << "CbmTrdRawBeamProfile::Finish Digis:                " << fDigis->GetEntriesFast();
    LOG(info) << "CbmTrdRawBeamProfile::Finish Clusters:             " << fClusters->GetEntriesFast();
  }
