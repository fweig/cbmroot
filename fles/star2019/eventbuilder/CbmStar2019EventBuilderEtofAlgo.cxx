...
//      LOG(info) << Form( "TS %5d Start %12f Stop %12f", fulCurrentTsIndex, fdTsStartTime, fdTsStopTimeCore );
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      if( fdStarTriggerDelay[ uGdpb ] < 0.0 )
      {
         ...
         if( fdStarTriggerDelay[ uGdpb ] + fdStarTriggerWinSize[ uGdpb ] < 0.0 )
         {
            ...
            fvdMessCandidateTimeStop[ uGdpb ]  = fdTsStopTimeCore + fdAllowedTriggersSpread + 2.0 * fdStarTriggerWinSize[ uGdpb ]; // + fdStarTriggerWinSize[ uGdpb ];
            ...
            else
            {
               ...
               fvdMessCandidateTimeStart[ uGdpb ] = fdTsStartTime;
               // << Accept more than needed as this should be safer and small amounts >>
               fvdMessCandidateTimeStop[ uGdpb ]  = fdTsStopTimeCore + fdAllowedTriggersSpread + 2.0 * fdStarTriggerWinSize[ uGdpb ]; // + fdStarTriggerDelay[ uGdpb ];
               fvdTrigCandidateTimeStart[ uGdpb ] = fdTsStartTime    + fdAllowedTriggersSpread - fdStarTriggerDelay[ uGdpb ];
               fvdTrigCandidateTimeStop[ uGdpb ]  = fdTsStopTimeCore + fdAllowedTriggersSpread - fdStarTriggerDelay[ uGdpb ];
         ...
         else
         {
            ...
            fvdMessCandidateTimeStop[ uGdpb ]  = fdTsStopTimeCore + fdAllowedTriggersSpread + fdStarTriggerDelay[ uGdpb ] + fdStarTriggerWinSize[ uGdpb ];
   ...
   for( UInt_t uMsCompIdx = 0; uMsCompIdx < fvMsComponentsList.size(); ++uMsCompIdx )
   {
      ...
      for( fuMsIndex = 0; fuMsIndex < fuNbMsLoop; fuMsIndex ++ )
      {
         ...
         {
            LOG(error) << "Failed to process ts " << fulCurrentTsIndex << " MS " << fuMsIndex
...
*/
   ...
   if( it == fGdpbIdIndexMap.end() )
   {
      ...
      if( iWarn == 100)
         LOG(fatal) << "Got max number of Warnings!";
      return kFALSE;
   } // if( it == fGdpbIdIndexMap.end() )
   ...
   for( uint32_t uIdx = 0; uIdx < uNbMessages; uIdx++ )
   {
      ...
      if( fuNbCoreMsPerTs <= fuCurrentMs &&
          kTRUE == fbEpochAfterCandWinFound &&
...
void CbmStar2019EventBuilderEtofAlgo::ProcessStarTrigger( gdpbv100::Message mess )
{
   ...
   switch( iMsgIndex )
   {
      case 0:
      {
         ...
         if( fbMonitorMode && fbDebugMonitorMode )
         {
            ...
            if( fuCurrentMs < fuNbCoreMsPerTs )
               ...
               else fhRawTriggersStatsOver->Fill( 0., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );

            if( kTRUE == fbTriggerFoundA )
               LOG(info) << Form( "Problem: Overwritting a A trigger in TS %12llu MS %3u Core? %u",
      ...
      case 1:
      {
         ...
         if( fbMonitorMode && fbDebugMonitorMode )
         {
            ...
            if( fuCurrentMs < fuNbCoreMsPerTs )
               ...
               else fhRawTriggersStatsOver->Fill( 1., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );

            if( kFALSE == fbTriggerFoundA )
               LOG(info) << Form( "Problem: Found B trigger before A in TS %12llu MS %3u",
                                    fulCurrentTsIndex, fuMsIndex );

            if( kTRUE == fbTriggerFoundB )
               LOG(info) << Form( "Problem: Overwritting a B trigger in TS %12llu MS %3u Core? %u",
                                    fulCurrentTsIndex, fuMsIndex, fuCurrentMs < fuNbCoreMsPerTs )
                         << Form( " (%u %u %u)",
                                    fbTriggerFoundA, fbTriggerFoundB, fbTriggerFoundC);
         } // if( fbMonitorMode && fbDebugMonitorMode )
      ...
      case 2:
      {
         ...
         if( fbMonitorMode && fbDebugMonitorMode )
         {
            ...
            if( fuCurrentMs < fuNbCoreMsPerTs )
               ...
               else fhRawTriggersStatsOver->Fill( 2., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );

            if( kFALSE == fbTriggerFoundB || kFALSE == fbTriggerFoundA )
               LOG(info) << Form( "Problem: Found C trigger before A or B in TS %12llu MS %3u",
                                    fulCurrentTsIndex, fuMsIndex );

            if( kTRUE == fbTriggerFoundC )
               LOG(info) << Form( "Problem: Overwritting a C trigger in TS %12llu MS %3u Core? %u",
                                    fulCurrentTsIndex, fuMsIndex, fuCurrentMs < fuNbCoreMsPerTs )
                         << Form( " (%u %u %u)",
                                    fbTriggerFoundA, fbTriggerFoundB, fbTriggerFoundC);
         } // if( fbMonitorMode && fbDebugMonitorMode )
      ...
      case 3:
      {
         ...
         {
            if( fbMonitorMode && fbDebugMonitorMode )
               LOG(info) << Form( "Problem: Found D trigger before A or B or C in TS %12llu MS %3u Core? %u",
                                    fulCurrentTsIndex, fuMsIndex, fuCurrentMs < fuNbCoreMsPerTs )
                         << Form( " (%u %u %u) token %5u",
                                    fbTriggerFoundA, fbTriggerFoundB, fbTriggerFoundC,
                                    uNewToken );
         ...
         if( fbMonitorMode && fbDebugMonitorMode )
         {
            ...
            if( fuCurrentMs < fuNbCoreMsPerTs )
               ...
               else fhRawTriggersStatsOver->Fill( 3., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );
         } // if( fbMonitorMode && fbDebugMonitorMode )
             ...
             dTriggerTime < fvdTrigCandidateTimeStop[ fuGdpbNr ] )
         {
            ...
            if( fbMonitorMode && fbDebugMonitorMode )
            {
               ...
               if( fuCurrentMs < fuNbCoreMsPerTs )
                  ...
                  else fhRawTriggersStatsSel->Fill( 2., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );
            } // if( fbMonitorMode && fbDebugMonitorMode )
...
//            ULong64_t ulGdpbTsDiff = ulNewGdpbTsFull - fvulGdpbTsFullLast[fuGdpbNr];
         ...
         if( fbMonitorMode && fbDebugMonitorMode )
         {
            ...
            if( fuCurrentMs < fuNbCoreMsPerTs )
               ...
               else fhRawTriggersStatsOver->Fill( 4., fUnpackPar->GetGdpbToSectorOffset() + fuGdpbNr );
            ...
            if( (dTriggerTime - fdMsTime) / 1000.0 < -1000 )
            {
               ...
               LOG(info) << Form( "Full token, gDPB TS LSB bits: 0x%16llx, STAR TS MSB bits: 0x%16llx, token is %4u",
...
*/
               else StoreMessageInBuffer( fullMess, fuGdpbNr );
...
//            fvbGdpbLastResyncPattern[ fuGdpbNr ][ 32 * usIndex + uBit ] = bPattern[ uBit ];
   ...
   if( kuNbMsgPerPattern - 1 == usIndex )
      UpdateStatusPatternCurrGdpb();
...
gdpbv100::FullMessage CbmStar2019EventBuilderEtofAlgo::CreateStatusMessage(
            uint16_t uGdpbId, uint32_t uIndex, std::pair< uint64_t, std::bitset< kuNbAsicPerGdpb > > statusIn )
{
...
Bool_t CbmStar2019EventBuilderEtofAlgo::BuildEvents()
{
   ...
   std::vector< std::vector<
            std::pair< uint64_t, std::bitset< kuNbAsicPerGdpb > > >::iterator > itStatUpdtStart; /// [sector]
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
...
*/
               ...
               while( fvSectorStatusPattern[ uGdpb ].end() != itStatUpdtStart[ uGdpb ] &&
                      ( (*itStatUpdtStart[ uGdpb ]).first ) * gdpbv100::kuCoarseCounterSize <= (*itTrigger[ uGdpb ]).GetFullGdpbTs() )
               {
...
*/
               ...
               if( dWinBeg < dMessTime && dMessTime < dWinEnd )
               {
                  ...
                  if( fbMonitorMode )
                  {
                     ...
                     {
                        fvhHitsTimeToTriggerSelVsDaq[ uGdpb ]->Fill(  dMessTime - gdpbv100::kdClockCycleSizeNs * (*itTrigger[ uGdpb ]).GetFullGdpbTs(),
                                                                      usFirstStarDaqCmd );
                        fvhHitsTimeToTriggerSelVsTrig[ uGdpb ]->Fill( dMessTime - gdpbv100::kdClockCycleSizeNs * (*itTrigger[ uGdpb ]).GetFullGdpbTs(),
                                                                      usFirstStarTrigCmd );
                     } // if( fbDebugMonitorMode )
                  } // if( fbMonitorMode )
               } // if( dWinBeg < dMessTime && dMessTime < dWinEnd )
...
Bool_t CbmStar2019EventBuilderEtofAlgo::CreateHistograms()
{
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      ...
      if( kTRUE == fbDebugMonitorMode )
      {
         fvhHitsTimeToTriggerRawPulser.push_back( new TH2D(
            ...
            Form( "Time to trigger for all neighboring hits within pulser TOT range in sector %2u; t - Ttrigg [ns]; TOT [bins]; Hits []", uSector ),
            2000, -5000, 5000,
             256,     0,  256) );
   ...
   if( kTRUE == fbDebugMonitorMode )
   {
      ...
      fhRawTriggersStatsOver = new TH2I(
         "hRawTriggersStatsOver",
         "Raw triggers in Overlap MS statistics per sector; ; Sector []; Messages []",
         5, 0, 5,
         12, 13, 25  );
...
Bool_t CbmStar2019EventBuilderEtofAlgo::FillHistograms()
{
   ...
   for( UInt_t uEvent = 0; uEvent < uNbEvents; ++uEvent )
   {
      ...
      if( fbDebugMonitorMode )
         ...
         else fhEventSizeEvolution->Fill( dEventTimeMin, uEventSize );
      if( fbDebugMonitorMode )
         fhEventNbEvolution->Fill( dEventTimeSec );
         else fhEventNbEvolution->Fill( dEventTimeMin );
