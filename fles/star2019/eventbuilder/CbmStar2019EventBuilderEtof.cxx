...
//                   << " in " << vCanvases[ uCanv ].second.data();
      ...
      if( fbDebugMonitorMode )
      {
         fhRealTimeDistr = new TH1I( "hEvtBuildRealTimeDistr",
                                     "Realtime for processing a TS in event builder; Realtime [ms]; TS nb []",
                                     100000, 0.0, 100.0 );
         fhRealTimeEvo = new TH2I( "hEvtBuildRealTimeEvo",
                                     "Realtime Processing to duration ratio for processing a TS in event builder vs TS index; TS []; Realtime ratio []; TS Nb []",
                                      1000, 0,    100000,
                                     10000, 0.0,     100.0 );

         fhMeanRealTimeEvo = new TProfile( "hEvtBuildMeanRealTimeEvo",
                                     "Mean Realtime Processing to duration ratio for processing a TS in event builder vs TS index; TS []; Mean Realtime ratio []; TS Nb []",
                                      1000, 0,    100000 );
...
//         Bool_t fbSendEventToStar = kFALSE;
   ...
   if( fbDebugMonitorMode )
   {
      LOG(debug2) << Form( "Real time TS %12lu, Realtime: %12f ns", static_cast<unsigned long>(fulTsCounter), dRealT * 1e9 );
   ...
   {
      if( fbDebugMonitorMode )
         LOG(debug) << Form( "New min Real time TS %12lu, Real time: %9.6f ms Old Min %9.6f Diff %.9f",
   ...
   {
      if( fbDebugMonitorMode )
         LOG(debug) << Form( "New max Real time TS %12lu, Real time: %9.6f ms Old Max %9.6f Diff %.9f",
   ...
   if( 0 == fulTsCounter % 10000 ) {
      fdRealTime /= 10000;
      LOG(info) << Form( "Processed %12lu TS, Real time: %6.3f ms/TS (Min %6.3f, Max %6.3f), Events: %12lu (%9lu since last print)",
                        static_cast<unsigned long>(fulTsCounter), fdRealTime, fdRealTimeMin, fdRealTimeMax, 
                        static_cast<unsigned long>(fulNbEvents), static_cast<unsigned
long>(fulNbEventsSinceLastPrintout) );
      fdRealTime = 0.0;
      fdRealTimeMin = 1e6;
      fdRealTimeMax = 0.0;
      fulNbEventsSinceLastPrintout = 0;
