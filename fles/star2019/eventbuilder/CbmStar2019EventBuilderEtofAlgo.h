...
class CbmStar2019EventBuilderEtofAlgo : public CbmStar2019Algo<CbmTofDigi>
{
   ...
   private:
      ...
      Bool_t fbStoreLostEventMsg; //! Switch ON the insertion of the LostEvent messages from GET4s with the critical errors (default is false)
      ...
      std::vector<
         std::vector<
            std::pair< uint64_t,
                       std::bitset< kuNbAsicPerGdpb > > > > fvSectorStatusPattern; //! [sector][Update]<time, [asic]> Exclude from dictionnary
