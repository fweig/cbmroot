...
private:
  ...
  std::vector<std::vector<gdpbv100::FullMessage>>
    fvvBufferMessages;  //! [sector]
  std::vector<std::vector<CbmTofStarTrigger2019>>
    fvvBufferTriggers;                                 //! [sector]
  ...
  std::vector<std::bitset<kuNbAsicPerGdpb>>
    fvbGdpbLastMissmatchPattern;  //! [sector][asic] Exclude from dictionnary
  std::vector<std::bitset<kuNbAsicPerGdpb>>
    fvbGdpbLastEnablePattern;  //! [sector][asic] Exclude from dictionnary
  std::vector<std::bitset<kuNbAsicPerGdpb>>
    fvbGdpbLastResyncPattern;  //! [sector][asic] Exclude from dictionnary
  ...
  std::vector<TH2*>
    fvhHitsTimeToTriggerRawPulser;  //! [sector], extra monitor for debug
  std::vector<TH1*> fvhHitsTimeToTriggerSel;  //! [sector]
  std::vector<TH2*>
    fvhHitsTimeToTriggerSelVsDaq;  //! [sector], extra monitor for debug
  std::vector<TH2*>
    fvhHitsTimeToTriggerSelVsTrig;  //! [sector], extra monitor for debug
  std::vector<TH1*> fvhTriggerDt;   //! [sector], extra monitor for debug
  std::vector<TH1*>
    fvhTriggerDistributionInTs;  //! [sector], extra monitor for debug
  std::vector<TH1*>
    fvhTriggerDistributionInMs;  //! [sector], extra monitor for debug
  std::vector<TH1*>
    fvhMessDistributionInMs;              //! [sector], extra monitor for debug
  TH1* fhEventNbPerTs;                    //!
  ...
  gdpbv100::FullMessage CreateStatusMessage(
    uint16_t uGdpbId,
    uint32_t uIndex,
  ...
  CbmStar2019EventBuilderEtofAlgo
  operator=(const CbmStar2019EventBuilderEtofAlgo&);
