...
Bool_t CbmStar2019MonitorAlgo::InitParameters()
{
   ...
   if( kTRUE == fbIgnoreCriticalErrors )
      LOG( warning ) << "Monitor set to ignore critical GET4 errors!!! No printout will be delivered for those!!!!";
   ...
   if( -1 < fiSectorIndex )
   {
      ...
      if( fuNrOfGdpbs <= static_cast< UInt_t >( fiSectorIndex ) )
         ...
         else LOG(info) << "Selected sector " << fiSectorIndex + fUnpackPar->GetGdpbToSectorOffset()
   ...
   if( kTRUE == fbDebugMonitorMode )
      fuNbMissmatchPattern.resize(  fuNrOfGdpbs, 0 );
...
*/
         ...
         default:
            LOG(fatal) << "Message type " << std::hex
...
void CbmStar2019MonitorAlgo::ProcessEpoch( gdpbv100::Message mess )
{
   ...
   if( bDataLoss )
      fhGdpbEpochFlags->Fill( fuCurrSector, 1 );

   if( bEpochLoss )
      fhGdpbEpochFlags->Fill( fuCurrSector, 2 );
...
//   Double_t dHitTot   = uTot;     // in bins
   ...
   if( 4096 <= uCts )
   {
      LOG(debug) << "CbmStar2019MonitorAlgo::ProcessHit => Coarse time above 4096 detected."
                   << Form( " sector %02u GET4 %03u Channel %u, TS %8llu MS %3u (MS time %12llu)",
                            fuCurrDpbIdx + fUnpackPar->GetGdpbToSectorOffset(),
                            fuGet4Id, uChannel, fulCurrentTsIdx, fuMsIndex, fulCurrentMsIdx ) ;
   } // if( 4096 <= uCts )
   ...
   switch( uErrorType )
   {
      ...
      case gdpbv100::GET4_V2X_ERR_DLL_RESET:
      {
         ...
         if( kFALSE == fbIgnoreCriticalErrors )
            LOG(info) << " +++++++ > gDPB: " << std::hex << std::setw(4) << fuCurrDpbIdx
                       ...
                       << ", Data = " << std::hex << std::setw(2) << uErrorType
      ...
      case gdpbv100::GET4_V2X_ERR_SEQUENCE_ER:
      {
         ...
         LOG(debug) << " +++++++ >gDPB: " << std::hex << std::setw(4) << fuCurrDpbIdx
                      << std::dec << ", Chip = " << std::setw(2)
                      << mess.getGdpbGenChipId() << ", Chan = " << std::setw(1)
                      << mess.getGdpbSysErrChanId() << ", Edge = "
                      << std::setw(1) << mess.getGdpbSysErrEdge() << ", Empt = "
   ...
   switch( usType )
   {
      case gdpbv100::PATT_MISSMATCH:
      {
         ...
         if( kTRUE == fbDebugMonitorMode && 0 == usIndex )
         {
            ...
            LOG(debug) << Form( "Missmatch pattern message => Type %d, Index %2d, Pattern 0x%08X TS %12llu MS %3u Epoch %12llu",
         ...
         if( kTRUE == fbDebugMonitorMode )
            for( UInt_t uBit = 0; uBit < uNbBits; ++uBit )
            {
               ...
               if( ( uPattern >> uBit ) & 0x1 )
               {
                  ...
                  else fvvbGdpbLastMissmatchPattern[ fuCurrDpbIdx ][ uBadAsic ] = kFALSE;
         ...
         if( kTRUE == fbDebugMonitorMode )
            for( UInt_t uBit = 0; uBit < uNbBits; ++uBit )
            {
               ...
               if( ( uPattern >> uBit ) & 0x1 )
               {
                  ...
                  else fvvbGdpbLastEnablePattern[ fuCurrDpbIdx ][ uAsic ] = kTRUE;
         ...
         if( kTRUE == fbDebugMonitorMode )
            for( UInt_t uBit = 0; uBit < uNbBits; ++uBit )
            {
               ...
               if( ( uPattern >> uBit ) & 0x1 )
               {
                  ...
                  else fvvbGdpbLastResyncPattern[ fuCurrDpbIdx ][ uBadAsic ] = kFALSE;
   ...
   switch( uType )
   {
      ...
      case gdpbv100::GET4_32B_SLC_SPIREAD:
      {
         ...
         LOG(info) << "GET4 Slow Control SPI message, time "
                    ...
                    << std::dec << "\n"
                    << " +++++++ > Chip = "
                    << std::setw(3) << fuGet4Id << ", Chan = "
                    << std::setw(1) << uChan << ", Edge = "
                    << std::setw(1) << uEdge << ", Type = "
                    << std::setw(1) << mess.getGdpbSlcType() << ", "
...
Bool_t CbmStar2019MonitorAlgo::CreateHistograms()
{
   ...
   /*******************************************************************/
   fhMessType = new TH1I( "hMessageType", "Nb of message for each type; Type",
                          1 + gdpbv100::MSG_STAR_TRI_D, 0., 1 + gdpbv100::MSG_STAR_TRI_D);
   ...
   fhSysMessType = new TH1I( "hSysMessType", "Nb of system message for each type; System Type",
                             1 + gdpbv100::SYS_PATTERN, 0., 1 + gdpbv100::SYS_PATTERN);
   ...
   fhGet4MessType = new TH2I( "hGet4MessType", "Nb of message for each type per GET4; GET4 chip # ; Type",
                              fuNrOfGet4, 0., fuNrOfGet4, 4, 0., 4.);
...
//   fhGet4MessType->GetYaxis()->SetBinLabel( 6, "STAR Trigger");
   ...
   fhGet4EpochFlags = new TH2I( "hGet4EpochFlags", "Epoch flags per GET4; GET4 chip # ; Type",
                                fuNrOfGet4, 0., fuNrOfGet4, 4, 0., 4.);
   ...
   fhGdpbAsicSpiCounts = new TH2I( "hGdpbAsicSpiCounts", "SPI messages count per Sector and ASIC; ASIC Idx []; Sector []; SPI msg[]",
                                   fuNrOfGet4PerGdpb, -0.5, fuNrOfGet4PerGdpb - 0.5,
                                   fuNrOfGdpbs, dSectorMin, dSectorMax );

   fhGdpbMessType = new TH2I( "hGdpbMessageType", "Nb of message for each type per Sector; Type; Sector []",
                              1 + gdpbv100::MSG_STAR_TRI_D, 0., 1 + gdpbv100::MSG_STAR_TRI_D,
                              fuNrOfGdpbs, dSectorMin, dSectorMax );
   ...
   fhGdpbSysMessType = new TH2I( "hGdpbSysMessType", "Nb of system message for each type per Sector; System Type; Sector []",
                                 1 + gdpbv100::SYS_PATTERN, 0., 1 + gdpbv100::SYS_PATTERN,
                                 fuNrOfGdpbs, dSectorMin, dSectorMax );
   ...
   fhGdpbSysMessPattType = new TH2I( "hGdpbSysMessPattType", "Nb of pattern message for each type per Sector; Pattern Type; Sector []",
                                     1 + gdpbv100::PATT_RESYNC, 0., 1 + gdpbv100::PATT_RESYNC,
                                     fuNrOfGdpbs, dSectorMin, dSectorMax );
   ...
   fhGdpbEpochFlags = new TH2I( "hGdpbEpochFlags", "Epoch flags per Sector; Sector # ; Type",
                                fuNrOfGdpbs, dSectorMin, dSectorMax,
                                4, 0., 4.);
   ...
   fhGdpbEpochMissEvo = new TH2D( "hGdpbEpochMissEvo", "Epoch Missmatch per second and Sector; Time[s];  Sector #; Missmatch Nb",
                                  fuHistoryHistoSize, 0, fuHistoryHistoSize,
                                  fuNrOfGdpbs, dSectorMin, dSectorMax );

   fhGdpbEndMsBufferNotEmpty = new TH1D( "hGdpbEndMsBufferNotEmpty", "MS where buffer is not empty at end, per Sector; Sector #; Bad MS",
                                  fuNrOfGdpbs, dSectorMin, dSectorMax );

   fhGdpbEndMsDataLost = new TH2D( "hGdpbEndMsDataLost", "Amount of lost data when buffer not empty at end, per MS and Sector; Sector #; Lost Data per bad MS []; Bad MS",
                                  fuNrOfGdpbs, dSectorMin, dSectorMax,
   ...
   if( kTRUE == fbDebugMonitorMode )
   {
      fhNbMissPatternPerMs = new TH2I( "hNbMissPatternPerMs",
            "Nb of missmatch pattern per MS for each sector; Number of pattern messages []; sector []; MS",
            1000, -0.5, 999.5,
            fuNrOfGdpbs, dSectorMin, dSectorMax );

      fhPatternMissmatch = new TH2I( "hPatternMissmatch", "Missmatch pattern integral per Sector; ASIC Pattern []; Sector []",
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      ...
      fvhGdpbGet4MessType.push_back( new TH2I(
               Form("hGdpbGet4MessType_%02u", uGdpb),
               Form("Nb of message for each type per GET4 in Sector %02u; GET4 chip # ; Type", uSectorIndex),
               fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb,
               4, 0., 4.) );
      ...
      fvhGdpbGet4ChanScm.push_back( new TH2I(
               ...
               2 * fuNrOfChannelsPerGdpb, 0., fuNrOfChannelsPerGdpb,
               5, 0., 5.) );
      ...
      fvhGdpbGet4ChanErrors.push_back( new TH2I(
               Form("hGdpbGet4ChanErrors_%02u", uGdpb),
               Form("Error messages per GET4 channel in Sector %02u; GET4 channel # ; Error", uSectorIndex),
      ...
      fvhRemapTot_gDPB.push_back( new TH2I(
               Form("RemapTot_gDPB_%02u", uSectorIndex),
               Form("Raw TOT Sector %02u remapped; PADI channel; TOT [bin]", uSectorIndex),
               fuNrOfChannelsPerGdpb, 0, fuNrOfChannelsPerGdpb,
               256, 0, 256 ) );

      fvhRemapChCount_gDPB.push_back( new TH1I(
               Form("RemapChCount_gDPB_%02u", uSectorIndex),
      ...
      fvhRemapChRate_gDPB.push_back( new TH2D(
               Form("RemapChRate_gDPB_%02u", uSectorIndex),
               Form("PADI channel rate Sector %02u; Time in run [s]; PADI channel; Rate [1/s]", uSectorIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize,
               fuNrOfFeePerGdpb * fuNrOfChannelsPerFee, 0, fuNrOfFeePerGdpb * fuNrOfChannelsPerFee ) );
      ...
      if( kTRUE == fbDebugMonitorMode )
      {
         fvhGdpbPatternMissmatchEvo.push_back( new TH2I(
                  Form("hGdpbPatternMissmatchEvo_%02u", uSectorIndex),
                  Form("Missmatch pattern vs TS index in Sector %02u; TS # ; ASIC Pattern []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbPatternEnableEvo.push_back( new TH2I(
                  Form("hGdpbPatternEnableEvo_%02u", uSectorIndex),
                  Form("Enable pattern vs TS index in Sector %02u; TS # ; ASIC Pattern []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbPatternResyncEvo.push_back( new TH2I(
                  Form("hGdpbPatternResyncEvo%02u", uSectorIndex),
                  Form("Resync pattern vs TS index in Sector %02u; TS # ; ASIC Pattern []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbMissmatchEvoPerTs.push_back( new TH2I(
                  Form("hGdpbMissmatchEvoPerTs%02u", uSectorIndex),
                  Form("Missmatch vs TS index in Sector %02u; TS # ; Asic []; Missmatch? []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbMissmatchEnaEvoPerTs.push_back( new TH2I(
                  Form("hGdpbMissmatchEnaEvoPerTs%02u", uSectorIndex),
                  Form("Enable+Missmatch vs TS index in Sector %02u; TS # ; Asic []; Enabled & Missmatch? []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbEnableEvoPerTs.push_back( new TH2I(
                  Form("hGdpbEnableEvoPerTs%02u", uSectorIndex),
                  Form("Enable vs TS index in Sector %02u; TS # ; Asic []; Enabled? []", uSectorIndex),
                  100000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbResyncEvoPerTs.push_back( new TH2I(
                  Form("hGdpbResyncEvoPerTs%02u", uSectorIndex),
                  Form("Resync vs TS index in Sector %02u; TS # ; Asic []; Resync? []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbResyncEnaEvoPerTs.push_back( new TH2I(
                  Form("hGdpbResyncEnaEvoPerTs%02u", uSectorIndex),
                  Form("Enable+Resync vs TS index in Sector %02u; TS # ; Asic []; Enabled & Resync? []", uSectorIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbStateEvoPerTs.push_back( new TH2I(
                  Form("hGdpbStateEvoPerTs%02u", uSectorIndex),
                  Form("ASIC State vs TS index in Sector %02u; TS # ; Asic []; 0 = Off, 1 = OK, 2 = Miss, 3 = Resync, 4 = Miss + Resync []", uSectorIndex),
                  100000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );
      } // if( kTRUE == fbDebugMonitorMode )
      ...
      /// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ///
      /// STAR Trigger decoding and monitoring
      fvhTokenMsgType.push_back(  new TH1F(
               Form( "hTokenMsgType_gDPB_%02u", uSectorIndex),
               Form( "STAR trigger Messages type Sector %02u; Type ; Counts", uSectorIndex),
               4, 0, 4) );
      ...
      fvhTriggerRate.push_back(  new TH1F(
               ...
               Form( "STAR trigger signals per second Sector %02u; Time[s] ; Counts", uSectorIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize) );

      fvhCmdDaqVsTrig.push_back( new TH2I(
               Form( "hCmdDaqVsTrig_gDPB_%02u", uSectorIndex),
               Form( "STAR daq command VS STAR trigger command Sector %02u; DAQ ; TRIGGER", uSectorIndex),
               16, 0, 16,
               16, 0, 16 ) );
      ...
      fvhStarTokenEvo.push_back( new TH2I(
               Form( "hStarTokenEvo_gDPB_%02u", uSectorIndex),
               Form( "STAR token value VS time Sector %02u; Time in Run [s] ; STAR Token; Counts", uSectorIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize,
               410, 0, 4100 ) );

      fvhStarTrigGdpbTsEvo.push_back( new TProfile(
               Form( "hStarTrigGdpbTsEvo_gDPB_%02u", uSectorIndex),
               Form( "gDPB TS in STAR triger tokens for Sector %02u; Time in Run [s] ; gDPB TS;", uSectorIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize ) );

      fvhStarTrigStarTsEvo.push_back( new TProfile(
               Form( "hStarTrigStarTsEvo_gDPB_%02u", uSectorIndex),
               Form( "STAR TS in STAR triger tokens for Sector %02u; Time in Run [s] ; STAR TS;", uSectorIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize ) );
         ...
         for( UInt_t uAsic = 0; uAsic < fuNrOfGet4PerGdpb; ++ uAsic )
         {
            ...
            if( fvvbGdpbLastEnablePattern[ uGdpb ][ uAsic ] )
            {
               ...
               if( fvvbGdpbLastMissmatchPattern[ uGdpb ][ uAsic ] )
               {
                  ...
                  else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIdx, uAsic, 2 );
               } // if( fvvbGdpbLastMissmatchPattern[ uGdpb ][ uAsic ] )
                  else if( fvvbGdpbLastResyncPattern[ uGdpb ][ uAsic ] )
                  {
                     fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIdx, uAsic, 3 );
                  } // else if( fvvbGdpbLastResyncPattern[ uGdpb ][ uAsic ] )
                  else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIdx, uAsic, 1 );
            } // if( fvvbGdpbLastEnablePattern[ uGdpb ][ uAsic ] )
               else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIdx, uAsic, 0 );
