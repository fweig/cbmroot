...
void CbmStar2019MonitorTof::AddMsComponentToList(size_t component, UShort_t usDetectorId)
{
...
void CbmStar2019MonitorTof::CreateHistograms()
{
  ...
  for (UInt_t uMod = 0; uMod < fuNrOfModules; uMod++) {
    ...
    fvhRemapTotSideA_mod.push_back(new TH2F(name.Data(), title.Data(), kuNbFeeSide * fuNrOfChannelsPerFee, 0,
                                            kuNbFeeSide * fuNrOfChannelsPerFee, 256, 0, 256));
    name  = Form("RemapTotSideB_mod_%02u", uMod);
    title = Form("Raw TOT module %02u Side B; PADI channel; TOT [bin]", uMod);
    fvhRemapTotSideB_mod.push_back(new TH2F(name.Data(), title.Data(), kuNbFeeSide * fuNrOfChannelsPerFee, 0,
                                            kuNbFeeSide * fuNrOfChannelsPerFee, 256, 0, 256));
  ...
  TCanvas* cSummaryGdpb = new TCanvas("cSummaryGdpb", "gDPB Monitoring Summary", w, h);
  ...
  for (UInt_t uMod = 0; uMod < fuNrOfModules; uMod++) {
    cTotPnt =
      new TCanvas(Form("cTotRemapSides_m%02u", uMod), Form("Sides ch module %02u TOT distributions", uMod), w, h);
    cTotPnt->Divide(1, 2);
  ...
  for (UInt_t uMod = 0; uMod < fuNrOfModules; uMod++) {
    TCanvas* cModRates =
      new TCanvas(Form("cModRate_m%02u", uMod), Form("Hit and error Rates for module %02u", uMod), w, h);
    cModRates->cd();
    gPad->SetLogy();
...
Bool_t CbmStar2019MonitorTof::DoUnpack(const fles::Timeslice& ts, size_t component)
{
...
*/
      ...
      if (it == fGdpbIdIndexMap.end()) {
        ...
        LOG(info) << Form(
          ...
          static_cast<unsigned int>(msDescriptor.sys_id), static_cast<unsigned int>(msDescriptor.sys_ver),
          msDescriptor.idx, msDescriptor.crc, msDescriptor.size, msDescriptor.offset);
        LOG(fatal) << "Could not find the gDPB index for AFCK id 0x" << std::hex << fuGdpbId << std::dec
...
void CbmStar2019MonitorTof::ProcessEpochCycle(uint64_t ulCycleData)
{
  ...
  if (fuRawDataPrintMsgIdx < fuRawDataPrintMsgNb || fair::Logger::Logging(fair::Severity::debug2)) {
    LOG(info) << "CbmMcbm2018MonitorTof::ProcessEpochCyle => "
              << Form(" TS %5lu MS %3lu In data 0x%016lX Cycle 0x%016lX", fulCurrentTsIndex, fuCurrentMs, ulCycleData,
                      ulEpochCycleVal);
    fuRawDataPrintMsgIdx++;
  }
...
void CbmStar2019MonitorTof::FillHitInfo(gdpbv100::Message mess)
{
  ...
  if (uGbtxNrInSys < fuNrOfGbtx) {
    ...
    if (fviRpcSide[uGbtxNrInSys])
      ...
      fvhRemapTotSideA_mod[fviModuleId[uGbtxNrInSys]]->Fill(uRemappedChannelNr - uOffset, uTot);
  }  // if( uGbtxNrInSys < fuNrOfGbtx )
     /*
...
void CbmStar2019MonitorTof::UpdateZoomedFit()
{
  ...
  if (0.0 < fdFitZoomWidthPs) {
      ...
      for (UInt_t uFeeB = 0; uFeeB < fuNrOfFeePerGdpb * fuNrOfGdpbs; uFeeB++)
        if (NULL != fvhTimeDiffPulser[uFeeA][uFeeB]) {
          ...
          if (0 == fvhTimeDiffPulser[uFeeA][uFeeB]->GetEntries()) {
            ...
            LOG(info) << "CbmStar2019MonitorTof::UpdateZoomedFit => Empty input "
          ...
          if ((dZoomCounts / dNbCounts) < 0.99) {
            fhTimeRmsZoomFitPuls->Fill(uFeeA, uFeeB, 0.0);
            fhTimeResFitPuls->Fill(uFeeA, uFeeB, 0.0);
            LOG(warning) << "CbmStar2019MonitorTof::UpdateZoomedFit => Zoom too strong, "
    ...
    LOG(error) << "CbmStar2019MonitorTof::UpdateZoomedFit => Zoom width not defined, "
