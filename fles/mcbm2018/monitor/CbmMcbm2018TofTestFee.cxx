...
Bool_t CbmMcbm2018TofTestFee::ReInitContainers() {
  ...
  ///* Pulser monitoring *///
  fuGlobalIdxFeeA = fuNrOfFeePerGdpb * fuGdpbA
                    + fUnpackPar->GetNrOfFeePerGbtx() * fuGbtxA + fuFeeA;
  fuGlobalIdxFeeB = fuNrOfFeePerGdpb * fuGdpbB
                    + fUnpackPar->GetNrOfFeePerGbtx() * fuGbtxB + fuFeeB;
...
void CbmMcbm2018TofTestFee::CreateHistograms() {
    ...
    for (UInt_t uChanB = 0; uChanB < fuNrOfChannelsPerFee; uChanB++) {
      if (uChanA < uChanB) {
        fvhTimeDiffPulserFeeA[uChanA][uChanB] =
          new TH1I(Form("hTimeDiffPulser_g%02u_gbt%1u_f%1u_ch%02u_ch%02u",
                        fuGdpbA,
                        fuGbtxA,
                        fuFeeA,
                        uChanA,
  ...
  fhTimeMeanPulserFeeA = new TH2D(name.Data(),
                                  ...
                                  fuNrOfChannelsPerFee - 1,
                                  -0.5,
                                  fuNrOfChannelsPerFee - 1.5,
                                  fuNrOfChannelsPerFee - 1,
                                  0.5,
  ...
  for (UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb) {
    ...
    fvhPulserCountEvoPerFeeGdpb.push_back(new TH2D(
      name.Data(),
  ...
  fcPulserFeeA = new TCanvas(
    "cPulserFeeA", "Time difference RMS for channels on FEE A", w, h);
  ...
  fcPulserFeeB = new TCanvas(
    "cPulserFeeB", "Time difference RMS for channels on FEE A", w, h);
  ...
  fcPulseProp = new TCanvas(
    "cPulseProp", "Pulse properties for each channel on FEE A and FEE B", w, h);
...
Bool_t CbmMcbm2018TofTestFee::DoUnpack(const fles::Timeslice& ts,
  ...
  if (bMcbmTofTestFeeUpdateZoomedFit) {
    UpdateZoomedFit(
      fvhTimeDiffPulserFeeA, fhTimeRmsZoomFitPulsFeeA, fhTimeResFitPulsFeeA);
    UpdateZoomedFit(
      fvhTimeDiffPulserFeeB, fhTimeRmsZoomFitPulsFeeB, fhTimeResFitPulsFeeB);
    UpdateZoomedFit(fvhTimeDiffPulserFeeFee,
                    fhTimeRmsZoomFitPulsFeeFee,
                    fhTimeResFitPulsFeeFee);
...
*/
    ...
    for (UInt_t uChanA = 0; uChanA < fuNrOfChannelsPerFee; uChanA++) {
      ...
      for (UInt_t uChanB = 0; uChanB < fuNrOfChannelsPerFee; uChanB++) {
        if (bChanOkFeeA) {
          /// Fill FEE A plots
          if (uChanA < uChanB
              && 1 == fvuFeeChanNbHitsLastMs[fuGlobalIdxFeeA][uChanB]) {
            Double_t dTimeDiff =
              1e3
        ...
        if (bChanOkFeeB) {
          /// Fill FEE B plots
          if (uChanA < uChanB
              && 1 == fvuFeeChanNbHitsLastMs[fuGlobalIdxFeeB][uChanB]) {
            Double_t dTimeDiff =
              1e3
              * (fvdFeeChanMsLastPulserHit[fuGlobalIdxFeeB][uChanB]
  ...
  if (10.0 < dTsStartTime - fdLastRmsUpdateTime) {
    ...
    for (UInt_t uChanA = 0; uChanA < fuNrOfChannelsPerFee; uChanA++)
      for (UInt_t uChanB = 0; uChanB < fuNrOfChannelsPerFee; uChanB++) {
        if (NULL != fvhTimeDiffPulserFeeA[uChanA][uChanB]) {
          fhTimeMeanPulserFeeA->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeA[uChanA][uChanB]->GetMean());
          fhTimeRmsPulserFeeA->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeA[uChanA][uChanB]->GetRMS());
        }  // if( NULL != fvhTimeDiffPulserFeeA[ uChanA ][ uChanB ] )

        if (NULL != fvhTimeDiffPulserFeeB[uChanA][uChanB]) {
          fhTimeMeanPulserFeeB->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeB[uChanA][uChanB]->GetMean());
          fhTimeRmsPulserFeeB->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeB[uChanA][uChanB]->GetRMS());
        }  // if( NULL != fvhTimeDiffPulserFeeB[ uChanA ][ uChanB ] )

        if (NULL != fvhTimeDiffPulserFeeFee[uChanA][uChanB]) {
          fhTimeMeanPulserFeeFee->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeFee[uChanA][uChanB]->GetMean());
          fhTimeRmsPulserFeeFee->Fill(
            uChanA, uChanB, fvhTimeDiffPulserFeeFee[uChanA][uChanB]->GetRMS());
        }  // if( NULL != fvhTimeDiffPulserFeeFee[ uChanA ][ uChanB ] )
...
void CbmMcbm2018TofTestFee::FillHitInfo(gdpbv100::Message mess) {
  ...
  if (fuGlobalIdxFeeA == uFeeNrInSys) {
    ...
    fhChanPulseIntervalFeeA->Fill(
      dHitTime
        - fvdFeeChanMsLastPulserHit[uFeeNrInSys][uRemappedChannelNrInFee],
      uRemappedChannelNrInFee);
  }  // if( fuGlobalIdxFeeA == uFeeNrInSys )
  else if (fuGlobalIdxFeeB == uFeeNrInSys) {
    fhChanTotFeeB->Fill(uRemappedChannelNrInFee, uTot);
    fhChanPulseIntervalFeeB->Fill(
      dHitTime
        - fvdFeeChanMsLastPulserHit[uFeeNrInSys][uRemappedChannelNrInFee],
      uRemappedChannelNrInFee);
  ...
  if (0 <= fdStartTime)
    fvhPulserCountEvoPerFeeGdpb[fuGdpbNr]->Fill(1e-9 * (dHitTime - fdStartTime),
                                                uFeeNr);
...
void CbmMcbm2018TofTestFee::UpdateZoomedFit(
  std::vector<std::vector<TH1*>> phTimeDiff,
  TH2* phTimeRmsZoom,
  TH2* phTimeResFit) {
    ...
    for (UInt_t uChanA = 0; uChanA < fuNrOfChannelsPerFee; uChanA++)
      for (UInt_t uChanB = 0; uChanB < fuNrOfChannelsPerFee; uChanB++)
        if (NULL != phTimeDiff[uChanA][uChanB]) {
          ...
          Double_t dPeakPos =
            phTimeDiff[uChanA][uChanB]->GetXaxis()->GetBinCenter(iBinWithMax);
          phTimeDiff[uChanA][uChanB]->GetXaxis()->SetRangeUser(
          ...
          if ((dZoomCounts / dNbCounts) < 0.99) {
            ...
            LOG(warning)
              ...
              << "more than 1% loss for Chan pair " << uChanA << " and "
              << uChanB << " !!! ";
            continue;
          }  // if( ( dZoomCounts / dNbCounts ) < 0.99 )
          ...
          phTimeRmsZoom->Fill(
            uChanA, uChanB, phTimeDiff[uChanA][uChanB]->GetRMS());


          // Fit using zoomed boundaries + starting gaussian width, store into summary histo
          dRes = 0;
          fitFuncPairs[uChanA][uChanB] =
            new TF1(Form("fPair_%02d_%02d", uChanA, uChanB),
                    "gaus",
          ...
          fitFuncPairs[uChanA][uChanB]->SetParameter(
          ...
          phTimeDiff[uChanA][uChanB]->Fit(
            Form("fPair_%02d_%02d", uChanA, uChanB), "QRM0");
          // Get Sigma
          ...
          LOG(debug) << "CbmMcbm2018TofTestFee::UpdateZoomedFit => "
                     << "For chan pair " << uChanA << " and " << uChanB
                     << " we have zoomed RMS = "
                     << phTimeDiff[uChanA][uChanB]->GetRMS()
