...
Bool_t CbmMcbm2018MonitorAlgoTof::InitParameters()
{
   ...
   if( -1 < fiGdpbIndex )
   {
      if( fuNrOfGdpbs <= static_cast< UInt_t >( fiGdpbIndex ) )
         ...
         else LOG(info) << "Selected gDPB " << fiGdpbIndex
...
void CbmMcbm2018MonitorAlgoTof::AddMsComponentToList( size_t component, UShort_t usDetectorId )
{
   ...
   LOG(info) << "CbmMcbm2018MonitorAlgoTof::AddMsComponentToList => Component "
...
void CbmMcbm2018MonitorAlgoTof::ProcessEpoch( gdpbv100::Message mess )
{
   ...
   if( bDataLoss )
      fhGdpbEpochFlags->Fill( fuCurrDpbIdx, 1 );

   if( bEpochLoss )
      fhGdpbEpochFlags->Fill( fuCurrDpbIdx, 2 );
...
//   Double_t dHitTot   = uTot;     // in bins
   ...
   if( 4096 <= uCts )
   {
      LOG(debug) << "CbmMcbm2018MonitorAlgoTof::ProcessHit => Coarse time above 4096 detected."
                   << Form( " gDPB %02u GET4 %03u Channel %u, TS %8llu MS %3u (MS time %12llu)",
                            fuCurrDpbIdx,
...
//   fhGet4MessType->GetYaxis()->SetBinLabel( 6, "STAR Trigger");
   ...
   fhGdpbAsicSpiCounts = new TH2I( "hGdpbAsicSpiCounts", "SPI messages count per gDPB and ASIC; ASIC Idx []; gDPB []; SPI msg[]",
                                   fuNrOfGet4PerGdpb, -0.5, fuNrOfGet4PerGdpb - 0.5,
                                   fuNrOfGdpbs, dGdpbMin, dGdpbMax );

   fhGdpbMessType = new TH2I( "hGdpbMessageType", "Nb of message for each type per gDPB; Type; gDPB []",
                              1 + gdpbv100::MSG_STAR_TRI_D, 0., 1 + gdpbv100::MSG_STAR_TRI_D,
                              fuNrOfGdpbs, dGdpbMin, dGdpbMax );
   ...
   fhGdpbSysMessType = new TH2I( "hGdpbSysMessType", "Nb of system message for each type per gDPB; System Type; gDPB []",
                                 1 + gdpbv100::SYS_PATTERN, 0., 1 + gdpbv100::SYS_PATTERN,
                                 fuNrOfGdpbs, dGdpbMin, dGdpbMax );
   ...
   fhGdpbSysMessPattType = new TH2I( "hGdpbSysMessPattType", "Nb of pattern message for each type per gDPB; Pattern Type; gDPB []",
   ...
   fhGdpbEpochFlags = new TH2I( "hGdpbEpochFlags", "Epoch flags per gDPB; gDPB # ; Type",
                                fuNrOfGdpbs, dGdpbMin, dGdpbMax,
                                4, 0., 4.);
   ...
   fhGdpbEpochMissEvo = new TH2D( "hGdpbEpochMissEvo", "Epoch Missmatch per second and gDPB; Time[s];  gDPB #; Missmatch Nb",
                                  fuHistoryHistoSize, 0, fuHistoryHistoSize,
                                  fuNrOfGdpbs, dGdpbMin, dGdpbMax );

   fhGdpbEndMsBufferNotEmpty = new TH1D( "hGdpbEndMsBufferNotEmpty", "MS where buffer is not empty at end, per gDPB; gDPB #; Bad MS",
                                  fuNrOfGdpbs, dGdpbMin, dGdpbMax );

   fhGdpbEndMsDataLost = new TH2D( "hGdpbEndMsDataLost", "Amount of lost data when buffer not empty at end, per MS and gDPB; gDPB #; Lost Data per bad MS []; Bad MS",
                                  fuNrOfGdpbs, dGdpbMin, dGdpbMax,
                                  iNbBinsLog, dBinsLog );
   ...
   if( kTRUE == fbDebugMonitorMode )
   {
      fhNbMissPatternPerMs = new TH2I( "hNbMissPatternPerMs",
            "Nb of missmatch pattern per MS for each gDPB; Number of pattern messages []; gDPB []; MS",
            1000, -0.5, 999.5,
            fuNrOfGdpbs, dGdpbMin, dGdpbMax );
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      ...
      if( kTRUE == fbDebugMonitorMode )
      {
         fvhGdpbPatternMissmatchEvo.push_back( new TH2I(
                  Form("hGdpbPatternMissmatchEvo_%02u", uGdpbIndex),
                  Form("Missmatch pattern vs TS index in gDPB %02u; TS # ; ASIC Pattern []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbPatternEnableEvo.push_back( new TH2I(
                  Form("hGdpbPatternEnableEvo_%02u", uGdpbIndex),
                  Form("Enable pattern vs TS index in gDPB %02u; TS # ; ASIC Pattern []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbPatternResyncEvo.push_back( new TH2I(
                  Form("hGdpbPatternResyncEvo%02u", uGdpbIndex),
                  Form("Resync pattern vs TS index in gDPB %02u; TS # ; ASIC Pattern []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbMissmatchEvoPerTs.push_back( new TH2I(
                  Form("hGdpbMissmatchEvoPerTs%02u", uGdpbIndex),
                  Form("Missmatch vs TS index in gDPB %02u; TS # ; Asic []; Missmatch? []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbMissmatchEnaEvoPerTs.push_back( new TH2I(
                  Form("hGdpbMissmatchEnaEvoPerTs%02u", uGdpbIndex),
                  Form("Enable+Missmatch vs TS index in gDPB %02u; TS # ; Asic []; Enabled & Missmatch? []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbEnableEvoPerTs.push_back( new TH2I(
                  Form("hGdpbEnableEvoPerTs%02u", uGdpbIndex),
                  Form("Enable vs TS index in gDPB %02u; TS # ; Asic []; Enabled? []", uGdpbIndex),
                  100000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbResyncEvoPerTs.push_back( new TH2I(
                  Form("hGdpbResyncEvoPerTs%02u", uGdpbIndex),
                  Form("Resync vs TS index in gDPB %02u; TS # ; Asic []; Resync? []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbResyncEnaEvoPerTs.push_back( new TH2I(
                  Form("hGdpbResyncEnaEvoPerTs%02u", uGdpbIndex),
                  Form("Enable+Resync vs TS index in gDPB %02u; TS # ; Asic []; Enabled & Resync? []", uGdpbIndex),
                  10000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

         fvhGdpbStateEvoPerTs.push_back( new TH2I(
                  Form("hGdpbStateEvoPerTs%02u", uGdpbIndex),
                  Form("ASIC State vs TS index in gDPB %02u; TS # ; Asic []; 0 = Off, 1 = OK, 2 = Miss, 3 = Resync, 4 = Miss + Resync []", uGdpbIndex),
                  100000, 0., 100000,
                  fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );
      } // if( kTRUE == fbDebugMonitorMode )
      ...
      fvhTokenMsgType.push_back(  new TH1F(
               Form( "hTokenMsgType_gDPB_%02u", uGdpbIndex),
               Form( "STAR trigger Messages type gDPB %02u; Type ; Counts", uGdpbIndex),
               4, 0, 4) );
      ...
      fvhTriggerRate.push_back(  new TH1F(
               Form( "hTriggerRate_gDPB_%02u", uGdpbIndex),
      ...
      fvhCmdDaqVsTrig.push_back( new TH2I(
               ...
               Form( "STAR daq command VS STAR trigger command gDPB %02u; DAQ ; TRIGGER", uGdpbIndex),
               16, 0, 16,
               16, 0, 16 ) );
      ...
      fvhStarTokenEvo.push_back( new TH2I(
               Form( "hStarTokenEvo_gDPB_%02u", uGdpbIndex),
               Form( "STAR token value VS time gDPB %02u; Time in Run [s] ; STAR Token; Counts", uGdpbIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize,
               410, 0, 4100 ) );

      fvhStarTrigGdpbTsEvo.push_back( new TProfile(
               Form( "hStarTrigGdpbTsEvo_gDPB_%02u", uGdpbIndex),
               Form( "gDPB TS in STAR triger tokens for gDPB %02u; Time in Run [s] ; gDPB TS;", uGdpbIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize ) );

      fvhStarTrigStarTsEvo.push_back( new TProfile(
               Form( "hStarTrigStarTsEvo_gDPB_%02u", uGdpbIndex),
               Form( "STAR TS in STAR triger tokens for gDPB %02u; Time in Run [s] ; STAR TS;", uGdpbIndex),
               fuHistoryHistoSize, 0, fuHistoryHistoSize ) );
...
//   delete dBinsLog;
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      ...
      fvcSumGdpbGet4.push_back( new TCanvas( Form("cSumGdpb%02u", uGdpbIndex),
                                             Form("Summary per GET4 or channel for gDPB %02u", uGdpbIndex) ) );
