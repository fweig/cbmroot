...
void CbmMcbm2018MonitorAlgoHodo::AddMsComponentToList( size_t component, UShort_t usDetectorId )
{
...
Bool_t CbmMcbm2018MonitorAlgoHodo::ProcessMs( const fles::Timeslice& ts, size_t uMsCompIdx, size_t uMsIdx )
{
   ...
   if( static_cast<Int_t>( fvdPrevMsTime[ uMsCompIdx ] ) < static_cast<Int_t>( dMsTime )  )
   {
      ...
      for( UInt_t uHodoIdx = 0; uHodoIdx < kuNbHodos; ++uHodoIdx )
      {
         ...
         if( fiTimeIntervalRateUpdate <= iTimeInt )
         {
            for( UInt_t uChan = 0; uChan < kuNbChanPerAsic; ++uChan )
            {
               fhHodoChanHitRateProf[ uHodoIdx ]->Fill( uChan,
...
// -------------------------------------------------------------------------
void CbmMcbm2018MonitorAlgoHodo::ProcessHitInfo( stsxyter::Message mess, const UInt_t & uHodoIdx, const UInt_t & /*uMsIdx*/ )
{
...
void CbmMcbm2018MonitorAlgoHodo::ProcessTsMsbInfo( stsxyter::Message mess, UInt_t uMessIdx, UInt_t uMsIdx )
{
   ...
   if( uVal != fvulCurrentTsMsb + 1 &&
       0 != uVal && 4194303 != fvulCurrentTsMsb &&
...
{
   fhHodoMessType = new TH1I( "hHodoMessType", "Nb of message for each type; Type",
                              6, 0., 6.);
   ...
   fhHodoStatusMessType = new TH2I( "hHodoStatusMessType", "Nb of status message of each type for each DPB; ASIC; Status Type",
                                    kuNbHodos, 0, kuNbHodos,
                                    16, 0., 16.);
/*
...
*/
   ...
   fhHodoMsStatusFieldType = new TH2I( "hHodoMsStatusFieldType", "For each flag in the MS header, ON/OFF counts; Flag bit []; ON/OFF; MS []",
                                       16, -0.5, 15.5,
                                       2, -0.5, 1.5 );
/*
   fhHodoStatusMessType->GetYaxis()->SetBinLabel( 1, "Dummy");
...
*/
   ...
   for( UInt_t uHodoIdx = 0; uHodoIdx < kuNbHodos; ++uHodoIdx )
   {
      ...
      fhHodoChanAdcRaw[ uHodoIdx ] =  new TH2I( Form( "hHodoChanAdcRaw_%u", uHodoIdx ),
                                 Form( "Raw Adc distribution per channel, Hodo #%u; Channel []; Adc []; Hits []", uHodoIdx ),
                                 kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5,
                                 stsxyter::kuHitNbAdcBins, -0.5, stsxyter::kuHitNbAdcBins -0.5 );

         /// Raw Adc Distribution profile
      fhHodoChanAdcRawProf[ uHodoIdx ] =  new TProfile( Form( "hHodoChanAdcRawProf_%u", uHodoIdx ),
                                 Form( "Raw Adc prodile per channel, Hodo #%u; Channel []; Adc []", uHodoIdx ),
                                 kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5 );
/*
...
*/
      ...
      fhHodoChanRawTs[ uHodoIdx ] =  new TH2I( Form( "hHodoChanRawTs_%u", uHodoIdx ),
                                    Form( "Raw Timestamp distribution per channel, FEB #%03u; Channel []; Ts []; Hits []", uHodoIdx ),
                                    kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5,
      ...
      fhHodoChanMissEvt[ uHodoIdx ] =  new TH2I( Form( "hHodoChanMissEvt_%u", uHodoIdx ),
                                    Form( "Missed Event flags per channel, Hodo #%u; Channel []; Miss Evt []; Hits []", uHodoIdx ),
                                    kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5,
                                    2, -0.5, 1.5 );

         /// Missed event flag counts evolution
      fhHodoChanMissEvtEvo[ uHodoIdx ] =  new TH2I( Form( "hHodoChanMissEvtEvo_%u", uHodoIdx ),
                                    Form( "Missed Evt flags per second & channel in Hodo #%u; Time [s]; Channel []; Missed Evt flags []", uHodoIdx ),
                                    1800, 0, 1800,
                                    kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5 );

         /// Hit rates evo per channel
      fhHodoChanHitRateEvo[ uHodoIdx ] =  new TH2I( Form( "hHodoChanHitRateEvo_%u", uHodoIdx ),
                                                Form( "Hits per second & channel in Hodo #%u; Time [s]; Channel []; Hits []", uHodoIdx ),
                                                1800, 0, 1800,
                                                kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5 );

         /// Hit rates profile per channel
      fhHodoChanHitRateProf[ uHodoIdx ] =  new TProfile( Form( "hHodoChanHitRateProf_%u", uHodoIdx ),
      ...
      fhHodoChanDistT[ uHodoIdx ] =  new TH2I( Form( "hHodoChanDistT_%u", uHodoIdx ),
                                             Form( "Time distance between hits on same channel in Hodo #%u; Time difference [ns]; Channel []; ", uHodoIdx ),
                                             1000, -0.5, 6250.0 - 0.5,
                                             kuNbChanPerAsic, -0.5, kuNbChanPerAsic - 0.5);
      ...
      for( UInt_t uAxis = 0; uAxis < kuNbAxis; ++uAxis )
      {
         ...
         /// Fibers counts
         fhHodoFiberCnt[ uHodoIdx ][ uAxis ] =  new TH1I( Form( "hHodoFiberCnt%c_%u", cAxisName, uHodoIdx ),
         ...
         fhHodoFiberAdc[ uHodoIdx ][ uAxis ] =  new TH2I( Form( "fhHodoFiberAdc%c_%u", cAxisName, uHodoIdx ),
                                 Form( "Raw Adc distribution per Fiber, Hodo #%u Axis %c; Channel []; Adc []; Hits []", uHodoIdx, cAxisName ),
                                 kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                                 stsxyter::kuHitNbAdcBins, -0.5, stsxyter::kuHitNbAdcBins -0.5 );

         /// Fibers Adc Distribution profile
         fhHodoFiberAdcProf[ uHodoIdx ][ uAxis ] =  new TProfile( Form( "hHodoFiberAdcProf%c_%u", cAxisName, uHodoIdx ),
                                 Form( "Raw Adc prodile per Fiber, Hodo #%u Axis %c; Channel []; Adc []", uHodoIdx, cAxisName ),
                                 kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );

         /// Hit rates evo per fiber
         fhHodoFiberHitRateEvo[ uHodoIdx ][ uAxis ] =  new TH2I( Form( "hHodoFiberHitRateEvo%c_%u", cAxisName, uHodoIdx ),
                                 Form( "Hits per second & Fiber in Hodo #%u Axis %c; Time [s]; Channel []; Hits []", uHodoIdx, cAxisName ),
                                 1800, 0, 1800,
                                 kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
      ...
      fhHodoFiberCoincMapXY[ uHodoIdx ] =  new TH2I( Form( "hHodoFiberCoincMapXY_%u", uHodoIdx ),
                              Form( "Map of coincident (X, Y) pairs in Hodo #%u; X [Fiber]; Y [Fiber]; Hits []", uHodoIdx ),
                              kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                              kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
         /// Time diff
      fhHodoFiberCoincTimeXY[ uHodoIdx ] =  new TH1I( Form( "hHodoFiberCoincTimeXY_%u", uHodoIdx ),
                              Form( "Time difference of coincident (X, Y) pairs in Hodo #%u; t_Y - t_X [ns]; Hits []", uHodoIdx ),
      ...
      fhHodoFiberCoincWalkXY_X[ uHodoIdx ] =  new TH2I( Form( "hHodoFiberCoincWalkXY_X_%u", uHodoIdx ),
                              Form( "Walk X of coincident (X, Y) pairs in Hodo #%u; ADC X [bin]; t_Y - t_X [ns]; Hits []", uHodoIdx ),
                              stsxyter::kuHitNbAdcBins, -0.5, stsxyter::kuHitNbAdcBins -0.5,
                              2 * fdTimeCoincLimit + 1, -fdTimeCoincLimitNs, fdTimeCoincLimitNs );
...
//         AddHistoToVector( fhHodoChanAdcCalProf[  uHodoIdx ], "Raw" );
   ...
   for( UInt_t uAxis = 0; uAxis < kuNbAxis; ++uAxis )
   {
      ...
      /// Coincidences in different Hodos between axis <= Valid only for kuNbHodos = 2 !!!
         /// Map Same axis
      fhHodoFiberCoincMapSameAB[ uAxis ] =  new TH2I(
                              Form( "hHodoFiberCoincMapSameAB_%c%c", cAxisName, cAxisName ),
                              Form( "Map of coincident (%c, %c) pairs in Hodo A and B; %c_A [Fiber]; %c_B [Fiber]; Hits []",
                                    cAxisName, cAxisName, cAxisName, cAxisName ),
                              kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                              kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
         /// Time diff Same axis
      fhHodoFiberCoincTimeSameAB[ uAxis ] =  new TH1I(
                              Form( "hHodoFiberCoincTimeSameAB_%c%c", cAxisName, cAxisName ),
                              Form( "Time difference of coincident (%c, %c) pairs in Hodo A and B; t_%c_B - t_%c_A [ns]; Hits []",
                                    cAxisName, cAxisName, cAxisName, cAxisName ),
                              2 * fdTimeCoincLimit + 1, -fdTimeCoincLimitNs, fdTimeCoincLimitNs );

         /// Map different axis
      fhHodoFiberCoincMapDiffAB[ uAxis ] =  new TH2I(
                              Form( "hHodoFiberCoincMapDiffAB_%c%c", cAxisName, cOtherAxisName ),
                              Form( "Map of coincident (%c, %c) pairs in Hodo A and B; %c_A [Fiber]; %c_B [Fiber]; Hits []",
   ...
   fhHodoFullCoincPosA =  new TH2I( "fhHodoFullCoincPosA",
                           "Position on Hodo A for coincident pairs in Hodo A and B;  X_A [Fiber]; Y_A [Fiber]; Hits []",
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
      /// Position on hodo B
   fhHodoFullCoincPosB =  new TH2I( "fhHodoFullCoincPosB",
                           "Position on Hodo B for coincident pairs in Hodo A and B;  X_B [Fiber]; Y_B [Fiber]; Hits []",
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
      /// Comp X axis
   fhHodoFullCoincCompX =  new TH2I( "hHodoFullCoincCompX",
                           "Comparison of X pos for coincident pairs in Hodo A and B;  X_A [Fiber]; X_B [Fiber]; Hits []",
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
      /// Comp Y axis
   fhHodoFullCoincCompY =  new TH2I( "hHodoFullCoincCompY",
                           "Comparison of Y pos for coincident pairs in Hodo A and B;  Y_A [Fiber]; Y_B [Fiber]; Hits []",
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5,
                           kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );

      /// Residuals X vs Y ("alignment")
   fhHodoFullCoincResidualXY =  new TH2I( "hHodoFullCoincResidualXY",
                           "X and Y residuals for coincident pairs in Hodo A and B; X_B - X_A [Fiber]; Y_B - Y_A [Fiber]; Hits []",
                           kuNbChanPerAsic + 1, -1.0 * kuNbChanPerAsic/2 - 0.5, kuNbChanPerAsic/2 + 0.5,
                           kuNbChanPerAsic + 1, -1.0 * kuNbChanPerAsic/2 - 0.5, kuNbChanPerAsic/2 + 0.5 );
      /// Time diff different axis
   fhHodoFullCoincTimeDiff =  new TH1I( "hHodoFullCoincTimeDiff",
                           "Time difference of coincident pairs in Hodo A and B; (t_X_B + t_Y_B)/2 - (t_X_A + t_Y_A)/2 [ns]; Hits []",
                           2 * fdTimeCoincLimit + 1, -fdTimeCoincLimitNs, fdTimeCoincLimitNs );

   fhHodoFullCoincRateEvo =  new TH1I( "fhHodoFullCoincRateEvo",
                           "Evolution of the full coincidence rate; Time in run [s]; Full coincidences;",
                           1800, -0.5, 1800 - 0.5 );
   ...
   for( UInt_t uHodoIdx = 0; uHodoIdx < kuNbHodos; ++uHodoIdx )
   {
      ...
      {
         Char_t cAxisName = ( uAxis ? 'Y' : 'X' );
         fhHodoFullCoincTimeWalk[ uHodoIdx ][ uAxis ] =  new TH2I( Form( "hHodoFullCoincTimeWalk_%u%c", uHodoIdx, cAxisName ),
                                 Form( "Time walk of coincident (A, B) pairs in Hodo #%u Axis %c; ADC %u_%c [bin]; Time Diff <B> - <A> [ns]; Hits []",
                                       uHodoIdx, cAxisName, uHodoIdx, cAxisName ),
                                 stsxyter::kuHitNbAdcBins, -0.5, stsxyter::kuHitNbAdcBins -0.5,
                                 2 * fdTimeCoincLimit + 1, -fdTimeCoincLimitNs, fdTimeCoincLimitNs );
         AddHistoToVector( fhHodoFullCoincTimeWalk[ uHodoIdx ][  uAxis ], "Walk" );

         fhHodoFullCoincPosEvo[ uHodoIdx ][ uAxis ] =  new TH2I( Form( "hHodoFullCoincPosEvo_%u%c", uHodoIdx, cAxisName ),
                                 Form( "Time evolution of coincident (A, B) pairs position in Hodo #%u Axis %c; Time in run [s]; Position %u_%c [Fiber]; Hits []",
                                       uHodoIdx, cAxisName, uHodoIdx, cAxisName ),
                                 2000, -0.5, 1000 -0.5,
                                 kuNbChanPerAsic/2, -0.5, kuNbChanPerAsic/2 - 0.5 );
   ...
   fhPrevHitDtAllAsics =  new TH1I( "hPrevHitDtAllAsics",
                           "Time difference between current and previous hits in any ASIC; t - t_prev [ns]; Hit pairs []",
                           10000, 0.0, 10000 * stsxyter::kdClockCycleNs );
   fhPrevHitDtAsicA    =  new TH1I( "hPrevHitDtAsicA",
                           "Time difference between current and previous hits in ASIC A; t - t_prev [ns]; Hit pairs []",
                           10000, 0.0, 10000 * stsxyter::kdClockCycleNs );
   fhPrevHitDtAsicB    =  new TH1I( "hPrevHitDtAsicB",
                           "Time difference between current and previous hits in ASIC B; t - t_prev [ns]; Hit pairs []",
                           10000, 0.0, 10000 * stsxyter::kdClockCycleNs );
   fhPrevHitDtAsicsAB  =  new TH1I( "hPrevHitDtAsicsAB",
                           "Time difference between current in ASIC A and previous hit in ASIC B; t - t_prev [ns]; Hit pairs []",
                           10000, 0.0, 10000 * stsxyter::kdClockCycleNs );
   ...
   for( UInt_t uHodoIdx = 0; uHodoIdx < kuNbHodos; ++uHodoIdx )
   {
      ...
      fcHodoSummaryRaw[ uHodoIdx ]  = new TCanvas( Form( "cHodoSummaryRaw%u", uHodoIdx), Form("Raw Summary for Hodo %u", uHodoIdx ), w, h);
      ...
      fcHodoSummaryFiber[ uHodoIdx ]  = new TCanvas( Form( "cHodoSummaryFiber%u", uHodoIdx), Form("Fiber Summary for Hodo %u", uHodoIdx ), w, h);
   ...
   fcHodoFullCoincPos = new TCanvas( "fcHodoFullCoincPos", "Hit Positions for Full coincidences between hodoscopes", w, h);
   ...
   fcHodoFullCoincPosEvo = new TCanvas( "fcHodoFullCoincPosEvo", "Hit Positions Evo for Full coincidences between hodoscopes", w, h);
...
//      UInt_t uFiber = fvmHitsInMs[ uHit ].GetChan();
      ...
      if( 0 == uHodo )
      {
         ...
         else fhPrevHitDtAsicB->Fill( dCurrentHitTime - fvdPreviousHitTimePerAsic [ 1 ] );
          ...
          bHitFoundHodoAxis[ 1 ][ 0 ] && bHitFoundHodoAxis[ 1 ][ 1 ] )
      {
         ...
         Double_t dTimeDiffHodoAB = (  lastHitHodoAxis[ 1 ][ 1 ].GetTs() * stsxyter::kdClockCycleNs
                                     + lastHitHodoAxis[ 1 ][ 0 ].GetTs() * stsxyter::kdClockCycleNs ) / 2.0
                                  - (  lastHitHodoAxis[ 0 ][ 1 ].GetTs() * stsxyter::kdClockCycleNs
                                     + lastHitHodoAxis[ 0 ][ 0 ].GetTs() * stsxyter::kdClockCycleNs ) / 2.0;
         Double_t dTimeHitHodoAB = (  lastHitHodoAxis[ 1 ][ 1 ].GetTs() * stsxyter::kdClockCycleNs
                                    + lastHitHodoAxis[ 1 ][ 0 ].GetTs() * stsxyter::kdClockCycleNs
                                    + lastHitHodoAxis[ 0 ][ 1 ].GetTs() * stsxyter::kdClockCycleNs
                                    + lastHitHodoAxis[ 0 ][ 0 ].GetTs() * stsxyter::kdClockCycleNs ) / 4.0;
             ...
             -fdTimeCoincLimitNs < dTimeDiffHodoAB && dTimeDiffHodoAB < fdTimeCoincLimitNs )
         {
            ...
            if( fvbHodoInvertX[ 0 ] )
               uPosXA = kuNbChanPerAsic/2 - 1 - uPosXA;
            if( fvbHodoInvertY[ 0 ] )
               uPosYA = kuNbChanPerAsic/2 - 1 - uPosYA;
            if( fvbHodoInvertX[ 1 ] )
               uPosXB = kuNbChanPerAsic/2 - 1 - uPosXB;
            if( fvbHodoInvertY[ 1 ] )
               uPosYB = kuNbChanPerAsic/2 - 1 - uPosYB;
