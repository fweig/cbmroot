...
//   fhGet4MessType->GetYaxis()->SetBinLabel( 6, "STAR Trigger");
   ...
   fhGdpbEpochSyncEvo = new TH2D(name.Data(), title.Data(), fuHistoryHistoSize, 0, fuHistoryHistoSize,
   ...
   fhGdpbEpochMissEvo = new TH2D(name.Data(), title.Data(), fuHistoryHistoSize, 0, fuHistoryHistoSize,
                                                            fuNrOfGdpbs, 0., fuNrOfGdpbs);
  ...
  for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
  {
      ...
      fvhGdpbGet4ChanScm.push_back( new TH2I(name, title,
      ...
      fvhGdpbMissmatchEvoPerTs.push_back( new TH2I(name, title,
                                                      10000, 0., 100000,
                                                      fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );
      name = Form("hGdpbMissmatchEnaEvoPerTs%02u", uGdpb);
      title = Form("Enable+Missmatch vs TS index in gDPB %02u; TS # ; Asic []; Enabled & Missmatch? []", uGdpb);
      fvhGdpbMissmatchEnaEvoPerTs.push_back( new TH2I(name, title,
                                                      10000, 0., 100000,
                                                      fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

      name = Form("hGdpbEnableEvoPerTs%02u", uGdpb);
      title = Form("Enable vs TS index in gDPB %02u; TS # ; Asic []; Enabled? []", uGdpb);
      fvhGdpbEnableEvoPerTs.push_back( new TH2I(name, title,
                                                      100000, 0., 100000,
                                                      fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

      name = Form("hGdpbResyncEvoPerTs%02u", uGdpb);
      title = Form("Resync vs TS index in gDPB %02u; TS # ; Asic []; Resync? []", uGdpb);
      fvhGdpbResyncEvoPerTs.push_back( new TH2I(name, title,
                                                      10000, 0., 100000,
                                                      fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );
      name = Form("hGdpbResyncEnaEvoPerTs%02u", uGdpb);
      title = Form("Enable+Resync vs TS index in gDPB %02u; TS # ; Asic []; Enabled & Resync? []", uGdpb);
      fvhGdpbResyncEnaEvoPerTs.push_back( new TH2I(name, title,
                                                      10000, 0., 100000,
                                                      fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );

      name = Form("hGdpbStateEvoPerTs%02u", uGdpb);
      title = Form("ASIC State vs TS index in gDPB %02u; TS # ; Asic []; 0 = Off, 1 = OK, 2 = Miss, 3 = Resync, 4 = Miss + Resync []", uGdpb);
      fvhGdpbStateEvoPerTs.push_back( new TH2I(name, title,
                                                   100000, 0., 100000,
                                                   fuNrOfGet4PerGdpb, 0., fuNrOfGet4PerGdpb) );
...
*/
   ...
   for( UInt_t uGdpb = 0; uGdpb < fuNrOfGdpbs; ++uGdpb )
   {
      TCanvas* cSumGdpbGet4 = new TCanvas( Form("cSumGdpbGet4_g%02u", uGdpb),
      ...
      for( UInt_t uAsic = 0; uAsic < fuNrOfGet4PerGdpb; ++ uAsic )
      {
         ...
         if( fvvbGdpbLastEnablePattern[ uGdpb ][ uAsic ] )
         {
            ...
            if( fvvbGdpbLastMissmatchPattern[ uGdpb ][ uAsic ] )
            {
               ...
               else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIndex, uAsic, 2 );
            } // if( fvvbGdpbLastMissmatchPattern[ uGdpb ][ uAsic ] )
               else if( fvvbGdpbLastResyncPattern[ uGdpb ][ uAsic ] )
               {
                  fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIndex, uAsic, 3 );
               } // else if( fvvbGdpbLastResyncPattern[ uGdpb ][ uAsic ] )
               else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIndex, uAsic, 1 );
         } // if( fvvbGdpbLastEnablePattern[ uGdpb ][ uAsic ] )
            else fvhGdpbStateEvoPerTs[ uGdpb ]->Fill( fulCurrentTsIndex, uAsic, 0 );
      ...
      case gdpbv100::PATT_MISSMATCH:
      {
         ...
         for( UInt_t uBit = 0; uBit < 32; ++uBit )
         {
            ...
            if( ( uPattern >> uBit ) & 0x1 )
            {
               ...
               else fvvbGdpbLastMissmatchPattern[ fuGdpbNr ][ uBadAsic ] = kFALSE;
      ...
      case gdpbv100::PATT_ENABLE:
      {
         for( UInt_t uBit = 0; uBit < 32; ++uBit )
         {
            ...
            if( ( uPattern >> uBit ) & 0x1 )
            {
               ...
               else fvvbGdpbLastEnablePattern[ fuGdpbNr ][ uAsic ] = kTRUE;
      ...
      case gdpbv100::PATT_RESYNC:
      {
         ...
         for( UInt_t uBit = 0; uBit < 32; ++uBit )
         {
            ...
            if( ( uPattern >> uBit ) & 0x1 )
            {
               ...
               else fvvbGdpbLastResyncPattern[ fuGdpbNr ][ uBadAsic ] = kFALSE;
