...
Bool_t CbmMcbm2018UnpackerAlgoHodo::InitParameters()
{
      ...
      for( UInt_t uCrobIdx = 0; uCrobIdx < fUnpackPar->GetNbCrobsPerDpb(); ++uCrobIdx )
      {
         ...
         for( UInt_t uFebIdx = 0; uFebIdx < fUnpackPar->GetNbFebsPerCrob(); ++ uFebIdx )
         {
                ...
                static_cast< UInt_t >( fviFebModuleIdx[ uDpb ][ uCrobIdx ][ uFebIdx ] ) < fuNbModules )
            {
               ...
               {
                  LOG(fatal) << Form( "Bad module Index for DPB #%02u CROB #%u FEB %02u: %d",
   ...
   for( UInt_t uDpb = 0; uDpb < fuNrOfDpbs; ++uDpb )
   {
      for( UInt_t uCrobIdx = 0; uCrobIdx < fUnpackPar->GetNbCrobsPerDpb(); ++uCrobIdx )
      {
         ...
         {
            if( 0 <= fviFebModuleIdx[ uDpb ][ uCrobIdx ][ uFebIdx ] )
               LOG(info) << Form( "      FEB #%02u (%02u): Mod. Idx = %03d (Addr. 0x%08x) ADC gain %4.0f e- ADC Offs %5.0f e-",
...
void CbmMcbm2018UnpackerAlgoHodo::AddMsComponentToList( size_t component, UShort_t usDetectorId )
{
...
//      LOG(info) << Form( "TS %5d Start %12f Stop %12f", fulCurrentTsIdx, fdTsStartTime, fdTsStopTimeCore );
   ...
   for( fuMsIndex = 0; fuMsIndex < fuNbMsLoop; fuMsIndex ++ )
   {
      ...
      for( auto itHitIn = fvmHitsInMs.begin(); itHitIn < fvmHitsInMs.end(); ++itHitIn )
      {
         ...
         if( -1 == fviFebModule[ uFebIdx ] )
            continue;

         UInt_t uChanInMod = fUnpackPar->GetChannelInModule( fviFebModule[ uFebIdx ], itHitIn->GetChan() ) ; // 0-63 = X, 64-127 = Y
...
Bool_t CbmMcbm2018UnpackerAlgoHodo::ProcessMs( const fles::Timeslice& ts, size_t uMsCompIdx, size_t uMsIdx )
{
...
//         LOG(info) << msDescriptor;
         LOG(warning) << "Could not find the sDPB index for AFCK id 0x"
                   ...
                   << "\n"
                   << "If valid this index has to be added in the HODO parameter file in the DbpIdArray field";
   ...
   uint32_t uNbMessages = ( uSize - ( uSize % sizeof( stsxyter::Message ) ) )
...
// -------------------------------------------------------------------------
void CbmMcbm2018UnpackerAlgoHodo::ProcessHitInfo( const stsxyter::Message & mess, const UShort_t & usElinkIdx, const UInt_t & uAsicIdx, const UInt_t & /*uMsIdx*/ )
{
...
void CbmMcbm2018UnpackerAlgoHodo::ProcessTsMsbInfo( const stsxyter::Message & mess, UInt_t uMessIdx, UInt_t uMsIdx )
{
...
void CbmMcbm2018UnpackerAlgoHodo::ProcessEpochInfo( const stsxyter::Message & /*mess*/ )
{
...
void CbmMcbm2018UnpackerAlgoHodo::ProcessStatusInfo( const stsxyter::Message & /*mess*/ )
{
/*
   UInt_t   uCrobIdx   = usElinkIdx / fUnpackPar->GetNbElinkPerCrob();
   Int_t   uFebIdx    = fUnpackPar->ElinkIdxToFebIdx( usElinkIdx );
...
Bool_t CbmMcbm2018UnpackerAlgoHodo::CreateHistograms()
{
   ...
   fhDigisTimeInRun = new TH1I( "hStsDigisTimeInRun",
...
void CbmMcbm2018UnpackerAlgoHodo::SetTimeOffsetNsAsic( UInt_t uAsicIdx, Double_t dOffsetIn )
{
   if( uAsicIdx >= fvdTimeOffsetNsAsics.size() )
   {
