      ...
      for( UInt_t uCrobIdx = 0; uCrobIdx < fUnpackPar->GetNbCrobsPerDpb(); ++uCrobIdx )
      {
         ...
         for( UInt_t uFebIdx = 0; uFebIdx < fUnpackPar->GetNbFebsPerCrob(); ++ uFebIdx )
         {
            if( 0 <= fviFebModuleIdx[ uDpb ][ uCrobIdx ][ uFebIdx ] )
               ...
               else LOG(info) << Form( "Disabled FEB #%02u (%02u): Mod. Idx = %03d Side %c (%2d) Type %c (%2d) (Addr. 0x%08x) ADC gain %4.0f e- ADC Offs %5.0f e-",
                                    uFebIdx, uGlobalFebIdx,
                                    fviFebModuleIdx[ uDpb ][ uCrobIdx ][ uFebIdx ],
                                    1 == fviFebModuleSide[ uDpb ][ uCrobIdx ][ uFebIdx ] ? 'N': 'P',
                                    fviFebModuleSide[ uDpb ][ uCrobIdx ][ uFebIdx ],
...
*/
         ...
         {
            LOG(error) << Form( "Digi on disabled FEB %02u has address 0x%08x and side %d", uFebIdx, fviFebAddress[ uFebIdx ], fviFebSide[ uFebIdx ] );
         } // if( 0 == fviFebAddress[ uFebIdx ] || -1 == fviFebSide[ uFebIdx ] )
...
// -------------------------------------------------------------------------
void CbmMcbm2018UnpackerAlgoSts::ProcessHitInfo( const stsxyter::Message & mess, const UShort_t & usElinkIdx, const UInt_t & uAsicIdx, const UInt_t & /*uMsIdx*/ )
{
...
*/
   ...
   if( mess.IsHitMissedEvts() )
      fErrVect.push_back( CbmErrorMessage( ECbmModuleId::kSts, dHitTimeNs, uAsicIdx, 1 << stsxyter::kusLenStatStatus, usChan ) );
...
*/
   ...
   if( 4194303 == uVal && 1 == uMessIdx )
      fvulCurrentTsMsb[fuCurrDpbIdx] = 0;
      else fvulCurrentTsMsb[fuCurrDpbIdx] = uVal;
/*
   if( 1 < uMessIdx )
   {
...
*/
   /// Compute the Full time stamp
      /// Use TS w/o overlap bits as they will anyway come from the TS_MSB
   Long64_t ulTime =  static_cast<ULong64_t>( stsxyter::kuHitNbTsBins )
                    * static_cast<ULong64_t>( fvulCurrentTsMsb[fuCurrDpbIdx])
                    + static_cast<ULong64_t>( stsxyter::kulTsCycleNbBins )
                    * static_cast<ULong64_t>( fvuCurrentTsMsbCycle[fuCurrDpbIdx] );
...
Bool_t CbmMcbm2018UnpackerAlgoSts::CreateHistograms()
{
   ...
   fhMsCntEvo = new TH1I( "fhMsCntEvo", "; MS index [s]; Counts []",
       600, 0.0,  600.0 );
...
void CbmMcbm2018UnpackerAlgoSts::MaskNoisyChannel( UInt_t uFeb, UInt_t uChan, Bool_t bMasked )
{
   ...
   if( uFeb < fuNbFebs && uChan < fUnpackPar->GetNbChanPerFeb() )
      ...
      else LOG( fatal ) << "CbmMcbm2018UnpackerAlgoSts::MaskNoisyChannel => Invalid FEB and/or CHAN index:"
