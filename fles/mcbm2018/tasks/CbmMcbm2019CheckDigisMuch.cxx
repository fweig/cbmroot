...
CbmMcbm2019CheckDigisMuch::CbmMcbm2019CheckDigisMuch()
  : FairTask("CbmMcbm2019CheckDigisMuch")
{
}
...
void CbmMcbm2019CheckDigisMuch::CreateHistos()
{
   ...
   fDigisPerAsicEvo = new TH2F( "fDigisPerAsicEvo",
         ...
         dZoomDuration / 1000, 0, dZoomDuration,
         kuMaxNbAsics, 0, kuMaxNbAsics);
   ...
   fDigisPerChanEvo = new TH2F( "fDigisPerChanEvo",
      "Time dist of digis in same chan evo; Time [ ns ]; Channel [ ]",
       5000, 0, 5000.,
       kuMaxNbAsics * kuNbChansAsic, 0., kuMaxNbAsics * kuNbChansAsic );

   std::cout << Form( "TS with jump %5u, first TS time off %12.0f, start TS %5u stop TS %5u, TS length %9.0f Start time %12.0f",
...
void CbmMcbm2019CheckDigisMuch::Exec(Option_t* /*option*/)
{
  ...
  for (Int_t iMuch = 0; iMuch < nrMuchDigis; ++iMuch) {
    ...
    if( 0 == iMuch )
       std::cout << Form( "Much first hit in TS %5d: asic %2u chan %3u time %12.0f T0 time %12.0f check time %12.0f ADC %2.0f",
...
//    if( 9 != uAsic )
    if( 9 != uAsic || uChan < 63 )
      continue;
    if( fuMaxAdcPulserMuch < Digi->GetAdc() || Digi->GetAdc() < fuMinAdcPulserMuch )
      continue;

    std::cout << Form( "Much pulser in TS %5d: chan %3u T0 time %12.0f time start %12.0f ADC %2.0f dt %12.0f",
