...
InitStatus CbmCheckDigisNbCorr::Init()
{
  ...
  if ( ! fTsMetaData ) {
    LOG(info) << "No TClonesArray with TS meta data found.";
  }
  // DigiManager
...
void CbmCheckDigisNbCorr::CreateHistos()
{
  ...
  fT0StsCorr = new TH2F("fT0StsCorr",
      Form( "T0 - STS digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb STS Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // T0 vs. MUCH
  fT0MuchCorr = new TH2F("fT0MuchCorr",
      Form( "T0 - MUCH digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb MUCH Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // T0 vs. TOF
  fT0TofCorr = new TH2F("fT0TofCorr",
      Form( "T0 - TOF digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb TOF Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // T0 vs. RICH
  fT0RichCorr = new TH2F("fT0RichCorr",
      Form( "T0 - RICH digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb RICH Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);

  // STS vs. MUCH
  fStsMuchCorr = new TH2F("fStsMuchCorr",
      Form( "STS - MUCH digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb STS Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // STS vs. TOF
  fStsTofCorr = new TH2F("fStsTofCorr",
      Form( "STS - TOF digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb TOF Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // STS vs. RICH
  fStsRichCorr = new TH2F("fStsRichCorr",
      Form( "STS - RICH digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb RICH Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);

  // MUCH vs. TOF
  fMuchTofCorr = new TH2F("fMuchTofCorr",
      Form( "MUCH - TOF digis Nb correlation per %.0f ns time interval; Nb MUCH Digis []; Nb TOF Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  // MUCH vs. RICH
  fMuchRichCorr = new TH2F("fMuchRichCorr",
      Form( "MUCH - RICH digis Nb correlation per %.0f ns time interval; Nb MUCH Digis []; Nb RICH Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);

  // TOF vs. RICH
  fTofRichCorr = new TH2F("fTofRichCorr",
      Form( "TOF - RICH digis Nb correlation per %.0f ns time interval; Nb TOF Digis []; Nb RICH Digis []; Counts", fdBinWidthNs ),
      1000, 0, 1000,
			1000, 0, 1000);
  ...
  fT0StsCorrProf = new TProfile("fT0StsCorrProf",
      Form( "T0 - STS digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb STS Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // T0 vs. MUCH
  fT0MuchCorrProf = new TProfile("fT0MuchCorrProf",
      Form( "T0 - MUCH digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb MUCH Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // T0 vs. TOF
  fT0TofCorrProf = new TProfile("fT0TofCorrProf",
      Form( "T0 - TOF digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb TOF Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // T0 vs. RICH
  fT0RichCorrProf = new TProfile("fT0RichCorrProf",
      Form( "T0 - RICH digis Nb correlation per %.0f ns time interval; Nb T0 Digis []; Nb RICH Digis []", fdBinWidthNs ),
      1000, 0, 1000 );

  // STS vs. MUCH
  fStsMuchCorrProf = new TProfile("fStsMuchCorrProf",
      Form( "STS - MUCH digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb STS Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // STS vs. TOF
  fStsTofCorrProf = new TProfile("fStsTofCorrProf",
      Form( "STS - TOF digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb TOF Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // STS vs. RICH
  fStsRichCorrProf = new TProfile("fStsRichCorrProf",
      Form( "STS - RICH digis Nb correlation per %.0f ns time interval; Nb STS Digis []; Nb RICH Digis []", fdBinWidthNs ),
      1000, 0, 1000 );

  // MUCH vs. TOF
  fMuchTofCorrProf = new TProfile("fMuchTofCorrProf",
      Form( "MUCH - TOF digis Nb correlation per %.0f ns time interval; Nb MUCH Digis []; Nb TOF Digis []", fdBinWidthNs ),
      1000, 0, 1000 );
  // MUCH vs. RICH
  fMuchRichCorrProf = new TProfile("fMuchRichCorrProf",
      Form( "MUCH - RICH digis Nb correlation per %.0f ns time interval; Nb MUCH Digis []; Nb RICH Digis []", fdBinWidthNs ),
      1000, 0, 1000 );

  // TOF vs. RICH
  fTofRichCorrProf = new TProfile("fTofRichCorrProf",
      Form( "TOF - RICH digis Nb correlation per %.0f ns time interval; Nb TOF Digis []; Nb RICH Digis []", fdBinWidthNs ),
      1000, 0, 1000 );

  for( UInt_t uStsDpb = 0; uStsDpb < kuMaxNbStsDpbs; ++uStsDpb )
  {
    fT0StsDpbCorr[uStsDpb] = new TH2F( Form("fT0StsDpbCorr%02u", uStsDpb),
        Form( "T0 - STS digis Nb correlation per %.0f ns time interval, DPB %02u; Nb T0 Digis []; Nb STS Digis []; Counts", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000,
        1000, 0, 1000);
    fStsMuchDpbCorr[uStsDpb] = new TH2F( Form("fStsMuchDpbCorr%02u", uStsDpb),
        Form( "STS - MUCH digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb STS Digis []; Counts", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000,
        1000, 0, 1000);
    fStsTofDpbCorr[uStsDpb] = new TH2F( Form("fStsTofDpbCorr%02u", uStsDpb),
        Form( "STS - TOF digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb TOF Digis []; Counts", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000,
        1000, 0, 1000);
    fStsRichDpbCorr[uStsDpb] = new TH2F( Form("fStsRichDpbCorr%02u", uStsDpb),
        Form( "STS - RICH digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb RICH Digis []; Counts", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000,
        1000, 0, 1000);

    fT0StsDpbCorrProf[uStsDpb] = new TProfile( Form("fT0StsDpbCorrProf%02u", uStsDpb),
      Form( "T0 - STS digis Nb correlation per %.0f ns time interval, DPB %02u; Nb T0 Digis []; Nb STS Digis []", fdBinWidthNs, uStsDpb ),
      1000, 0, 1000 );
    fStsMuchDpbCorrProf[uStsDpb] = new TProfile( Form("fStsMuchDpbCorrProf%02u", uStsDpb),
        Form( "STS - MUCH digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb STS Digis []", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000 );
    fStsTofDpbCorrProf[uStsDpb] = new TProfile( Form("fStsTofDpbCorrProf%02u", uStsDpb),
        Form( "STS - TOF digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb TOF Digis []", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000 );
    fStsRichDpbCorrProf[uStsDpb] = new TProfile( Form("fStsRichDpbCorrProf%02u", uStsDpb),
        Form( "STS - RICH digis Nb correlation per %.0f ns time interval, DPB %02u; Nb STS Digis []; Nb RICH Digis []", fdBinWidthNs, uStsDpb ),
        1000, 0, 1000 );
  } // for( UInt_t uStsDpb = 0; uStsDpb < kuMaxNbStsDpbs; ++uStsDpb )
...
void CbmCheckDigisNbCorr::Exec(Option_t* /*option*/)
{
  ...
  for( Int_t iDigi = 0; iDigi < nrT0Digis; ++iDigi ) {
    ...
    else if ( fT0DigiArr ) pDigi = dynamic_cast<const CbmTofDigi*>(fT0DigiArr->At(iDigi));
    assert(pDigi);

    /// Ignore pulser hits in T0
    if( fuMinTotPulserT0 < pDigi->GetCharge() && pDigi->GetCharge() < fuMaxTotPulserT0 )
      continue;
  ...
  for( Int_t iDigi = 0; iDigi < nrTofDigis; ++iDigi ) {
    ...
    if( 92 < pDigi->GetCharge() && pDigi->GetCharge() < 96 )
      continue;
