...
{
  fEventSize = new TH1F("fEventSize","Event Size; # Digis; Counts",
			1000, -0.5, 999.5);
  fEventLength = new TH1F("fEventLength","Event Length; time [ns]; Counts",
			1000, -0.5, 999.5);
  ...
  fTofInEvent = new TH1F("fTofInEvent",
			"Number of Tof digis in Event; # digis; Counts",
			 1000, -0.5, 999.5);
  fT0DeltaT = new TH1F("fT0DeltaT",
			"Time diff between first and last T0 digi;dt [ns]; Counts",
			 1000, -0.5, 999.5);
  fStsDeltaT = new TH1F("fStsDeltaT",
			"Time diff between first and last Sts digi;dt [ns]; Counts",
			 1000, -0.5, 999.5);
  fMuchDeltaT = new TH1F("fMuchDeltaT",
			"Time diff between first and last Much digi;dt [ns]; Counts",
			 1000, -0.5, 999.5);
  fTofDeltaT = new TH1F("fTofDeltaT",
			"Time diff between first and last Tof digi;dt [ns]; Counts",
			 1000, -0.5, 999.5);

  fEventsvsTS = new TH2F("fEventsvsTS", "Nr. of events as fct. of TS",
			 10000, -0.5, 9999.5, 1000, -0.5, 999.5);
  fLengthvsTS = new TProfile("fLengthvsTS", "Length of events as fct. of TS",
			 10000, -0.5, 9999.5, -0.5, 999.5);
}
// ---- Exec ----------------------------------------------------------
...
template <class Digi>
void CbmCheckEvents::GetTimeDiff(CbmEvent* event, TH1* deltaT, TH1* size,
...
void CbmCheckEvents::GetTimeDiffT0(CbmEvent* event, TH1* deltaT, TH1* size)
{
  ...
  for (Int_t iDigi = 0; iDigi < nDigis; iDigi++) {
    ...
    else if ( fT0DigiArr ) digi = dynamic_cast<CbmTofDigi*>(fT0DigiArr->At(index));
    assert(digi);
    if (digi->GetTime() < startTime) startTime=digi->GetTime();
    if (digi->GetTime() > stopTime) stopTime=digi->GetTime();
  }
