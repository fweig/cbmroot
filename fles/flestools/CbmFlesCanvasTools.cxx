...
CanvasConfig::CanvasConfig( std::string sName, std::string sTitle,
                            uint32_t uNbPadsX, uint32_t uNbPadsY ) :
   fsName( sName ),
   fsTitle( sTitle ),
...
bool        CanvasConfig::GetGridx( uint32_t uPadIdx ) const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetGridx => Pad index out of bounds, returning false! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return false;
...
bool        CanvasConfig::GetGridy( uint32_t uPadIdx ) const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetGridy => Pad index out of bounds, returning false! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return false;
      } // else of if( uPadIdx < fuNbPads )
...
bool        CanvasConfig::GetLogx( uint32_t uPadIdx )  const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetLogx => Pad index out of bounds, returning false! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return false;
      } // else of if( uPadIdx < fuNbPads )
...
bool        CanvasConfig::GetLogy( uint32_t uPadIdx )  const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetLogy => Pad index out of bounds, returning false! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return false;
      } // else of if( uPadIdx < fuNbPads )
...
bool        CanvasConfig::GetLogz( uint32_t uPadIdx )  const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetLogz => Pad index out of bounds, returning false! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return false;
      } // else of if( uPadIdx < fuNbPads )
...
uint32_t    CanvasConfig::GetNbObjsInPad( uint32_t uPadIdx ) const
{
   ...
   if( uPadIdx < fuNbPads )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetLogz => Pad index out of bounds, returning 0! "
                   << uPadIdx << " VS " << fuNbPads
                   << std::endl;
         return 0;
      } // else of if( uPadIdx < fuNbPads )
...
std::string CanvasConfig::GetObjName( uint32_t uPadIdx, uint32_t uObjIdx ) const
{
   ...
   if( uPadIdx >= fuNbPads )
   {
      std::cerr << "CanvasConfig::GetObjName => Pad index out of bounds, returning nullptr! "
                << uPadIdx << " VS " << fuNbPads
                << std::endl;
   ...
   if( uObjIdx < GetNbObjsInPad( uPadIdx ) )
   {
      ...
      {
         std::cerr << "CanvasConfig::GetObjName => Object index out of bounds, returning nullptr! "
                   << "Pad " << uPadIdx << " " << uObjIdx << " VS " << GetNbObjsInPad( uPadIdx )
                   << std::endl;
         return std::string( "nullptr" );
...
void CanvasConfig::SetNbPadsX( uint32_t uNbColumns )
{
   if( fuNbPadsX != uNbColumns )
   {
      ...
      if( 0 < fuNbPads )
         std::cout << "CanvasConfig::SetNbPadsX => Warning: Number of pads changed, stored configuration cleared!"
                   << std::endl;
...
void CanvasConfig::SetNbPadsY( uint32_t uNbRows  )
{
   if( fuNbPadsY != uNbRows )
   {
      /// Print warning in case some pads were already defined
      if( 0 < fuNbPads )
         std::cout << "CanvasConfig::SetNbPadsY => Warning: Number of pads changed, stored configuration cleared!"
                   << std::endl;
...
bool CanvasConfig::SetConfig( uint32_t uPadIdx, bool bGridx, bool bGridy,
                              bool bLogx, bool bLogy, bool bLogz,
...
CanvasConfig ExtractCanvasConfigFromString( std::string sFullConfig )
{
   ...
   for( UInt_t uPadIdx = 0; uPadIdx < uNbPads; ++uPadIdx )
   {
      if( 0 == sNext.size() )
      {
         std::cerr << "ExtractCanvasConfigFromString => Empty configuration string while "
                << uPadIdx << " over " << uNbPads << " pads remain! "
                << "last ones will have default config!"
                << std::endl;
      } // if( 0 == sNext.size() )
      ...
      /// Load Pad config
      conf.SetConfig( uPadIdx, bGridx, bGridy, bLogx, bLogy, bLogz,
                     vsObjName, vsOptions );
