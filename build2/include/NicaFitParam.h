 ...
 */
#ifndef NICAFEMTO_FEATURES_MINIMIZER_NICAFITPARAM_H_
#define NICAFEMTO_FEATURES_MINIMIZER_NICAFITPARAM_H_


#include <TObject.h>
#include <TString.h>

class NicaFitParam : public TObject {
  Bool_t fIsFixed;
  Bool_t fIsDiscrete;
  Double_t fMin;
  Double_t fMax;
...
public:
  ...
  void Init();
  virtual ~NicaFitParam();
  NicaFitParam& operator=(const NicaFitParam& other) = default;

  void SetMapRange(Double_t min, Double_t max, Int_t points);
  void SetDParam(Double_t step) { fDParam = step; }
  ...
  ClassDef(NicaFitParam, 1)
};


#endif /* NICAFEMTO_FEATURES_MINIMIZER_NICAFITPARAM_H_ */
