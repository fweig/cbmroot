 ...
 */
class NicaTrack : public TObject {

protected:
  /**
   * pointer to parent event
   */
  NicaEvent* fEvent;  //! don't stream it will be build on the fly
  TLorentzVector* fP;
  Int_t fID;
  Int_t fStatus;
  Int_t fHiddenInfo;
...
public:
   ...
   * @param track
   */
  NicaTrack(const NicaTrack& track);
  /**
   * assignement operator
   * @param track
   * @return
   */
  NicaTrack& operator=(const NicaTrack& track);
  /**
   ...
   * @return 4-momentum
   */
  inline TLorentzVector* GetMomentum() const { return fP; };
  /**
   * boost this particle
   * @param vx
   * @param vy
  ...
  /**
   * clean variables before filling with values
   * @param thisID
   * @param event
   */
   ...
   */
  NicaEvent* GetEvent() const { return fEvent; };
  /**
   *
   ...
   * @param event
   */
  void SetEvent(NicaEvent* event) { fEvent = event; };
  /**
