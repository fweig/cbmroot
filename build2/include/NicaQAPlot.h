 ...
 */
#ifndef NICAFEMTO_ANALYSIS_QA_NICAQAPLOT_H_
#define NICAFEMTO_ANALYSIS_QA_NICAQAPLOT_H_
...
/**
 * base class for QA plots, base on NicaQAReport that is representation of
...
class NicaQAPlot : public TNamed {
  ENicaCutUpdate fUpdate;
  std::vector<NicaQAHistoSettings> fSettings[3];
  NicaQAPlotReport* fReport;
...
protected:
  ...
  void FillTrack(NicaTrack* track);
  void FillEvent(NicaEvent* event);
  /**
   * @param track track to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillTrackCustom1D(NicaTrack* track, TH1D* h, Int_t no) {};
  /**
   * @param event event to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillEventCustom1D(NicaEvent* event, TH1D* h, Int_t no) {};
  /**
   * @param track track to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillTrackCustom2D(NicaTrack* track, TH2D* h, Int_t no) {};
  /**
   * @param event event to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillEventCustom2D(NicaEvent* event, TH2D* h, Int_t no) {};
  /**
   * @param track track to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillTrackCustom3D(NicaTrack* track, TH3D* h, Int_t no) {};
  /**
   * @param event event to fill
   * @param h histogram to fill
   * @param no id filled histogram
   */
  virtual void FillEventCustom3D(NicaEvent* event, TH3D* h, Int_t no) {};

public:
  NicaQAPlot();
  NicaQAPlot(ENicaCutUpdate upd);
  /**
   *
   ...
   */
  NicaQAPlot(TString name, Int_t oneDimHist, Int_t twoDimHist, Int_t threeDimHist, ENicaCutUpdate upd);
  /**
   * adds TH1 histogram
   * @param name
   ...
   * @return
   */
  virtual NicaQAPlot* MakeCopy() const { return (NicaQAPlot*) this->Clone(); };
  /**
  ...
  ENicaCutUpdate GetUpdateRatio() const { return fUpdate; }
  virtual void Print(Option_t* opt = "") const;
  virtual ~NicaQAPlot();
  NicaQAPlotReport* GetReport() const;
  NicaQAPlot(const NicaQAPlot& other);
  NicaQAPlot& operator=(const NicaQAPlot& other);
  ClassDef(NicaQAPlot, 1)
};

#endif /* NICAFEMTO_ANALYSIS_QA_NICAQAPLOT_H_ */
