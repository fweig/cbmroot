 ...
 */
#ifndef NICAFEMTO_ANALYSIS_QA_NICAQAPLOTREPORT_H_
#define NICAFEMTO_ANALYSIS_QA_NICAQAPLOTREPORT_H_
...
/**
 * base class for storing plots from NicaQAPlot based  task, this class only
 * stores histograms and information about operations that have to be performed
 * on histograms. Using NicaQAPlotReport instead of saving custon QAPlot allow
 * to keep simplicity of output files. To extract such files please use
 * NicaQAExtractor.
 */
class NicaQAPlot;

class NicaQAPlotReport : public NicaPack {
  friend class NicaQAPlot;
...
private:
  ...
  NicaPack* fExtraInfo;
  NicaParameterString fOriginName;

protected:
  NicaHistogramManager_1_1D<TH1D>* f1dHistos;
  NicaHistogramManager_1_2D<TH2D>* f2dHistos;
  NicaHistogramManager_1_3D<TH3D>* f3dHistos;
  /**
   ...
   * @param upd
   */
  NicaQAPlotReport(TString name, Int_t oneDim, Int_t twoDim, Int_t threeDIm);

  NicaHistogramManager_1_1D<TH1D>* GetHistArray1D() const { return f1dHistos; };
  NicaHistogramManager_1_2D<TH2D>* GetHistArray2D() const { return f2dHistos; };
  NicaHistogramManager_1_3D<TH3D>* GetHistArray3D() const { return f3dHistos; };

public:
  NicaQAPlotReport();
  NicaQAPlotReport(const NicaQAPlot& other);
  NicaQAPlotReport(const NicaQAPlotReport& other);
  NicaQAPlotReport& operator=(const NicaQAPlotReport& other);
   ...
   * @param p
   */
  virtual void SetExtraInfo(NicaPack* p);
   ...
   */
  NicaPack* GetExtraInfo() const { return fExtraInfo; };
  virtual Long64_t Merge(TCollection* collection);
  virtual ~NicaQAPlotReport();
  ClassDef(NicaQAPlotReport, 1)
};

#endif /* NICAFEMTO_ANALYSIS_QA_NICAQAPLOTREPORT_H_ */
