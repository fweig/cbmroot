...
#define Draw_3D_option ""
#define NICA_PHYSICALANALYSYS_VER "sep2020"

// some enums used almost everywhere
enum class ENicaInfo {
  kLessInfo         = 0,
  kImportantInfo    = 1,
  kLessWarning      = 2,
  kImportantWarning = 3,
...
enum class ENicaFormatType { kSim = 0, kReco = 1, kComplexReco = 2, kComplexSim = 3 };

enum class ENicaRound { kSeparator, kPrefix };
enum class ENicaCutUpdate {
  kNoUpdate                 = -1,
  kEventUpdate              = 0,
  kTrackUpdate              = 1,
  kTwoTrackUpdate           = 2,
  kTwoTrackBackgroundUpdate = 3
};

enum class ENicaFormatDepth { kAll, kBuffered, kNonBuffered };
...
namespace NicaStd {
   ...
   * @return
   */
  TString UpdateEnumToString(ENicaCutUpdate upd);
  /**
   * copy files
  ...
  /**
   *
   * @return path to nicaroot plus directory - in case of installation with
   * FAIRROOT its $FAIRROOT/share/fairbase/examples/nicaroot_plus otherwise
   * $VMCWORKIDR/nicaroot/features/nicaroot_plus
   */
  TString GetNicarootPlus();
  /**
   *
   * @param par_name parameter name
   * @return value of parameter from .nicaroot_config.xml file
   */
