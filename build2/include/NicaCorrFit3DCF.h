...
#ifndef NICACORRFIT3DCF_H_
#define NICACORRFIT3DCF_H_

#include "NicaCorrFitFunc.h"

class NicaCorrFitMath3DCF;
...
class NicaCorrFit3DCF : public NicaCorrFitFunc {
...
private:
  friend class NicaCorrFitMath3DCF;
  friend class NicaCorrFitSHCF;
  ...
  NicaArray_1<Double_t>* fXbins;
  NicaArray_1<Double_t>* fYbins;
  NicaArray_1<Double_t>* fZbins;
  Double_t fXBinHalf;
  Double_t fYBinHalf;
  Double_t fZBinHalf;
  Double_t fXAxisHalf;
  ...
  void SwapMap(TH3* h);
  void CalculateHalf(Double_t width);
...
public:
   ...
   */
  NicaCorrFit3DCF(Int_t parameters = 3);
  /**
   ...
   * @param r fit mask  - kStandard - nothing to chage, kSlice - use only "slice bins" , kCrossSlice - use only diagonals + slice
   */
  void SetFitExtraMask(EFitExtraMask r) { fFitMaskFlag = r; };
  virtual ~NicaCorrFit3DCF();
  ClassDef(NicaCorrFit3DCF, 1)
};

#endif /* NICACORRFIT3DCF_H_ */
