 ...
 */
#ifndef NICAFEMTOSHCF_H_
#define NICAFEMTOSHCF_H_
...
class NicaCorrFitSHCF;

/**
 * class for storing sphercial harmonics correlation functions
 */
class NicaFemtoSHCF : public NicaDividedHisto1D {
  const Int_t fMaxJM;
  ...
  Int_t fMaxL;
  NicaArray_1<Double_t>* covmnum;  // Covariance matrix for the numerator
  NicaArray_1<Double_t>* covmden;  // Covariance matrix for the denominator
  NicaArray_1<Double_t>* covmcfc;  // Covariance matrix for the  CF
...
protected:
   ...
   * @param obj
   */
  void FastAdd(const NicaFemtoSHCF* obj);
...
public:
   ...
   */
  NicaFemtoSHCF(TString name, Int_t maxL, Int_t bins, Double_t min, Double_t max, ENicaFemtoKinematics kinematics);
  /**
   * copy constructor
   * @param other
   */
  NicaFemtoSHCF(const NicaFemtoSHCF& other);
  void FillNumObj(TObject* obj);
  void FillDenObj(TObject* obj);
  /**
   * primitive constructor to conver angular NicaFemto3DCF into SH CF
   * @param cf
   * @param maxL
   */
  /* TODO implenet this function in proper way
   NicaFemtoSHCF(NicaFemto3DCF *cf, Int_t maxL=3);
   */
  /**
  ...
  NicaArray_1<Float_t>* ExportToFlatNum() const;
  virtual void Add(const NicaPack* pack);
  virtual Long64_t Merge(TCollection* collection);
  virtual TString HTMLExtract(Int_t counter = 0, TString dir = " ") const;
  virtual ~NicaFemtoSHCF();
  ClassDef(NicaFemtoSHCF, 4)
};

#endif /* NICAFEMTOSHCF_H_ */
