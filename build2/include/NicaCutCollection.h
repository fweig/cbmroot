...
#ifndef NICACUTSUBCONTAINER_H_
#define NICACUTSUBCONTAINER_H_
 ...
 */
class NicaCutCollection : public TObject {
private:
  ...
  NicaArray_2<Int_t>* fPreviousAddress;
  NicaArray_2<Int_t>* fNextAddress;
  NicaArray_1<Int_t>* fNextNo;
  NicaArray_1<Int_t>* fPreviousNo;
  ENicaCutUpdate fMode;  // mode event/ track. two track
  Bool_t fInit, fDummy;
   ...
   */
  Int_t MatchCut(NicaCut* address, NicaCutCollection* input);
  /**
   ...
   * false otherwise
   */
  Bool_t CheckIfComptatiblie(NicaCut* cut1, NicaCut* cut2);
  /**
   *
   * @param cutname cut name (not cut-class name)
   * @return cut update ratio
   */
  ENicaCutUpdate GetUpdateFromName(TString cutname) const;
  inline TObjArray* GetCutContainerArray(ENicaCutUpdate upd) const {
...
public:
  /**
   * Constructor used by NicaCutCollection
   ...
   */
  NicaCutCollection(TObjArray** container, Int_t cont_size, ENicaCutUpdate update, Int_t collectionNo);
  /**
   ...
   * @return
   */
  NicaCutCollection* MakeNewCopy(TObjArray** container) const;
  /**
   ...
   */
  void AddCut(NicaCut* cut, Option_t* opt = " ");
  /**
   ...
   */
  void AddCutMonitor(NicaCutMonitor* monitor);
  /**
   ...
   */
  void MakeDummyCopy(const NicaCutCollection* copy, Bool_t copy_high_links = kTRUE);
  /**
   * compare to NicaCutCollection class object
   ...
   */
  Bool_t PassEvent(NicaEvent* obj);
  /**
   * check track cuts
   * @param obj track to check
   * @return true if object ok, false if object shoudn't pass cuts
   */
  Bool_t PassTrack(NicaTrack* obj);
  /**
   * check pair cuts
   * @param pair pair to check
   * @return true if object ok, false if object shoudn't pass cuts
   */
  Bool_t PassTwoTrack(NicaTwoTrack* pair);
  /**
  ...
  /**
   * In some analysis like NicaEventAnaChain it's possible to use subtasks. In
   ...
   * @return slow cut
   */
  NicaCut* GetCut(Int_t index) const { return (NicaCut*) fCuts->UncheckedAt(index); };
  /**
   * return fast cut with given position
   * @param index cut position
   * @return fast cut
   */
  NicaCut* GetFastCut(Int_t index) const { return (NicaCut*) fFastCuts->UncheckedAt(index); };
  /**
   * return cut with given name
   * @param name cut name (name of class)
   * @return cut
   */
  NicaCut* FindCut(TString name) const;
  /**
   *
   * @param index cut monitor index
   * @return cut monitor
   */
  NicaCutMonitor* GetCutMonitor(Int_t index) const;
  /**
   * copy this cut container with new collection number
   * @param new_collection collection number of copy
   * @return copy of this
   */
  NicaCutCollection* Replicate(Int_t new_collection) const;
  /**
   *
  ...
  virtual ~NicaCutCollection();
  ClassDef(NicaCutCollection, 1)
};
#endif /* NICACUTSUBCONTAINER_H_ */
