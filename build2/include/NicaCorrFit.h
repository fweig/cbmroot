 ...
 */
#ifndef NICACORRFIT_H_
#define NICACORRFIT_H_
 ...
 * functions
 */
class NicaCorrFitGUI;
class NicaCorrFit : public TObject {
  friend class NicaCorrFitGUI;
...
protected:
   ...
   * max step
   */
  std::vector<NicaFitParam> fParameters;
...
public:
  NicaCorrFit(Int_t parameters_no = 2);
  /**
  ...
  /**
   * set maximum iteration step
   * @param val
   * @param par
   */
  void SetMaxStep(Double_t val, Int_t par) { fParameters[par].SetDParam(val); };
  /**
   *
   ...
   */
  const NicaFitParam& GetParamConf(Int_t par) const { return fParameters[par]; }
  /**
   *
  ...
  virtual ~NicaCorrFit();
  ClassDef(NicaCorrFit, 2)
};

#endif /* NICACORRFIT_H_ */
