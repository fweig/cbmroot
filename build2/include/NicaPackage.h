...
#ifndef NICAPACKAGE_H_
#define NICAPACKAGE_H_
 ...
 *  In contrast to NicaPack its not one large structure but rather bunch of
 * objects. Forr example each cut reate own report in form of NicaPackage. Thank
 * to this saving cuts is avoided (what could lead to problems because cuts are
 * frequently changed and framework-dependent.
 */
class NicaPackage : public TObject {
  ...
  void SumNicaParameter(TObject *first, const TObject *second);
 ...
 public:
   ...
   */
  NicaPackage(const TObject *object, Bool_t cut_template = kFALSE);
  /**
   ...
   */
  void Add(const NicaPackage *pack);
  /**
   ...
   */
  void RemoveObjectByName(TString name, Int_t index = 0,
   ...
   * @return this
   */
  NicaPackage &operator+(const NicaPackage &rhs);
  virtual ~NicaPackage();
  ClassDef(NicaPackage, 1)
};

#endif /* NICAPACKAGE_H_ */
