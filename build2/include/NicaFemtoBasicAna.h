...
#ifndef NICAFEMTOBASICANA_H_
#define NICAFEMTOBASICANA_H_
 ...
 */
class NicaFemtoBasicAna : public NicaTwoTrackAna {
protected:
  /**
   ...
   */
  NicaFemtoWeightGenerator* fCalc;
  /**
  ...
  // converting to NicaPair, using preprocess method to compute stuff like
...
public:
   ...
   * @param ana
   */
  NicaFemtoBasicAna(const NicaFemtoBasicAna& ana);
  /**
   * assignement operator
   * @param other
   * @return
   */
  NicaFemtoBasicAna& operator=(const NicaFemtoBasicAna& other);
...
#endif
  virtual void
  SetCorrFctn(NicaFemtoCorrFunc* h);
  /**
   * set correlaction function
   * @param h correlaction function class
   */
  virtual void SetCorrFctn(const NicaFemtoCorrFunc& h);
  /**
   * set weight algorithm used for calculation CF in MC data
   * @param gen
   */
  void SetWeight(const NicaFemtoWeightGenerator& gen) { fCalc = gen.MakeCopy(); }
  /**
   * set freezout generator that creates points of particles creation
   * @param gen freezout generator
   */
  void SetFreezoutGenerator(const NicaFemtoFreezoutGenerator& gen) { fFreezoutGenerator = gen.MakeCopy(); }
  /**
  ...
  /**
   * set option
   * @param option - option from NicaTwoTrackAna or  "ignore_sign" if user want
  ...
  virtual ~NicaFemtoBasicAna();
  ClassDef(NicaFemtoBasicAna, 1)
};

#endif /* NICAFEMTOBASICANA_H_ */
