/*
 * NicaMemoryMapManager.h
 *
 *  Created on: 28-10-2013
 *      Author: Daniel Wielanek
 *		E-mail: daniel.wielanek@gmail.com
 *		Warsaw University of Technology, Faculty of Physics
 */

#ifndef NICAMEMORYMAPMANAGER_H_
#define NICAMEMORYMAPMANAGER_H_


#include "NicaEvent.h"
#include "NicaArray.h"

class FairRootManager;
class NicaCutContainer;

class NicaMiniEventTask;
/**
 * class for storing  fake events
 */
class NicaEventArray : public TObject {
 public:
  /**
   * size of array
   */
  Int_t fSize;
  /**
   * array of events
   */
  NicaEvent **fArray;  //[fSize]
  /**
   * default ctor
   */
  NicaEventArray(Int_t task_id, Int_t size = 1);
  /**
   * return pointer ot fake event
   * @param size position of event into array
   * @return
   */
  inline NicaEvent *At(const Int_t size) { return fArray[size]; };
  /**
   * default d-tor
   */
  virtual ~NicaEventArray() {
    for (int i = 0; i < fSize; i++) delete fArray[i];
    delete[] fArray;
  };
  ClassDef(NicaEventArray, 1)
};
/**
 * class for managing buffered events
 */
class NicaMemoryMapManager : public TObject {
  friend class NicaMiniEventTask;

 private:
  Bool_t fUseCompression;
  Bool_t fDirectAcces;
  Int_t fTrackMapSize, fFormatID;
  Int_t fEventCollectionsNo, fTrackCollectionsNo, fMixSize;
  Int_t *fEventToTrackNo;  //[fEventCollectionsNo]
  Int_t *fCounter;         //[fEventCollectionsNo]
  Int_t *fTotalTracks;     //[fEventCollectionsNo]
  Bool_t *fReadyToMix;     //[fEventCollectionsNo]
  FairRootManager *fManager;
  NicaEventArray **fEvents;  //[fEventCollectionsNo]
  //"true" collections x mix_size
  Int_t *fSumMap;    //[fTrackMapSize]
  Int_t *fIndexMap;  //[fTrackMapSize]
  Int_t fSumMapSize;
  Int_t fMaxTrackCollectionNo;
  NicaEvent *fCurrentEvent;  // pointer to processed event
  NicaArray_4<Int_t> *fTrackMap;
  NicaArray_3<Int_t> *fTrackCounter;
  void ReloadMap(Int_t size);
  void CalculateCompressedMap(Int_t event_collection);
  Int_t GetTrackCollectionsNo(Int_t event_collection) const;
  Int_t GetCounter(Int_t event_collection, Int_t track_collection) const;
  /**
   * reset track maps for all track collections for given event
   * @param event_collection event collection
   * @param counter position of event in buffer
   */
  void ResetTrackMaps(Int_t event_collection, Int_t counter);
  /**
   * reset track maps
   * @param event_collection event collection
   * @param track_collection track collection
   * @param counter event position in buffer
   */
  void ResetTrackMaps(Int_t event_collection, Int_t track_collection,
                      Int_t counter);

 public:
  /**
   * empty constructor (not used)
   */
  NicaMemoryMapManager();
  /**
   * default constructor
   * @param const pointer to NicaCutContainer with cuts used in analysis
   */
  NicaMemoryMapManager(NicaCutContainer *cont);
  /**
   * return pointer to current event
   */
  NicaEvent *GetTemporaryEvent();
  /**
   * set size of mixing buffer
   * @param mix_size mixing buffer size
   */
  void SetMixSize(Int_t mix_size);
  /**
   *
   * @return number of track in currently processed event (stored in tree)
   */
  Int_t GetTemporaryTotalTracksNo() const;
  /**
   * initialize this manager
   * @param task_id data format (and  task id)
   * @param use_source if true then original structure is copied into memory
   * @param compress - if true then evets will be compressed
   * @param direct  - if true then direct reading NicaEvent-ana class from tree
   * is performed
   */
  void Init(Int_t task_id, Bool_t use_source, Bool_t compress, Bool_t direct);
  /**
   * initialize this manager
   * @param event_factor - numer of artificial multiplication of event triggers
   * @param task_id data format (and  task id)
   * @param use_source if true then original structure is copied into memory
   * @param compress - if true then evets will be compressed
   * @param direct  - if true then direct reading NicaEvent-ana class from tree
   * is performed
   */
  void Init(Int_t event_factor, Int_t task_id, Bool_t use_source,
            Bool_t compress, Bool_t direct);
  /**
   * add track to map
   * @param event_collection even collection number
   * @param track_collection track collection number
   * @param index position of added track in track array
   */
  void AddTrackToMapTrack(Int_t event_collection, Int_t track_collection,
                          Int_t index);
  /**
   * buffer event with memory
   * @param collection collection number of event
   */
  void BufferEvent(Int_t collection);
  /**
   * return processed event (from buffer)
   * @param collection - event collection
   * @return event
   */
  NicaEvent *GetEvent(Int_t collection) const;
  /**
   * fill track from buffered event (last buffered)
   * @param event_collection event collection number
   * @param track_collection track collection number
   * @param index position in map
   * @return track
   */
  NicaTrack *GetTrack(Int_t event_collection, Int_t track_collection,
                      Int_t index) const;
  /**
   *
   * @param track_collection track collection
   * @param index number
   * @return track from temporary event via map
   */
  NicaTrack *GetTemporaryTrack(Int_t event_collection, Int_t track_collection,
                               Int_t index) const;
  /**
   * get track from buffered event (last buffered)
   * @param event_collection event collection no
   * @param index position in track array
   * @return track
   */
  NicaTrack *GetRawTrack(Int_t event_collection, Int_t index) const;
  /**
   * @param collection event collection number
   * @param counter position in event buffer
   * @return event
   */
  NicaEvent *GetEvent(Int_t collection, Int_t counter) const;
  /**
   * return track from event
   * @param event_collection event collection number
   * @param track_collection track collection number
   * @param counter position of event in buffer
   * @param index position of particle in map
   * @return track from event
   */
  NicaTrack *GetTrack(Int_t event_collection, Int_t track_collection,
                      Int_t counter, Int_t index) const;
  /**
   * return raw track from event
   * @param event_collection event collection number
   * @param counter position of event in buffer
   * @param index position of particle in track array
   * @return raw track (without using maps)
   */
  NicaTrack *GetRawTrack(Int_t event_collection, Int_t counter,
                         Int_t index) const;
  /**
   * reject last event from buffer
   * @param event_collection event collection of rejected event
   */
  void RejectLastEvent(Int_t event_collection);
  /**
   * reset buffer, track maps and event buffer become empty
   * @param collection event collection number
   */
  void ResetEventCollection(Int_t collection);
  /**
   * clear event stored in tree
   */
  void ClearEvent();
  /**
   * preapre maps of tracks for event that potentialy will be buffered
   * @param collection
   */
  void PrepareMaps(Int_t collection);
  /**
   * print map of tracks
   */
  void PrintMap() const;
  /**
   *
   * @param event_collection event collection number
   * @return position of currently processed (or last buffered) event in array
   */
  Int_t GetCounter(Int_t event_collection) const;
  /**
   * return number of track in latest buffered event (or current)
   * @param event_collection event collection no
   * @param track_collection track collection no
   * @return number of tracks in map
   */
  Int_t GetTracksNo(Int_t event_collection, Int_t track_collection) const;
  /**
   *
   * @param event_collection event collection no
   * @param track_collection track collection no
   * @param counter event position
   * @return number of tracks
   */
  Int_t GetTracksNo(Int_t event_collection, Int_t track_collection,
                    Int_t counter) const;
  /**
   *
   * @param event_collection event collection number
   * @return  number of tracks in current (or latest buffered) event
   */
  Int_t GetRawTracksNo(Int_t event_collection) const;
  /**
   * check if can mix events
   * @param collection event collection no
   * @return true if buffer is full
   */
  Bool_t IsReadyToMixing(Int_t collection) const;
  virtual ~NicaMemoryMapManager();
  ClassDef(NicaMemoryMapManager, 1)
};

#endif /* NICAMEMORYMAPMANAGER_H_ */
