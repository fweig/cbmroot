 ...
 */
#ifndef NICADIVIDEDHISTO1D_H_
#define NICADIVIDEDHISTO1D_H_
 ...
 * copy value of pointer, in such case deleting NicaDividedHisto don't release
 * memory of all internal histograms, histograms added with option "clone" are
 * copied inside of this class. In such case NicaDividedHisto delete those
 * histograms if delete was called or if numerator/denominator is overwritten by
 * new histogram.
 */
class NicaDividedHisto1D : public NicaPack {
 private:
  /**
 ...
 protected:
   ...
   */
  NicaDividedHisto1D(TString name, const Int_t dim);
  /**
 ...
 public:
   ...
   * @param name,
   */
  NicaDividedHisto1D(TString name) : NicaDividedHisto1D(name, 1){};
  /**
   * main constructor
   ...
   */
  NicaDividedHisto1D(TString name, Int_t nbins, Double_t min, Double_t max,
                     Char_t type = 'D');
  /**
   * copy constructor
   * @param other object to copy
   */
  NicaDividedHisto1D(const NicaDividedHisto1D &other);
  /**
   * operator =
   * @param other
   * @return
   */
  NicaDividedHisto1D &operator=(const NicaDividedHisto1D &other);
  /**
  ...
  virtual ~NicaDividedHisto1D();
  ClassDef(NicaDividedHisto1D, 4)
};
/**
 * class for storing divided 2-dim histograms
 */
class NicaDividedHisto2D : public NicaDividedHisto1D {
 protected:
  virtual void Normalize(TH1 *h) const;
  virtual void SetHistoName(TH1 *h) const;
  /**
   ...
   * @param dim
   */
  NicaDividedHisto2D(TString name, const Int_t dim)
      : NicaDividedHisto1D(name, dim){};
  /**
   * return projection
 ...
 public:
   ...
   * @param name
   */
  NicaDividedHisto2D(TString name) : NicaDividedHisto1D(name, 2){};
  /**
   * main constructor
   * @param name name of this object
   ...
   */
  NicaDividedHisto2D(TString name, Int_t binsX, Double_t minX, Double_t maxX,
                     Int_t binsY, Double_t minY, Double_t maxY,
                     Char_t type = 'D');
  /**
   * copy constructor
   * @param other
   */
  NicaDividedHisto2D(const NicaDividedHisto2D &other);
  /**
  ...
  TH1D *GetProjection1D(Double_t min, Double_t max, Option_t *opt) const {
    return Projection2DTo1D(min, max, opt);
  }
  /**
   *  calucalte denominator/numerator ratio at given range
   * @param min lower norm range
   ...
   * @return
   */
  Double_t CalculateNorm(Double_t minX, Double_t maxX, Double_t minY,
                         Double_t maxY) const;
  virtual void Browse(TBrowser *b);
  /**
   * operator =
   ...
   * @return
   */
  NicaDividedHisto2D &operator=(const NicaDividedHisto2D &other);
  ~NicaDividedHisto2D();
  ClassDef(NicaDividedHisto2D, 2)
};
/**
 * array for storing divided 3-dim histograms
 */
class NicaDividedHisto3D : public NicaDividedHisto2D {
 protected:
   ...
   * @return projection
   */
  virtual TH1D *Projection3DTo1D(Double_t min1, Double_t max1, Double_t min2,
                                 Double_t max2, Option_t *opt) const;
 ...
 public:
   ...
   * @param name name
   */
  NicaDividedHisto3D(TString name) : NicaDividedHisto2D(name, 3){};
  /**
   *
   * @param name
   * @param binsX
   ...
   */
  NicaDividedHisto3D(TString name, Int_t binsX, Double_t minX, Double_t maxX,
                     Int_t binsY, Double_t minY, Double_t maxY, Int_t binsZ,
                     Double_t minZ, Double_t maxZ, Char_t type = 'D');
  /**
   * copy constructor
   * @param other
   */
  NicaDividedHisto3D(const NicaDividedHisto3D &other);
  /**
   * fill numerator
   * @param x x - value
   * @param y y - value
   ...
   * @return projection
   */
  TH1D *GetProjection1D(Double_t min1, Double_t max1, Double_t min2,
                        Double_t max2, Option_t *opt) const {
   ...
   * @see Fold1D
   */
  virtual void Fold3D(Double_t valX, Double_t valY, Double_t valZ,
                      Option_t *opt);
  /**
   * return projection
   * @param min projection lower range
   ...
   * NicaStd::GetProjection2D).
   * @return projection
   */
  virtual TH2D *GetProjection2D(Double_t min, Double_t max,
                                Option_t *opt) const;
  /**
   *  calucalte denominator/numerator ratio at given range
   ...
   */
  Double_t CalculateNorm(Double_t minX, Double_t maxX, Double_t minY,
                         Double_t maxY, Double_t minZ, Double_t maxZ) const;
  /**
   * operator =
   * @param other
   * @return
   */
  NicaDividedHisto3D &operator=(const NicaDividedHisto3D &other);
  virtual TString HTMLExtract(Int_t counter = 0, TString dir = " ") const;
  ~NicaDividedHisto3D();
  ClassDef(NicaDividedHisto3D, 3)
};

#endif /* NICADIVIDEDHISTO1D_H_ */
