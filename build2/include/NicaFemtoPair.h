...
#ifndef NICAFEMTOPAIRSTRUCT_H_
#define NICAFEMTOPAIRSTRUCT_H_
 ...
 */
class NicaFemtoMiniPair;
class NicaFemtoMicroPair;
class NicaFemtoPicoPair;
class NicaFemtoPair : public TObject {
protected:
  /**
...
public:
   ...
   */
  NicaFemtoPair(Bool_t enable_fake = kFALSE);
  /**
   ...
   * @return
   */
  NicaFemtoPair& operator=(const NicaFemtoMiniPair& other);
  NicaFemtoPair& operator=(const NicaFemtoMicroPair& other);
  NicaFemtoPair& operator=(const NicaFemtoPicoPair& other);
  ...
  virtual ENicaFemtoKinematics GetFrame() const = 0;
  Bool_t Init(Int_t task_id);
  /**
   * build femtoscpic pair fro pair of tracks
   * @param tracks
   */
  inline void Build(NicaTwoTrack* tracks) { BuildMomenta(tracks); }
  virtual ~NicaFemtoPair();
  /**
   *
   ...
   */
  void BuildMomenta(NicaTwoTrack* tracks);
  virtual TString GetTags() const = 0;
  ClassDef(NicaFemtoPair, 1)
};

#endif /* NICAFEMTOPAIR_H_ */
