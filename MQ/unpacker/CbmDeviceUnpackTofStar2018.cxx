...
void CbmDeviceUnpackTofStar2018::InitTask() try {
  ...
  if (Send(req, "parameters") > 0) {
    if (Receive(rep, "parameters") >= 0) {
      if (rep->GetSize() != 0) {
        ...
        fUnpackPar =
          static_cast<CbmTofUnpackPar*>(tmsg.ReadObject(tmsg.GetClass()));
...
Bool_t CbmDeviceUnpackTofStar2018::DoUnpack(const fles::Timeslice& ts,
                                            size_t component) {
  ...
  for (size_t m = 0; m < numCompMsInTs; ++m) {
	 ...
	 */
      ...
      if (fuNrOfGet4PerGdpb <= fuGet4Id && ngdpb::MSG_STAR_TRI != iMessageType
        ...
        LOG(warn) << "Message with Get4 ID too high: " << fuGet4Id << " VS "
                  << fuNrOfGet4PerGdpb << " set in parameters, message ignored";
        continue;
      }
      ...
      switch (mess.getMessageType()) {
        ...
        case ngdpb::MSG_GET4_32B: {
          if (fbEpochSuppModeOn) {
            ...
            LOG(debug) << "Add 32B message from Gdpb " << fuGdpbNr
                       << " to EpSupprBuffer of Get4 " << fuGet4Nr << " size "
                       << fvmEpSupprBuffer[fuGet4Nr].size();
          } else
            FillHitInfo(mess);
          break;
...
void CbmDeviceUnpackTofStar2018::PrintGenInfo(ngdpb::Message mess) {
  ...
  if (100 > iMess++)
    LOG(info) << "Get4 MSG type " << mType << " from rocId " << rocId
              << ", getId " << get4Id << ", (hit channel) " << channel
              << Form(" hex data %0lx ", uData);
}

void CbmDeviceUnpackTofStar2018::PrintSysInfo(ngdpb::Message mess) {
  LOG(info) << "GET4 System message,       epoch "
            << (fvulCurrentEpoch[fuGet4Nr]) << ", time " << std::setprecision(9)
  ...
  switch (mess.getGdpbSysSubType()) {
    case ngdpb::SYSMSG_GET4_EVENT: {
      LOG(info) << " +++++++ > Chip = " << std::setw(2)
                ...
                << std::setw(2) << mess.getGdpbSysErrData() << std::dec
                << " -- GET4 V1 Error Event";
      break;
    }  //
...
*/
      ...
      fDigi = new CbmTofDigiExp(
        0x00005006, dTime, dTot);  // fake start counter signal
      fBuffer->InsertData<CbmTofDigiExp>(fDigi);
      break;
    }  // case 3
...
void CbmDeviceUnpackTofStar2018::PrintMicroSliceDescriptor(
