...
void    CbmDeviceHitBuilderTof::CreateHistograms()
{
   ...
   fhEvDetMul = new TH1F("hEvDetMul",
			 "Detector multiplicity of Selector; Mul",
			 50, 0, 50);
...
Bool_t CbmDeviceHitBuilderTof::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
  ...
  if( 1 < fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size() )
  {
    ...
    for(Int_t i1=0; i1<(Int_t)fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()-1; i1++)
    {
      if( AddedHit ) break;
      Int_t i2=i1+1;
      while(!AddedHit && i2 < (Int_t)fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size() )
      {
...
Bool_t   CbmDeviceHitBuilderTof::MonitorPulser()
{
  ...
  if( iDigi0 == -1 ) {
    LOG(error) << Form("Pulser reference %d not found in pulser event %d of mul %d, return",
		       iDet0,iNPulserFound,fiNDigiIn);
    return kTRUE;
  }
  ...
  for (Int_t iDigi=0; iDigi<fiNDigiIn; iDigi++) {
	...
	if (TMath::Abs(fvDigiIn[iDigi].GetTime() - fvPulserTimes[iDet][iSide].back() - TDif) > 1.E3) {  // probably due to parallel processing
	...
	LOG(info) << "Unexpected pulser offset at ev "<< fdEvent<<", pulcnt "<< iNPulserFound
