...
TGeoVolume* ConstructLadder(Int_t LadderIndex, TGeoVolume* halfLadderU, TGeoVolume* halfLadderD, Double_t gapY,
                            Double_t shiftZ);
TGeoVolume* ConstructStation(Int_t iStation, Int_t nLadders, Int_t* ladderTypes, Double_t rHole, Int_t zStation);
...
void create_stsgeo_v16w(const char* geoTag = "v16w")
{
  ...
  */
  ...
  for (Int_t iStation = 0; iStation < 8; iStation++) {
    ...
    mystation[iStation] = ConstructStation(iStation, nLadders, ladderTypes, rHole[iStation], statPos[iStation]);
