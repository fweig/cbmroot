...
void create_stsgeo_v16h(const char* geoTag = "v16h") {
  ...
  infoFile << "STS geometry created with create_stsgeo_v16h.C" << endl << endl;
  ...
  */
  ...
  const Bool_t DisplaceRandom =
    false;  // true;  // add random displacement of modules for alignment study
  const Bool_t RotateRandom =
    true;  // false;  // add random rotation of modules for alignment study
  ...
  const Double_t maxdrotx = 0.0;  // 20.0; // max rotation around x
  const Double_t maxdroty = 0.0;  // 20.0; // max rotation around y
  ...
  TGeoRotation* unit_rotation = new TGeoRotation();
  ...
  for (Int_t iUnit = 0; iUnit < 18; iUnit++) {
    ...
    if (RotateRandom) {
      ...
      unit_rotation->RotateZ(drotz);
      unit_rotation->RotateY(droty);
      unit_rotation->RotateX(drotx);
    }

    TGeoCombiTrans* unit_placement =
      new TGeoCombiTrans(0., 0., posZ, unit_rotation);
    sts->AddNode(station, iUnit + 1, unit_placement);
