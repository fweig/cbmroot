...
void create_stsgeo_v16h(const char* geoTag = "v16h")
{
  ...
  */
  ...
  const Bool_t DisplaceRandom = false;  // true;  // add random displacement of modules for alignment study
  const Bool_t RotateRandom   = true;   // false;  // add random rotation of modules for alignment study
  ...
  Int_t allUnitTypes[16][16] = {{-1, -1, -1, -1, 10, 0, 9, 0, 1, 0, 9, 0, -1, -1, -1, -1},          // unit00D
  ...
  for (Int_t iUnit = 0; iUnit < 18; iUnit++) {
    ...
    unit_rotation = new TGeoRotation();  // need to renew rotation to start from 0 degree angle
    ...
    TGeoCombiTrans* unit_placement = new TGeoCombiTrans(0., 0., posZ, unit_rotation);
