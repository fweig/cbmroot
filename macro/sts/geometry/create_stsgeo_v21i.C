 ...
 **
 ** @file create_stsgeo_v21i.C
 ** @author Volker Friese <v.friese@gsi.de>
 ** @since 15 June 2012
 ** @date 09.05.2014
 ** @author Tomas Balog <T.Balog@gsi.de>
 ** 
 ** v21i: Similar to the geometry version v21g (but, without beampipe flange on the back-side of the STS wall). 
...
void create_stsgeo_v21i(const char* geoTag = "v21i")
{
  ...
  infoFile << "STS geometry created with create_stsgeo_v21i.C" << endl;
...
TGeoVolume* ConstructModule(const char* name, TGeoVolume* sector, Double_t cableLength)
{
  ...
  if (gkConstructCables && cableLength > 0.0001) {
    ...
    TGeoMedium* cableMedium = gGeoMan->GetMedium("STS_Cable_21");
    if (!cableMedium) Fatal("CreateModule", "Medium STS_Cable_21 not found!");
