...
void create_stsgeo_v19s(const char* geoTag = "v19s") {
  ...
  */
  ...
  for (Int_t iSensor = 1; iSensor <= nSensors; iSensor++) {
    ...
    if (iSensor == 1) sensor2->SetLineColor(kBlue);

    if (iSensor == 2) sensor->SetLineColor(kRed);
    if (iSensor == 2) sensor2->SetLineColor(kBlue);

    if (iSensor == 3) sensor->SetLineColor(kRed);
    if (iSensor == 3) sensor2->SetLineColor(kBlue);

    if (iSensor == 4) sensor->SetLineColor(kRed);
    if (iSensor == 4) sensor2->SetLineColor(kBlue);
    /*
    // add color to sensors
    if (iSensor == 1)
