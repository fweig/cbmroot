 ...
 **
 ** v16z: hybrid of v16x and v16c: yellow mini sensors in the first 4 layers and reduced gap size in y
 ** v16y: comparison of v16a, v16x, v16y and v16b - no yellow mini sensors in stations 3 to 6
 ** v16x: comparison of v16a, v16x, v16y and v16b - no yellow mini sensors in stations 5 to 6
 ** v16c: like v16b, but senors of ladders beampipe next to beampipe
 **       shifted closer to the pipe, like in the CAD model
 ** v16b: like v16a, but yellow mini sensors removed in all stations
...
void create_stsgeo_v16z(const char* geoTag = "v16z") {
  ...
  infoFile << "STS geometry created with create_stsgeo_v16z.C" << endl << endl;
  ...
  */
  ...
  Int_t statPos[8] = {
    30, 40, 50, 60, 70, 80, 90, 100};  // z positions of stations

  Double_t rHole[8] = {
    2.0, 2.0, 2.0, 2.9, 3.7, 3.7, 4.2, 4.2};  // size of cutouts in stations
  ...
  for (Int_t iUnit = 0; iUnit < 16; iUnit++)
    for (Int_t iLadder = 0; iLadder < 16; iLadder++) {
      ...
      if ((iUnit % 2 == 0)
          && (allLadderTypes[iUnit / 2][iLadder]
              < 100))  // if carbon structure is oriented upstream
        allUnitTypes[iUnit][iLadder] = allLadderTypes[iUnit / 2][iLadder];
      if ((iUnit % 2 == 1)
          && (allLadderTypes[iUnit / 2][iLadder]
              >= 100))  // if carbon structure is oriented downstream
        allUnitTypes[iUnit][iLadder] = allLadderTypes[iUnit / 2][iLadder];
    }
  ...
  for (Int_t iStation = 0; iStation < 8; iStation++) {
    ...
    for (Int_t i = 0; i < 16; i++)
      if (allLadderTypes[iStation][i] != 0) {
        ...
        cout << "DE ladderTypes[" << nLadders
             << "] = " << allLadderTypes[iStation][i] << ";" << endl;
        nLadders++;
      }
    mystation[iStation] =
      ConstructStation(iStation, nLadders, ladderTypes, rHole[iStation]);
    ...
    if (gkConstructCones) {
      ...
      TGeoCombiTrans* conePosRot11 = new TGeoCombiTrans(
        "conePosRot2",
        0.,
        0.,
        -coneDz - cone_offset[cone_size[iStation]] - gkLadderGapZ / 2.,
        coneRot11);
  ...
  TGeoTranslation* trans1 =
    new TGeoTranslation("trans1", 0., 0., -(z3 - z1) / 2. + (z2 - z1) / 2.);
  trans1->RegisterYourself();
  new TGeoTrd2("stsCone2",
               r2,
               r3,
               r2,
               r3,
  ...
  TGeoTranslation* trans2 =
    new TGeoTranslation("trans2", 0., 0., +(z3 - z1) / 2. - (z3 - z2) / 2.);
  ...
  TGeoShape* stsShape = new TGeoCompositeShape(
    "stsShape", "stsBox-stsCone1:trans1-stsCone2:trans2");
...
Int_t CreateSensors() {
  ...
  //   1x 0.75 cm + 0.2 cm = 0.95 cm
  //   2x 0.75 cm + 0.2 cm = 1.70 cm
  //   3x 0.75 cm + 0.2 cm = 2.45 cm
  //   4x 0.75 cm + 0.2 cm = 3.20 cm
  //
  //   5x 0.75 cm + 0.2 cm = 3.95 cm
  //   6x 0.75 cm + 0.2 cm = 4.70 cm
  //   7x 0.75 cm + 0.2 cm = 5.45 cm
  //   8x 0.75 cm + 0.2 cm = 6.20 cm

  // --- Sensor Type 05: 5/8 width sensor for stations 1 and 2 - (4 cm x 2.5 cm)
  //  xSize = 4.0;
  //  ySize = 2.5;
  xSize = 4.0;
  ySize = 2.2;
  ...
  // ---  Sensor type 06: 4/8 width sensor for stations 3 and 4 - (3.1 cm x 4.2 cm)
  //  xSize = 3.1;
  //  ySize = 4.2;
  xSize = 3.2;
  ySize = 4.2;
...
Int_t CreateLadders() {
  ...
  Int_t allSectorTypes[23][6] = {
    {5,
     ...
     3,
     3,
     -1},  // ladder 01 - last column defines alignment of small sensors
    {5,
     1,
     2,
     3,
     3,
     0},  // ladder 02 - last column defines alignment of small sensors
    {6,
     2,
     2,
     3,
     4,
     -1},  // ladder 03 - last column defines alignment of small sensors
    {6,
     2,
     2,
     3,
     4,
     0},  // ladder 04 - last column defines alignment of small sensors
    {2,
     3,
     4,
     4,
  ...
  // from v16c  Double_t gapXYZ[23][3]      = { { 0.,              4.13, 0. },     // ladder 01
  // from v16c                                  { 0.,              4.13, 0. },     // ladder 02
  // from v16c                                  { 0.,              4.20, 0. },     // ladder 03
  // from v16c                                  { 0.,              4.86, 0. },     // ladder 04
  // from v16c                                  { 0.,              6.08, 0. },     // ladder 05
  // from v16c                                  { 0.,              6.70, 0. },     // ladder 06
  // from v16c                                  { 0.,              7.92, 0. },     // ladder 07
  // from v16c                                  { 0.,              8.80, 0. },     // ladder 08

  // for comparison:
  // gkSectorOverlapY  = 0.27

  // 01 - statt -0.27 - sensor too short in y
  // 02 - statt -0.27 - sensor too short in y
  // 03 - statt -0.27 - sensor too long  in y
  // 04 - statt -0.27 - sensor too long  in y
  // 05 - statt  6.08 <-> 7.59 = 2* 4.2 - 3* gkSectorOverlapY
  // 06 - statt  6.70 <-> 7.59 = 2* 4.2 - 3* gkSectorOverlapY


  Double_t gapXYZ[23][3] = {
    {0., -gkSectorOverlapY * 2. / 3., 0.},  // ladder 01
    {0., -gkSectorOverlapY * 2. / 3., 0.},  // ladder 02
    {0., -gkSectorOverlapY - 1.8, 0.},      // ladder 03
    {0., -gkSectorOverlapY - 1.8, 0.},      // ladder 04
    ...
    // from v16x                  = { { 0., -gkSectorOverlapY, 0. },     // ladder 01
    // from v16x                      { 0., -gkSectorOverlapY, 0. },     // ladder 02
    // from v16x                      { 0., -gkSectorOverlapY, 0. },     // ladder 03
    // from v16x                      { 0., -gkSectorOverlapY, 0. },     // ladder 04

    // from v16x			  { 0., 2* 4.2 - 3* gkSectorOverlapY, 0. },     // ladder 05
    // from v16x			  { 0., 2* 4.2 - 3* gkSectorOverlapY, 0. },     // ladder 06
    // from v16x			  { 0.,                         8.80, 0. },     // ladder 07 - gapY = 2 * 4.40
    // from v16x			  { 0.,                         9.14, 0. },     // ladder 08 - gapY = 2 * 4.57
...
void AddCarbonLadder(Int_t LadderIndex,
  ...
  for (i = 1; i <= inum; i++) {
    ...
    ladder->AddNode(
      fullFrameBoxVol,
      i,
      new TGeoCombiTrans(name + "_FullFrameBox_posrot",
                         0.,
                         j * gkFrameStep,
                         -ladderZ / 2.
                           - (xu / 2. + sqrt(2.) * gkFrameThickness / 2.) / 2.,
...
TGeoVolume* ConstructStation(Int_t iStation,
  ...
  for (Int_t iLadder = 0; iLadder < nLadders;
       iLadder++) {  // find maximum dz in this station
    Int_t ladderType = ladderTypes[iLadder] % 100;
    ladderName       = Form("Ladder%02d", ladderType);
    ladder           = gGeoManager->GetVolume(ladderName);
    TGeoBBox* shape  = (TGeoBBox*) ladder->GetShape();
  ...
  for (Int_t iLadder = 0; iLadder < nLadders; iLadder++) {
    ...
    zPos =
      0.5 * gkLadderGapZ
      + (2 * maxdz - shape->GetDZ() - subtractedVal / 2.);  // z-aligned ladders

    cout << "DE ladder" << ladderTypes[iLadder] % 100
         << "  dx: " << shape->GetDX() << "  dy: " << shape->GetDY()
