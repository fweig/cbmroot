...
void create_stsgeo_v16d(const char* geoTag = "v16d") {
  ...
  infoFile << "STS geometry created with create_stsgeo_v16d.C" << endl << endl;
  ...
  //  TGeoNode* magnet_node = geometryFromGDML->FindNode("plate_1");
  //  TGeoVolume* magnet_volume = magnet_node->GetVolume();


  //DE  // --------------   Create geometry and top volume  -------------------------
  //DE  gGeoMan = (TGeoManager*)gROOT->FindObject("FAIRGeom");
  //DE  gGeoMan->SetName("STSgeom");
  //DE  TGeoVolume* top = new TGeoVolumeAssembly("TOP");
  //DE  gGeoMan->SetTopVolume(top);
  //DE  // --------------------------------------------------------------------------
  ...
  */
  ...
  Double_t rHole[8] = {
    ...
    3.35,
    3.96,
    4.2};  // size of cutouts in stations, derived from gapXYZ[x][1]/2
  ...
  for (Int_t iStation = 1; iStation <= 8; iStation++) {
    ...
    geometryFromGDML->AddNode(station, iStation, trans);
    geometryFromGDML->GetShape()->ComputeBBox();
  }
  cout << endl;
  //  CheckVolume(sts);
  CheckVolume(geometryFromGDML);
  // --------------------------------------------------------------------------
  ...
  // ---------------   Finish   -----------------------------------------------
  //  magnet_volume->AddNode(sts, 1, stsTrans);
