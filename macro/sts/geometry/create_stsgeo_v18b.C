...
void create_stsgeo_v18b(const char* geoTag = "v18b")
{
  ...
  */
  ...
  static std::vector<std::vector<Double_t>> plateSizes = {
    {177.5 / 2., 100.0 / 2., 1.5 / 2.}, {177.5 / 2., 100.0 / 2., 1.5 / 2.}, {177.5 / 2., 100.0 / 2., 1.5 / 2.},
    {177.5 / 2., 111.5 / 2., 1.5 / 2.}, {177.5 / 2., 111.5 / 2., 1.5 / 2.}, {177.5 / 2., 111.5 / 2., 1.5 / 2.},
    {177.5 / 2., 130.0 / 2., 1.5 / 2.}, {177.5 / 2., 130.0 / 2., 1.5 / 2.}, {177.5 / 2., 130.0 / 2., 1.5 / 2.},
  };

  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    Int_t iUnit = iPlate * 2;
    TGeoBBox* outerPlate =
      new TGeoBBox(Form("outerPlate%02d", iPlate), plateSizes[iPlate][0], plateSizes[iPlate][1], plateSizes[iPlate][2]);
    ...
    TGeoShape* innerPlate =
      new TGeoBBox(Form("innerPlate%02d", iPlate), maxDx * 2. + 6.2 + 1e-7, maxDy, plateSizes[iPlate][2] + 1e-7);

    TGeoShape* plateShape =
      new TGeoCompositeShape(Form("PlateShape%02d", iPlate), Form("outerPlate%02d-innerPlate%02d", iPlate, iPlate));
  ...
  Double_t boxThicknessZ = 4.;  // about the mean value between front (6.) and back (2.4)
  ...
  infoFile << endl << "Adding cooling/insulation box. Material: carbon_foam" << endl;
  ...
  infoFile << Form("Front hole: %.1f x %.1f cm", boxHoleFrontX, boxHoleFrontY) << endl;
  infoFile << Form("Back hole: %.1f x %.1f cm", boxHoleBackX, boxHoleBackY) << endl;
  infoFile << endl;
  ...
  new TGeoBBox("StsBoxHoleFront", boxHoleFrontX / 2., boxHoleFrontY / 2., boxThicknessZ / 2. + 0.001);
  new TGeoBBox("StsBoxHoleBack", boxHoleBackX / 2., boxHoleBackY / 2., boxThicknessZ / 2. + 0.001);


  (new TGeoTranslation("trStsBoxYZL", -1 * stsX / 2., 0., 0.))->RegisterYourself();
  (new TGeoTranslation("trStsBoxYZR", +1 * stsX / 2., 0., 0.))->RegisterYourself();
  (new TGeoTranslation("trStsBoxXZL", 0., -1 * stsY / 2., 0.))->RegisterYourself();
  (new TGeoTranslation("trStsBoxXZR", 0., +1 * stsY / 2., 0.))->RegisterYourself();
  (new TGeoTranslation("trStsBoxXYL", 0., 0., -1 * stsZ / 2.))->RegisterYourself();
  (new TGeoTranslation("trStsBoxXYR", 0., 0., +1 * stsZ / 2.))->RegisterYourself();
  ...
  TGeoVolume* box     = new TGeoVolume("box", boxShape, gGeoManager->GetMedium("carbon_foam"));
  ...
  infoFile << endl << "Adding cooling plates (material aluminium) with sizes:" << endl;
  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    Bool_t lastStation = iPlate == 8;

    Double_t posZ;
    if (lastStation) posZ = statPos[(iPlate - 1) * 2] - stsPosZ;
    else
    ...
    infoFile << Form("%.1f x %.1f x %.1f cm, Positioned %.1f cm downstream the target", 2. * plateSizes[iPlate][0],
                     2. * plateSizes[iPlate][1], 2. * plateSizes[iPlate][2], stsPosZ + plateZ)
             << endl;
  }
  ...
  infoFile << endl << endl << Form("Increasing thickness by %.3f cm for sensors:", gkAddedSensorThickness) << endl;
  TObjArray* sensorNodes = GetSensorNodes();
  for (Int_t i = 0; i < sensorNodes->GetEntriesFast(); i++) {
    TGeoPhysicalNode* physicalNode = static_cast<TGeoPhysicalNode*>(sensorNodes->At(i));
    TGeoHMatrix* globalMatrix      = physicalNode->GetMatrix();
    Double_t* coordinates          = globalMatrix->GetTranslation();
    Double_t projectedRadius       = coordinates[2] * TMath::Tan(gkSelectionConeAngle * TMath::Pi() / 180.);
    Double_t distanceToCenter      = TMath::Sqrt(coordinates[0] * coordinates[0] + coordinates[1] * coordinates[1]);

    TGeoBBox* shape              = static_cast<TGeoBBox*>(physicalNode->GetShape());
    Double_t sensorFittingRadius = TMath::Sqrt(shape->GetDX() * shape->GetDX() + shape->GetDY() * shape->GetDY());
    ...
    newMatrix->SetName(Form("%s-mat%d", physicalNode->GetOriginalMatrix()->GetName(), i));
    Double_t* translation = newMatrix->GetTranslation();
    translation[2] += (newMatrix->IsRotation() ? 1. : 1.) * gkAddedSensorThickness / 2.;
    ...
    TGeoBBox* newShape =
      new TGeoBBox(newName, oldShape->GetDX(), oldShape->GetDY(), oldShape->GetDZ() + gkAddedSensorThickness / 2.);
    TGeoVolume* vol = gGeoManager->MakeBox(newName, gGeoMan->GetMedium("silicon"), oldShape->GetDX(), oldShape->GetDY(),
...
void TraverseGeometryForSensors(TGeoNode* currentNode, TObjArray* result)
{
  ...
  for (Int_t i = 0; i < daughters->GetEntriesFast(); i++) {
    ...
    TString daughterPath = Form("%s/%s", gGeoMan->GetPath(), daughter->GetName());
...
void AddCarbonLadder(Int_t LadderIndex, TGeoVolume* ladder, Double_t xu, Double_t ladderY, Double_t ladderZ)
{
  ...
  // DEDE
  TGeoBBox* fullFrameShp = new TGeoBBox(name + "_CarbonElement_shp", xu / 2., gkFrameStep / 2.,
  ...
  for (i = 1; i <= inum; i++) {
    ...
    ladder->AddNode(fullFrameBoxVol, i,
                    new TGeoCombiTrans(name + "_CarbonElement_posrot", 0., j * gkFrameStep,
