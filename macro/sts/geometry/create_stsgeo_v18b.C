...
TGeoVolume*
ConstructFrameElement(const TString& name, TGeoVolume* frameBoxVol, Double_t x);
TGeoVolume* ConstructSmallCone(Double_t coneDz);
TGeoVolume* ConstructBigCone(Double_t coneDz);

TObjArray* GetSensorNodes();
void TraverseGeometryForSensors(TGeoNode* currentNode, TObjArray* result);
...
// ---> Additional thiskness for sensor falling under the selection criterion [cm]
const Double_t gkAddedSensorThickness = 0.01;  // v18b

// ---> Cone opening angle to select sensors [cm]
const Double_t gkSelectionConeAngle = 7.5;
...
const Bool_t gkConstructCones =
  kFALSE;  // kTRUE;   // switch this false by default for v15c and v16x
const Bool_t gkConstructFrames =
  kTRUE;  // kFALSE;  // switch this true  by default for v15c and v16x
...
TString unitName[32] =  // names of units for v16g - while merging D and U parts
  {"Unit00R", "Unit00L", "Unit01R", "Unit01L", "Unit01R", "Unit01L", "Unit02R",
   "Unit02L", "Unit02R", "Unit02L", "Unit03R", "Unit03L", "Unit03R", "Unit03L",
   "Unit04R", "Unit04L", "Unit04R", "Unit04L", "Unit05R", "Unit05L", "Unit05R",
   "Unit05L", "Unit06R", "Unit06L", "Unit06R", "Unit06L", "Unit07R", "Unit07L",
   "Unit07R", "Unit07L", "Unit08R", "Unit08L"};
...
void create_stsgeo_v18b(const char* geoTag = "v18b") {
  ...
  // ---> carbon_foam
  FairGeoMedium* mCarbon_foam = geoMedia->getMedium("carbon_foam");
  if (!mCarbon_foam) Fatal("Main", "FairMedium carbon_foam not found");
  geoBuild->createMedium(mCarbon_foam);
  TGeoMedium* carbon_foam = gGeoMan->GetMedium("carbon_foam");
  if (!carbon_foam) Fatal("Main", "Medium carbon_foam not found");
  ...
  */
  ...
  Int_t cone_size[8] = {
    0, 0, 0, 1, 1, 1, 1, 1};  // size of cones: 0 = small, 1 = large
  ...
  for (Int_t iUnit = 0; iUnit < 16; iUnit++) {
    ...
    for (Int_t iLadder = 0; iLadder < 16; iLadder++)
      if (allUnitTypes[iUnit][iLadder] >= 0) {
        ...
        cout << "DE ladderTypes[" << nLadders
             << "] = " << allUnitTypes[iUnit][iLadder] << ";" << endl;
        nLadders++;
      }
    ...
    myunit[iUnit * 2 + 0] =
      ConstructUnit(0, iUnit * 2 + 0, nLadders, ladderTypes, iUnit / 2 + 1);
    myunit[iUnit * 2 + 1] =
      ConstructUnit(1, iUnit * 2 + 1, nLadders, ladderTypes, iUnit / 2 + 1);
  ...
  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    ...
    TGeoBBox* unitShapeR =
      (TGeoBBox*) gGeoManager->GetVolume(unitName18[iUnit + 0])->GetShape();
    TGeoBBox* unitShapeL =
      (TGeoBBox*) gGeoManager->GetVolume(unitName18[iUnit + 1])->GetShape();

    Double_t maxDx = (unitShapeR->GetDX() + unitShapeL->GetDX()) / 2.;
    Double_t maxDy = TMath::Max(unitShapeR->GetDY(), unitShapeL->GetDY());
    cout << maxDy << endl;

    TGeoShape* innerPlate = new TGeoBBox(Form("innerPlate%02d", iPlate),
                                         maxDx * 2. + 6.2 + 1e-7,
                                         maxDy,
                                         plateSizes[iPlate][2] + 1e-7);

    TGeoShape* plateShape = new TGeoCompositeShape(
      Form("PlateShape%02d", iPlate),
      Form("outerPlate%02d-innerPlate%02d", iPlate, iPlate));
    TGeoVolume* plate = new TGeoVolume(Form("Plate%02d", iPlate),
                                       plateShape,
                                       gGeoManager->GetMedium("aluminium"));
    plate->SetLineColor(kWhite);
  ...
  // --- Create box  around the stations
  Double_t boxThicknessX = 2.;
  Double_t boxThicknessY = 4.;
  Double_t boxThicknessZ =
    4.;  // about the mean value between front (6.) and back (2.4)

  Double_t boxHoleFrontX = 40.;
  Double_t boxHoleFrontY = 40.;

  Double_t boxHoleBackX = 98.0;
  Double_t boxHoleBackY = 98.0;

  Double_t stsBorder = 2 * 5.;
  Double_t stsX      = 180. + boxThicknessX + stsBorder;
  Double_t stsY      = 140. + boxThicknessY + stsBorder;
  Double_t stsZ = statPos[15] - statPos[0] + boxThicknessZ + stsBorder + 1.5;

  infoFile << endl
           << "Adding cooling/insulation box. Material: carbon_foam" << endl;
  infoFile << Form("Thickness X: %1.f cm", boxThicknessX) << endl;
  infoFile << Form("Thickness Y: %1.f cm", boxThicknessY) << endl;
  infoFile << Form("Thickness Z: %1.f cm", boxThicknessZ) << endl;

  infoFile << Form("Size X: %1.f cm", stsX) << endl;
  infoFile << Form("Size Y: %1.f cm", stsY) << endl;
  infoFile << Form("Size Z: %1.f cm", stsZ) << endl;

  infoFile << Form("Front hole: %.1f x %.1f cm", boxHoleFrontX, boxHoleFrontY)
           << endl;
  infoFile << Form("Back hole: %.1f x %.1f cm", boxHoleBackX, boxHoleBackY)
           << endl;
  infoFile << endl;

  new TGeoBBox("StsBoxYZ", boxThicknessX / 2., stsY / 2., stsZ / 2.);
  new TGeoBBox("StsBoxXZ", stsX / 2., boxThicknessY / 2., stsZ / 2.);
  new TGeoBBox("StsBoxXY", stsX / 2., stsY / 2., boxThicknessZ / 2.);

  new TGeoBBox("StsBoxHoleFront",
               boxHoleFrontX / 2.,
               boxHoleFrontY / 2.,
               boxThicknessZ / 2. + 0.001);
  new TGeoBBox("StsBoxHoleBack",
               boxHoleBackX / 2.,
               boxHoleBackY / 2.,
               boxThicknessZ / 2. + 0.001);


  (new TGeoTranslation("trStsBoxYZL", -1 * stsX / 2., 0., 0.))
    ->RegisterYourself();
  (new TGeoTranslation("trStsBoxYZR", +1 * stsX / 2., 0., 0.))
    ->RegisterYourself();
  (new TGeoTranslation("trStsBoxXZL", 0., -1 * stsY / 2., 0.))
    ->RegisterYourself();
  (new TGeoTranslation("trStsBoxXZR", 0., +1 * stsY / 2., 0.))
    ->RegisterYourself();
  (new TGeoTranslation("trStsBoxXYL", 0., 0., -1 * stsZ / 2.))
    ->RegisterYourself();
  (new TGeoTranslation("trStsBoxXYR", 0., 0., +1 * stsZ / 2.))
    ->RegisterYourself();

  TString expression;
  expression += "StsBoxYZ:trStsBoxYZL+StsBoxYZ:trStsBoxYZR";
  expression += "+StsBoxXZ:trStsBoxXZL+StsBoxXZ:trStsBoxXZR";
  expression += "+StsBoxXY:trStsBoxXYL+StsBoxXY:trStsBoxXYR";

  expression += "-StsBoxHoleFront:trStsBoxXYL";
  expression += "-StsBoxHoleBack:trStsBoxXYR";

  TGeoShape* boxShape = new TGeoCompositeShape("boxShape", expression.Data());
  TGeoVolume* box =
    new TGeoVolume("box", boxShape, gGeoManager->GetMedium("carbon_foam"));
  box->SetTransparency(45);
  box->GetShape()->ComputeBBox();
  ...
  Double_t stsPosZ =
    0.5 * (statPos[15] + statPos[0]);  // todo units: update statPos[7]
  ...
  infoFile << endl
           << "Adding cooling plates (material aluminium) with sizes:" << endl;
  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    Bool_t lastStation = iPlate == 8;

    Double_t posZ;
    if (lastStation)
      posZ = statPos[(iPlate - 1) * 2] - stsPosZ;
    else
      posZ = statPos[iPlate * 2] - stsPosZ;

    Double_t plateZ = posZ + (lastStation ? +5. : -5.);

    TGeoVolume* plate;
    plate = gGeoManager->GetVolume(Form("Plate%02d", iPlate));
    sts->AddNode(plate, iPlate, new TGeoTranslation(0., 0., plateZ));
    sts->GetShape()->ComputeBBox();

    infoFile << Form(
      "%.1f x %.1f x %.1f cm, Positioned %.1f cm downstream the target",
      2. * plateSizes[iPlate][0],
      2. * plateSizes[iPlate][1],
      2. * plateSizes[iPlate][2],
      stsPosZ + plateZ)
             << endl;
  }
  ...
  // ---------------   Increase sensor thickness for selected sensors   -------

  // // Draw selection cone
  // TGeoVolume *cone = gGeoManager->MakeCone("SelectionCone",gGeoMan->GetMedium("silicon"), 50,0,0,0,100 * TMath::Tan(25 * TMath::Pi()/180.));
  // cone->SetLineWidth(2);
  // top->AddNode(cone,1, new TGeoTranslation(0,0,50));

  infoFile << endl
           << endl
           << Form("Increasing thickness by %.3f cm for sensors:",
                   gkAddedSensorThickness)
           << endl;
  TObjArray* sensorNodes = GetSensorNodes();
  for (Int_t i = 0; i < sensorNodes->GetEntriesFast(); i++) {
    TGeoPhysicalNode* physicalNode =
      static_cast<TGeoPhysicalNode*>(sensorNodes->At(i));
    TGeoHMatrix* globalMatrix = physicalNode->GetMatrix();
    Double_t* coordinates     = globalMatrix->GetTranslation();
    Double_t projectedRadius =
      coordinates[2] * TMath::Tan(gkSelectionConeAngle * TMath::Pi() / 180.);
    Double_t distanceToCenter = TMath::Sqrt(coordinates[0] * coordinates[0]
                                            + coordinates[1] * coordinates[1]);

    TGeoBBox* shape = static_cast<TGeoBBox*>(physicalNode->GetShape());
    Double_t sensorFittingRadius = TMath::Sqrt(
      shape->GetDX() * shape->GetDX() + shape->GetDY() * shape->GetDY());

    if (projectedRadius < (distanceToCenter - sensorFittingRadius)) continue;

    infoFile << physicalNode->GetName() << endl;

    TGeoHMatrix* newMatrix = new TGeoHMatrix();
    newMatrix->CopyFrom(physicalNode->GetOriginalMatrix());
    newMatrix->SetName(
      Form("%s-mat%d", physicalNode->GetOriginalMatrix()->GetName(), i));
    Double_t* translation = newMatrix->GetTranslation();
    translation[2] +=
      (newMatrix->IsRotation() ? 1. : 1.) * gkAddedSensorThickness / 2.;
    newMatrix->SetTranslation(translation);

    TGeoBBox* oldShape  = static_cast<TGeoBBox*>(physicalNode->GetShape());
    const char* newName = Form("%s-shape%d", oldShape->GetName(), i);
    TGeoBBox* newShape =
      new TGeoBBox(newName,
                   oldShape->GetDX(),
                   oldShape->GetDY(),
                   oldShape->GetDZ() + gkAddedSensorThickness / 2.);
    TGeoVolume* vol =
      gGeoManager->MakeBox(newName,
                           gGeoMan->GetMedium("silicon"),
                           oldShape->GetDX(),
                           oldShape->GetDY(),
                           oldShape->GetDZ() + gkAddedSensorThickness / 2.);
    physicalNode->Align(NULL, vol->GetShape(), kFALSE);
  }
  // --------------------------------------------------------------------------
...
TObjArray* GetSensorNodes() {
  TObjArray* sensorNodes = new TObjArray(1000);
  sensorNodes->SetOwner(kTRUE);

  gGeoMan->cd("");
  TraverseGeometryForSensors(gGeoMan->GetTopNode(), sensorNodes);
  return sensorNodes;
}
...
void TraverseGeometryForSensors(TGeoNode* currentNode, TObjArray* result) {
  ...
  if (!currentNode->GetNdaughters()) return;

  TString currentPath(gGeoMan->GetPath());

  TObjArray* daughters = currentNode->GetNodes();

  for (Int_t i = 0; i < daughters->GetEntriesFast(); i++) {
    TGeoNode* daughter = static_cast<TGeoNode*>(daughters->At(i));
    TString daughterPath =
      Form("%s/%s", gGeoMan->GetPath(), daughter->GetName());
    if (TString(daughter->GetName()).Contains("Sensor")) {
      // std::cout << "Adding sensor to the node list: " << daughterPath.Data() << std::endl;
      result->Add(new TGeoPhysicalNode(daughterPath.Data()));
    }

    gGeoMan->cd(daughterPath.Data());
    TraverseGeometryForSensors(daughter, result);
    gGeoMan->cd(currentPath.Data());
  }
}
...
Int_t CreateSensors() {
  ...
  xSize = gkSensorSizeX;
  ySize = 2.2;
  TGeoBBox* shape_sensor01 =
    new TGeoBBox("sensor01", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = gkSensorSizeX;
  ySize = 4.2;
  TGeoBBox* shape_sensor02 =
    new TGeoBBox("sensor02", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = gkSensorSizeX;
  ySize = 6.2;
  TGeoBBox* shape_sensor03 =
    new TGeoBBox("sensor03", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = gkSensorSizeX;
  ySize = 12.4;
  TGeoBBox* shape_sensor04 =
    new TGeoBBox("sensor04", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = 4.0;
  ySize = 2.5;
  TGeoBBox* shape_sensor05 =
    new TGeoBBox("sensor05", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = 3.1;
  ySize = 4.2;
  TGeoBBox* shape_sensor06 =
    new TGeoBBox("sensor06", xSize / 2., ySize / 2., zSize / 2.);
  ...
  xSize = 1.5;
  ySize = 4.2;
  TGeoBBox* shape_sensor07 =
    new TGeoBBox("sensor07", xSize / 2., ySize / 2., zSize / 2.);
...
Int_t CreateLadders() {
  ...
  // --- Ladders 01-23
  Int_t allSectorTypes[23][6] = {
    {1,
     2,
     3,
     3,
     0,
     -1},  // ladder 01 - 5 - last column defines alignment of small sensors
    {1,
     2,
     3,
     3,
     0,
     0},  // ladder 02 - 5 - last column defines alignment of small sensors
    {2,
     2,
     3,
     4,
     0,
     -1},  // ladder 03 - 6 - last column defines alignment of small sensors
    {2,
     2,
     3,
     4,
     0,
     0},  // ladder 04 - 6 - last column defines alignment of small sensors
    {2,
     3,
     4,
     4,
     0,
     -1},  // ladder 05 - 7 - last column defines alignment of small sensors
    {2,
     3,
     4,
     4,
     0,
     0},  // ladder 06 - 7 - last column defines alignment of small sensors
    {2,
     2,
     3,
     4,
     4,
     0},  // ladder 07 - last column defines alignment of small sensors
    {3,
     4,
     4,
     4,
     0,
     0},  // ladder 08 - last column defines alignment of small sensors
    {1,
     1,
     2,
     3,
     3,
     0},  // ladder 09 - last column defines alignment of small sensors
    {1,
     1,
     2,
     2,
     3,
     0},  // ladder 10 - last column defines alignment of small sensors
    {2,
     2,
     0,
     0,
     0,
     0},  // ladder 11 - last column defines alignment of small sensors
    {2,
     2,
     2,
     3,
     4,
     0},  // ladder 12 - last column defines alignment of small sensors
    {2,
     2,
     3,
     4,
     0,
     0},  // ladder 13 - last column defines alignment of small sensors
    {2,
     3,
     4,
     0,
     0,
     0},  // ladder 14 - last column defines alignment of small sensors
    {3,
     3,
     0,
     0,
     0,
     0},  // ladder 15 - last column defines alignment of small sensors
    {2,
     2,
     3,
     4,
     4,
     0},  // ladder 16 - last column defines alignment of small sensors
    {2,
     3,
     4,
     4,
     0,
     0},  // ladder 17 - last column defines alignment of small sensors
    {3,
     4,
     4,
     0,
     0,
     0},  // ladder 18 - last column defines alignment of small sensors
    {4,
     4,
     0,
     0,
     0,
     0},  // ladder 19 - last column defines alignment of small sensors
    {1,
     2,
     4,
     4,
     4,
     0},  // ladder 20 - last column defines alignment of small sensors
    {4,
     0,
     0,
     0,
     0,
     0},  // ladder 21 - last column defines alignment of small sensors
    {2,
     3,
     4,
     4,
     4,
     0},  // ladder 22 - last column defines alignment of small sensors
    {2,
     3,
     3,
     4,
     4,
     0}};  // ladder 23 - last column defines alignment of small sensors
  ...
  for (Int_t iLadder = 0; iLadder < 23; iLadder++) {
    if (iLadder + 1 <= 8)
      ...
      gapXYZ[iLadder][2] =
        2. * shape->GetDZ()
        + gkSectorGapZ;  // set displacement in z for overlapping half ladders
  }
  ...
  for (Int_t iLadder = 0; iLadder < 23; iLadder++) {
    ...
    for (Int_t i = 0; i < 5; i++)
      if (allSectorTypes[iLadder][i] != 0) {
        ...
        cout << "DE sectorTypes[" << nSectors
             << "] = " << allSectorTypes[iLadder][i] << ";" << endl;
        nSectors++;
      }
    ...
    halfLadderU =
      ConstructHalfLadder(hlname, nSectors, sectorTypes, align);  // mirrored
...
ConstructModule(const char* name, TGeoVolume* sector, Double_t cableLength) {
  ...
  Double_t moduleZ   = TMath::Max(sectorZ + gkAddedSensorThickness, cableZ);
...
TGeoVolume* ConstructHalfLadder(const TString& name,
  ...
  for (Int_t iSector = 0; iSector < nSectors; iSector++) {
    ...
    if (!sector)
      Fatal("ConstructHalfLadder",
            Form("Volume %s not found", sectorName.Data()));
    TGeoBBox* box = (TGeoBBox*) sector->GetShape();
  ...
  for (Int_t iSector = 0; iSector < nSectors; iSector++) {
    ...
    TGeoVolume* module =
      ConstructModule(moduleName.Data(), sector, cableLength);
    ...
    TGeoTranslation* trans =
      new TGeoTranslation("t", xPosMod, yPosMod, zPosMod);
...
void AddCarbonLadder(Int_t LadderIndex,
  ...
  Int_t carbon_elem[23] = {
    11, 11, 16, 16, 20, 20, 22, 24, 11, 10, 6, 16,
  ...
  // DEDE
  TGeoBBox* fullFrameShp =
    new TGeoBBox(name + "_CarbonElement_shp",
                 xu / 2.,
                 gkFrameStep / 2.,
  ...
  TGeoVolume* fullFrameBoxVol =
    new TGeoVolume(name + "_CarbonElement", fullFrameShp, gStsMedium);
  ...
  for (i = 1; i <= inum; i++) {
    ...
    if (LadderIndex <= 3)  // central ladders in stations 1 to 3
    {
      if (
        (j >= -1)
        && (j
            <= 1))  // keep the inner 2 (even) or 3 (odd) elements free for the cone
        continue;
    ...
    ladder->AddNode(
      fullFrameBoxVol,
      i,
      new TGeoCombiTrans(name + "_CarbonElement_posrot",
                         0.,
                         j * gkFrameStep,
                         -ladderZ / 2.
                           - (xu / 2. + sqrt(2.) * gkFrameThickness / 2.) / 2.,
...
TGeoVolume* ConstructUnit(Int_t iSide,
  ...
  for (Int_t iLadder = 0; iLadder < nLadders; iLadder++) {
    ...
    if (ladderType > 0) {
      ...
      if (!ladder)
        Fatal("ConstructUnit", Form("Volume %s not found", ladderName.Data()));
  ...
  Double_t maxdz = 0.;
  for (Int_t iLadder = 0; iLadder < nLadders;
       iLadder++) {  // find maximum dz in this unit
    Int_t ladderType = ladderTypes[iLadder] % 100;
    if (ladderType > 0) {
      ladderName  = Form("LadderType%02d", ladderType);
      ladder      = gGeoManager->GetVolume(ladderName);
  ...
  for (Int_t iLadder = 0; iLadder < nLadders; iLadder++) {
    ...
    if (ladderType > 0) {
      ...
      zPos = 0.5 * gkLadderGapZ
             + (2 * maxdz - ladderShape->GetDZ()
                - subtractedVal / 2.);  // z-aligned ladders

      cout << "DE ladder" << ladderTypes[iLadder] % 100
           << "  dx: " << ladderShape->GetDX()
           << "  dy: " << ladderShape->GetDY()
           << "  dz: " << ladderShape->GetDZ() << "  max dz: " << maxdz << endl;

      cout << "DE ladder" << ladderTypes[iLadder] % 100
           << "  fra: " << gkFrameThickness / 2. << "  sub: " << subtractedVal
           << "  zpo: " << zPos << endl
           << endl;

      if (ladderTypes[iLadder] / 100
          == 1)  // flip some of the ladders to reproduce the CAD layout
        rot->RotateY(180.);
      else
        zPos = -zPos;
      ...
      if (iSide == 0) {
        if (iLadder < nLadders / 2)  // right side - only half unit -x
        {
          ...
          cout << "DEFG** iLadder: " << iLadder << " iStation: " << iStation
               << endl;
        }
      } else {
...
/** ======================================================================= **/
TGeoVolume* ConstructFrameElement(const TString& name,
                                  TGeoVolume* frameBoxVol,
                                  Double_t x) {
  ...
  if (gkCylindricalFrames)
    ...
    frameVertPillarShp =
      new TGeoTube(name + "_vertpillar_shape",
                   gkCylinderDiaInner / 2.,
                   gkCylinderDiaOuter / 2.,
                   gkFrameStep / 2.);  // circle crossection, along z
  else
    frameVertPillarShp =
      new TGeoBBox(name + "_vertpillar_shape",
                   t,
                   t,
                   gkFrameStep / 2.);  // square crossection, along z
  TGeoVolume* frameVertPillarVol =
    new TGeoVolume(name + "_vertpillar", frameVertPillarShp, framesMaterial);
  frameVertPillarVol->SetLineColor(kGreen);

  TGeoRotation* xRot90 = new TGeoRotation;
  xRot90->RotateX(90.);
  frameBoxVol->AddNode(frameVertPillarVol,
                       1,
                       new TGeoCombiTrans(name + "_vertpillar_pos_1",
                                          x - t,
                                          0.,
                                          -(x + sqrt(2.) * t - 2. * t) / 2.,
                                          xRot90));
  frameBoxVol->AddNode(frameVertPillarVol,
                       2,
                       new TGeoCombiTrans(name + "_vertpillar_pos_2",
                                          -(x - t),
                                          0.,
                                          -(x + sqrt(2.) * t - 2. * t) / 2.,
                                          xRot90));
  ...
  frameBoxVol->AddNode(
    ...
    3,
    new TGeoCombiTrans(
      name + "_vertpillar_pos_3", 0., 0., (x - sqrt(2.) * t) / 2., vertRot));

  // --- Small horizontal pillar
  TGeoBBox* frameHorPillarShp = new TGeoBBox(name + "_horpillar_shape",
                                             x - 2. * t,
                                             gkThinFrameThickness / 2.,
                                             gkThinFrameThickness / 2.);
  TGeoVolume* frameHorPillarVol =
    new TGeoVolume(name + "_horpillar", frameHorPillarShp, framesMaterial);
  frameHorPillarVol->SetLineColor(kCyan);
  frameBoxVol->AddNode(
    frameHorPillarVol,
    1,
    new TGeoTranslation(name + "_horpillar_pos_1",
                        0.,
                        -gkFrameStep / 2. + gkThinFrameThickness / 2.,
                        -(x + sqrt(2.) * t - 2. * t) / 2.));
  ...
  if (gkConstructSmallFrames) {
    ...
    TGeoPara* frameSlopePillarShp =
      new TGeoPara(name + "_slopepillar_shape",
                   (x - 2. * t) / TMath::Cos(31.4 / 180. * TMath::Pi()),
                   gkThinFrameThickness / 2.,
                   gkThinFrameThickness / 2.,
                   31.4,
                   0.,
                   90.);
    TGeoVolume* frameSlopePillarVol = new TGeoVolume(
      name + "_slopepillar", frameSlopePillarShp, framesMaterial);
    frameSlopePillarVol->SetLineColor(kCyan);
    TGeoRotation* slopeRot =
      new TGeoRotation(name + "_slopepillar_rot_1", 0., 0., 31.4);
    TGeoCombiTrans* slopeTrRot =
      new TGeoCombiTrans(name + "_slopepillar_posrot_1",
                         0.,
                         0.,
                         -(x + sqrt(2.) * t - 2. * t) / 2.,
                         slopeRot);
    ...
    Double_t angl = 23.;
    // --- Small sub pillar
    TGeoPara* frameSubPillarShp = new TGeoPara(
      name + "_subpillar_shape",
      (sqrt(2) * (x / 2. - t) - t / 2.) / TMath::Cos(angl / 180. * TMath::Pi()),
      gkThinFrameThickness / 2.,
      gkThinFrameThickness / 2.,
      angl,
      0.,
      90.);
    TGeoVolume* frameSubPillarVol =
      new TGeoVolume(name + "_subpillar", frameSubPillarShp, framesMaterial);
    frameSubPillarVol->SetLineColor(kMagenta);

    Double_t posZ = t * (1. - 3. / (2. * sqrt(2.)));

    // one side of X direction
    TGeoRotation* subRot1 =
      new TGeoRotation(name + "_subpillar_rot_1", 90., 45., -90. + angl);
    TGeoCombiTrans* subTrRot1 =
      new TGeoCombiTrans(name + "_subpillar_posrot_1",
                         -(-x / 2. + t - t / (2. * sqrt(2.))),
                         1.,
                         posZ,
                         subRot1);

    TGeoRotation* subRot2 =
      new TGeoRotation(name + "_subpillar_rot_2", 90., -90. - 45., -90. + angl);
    TGeoCombiTrans* subTrRot2 =
      new TGeoCombiTrans(name + "_subpillar_posrot_2",
                         -(-x / 2. + t - t / (2. * sqrt(2.))),
                         -1.,
                         posZ,
                         subRot2);
    ...
    TGeoRotation* subRot3 =
      new TGeoRotation(name + "_subpillar_rot_3", 90., 90. + 45., -90. + angl);
    TGeoCombiTrans* subTrRot3 =
      new TGeoCombiTrans(name + "_subpillar_posrot_3",
                         -x / 2. + t - t / (2. * sqrt(2.)),
                         1.,
                         posZ,
                         subRot3);

    TGeoRotation* subRot4 =
      new TGeoRotation(name + "_subpillar_rot_4", 90., -45., -90. + angl);
    TGeoCombiTrans* subTrRot4 =
      new TGeoCombiTrans(name + "_subpillar_posrot_4",
                         -x / 2. + t - t / (2. * sqrt(2.)),
                         -1.,
                         posZ,
                         subRot4);
...
TGeoVolume* ConstructSmallCone(Double_t coneDz) {
  ...
  TGeoConeSeg* A = new TGeoConeSeg("A",
                                   coneDz,
                                   radius,
                                   radius + thickness,
                                   radius,
...
TGeoVolume* ConstructBigCone(Double_t coneDz) {
  ...
  TGeoVolume* coneBigVol =
    new TGeoVolume("ConeBig", coneBigShp, framesMaterial);
  ...
  Double_t thickness  = 0.02;
  Double_t thickness2 = 0.022;
  TGeoConeSeg* bA2    = new TGeoConeSeg("bA2",
                                     coneDz - thickness,
                                     6. + thickness,
                                     7.6 - thickness2,
                                     5.99 + thickness,
