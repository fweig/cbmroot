...
TGeoVolume* ConstructLadder(Int_t LadderIndex,
  ...
  Double_t ladderZ           = TMath::Max(
    zu,
    zd + shiftZ + offsetZ);  // there are 6 slots - 5 x 1.5 mm + 0.3 mm = 7.8 mm
  //  Double_t ladderZ = TMath::Max(zu, zd + shiftZ);

  cout << "DERR iladder " << LadderIndex << " nSec " << nSectors << " zu " << zu
       << " zd " << zd << " zd+shi " << zd + shiftZ << " ladderZ " << ladderZ
  ...
  if (LadderIndex >= 1000)
    zPosU =
      -0.5 * (ladderZ - zu) + offsetZ;  // back aligned with possible offset
  //zPosU = -zPosU;
