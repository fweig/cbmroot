 ...
 **
 ** @file create_stsgeo_v21d.C
 ** @author Volker Friese <v.friese@gsi.de>
 ** @since 15 June 2012
 ** @date 09.05.2014
 ** @author Tomas Balog <T.Balog@gsi.de>
 ** 
 ** v21d: Airex foam in the front wall (solid) and back side wall (Honeycomb structure).
 * 		  New beam pipe R = 2.0 cm. 
...
void create_stsgeo_v21d(const char* geoTag = "v21d")
{
  ...
  infoFile << "STS geometry created with create_stsgeo_v21e.C" << endl;
  ...
  // ---> STS_foam
  FairGeoMedium* mAirexFoam = geoMedia->getMedium("STS_Airex_Foam");
  if (!mAirexFoam) Fatal("Main", "FairMedium STS_Airex_Foam not found");
  geoBuild->createMedium(mAirexFoam);
  TGeoMedium* Airex_Foam = gGeoMan->GetMedium("STS_Airex_Foam");
  if (!Airex_Foam) Fatal("Main", "Medium STS_Airex_Foam not found");
  ...
  */
  ...
  ///////////////	STS wall ///
  /* 	
  double carbon_frame_thickness    = 0.2;
  double Back_foamthickness        = 2;
  double back_side_wall_z_position = 54.450;
  double frontwall_foam            = 5.6;

  double front_side_wall_z_position = -49.120;

  // double cone_position = back_side_wall_z_position + carbon_frame_thickness + Back_foamthickness ;


  /////// Back side and front side carbon frame and foam/////////
  TGeoVolume* Back_CF1 = gGeoMan->MakeBox("carbon_frame_1", STSBoxCarbonFibre, 141, 71.25, carbon_frame_thickness / 2);

  TGeoVolume* gap = gGeoMan->MakeBox("gap", Airex_Foam, 141, 71.25, Back_foamthickness / 2);

  TGeoVolume* Frontgap = gGeoMan->MakeBox("Frontgap", Airex_Foam, 141, 71.25, frontwall_foam / 2);

  //TGeoVolume *tubs = geom->MakeTubs("tub", Carbon, 0, 9, 6, 0, 360);

  /////////// Volumes to cut from main volume ////////////////////////////
  TGeoVolume* tube1 = gGeoMan->MakeTube("tube1", Vacuum, 0, 5.20, carbon_frame_thickness / 2 + 0.000002);
  TGeoVolume* tube2 = gGeoMan->MakeTube("tube2", Vacuum, 0, 5.20, Back_foamthickness / 2 + 0.000002);

  TGeoVolume* box1 = gGeoMan->MakeBox("box1", STSBoxCarbonFibre, 40, 40, carbon_frame_thickness / 2 + 0.000001);
  TGeoVolume* box2 = gGeoMan->MakeBox("box2", STSBoxCarbonFibre, 40, 40, frontwall_foam / 2 + 0.000001);


  ////// Backside wall to cut tube /////
  TGeoCompositeShape* CFT = new TGeoCompositeShape("CFT", "(carbon_frame_1) - tube1");
  TGeoVolume* cfbt        = new TGeoVolume("cfbt", CFT, STSBoxCarbonFibre);

  TGeoCompositeShape* CGT = new TGeoCompositeShape("CGT", "(gap) - tube2");
  TGeoVolume* cgbt        = new TGeoVolume("cgbt", CGT, Airex_Foam);


  ///////////////////// Front side Wall to cut box  ///////
  TGeoCompositeShape* CFfront = new TGeoCompositeShape("CFfront", "(carbon_frame_1) - box1");
  TGeoVolume* cffront         = new TGeoVolume("cffront", CFfront, STSBoxCarbonFibre);

  TGeoCompositeShape* CGfront = new TGeoCompositeShape("CGfront", "(Frontgap) - box2");
  TGeoVolume* cgfront         = new TGeoVolume("cgfront", CGfront, Airex_Foam);

  ////// Back side wall loop///////
  for (int i1 = 0; i1 < 2; i1++) {
    TGeoTranslation* CF1 =
      new TGeoTranslation(0., 0., (i1 * carbon_frame_thickness) + i1 * Back_foamthickness + back_side_wall_z_position);

    // TGeoTranslation *tr_gap = new TGeoTranslation(0., 0,  back_side_wall_z_position + carbon_frame_thickness/2 + Back_foamthickness/2);

    sts->AddNode(cfbt, i1 + 1, CF1);  // Carbon Frame position
    //sts->AddNode(cgbt,1, tr_gap);    // Solid Foam Airex T90
  } 
  */

  /*	
	//////Front side wall loop ////////
  for (int i2=0; i2<2; i2++){
	  TGeoTranslation *CFFrontside = new TGeoTranslation(0., 0., (i2 * carbon_frame_thickness) + i2*frontwall_foam + front_side_wall_z_position  );
	
	  TGeoTranslation *CGFrontside = new TGeoTranslation(0., 0., front_side_wall_z_position + carbon_frame_thickness/2 + frontwall_foam/2 );
	
	  sts->AddNode(cffront, i2+1, CFFrontside);	// / Carbon Frame position
	  sts->AddNode(cgfront, 1, CGFrontside);	// Solid Foam Airex T90
		}	*/

  double Distance_Z = 54.550;
  double distance   = 0.45;
  double dshift     = 0.75;

  TGeoVolume* PGON = gGeoMan->MakePgon("PGON", Airex_Foam, 0, 360, 6, 2);
  TGeoPgon* pgon   = (TGeoPgon*) (PGON->GetShape());
  pgon->DefineSection(0, 0, 0.6, 1.0);
  pgon->DefineSection(1, 2, 0.6, 1.0);
  //pgon->DefineSection(2,-1, 10, 15);

  PGON->SetVisibility(kTRUE);
  PGON->SetLineColor(kYellow + 3);
  ...
  for (int i = 0; i < ncells; i++) {
    Double_t ycell = (1.9 * i + 1.2) * (dshift + 0.85) - 0.128;
    // Double_t xcell = (2*i + 1)*(dshift + 0.2);

    for (int i1 = 0; i1 < 39; i1++) {

      sts->AddNode(PGON, (ncells + i + 1), new TGeoCombiTrans(x + (i1 * 3.40) + 0.5, ycell + 1.0, Distance_Z, rot));
      sts->AddNode(PGON, (ncells - i), new TGeoCombiTrans(x + (i1 * 3.40) + 0.5, -ycell + 1.0, Distance_Z, rot));

      sts->AddNode(PGON, (ncells + i + 1), new TGeoCombiTrans(-x - (i1 * 3.40) - 0.5, ycell + 1.0, Distance_Z, rot));
      sts->AddNode(PGON, (ncells - i), new TGeoCombiTrans(-x - (i1 * 3.40) - 0.5, -ycell + 1.0, Distance_Z, rot));

      sts->AddNode(PGON, (ncells + i + 1), new TGeoCombiTrans(x + (i1 * 3.40) + 2.2, ycell - 0.5, Distance_Z, rot));
      sts->AddNode(PGON, (ncells - i), new TGeoCombiTrans(x + (i1 * 3.40) + 2.2, -ycell - 0.5, Distance_Z, rot));
    ...
    //	  sts->AddNode(PGON, (ncells+i+1), new TGeoTranslation((i2*2.65), ycell+5.6, Distance_Z));
    //	  sts->AddNode(PGON, (ncells-i),new TGeoTranslation(-(i2*2.65), ycell+5.6, Distance_Z));

    //sts->AddNode(PGON, (ncells+i+1), new TGeoTranslation((i2*2.65), -ycell-5.8, Distance_Z));
    //sts->AddNode(PGON, (ncells-i),new TGeoTranslation(-(i2*2.65), -ycell-5.8 , Distance_Z));
    //}
  }
  //////////// for central part of the wall due to beam pipe cut-outs //////
  // /*

  for (int i = 0; i < 21; i++) {
    Double_t ycell = (1.9 * i + 1.2) * (dshift + 0.87);


    for (int i2 = 0; i2 < 3; i2++) {

      sts->AddNode(PGON, (ncells + i + 1), new TGeoCombiTrans(-4.0 + (i2 * 3.0) + 0.5, ycell + 4.9, Distance_Z, rot));
      sts->AddNode(PGON, (ncells - i), new TGeoCombiTrans(-4.0 + (i2 * 3.0) + 0.5, -ycell - 4.9, Distance_Z, rot));


      //		  sts->AddNode(PGON, (ncells+i+1), new TGeoCombiTrans(-1.0-(i2*2.75), ycell+4.1, Distance_Z, rot));
      //		  sts->AddNode(PGON, (ncells-i),new TGeoCombiTrans(-1.2-(i2*2.65), -ycell-6.0, Distance_Z, rot));

      //  sts->AddNode(PGON, (ncells-i),new TGeoCombiTrans((i2*2.65)+1.30, -ycell-7.95, Distance_Z, rot));


      //   for (int i2=0; i2<2; i2++){
      sts->AddNode(PGON, (ncells + i + 1), new TGeoCombiTrans(-4.0 + (i2 * 3.0) + 2.0, ycell + 6.45, Distance_Z, rot));
      sts->AddNode(PGON, (ncells - i), new TGeoCombiTrans(-4.0 + (i2 * 3.0) + 2.0, -ycell - 6.45, Distance_Z, rot));

      //		  sts->AddNode(PGON, (ncells+i+1), new TGeoCombiTrans(-(i2*2.65)+0.05, ycell+7.95, Distance_Z, rot));
      //		  sts->AddNode(PGON, (ncells-i),new TGeoCombiTrans(1.2+(i2*2.65)+1.30, -ycell-7.95, Distance_Z, rot));
      //		 }
    }
  ...
  //// back side visibility
  //cfbt->SetVisibility(kTRUE);  //// Carbon-frame of back-side
  //cfbt->SetLineColor(kRed + 3);

  //cgbt->SetVisibility(kTRUE);               ////Airex foam gap back-side
  //cgbt->SetLineColor(kYellow + 3);

  ///// Front wall visibility
  // cffront->SetVisibility(kTRUE);            /// Front- side carbon frame
  // cffront->SetLineColor(kRed +3);

  // cgfront->SetVisibility(kTRUE);             //Airex foam front side
  // cgfront->SetLineColor(kYellow +3);
