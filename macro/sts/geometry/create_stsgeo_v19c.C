 ...
 **
 ** v19c: drop station 8 and increase spacing between remaining 7 stations from 10 cm to 12 cm
...
void create_stsgeo_v19c(const char* geoTag = "v19c") {
  ...
  infoFile << "STS geometry created with create_stsgeo_v19c.C" << endl;
  ...
  */
  ...
  // v19b
  //v19b  Int_t statPos[16]   = { 30, 30, 40, 40, 50, 50,  60,  60,
  //v19b                          70, 70, 80, 80, 90, 90, 100, 100 };  // z positions of units
  //v19b
  //v19b  Int_t statPos18[18] = { 30, 30,                              // expanded for placement of Unit00
  //v19b                          30, 30, 40, 40, 50, 50,  60,  60,
  //v19b                          70, 70, 80, 80, 90, 90, 100, 100 };  // z positions of units
  ...
  // --- Units 01 - 16
  //  for (Int_t iUnit = 0; iUnit < 16; iUnit++)
  for (Int_t iUnit = 0; iUnit < 14; iUnit++) {
    cout << endl;
  ...
  //  Double_t stsPosZ = 0.5 * ( statPos[13] + statPos[0] );  // todo units: update statPos[7]
  ...
  for (Int_t iUnit = 0; iUnit < 16; iUnit++) {
    ...
    cout << "DEzpos " << statPos18[iUnit] << " iUnit " << iUnit << endl;
    //    if (statPos18[iUnit] <= 100)
    //    {
...
void AddCarbonLadder(Int_t LadderIndex,
  ...
  Int_t YnumOfFrameBoxes =
    round(ladderY / gkFrameStep);  //carbon_elem[LadderIndex-1];
...
TGeoVolume* ConstructUnit(Int_t iSide,
  ...
  for (Int_t iLadder = 0; iLadder < nLadders; iLadder++) {
    ...
    if (ladderType > 0) {
      ...
      cout << "DE ladder" << ladderTypes[iLadder] % 1000
      ...
      if (!isFirstPartOfHalfUnit) zPos += 12;
      //        zPos += 10;
