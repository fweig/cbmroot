...
void create_stsgeo_v19c(const char* geoTag = "v19c")
{
  ...
  */
  ...
  Int_t statPos[16] = {30, 30, 42, 42, 54, 54, 66, 66, 78, 78, 90, 90, 102, 102, 150, 150};  // z positions of units

  Int_t statPos18[18] = {30,
                         30,  // expanded for placement of Unit00
                         30, 30, 42, 42, 54, 54, 66, 66, 78, 78, 90, 90, 102, 102, 150, 150};  // z positions of units
...
TGeoVolume* ConstructUnit(Int_t iSide, Int_t iUnit, Int_t nLadders, Int_t* ladderTypes, Int_t iStation)
{
  ...
  for (Int_t iLadder = 0; iLadder < nLadders; iLadder++) {
    ...
    if (ladderType > 0) {
      ...
      cout << "DE ladder" << ladderTypes[iLadder] % 1000 << "  dx: " << ladderShape->GetDX()
           << "  dy: " << ladderShape->GetDY() << "  dz: " << ladderShape->GetDZ() << "  max dz: " << maxdz << endl;

      // v19a      cout << "DE ladder" << ladderTypes[iLadder]%100
      cout << "DE ladder" << ladderTypes[iLadder] % 1000 << "  fra: " << gkFrameThickness / 2.
