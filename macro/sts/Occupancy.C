...
void MyMainFrame::ChangeView(Int_t itemp)
{
  ...
  if (fGeoRead && (qaRead || qasimRead)) DoDraw();
}
//-----------------------------------------------------------------------------
...
void MyMainFrame::DoDraw()
{
  ...
  if (fSelView == 5 && qaRead) {
    ...
    if (qaRecoToPlot == 4) {
      ...
      legend1->AddEntry(projX, Form("X,#sigma=%3.2f#mum", resolution[0]), "l");
      legend1->AddEntry(projY, Form("Y,#sigma=%3.2f#mum", resolution[1]), "l");
      legend1->Draw();
    }
  ...
  if (fSelView >= 5 && qasimRead) {
    ...
    if (fSelView == 5) beingDrawn = "eLoss";
    if (fSelView == 6) beingDrawn = "mips";
    if (fSelView == 7) beingDrawn = "mrads";
    ...
    for (Int_t iwaf = 0; iwaf < nofSensors[fSelStation]; iwaf++) {
      ...
      up->Draw();
      TLine* bo = new TLine(x1, y2, x2, y2);
      bo->Draw();
      TLine* ri = new TLine(x1, y1, x1, y2);
      ri->Draw();
      TLine* le = new TLine(x2, y1, x2, y2);
      le->Draw();
    }

    TLegend* unitL;
    if (fSelView == 5) unitL = new TLegend(.8, .9, 1., 1., "GeV/cm^{3}");
    if (fSelView == 6) unitL = new TLegend(.8, .9, 1., 1., "#MIPs/6yr");
    if (fSelView == 7) unitL = new TLegend(.8, .9, 1., 1., "Mrad/6yr");
  ...
  line = new TLine(0, -ymax, 0, ymax);
  line->Draw();
  line = new TLine(-xmax, 0, xmax, 0);
  line->Draw();
  // draw axis
  Double_t maxx = (int) xmax / 10 * 10;
  Double_t maxy = (int) ymax / 10 * 10;
  TGaxis* axis1 = new TGaxis(-maxx, 0, maxx, 0, -maxx, maxx, 412, "-+");
  axis1->SetName("axis1");
  axis1->SetLabelSize(0.03);
  axis1->Draw();
  TGaxis* axis2 = new TGaxis(0, -maxy, 0, maxy, -maxy, maxy, 412, "-+");
  axis2->SetName("axis2");
  axis2->SetLabelSize(0.03);
  axis2->Draw();
  // draw axis text
  TLatex* tx = new TLatex(maxx - maxx / 12, 4, "X/cm");
  tx->SetTextSize(0.03);
  tx->Draw();
  TLatex* ty = new TLatex(4.2, maxy - 1.3, "Y/cm");
  ty->SetTextSize(0.03);
  ty->Draw();
  // draw Station text
  ...
  if (fGeoRead) {
	...
	sensorPave->AddText(0.,0.,Form("%2.1f",x1));*/
        ...
        if (isens == 0) {
          TLine* bo = new TLine(x1, y1, x2, y1);
          bo->SetLineWidth(2);
          bo->Draw();
        }
        if (isens == sector->GetNSensors() - 1) {
          TLine* up = new TLine(x1, y2, x2, y2);
          up->SetLineWidth(2);
          up->Draw();
        }
      }
...
void MyMainFrame::ReadGeometry()
{
  ...
  if (!fin) {
    cout << "sorry, no file \"" << fGeoFileName.Data() << "\"" << endl;
    return;
  }
