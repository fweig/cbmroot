...
void test_address(Int_t addressVersion = -1)
{
  ...
  if (addressVersion < 0 || addressVersion >= CbmStsAddress::kCurrentVersion) version = CbmStsAddress::kCurrentVersion;
          ...
          for (UInt_t sens = 0; sens < nSensors; sens++) {
            for (UInt_t side = 0; side < nSides; side++) {
              ...
              if (CbmStsAddress::GetSystemId(address) != kSts) {
                std::cout << "GetSystemId " << CbmStsAddress::GetSystemId(address) << " does not return STS"
                          << std::endl;
                success = kFALSE;
              }
              if (rUnit != unit) {
              ...
              if (!success) {
                std::cout << "Test " << nTests << " " << CbmStsAddress::ToString(address) << std::endl;
  ...
  std::cout << "Consumption: CPU " << timer.CpuTime() << " s, Real " << timer.RealTime() << std::endl;
