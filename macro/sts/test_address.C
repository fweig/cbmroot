        ...
        for (UInt_t modu = 0; modu < nModules; modu++) {
          for (UInt_t sens = 0; sens < nSensors; sens++) {
            for (UInt_t side = 0; side < nSides; side++) {
              ...
              Int_t address =
                CbmStsAddress::GetAddress(unit, ladd, hlad, modu, sens, side);
              ...
              UInt_t rHlad =
                CbmStsAddress::GetElementId(address, kStsHalfLadder);
              ...
              if (CbmStsAddress::GetSystemId(address) != kSts) {
                std::cout << "GetSystemId "
                          << CbmStsAddress::GetSystemId(address)
                          << " does not return STS" << std::endl;
                success = kFALSE;
              }
              if (rUnit != unit) {
                std::cout << "Different unit " << unit << " " << rUnit
                          << std::endl;
                success = kFALSE;
              }
              if (rLadd != ladd) {
                std::cout << "Different ladder " << ladd << " " << rLadd
                          << std::endl;
                success = kFALSE;
              }
              if (rHlad != hlad) {
                std::cout << "Different half-ladder " << hlad << " " << rHlad
                          << std::endl;
                success = kFALSE;
              }
              if (rModu != modu) {
                std::cout << "Different module " << modu << " " << rModu
                          << std::endl;
                success = kFALSE;
              }
              if (rSens != sens) {
                std::cout << "Different sensor " << sens << " " << rSens
                          << std::endl;
                success = kFALSE;
              }
              if (rSide != side) {
                std::cout << "Different side " << side << " " << rSide
                          << std::endl;
                success = kFALSE;
              }
              ...
              Int_t sensAdd1 =
                CbmStsAddress::GetAddress(unit, ladd, hlad, modu, sens);
              Int_t sensAdd2 =
                CbmStsAddress::GetMotherAddress(address, kStsSensor);
              if (sensAdd1 != sensAdd2) {
                std::cout << "Sensor address " << sensAdd1 << " " << sensAdd2
                          << std::endl;
                success = kFALSE;
              }
              Int_t moduAdd1 =
                CbmStsAddress::GetAddress(unit, ladd, hlad, modu);
              Int_t moduAdd2 =
                CbmStsAddress::GetMotherAddress(address, kStsModule);
              if (sensAdd1 != sensAdd2) {
                std::cout << "Module address " << moduAdd1 << " " << moduAdd2
                          << std::endl;
                success = kFALSE;
              }
              Int_t hladAdd1 = CbmStsAddress::GetAddress(unit, ladd, hlad);
              Int_t hladAdd2 =
                CbmStsAddress::GetMotherAddress(address, kStsHalfLadder);
              if (hladAdd1 != hladAdd2) {
                std::cout << "Half-ladder address " << hladAdd1 << " "
                          << hladAdd2 << std::endl;
                success = kFALSE;
              }
              Int_t laddAdd1 = CbmStsAddress::GetAddress(unit, ladd);
              Int_t laddAdd2 =
                CbmStsAddress::GetMotherAddress(address, kStsLadder);
              if (laddAdd1 != laddAdd2) {
                std::cout << "Ladder address " << laddAdd1 << " " << laddAdd2
                          << std::endl;
                success = kFALSE;
              }
              Int_t unitAdd1 = CbmStsAddress::GetAddress(unit);
              Int_t unitAdd2 =
                CbmStsAddress::GetMotherAddress(address, kStsUnit);
              if (unitAdd1 != unitAdd2) {
                std::cout << "Unit address " << unitAdd1 << " " << unitAdd2
                          << std::endl;
                success = kFALSE;
              }
