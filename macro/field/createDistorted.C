
{
  ...
  CbmFieldMapDistorted* dist_field = new CbmFieldMapDistorted(
    "Tst1", "DistortedFormulas_001", parentMapName.Data());
  ...
  dist_field->SetDistortionFormula(
    "0. +1.0*(z>100 && z<200)", "x", "m");  // =0 for z<100 and z>200

  // By_new =By_old*2 [z>100 && z<200]; =By_old+0.5 [z>250]; =By_old for other z
  dist_field->SetDistortionFormula(
    "1.0+2.0*(z>100 && z<200)", "y", "m");  // =1 for z<100 and z>200
  ...
  dist_field->SetDistortionFormula(
    "TMath::Landau(z)*sin(x)", "z", "a");  // Landau oscillations along z
  ...
  cout
    << " x     y      z       Bx_new  By_new  Bz_new    Bx_old  By_old  Bz_old"
    << endl;
