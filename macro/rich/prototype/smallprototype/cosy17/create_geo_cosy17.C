...
void create_geo_cosy17()
{
  gSystem->Load("libGeom");
  //gGeoMan = gGeoManager;// (TGeoManager*)gROOT->FindObject("FAIRGeom");
  //new TGeoManager ("Testbox", "Testbox");

  RichGeomType richGeomType = kQuarzPlate;

  TString geoName     = "rich_cosy17_quartz";
  TString geoFileName = "/Users/slebedev/Development/cbm/trunk/cbmroot/geometry/rich/" + geoName + ".geo.root";
  ...
  const Double_t pmtMatrixGap     = 5.1;  //3.5;
  ...
  const Double_t quartzPmtDistance = 2.;
  ...
  TGeoVolume* caveVol    = gGeoMan->MakeBox(geoName, medAl, (boxWidth + boxThickness) / 2.,
