...
void run_analysis_rings2(Bool_t generateCalib = kTRUE, TString inputFile, TString outputFile, TString calibFile,
                         TString histoFile, TString histo2File, TString fileBasename, Int_t inNevents = -1)
{
  ...
  fgCalibrator->SetCorrInputFilename("/store/cbm_rich_data_2014/Corrections_WLS_on_no29channels.txt");
  ...
  CbmRichProtoAnalysis* unpack_qa = new CbmRichProtoAnalysis();
  //unpack_qa->SetWlsState("on");
