...
void draw_a_epi()
{
  ...
  for (Int_t i = 45; i < 290; i++) {
    ...
    if (npi_all) { h_mpid->SetBinContent(i + 1, 100 * npi_el / npi_all); }
  }
...
void draw_ntruehits_vs_ba(Int_t nev = 1)
{
  ...
  for (Int_t i = 0; i < nev; i++) {
    ...
    for (Int_t r = 0; r < nr; r++) {
      ...
      if (NULL == ring || NULL == ringMatch) { continue; }
      a         = ring->GetAaxis();
      b         = ring->GetBaxis();
