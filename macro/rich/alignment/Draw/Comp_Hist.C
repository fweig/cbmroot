...
void Comp_Hist()
{
  ...
  TString Path_Histo = "/lustre/nyx/cbm/users/jbendar/Sim_Outputs/"
                       "Ring_Track_VS_Position/Aligned/merge.root";
  TFile* Histograms = new TFile(Path_Histo, "READ");
  Aligned           = (TH2D*) gDirectory->Get(Histo_Name);

  Path_Histo = "/lustre/nyx/cbm/users/jbendar/Sim_Outputs/"
               "Ring_Track_VS_Position/Misaligned_1mrad_Full/merge.root";
  TFile* Histograms = new TFile(Path_Histo, "READ");
  Misaligned_1mrad  = (TH2D*) gDirectory->Get(Histo_Name);

  Path_Histo = "/lustre/nyx/cbm/users/jbendar/Sim_Outputs/"
               "Ring_Track_VS_Position/Misaligned_5mrad_Full/merge.root";
  TFile* Histograms = new TFile(Path_Histo, "READ");
