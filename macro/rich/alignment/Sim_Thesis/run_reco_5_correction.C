...
void run_reco_5_correction(Int_t nEvents)
{
  ...
  //    TString setupName = "setup_misalign_gauss_sigma_1", outDir = "/data/Sim_Outputs/Gauss_sigma_1_new/";
  ...
  std::cout << std::endl;
  macroName = srcDir + "/macro/run/modules/reconstruct_corr.C";
  std::cout << "Loading macro " << macroName << std::endl;
  gROOT->LoadMacro(macroName);
  Bool_t recoSuccess = gROOT->ProcessLine("reconstruct_corr()");
  if (!recoSuccess) {
