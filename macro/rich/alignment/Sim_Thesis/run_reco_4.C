void run_reco_4(Int_t nEvents = 10000) {
  ...
  TString setupName = "setup_misalign_gauss_sigma_1",
          outDir =
            "/data/Sim_Outputs/Sim_Thesis/AuAu_10AGeV/Misaligned_1mrad_new/";
  ...
  std::cout << std::endl;
  macroName = srcDir + "/macro/run/modules/reconstruct_batch.C";
  std::cout << "Loading macro " << macroName << std::endl;
  gROOT->LoadMacro(macroName);
  Bool_t recoSuccess = gROOT->ProcessLine("reconstruct_batch()");
  if (!recoSuccess) {
