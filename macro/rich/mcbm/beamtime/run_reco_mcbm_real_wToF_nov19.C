void run_reco_mcbm_real_wToF_nov19(
  const string& parFile =
    "/lustre/nyx/cbm/users/adrian/data/nov19/unp_mcbm_params_370.root",
  const string& digiFile =
    "/lustre/nyx/cbm/users/adrian/data/nov19/unp_mcbm_370.root",
  const string& recoFile = "reco_mcbm_nov19.root",
  int nEvents            = 1000) {
  const unsigned int runId = 370;
  ...
  TString TofFileFolder = Form(
    "/lustre/nyx/cbm/users/nh/CBM/cbmroot/trunk/macro/beamtime/mcbm2018/%s",
    cFileId.Data());
  ...
  std::cout << std::endl
            << "-I- digibdf.par file : " << workDir << "/parameters/tof/"
  ...
  TString geoFile = "/lustre/nyx/cbm/users/adrian/cbmsource/macro/mcbm/data/"
                    "mcbm_beam_2019_11.geo.root";
  ...
  qaTask->SetOutputDir(Form("result_nov19_run%d", runId));
