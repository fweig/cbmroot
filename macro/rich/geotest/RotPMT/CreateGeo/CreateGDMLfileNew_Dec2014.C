...
void CreateGDMLfileNew(float PMTrotX = 5, float PMTrotY = 5, int RotMir = -10) {
  ...
  float DefaultRotX       = 27.952765 + 5.;
  float DefaultRotY       = 13.477 + 5.;
  int RICH_mirror_Y_shift = 805;
  float pmt_pos_x_addend = 0., pmt_pos_y_addend = 0., pmt_pos_z_addend = 0.;
  float pmt_width  = 1300.;
  float pmt_height = 750;

  if (RotMir == 1) {
    DefaultRotX         = 6.418315 + 5.;
    DefaultRotY         = 13.477 + 5.;
    RICH_mirror_Y_shift = 835;
    pmt_pos_x_addend    = 0.;
    pmt_pos_y_addend    = 0.;
    pmt_pos_z_addend    = 0.;
  }
  ...
  char* InFileUpper = "CreateGeo/GdmlUpperPart_Dec2014.txt";  //[256];
  char* InFileLower = "CreateGeo/GdmlLowerPart_Dec2014.txt";  //[256];
  ...
  sprintf(GeoFileName,
          "/hera/cbm/users/tariq/cbmroot/geometry/rich/GeoOpt/RotPMT/NewGeo/"
  ...
  outfile << "<variable name=\"RICH_mirror_angle\" value=\"" << RotMir << "\"/>"
          << std::endl;
