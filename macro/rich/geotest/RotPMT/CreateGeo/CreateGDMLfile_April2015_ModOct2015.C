...
void CreateGDMLfile_April2015_ModOct2015(float pmt_pos_y_addend = 0, float pmt_pos_z_addend = 0, int RotMir = -10,
                                         float PMTrotX = 5, float PMTrotY = 5, float EnlargedPMTWidth = 40.,
                                         float EnlargedPMTHight = 20., int DefaultDims = 0,
                                         int DefaultDims_LargePMT = 0)
{
  ...
  if (DefaultDims == 1) {
    Delta_pmt_width  = 0.;
    Delta_pmt_height = 0.;
    if (DefaultDims_LargePMT == 1) {
      Delta_pmt_width  = EnlargedPMTWidth;
      Delta_pmt_height = EnlargedPMTHight;
    }

    Delta_RICH_height         = 2.2 * Delta_pmt_height;
    Delta_RICH_entrance_width = 2.2 * Delta_pmt_width;
    Delta_RICH_exit_width     = 2.2 * Delta_pmt_width;

    pmt_pos_z_MyAddend = 0.;
    Delta_RICH_length  = 0;

    Delta_RICH_position_from_IP = 0.;
  ...
  char* InFileUpper = "CreateGeo/2015_minus10deg_UpperPart.gdml";
  ...
  sprintf(GeoFileName, "/geometry/rich/GeoOpt/rich_geo_%s_RotPMT_%s_%s_TransPMT_%s_%s_%s%s", RotMirText, ShiftXTxt,
          ShiftYTxt, YTransText, ZTransText, PMTDimsText, ExtraText.Data());
  ...
  //get upper part
  // while ( getline (infile1,line) ){OutFile_GDML << line<<std::endl;}
