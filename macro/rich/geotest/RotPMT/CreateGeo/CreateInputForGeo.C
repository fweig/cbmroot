...
void CreateInputForGeo(float PMTrotX = 5., float PMTrotY = 5.) {
  ...
  outfile << "  z     length   dWindows   dVessel    lRadiator  radiator"
          << std::endl;
  outfile
    << "1800.     1900.       0.25        5.        1500.    RICHgas_CO2_dis"
    << std::endl;
  ...
  outfile << "  z    y      dx      dy    angle_x  angle_y  dGlass  dCathode  "
             "lTube   dSupport"
          << std::endl;
  outfile << " 200.  1275.  1000.   300.   " << PMTrotX << "      "
          << -1 * PMTrotY << "        0.     0.5       0.        0."
          << std::endl;
  outfile << "                  " << std::endl;
  outfile.close();
