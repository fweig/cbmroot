void create_RICH_geo_file(
  float PMTrotX = 5.,
  float PMTrotY = 5.) {  //char *geometry="rich-2900-N2-0"){
  ...
  sprintf(infile,
          "/data/cbmroot/geometry/rich/GeoOpt/RotPMT/GeoFiles/VeryOldGeo/"
          "DatFiles/rich_inputfile_RotPMT_%s_%s.dat",
          ShiftXTxt,
          ShiftYTxt);
  sprintf(
    outfile,
    "/data/cbmroot/geometry/rich/GeoOpt/RotPMT/GeoFiles/VeryOldGeo/"
    "rich_geo_RotMir_p1_RotPMT_%s_%s.geo",
    ShiftXTxt,
    ShiftYTxt);  //_X%dpoint%d_Y%dpoint%d.geo",IntegerXValue,ShiftXmod10, IntegerYValue, ShiftYmod10);  //sprintf(outfile,"%s.geo",geometry);
  ...
  if (!fopen(infile, "r")) {
    ...
    zI1 = zPMT + lRadiator - 947.;
    xI1 = (xB2 - xB1) / lBarrel * (zI1 - dWindow) + (xB1 - dVessel);
    // end
    zI2 = zPMT + lRadiator - 947. + 2.;  //change
    // zI2 = zI1+1.;
    ...
    //sprintf(outfile,"/data/cbmroot/geometry/rich/GeoOpt/RotPMT/GeoFiles/VeryOldGeo/rich_geo_RotMir_p1_RotPMT_X%dpoint%d_Y%dpoint%d.gdml",IntegerXValue,ShiftXmod10, IntegerYValue, ShiftYmod10);
