#include <fstream>
#include <iostream>
void CreateGDMLfileOld(float PMTrotX = 5,
                       float PMTrotY = -5,
                       int RotMir    = -10) {  //int RotMir=-10){
  ...
  char MirrText[256];
  if (RotMir < 0) {
    sprintf(MirrText, "RotMir_m%d", RotMir * -1);
  } else {
    sprintf(MirrText, "RotMir_p%d", RotMir);
  }
  ...
  if (RotMir < 0) {
    sprintf(InFileUpper, "CreateGeo/Upper_RotMir_m%d.txt", RotMir * -1);
  }
  if (RotMir == 1) {
    sprintf(InFileUpper, "CreateGeo/Upper_RotMir_p%d.txt", RotMir);
  }
  std::ifstream infile1(InFileUpper);
  std::ifstream infile2("CreateGeo/Lower.txt");
  sprintf(GeoFileName,
          "/hera/cbm/users/tariq/cbmroot/geometry/rich/GeoOpt/RotPMT/OldGeo/"
