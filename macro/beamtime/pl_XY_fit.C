void pl_XY_fit(Int_t iNSt      = 4,
  ...
  for (Int_t iSt = 0; iSt < iNSt; iSt++) {
    ...
    if (h3f != NULL) {
      ...
      cout << "Nbins: " << Nxbins << "," << Nybins << "," << Nzbins
           << " -> Dbin" << iDeltaX << "," << iDeltaY << endl;

      h2n = new TH2D(Form("hTH3FitNorm%s%d", cOpt.Data(), iSt),
                     Form("#Delta%s Fit Entries; x (cm); y (cm)", cOpt.Data()),
                     NewNbinsX,
      ...
      h2m = new TH2D(Form("hTH3FitMean%s%d", cOpt.Data(), iSt),
                     Form("#Delta%s Fit Mean; x (cm); y (cm)", cOpt.Data()),
                     NewNbinsX,
                     h3f->GetXaxis()->GetXmin(),
                     h3f->GetXaxis()->GetXmax(),
                     NewNbinsY,
      ...
      h2s = new TH2D(Form("hTH3FitSigma%s%d", cOpt.Data(), iSt),
                     Form("#Delta%s Fit Sigma; x (cm); y (cm)", cOpt.Data()),
                     NewNbinsX,
                     h3f->GetXaxis()->GetXmin(),
                     h3f->GetXaxis()->GetXmax(),
                     NewNbinsY,
      ...
      for (Int_t ix = 0; ix < NewNbinsX; ix++) {
        for (Int_t iy = 0; iy < NewNbinsY; iy++) {
          ...
          TH1* hsel =
            h3f->ProjectionZ(Form("Delta%s%d_%d_%d", cOpt.Data(), iSt, ix, iy),
                             ix * iDeltaX + 1,
                             (ix + 1) * iDeltaX + 1,
                             iy * iDeltaY + 1,
                             (iy + 1) * iDeltaY + 1);
          ...
          cout << " fit ix=" << ix << ", iy=" << iy
               << ", Integral: " << hsel->Integral() << ", Mean " << dFMean
               << ", Lim " << dFLim << endl;
          if (hsel->Integral() > 10) {
            TFitResultPtr fRes =
            ...
            if (iFitStatus == 0) {  // check validity of fit
              ...
              cout << "Fres: " << fRes->Parameter(1) << ", "
                   << fRes->Parameter(2) << endl;
            }
          }
        }
      }
