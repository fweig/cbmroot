void pl_ana_DYXY(Int_t iDut = 901, Int_t NewNbinsX = 4, Int_t NewNbinsY = 4, Int_t iOpt = 0)
{
  ...
  h2n = new TH2D("hTH3FitMean", "#DeltaY Fit Entries; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());
  h2m = new TH2D("hTH3FitMean", "#DeltaY Fit Mean; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());
  h2s = new TH2D("hTH3FitSigma", "#DeltaY Fit Sigma; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());

  Int_t iCol = 0;
  for (Int_t ix = 0; ix < NewNbinsX; ix++) {
    for (Int_t iy = 0; iy < NewNbinsY; iy++) {
      TH1D* hsel      = h3f->ProjectionZ(Form("DeltaY_%d_%d", ix, iy), ix * iDeltaX + 1, (ix + 1) * iDeltaX + 1,
