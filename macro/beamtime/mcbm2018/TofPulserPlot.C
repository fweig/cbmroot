...
Bool_t TofPulserPlot(TString sFilename)
{
  ...
  for (UInt_t uGdpb = 0; uGdpb < kuNbGdpbs; ++uGdpb) {
    TCanvas* cGdpb = new TCanvas(Form("cGdpb%02u", uGdpb), Form("Evolution of Time difference profile "
    ...
    for (UInt_t uFee = 0; uFee < kuNbFeePerGdpb; ++uFee) {
      ...
      switch (uFee % 3) {
        case 0: phTofPulserEvoFeeToRef[uGdpb][uFee]->SetLineColor(kBlack); break;
