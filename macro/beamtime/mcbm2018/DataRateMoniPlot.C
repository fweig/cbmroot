...
Bool_t DataRateMoniPlot()
{
   ...
   for( UInt_t uRun = 0; uRun < kuNbRuns; ++ uRun )
   {
      ...
      for( UInt_t uLink = 0; uLink < kuNbFlimLinks; ++uLink )
      {
         ...
         if( NULL != tempProf )
         {
            ...
            else phMsSzTime[ uRun ][ uLink ] = nullptr;
      } // for( UInt_t uLink = 0; uLink < kuNbFlimLinks; ++uLink )
   } // for( UInt_t uRun = 0; uRun < kuNbRuns; ++ uRun )
   ...
   for( UInt_t uRun = 0; uRun < kuNbRuns; ++ uRun )
   {
      cMsSzTime[ uRun ] = new TCanvas( Form( "cMsSzTime_%03u", uRunId[ uRun ]), Form( "Mean MS size in Bytes per link in run %3u", uRunId[ uRun ]) );
      cMsSzTime[ uRun ]->Divide( 4, 4 );

      for( UInt_t uLink = 0; uLink < kuNbFlimLinks; ++uLink )
      {
         if( nullptr == phMsSzTime[ uRun ][ uLink ] )
            continue;
