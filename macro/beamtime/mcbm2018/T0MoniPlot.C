...
Bool_t T0MoniPlot()
{
   ...
   UInt_t uRunId[ kuNbRuns ] = {
       89,  90,  91,  92,  93,  94,  95,  96,  97, // Detectors parameters scan
       98,  99, 100, 101, 102, 103, 104, 105, 106, // Beam intensity and target scan (=> collision rate scan up, empty, no beam)
   ...
   for( UInt_t uRun = 0; uRun < kuNbRuns; ++ uRun )
   {
      ...
      {
         pStacksHitSpill[ uRun / kuNbRunPerStack ] = new THStack( Form( "stackHitSpill_%02u", uRunId[ uRun ] ),
                                            Form( "Evolution of T0 hits per spill for runs %02u to %02u", uRunId[ uRun ], uRunId[ uRun + kuNbRunPerStack - 1]) );
         pStacksHitEvo[ uRun / kuNbRunPerStack ] = new THStack( Form( "stackHitCnt_%02u", uRunId[ uRun ] ),
                                            Form( "Evolution of To hits count for runs %02u to %02u", uRunId[ uRun ], uRunId[ uRun + kuNbRunPerStack - 1]) );
      } // if( 0 == uRun % kuNbRunPerStack )
