...
void PlotCoinc( TString sFilename, Int_t iNbTs = -1, Double_t dOffsetX = 0.0, Double_t dOffsetY = 0.0 )
{
   ...
   TH2 * phHitsPositionHodoA  = new TH2I( "phHitsPositionHodoA", "Position of the hits in hodoscope A; X [cm]; Y [cm]; Hits []",
                                           80,  -4.0,  4.0,
                                           80,  -4.0,  4.0 );
   TH2 * phHitsPositionSts    = new TH2I( "phHitsPositionSts",  "Position of the hits in hodoscope B; X [cm]; Y [cm]; Hits []",
                                          200, -10.0, 10.0,
                                           80,  -4.0,  4.0 );
   TH2 * phHitsPositionHodoB  = new TH2I( "phHitsPositionHodoB", "Position of the hits in hodoscope B; X [cm]; Y [cm]; Hits []",
                                           80,  -4.0,  4.0,
                                           80,  -4.0,  4.0 );
///---------------------------------------------------------------------///

   TH2 * phNbHitsCompHodo     = new TH2I( "phNbHitsCompHodo", "Number of hits per TS in Hodo A vs Hodo B; Nb Hits A[]; Nb Hits B []; TS []",
                                          10, 0.0, 10.0,
                                          10, 0.0, 10.0 );
   TH2 * phNbHitsCompStsHodoA = new TH2I( "phNbHitsCompStsHodoA", "Number of hits per TS in STS vs Hodo A; Nb Hits STS[]; Nb Hits A []; TS []",
                                          10, 0.0, 10.0,
                                          10, 0.0, 10.0 );
   TH2 * phNbHitsCompStsHodoB = new TH2I( "phNbHitsCompStsHodoB", "Number of hits per TS in STS vs Hodo B; Nb Hits STS[]; Nb Hits B []; TS []",
                                          10, 0.0, 10.0,
                                          10, 0.0, 10.0 );
///---------------------------------------------------------------------///

   TH2 * phHitsCoincCorrXX   = new TH2I( "phHitsCoincCorrXX", "XX correlation of the coincident hits; X_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsCoincCorrYY   = new TH2I( "phHitsCoincCorrYY", "YY correlation of the coincident hits; Y_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsCoincCorrXY   = new TH2I( "phHitsCoincCorrXY", "XY correlation of the coincident hits; X_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsCoincCorrYX   = new TH2I( "phHitsCoincCorrYX", "YX correlation of the coincident hits; Y_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );

   TH2 * phHitsPositionCoincA = new TH2I( "phHitsPositionCoincA", "Position of the coincident hits in hodoscope A; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsPositionCoincB = new TH2I( "phHitsPositionCoincB", "Position of the coincident hits in hodoscope B; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsPositionDiff   = new TH2I( "phHitsPositionDiff", "Position difference of the coincident hits; X_B - X_A [cm]; Y_B- Y_A [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   ...
   TH1 * phHitsCoincDist     = new TH1I( "phHitsCoincDist", "XY distance of the coincident hits; Dist. [cm]; Hits []",
                                         100,   0.0, 10.0 );
   TH1 * phHitsCoincAngle     = new TH1I( "phHitsCoincAngle", "Vertical angle of the coincident hits; Angle [deg.]; Hits []",
                                         180, -90.0, 90.0 );
///---------------------------------------------------------------------///

   TH2 * phHitsSingleCoincCorrXX   = new TH2I( "phHitsSingleCoincCorrXX", "XX correlation of the coincident hits; X_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsSingleCoincCorrYY   = new TH2I( "phHitsSingleCoincCorrYY", "YY correlation of the coincident hits; Y_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsSingleCoincCorrXY   = new TH2I( "phHitsSingleCoincCorrXY", "XY correlation of the coincident hits; X_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsSingleCoincCorrYX   = new TH2I( "phHitsSingleCoincCorrYX", "YX correlation of the coincident hits; Y_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );

   TH2 * phHitsSinglePositionCoincA = new TH2I( "phHitsSinglePositionCoincA", "Position of the coincident hits in hodoscope A; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsSinglePositionCoincB = new TH2I( "phHitsSinglePositionCoincB", "Position of the coincident hits in hodoscope B; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsSinglePositionDiff   = new TH2I( "phHitsSinglePositionDiff", "Position difference of the coincident hits; X_B - X_A [cm]; Y_B- Y_A [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH1 * phHitsSingleTimeDiff       = new TH1I( "phHitsSingleTimeDiff", "Time difference of the coincident hits; t_B - t_A [ns]; Hits []",
                                          2 * iCoincLimitClk, -dCoincLimit, dCoincLimit );

   TH1 * phHitsSingleCoincDist     = new TH1I( "phHitsSingleCoincDist", "XY distance of the coincident hits; Dist. [cm]; Hits []",
                                         100,   0.0, 10.0 );
   TH1 * phHitsSingleCoincAngle     = new TH1I( "phHitsSingleCoincAngle", "Vertical angle of the coincident hits; Angle [deg.]; Hits []",
                                         180, -90.0, 90.0 );
///---------------------------------------------------------------------///

   TH2 * phHitsBestCoincCorrXX   = new TH2I( "phHitsBestCoincCorrXX", "XX correlation of the coincident hits; X_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsBestCoincCorrYY   = new TH2I( "phHitsBestCoincCorrYY", "YY correlation of the coincident hits; Y_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsBestCoincCorrXY   = new TH2I( "phHitsBestCoincCorrXY", "XY correlation of the coincident hits; X_A [cm]; Y_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH2 * phHitsBestCoincCorrYX   = new TH2I( "phHitsBestCoincCorrYX", "YX correlation of the coincident hits; Y_A [cm]; X_B [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );

   TH2 * phHitsBestPositionCoincA = new TH2I( "phHitsBestPositionCoincA", "Position of the coincident hits in hodoscope A; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsBestPositionCoincB = new TH2I( "phHitsBestPositionCoincB", "Position of the coincident hits in hodoscope B; X [cm]; Y [cm]; Hits []",
                                          80, -4.0, 4.0,
                                          80, -4.0, 4.0 );
   TH2 * phHitsBestPositionDiff   = new TH2I( "phHitsBestPositionDiff", "Position difference of the coincident hits; X_B - X_A [cm]; Y_B- Y_A [cm]; Hits []",
                                         160, -8.0, 8.0,
                                         160, -8.0, 8.0 );
   TH1 * phHitsBestTimeDiff       = new TH1I( "phHitsBestTimeDiff", "Time difference of the coincident hits; t_B - t_A [ns]; Hits []",
                                          2 * iCoincLimitClk, -dCoincLimit, dCoincLimit );

   TH1 * phHitsBestCoincDist      = new TH1I( "phHitsBestCoincDist", "XY distance of the coincident hits; Dist. [cm]; Hits []",
                                         100,   0.0, 10.0 );
   TH1 * phHitsBestCoincAngle     = new TH1I( "phHitsBestCoincAngle", "Vertical angle of the coincident hits; Angle [deg.]; Hits []",
                                         180, -90.0, 90.0 );
///---------------------------------------------------------------------///
   TH1 * phHitsStsTimeDiffBestHodo = new TH1I( "phHitsStsTimeDiffBestHodo", "Position difference of STS hit with the best coincident hits; t_Sts - t_(AB) [ns]; Hits []",
                                          2 * iCoincLimitClk, -dCoincLimit, dCoincLimit );
   TH2 * phHitsStsPosDiffBestHodo = new TH2I( "phHitsStsPosDiffBestHodo", "Position difference of STS hit with the best coincident hits; X_Sts - X_extr(AB) [cm]; Y_Sts- Y_extr(AB) [cm]; Hits []",
                                         400, -2.0, 2.0,
                                         400, -2.0, 2.0 );
         ...
         if( uBestB < vHitsHodoB.size() )
         {
            ...
            for( UInt_t uHitSts = 0; uHitSts < vHitsSts.size(); ++uHitSts )
            {
               ...
               if( TMath::Abs( dTimeDiffSts ) < dCoincLimit )
               {
                  ...
                  else std::cout << iTs << " " << vHitsHodoA.size() << " " << vHitsSts.size() << " " << vHitsHodoB.size()
