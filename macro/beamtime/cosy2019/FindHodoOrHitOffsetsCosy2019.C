...
Bool_t FindHodoOrHitOffsetsCosy2019( Long64_t liNbEntryToRead = -1, UInt_t uRunId = 25,
                        Double_t dWinStart = -100., Double_t dWinStop = 100.,
                        UInt_t uHodoWinLimClk = 100, UInt_t uStsWinLimClk  = 500,
                        ...
                        TString sInputFileNameHits = "data/cosy2019" )
{
   ...
   /// Histograms
      /// Raw
   TH2 * hDigisNbEvoTs = new TH2D( "hDigisNbEvoTs", "Nb Digis vs Tree entry (TS), per OR channel; Entry []; Channel []; Counts [Digis]",
                                 10000, 0, 10000,
                                     4, 0,     4 );
   TH2 * hDigisNbEvo = new TH2D( "hDigisNbEvo", "Nb Digis vs Time, per OR channel; Time in run [s]; Channel []; Counts [Digis]",
                                 12000, 0, 120,
                                     4, 0,     4 );
      /// Time diff
   TH2 * hTimeDiffAxis = new TH2D( "hTimeDiffAxis", "Time difference to A_X for A_Y, B_X and B_Y; T_n - t_AX [s]; Axis; Counts [Digis]",
                                 uNbTimeBins, dWinStart, dWinStop,
                                 3, 0.5, 3.5 );
   TH2 * hTimeDiffBest = new TH2D( "hTimeDiffBest", "Time difference to A_X for A_Y, B_X and B_Y, best match; T_n - t_AX [s]; Axis; Counts [Digis]",
                                 uNbTimeBins, dWinStart, dWinStop,
                                 3, 0.5, 3.5 );
   TH2 * hTimeDiffEvoAY = new TH2D( "hTimeDiffEvoAY", "Time difference to A_X vs Time; Time in run [s]; t_AY - t_AX; Counts [Digis]",
                                 1200, 0, 120,
                                 uNbTimeBins, dWinStart, dWinStop);
   TH2 * hTimeDiffEvoBX = new TH2D( "hTimeDiffEvoBX", "Time difference to A_X vs Time; Time in run [s]; t_BX - t_AX; Counts [Digis]",
                                 1200, 0, 120,
                                 uNbTimeBins, dWinStart, dWinStop);
   TH2 * hTimeDiffEvoBY = new TH2D( "hTimeDiffEvoBY", "Time difference to A_X vs Time; Time in run [s]; t_BY - t_AX; Counts [Digis]",
                                 1200, 0, 120,
                                 uNbTimeBins, dWinStart, dWinStop);
      /// Coincidence stats
   TH1 * hCoincNbEvo = new TH1D( "hCoincNbEvo", "Nb OR Coinc vs Time; Time in run [s]; Coinc Counts []",
                                 12000, 0, 120 );
   TH2 * hTimeDiffEvoAB = new TH2D( "hTimeDiffEvoAB", "Mean Time difference of hodo B to A vs Time; Time in run [s]; (t_BX + t_BY)/2 - (t_AX + t_AY)/2; Counts [Digis]",
                                 1200, 0, 120,
                                 uNbTimeBins, dWinStart, dWinStop);

      /// Hits Raw
   TH2 * hHitsNbEvoTs = new TH2D( "hHitsNbEvoTs", "Nb Hits vs Tree entry (TS), per system; Entry []; System []; Counts [Hits]",
                                 10000, 0, 10000,
                                     2, 0,     2 );
   TH2 * hHitsNbEvo = new TH2D( "hHitsNbEvo", "Nb Hits vs Tree entry (TS), per system; Time in run [s]; System []; Counts [Hits]",
                                 12000, 0, 1200,
                                     2, 0,     2 );
      /// Hits Detectors
   TH2 * hDetHitsNbEvoTs = new TH2D( "hDetHitsNbEvoTs", "Nb Hits vs Tree entry (TS), per detector; Entry []; System []; Counts [Hits]",
                                 10000, 0, 10000,
                                     4, 0,     4 );
   TH2 * hDetHitsNbEvo = new TH2D( "hDetHitsNbEvo", "Nb Hits vs Tree entry (TS), per detector; Time in run [s]; System []; Counts [Hits]",
                                 12000, 0, 1200,
                                     4, 0,     4 );

      /// Hodo Time difference
   TH1 * hTimeDiffHitHodoA = new TH1D( "hTimeDiffHitHodoA", "Time difference between Digis in the front hodoscopes and OR coincidences; T_A - T_Or [ns]; Counts [Digis]",
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );
   TH1 * hTimeDiffHitHodoB = new TH1D( "hTimeDiffHitHodoB", "Time difference between Digis in the back hodoscopes and OR coincidences; T_B - T_Or [ns]; Counts [Digis]",
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );
   TH2 * hTimeDiffEvoHitHodoA = new TH2D( "hTimeDiffEvoHitHodoA", "Evolution of Time difference between Digis in the front hodoscopes and OR coinc; Time in Run [s]; T_A - T_Or [ns]; Counts [Digis]",
                                   1200, 0, 120,
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );
   TH2 * hTimeDiffEvoHitHodoB = new TH2D( "hTimeDiffEvoHitHodoB", "Evolution of Time difference between Digis in the back hodoscopes and OR coinc; Time in Run [s]; T_B - T_Or [ns]; Counts [Digis]",
                                   1200, 0, 120,
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );

      /// STS time difference
   TH1 * hTimeDiffHitSts = new TH1D( "hTimeDiffHitSts", "Time difference between Digis on the STS and hodoscopes OR; T_Sts - T_Or [ns]; Counts [Digis]",
                                   ( 2 * uStsWinLimClk + 1 ), dStsWinStart, dStsWinStop);
   TH2 * hTimeDiffEvoHitSts = new TH2D( "hTimeDiffEvoHitSts", "Evolution of Time difference between Digis in the front hodoscopes and OR coinc; Time in Run [s]; T_Sts - T_Or [ns]; Counts [Digis]",
                                   1200, 0, 120,
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );

      /// STS if Hodo there
   TH2 * hStsPosAll  = new TH2D( "hStsPosAll", "Position of all STS hits; X [cm]; Y [cm]; Counts [Hits]",
                                   180, -4.0, 4.0,
                                   700, -4.0, 4.0 );
   TH2 * hStsPosHodo = new TH2D( "hStsPosHodo", "Position of STS hits within time window of hodoscopes coincidence; X [cm]; Y [cm]; Counts [Hits]",
                                   180, -4.0, 4.0,
                                   700, -4.0, 4.0 );
   ...
   for( Long64_t liEntry = 1; liEntry < liNbEntryToRead; liEntry++)
   {
      ...
      {
         CbmTofDigi & pDigi = vDigisT0->at( uT0Digi )
         UInt_t uChannel = pDigi.GetChannel();
         ...
         {
            case 0:
               vDigisHodoAX.push_back( pDigi );
               break;
            case 1:
               vDigisHodoAY.push_back( pDigi );
               break;
            case 2:
               vDigisHodoBX.push_back( pDigi );
               break;
            case 3:
               vDigisHodoBY.push_back( pDigi );
               break;
            default:
               break;
         } // switch( uChannel )
   ...
   std::cout << " Nb Entries: " << liNbEntriesHits
             << " Tree addr: " << pTreeHits
             << " Branch addr Sts: " << pBranchSts
             << std::endl;

   if( -1 == liNbEntryToRead || liNbEntriesHits < liNbEntryToRead )
      liNbEntryToRead = liNbEntriesHits;
   ...
   for( Long64_t liEntry = 1; liEntry < liNbEntryToRead; liEntry++)
   {
      ...
      if( 0 == liEntry % 100 )
      std::cout << "Event " << std::setw( 6 ) << liEntry
                << " Nb Sts hits is "  << std::setw( 6 ) << uNbHitsSts
                << std::endl;
   ...
   for( UInt_t uOrCoinc = 0; uOrCoinc < vdCoincTimeHodoOr.size(); ++ uOrCoinc )
   {
      if( 0 == uOrCoinc % 1000 )
      {
         std::cout << "Coinc " << std::setw( 8 ) << uOrCoinc
                   << " / "  << std::setw( 8 ) << vdCoincTimeHodoOr.size()
                   << std::endl;
         std::cout << "HodoA " << std::setw( 8 ) << uFirstCandLastOrHodoA
                   << " / "  << std::setw( 8 ) << vHitsHodoA.size()
                   << std::endl;
         std::cout << "STS   " << std::setw( 8 ) << uFirstCandLastOrSts
                   << " / "  << std::setw( 8 ) << vHitsSts.size()
                   << std::endl;
         std::cout << "HodoB " << std::setw( 8 ) << uFirstCandLastOrHodoB
                   << " / "  << std::setw( 8 ) << vHitsHodoB.size()
                   << std::endl;
      } // if( 0 == uOrCoinc % 1000 )
