...
Bool_t
FindHodoOrHitOffsetsCosy2019(Long64_t liNbEntryToRead   = -1,
                             UInt_t uRunId              = 25,
  ...
  TH2* hTimeDiffEvoHitHodoA = new TH2D(
    ...
    dHodoWinStart,
    dHodoWinStop);
  TH2* hTimeDiffEvoHitHodoB = new TH2D(
    "hTimeDiffEvoHitHodoB",
  ...
  std::cout << " Nb Entries: " << liNbEntriesHits << " Tree addr: " << pTreeHits
            << " Branch addr Sts: " << pBranchSts << std::endl;
  ...
  for (Long64_t liEntry = 1; liEntry < liNbEntryToRead; liEntry++) {
    ...
    if (0 == liEntry % 100)
      std::cout << "Event " << std::setw(6) << liEntry << " Nb Sts hits is "
                << std::setw(6) << uNbHitsSts << std::endl;
  ...
  for (UInt_t uOrCoinc = 0; uOrCoinc < vdCoincTimeHodoOr.size(); ++uOrCoinc) {
    if (0 == uOrCoinc % 1000) {
      std::cout << "Coinc " << std::setw(8) << uOrCoinc << " / " << std::setw(8)
                << vdCoincTimeHodoOr.size() << std::endl;
      std::cout << "HodoA " << std::setw(8) << uFirstCandLastOrHodoA << " / "
                << std::setw(8) << vHitsHodoA.size() << std::endl;
      std::cout << "STS   " << std::setw(8) << uFirstCandLastOrSts << " / "
                << std::setw(8) << vHitsSts.size() << std::endl;
      std::cout << "HodoB " << std::setw(8) << uFirstCandLastOrHodoB << " / "
                << std::setw(8) << vHitsHodoB.size() << std::endl;
    }  // if( 0 == uOrCoinc % 1000 )

    for (UInt_t uHitA = uFirstCandLastOrHodoA; uHitA < vHitsHodoA.size();
         ++uHitA) {
      ...
      hTimeDiffEvoHitHodoA->Fill(vdCoincTimeHodoOr[uOrCoinc] * 1e-9,
                                 dTimeDiffHodo);
    ...
    for (UInt_t uHitSts = uFirstCandLastOrSts; uHitSts < vHitsSts.size();
         ++uHitSts) {
      ...
      hTimeDiffEvoHitSts->Fill(vdCoincTimeHodoOr[uOrCoinc] * 1e-9,
                               dTimeDiffHodo);
    ...
    for (UInt_t uHitB = uFirstCandLastOrHodoB; uHitB < vHitsHodoB.size();
         ++uHitB) {
      ...
      hTimeDiffEvoHitHodoB->Fill(vdCoincTimeHodoOr[uOrCoinc] * 1e-9,
                                 dTimeDiffHodo);
  ...
  TCanvas* cTimeDiffHits = new TCanvas(
    "cTimeDiffHits", "Time difference between Hits and HODO OR coinc");
