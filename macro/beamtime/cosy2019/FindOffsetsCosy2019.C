...
Bool_t FindOffsetsCosy2019( Long64_t liNbEntryToRead = -1, UInt_t uRunId = 12,
                                ...
                                TString sInputFileName = "data/unp_cosy" )
{
   ...
   TH1 * hHodoTimeDiff = new TH1D( "hHodoTimeDiff", "Time difference between Digis in the front and back hodoscopes; T_B - T_A [ns]; Counts [Digis]",
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );
   TH2 * hSameHodoTimeDiff = new TH2D( "hSameHodoTimeDiff", "Time difference between Digis in the same hodoscopes; T_m - T_n [ns]; Hodo;  Counts [Digis]",
                                   10000, -0.5, 9999.5,
                                   2, -0.5, 1.5 );
   TH2 * hHodoTimeDiffEvo = new TH2D( "hHodoTimeDiffEvo", "Evolution of Time difference between Digis in the front and back hodoscopes; Time in Run [s]; T_B - T_A [ns]; Counts [Digis]",
                                   120, 0, 1200,
                                   ( 2 * uHodoWinLimClk + 1 ), dHodoWinStart, dHodoWinStop );

      /// Hodo-STS time difference
   TH2 * hStsTimeDiff = new TH2D( "hStsTimeDiff", "Time difference between Digis on the STS and hodoscopes; T_Sts - T_Hodo [ns]; ASIC; Counts [Digis]",
                                   ( 2 * uStsWinLimClk + 1 ), dStsWinStart, dStsWinStop,
                                   kuNbAsics, -0.5, kuNbAsics - 0.5 );
   TH2 * hStsAsicsTimeDiff = new TH2D( "hStsAsicsTimeDiff", "Time difference between Digis on the First and last channel of STS ASICs; T_StsM0 - T_StsN127 [ns]; ASIC N; Counts [Digis]",
                                   ( 2 * uStsWinLimClk + 1 ), dStsWinStart, dStsWinStop,
                                   kuNbAsics, -0.5, kuNbAsics - 0.5 );
   TH2 * hSameStsTimeDiff = new TH2D( "hSameStsTimeDiff", "Time difference between Digis on the same STS ASIC; T_m - T_n [ns]; ASIC; Counts [Digis]",
                                   10000, -0.5, 9999.5,
                                   kuNbAsics, -0.5, kuNbAsics - 0.5 );
/*
...
*/
   ...
   for( Long64_t liEntry = 1; liEntry < liNbEntryToRead; liEntry++)
   {
      ...
      for( UInt_t uStsDigi = 0; uStsDigi < uNbDigisSts; ++uStsDigi )
      {
         ...
         switch( uAddress )
         {
            ...
            case 0x12008012:
            {
               ...
               if( 0 == uChanInAsic )
                  vDigisStsFirstChan[ uAsic ].push_back( pDigi );
               if( kuNbChanPerAsic -1 == uChanInAsic )
                  vDigisStsLastChan[ uAsic ].push_back( pDigi );
               break;
            } // STS
            case 0x10008022:
            case 0x12008022:
      ...
      for( UInt_t uDigiA = 0; uDigiA < vDigisHodoA.size(); ++uDigiA )
      {
         ...
         if( -1 < dPrevTime )
         {
       	    hSameHodoTimeDiff->Fill( dTimeA - dPrevTime, 0. );
         } // dPrevTime
         dPrevTime = dTimeA;
      ...
      for( UInt_t uDigiB = 0; uDigiB < vDigisHodoB.size(); ++uDigiB )
      {
         ...
         if( -1 < dPrevTime )
         {
            hSameHodoTimeDiff->Fill( dTimeB - dPrevTime, 1. );
         } // dPrevTime
         dPrevTime = dTimeB;
      } // for( UInt_t uDigiB = 0; uDigiB < vDigisHodoB.size(); ++uDigiB )
      ...
      for( UInt_t uAsic = 0; uAsic < kuNbAsics; ++uAsic )
      {
         ...
         for( UInt_t uSts = 0; uSts < vDigisSts[ uAsic ].size(); ++uSts )
         {
            ...
            if( -1 < dPrevTime )
            {
               hSameStsTimeDiff->Fill( dTimeSts - dPrevTime, uAsic );
            } // dPrevTime
            dPrevTime = dTimeSts;
         } // for( UInt_t uSts = 0; uSts < vDigisSts[ uAsic ].size(); ++uSts )
