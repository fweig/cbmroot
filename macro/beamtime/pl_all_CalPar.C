  ...
  for (Int_t iSt = 0; iSt < iNSt; iSt++) {
    ...
    for (Int_t iSm = 0; iSm < iSmNum[iSt]; iSm++) {
      ...
      for (Int_t iRp = 0; iRp < iRpcNum[iSt]; iRp++) {
        ...
        TString hname;
        switch (item) {
          case 0: hname = Form("cl_CorSmT%d_sm%03d_rpc%03d_Tot_Mean", iType[iSt], iSm, iRp); break;

          case 1: hname = Form("cl_CorSmT%d_sm%03d_rpc%03d_Pos_pfx", iType[iSt], iSm, iRp); break;

          case 2: hname = Form("cl_CorSmT%d_sm%03d_rpc%03d_TOff_pfx", iType[iSt], iSm, iRp); break;
        }
