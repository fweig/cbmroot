                         ...
                         UInt_t uNbFeeGdpb = 3)
{
  ...
  std::cout << "Assigned new TOF Run Start Date-Time: " << fRunStartDateTime->AsString() << std::endl;
    ...
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      ...
      if (NULL != pTemp) {
        ...
        std::cerr << "ERROR: Failed to recover the hits rate histogram for "
                  << Form("FeetRateLong_gDPB_g%02u_f%1u", uGdpb, uFeet) << ", stopping there." << std::endl;
        pSourceFile->Close();
        return kFALSE;
      ...
      if (NULL != pTemp) {
        ...
        std::cerr << "ERROR: Failed to recover the errors rate histogram for "
                  << Form("FeetErrorRateLong_gDPB_g%02u_f%1u", uGdpb, uFeet) << ", stopping there." << std::endl;
        pSourceFile->Close();
        return kFALSE;
      }  // else of if( NULL != pTemp )
  ...
  for (UInt_t uGdpb = 0; uGdpb < uNbGdpb; uGdpb++)
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      ...
      hHitsDate[uGdpb][uFeet] =
        new TH1D(Form("FeetRateLongDate_gDPB_g%02u_f%1u", uGdpb, uFeet),
  ...
  for (UInt_t uGdpb = 0; uGdpb < uNbGdpb; uGdpb++)
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      ...
      stackLong[uGdpb][uFeet] =
        new THStack(Form("hsRateLong_g%02u_f%1u", uGdpb, uFeet),
