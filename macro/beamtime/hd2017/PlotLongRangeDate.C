...
Bool_t PlotLongRangeDate(UInt_t uGdpbSel     = 0,
                         UInt_t uFeetSel     = 0,
                         Int_t dateIn        = 20170510,
                         Int_t timeIn        = 1648,
  ...
  std::cout << "Assigned new TOF Run Start Date-Time: "
            << fRunStartDateTime->AsString() << std::endl;

  /// Open Histo file
  TFile* pSourceFile = new TFile(sInFileName, "READ");
  if (NULL == pSourceFile) {
    std::cerr << "Failed to open the file with histos!" << std::endl;
    return kFALSE;
  }  // if( NULL == pSourceFile )
  ...
  /// Get Input histos
  std::vector<std::vector<TH1*>> pHitsMin(uNbGdpb,
                                          std::vector<TH1*>(uNbFeeGdpb, NULL));
  std::vector<std::vector<TH1*>> pErrorsMin(
    uNbGdpb, std::vector<TH1*>(uNbFeeGdpb, NULL));
  TH1* pTemp;
  for (UInt_t uGdpb = 0; uGdpb < uNbGdpb; uGdpb++)
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      // Recover Hits histo and clone it
      pTemp = NULL;
      pTemp = (TH1F*) (pSourceFile->FindObjectAny(
        Form("FeetRateLong_gDPB_g%02u_f%1u", uGdpb, uFeet)));
      if (NULL != pTemp) {
        pHitsMin[uGdpb][uFeet] =
          (TH1F*) (pTemp->Clone(Form("HitsMin_gDPB_g%02u_f%1u", uGdpb, uFeet)));
      }  // if( NULL != pTemp )
      else {
        std::cerr << "ERROR: Failed to recover the hits rate histogram for "
                  << Form("FeetRateLong_gDPB_g%02u_f%1u", uGdpb, uFeet)
                  << ", stopping there." << std::endl;
        pSourceFile->Close();
        return kFALSE;
      ...
      if (NULL != pTemp) {
        pErrorsMin[uGdpb][uFeet] = (TH1F*) (pTemp->Clone(
          Form("ErrorsMin_gDPB_g%02u_f%1u", uGdpb, uFeet)));
      }  // if( NULL != pTemp )
      else {
      ...
      if (pHitsMin[uGdpb][uFeet]->GetNbinsX()
            != pErrorsMin[uGdpb][uFeet]->GetNbinsX()
          || pHitsMin[uGdpb][uFeet]->GetXaxis()->GetXmin()
               != pErrorsMin[uGdpb][uFeet]->GetXaxis()->GetXmin()
          || pHitsMin[uGdpb][uFeet]->GetXaxis()->GetXmax()
               != pErrorsMin[uGdpb][uFeet]->GetXaxis()->GetXmax()) {
        std::cerr << "X axis of hits and errors input histos are not the same, "
                     "do nothing!"
                  << std::endl;
        return kFALSE;
  ...
  std::vector<std::vector<TH1*>> hHitsDate(uNbGdpb,
                                           std::vector<TH1*>(uNbFeeGdpb, NULL));
  std::vector<std::vector<TH1*>> hErrsDate(uNbGdpb,
                                           std::vector<TH1*>(uNbFeeGdpb, NULL));
  for (UInt_t uGdpb = 0; uGdpb < uNbGdpb; uGdpb++)
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      Double_t dStartTime = pHitsMin[uGdpb][uFeet]->GetXaxis()->GetXmin()
                            * 60.0;  // Convert Minutes to Seconds
      Double_t dStopTime = pHitsMin[uGdpb][uFeet]->GetXaxis()->GetXmax()
                           * 60.0;  // Convert Minutes to Seconds
      hHitsDate[uGdpb][uFeet] = new TH1D(
        Form("FeetRateLongDate_gDPB_g%02u_f%1u", uGdpb, uFeet),
        Form("Counts per second in Feet %1u of gDPB %02u; Time[s] ; Counts",
             uFeet,
             uGdpb),
        ((dStopTime - dStartTime) / (iBinSizeMin * 60.0)),
        dStartTime,
        dStopTime);
      hHitsDate[uGdpb][uFeet]->GetXaxis()->SetTimeDisplay(1);
      hHitsDate[uGdpb][uFeet]->GetXaxis()->SetTimeOffset(fiRunStartDateTimeSec);
      hErrsDate[uGdpb][uFeet] =
        new TH1D(Form("FeetErrorRateLongDate_gDPB_g%02u_f%1u", uGdpb, uFeet),
                 Form("Error Counts per second in Feet %1u of gDPB %02u; "
                      "Time[s] ; Error Counts",
                      uFeet,
                      uGdpb),
                 ((dStopTime - dStartTime) / (iBinSizeMin * 60.0)),
      ...
      /// Fill output histos
      for (Int_t iBin = 1; iBin <= pHitsMin[uGdpb][uFeet]->GetNbinsX();
           iBin++) {
        hHitsDate[uGdpb][uFeet]->Fill(
          60.0 * pHitsMin[uGdpb][uFeet]->GetBinCenter(iBin),
          pHitsMin[uGdpb][uFeet]->GetBinContent(iBin));
        hErrsDate[uGdpb][uFeet]->Fill(
          60.0 * pErrorsMin[uGdpb][uFeet]->GetBinCenter(iBin),
  ...
  for (UInt_t uGdpb = 0; uGdpb < uNbGdpb; uGdpb++)
    for (UInt_t uFeet = 0; uFeet < uNbFeeGdpb; uFeet++) {
      ...
      stackLong[uGdpb][uFeet] = new THStack(
        ...
        Form("Rate of hits and errors (red), long duration, for g%02u f%1u",
             uGdpb,
             uFeet));
  ...
  TCanvas* cFeeRatesSel =
    new TCanvas("cFeeRatesSel", "gDPB FEET rate selected");
  cFeeRatesSel->cd();
  gPad->SetLogy();
  gPad->SetGridx();
  gPad->SetGridy();
