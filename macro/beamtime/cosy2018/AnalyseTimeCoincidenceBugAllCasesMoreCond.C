...
void save_plot_to_pdf(TH1* plot,
  ...
  canvTmp->SaveAs(
    TString("plotsMoreCondNoise/") + runCase + TString(plot->GetName())
...
void save_canvas_to_pdf(TCanvas* canv, TString runCase) {
  canv->SaveAs(TString("canvasesMoreCondNoise/") + runCase
               + TString(canv->GetName()) + TString(".pdf"));
...
Bool_t AnalyseTimeCoincidenceBugAllCasesMoreCond(
  TString sFilename = "STSXyterPulserNoiseCond.root") {
