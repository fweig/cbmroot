...
void save_plot_to_pdf(TH1* plot,
  ...
  canvTmp->SaveAs(
    TString("plots2elinks/") + runCase + TString(plot->GetName())
...
void save_canvas_to_pdf(TCanvas* canv, TString runCase) {
  canv->SaveAs(TString("canvases2eLinks/") + runCase + TString(canv->GetName())
               + TString(".pdf"));
...
Bool_t AnalyseTimeCoincidenceBugAllCases2elinks(
  TString sFilename = "STSXyterPulserNoiseCond_2Elinks.root") {
