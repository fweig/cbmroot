void inspect_sync_histo(TString hFile="data/HistosMonitorTofSync.root", Double_t MaxMismatch=1.){
 ...
 for (Int_t iGdpb=0; iGdpb<NGdpb; iGdpb++){
   ...
   cout << Form("%d,%d",pTime->GetDate(),pTime->GetTime()) << ": AFCK " << iGdpb << " has " << NGet4 <<" active GET4s" << endl;  
   if (NGet4 < 130) {  // max: 6*24 = 144 
     Int_t iSec=iGdpb+13;
     gSystem->Exec(Form("/home/cbm/bin/prog_etof_sector.sh %d > /home/cbm/starsoft/dpbcontrols/LOG/PROG%d ",iSec,iSec));
 ...
 gSystem->Exec(Form("cd /home/cbm/starsoft/startup_scripts; ./DisableDataOut.sh > /dev/null; ./stop_fles.sh > /dev/null "));
 ...
 for (Int_t iGdpb=0; iGdpb<NGdpb; iGdpb++){
   ...
   if( h24->GetBinContent(iGdpb+1) > MaxMismatch || bSectorFW[iGdpb] ) {
     TDatime *pTime=new TDatime();
     cout << Form("%d,%d",pTime->GetDate(),pTime->GetTime()) << Form(": Init Gdpb %d, sector %d with %d mismatches",
								     iGdpb,iSec,(Int_t)h24->GetBinContent(iGdpb+1)) << endl;
     
     gSystem->Exec(Form("cd /home/cbm/starsoft/dpbcontrols; nohup ./start_etof_sector.sh %d  > ./LOG/EtofSector%d & ",iSec,iSec));
 ...
 gSystem->Exec( Form("echo %d > /tmp/MissingGbtx",NMissingGbtx) );  // communicate status to shell script
 
 gSystem->Exec(Form("/home/cbm/starsoft/dpbcontrols/afck_wait_configured.sh "));  // wait for completion
