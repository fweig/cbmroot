...
void convert_tsa_sdpb(TString inFile = "", Int_t nrEvents = 0) {
  ...
  source->AddUnpacker(
    raw_conv_sdpb, 0x10, ECbmModuleId::kSts);  //STS xyter from STS and MUCH
  ...
  std::cout << ">>> convert_tsa_sdpb: Macro finished successfully."
            << std::endl;
  std::cout << ">>> convert_tsa_sdpb: Output file is " << outFile << std::endl;
  std::cout << ">>> convert_tsa_sdpb: Real time " << rtime << " s, CPU time "
            << ctime << " s" << std::endl;
  std::cout << std::endl;
