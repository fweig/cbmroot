...
void check_media(const char* dataset = "test")
{
  ...
  if ( ! f->IsOpen() ) {
    std::cout << "check_media_1: geometry file " << geoFile 
              << " with the ROOT TGeoManager is not accessible!" 
              << std::endl;
    return;
  }
  ...
  for (Int_t iNode = 0; iNode < nodes->GetEntriesFast(); iNode++) {
    ...
    std::cout << "Checked " << retval.first << " sub nodes from " 
              << nodename << " and found " 
              << retval.second << " with wrongly assigned media" << std::endl;
  }
...
std::pair<int, int> loop_over_nodes(const std::vector<std::pair<TString, TString>>& matlist, TString& substitution, TString& toReplace)
{
  int media_checked{0};
  int wrong_media{0};
