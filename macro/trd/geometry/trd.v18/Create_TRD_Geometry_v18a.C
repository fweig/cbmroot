...
const Int_t layer1i[9][4] = { { 323,  323,  321,  321 },    // abc: a module type - b orientation (x90 deg) in odd - c even layers
                              { 323,  323,  321,  321 },
                              { 323,  323,  321,  321 },
                              { 900,  900,  900,  900 },  
                              { 900,    0,    0,  900 },  
...
const Int_t ModuleType[NofModuleTypes]    = {  0,  0,  0,  0,  1,  1,  1,  1, 2}; // 0 = small module, 1 = large module, 2 = bucharest small modules

// FEB inclination angle
const Double_t feb_rotation_angle[NofModuleTypes] = { 60,  90,  90,  80,  60,  60,  90,  90, 90}; // rotation around x-axis, 0 = vertical, 90 = horizontal
...
const Int_t GbtxPerModule[NofModuleTypes] = {   12,  6,  3,  0,  18,  18,      6,  6, 18 }; // for .geo.info - TODO: merge with above GbtxPerRob
const Int_t RobTypeOnModule[NofModuleTypes]={ 3333, 33,  3,  0,333333,333333, 33,  33, 333333}; // for .geo.info - TODO: merge with above GbtxPerRob
...
const Int_t AsicsPerFeb[NofModuleTypes]   = {210,210,210,105,109,109,109,109, 6}; // %100 gives number of ASICs on FEB, /100 gives grouping
...
void Create_TRD_Geometry_v18a() {
  ...
  for (Int_t iModule = 0; iModule < NofModuleTypes; iModule++) {
     ...
     else gModules[iModule] = create_trdi_module_type(moduleType-9);
  }
...
//  fprintf(ifile,"number of modules per type and layer:\n");
  ...
  for (Int_t iLayer = 0; iLayer < MaxLayers; iLayer++)
	...
	//        fprintf(ifile,"y %10.4f cm   x %10.4f cm\n", 2.5 * DetectorSizeY[1], 3.5 * DetectorSizeX[1]);
        ...
        xangle = atan((2.0 * DetectorSizeX[0] + 2.0 * DetectorSizeX[1]) / (LayerPosition[iLayer] + LayerThickness/2. + padplane_position)) * 180. / acos(-1.);
      }
...
//  TGeoMedium* electronicsVolMed = gGeoMan->GetMedium(ElectronicsVolumeMedium);
  ...
  if(IncludeLattice){ // Entrance window in the case of the Bucharest prototype
    ...
    TGeoVolume* vol_winIn_C = new TGeoVolume("vol_winIn_C", winIn_C, carbonVolMed); vol_winIn_C->SetLineColor(kGray);
    // Honeycomb layer
    TGeoBBox* winIn_HC = new TGeoBBox("winIn_HC", -0.3+activeAreaX /2., 0.3+activeAreaY/2., winIn_HC_thickness /2.);
    TGeoVolume* vol_winIn_HC = new TGeoVolume("vol_winIn_HC", winIn_HC, honeycombVolMed); vol_winIn_HC->SetLineColor(kOrange);
    // framex
    TGeoBBox* winIn_fx = new TGeoBBox("winIn_fx", -0.3+activeAreaX/2, WIN_Frame_thickness/2, winIn_HC_thickness/2.);
    TGeoVolume* vol_winIn_fx = new TGeoVolume("vol_winIn_fx", winIn_fx, frameVolMed); vol_winIn_fx->SetLineColor(kBlue);
    // framey
    TGeoBBox* winIn_fy = new TGeoBBox("winIn_fy", WIN_Frame_thickness/2, (1.8+activeAreaY)/2, winIn_HC_thickness/2.);
    TGeoVolume* vol_winIn_fy = new TGeoVolume("vol_winIn_fy", winIn_fy, frameVolMed); vol_winIn_fy->SetLineColor(kCyan);
    // Add up all components
    TGeoVolumeAssembly* trd_win_in = new TGeoVolumeAssembly("EntranceWin");
    trd_win_in->AddNode(vol_winIn_fx, 1, 
                    new TGeoTranslation("", 0., 0.6+activeAreaY /2.,  0));
    trd_win_in->AddNode(vol_winIn_fx, 2, 
                    new TGeoTranslation("", 0., -(activeAreaY /2. + 0.6), 0));
    trd_win_in->AddNode(vol_winIn_fy, 1, 
                    new TGeoTranslation("", activeAreaX /2., 0., 0));
    trd_win_in->AddNode(vol_winIn_fy, 2, 
                    new TGeoTranslation("", -activeAreaX /2., 0., 0));
...
//                     new TGeoTranslation("", 0., 0., -(winIn_thickness-winIn_C_thickness)/2.));
    module->AddNode(trd_win_in, 1, 
                    new TGeoTranslation("", 0., 0., gasBu_position-gas_thickness/2.-winIn_C_thickness-winIn_HC_thickness/2.));
  }
  ...
  TGeoVolume* vol_gas_dstr = new TGeoVolume("inlet", trd_gas_dstr, gasVolMed); vol_gas_dstr->SetLineColor(kRed);
  ...
  if(IncludePadplane) { 
    ...
    TGeoVolume* vol_trd_pp = new TGeoVolume("vol_pp_cu", trd_pp, padcopperVolMed); vol_trd_pp->SetLineColor(kRed);
    // Pad Plane
    TGeoBBox* trd_ppPCB = new TGeoBBox("pp_pcb", 1.0+activeAreaX /2., 0.9+activeAreaY /2., pp_pcb_thickness /2.);
    TGeoVolume* vol_trd_ppPCB = new TGeoVolume("vol_pp_pcb", trd_ppPCB, padpcbVolMed); vol_trd_ppPCB->SetLineColor(kGreen);
    // Pad Plane HC
    TGeoBBox* trd_ppHC_bd = new TGeoBBox("pp_hc_bd", 1.0+activeAreaX /2., 0.9+activeAreaY /2., pp_hc_thickness /2.);
    ...
    vol_pp->AddNode(vol_trd_ppHC, 1, new TGeoTranslation("", 0., 0., -pp_thickness/2+pp_pcb_thickness+pp_hc_thickness/2));
    vol_pp->AddNode(vol_trd_ppC, 1, new TGeoTranslation("", 0., 0., pp_thickness/2-pp_c_thickness/2));
    module->AddNode(vol_pp, 1, new TGeoTranslation("", 0., 0., gasBu_position+gas_thickness/2.+pp_thickness/2.));
  }
  ...
  if(IncludeGasFrame) {
    ...
    TGeoVolume* vol_frame_fx0 = new TGeoVolume("vol_frame_fx0", frame_fx0, frameVolMed); vol_frame_fx0->SetLineColor(kYellow-2);
    ...
    TGeoVolume* vol_frame_fy_0 = new TGeoVolume("vol_frame_fy_0", frame_fy_0, frameVolMed); vol_frame_fy_0->SetLineColor(kBlue);
    TGeoBBox* frame_fy_1 = new TGeoBBox("frame_fy_1", 1.0/2., (1.8 + activeAreaY) /2., 0.4/2.); // catode wire support
    TGeoVolume* vol_frame_fy_1 = new TGeoVolume("vol_frame_fy_1", frame_fy_1, frameVolMed); vol_frame_fy_1->SetLineColor(kBlue-3);
    TGeoBBox* frame_fy_2 = new TGeoBBox("frame_fy_2", 0.7/2., (1.8 + activeAreaY) /2., 0.4/2.); // anode wire support
    TGeoVolume* vol_frame_fy_2 = new TGeoVolume("vol_frame_fy_2", frame_fy_2, frameVolMed); vol_frame_fy_2->SetLineColor(kOrange+4);
    TGeoBBox* frame_fy_3 = new TGeoBBox("frame_fy_3", 0.4/2., (1.8 + activeAreaY) /2., 0.4/2.); // pad-plane support
    TGeoVolume* vol_frame_fy_3 = new TGeoVolume("vol_frame_fy_3", frame_fy_3, frameVolMed); vol_frame_fy_3->SetLineColor(kYellow+3);
    // add up framey components 
    TGeoVolumeAssembly* vol_frame_fy0 = new TGeoVolumeAssembly("vol_frame_fy0");  // the mother volume of wire support ledge
    vol_frame_fy0->AddNode(vol_frame_fy_0, 1, 
                    new TGeoTranslation("", -0.3-0.7/2., 0., -(0.4*1.5+winIn_thickness/2.))); 
    vol_frame_fy0->AddNode(vol_frame_fy_1, 1, 
                    new TGeoTranslation("", -1.0/2.,  0., -0.4)); 
    vol_frame_fy0->AddNode(vol_frame_fy_2, 1, 
                    new TGeoTranslation("", -0.7/2.,  0., 0.)); 
    vol_frame_fy0->AddNode(vol_frame_fy_3, 1, 
                    new TGeoTranslation("", -0.4/2.,  0., 0.4)); 
    ...
    trd_gas_frame->AddNode(vol_frame_fx0, 1, 
                    new TGeoTranslation("", 0., activeAreaY /2. + 0.4+0.5/2,  0));
    trd_gas_frame->AddNode(vol_frame_fx0, 2,
                    new TGeoTranslation("", 0., -(activeAreaY /2. + 0.4+0.5/2), 0));
    ...
    trd_gas_frame->AddNode(vol_frame_fy0, 1, 
                    new TGeoTranslation("",  -activeAreaX /2., 0.,  0));
    TGeoRotation *fy_rot   = new TGeoRotation(); fy_rot->RotateZ(180.);
    TGeoTranslation *fy_tra = new TGeoTranslation("", -activeAreaX /2., 0., 0);
    TGeoHMatrix  *fy_transform = new TGeoHMatrix(""); (*fy_transform)=(*fy_rot)*(*fy_tra);
  ...
  if(IncludeBackpanel) { 
    ...
    TGeoBBox* bp_pcb_bd = new TGeoBBox("bp_pcb_bd", 0.5+activeAreaX /2., 0.5+activeAreaY /2., bp_pcb_thickness/2.);
    ...
    trd_supp->AddNode(vol_bp_cu, 1, new TGeoTranslation("", 0., 0., 0.5*(bp_hc_thickness+2*bp_pcb_thickness+bp_cu_thickness)));
    ...
    module->AddNode(trd_supp, 1, new TGeoTranslation("", 0., 0., gasBu_position+0.5*gas_thickness+pp_thickness+0.5*bp_hc_thickness));
  }
...
//  const Double_t robConn_xoffset      =  6.0;
  if (IncludeFebs) {
    ...
    if (IncludeAsics) {
      ...
      TGeoVolume* vol_fasp_asic = new TGeoVolume("FASP", fasp_asic, padpcbVolMed); vol_fasp_asic->SetLineColor(kBlack);                                                                  
      TGeoBBox* fasp_conn = new TGeoBBox("fasp_conn", 0.5*faspConn_size[0], 0.5*faspConn_size[1], 0.5*faspConn_size[2]);
      TGeoVolume* vol_fasp_conn = new TGeoVolume("fasp_conn", fasp_conn, padpcbVolMed); vol_fasp_conn->SetLineColor(kRed+4);                                                                  
      for (Int_t cAsic(-1), iAsic(0); cAsic <=1; cAsic++) {
        faspro->AddNode(vol_fasp_asic, iAsic, new TGeoTranslation("", cAsic*fasp_xoffset, fasp_yoffset, 
                              -1*(0.5*FASPRO_thickness+0.5*fasp_size[2])));
        faspro->AddNode(vol_fasp_conn, iAsic, new TGeoTranslation("", cAsic*fasp_xoffset, 0.5*FASPRO_width-0.2-0.5*faspConn_size[1], 
      ...
      TGeoVolume* vol_fpga_asic = new TGeoVolume("FPGA", fpga_asic, asicVolMed); vol_fpga_asic->SetLineColor(kBlack);                                                                        
      faspro->AddNode(vol_fpga_asic, 0, new TGeoTranslation("", 0, -fasp_yoffset, 
                              GETS_zpos+0.5*GETS_thickness+0.5*fpga_size[2]));
      faspro->AddNode(vol_fpga_asic, 1, new TGeoTranslation("", 0,  fasp_yoffset, 
                              GETS_zpos+0.5*GETS_thickness+0.5*fpga_size[2]));
    }
    ...
    for (Int_t iFeb(0); cFeb < 2; cFeb++) {
      vol_feb->AddNode(vol_faspro_fy, cFeb+1, 
          new TGeoTranslation("", (cFeb-0.5)*(FASPRO_length+FASPRO_dx), 0., -0.5*(FASPRO_thickness+FASPRO_zspace)));
    ...
    vol_feb->AddNode(vol_lv_bd, 0, new TGeoTranslation("",  0., 0.5+0.5*(activeAreaY-LVB_width), LVB_pos+FMCheight+0.5*LVB_thickness));
    vol_feb->AddNode(vol_lv_bd, 1, new TGeoTranslation("",  0., -0.5+0.5*(-activeAreaY+LVB_width), LVB_pos+FMCheight+0.5*LVB_thickness));
    ...
    if (IncludeRobs) {
      ...
      TGeoRotation *crob_fmc_rot = new TGeoRotation("crob_fmc_rot"); crob_fmc_rot->RotateZ(90.); crob_fmc_rot->RegisterYourself();
      TGeoTranslation *crob_fmc_tra = new TGeoTranslation("crob_fmc_tra", 0., 0.5*(FMCwidth-robConn_size_x)+0.2, 0.5*(FMCheight+rob_thickness)); crob_fmc_tra->RegisterYourself();
      TGeoHMatrix *crob_fmc_transform = new TGeoHMatrix(""); (*crob_fmc_transform)=(*crob_fmc_rot)*(*crob_fmc_tra);
      ...
      crob->AddNode(vol_crob_bd, 1, new TGeoTranslation("", -0.5*(rob_size_x-robConn_size_x)+1.5, 0., FMCheight+rob_thickness));
    ...
    // put FEB box on module
    module->AddNode(vol_feb, 1, new TGeoTranslation("", 0., 0., FASPRO_position));  // put febvolume at correct z position wrt to the module
  }
...
TGeoVolume* create_support_inner()
{
  ...
  TGeoVolume* vol_wall_fr4 = new TGeoVolume("wall_fr4", wall_fr4, frameVolMed); vol_wall_fr4->SetLineColor(kBlue-9);
  TGeoCompositeShape *wall_al = new TGeoCompositeShape("wall_al", "wall_al_b-wall_fr4");
  TGeoVolume* vol_wall_al = new TGeoVolume("wall_al", wall_al, aluminiumVolMed); vol_wall_al->SetLineColor(kGray);
...
//          gGeoMan->GetVolume(geoVersion)->AddNode(gModules[type - 1], copy, module_placement);
  ...
  for(Int_t ib(0); ib<4; ib++){
    layer->AddNode(inner, ib+1, new TGeoTranslation("", 0., (1.5-ib)*sizeY, 
        LayerPosition[layerId] + LayerThickness/2+ gasBu_position +0.5*gas_thickness+0.286+1.));
  }
}
...
//  power_1->AddNode(power1_vol, 4, power1_trans);
  ...
  power2     = new TGeoBBox("power2", powerbar_width /2., (9 * DetectorSizeY[0] + powerbar_width) /2., powerbar_thickness /2.);
