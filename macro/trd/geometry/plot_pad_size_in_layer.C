...
void plot_pad_size_in_layer(TString digiPar = "trd.v13/trd_v13g.digi.par", Int_t nlines = 1, Int_t nrows_in_sec = 0,
                            Int_t alllayers = 1)
{
  ...
  while (digipar.good()) {
    ...
    if (blockCounter == 19) firstModule = buffer;
    ...
    if (read) {
      ...
      if (startCounter == 23) {  // if last element is reached
        ...
        if (nrows_in_sec == 1)  // print number of rows in sector
        {
          ...
          title2.Form("%3.1f",
                      ps2X * ps2Y);  // print pad size - 2 digits - sector 2
          title3.Form("%3.1f",
                      ps3X * ps3Y);  // print pad size - 2 digits - sector 3
        }
