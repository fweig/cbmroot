...
void standalone_event_build(TString inFileName) 
{
  TString srcDir = gSystem->Getenv("VMCWORKDIR");
   ...
   if(fPsdDigis == nullptr) { cout<<"no psd digis vector in tree"<<endl; return; }
   ...
   th1_hist_ptr = new TH1F("Energy_profile", "PSD energy profile per event; cell # []; Energy [adc counts]", total_channels, 0, total_channels);
   th1_hist_ptr = new TH1F("Energy_distrib", "PSD energy distribution; Energy over event all channels [adc counts]; Counts []", 1000, 0, 10000);
   ...
   th1_hist_ptr = new TH1F("Fired_psd_chs", "Fired PSD channels in event; fired channels []; Counts []", total_cells, 0, total_cells);
   th1_hist_ptr = new TH1F("Cosmic_check", "Half a sum from two hodo channels; Energy []; Counts []", 100, 0, 100);

   th2_hist_ptr = new TH2F("PsdVsHodo", "Energy in Psd vs energy in Hodo; Psd energy [adc counts]; Hodo energy [adc counts]", 1000, 0, 2000, 140, 0, 70);

   for(UInt_t ch_iter = 0; ch_iter < total_channels; ch_iter++)
      th1_hist_ptr = new TH1F( Form("Energy_distrib_ch%i", ch_iter), Form("Energy_distrib_ch%i; Energy [adc counts]; Counts []", ch_iter), 100, 0, 2000);

   for(UInt_t ch_iter = 0; ch_iter < total_channels; ch_iter++)
      th1_hist_ptr = new TH1F( Form("Energy_distrib_good_ev_ch%i", ch_iter), Form("Energy_distrib_good_ev_ch%i; Energy [adc counts]; Counts []", ch_iter), 100, 0, 2000);
   ...
   for(UInt_t ev = 0; ev < inTree->GetEntries(); ev++)
   {
      ...
      if (ev % 10 == 0 ) std::cout  << "Entry " << ev << " / " << inTree->GetEntries() << " Psd Digis: " << nPsdDigis  << std::endl;
	 ...
	 if(IsNewEvent)
	 {
            if(IsFirstEvent) { IsFirstEvent = false; continue; } //skip info before first event
