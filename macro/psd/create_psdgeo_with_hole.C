...
void create_psdgeo_with_hole() {
  ...
  infoFile << "Side length of the square shaped hole in PSD center: "
           << holeSize << " cm" << endl
           << endl;
  // --------------------------------------------------------------------------
  ...
  for (Int_t iModX = 0; iModX < nModulesX; iModX++) {
    ...
    for (Int_t iModY = 0; iModY < nModulesY; iModY++) {
      ...
      if ((iModX == nModulesX / 2 || iModX == nModulesX / 2 - 1)
        ...
        psd->AddNode(ConstructModuleWithHole(Form("module2060_hole_%d", iHole),
                                             bigModuleSize,
                                             nLayers,
                                             &infoFile,
                                             holeSize,
                     ...
                     iModule,
                     trans);
        psd->AddNode(ConstructShield(Form("shield20_hole_%d", iHole),
                                     bigModuleSize,
                                     holeSize,
                                     iHole,
                                     &infoFile),
