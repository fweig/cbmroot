void check_overlaps(const char* dataset = "test")
{
  ...
  while ((ov=(TGeoOverlap*)next())) {
    ...
    else if (OverlapName.Contains("cave/magnet_container_0 overlapping cave/rich_v17a_3e_0/rich_container_288") ) {
        std::cout << "Expected Overlap between rich and magnet" << endl;
        std::cout << ov->GetTitle() << std::endl << std::endl;
    }
    else if (OverlapName.Contains("cave/magnet_container_0 overlapping cave/rich_v17a_1e_0/rich_container_290") ) {
        std::cout << "Expected Overlap between rich and magnet" << endl;
        std::cout << ov->GetTitle() << std::endl << std::endl;
    }
    else {
  ...
  while ((ov=(TGeoOverlap*)next1())) {
    ...
    else if ( OverlapName.Contains("magnet_container_0 overlapping rich_v17a_1e_0/rich_container_290") ) {
      std::cout << "Expected Overlap between rich and magnet" << endl;
      std::cout << ov->GetTitle() << std::endl << std::endl;
    }
    else if (OverlapName.Contains("magnet_container_0 overlapping rich_v17a_3e_0/rich_container_288") ) {
        std::cout << "Expected Overlap between rich and magnet" << endl;
        std::cout << ov->GetTitle() << std::endl << std::endl;
    }
