...
void anaFlow(Int_t mode    = 7,
  ...
  if (mode == 0) {
    ...
    TString infilename2 =
      dir + "sim/mc_" + numEvt + "evt_" + sfileNum + ".root";
    // Input file (reco events)
    TString infilename =
      dir + "reco/reco_" + numEvt + "evt_" + sfileNum + ".root";
    //param
    TString parFile =
      dir + "sim/params_" + numEvt + "evt_" + sfileNum + ".root";

    TString outfilename =
      dir + "ana/anaFlow_gen_" + numEvt + "evt_" + Bcut + sfileNum + ".root";
    ...
    TString infilename_gen = "/hera/cbm/prod/gen/" + sGen + "/auau/" + sEn
                             + "gev/mbias/urqmd.auau." + sEn + "gev.mbias."
                             + sfileNumL + ".root";
  ...
  if (mode == 1) {
    ...
    TString EPfilename_Qcorr =
      dir + "ana/ana_" + numEvt + "evt_" + sfileNum
      + "_RECOtracks_STScutY_lt0.8_Qcorr_FlatBin_FixStep.root";
    cout << "EPfilename_Qcorr: " << EPfilename_Qcorr << endl;

    // Output file
    if (mcreco == 0)
      TString outfilename = dir + "ana/anaFlow_reco_assMC_" + numEvt + "evt_"
                            + Bcut + sfileNum + ".root";
    if (mcreco == 1)
      TString outfilename = dir + "ana/anaFlow_reco_track_" + numEvt + "evt_"
                            + Bcut + sfileNum + ".root";
  ...
  if (mode == 2) {
    // Input file (reco events)
    TString infilename =
      dir + "reco/kfparticlefinder_" + numEvt + "evt_" + sfileNum + ".root";
    // Input file (MC events)
    TString infilename2 =
      dir + "sim/mc_" + numEvt + "evt_" + sfileNum + ".root";
    //param
    ...
    TString EPfilename_Qcorr =
      dir + "ana/ana_" + numEvt + "evt_" + sfileNum + "_Qcorr.root";
    cout << "EPfilename_Qcorr: " << EPfilename_Qcorr << endl;

    // Output file
    TString outfilename =
      dir + "ana/anaFlow_kfpart_" + numEvt + "evt_" + Bcut + sfileNum + ".root";
  ...
  if (mode == 3 || mode == 4 || mode == 5 || mode == 6 || mode == 7) {
    ...
    TString outfilename2 =
      dir + "ana/dummyOut_" + numEvt + "evt_" + sfileNum + ".root";
  ...
  if (mode == 3 || mode == 4) {
    if (mcreco == 0)
      TString file_tree = dir + "ana/anaFlow_reco_assMC_1000evt_" + sfileNum
                          + ".root";  // reco. or KFParticle finder output
    if (mcreco == 1)
      TString file_tree = dir + "ana/anaFlow_reco_track_1000evt_" + sfileNum
                          + ".root";  // reco. or KFParticle finder output
    TString file_tree_gen = dir + "ana/anaFlow_gen_1000evt_" + sfileNum
                            + ".root";  // particles at gen. level
    TString
      EPcorrFactor_Qcorr = dir
                           + "ana/"
                             "ana_1Mevt_RECOtracks_STScutY_lt0.8_Qcorr_FlatBin_"
                             "FixStep.root";  // correction for reaction plane resolution
    ...
    TString outfilename = dir + "ana/pion/v" + harmo + "_pion_" + sGenReco
                          + "_N_eff_vspty_" + Bcut
                          + "_mode4_EP_PSD2_effCorr.root";
    ...
    TString result_y = "result/au" + sEn + "au/pion/result_pion_mode4_cut6_"
                       + sGenReco + "_v" + harmo + "_y_" + Bcut + sEP
                       + "_30bins.txt";
    TString result_pt = "result/au" + sEn + "au/pion/result_pion_mode4_cut6_"
                        + sGenReco + "_v" + harmo + "_pt_" + Bcut + sEP
                        + "_30bins.txt";
    ...
    anaFlow->setQcut(
      "");  // particle charge: "positive"/"negative"/"neutral"/"charged", no cut if not set
    anaFlow->setS_BG(
      "signal");  // or "bg". Both included when not set (only effective with kfparticle output files for now).
    //anaFlow->setCalcFlowCoef_meth("phifit");    // method: sdt: "cosnphi", alternative: "phifit"
    if (genreco == 0)
      anaFlow
        ->useTrackatGen();  // use generated events directly: in which case file_tree = file_tree_gen -> file_tree not needed
    ...
    // Final Draw
    anaFlow->setdoInt_diffvn(
      2);  // 0: int. vn, 1: diff. vn (both f(y) & f(pt)), 2: both int. & diff. vn
  ...
  if (mode == 5) {
    ...
    if (genreco == 0)
      TString infilename =
        dir + "ana/anaFlow_gen_" + numEvt + "evt_" + sfileNum;
    if (genreco == 1)
      TString infilename =
        dir + "ana/anaFlow_reco_track_" + numEvt + "evt_" + sfileNum;

    if (genreco == 0)
      TString outfilename = dir + "ana/pion/anaFlow_pion_v" + harmo + "_gen_"
                            + numEvt + "evt_" + Bcut + "_" + sfileNum;
    if (genreco == 1)
      TString outfilename = dir + "ana/pion/anaFlow_pion_v" + harmo
                            + "_reco_track_" + numEvt + "evt_" + Bcut + "_"
                            + sfileNum;

    infilename += ".root";
    if (genreco == 0) outfilename += "_2Dhistos_mode567_bin300-400.root";
    if (genreco == 1)
      outfilename += "_2Dhistos_mode567_bin300-400_EP_PSD2.root";

    cout << "infilenme: " << infilename << endl;
    cout << "outfilename: " << outfilename << endl;

    // STS centrality
    TString EPcorrFactor_Qcorr =
      dir + "ana/ana_1Mevt_RECOtracks_STScutY_lt0.8_Qcorr_FlatBin_FixStep.root";
  ...
  if (mode == 6) {
    ...
    if (genreco == 0)
      TString outfilename = dir + "ana/pion/anaFlow_pion_v" + harmo
                            + "_gen_1Mevt_" + Bcut
                            + "_2Dhistos_mode567_bin300-400.root";
    if (genreco == 1)
      TString outfilename = dir + "ana/pion/anaFlow_pion_v" + harmo
                            + "_reco_track_1Mevt_" + Bcut
                            + "_2Dhistos_mode567_bin300-400_EP_PSD2.root";
  ...
  if (mode == 7) {
    ...
    TString file_2Dmap = dir + "ana/pion/anaFlow_pion_v" + harmo
                         + "_reco_track_1Mevt_" + Bcut
                         + "_2Dhistos_mode567_bin300-400_EP_PSD2.root";
    TString file_2Dmap_gen = dir + "ana/pion/anaFlow_pion_v" + harmo
                             + "_gen_1Mevt_" + Bcut
                             + "_2Dhistos_mode567_bin300-400.root";
    TString EPcorrFactor_Qcorr =
      dir + "ana/ana_1Mevt_RECOtracks_STScutY_lt0.8_Qcorr_FlatBin_FixStep.root";
    TString outfilename = dir + "ana/pion/eff_pion_v" + harmo + "_pion_"
                          + sGenReco + "_vspty_1Mevt_" + Bcut
                          + "_mode567_bin300-400.root";
    ...
    TString result_y =
      "result/au" + sEn + "au/pion/result_pion_mode567_bin300-400_cut9_"
      + sGenReco + "_v" + harmo + "_y_1Mevt_" + Bcut + sEP + "_30bins.txt";
    TString result_pt =
      "result/au" + sEn + "au/pion/result_pion_mode567_bin300-400_cut9_"
      + sGenReco + "_v" + harmo + "_pt_1Mevt_" + Bcut + sEP + "_30bins.txt";
    ...
    anaFlow->SetFileName_ascii(result_y, result_pt);
    cout << "-------- Ascii output file: " << result_y << ", " << result_pt
         << endl;
    ...
    // == EP corr
    anaFlow->setEPmeth(
      "EP_PSD2");  // if useTrackatGen(): only RP; if "RP", 2 lines below have no effect
    anaFlow->setdoEPCorr(kTRUE);
    anaFlow->setEPsubevent(kTRUE);

    // == Final Draw
    anaFlow->setdoInt_diffvn(
      1);  // 0: int. vn, 1: diff. vn (both f(y) & f(pt)), 2: both int. & diff. vn
