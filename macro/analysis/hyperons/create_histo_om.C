...
void create_histo_om(Int_t file_nr = 0)
{
  ...
  Double_t m0        = 1.67245;
  Double_t dm        = 0.0020;
  Double_t cutChi1   = 0.00;
  Double_t cutChi2   = 4.60;
  Double_t cutB1     = 0.16;
  Double_t cutB2     = 0.00;
  Double_t cutChi    = 3.5;
  Double_t cutDca    = 0.044;
  Double_t cutPca    = 2.60;
  Double_t cutBmo    = 0.07;
  ...
  t3->SetBranchAddress("CascadeCandidates", &fV0Tracks);
  ...
  TH2D* hBgdB1Chi2 = new TH2D("hBgdB1Chi2", title, nBins, 0., b1Max, nBins, 0., chi2Max);
  TH2D* hSigB1Chi2 = new TH2D("hSigB1Chi2", title, nBins, 0., b1Max, nBins, 0., chi2Max);
  ...
  TH1D* hBgdB1Chi2M   = new TH1D("hBgdB1Chi2M", title, 50, mMin, mMax);
  TH1D* hSigB1Chi2M   = new TH1D("hSigB1Chi2M", title, 50, mMin, mMax);
  ...
  for (Int_t iEv = 0; iEv < nEvents; iEv++) {
    ...
    for (Int_t i = 0; i < fV0Tracks->GetEntriesFast(); i++) {
      ...
      if (sig) hSigB1Chi2->Fill(b1, chi2);
      else
        hBgdB1Chi2->Fill(b1, chi2);

      if (!(b1 > cutB1 && chi2 > cutChi2)) continue;
      /*
