		...
		TString fit_type="expo")
{
 ...
 TString name= dir + "/parameters/much/" 
                   + ACCfile;
 ...
 TH2D *accYPt  = (TH2D*)plutoYPt->Clone("accYPt"); accYPt->SetTitle("acceptance");
 TH2D *effYPt  = (TH2D*)plutoYPt->Clone("effYPt"); effYPt->SetTitle("efficiency");
 TH2D *recoYPt = (TH2D*)plutoYPt->Clone("recoYPt");recoYPt->SetTitle("reconstructed #omega");
 TH2D *corrYPt = (TH2D*)plutoYPt->Clone("corrYPt");corrYPt->SetTitle("corrected #omega");
 ...
 //-------------------------------
 for(int i=1; i<NofBinsX; i++){for(int j=1; j<NofBinsY; j++){
 ...
 else accYPt->SetBinContent(i,j,0);
     
 name.Form("Y[%2.2f,%2.2f], Pt[%2.2f,%2.2f]",(Double_t)(i-1)*Xbin+Xmin,(Double_t)i*Xbin+Xmin,(Double_t)(j-1)*Ybin+Ymin,(Double_t)j*Ybin+Ymin);
