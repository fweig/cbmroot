...
void InvariantMassSpectra_mix(Int_t energy=8, TString type="invM_mix", Int_t NofFiles=5000, Int_t NofEvents=1e8, Int_t index=0)
{
 ...
 while(events<NofEvents)
    {
     ...
     name.Form("/lustre/cbm/prod/mc/OCT19/sis100_muon_lmvm/%dgev/centr/%d/muons.ana.root",energy,k1);
     
     TFile *f1 = new TFile(name);
     if (f1->IsZombie() || f1->GetNkeys() < 1 || f1->TestBit(TFile::kRecovered)){f1->Close();continue;}
     
     name.Form("/lustre/cbm/prod/mc/OCT19/sis100_muon_lmvm/%dgev/centr/%d/muons.ana.root",energy,k2);
     
     TFile *f2 = new TFile(name);
     if (f2->IsZombie() || f2->GetNkeys() < 1 || f2->TestBit(TFile::kRecovered)){f2->Close();continue;}

     if(events % 1000000 ==0)cout << events/1000000 << "e6" <<  endl;

          
     InputTree1 = (TTree*)f1->Get("cbmsim");  
     if(!InputTree1){f1->Close();continue;}
     
     InputTree1->SetBranchAddress("MuPlus",  &MuPlus);
         
     InputTree2 = (TTree*)f2->Get("cbmsim");  
     if(!InputTree2){f2->Close();continue;}
