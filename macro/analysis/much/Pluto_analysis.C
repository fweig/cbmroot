...
void Pluto_analysis(int part=0, int energy=8, int NofFiles=5000)
{
 ...
 if(part==7){cktName.Form("charm");energy=10;}
 ...
 Double_t partMulti[] = {multRho*brRho, multOmega*brOmega, multOmegaD*brOmegaD, multPhi*brPhi, multEta*brEta, multEtaD*brEtaD, multQGP, multJPsi*brJPsi};
...
//----------------------------------
	...
	TH3D *YPtM_part = new TH3D(histoName.Data(),histoName.Data(), NofBins,minY,maxY, NofBins,min,maxPt, NofBinsM,min,maxM);
...
for(int i=1; i<NofFiles+1; i++)
{
  ...
  fileName.Form("/lustre/cbm/prod/gen/pluto/auau/%s/%dgev/%s/%s/pluto.auau.%dgev.%s.%s.%s.root",cktName.Data(),energy,partpartName.Data(),chanName.Data(),energy,partpartName.Data(),chanName.Data(),str);
