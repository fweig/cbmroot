...
void plot_sts_big()
{
  ...
  TH1F* H_mom_rec_all  = new TH1F("h_mom_rec_all", "", 40, 0., 10.);
  TH1F* H_mom_acc_prim = new TH1F("h_mom_acc_prim", "", 40, 0., 10.);
  TH1F* H_mom_rec_prim = new TH1F("h_mom_rec_prim", "", 40, 0., 10.);
...
urqmd.auau.25gev.centr.%4d.recoSts.root",
  ...
  h_eff_all->Sumw2();
  h_eff_all->Divide(H_mom_rec_all, H_mom_acc_all, 1, 1, "B");
  TH1F* h_eff_prim = new TH1F("h_eff_prim", "", 40, 0., 10.);
  h_eff_prim->Sumw2();
  h_eff_prim->Divide(H_mom_rec_prim, H_mom_acc_prim, 1, 1, "B");
  Style(h_eff_all, "p (GeV/c)", "efficiency");
  h_eff_all->GetYaxis()->SetRangeUser(0., 1.);
  h_eff_all->SetMarkerStyle(20);
  h_eff_all->SetMarkerColor(2);
