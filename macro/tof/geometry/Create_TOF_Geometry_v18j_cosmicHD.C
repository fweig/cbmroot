...
const Float_t Module_Size_Z[NofModuleTypes] = {11., 11., 13., 11., 11., 11.};
...
void Create_TOF_Geometry_v18j_cosmicHD() {
...
void position_inner_tof_modules(Int_t modNType) {
  ...
  module_trans = new TGeoTranslation("", xPos, 0, zPos + 11.8);
  gGeoMan->GetVolume(geoVersion)
    ->AddNode(gModules[modType], modNum, module_trans);
  modNum++;

  // module_trans = new TGeoTranslation("", xPos, 49+3, zPos);
  module_trans = new TGeoTranslation("", xPos, 0, zPos + 11.8 + 19);
  gGeoMan->GetVolume(geoVersion)
    ->AddNode(gModules[modType], modNum, module_trans);
  modNum++;

  // module_trans = new TGeoTranslation("", xPos,-26, zPos+Module_Size_Z[modType]);
  module_trans = new TGeoTranslation("", xPos, 0, zPos + 11.8 + 19 + 20.2);
  gGeoMan->GetVolume(geoVersion)
    ->AddNode(gModules[modType], modNum, module_trans);
  modNum++;
  ...
  // module_trans = new TGeoTranslation("", xPos, 26, zPos+Module_Size_Z[modType]);
  module_trans =
    new TGeoTranslation("", xPos, 0, zPos + 11.8 + 19 + 20.2 + 19.8);
  gGeoMan->GetVolume(geoVersion)
    ->AddNode(gModules[modType], modNum, module_trans);
  modNum++;
}
