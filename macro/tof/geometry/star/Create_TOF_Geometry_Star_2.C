...
const Float_t Wall_Z_Position = 489.01;  // -240, positiv for testing in CBMROOT
//const Float_t Wall_Z_Position = 279.01;  // target position for collider mode
...
const Float_t Module_Size_X[NofModuleTypes] = {102.15};
const Float_t Module_Size_Y[NofModuleTypes] = {48.3};
const Float_t Module_Size_Z[NofModuleTypes] = {11.1};
...
const Float_t CounterXStartPosition[NofModuleTypes] = {-27.74};
const Float_t CounterXDistance[NofModuleTypes]      = {31.257};
const Float_t CounterZDistance[NofModuleTypes]      = {4.0332};
const Float_t CounterRotationAngle[NofModuleTypes]  = {8.7};

// Position for module placement
const Float_t Module_X_Position     = 158.35;
...
void Create_TOF_Geometry_Star_2()
{
...
TGeoVolume* create_counter(Int_t modType)
{
  ...
  //new TGeoVolume("tof_glass", glass_plate, noActiveGasVolMed);
...
void position_tof_modules(Int_t modNType)
{
  ...
  for (Int_t j = 0; j < modNType; j++) {
    ...
    for (Int_t i = 0; i < Module_Number[j]; i++) {
      ...
      if (i == 12 || i == 24) {
        xPos   = Module_X_Position + Ring_dX_Position[iRing];
        yPos   = Module_Y_Position + Ring_dY_Position[iRing];
        zPos   = Module_Z_Position + Ring_dZ_Position[iRing];
        Radius = TMath::Sqrt(xPos * xPos + yPos * yPos);
      }
