...
const Float_t Glass_X[NumberOfDifferentCounterTypes] =
  {32., 52., 15.04, 18., 2., 0.2, 26.816, 32., 17.4, 25.};
const Float_t Glass_Y[NumberOfDifferentCounterTypes] =
  {26.9, 53., 4., 4.6, 10., 0.2, 20.0, 27., 2.0, 26.7};
const Float_t Glass_Z[NumberOfDifferentCounterTypes] =
  {0.1, 0.1, 0.1, 0.1, 0.01, 0.01, 0.1, 0.1, 0.07, 0.1};
...
const Float_t CounterXDistance[NofModuleTypes] =
  {30.0, 30.6, 0.0, 30.0, 0.0, 0., 0., 0., 0., 0.};
...
const Float_t CounterZDistance[NofModuleTypes] =
  {0.0, 0.0, 0.0, 2.5, 0.0, 0., 0., 0., 4.0, 0.};
...
const Float_t Inner_Module_Number[Inner_Module_NTypes] = {
  1.,
  2.,
  0.};  //debugging, V14b
...
const Float_t InnerSide_Module_Number[Inner_Module_NTypes] = {
  0.};  //debug, V14b
...
TGeoVolume* create_new_tof_module(Int_t modType) {
  ...
  */
  ...
  for (Int_t j = 0; j < NCounterInModule[modType];
    ...
    TGeoTranslation* counter_trans = new TGeoTranslation(
      "", startxpos + j * dxpos, startypos + j * dypos, zpos);
    TGeoRotation* counter_rot = new TGeoRotation();
    counter_rot->RotateY(rotangle);
    TGeoCombiTrans* counter_combi_trans =
      new TGeoCombiTrans(*counter_trans, *counter_rot);
...
TGeoVolume* create_tof_pole() {
  ...
  TGeoBBox* pole_air_box = new TGeoBBox(
...
void position_tof_poles(Int_t modType) {
  ...
  for (Int_t i = 0; i < NumberOfPoles; i++) {
    if (i < 2) {
      ...
      Float_t xPos = Pole_Offset + Pole_Size_X / 2.
                     + DxColl[(Int_t) Pole_Col[i]
                              - 1];  // add x offset of this module column
      for (Int_t jj = 0; jj < Pole_Col[i] - 1;
           jj++)  // add X offset of previous modules columns
        xPos += DxColl[jj];
  ...
  for (Int_t j = 0; j < modNType; j++) {
    ...
    for (Int_t i = 0; i < Inner_Module_Number[j]; i++) {
      ...
      if (kTRUE) {
        ...
        cout << "Position Inner Module " << i << " of "
             << Inner_Module_Number[j] << " Type " << modType
    ...
    */
        ...
        if (ii > 0) {
          ...
          yPos = GetShiftedY(fPrevModPosY,
                             iPrevModType,
                             fPrevModPosZ,
                             modType,
                             zPos + Module_Size_Z[modType],
                             kfAngOverlap);

          cout << "Position Inner Module " << i << " of "
               << Inner_Module_Number[j] << " Type " << modType
               << " at X = " << xPos << " Y = " << yPos
               << " Z = " << zPos + Module_Size_Z[modType]
               << " Ysize = " << Module_Size_Y[modType]
               << " Back, Diff to last " << yPos - fPrevModPosY << " Ov. "
               << ((Module_Size_Y[iPrevModType] + Module_Size_Y[modType]) / 2.0
                   - (yPos - fPrevModPosY))
               << endl;
          ...
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 2, module_trans);
          //         modNum++;
          // Back staggered module, bottom
          module_trans =
            new TGeoTranslation("", xPos, -yPos, zPos + Module_Size_Z[modType]);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 3, module_trans);
          //         modNum++;
          fPrevModPosY = yPos;
          ...
          module_trans = new TGeoTranslation("", xPos, -yPos, zPos);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 1, module_trans);
          //       modNum++;
          fPrevModPosY = yPos;
          iPrevModType = modType;
          fPrevModPosZ = zPos;
          ...
          cout << "Position Inner Module " << i << " of "
               << Inner_Module_Number[j] << " Type " << modType
               << " at X = " << xPos << " Y = " << yPos << " Z = " << zPos
               << " Ysize = " << Module_Size_Y[modType] << ", Diff to last "
               << yPos - fPrevModPosY << " Ov. "
               << ((Module_Size_Y[iPrevModType] + Module_Size_Y[modType]) / 2.0
                   - (yPos - fPrevModPosY))
               << endl;
...
void position_pla_modules(Int_t modNType) {
  ...
  for (Int_t j = 0; j < modNType; j++) {
    ...
    for (Int_t i = 0; i < Pla_Number[j]; i++) {
      ...
      cout << "Position Pla Module " << i << " of " << Pla_Number[j] << " Type "
           << modType << " at X = " << xPos << ", Y = " << yPos
           << ", Z = " << zPos << endl;
      // Front staggered module (Top if pair), top
      module_trans       = new TGeoTranslation("", xPos, yPos, zPos);
      module_combi_trans = new TGeoCombiTrans(*module_trans, *module_rot);
      ...
      if (kTRUE == bNhVersion) {
        ...
        if (ii > 1) {
          ...
          module_trans =
            new TGeoTranslation("", xPos, yPos, zPos + Module_Size_Z[modType]);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 4, module_trans);
          //          modNum++;
          ...
          module_combi_trans = new TGeoCombiTrans(*module_trans, *module_rot);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 5, module_combi_trans);
          //          modNum++;
          ...
          module_trans =
            new TGeoTranslation("", xPos, -yPos, zPos + Module_Size_Z[modType]);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 6, module_trans);
          //          modNum++;

          // Back staggered module bottom in pair, bottom left
          module_trans = new TGeoTranslation(
            "", -xPos, -yPos, zPos + Module_Size_Z[modType]);
          module_combi_trans = new TGeoCombiTrans(*module_trans, *module_rot);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 7, module_combi_trans);
          //          modNum++;
          fPrevModPosY = yPos;
          iPrevModType = modType;
          fPrevModPosZ = zPos + Module_Size_Z[modType];
          ...
          module_combi_trans = new TGeoCombiTrans(*module_trans, *module_rot);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 1, module_combi_trans);
          //          modNum++;
          ...
          module_combi_trans = new TGeoCombiTrans(*module_trans, *module_rot);
          gGeoMan->GetVolume(geoVersion)
            ->AddNode(gModules[modType], modNum + 3, module_combi_trans);
          //          modNum++;
          fPrevModPosY = yPos;
          iPrevModType = modType;
          fPrevModPosZ = zPos;
...
void position_outer_tof_modules(Int_t nCol)  //modType, Int_t col1, Int_t col2)
{
  ...
  for (Int_t j = 0; j < nCol; j++) {
    ...
    Float_t xPos = Outer_Module_X_Offset
                   + DxColl[j];  // // add x offset of this module column
    for (Int_t jj = 0; jj < j;
         jj++)  // add X offset of previous modules columns
      xPos += DxColl[jj];
    Last_Size_Y   = 0.;
        ...
        if (kTRUE) {
          ...
          if (ii > 1) {
            ...
            yPos = GetShiftedY(fPrevModPosY,
                               ...
                               zPos + DzPos,
                               kfAngOverlap);
            cout << "Position Outer Module Module " << i << " of "
                 << Outer_Module_Number[k][j] << " Type " << modType << "(#"
                 << modNum[modType] << ") "
                 << " at X = " << xPos << " Y = " << yPos
                 << " Z = " << zPos + DzPos
                 << " Ysize = " << Module_Size_Y[modType]
                 << " Back, Diff to last " << yPos - fPrevModPosY << " Ov. "
                 << ((Module_Size_Y[iPrevModType] + Module_Size_Y[modType])
                       / 2.0
                     - (yPos - fPrevModPosY))
                 << endl;
            ...
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(gModules[modType], modNum[modType] + 4, module_trans);
            //            modNum[modType]++;
            ...
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(
                gModules[modType], modNum[modType] + 5, module_combi_trans);
            //            modNum[modType]++;

            // Back staggered module, bottom right, bottom in staggering pair
            module_trans = new TGeoTranslation("", xPos, -yPos, zPos + DzPos);
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(gModules[modType], modNum[modType] + 6, module_trans);
            //            modNum[modType]++;
            ...
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(
                gModules[modType], modNum[modType] + 7, module_combi_trans);
            //            modNum[modType]++;
            fPrevModPosY = yPos;
            iPrevModType = modType;
            ...
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(
                gModules[modType], modNum[modType] + 1, module_combi_trans);
            //            modNum[modType]++;

            // Front staggered module, bottom right, Top in staggering pair
            module_trans = new TGeoTranslation("", xPos, -yPos, zPos);
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(gModules[modType], modNum[modType] + 2, module_trans);
            //            modNum[modType]++;
            ...
            gGeoMan->GetVolume(geoVersion)
              ->AddNode(
                gModules[modType], modNum[modType] + 3, module_combi_trans);
            //            modNum[modType]++;
            fPrevModPosY = yPos;
            iPrevModType = modType;
            fPrevModPosZ = zPos + DzPos;
...
Float_t GetShiftedY(Double_t dPrevModPosY,
                    Int_t iPrevModType,
                    Double_t dPrevModPosZ,
                    Int_t iModType,
                    Double_t dModPosZ,
  ...
  Double_t dModThetaY =
    TMath::ATan2((dPrevModPosY + dPrevModSizeY / 2.0 - dPrevModDeadY / 2.0),
