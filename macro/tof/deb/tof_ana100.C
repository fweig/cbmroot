tof_ana100(Int_t nEvents = 1000, Int_t iout = 1)
{
  ...
  TH2F* cdrp_b_gen = new TH2F("cdrp_b_gen", "reaction plane resolution;impact parameter b; cos#Delta#phi_{rp};", 15, 0.,
                              15., 20, 0., 1.);
  TH2F* drp_b_gen =
    new TH2F("drp_b_gen", "#Delta#phi-reaction plane;impact parameter b;", 15, 0., 15., 36, -180., 180.);
  ...
  TH2F* cdrp_b_poi = new TH2F("cdrp_b_poi", "Poi reaction plane resolution;impact parameter b; cos#Delta#phi_{rp};", 15,
                              0., 15., 20, 0., 1.);
  TH2F* drp_b_poi =
    new TH2F("drp_b_poi", "Poi-reaction plane;impact parameter b; #Delta#phi;", 15, 0., 15., 36, -180., 180.);
  ...
  TH2F* cdrp_b_hit = new TH2F("cdrp_b_hit", "TofHit reaction plane resolution;impact parameter b; cos#Delta#phi_{rp};",
                              15, 0., 15., 20, 0., 1.);
  TH2F* drp_b_hit =
    new TH2F("drp_b_hit", "TofHit-reaction plane;impact parameter b; #Delta#phi;", 15, 0., 15., 36, -180., 180.);
  ...
  TH2F* pv_gloprim =
    new TH2F("pv_gloprim", "Global primary tracks; velocity(cm/ns); momentum(GeV/c);", 100, 0., 32., 100., 0., 4.);
  TH2F* tm_gloprim =
    new TH2F("tm_gloprim", "Global primary tracks; momentum (GeV/c); mass;", 100, 0., 10., 100., 0., 2.5);
  TH2F* tm_glop_prim =
    new TH2F("tm_glop_prim", "Global primary proton tracks; momentum (GeV/c); mass;", 100, 0., 10., 100., 0., 2.5);
  ...
  TH2F* cdrp_b_glo = new TH2F("cdrp_b_glo", "GloHit reaction plane resolution;impact parameter b; cos #Delta#phi_{rp};",
                              15, 0., 15., 20, 0., 1.);
  TH2F* drp_b_glo =
    new TH2F("drp_b_glo", "Glo-reaction plane;impact parameter b; #Delta#phi; ", 15, 0., 15., 36, -180., 180.);
  // var
  ...
  cout << "total number of events to process: " << nevent << ", wxy2=" << wxy2 << ", rxy2=" << rxy2 << endl;
      ...
      if (TrackP[k] == 0) {  // for efficiency
        ...
        if (MCTrack->GetMotherId() == -1) {
          ...
          if (tofmass > 0.98) {
            ...
            cout << "Bad primary TofPoint from MCtrack " << k << " pid " << pid << ", M " << tofmass << ", DetID "
                 << TofPoint->GetDetectorID() << ", SM " << SMtype_poi << "," << mod_poi << "," << cel_poi << ","
    ...
    for (Int_t i = 0; i < nGlobTracks; i++) {  // inspect global tracks
      ...
      if (j > 0) {
        ...
        if (MCTrack->GetMotherId() == -1) {  // select primaries
          ...
          if (pid == 321) { ptm_rap_glo_kp->Fill(MCTrack->GetRapidity(), MCTrack->GetPt() / MCTrack->GetMass()); }
          if (pid == -321) { ptm_rap_glo_km->Fill(MCTrack->GetRapidity(), MCTrack->GetPt() / MCTrack->GetMass()); }
          if (pid == 2212) {  //protons
