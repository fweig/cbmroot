void pl_all_cluWalkTrg(Int_t iNch = 1,
                       Int_t i1   = 0,
                       Int_t iTrg = 0,
                       Int_t SmT  = 0,
                       Int_t iSm  = 0) {
  ...
  for (Int_t iCh = i1; iCh < i1 + iNch / 2; iCh++) {
    ...
    gROOT->cd();
    TString hname = Form(
      "cl_SmT%d_sm%03d_rpc000_Ch%03d_S0_Trg%02d_Walk", SmT, iSm, iCh, iTrg);
    ...
    TString hname =
      Form("Cor_SmT%d_sm%03d_rpc000_Ch%03d_S0_Walk_px", SmT, iSm, iCh);
    ...
    gROOT->cd();
    TString hname = Form(
      "cl_SmT%d_sm%03d_rpc000_Ch%03d_S1_Trg%02d_Walk", SmT, iSm, iCh, iTrg);
    h2 = (TH2*) gROOT->FindObjectAny(hname);
    if (h2 != NULL) {
      h2->UseCurrentStyle();
      h2->GetYaxis()->SetLabelSize(lsize);
    ...
    TString hname =
      Form("Cor_SmT%d_sm%03d_rpc000_Ch%03d_S1_Walk_px", SmT, iSm, iCh);
    h1 = (TH1*) gROOT->FindObjectAny(hname);
    if (h1 != NULL) {
      h1->Draw("same");
    } else {
