...
void pl_all_cluWalkTrg(Int_t iNch = 1, Int_t i1 = 0, Int_t iTrg = 0, Int_t SmT = 0, Int_t iSm = 0)
{
  ...
  for (Int_t iCh = i1; iCh < i1 + iNch / 2; iCh++) {
    can->cd(++ic);
    gROOT->cd();
    TString hname = Form("cl_SmT%d_sm%03d_rpc000_Ch%03d_S0_Trg%02d_Walk", SmT, iSm, iCh, iTrg);
    ...
    can->cd(++ic);
    gROOT->cd();
    TString hname = Form("cl_SmT%d_sm%03d_rpc000_Ch%03d_S1_Trg%02d_Walk", SmT, iSm, iCh, iTrg);
    h2            = (TH2*) gROOT->FindObjectAny(hname);
    if (h2 != NULL) {
      h2->UseCurrentStyle();
      h2->GetYaxis()->SetLabelSize(lsize);
