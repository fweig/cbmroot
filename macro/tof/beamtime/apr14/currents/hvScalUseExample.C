...
Bool_t hvScalUseExample(Int_t iFirstRPC = 0, Int_t iSecondRpc = 2, Int_t iScalerIndex = 3, Double_t dScalerArea = -1)
{
  ...
  TH1* hCurrentEvoNegA =
    new TH1D("hCurrentEvoNegA",
             Form("Current evolution for the Negative HV of %s; Time [s]; Current [uA]", sChNameRpc[iFirstRPC].Data()),
             2 * iTimeIntervalsec, 0, iTimeIntervalsec);
  hCurrentEvoNegA->SetLineColor(kBlue);
  TH1* hCurrentEvoPosA =
    new TH1D("hCurrentEvoPosA",
             Form("Current evolution for the Positive HV of %s; Time [s]; Current [uA]", sChNameRpc[iFirstRPC].Data()),
             2 * iTimeIntervalsec, 0, iTimeIntervalsec);
  hCurrentEvoPosA->SetLineColor(kRed);

  TH1* hCurrentEvoNegB =
    new TH1D("hCurrentEvoNegB",
             Form("Current evolution for the Negative HV of %s; Time [s]; Current [uA]", sChNameRpc[iSecondRpc].Data()),
             2 * iTimeIntervalsec, 0, iTimeIntervalsec);
  hCurrentEvoNegB->SetLineColor(kBlue);
  TH1* hCurrentEvoPosB =
    new TH1D("hCurrentEvoPosB",
             Form("Current evolution for the Positive HV of %s; Time [s]; Current [uA]", sChNameRpc[iSecondRpc].Data()),
             2 * iTimeIntervalsec, 0, iTimeIntervalsec);
...
*/
  ...
  cout << " Best Match in time with first MBS event found for HV point #" << uHvStartPoint
       << " : DiffT_A_p = " << dTimeDistMbsPrevA << "   DiffT_B_p = " << dTimeDistMbsPrevB
       << " - DiffT_A_l = " << dTimeDistMbsLastA << "   DiffT_B_l = " << dTimeDistMbsLastB << endl;
  ...
  cout << " Best Match in time with last MBS event found for HV point  #" << uHvStopPoint
       << " : DiffT_A_p = " << dTimeDistLastMbsPrevA << "   DiffT_B_p = " << dTimeDistLastMbsPrevB
       << " - DiffT_A_l = " << dTimeDistLastMbsLastA << "   DiffT_B_l = " << dTimeDistLastMbsLastB << endl;
  ...
  hRateCurrCompA->Fill(dRateKhzCm, tRpcHvEvtA.dCurrentNeg + tRpcHvEvtA.dCurrentPos);
  hRateCurrCompB->Fill(dRateKhzCm, tRpcHvEvtB.dCurrentNeg + tRpcHvEvtB.dCurrentPos);
  ...
  for (UInt_t uHvPoint = uHvStartPoint + 1; uHvPoint < uHvStopPoint; uHvPoint++) {
    ...
    tTimeCurrMbsEvent.SetSec(fTriglogBoard->GetMbsTimeSec());
    tTimeCurrMbsEvent.SetNanoSec(fTriglogBoard->GetMbsTimeMilliSec() * 1000000);
    Double_t dCurrMbsTime = (Double_t)(tTimeCurrMbsEvent.GetSec()) + (Double_t)(tTimeCurrMbsEvent.GetNanoSec()) / 1e9;
...
*/
  ...
  tInputTreeScal->GetEntry(uNTreeEntriesScal - 1);
  tInputTreeHv->GetEntry(uHvStopPoint);
  Double_t dLastMbsTime = (Double_t)(tTimeLastMbsEvent.GetSec()) + (Double_t)(tTimeLastMbsEvent.GetNanoSec()) / 1e9;
  dRateKhzCm            = fCalTrloBoard->GetScalerValue(iScalerIndex) / 1000.0 / dScalerArea;
...
*/
  ...
  cout << "Comparison histos: " << hRateCurrCompA->GetEntries() << " " << hRateCurrCompB->GetEntries() << endl;
