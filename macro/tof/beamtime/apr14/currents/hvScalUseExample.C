...
Bool_t hvScalUseExample(Int_t iFirstRPC      = 0,
                        Int_t iSecondRpc     = 2,
                        Int_t iScalerIndex   = 3,
                        Double_t dScalerArea = -1) {
  ...
  if (kiNbRpc <= iFirstRPC || kiNbRpc <= iSecondRpc) {
    cout << " One of the RPC indices is out of bound, values should be between "
            "0 and "
         << kiNbRpc - 1 << endl;
    return kFALSE;
  }
  ...
  cout << uNTreeEntriesHv << " entries in HV tree and " << uNTreeEntriesScal
       << " entries in scalers tree" << endl;
  ...
  TH1* hRateEvoScal =
    new TH1D("hRateEvoScal",
             Form("Scaler rate evolution for the input scaler #%d in TRIGLOG; "
                  "Time [s]; Rate [kHz/cm2]",
                  iScalerIndex),
             2 * iTimeIntervalsec,
             0,
...
*/
  TH2* hRateCurrCompA =
    new TH2D("hRateCurrCompA",
             Form("Comparison of scaler #%d rate and RPC %s current; Rate "
                  "[kHz/cm2]; Total current [uA]",
                  iScalerIndex,
                  sChNameRpc[iFirstRPC].Data()),
             1000,
             0,
  ...
  for (uHvStartPoint = 0; uHvStartPoint < uNTreeEntriesHv; uHvStartPoint++) {
    ...
    dTimeDistMbsLastA = (Double_t) tRpcHvEvtA.iTimeSec
                        + (Double_t) tRpcHvEvtA.iTimeMilliSec / 1000.0
                        - (Double_t)(tTimeFirstMbsEvent.GetSec())
                        - (Double_t)(tTimeFirstMbsEvent.GetNanoSec()) / 1e9;

    dTimeDistMbsLastB = (Double_t) tRpcHvEvtB.iTimeSec
                        + (Double_t) tRpcHvEvtB.iTimeMilliSec / 1000.0
                        - (Double_t)(tTimeFirstMbsEvent.GetSec())
                        - (Double_t)(tTimeFirstMbsEvent.GetNanoSec()) / 1e9;

    if (0 < dTimeDistMbsLastA && 0 < dTimeDistMbsLastB) break;
  ...
  cout << " Best Match in time with first MBS event found for HV point #"
       << uHvStartPoint << " : DiffT_A_p = " << dTimeDistMbsPrevA
       << "   DiffT_B_p = " << dTimeDistMbsPrevB
       << " - DiffT_A_l = " << dTimeDistMbsLastA
       << "   DiffT_B_l = " << dTimeDistMbsLastB << endl;
  ...
  for (uHvStopPoint = 0; uHvStopPoint < uNTreeEntriesHv; uHvStopPoint++) {
    ...
    dTimeDistLastMbsLastA = (Double_t) tRpcHvEvtA.iTimeSec
                            + (Double_t) tRpcHvEvtA.iTimeMilliSec / 1000.0
                            - (Double_t)(tTimeLastMbsEvent.GetSec())
                            - (Double_t)(tTimeLastMbsEvent.GetNanoSec()) / 1e9;

    dTimeDistLastMbsLastB = (Double_t) tRpcHvEvtB.iTimeSec
                            + (Double_t) tRpcHvEvtB.iTimeMilliSec / 1000.0
                            - (Double_t)(tTimeLastMbsEvent.GetSec())
                            - (Double_t)(tTimeLastMbsEvent.GetNanoSec()) / 1e9;

    if (0 < dTimeDistLastMbsLastA && 0 < dTimeDistLastMbsLastB) break;
  }  // for( UInt_t uHvPoint = 0; uHvPoint < uNTreeEntriesHv; uHvPoint++)
  ...
  cout << " Best Match in time with last MBS event found for HV point  #"
       << uHvStopPoint << " : DiffT_A_p = " << dTimeDistLastMbsPrevA
       << "   DiffT_B_p = " << dTimeDistLastMbsPrevB
       << " - DiffT_A_l = " << dTimeDistLastMbsLastA
       << "   DiffT_B_l = " << dTimeDistLastMbsLastB << endl;
  ...
  Int_t iNbHvPoint               = 0;
  for (UInt_t uHvPoint = uHvStartPoint + 1; uHvPoint < uHvStopPoint;
       uHvPoint++) {
    ...
    for (uMbsEvtIndex = uLastBestMbsMatch; uMbsEvtIndex < uNTreeEntriesScal;
         uMbsEvtIndex++) {
      tInputTreeScal->GetEntry(uMbsEvtIndex);
      tTimeCurrMbsEvent.SetSec(fTriglogBoard->GetMbsTimeSec());
      tTimeCurrMbsEvent.SetNanoSec(fTriglogBoard->GetMbsTimeMilliSec()
                                   * 1000000);

      dTimeDistCurrMbsPrevA = dTimeDistCurrMbsLastA;
      dTimeDistCurrMbsPrevB = dTimeDistCurrMbsLastB;

      dTimeDistCurrMbsLastA = (Double_t)(tTimeCurrMbsEvent.GetSec())
                              + (Double_t)(tTimeCurrMbsEvent.GetNanoSec()) / 1e9
                              - (Double_t) tRpcHvEvtA.iTimeSec
                              - (Double_t) tRpcHvEvtA.iTimeMilliSec / 1000.0;

      dTimeDistCurrMbsLastB = (Double_t)(tTimeCurrMbsEvent.GetSec())
                              + (Double_t)(tTimeCurrMbsEvent.GetNanoSec()) / 1e9
                              - (Double_t) tRpcHvEvtB.iTimeSec
                              - (Double_t) tRpcHvEvtB.iTimeMilliSec / 1000.0;
      if (0 < dTimeDistCurrMbsLastA && 0 < dTimeDistCurrMbsLastB) break;
    ...
    if (TMath::Abs(dTimeDistCurrMbsPrevA) < TMath::Abs(dTimeDistCurrMbsLastA)
        && TMath::Abs(dTimeDistCurrMbsPrevB)
             < TMath::Abs(dTimeDistCurrMbsLastB))
      uLastBestMbsMatch--;
...
*/
  ...
  TCanvas* tCanvasA =
    new TCanvas("tCanvasA", "Currents and rate evolution", 0, 0, 2000, 1000);
  ...
  TCanvas* tCanvasB =
    new TCanvas("tCanvasB", "Currents VS rate", 0, 0, 2000, 1000);
