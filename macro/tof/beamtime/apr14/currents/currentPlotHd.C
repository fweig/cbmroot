...
Bool_t currentPlotHd(Int_t iDetectorIndex = -1, Int_t iDate = 0, Int_t iTime = -1, Int_t iSecOffset = 0)
{
  ...
  if (kFALSE == bAllSameSize) continue;
  ...
  if (kFALSE == bDetType) {
    ...
    hCurrentEvoNeg = new TH1D("hCurrentEvoNeg",
                              Form("Current evolution for the Negative HV of %s; CAEN readout "
                                   "time intervals ; Current [uA]",
                                   sChNameRpc[iDetectorIndex].Data()),
                              iNumHours * 3600, 0, iNumHours * 3600);

    hCurrentEvoPos = new TH1D("hCurrentEvoPos",
                              Form("Current evolution for the Positive HV of %s; CAEN readout "
                                   "time intervals ; Current [uA]",
                                   sChNameRpc[iDetectorIndex].Data()),
                              iNumHours * 3600, 0, iNumHours * 3600);

    hCurrentEvoTot = new TH1D("hCurrentEvoTot",
                              Form("Total Current evolution of %s; CAEN "
                                   "readout time intervals; Current [uA]",
                                   sChNameRpc[iDetectorIndex].Data()),
                              iNumHours * 3600, 0, iNumHours * 3600);
    ...
    hTimeBinCor = new TH1D("hTimeBinCor", "Time bin correlation; CAEN readout time intervals; Time [s]",
                           iNumHours * 3600, 0, iNumHours * 3600);
  ...
  cout << hTimeBinCor->GetMaximumBin() << endl;
  cout << hTimeBinCor->GetBinContent(hTimeBinCor->GetMaximumBin()) << endl;
