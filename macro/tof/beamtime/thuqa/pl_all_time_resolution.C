void pl_all_time_resolution(
  ...
  UInt_t iZoomChannel = 0, UInt_t DEBUG = 0)
{
...
*/
  ...
  if (DEBUG) {
    ...
    TH2D* hTotVsTimeDistTdc1 =
      new TH2D("hTotVsTimeDistTdc1",
               Form(" Tot-Distribution on TDC %u vs TimeDifference on Channel %u", uTdcNumber1, iZoomChannel), 5000,
               -1000000, 1000000, 20000, 0, 2000000);
    TH2D* hTotVsTimeDistTdc2 =
      new TH2D("hTotVsTimeDistTdc2",
               Form(" Tot-Distribution on TDC %u vs TimeDifference on Channel %u", uTdcNumber2, iZoomChannel), 5000,
               -1000000, 1000000, 20000, 0, 2000000);
  }

  TH2D* hTotDistLeft  = new TH2D("hTotDistLeft", Form("TDC %u: Tot-Distribution per channnel", uTdcNumber1), kTdcChNb,
                                -0.5, (kTdcChNb - 0.5), 20000, 0, 2000000);
  TH2D* hTotDistRight = new TH2D("hTotDistRight", Form("TDC %u: Tot-Distribution per channnel", uTdcNumber2), kTdcChNb,
                                 -0.5, (kTdcChNb - 0.5), 20000, 0, 2000000);
  ...
  for (Int_t iHistogram = 0; iHistogram < kTdcChNb; iHistogram++) {
    ...
    tHistograms[iHistogram] = new TH1D(Form("tHistogram%u", iHistogram),
                                       Form("TDCs %u-%u,chs %u-%u", uTdcNumber1, uTdcNumber2, iHistogram, iHistogram),
                                       8000000, -40000000, 40000000);
  }
  ...
  for (Int_t iHistogram = 0; iHistogram < kTdcChNb; iHistogram++) {
    ...
    tTimeHistograms[iHistogram] =
      new TH1D(Form("tTimeHistogramTDC1%u", iHistogram),
               Form("TDC %u,chs %u-%u", uTdcNumber1, iHistogram, ((iHistogram + 1) % 4)), 800000, -400000, 400000);

    tTimeHistograms[(iHistogram + kTdcChNb)] =
      new TH1D(Form("tTimeHistogramTDC2%u", iHistogram),
               Form("TDC %u,chs %u-%u", uTdcNumber2, iHistogram, ((iHistogram + 1) % 4)), 800000, -400000, 400000);
  }
  ...
  for (Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++) {
    ...
    for (Int_t iArrayEntry = 0; iArrayEntry < iArrayEntries; iArrayEntry++) {
      ...
      if (tCalibTdcData->GetBoard() == uTdcNumber2) { iDataIndexTdc2[tCalibTdcData->GetChannel()] = iArrayEntry; }
    }
...
*/
        ...
        if (iTdcCh == iZoomChannel && DEBUG) {
          hTotVsTimeDistTdc1->Fill(((TTofCalibData*) tArray->At(iDataIndexTdc1[iTdcCh]))->GetTot(),
                                   ((TTofCalibData*) tArray->At(iDataIndexTdc1[iTdcCh]))->GetTime()
                                     - ((TTofCalibData*) tArray->At(iDataIndexTdc2[iTdcCh]))->GetTime());
          hTotVsTimeDistTdc2->Fill(((TTofCalibData*) tArray->At(iDataIndexTdc2[iTdcCh]))->GetTot(),
                                   ((TTofCalibData*) tArray->At(iDataIndexTdc1[iTdcCh]))->GetTime()
                                     - ((TTofCalibData*) tArray->At(iDataIndexTdc2[iTdcCh]))->GetTime());
        }
      ...
      if (iDataIndexTdc1[iTdcCh] != -1 && iDataIndexTdc1[(iTdcCh + 1) % 4] != -1) {
        tTimeHistograms[iTdcCh]->Fill(((TTofCalibData*) tArray->At(iDataIndexTdc1[iTdcCh]))->GetTime()
                                      - ((TTofCalibData*) tArray->At(iDataIndexTdc1[(iTdcCh + 1) % 4]))->GetTime());
      }
      if (iDataIndexTdc2[iTdcCh] != -1 && iDataIndexTdc2[(iTdcCh + 1) % 4] != -1) {
        tTimeHistograms[iTdcCh + kTdcChNb]->Fill(
          ((TTofCalibData*) tArray->At(iDataIndexTdc2[iTdcCh]))->GetTime()
          - ((TTofCalibData*) tArray->At(iDataIndexTdc2[(iTdcCh + 1) % 4]))->GetTime());
      }
    }
  }
  ...
  for (Int_t iHistogram = iZoomChannel; iHistogram < iZoomChannel + 4; iHistogram++) {
    ...
    gPad->SetLogy();
    tHistograms[iHistogram]->GetXaxis()->SetRangeUser(tHistograms[iHistogram]->GetMean(1) - 10000000,
                                                      tHistograms[iHistogram]->GetMean(1) + 10000000);
  ...
  if (DEBUG) {
    ...
    for (Int_t iHistogram = 0; iHistogram < 4; iHistogram++) {
      ...
      gPad->SetLogy();
      tTimeHistograms[iHistogram]->GetXaxis()->SetRangeUser(tHistograms[iHistogram]->GetMean(1) - 10000,
                                                            tTimeHistograms[iHistogram]->GetMean(1) + 10000);
      tTimeHistograms[iHistogram]->Draw();

      tCanvas3->cd(iHistogram + 5);
      gPad->SetLogy();
      tTimeHistograms[iHistogram + kTdcChNb]->GetXaxis()->SetRangeUser(
        tTimeHistograms[iHistogram + kTdcChNb]->GetMean(1) - 10000,
        tTimeHistograms[iHistogram + kTdcChNb]->GetMean(1) + 10000);
