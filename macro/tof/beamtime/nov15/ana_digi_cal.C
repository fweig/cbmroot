void ana_digi_cal(Int_t nEvents = 1000000,
  ...
  TObjString tofDigiFile =
    workDir + "/parameters/tof/" + TofGeo + ".digi.par";  // TOF digi file
  ...
  tofTestBeamClust->SetTotMin(
    0.001);  //(12000.);  // Tot lower limit for walk correction
  tofTestBeamClust->SetTotPreRange(
    5.);  // effective lower Tot limit  in ps from peak position
  ...
  cout << "set BeamRefId to " << iRSel << ", " << iRSelSm << ", " << iRSelRpc
       << endl;
