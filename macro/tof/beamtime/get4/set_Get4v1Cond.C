...
void set_Get4v1Cond()
{
  ...
  if ((wincond0 != 0) && (strcmp(wincond0->ClassName(), "TimeWindowCond") == 0)) {
    ...
    if (kTRUE == bSyncTriggeredTimeWindow) wincond0->SetValues(-250.0, -50.0);  // Sync triggered triggering
    else
      wincond0->SetValues(-100.0, 100.0);  // Data self triggering
  }
  ...
  for (int rocid = 0; rocid < 20; rocid++) {
    ...
    if (kTRUE == bSyncTriggeredTimeWindow) wincond1->SetValues(-250.0, -50.0);  // Sync triggered triggering
    else
      wincond1->SetValues(-100.0, 100.0);  // Data self triggering
    ...
    if (kTRUE == bSyncTriggeredTimeWindow) wincond2->SetValues(-250.0, -50.0);  // Sync triggered triggering
    else
      wincond2->SetValues(-100.0, 100.0);  // Data self triggering
  }
}
