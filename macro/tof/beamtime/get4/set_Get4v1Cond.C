...
void set_Get4v1Cond() {
  ...
  if ((wincond0 != 0)
      && (strcmp(wincond0->ClassName(), "TimeWindowCond") == 0)) {
  ...
  for (int rocid = 0; rocid < 20; rocid++) {
    ...
    wincond1 =
      (TimeWindowCond*) gROOT->FindObject(Form("Roc%d_EventWindow", rocid));
    if (wincond1 == 0) continue;

    if (strcmp(wincond1->ClassName(), "TimeWindowCond")) {
      cout << "Condition " << wincond1->ClassName() << " has wrong class"
           << endl;
      continue;
    }
    ...
    wincond2 =
      (TimeWindowCond*) gROOT->FindObject(Form("Roc%d_AUXWindow", rocid));
    if (wincond2 == 0) continue;

    if (strcmp(wincond2->ClassName(), "TimeWindowCond")) {
      cout << "Condition " << wincond2->ClassName() << " has wrong class"
           << endl;
      continue;
    }
