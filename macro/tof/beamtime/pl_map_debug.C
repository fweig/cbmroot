...
void pl_map_debug(Int_t iFirstTdc = 30, Int_t iSecondTdc = 30)
{
  ...
  TH2* fhDebTdcChEvtCoinc = new TH2I(Form("fhDebTdcChEvtCoinc_tdc%03d_tdc%03d", iFirstTdc, iSecondTdc),
                                     Form("Number of events with hits for in both channels for TDC #%03d and %03d; "
                                          "Channel TDC #%03d []; Channel TDC #%03d []; Events with hits in both[]",
                                          iFirstTdc, iSecondTdc, iFirstTdc, iSecondTdc),
                                     kiNbCh, 0, kiNbCh, kiNbCh, 0, kiNbCh);
  ...
  for (Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++) {
    ...
    for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
      for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
        if (kTRUE == fbDebTdcChEvtThere[0][iChA] && kTRUE == fbDebTdcChEvtThere[1][iChB])
