void pl_ana_DTXY(Int_t iDut = 9, Int_t NewNbinsX = 3, Int_t NewNbinsY = 3, Int_t iOpt = 0)
{
  ...
  if (h3f != NULL) { h3f->ProjectionZ()->Draw(); }
  else {
  ...
  h2n = new TH2D("hTH3FitMean", "#Deltat Fit Entries; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());
  h2m = new TH2D("hTH3FitMean", "#Deltat Fit Mean; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());
  h2s = new TH2D("hTH3FitSigma", "#Deltat Fit Sigma; x (cm); y (cm)", NewNbinsX, h3f->GetXaxis()->GetXmin(),
                 h3f->GetXaxis()->GetXmax(), NewNbinsY, h3f->GetYaxis()->GetXmin(), h3f->GetYaxis()->GetXmax());

  for (Int_t ix = 0; ix < NewNbinsX; ix++) {
    for (Int_t iy = 0; iy < NewNbinsY; iy++) {
      TH1D* hsel      = h3f->ProjectionZ(Form("Deltat_%d_%d", ix, iy), ix * iDeltaX, (ix + 1) * iDeltaX, iy * iDeltaY,
                                    (iy + 1) * iDeltaY);
