void ana_digi_ini(Int_t nEvents = 10000,
                  ...
                  Int_t calSm   = 200,
                  Int_t RefSel  = 1,
                  char* cFileId = "CbmTofPiHd_22Aug1616",
                  Int_t iCalSet = 0,
                  Int_t iBRef   = 500) {
  ...
  tofTestBeamClust->SetCaldXdYMax(
    500.);  // 3.   		// Geometrical matching window in cm
  ...
  tofTestBeamClust->SetMaxTimeDist(
    500.);  //50000.	// Default cluster range in ps, aa: cluster building radius
  ...
  switch (calMode) {
    case 0:                                  // initial calibration
      tofTestBeamClust->SetTotMax(100000.);  // 100 ns (5 >> 1)
  ...
  tofAnaTestbeam->SetMul4Max(5);   // Max Multiplicity in HDRef - RPC
  ...
  cout << "dispatch: Set BeamRefId to " << iRSel << ", " << iRSelSm << ", "
       << iRSelRpc << endl;
