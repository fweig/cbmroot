...
*/
  ...
  for (Int_t jEvent = 0; jEvent < nentries; jEvent++) {
    ...
    if (!jEvent % 10000) cout << jEvent << " Events processed" << endl;
    ...
    for (Int_t jArray = 0; jArray < nentriesArray; jArray++) {
      ...
      if (DigiAdr->GetChannel() < 16) {
        ...
        if (21 == DigiAdr->GetChannel()) bPlaTrig++;
      }
    }
    //------------------------------------------------------------------------
    ...
    if (usePlaMultEvents == 1) {
      ...
      if (bRpcHit && bPla1Left && bPla1Right) h3->Fill(3);

      if (bPlaTrig) h3->Fill(4);
      ...
      if (bRpcHit && bPla1Left == 1 && bPla1Right == 1) h3->Fill(3);

      if (bPlaTrig == 1) h3->Fill(4);
  ...
  /*
	...
	TCanvas* c3 = new TCanvas( 
      ...
      c3->Update();
*/
  TCanvas* c4 = new TCanvas("c4", "c4", 1680., 1000);
  c4->SetFillColor(0);
  c4->Divide(1, 3);
  c4->SetGridx(0);
  c4->SetGridy(0);
