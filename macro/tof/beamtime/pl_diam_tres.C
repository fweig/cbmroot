void pl_diam_tres(UInt_t uTdcNumber1 = 32, UInt_t uTdcNumber2 = 33) {
  ...
  TH2D* tHistogram_Entries =
    new TH2D("tHistogram_Entries", "", kiNbCh, 0, kiNbCh, kiNbCh, 0, kiNbCh);
  TH2D* tHistogram_Mean =
    new TH2D("tHistogram_Mean", "", kiNbCh, 0, kiNbCh, kiNbCh, 0, kiNbCh);
  TH2D* tHistogram_Summary =
    new TH2D("tHistogram_Summary", "", kiNbCh, 0, kiNbCh, kiNbCh, 0, kiNbCh);
  TH1D* tHistogram[kiNbCh][kiNbCh];
  TH1D* tHistogramFit[kiNbCh][kiNbCh];
  for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
    for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
      tHistogram[iChA][iChB] =
        new TH1D(Form("tHistogram_%02d%_%02d", iChA, iChB),
                 "",
                 10000,
                 -1000000,
                 1000000);
    ...
    for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
      if (0 < tHistogram[iChA][iChB]->GetEntries()) {
        ...
        if (0 == tHistogram[iChA][iChB]->Integral()) {
          cout << "Check overflow and underflow for ch " << iChA << " vs "
               << iChB << endl;
          tHistogramFit[iChA][iChB] =
            new TH1D(Form("tHistogramFit_%02d%_%02d", iChA, iChB),
                     "",
                     10000,
                     -1000000,
                     1000000);
        ...
        Double_t dLow = tHistogram[iChA][iChB]->GetMean()
                        - 3 * tHistogram[iChA][iChB]->GetRMS();
        Double_t dHigh = tHistogram[iChA][iChB]->GetMean()
                         + 3 * tHistogram[iChA][iChB]->GetRMS();
        ...
        //cout<<iNbBins<<endl;
        if (100000 < iNbBins) iNbBins = 100000;
  ...
  TCanvas* tCanvasBAD =
    new TCanvas("tCanvasBAD", "singles zoom", 0, 0, 700, 700);
  ...
  for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
    for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
      if (0 < tHistogramFit[iChA][iChB]->GetEntries()) {
        ...
        tHistogram_Entries->Fill(
          iChA, iChB, tHistogram[iChA][iChB]->GetEntries());
        ...
        fitFunc[iChA][iChB] =
          new TF1(Form("f_%02d_%02d", iChA, iChB),
                  "gaus",
                  tHistogramFit[iChA][iChB]->GetMean()
                    - 1 * tHistogramFit[iChA][iChB]->GetRMS(),
                  tHistogramFit[iChA][iChB]->GetMean()
                    + 1 * tHistogramFit[iChA][iChB]->GetRMS());
        ...
        tHistogram_Summary->Fill(
          iChA, iChB, fitFunc[iChA][iChB]->GetParameter(2));
      }  // if( 0 < tHistogram[iChA][iChB]->GetEntries() )
