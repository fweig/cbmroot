...
void pl_diam_tres(UInt_t uTdcNumber1 = 32, UInt_t uTdcNumber2 = 33)
{
  ...
  for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
    for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
      tHistogram[iChA][iChB] = new TH1D(Form("tHistogram_%02d%_%02d", iChA, iChB), "", 10000, -1000000, 1000000);
  ...
  for (Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++) {
    ...
    for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
      for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
        if (iDataIndex1[iChA] != -1 && iDataIndex2[iChB] != -1 && 1 == iDataMulti1[iChA] && 1 == iDataMulti2[iChB]) {
          tHistogram[iChA][iChB]->Fill(((TTofCalibData*) tArray->At(iDataIndex1[iChA]))->GetTime()
      ...
      if (0 < tHistogram[iChA][iChB]->GetEntries()) {
        ...
        if (0 == tHistogram[iChA][iChB]->Integral()) {
          ...
          tHistogramFit[iChA][iChB] =
            new TH1D(Form("tHistogramFit_%02d%_%02d", iChA, iChB), "", 10000, -1000000, 1000000);
          continue;
        }  // if( 0 == tHistogram[iChA][iChB]->Integral() )
        ...
        tHistogramFit[iChA][iChB] = new TH1D(Form("tHistogramFit_%02d%_%02d", iChA, iChB), "", iNbBins, dLow, dHigh);
      }  // if( 0 < tHistogram[iChA][iChB]->GetEntries() )
      else
        tHistogramFit[iChA][iChB] =
          new TH1D(Form("tHistogramFit_%02d%_%02d", iChA, iChB), "", 10000, -1000000, 1000000);
  ...
  for (Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++) {
    ...
    for (Int_t iArrayEntry = 0; iArrayEntry < iArrayEntries; iArrayEntry++) {
      ...
      if (tCalibTdcData->GetBoard() == uTdcNumber1 && 0 < tCalibTdcData->GetTime()) {
        iDataIndex1[tCalibTdcData->GetChannel()] = iArrayEntry;
        iDataMulti1[tCalibTdcData->GetChannel()]++;
      }
      if (tCalibTdcData->GetBoard() == uTdcNumber2 && 0 < tCalibTdcData->GetTime()) {
        iDataIndex2[tCalibTdcData->GetChannel()] = iArrayEntry;
        iDataMulti2[tCalibTdcData->GetChannel()]++;
      }
    }
    ...
    for (Int_t iChA = 0; iChA < kiNbCh; iChA++)
      for (Int_t iChB = 0; iChB < kiNbCh; iChB++)
        if (iDataIndex1[iChA] != -1 && iDataIndex2[iChB] != -1 && 1 == iDataMulti1[iChA] && 1 == iDataMulti2[iChB]) {
          tHistogramFit[iChA][iChB]->Fill(((TTofCalibData*) tArray->At(iDataIndex1[iChA]))->GetTime()
                                          - ((TTofCalibData*) tArray->At(iDataIndex2[iChB]))->GetTime());
        }  // if(iDataIndex1[iChA] != -1 && iDataIndex2[iChB] != -1 )
  }        // for(Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++)
