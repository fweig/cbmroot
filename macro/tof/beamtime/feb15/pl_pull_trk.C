...
void pl_pull_trk(Int_t NSt = 5, char* var = "X", Int_t iFit = 0)
{
  ...
  switch (var) {
    case "X":
    case "Y":
    case "Z": grr->GetYaxis()->SetTitle("resolution (cm)"); break;
    default: grr->GetYaxis()->SetTitle("resolution (ps)");
  }
  grr->Draw("APL");
  grr->SetMarkerStyle(20);
  for (Int_t i = 0; i < NSt; i++)
    cout << " Resolution of counter " << i << ": " << vRes[i] << endl;
}
