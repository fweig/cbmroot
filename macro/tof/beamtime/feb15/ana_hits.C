void ana_hits(Int_t nEvents        = 10,
              Int_t iSel           = 1,
              Int_t iGenCor        = 1,
              char* cFileId        = "Cern",
              Int_t iSet           = 0,
              Int_t iSel2          = 0,
              Int_t iTrackingSetup = 3) {
  Int_t iVerbose = 1;
  ...
  TString InputDigiFile =
    paramDir + "/digi_" + cFileId + Form("_%03d", iSet) + ".out.root";
  TString OutputFile = paramDir + "/hits_" + cFileId
                       + Form("_%03d%02d%1d", iSet, iSel, iSel2) + ".out.root";
  TString cAnaFile =
    Form("%s_%03d_%02d%1d_tofAnaTestBeam.hst.root", cFileId, iSet, iSel, iSel2);
  ...
  switch (iSel) {
      ...
      switch (iSel2) {
        case 3:
          ...
          tofAnaTestbeam->SetChS2Sel(
            16.);  // Center of channel selection window
          tofAnaTestbeam->SetDChS2Sel(
            1.5);  // Width  of channel selection window
          break;
        case 9:
          tofAnaTestbeam->SetChi2Lim(
            6.);  // initialization of Chi2 selection limit
    ...
    case 94:
      ...
      switch (iSel2) {
        case 3:
          ...
          tofAnaTestbeam->SetPosYS2Sel(
            0.2);  // Y Position selection in fraction of strip length
          tofAnaTestbeam->SetPosYS2SelOff(
            -1.5);  // Y Position selection offset in cm
          tofAnaTestbeam->SetChS2Sel(
            16.);  // Center of channel selection window
          tofAnaTestbeam->SetDChS2Sel(
            1.5);  // Width  of channel selection window
    ...
    case 16:
      ...
      tofAnaTestbeam->SetPosY4Sel(
        0.1);  // Y Position selection in fraction of strip length
    ...
    case 18:
      ...
      tofAnaTestbeam->SetTOffD4(11000.);  // Shift DTD4 to physical value
      tofAnaTestbeam->SetChS2Sel(28.);    // Center of channel selection window
      tofAnaTestbeam->SetDChS2Sel(12.);   // Width  of channel selection window
      tofAnaTestbeam->SetPosYS2Sel(
        0.1);  // Y Position selection in fraction of strip length
      tofAnaTestbeam->SetChi2Lim(5.);  // initialization of Chi2 selection limit
    ...
    case 43:                            // upper part of setup: P2 - THUstrip
      ...
      switch (iSel2) {
        case 7:
          ...
          tofAnaTestbeam->SetSel2TOff(50.);  // Shift Sel2 time peak to 0
          tofAnaTestbeam->SetChS2Sel(
            8.5);  // Center of channel selection window
          tofAnaTestbeam->SetDChS2Sel(
            1.);  // Width  of channel selection window
          tofAnaTestbeam->SetPosYS2Sel(
            0.1);  // Y Position selection in fraction of strip length
          tofAnaTestbeam->SetPosYS2SelOff(
            -2.);  // Y Position selection offset in cm
          break;

        case 9:
          tofAnaTestbeam->SetChi2Lim(
            4.);  // initialization of Chi2 selection limit
          tofAnaTestbeam->SetSel2TOff(100.);  // Shift Sel2 time peak to 0
          tofAnaTestbeam->SetChS2Sel(
            11.5);  // Center of channel selection window
          tofAnaTestbeam->SetDChS2Sel(
            1.);  // Width  of channel selection window
          tofAnaTestbeam->SetPosYS2Sel(
            0.1);  // Y Position selection in fraction of strip length
    ...
    case 73:                            // upper part of setup: P2 - THUstrip
      ...
      switch (iSel2) {
        ...
        case 9:  // cuts are the same as for 43-9
          tofAnaTestbeam->SetCh4Sel(
            15.5);  // Center of channel selection window
    ...
    case 93:                            // upper part of setup: THU - P2
      ...
      tofAnaTestbeam->SetPosY4Sel(
        0.6);  // Y Position selection in fraction of strip length
      tofAnaTestbeam->SetChi2Lim(
        10.);  // initialization of Chi2 selection limit
    ...
    case 49:                            // upper part of setup: P5 - THUstrip
      ...
      tofAnaTestbeam->SetPosY4Sel(
        0.1);  // Y Position selection in fraction of strip length
      tofAnaTestbeam->SetPosY4SelOff(
        -1.5);  // Y Position selection offset in cm
      switch (iSel2) {
        case 3:
          tofAnaTestbeam->SetDTD4MAX(
            2000.);  // initialization of Max time difference Ref - BRef
          tofAnaTestbeam->SetChi2Lim(
            2.);  // initialization of Chi2 selection limit
          ...
          tofAnaTestbeam->SetChS2Sel(
            15.5);  // Center of channel selection window
          tofAnaTestbeam->SetDChS2Sel(
            0.7);  // Width  of channel selection window
