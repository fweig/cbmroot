...
/*
 * Mapping:
 * Scaler 0, OR ch.  0-15, AND ch. 1- 8: HDP2
 * Scaler 0, OR ch. 16-31, AND ch. 9-15: HDP5
 * Scaler 1, OR ch.  0-15, AND ch. 1- 8: BREF
 * Scaler 1, OR ch. 16-31, AND ch. 9-15: BR13
 * Scaler 2, OR ch.  0-15, AND ch. 1- 8: TSUS
 * Scaler 2, OR ch. 16-31, AND ch. 9-15: TSUP
 * Scaler 3, OR ch.  0-15, AND ch. 1- 8: USTC
 * Scaler 4, OR ch. 16-31, AND ch. 9-15: DIAM
 * TRLO   1, IN ch.  0- 7: Raw PMTs
 * TRLO   1, LMU OUT 0- 3: Plastics (PMT L AND PMT R)
 * TRLO   1, LMU OUT 4- 5: Setups   (PMT FL AND PMT FR AND PMT BL AND PMT BR)
 * Known problems:
 ...
 * 2) BRPC 13 had some problems with the ( (Or L #n) AND (Or R #n) ) operation
 *    => Therefore we try to use the summed OR of a single side (First 8 OR chans)
 *    => It has to be checked whether the side to use does not change with runs!!!
 *    => For this RPC the area should be the full area!!!
 * 3) BREF had the same kind of problems until ??/02/2015
 *    => may be needed to also use a single side for early runs
 */
...
void plot_Flux_Date(TString sInputName = "", TString sStartDate = "2015-02-27 23:11:00",
                    TString sStopDate = "2015-03-05 07:53:00", Int_t iNbSecPerBin = 5,
                      ...
                      Int_t iSpillDistSec = kiSpillDistSec, Int_t nEvents = -1)
{
  ...
  if (!tBranch) {
    cout << "Branch 'TofCalibScaler' not found in output tree. Abort macro "
            "execution."
         << endl;
    return;
  }  // if(!tBranch)
  ...
  for (Long64_t lBranchEntry = 0; lBranchEntry < lBranchEntries; lBranchEntry++) {
    ...
    if (tEventTime < tStartTime) continue;
    // Stop looping when chosen stop time is reached
    if (tStopTime < tEventTime) break;
