...
void TOFSMi(int, float, float, float, float);  //inner modules - dummies
...
int main(void)
{
  ...
  // now place small outer supermodules
  xpos = 120;  // offset
  ...
  // now place big outer supermodules
  //xpos=xpos+3*DXcol;  // offset

  for (int i = 4; i < 5; i++) {
    for (float ypos = 0; ypos < yposmax; ypos += dyposb) {
      nSMb++;
      TOFSMb(nSMb, Dwall - i * DZwall, 0., xpos + i * DXcol, ypos);
      nStrip += 3 * 52;
      nSMb++;
      TOFSMb(nSMb, Dwall - i * DZwall, 180., -xpos - i * DXcol, ypos);
      nStrip += 3 * 52;
  ...
  cout << nSMb << " SMb placed." << endl;

  // now place small innermost supermodule dummies
  xpos = 0.;

  for (float ypos = 0.; ypos < ypos1; ypos += dyposi) {
    nSMi++;
    TOFSMi(nSMi, Dwall + DZwall, 0., xpos, ypos);
    nStrip += 5 * 64;
    ...
    if (nSMi > 1) {
      ...
      nSMi++;
      TOFSMi(nSMi, Dwall + DZwall + dzpos, 0., xpos, ypos - dyposi / 2.);
      nStrip += 5 * 64;
  ...
  cout << nSMi << " SMi placed." << endl;
  // add mounting structure
  nPole++;
  TofPole(nPole, Dwall + DZwall, 0., xpos);

  nPole++;
  TofPole(nPole, Dwall + DZwall + dzpos, 0., xpos);
...
void TOFSMb(int i, float Z, float phi, float xpos, float ypos)
{
  ...
  float dxpos     = 490.;
  float startxpos = -560.;
  float phic      = 10. / 180. * PI;  // counter tilt angle
  float cphi      = cos(phic);
  float sphi      = sin(phic);
  ...
  if (i == 1) {
    ...
    for (int j = 1; j < 4; j++) {  //loop over counters
      if (j == 1) {
        fprintf(geof, "t%dreg%dmod#%d \n", 3, 1, j);
        fprintf(geof, "tof%dgas \n", 3);
        fprintf(geof, "BOX\n");
        ...
        for (int k = 1; k < 10; k++) {  //loop over glass plates
          if (k == 1) {
            ...
            if (k < 9) {
              ...
              fprintf(geof, "//------------------------------------------ \n");
              if (k == 4) {  //take geometry from center gap (#4)
                cout << "write geo para3 for i=" << i << " j=" << j << endl;
    ...
    for (int j = 1; j < 4; j++) {  //loop over counters (modules)
      for (int l = 0; l < nstrips; l++) {
        int k = 4;
        fprintf(parf,
                "%d \t %d \t %d \t %2d \t %5.1f \t %5.1f \t %5.1f \t %2.1f \t "
