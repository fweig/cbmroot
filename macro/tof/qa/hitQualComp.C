...
Bool_t hitQualComp(
  ...
  for (UInt_t uFile = 0; uFile < kuNbFiles; uFile++)
    for (UInt_t uDim = 0; uDim < kuNbDim; uDim++) {
      ...
      if (NULL != tempTwoDimHist)
        hMultiPntHitMeanDelta[uFile][uDim] =
      ...
      if (NULL != tempTwoDimHist)
        hMultiTrkHitMeanDelta[uFile][uDim] =
          (TH2*) (tempTwoDimHist->Clone(Form("%s_MultiTrkHitMeanDelta%s",
                                             sTagFiles[uFile].Data(),
                                             ksDimName[uDim].Data())));
      else
        return kFALSE;
      ...
      hMultiPntHitFurthestDelta_proj[uFile][uDim] =
        hMultiPntHitFurthestDelta[uFile][uDim]->ProjectionX(
          Form("%s_MultiPntHitFurthestDelta%s_proj",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data()));
      hMultiPntHitMeanDelta_proj[uFile][uDim] =
        hMultiPntHitMeanDelta[uFile][uDim]->ProjectionX(
      ...
      hMultiTrkHitFurthestDelta_proj[uFile][uDim] =
        hMultiTrkHitFurthestDelta[uFile][uDim]->ProjectionX(
          Form("%s_MultiTrkHitFurthestDelta%s_proj",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data()));
      hMultiTrkHitMeanDelta_proj[uFile][uDim] =
        hMultiTrkHitMeanDelta[uFile][uDim]->ProjectionX(
          Form("%s_MultiTrkHitMeanDelta%s_proj",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data()));
      ...
      hAllPntHitFurthestDelta[uFile][uDim] =
        (TH1*) (hMultiPntHitFurthestDelta_proj[uFile][uDim]->Clone(
          Form("%s_AllPntHitFurthestDelta%s",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data())));
      hAllPntHitMeanDelta[uFile][uDim] =
        (TH1*) (hMultiPntHitMeanDelta_proj[uFile][uDim]->Clone(
          Form("%s_AllPntHitMeanDelta%s",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data())));
      ...
      hAllTrkHitFurthestDelta[uFile][uDim] =
        (TH1*) (hMultiTrkHitFurthestDelta_proj[uFile][uDim]->Clone(
          Form("%s_AllTrkHitFurthestDelta%s",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data())));
      hAllTrkHitMeanDelta[uFile][uDim] =
        (TH1*) (hMultiTrkHitMeanDelta_proj[uFile][uDim]->Clone(
          Form("%s_AllTrkHitMeanDelta%s",
               sTagFiles[uFile].Data(),
               ksDimName[uDim].Data())));
      ...
      if ("T" == ksDimName[uDim]) {
        ...
        hRatioPntHitFurthestDelta[uFile][uDim] =
          new TH1D(Form("%s_RatioPntHitFurthestDelta%s",
                   ...
                   -0.5,
                   kuNbStepsTime * kdTimeStep - 0.5);
        hRatioPntHitMeanDelta[uFile][uDim] =
          new TH1D(Form("%s_RatioPntHitMeanDelta%s",
                        sTagFiles[uFile].Data(),
                        ksDimName[uDim].Data()),
        ...
        hRatioTrkHitFurthestDelta[uFile][uDim] =
          new TH1D(Form("%s_RatioTrkHitFurthestDelta%s",
                   ...
                   -0.5,
                   kuNbStepsTime * kdTimeStep - 0.5);
        hRatioTrkHitMeanDelta[uFile][uDim] =
          new TH1D(Form("%s_RatioTrkHitMeanDelta%s",
                        sTagFiles[uFile].Data(),
                        ksDimName[uDim].Data()),
        ...
        for (UInt_t uStep = 0; uStep < kuNbStepsTime; uStep++) {
          ...
          hRatioPntHitMeanDelta[uFile][uDim]->Fill(
            ...
            (hAllPntHitMeanDelta[uFile][uDim]->Integral(iBinMinNeg, iBinMaxNeg)
             + hAllPntHitMeanDelta[uFile][uDim]->Integral(iBinMinPos,
                                                          iBinMaxPos))
              * 100.0 / uNbTotHits[uFile]);
          ...
          hRatioTrkHitMeanDelta[uFile][uDim]->Fill(
            ...
            (hAllTrkHitMeanDelta[uFile][uDim]->Integral(iBinMinNeg, iBinMaxNeg)
             + hAllTrkHitMeanDelta[uFile][uDim]->Integral(iBinMinPos,
                                                          iBinMaxPos))
              * 100.0 / uNbTotHits[uFile]);
        }  // for( UInt_t uStep = 0; uStep < kuNbStepsTime; uStep++)
      else
  ...
  for (UInt_t uDim = 0; uDim < kuNbDim; uDim++) {
    ...
    legPntMean[uDim]->SetHeader(
      Form("Fract. of hits vs Dist. in %s, Mean", ksDimName[uDim].Data()));
    legTrkClos[uDim]->SetHeader(
      Form("Fract. of hits vs Dist. in %s, Closest", ksDimName[uDim].Data()));
    legTrkFurt[uDim]->SetHeader(
      Form("Fract. of hits vs Dist. in %s, Furthest", ksDimName[uDim].Data()));
    legTrkMean[uDim]->SetHeader(
      Form("Fract. of hits vs Dist. in %s, Mean", ksDimName[uDim].Data()));
    ...
    for (UInt_t uFile = 0; uFile < kuNbFiles; uFile++) {
      ...
      legPntMean[uDim]->AddEntry(
        hRatioPntHitMeanDelta[uFile][uDim], sTagFiles[uFile], "l");
      ...
      legTrkMean[uDim]->AddEntry(
        hRatioTrkHitMeanDelta[uFile][uDim], sTagFiles[uFile], "l");
  ...
  for (UInt_t uDim = 0; uDim < kuNbDim; uDim++) {
    if ("T" == ksDimName[uDim]) {
      ...
      histosStackMultiPntProjFurt[uDim] = new THStack(
        ...
        Form("Distance in %s from Pnt, Hits from multi Pnt, Furthest Pnt; %s "
             ...
             ksDimName[uDim].Data(),
             ksDimName[uDim].Data()));
      histosStackMultiPntProjMean[uDim] = new THStack(
        Form("histosStackMultiPntProjMean%s", ksDimName[uDim].Data()),
      ...
      histosStackMultiTrkProjFurt[uDim] = new THStack(
        ...
        Form("Distance in %s from Trk, Hits from multi Trk, Furthest Trk; %s "
             ...
             ksDimName[uDim].Data(),
             ksDimName[uDim].Data()));
      histosStackMultiTrkProjMean[uDim] = new THStack(
        Form("histosStackMultiTrkProjMean%s", ksDimName[uDim].Data()),
    ...
    legMultiPntProjMean[uDim]->SetHeader(
      Form("Dist. in %s, Mean", ksDimName[uDim].Data()));
    ...
    for (UInt_t uFile = 0; uFile < kuNbFiles; uFile++) {
      ...
      legMultiPntProjMean[uDim]->AddEntry(
        hMultiPntHitMeanDelta_proj[uFile][uDim], sTagFiles[uFile], "l");
      ...
      legMultiTrkProjMean[uDim]->AddEntry(
        hMultiTrkHitMeanDelta_proj[uFile][uDim], sTagFiles[uFile], "l");
  ...
  for (UInt_t uDim = 0; uDim < kuNbDim; uDim++) {
    ...
    for (UInt_t uFile = 0; uFile < kuNbFiles; uFile++) {
      ...
      legAllPntHitMean[uDim]->AddEntry(
        hAllPntHitMeanDelta[uFile][uDim], sTagFiles[uFile], "l");
      ...
      legMultiTrkProjMean[uDim]->AddEntry(
        hAllTrkHitMeanDelta[uFile][uDim], sTagFiles[uFile], "l");
