 ...
 **
 ** SIS-300
 ** pipe_v14p = pipe_v14j + fixed sizes of vacuum chamber for mvd_v14a
 **
...
TString macrosname = "create_bpipe_geometry_v14p.C";
// ---> Geometry file name (output)
TString rootFileName = "pipe_v14p.root";
// ---> Geometry name
TString pipeName = "pipe_v14p";
// ----------------------------------------------------------------------------
...
void create_bpipe_geometry_v14p() {
  ...
  TString pipe2name =
    "pipe2 - first window @ 220mm, h=0.5mm, R=600mm";  // beryllium !!!
  const Int_t nSects2  = 7;
  Double_t z2[nSects2] = {
    220., 220.5, 221.25, 223.51, 227.29, 230.17, 230.67};  // mm
  ...
  Double_t z01[nSects01]  = {
    -50., -5., -5., 220., 220., 220.5, 221.25, 223.51, 227.29, 230.17};  // mm
  ...
  infoFile << "SIS-300. Beam pipe geometry created with " + macrosname << endl
           << endl;
  infoFile << "	  pipe_v14p = pipe_v14j + fixed sizes of vacuum chamber for "
              "mvd_v14a"
           << endl
           << endl;
  infoFile << " The beam pipe is composed of aluminium (excluding STS section "
              "(220-1700mm) "
           << endl;
  infoFile
    << " made of beryllium with constant thickness 0.5mm) with a thickness "
    << endl;
  infoFile << " proportional to the diameter (D(z)mm/60). It is placed "
              "directly into the "
           << endl;
  infoFile << " cave as mother volume. The beam pipe consists of few sections "
              "excluding "
           << endl;
  infoFile << " RICH section (1700-3700mm) because it is part of the RICH "
              "geometry. Each "
           << endl;
  infoFile << " section has a PCON shape (including windows). There are two "
              "windows: first "
           << endl;
  infoFile << " one @ 220mm with R600mm and 0.7mm thickness, second one of "
              "iron @ 8800mm "
           << endl;
  infoFile << " with R600mm and 0.2mm thickness. The STS section is composed "
              "of cylinder "
           << endl;
  infoFile << " D(z=220-500mm)=22mm and cone (z=500-1700mm). The STS (conical "
              "part) and "
           << endl;
  infoFile << " RICH sections have half opening angle 1.6deg as a result of "
              "optimization. "
           << endl;
  infoFile << " For downstream detectors the half opening angle of sections is "
              "left the "
           << endl;
  infoFile << " same (2.5deg) as in SIS-100 and section have cylindrical shape "
              "at z=6000-"
           << endl;
  infoFile
    << " 8800mm - it must be regarded as an intermediate iteration, expecting "
    << endl;
  infoFile << " feedback from subdetectors' groups. The PSD section of the "
              "beam pipe is "
           << endl;
  infoFile
    << " missing because it is planned that it will be part of PSD geometry."
  ...
  infoFile << endl
           << "Beam pipe section: " << pipe2name << ", material: beryllium"
           << endl;
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  ...
  infoFile << endl
           << "Beam pipe section: " << pipe3name << ", material: beryllium"
           << endl;
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
