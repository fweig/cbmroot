 ...
 ** and 0.7mm thickness, second one of iron @ 8800mm with R600mm and 0.2mm 
 ** thickness. The STS section is composed of cylinder D(z=220-500mm)=22mm and 
 ** cone (z=500-1700mm). The STS (conical part) and RICH sections have half 
 ** opening angle 1.6deg as a result of optimization. For downstream detectors 
 ** the half opening angle of sections is left the same (2.5deg) as in SIS-100 
 ** and section have cylindrical shape at z=6000-8800mm - it must be regarded 
 ** as an intermediate iteration, expecting feedback from subdetectors' groups.
...
TString macrosname = "create_bpipe_geometry_v14h.C";
// ---> Geometry file name (output)
TString rootFileName = "pipe_v14h.root";
// ---> Geometry name
TString pipeName = "PIPE_v14h";
// ----------------------------------------------------------------------------
...
void create_bpipe_geometry_v14h() {
  ...
  Double_t rin01[nSects01] = {
    0., 0., 0., 0., 11., 28.69, 39.3, 65.55, 94.14, 110.};
  ...
  Double_t rin04[nSects04] = {
    0., 0., 0., 0., 0., 3.42, 70., 140., 210., 253.23};
  Double_t rout04[nSects04] = {
    99.9, 99.9, 156.17, 253.27, 253.27, 253.27, 253.27, 253.27, 253.27, 253.27};

  TString pipe6name =
    "pipe6 - second window @ 6000mm, h=0.2mm, R=600mm";  // iron !!!
  const Int_t nSects6  = 7;
  Double_t z6[nSects6] = {
    8743.24, 8743.44, 8747.34, 8759.8, 8781.19, 8799.8, 8800.};  // mm
  ...
  infoFile << "SIS-300. Beam pipe geometry created with " + macrosname << endl
           << endl;
  ...
  infoFile << " diameter (D(z)mm/60). It is placed directly into the cave as "
              "mother volume."
           << endl;
  infoFile << " The beam pipe consists of few sections excluding RICH section "
              "(1700-3700mm)"
           << endl;
  ...
  infoFile << " (including windows). There are two windows: first one @ 220mm "
              "with R600mm "
           << endl;
  infoFile << " and 0.7mm thickness, second one of iron @ 8800mm with R600mm "
              "and 0.2mm "
           << endl;
  infoFile << " thickness. The STS section is composed of cylinder "
              "D(z=220-500mm)=22mm and "
           << endl;
  infoFile << " cone (z=500-1700mm). The STS (conical part) and RICH sections "
              "have half "
           << endl;
  infoFile << " opening angle 1.6deg as a result of optimization. For "
              "downstream detectors "
           << endl;
  infoFile << " the half opening angle of sections is left the same (2.5deg) "
              "as in SIS-100 "
           << endl;
  infoFile << " and section have cylindrical shape at z=6000-8800mm - it must "
              "be regarded "
           << endl;
  infoFile << " as an intermediate iteration, expecting feedback from "
              "subdetectors' groups."
           << endl;
  ...
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  TGeoVolume* pipe5 =
    MakePipe(5, nSects5, z5, rin5, rout5, pipeMedium, &infoFile);
  pipe5->SetLineColor(kGreen);
  pipe->AddNode(pipe5, 0);
  TGeoVolume* pipevac4 =
    MakeVacuum(4, nSects04, z04, rin04, rout04, vacuum, &infoFile);
  pipevac4->SetLineColor(kCyan);
  pipe->AddNode(pipevac4, 0);

  infoFile << endl
           << "Beam pipe section: " << pipe6name << ", material: iron" << endl;
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
