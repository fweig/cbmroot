...
TString macrosname = "create_bpipe_geometry_v14i.C";
// ---> Geometry file name (output)
TString rootFileName = "pipe_v14i.root";
// ---> Geometry name
TString pipeName = "PIPE_v14i";
// ----------------------------------------------------------------------------
...
void create_bpipe_geometry_v14i() {
  ...
  infoFile << " cave as mother volume. The beam pipe consists of few sections "
              "(including "
           << endl;
  infoFile << " RICH/MUCH section). Each section has a PCON shape (including "
              "windows). "
           << endl;
  infoFile << " There are two windows: first one @ 220mm with R600mm and 0.7mm "
              "thickness, "
           << endl;
  infoFile
    << " second one of iron @ 8800mm with R600mm and 0.2mm thickness. The STS "
  ...
  infoFile
    << " section is composed of cylinder D(z=220-500mm)=22mm and cone (z=500-"
    << endl;
  infoFile << " 1700mm). The STS (conical part) and RICH sections have half "
              "opening angle "
           << endl;
  infoFile
    << " 1.6deg as a result of optimization. For downstream detectors the half "
  ...
  infoFile
    << " opening angle of sections is left the same (2.5deg) as in SIS-100 and "
    << endl;
  infoFile << " section have cylindrical shape at z=6000-8800mm - it must be "
              "regarded as an"
           << endl;
  infoFile << " intermediate iteration, expecting feedback from subdetectors' "
              "groups. The "
           << endl;
  infoFile << " PSD section of the beam pipe is missing because it is planned "
              "that it will "
           << endl;
  ...
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  TGeoVolume* pipe4 =
    MakePipe(4, nSects4, z4, rin4, rout4, pipeMedium, &infoFile);
