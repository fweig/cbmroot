...
void create_bpipe_geometry_v14d()
{
  ...
  */ //for TEST
  ...
  Double_t rin04[nSects04]  = {0.00001, 0.00001, 30.,  75.41,
                              102.26,  146.38,  183.6};  // TODO - volume boolean operation correction
  ...
  infoFile << " thickness. The last three sections (and second window @ "
              "6000mm) of beam "
           << endl;
  infoFile << " pipe (1800-7990mm) are rotated by an angle of " << Form("%.2f", rotangle) << " degrees relative to "
           << endl;
  ...
  t1->RegisterYourself();
  TGeoTranslation* t2 = new TGeoTranslation("t2", 0., 0., 190.);  // cm (370-180)
  t2->RegisterYourself();
  Double_t shift      = rotangle / 10. + 0.25;  // cm- volume boolean operation correction - TODO
  TGeoTranslation* t3 = new TGeoTranslation("t3", 0., 0., 180. - shift);  // cm - shift
  t3->RegisterYourself();
  dx = 0., dy = 0., dz = (1800. - shift * 10.);                                     // mm
  TGeoCombiTrans* c2 = new TGeoCombiTrans("c2", dx / 10., dy / 10., dz / 10., r1);  // mm->cm
  c2->RegisterYourself();
  ...
  TGeoPcon* p04 = MakeShape(nSects04, "p04", z04, rin04, rout04, &infoFileEmpty);
