 ...
 ** thickness. The last three sections (and second window @ 6000mm) of beam 
 ** pipe (1800-7990mm) are rotated by an angle of X.XX degrees relative to 
 ** point (0,0,1800) mm and the OY axis, following the bend of the ions in the 
 ** magnetic field (field_v12b was used).
 *****************************************************************************/
...
TString macrosname = "create_bpipe_geometry_v14d.C";
// ---> Geometry file name (output)
TString rootFileName = "pipe4rich_v14d.root";
// ---> Geometry name
TString pipeName = "PIPE_v14d";
// ----------------------------------------------------------------------------
...
void create_bpipe_geometry_v14d() {
  ...
  Double_t rout02[nSects02] = {17.4, 17.4, 53.17, 58.};
  /*
  ...
  */ //for TEST
  ...
  Double_t z04[nSects04] = {
    1900., 4176.19, 4176.19, 4179.19, 4182.19, 4189.13, 4196.97};  // mm - 1800
  //Double_t rin04[nSects04]  = {    0.,      0.,     30.,     75.41,  102.26,  146.38,  183.6  };
  Double_t rin04[nSects04] = {
    0.00001,
    0.00001,
    30.,
    75.41,
    102.26,
    146.38,
    183.6};  // TODO - volume boolean operation correction
  Double_t rout04[nSects04] = {
    125.24, 182.86, 182.86, 182.86, 183., 183.19, 183.6};
  ...
  Double_t z6[nSects6] = {
    4176.19, 4179.19, 4182.19, 4189.13, 4196.97, 4200.};  // mm - 1800
  ...
  infoFile << " The beam pipe is composed of aluminium with a thickness "
              "proportional to the"
           << endl;
  infoFile << " diameter (0.017*D(z)mm). It is placed directly into the cave "
              "as mother "
           << endl;
  infoFile
    << " volume. The beam pipe consists of few sections excluding RICH section"
    << endl;
  infoFile << " (1700-3700mm) because it is part of the RICH geometry. Each "
              "section has a "
           << endl;
  infoFile << " PCON shape (including windows). There are two windows: first "
              "one @ 220mm "
           << endl;
  infoFile << " with R600mm and 0.7mm thickness, second one @ 6000mm with "
              "R800mm and 3mm "
           << endl;
  infoFile << " thickness. The last three sections (and second window @ "
              "6000mm) of beam "
           << endl;
  infoFile << " pipe (1800-7990mm) are rotated by an angle of "
           << Form("%.2f", rotangle) << " degrees relative to " << endl;
  infoFile << " point (0,0,1800) mm and the OY axis, following the bend of the "
              "ions in the "
           << endl;
  infoFile << " magnetic field (field_v12b was used)." << endl << endl;
  ...
  TGeoVolume* pipevac2 =
    MakeVacuum(2, nSects02, z02, rin02, rout02, vacuum, &infoFile);
  ...
  TGeoRotation* r1 =
    new TGeoRotation("r1", 90., rotangle, 0.);  // deg - Euler angles
  r1->RegisterYourself();
  Double_t dx = 0., dy = 0., dz = 1800.;  // mm
  TGeoCombiTrans* c1 =
    new TGeoCombiTrans("c1", dx / 10., dy / 10., dz / 10., r1);  // mm->cm
  c1->RegisterYourself();
  TGeoTranslation* t1 = new TGeoTranslation("t1", 0., 0., 180.);  // cm
  t1->RegisterYourself();
  TGeoTranslation* t2 =
    new TGeoTranslation("t2", 0., 0., 190.);  // cm (370-180)
  t2->RegisterYourself();
  Double_t shift =
    rotangle / 10. + 0.25;  // cm- volume boolean operation correction - TODO
  TGeoTranslation* t3 =
    new TGeoTranslation("t3", 0., 0., 180. - shift);  // cm - shift
  t3->RegisterYourself();
  dx = 0., dy = 0., dz = (1800. - shift * 10.);  // mm
  TGeoCombiTrans* c2 =
    new TGeoCombiTrans("c2", dx / 10., dy / 10., dz / 10., r1);  // mm->cm
  c2->RegisterYourself();
  ...
  // for TEST
  TGeoVolume* pipevac3 =
    MakeVacuum(3, nSects03, z03, rin03, rout03, vacuum, &infoFile);
  pipevac3->SetLineColor(kCyan);
  //pipe->AddNode(pipevac3, 0);

  //*
  infoFile << endl << "Beam pipe section: " << pipe5name << endl;
  infoFile << "Traslation(dx,dy,dz):    " << dx << " " << dy << " " << dz
           << " mm" << endl;
  infoFile << "Rotation(phi,theta,psi): " << phi << " " << theta << " " << psi
           << " deg" << endl;
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  ////TGeoVolume* pipe5    = MakePipe  (5, nSects5,  z5,  rin5,  rout5,  pipeMedium, &infoFile);
  // create shape components for pipe5
  TGeoPcon* p5 = MakeShape(nSects5, "p5", z5, rin5, rout5, &infoFile);
  TGeoBBox* b  = new TGeoBBox("b", 20., 20., shift);  // cm
  // create a composite for pipe5
  TGeoCompositeShape* cs1 = new TGeoCompositeShape("cs1", "p5:r1-b:t2");
  TGeoVolume* pipe5       = new TGeoVolume("pipe5", cs1, pipeMedium);
  pipe5->SetLineColor(kGreen);
  pipe->AddNode(pipe5, 0, t3);
  ////pipe->AddNode(pipe5, 0, c2);
  ////TGeoVolume* pipevac4 = MakeVacuum(4, nSects04, z04, rin04, rout04, vacuum,     &infoFile);
  // create shape components for pipevac4
  TGeoPcon* p04 =
    MakeShape(nSects04, "p04", z04, rin04, rout04, &infoFileEmpty);
  // create a composite for pipevac4
  TGeoCompositeShape* cs2 = new TGeoCompositeShape("cs2", "p04:r1-b:t2");
  TGeoVolume* pipevac4    = new TGeoVolume("pipevac4", cs2, vacuum);
  pipevac4->SetLineColor(kCyan);
  pipe->AddNode(pipevac4, 0, t3);
  ////pipe->AddNode(pipevac4, 0, c2);
  //*
  ...
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  TGeoVolume* pipe6 =
    MakePipe(6, nSects6, z6, rin6, rout6, pipeMedium, &infoFile);
  pipe6->SetLineColor(kBlue);
  pipe->AddNode(pipe6, 0, c2);
  ...
  infoFile << setw(2) << "i" << setw(10) << "Z,mm" << setw(10) << "Rin,mm"
           << setw(10) << "Rout,mm" << setw(10) << "h,mm" << endl;
  TGeoVolume* pipe7 =
    MakePipe(7, nSects7, z7, rin7, rout7, pipeMedium, &infoFile);
  pipe7->SetLineColor(kGreen);
  pipe->AddNode(pipe7, 0, c2);
  TGeoVolume* pipevac5 =
    MakeVacuum(5, nSects05, z05, rin05, rout05, vacuum, &infoFile);
  pipevac5->SetLineColor(kCyan);
  pipe->AddNode(pipevac5, 0, c2);
  //*/
