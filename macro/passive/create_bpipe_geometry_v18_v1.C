...
void create_bpipe_geometry_v18_v1() {
  ...
  Double_t pipeRotationAngle =
    atan2(psdXposition - 5, psdZposition - 370.) * TMath::RadToDeg();

  Double_t pipeXshift1 =
    -370. * TMath::Tan(pipeRotationAngle * TMath::DegToRad());
  ...
  Double_t dZposMuch[nSectsMuch] = {1700., 3700. + 10};  // mm
  ...
  TString pipeNameWinVac   = "pipevac6";
  const Int_t nSectsWinVac = nSectsWin;
  Double_t dZposWinVac[nSectsWinVac];
  for (Int_t i = 0; i < nSectsWinVac; i++)
    dZposWinVac[i] = dZposWin[i];
  ...
  infoFile << " There is one window of iron the end of the pipe with R95mm and "
              "0.2mm thickness."
           << endl;
  infoFile
    << " The STS section is composed of cylinder D(z=220-500mm)=36mm and cone"
  ...
  infoFile << " MuCh/RICH section of the beam pipe with conical shape have "
           << endl;
  infoFile
    << " half opening angle 2.5deg. TRD-ToF section is composed of cylinder "
    << endl;
  infoFile << " with R161.1mm, PSD section with R95mm " << endl;
  infoFile << " and with 5 cm shift in XZ plane relative to the beam direction"
           << endl
           << endl;
  infoFile << " Detailed information find at "
              "https://cbm-wiki.gsi.de/foswiki/pub/PWG/C2F/BeamPipe/"
              "20180530_Internal_Note_beam_pipe-AS.pdf"
           << endl
           << endl;
  ...
  pipe->AddNode(pipeTrd, 0, combi_trans1);
  ...
  pipe->AddNode(pipeVacTrd, 0, combi_trans1);
