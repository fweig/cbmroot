 ...
 ** 6000mm with R800mm and 3mm thickness. The last section of beam pipe (6000-
 ** 7990mm) is rotated by an angle of X.XX degrees relative to point (0,0,1800)
 ** mm and the OY axis, following the bend of the ions in the magnetic field
 ** (field_v12b was used).
  *****************************************************************************/
...
void create_bpipe_geometry_v14a() {
  ...
  Double_t rin3[nSects3]    = {17.4, 17.4, 73.47, 100.53, 100.53};
  Double_t rout3[nSects3]   = {18., 18., 76., 104., 104.};
  ...
  Double_t rout02[nSects02] = {17.4, 17.4, 73.47, 100.53, 100.53};
  ...
  Double_t z04[nSects04]    = {
    3700., 5931.94, 5934.97, 5944.26, 5959.79, 5982.71, 5996.93};  // mm
  Double_t rin04[nSects04]  = {0., 0., 73.6, 153.6, 233.6, 313.6, 354.6};
  Double_t rout04[nSects04] = {
    218.22, 350.72, 350.9, 351.46, 352.38, 353.74, 354.6};
  ...
  Double_t z6[nSects6] = {
    5931.94, 5934.97, 5944.26, 5959.79, 5982.71, 5996.93, 6000.};  // mm
  Double_t z60[nSects6] = {
    5928.94, 5931.97, 5941.26, 5956.79, 5979.71, 5993.93, 5997.};  // mm - 3.
  ...
  infoFile
    << " volume. The beam pipe consists of few sections (including RICH/MUCH "
    << endl;
  infoFile << " section). Each section has a PCON shape (including windows). "
              "There are two "
           << endl;
  infoFile << " windows: first one @ 220mm with R600mm and 0.7mm thickness, "
              "second one @ "
           << endl;
  infoFile << " 6000mm with R800mm and 3mm thickness. The last section of beam "
              "pipe (6000-"
           << endl;
  infoFile << " 7990mm) is rotated by an angle of " << Form("%.2f", rotangle);
  infoFile << " degrees relative to point (0,0,1800)" << endl;
  infoFile << " mm and the OY axis, following the bend of the ions in the "
              "magnetic field"
           << endl;
  infoFile << " (field_v12b was used)." << endl << endl;
  ...
  TGeoCombiTrans* c1 =
    new TGeoCombiTrans("c1", dx / 10., dy / 10., dz / 10., r1);
  c1->RegisterYourself();
  ...
  TGeoPcon* p05 =
    MakeShape(nSects05, "p05", z05, rin05, rout05, &infoFileEmpty);
