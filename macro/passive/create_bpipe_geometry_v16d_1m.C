 ...
 ** @date 04.06.2014
 **
 ** pipe v16d_1m is based on v16b_1m and adapted to the streched 105 mm pitch STS v19i
...
TString macrosname = "create_bpipe_geometry_v16d_1m.C";
// ---> Geometry file name (output)
TString rootFileName    = "pipe_v16d_1m.geo.root";
TString rootFileNameExp = "pipe_v16d_1m-geo.root";
// ---> Geometry name
TString pipeName = "pipe_v16d_1m";
// ----------------------------------------------------------------------------
...
void create_bpipe_geometry_v16d_1m() {
  ...
  Double_t z2[nSects2] = {
    180., 180.7, 181.45, 183.71, 187.49, 190.17, 190.87};  // mm
  ...
  Double_t z01[nSects01] = {
    -90., -45., -45., 180., 180., 180.7, 181.45, 183.71, 187.49, 190.17};  // mm
  ...
  Double_t z3[nSects3]    = {180., 470., 1250.};  // mm
  ...
  Double_t z03[nSects03]   = {180., 470., 1250.};  // mm
  ...
  Double_t rout05[nSects05];
  for (Int_t i = 0; i < nSects05; i++) {
    rout05[i] = rin5[i];
  }
  ...
  infoFile << "SIS-100. Beam pipe geometry created with " + macrosname << endl
           << endl;
  infoFile << "	  pipe_v16d_1e = pipe_v14f + fixed sizes of vacuum chamber for "
              "mvd_v14a"
           << endl
           << endl;
  ...
  // Geometry Export - begin
  TFile* rootFileExp = new TFile(rootFileNameExp, "RECREATE");
  pipe->Export(rootFileNameExp);

  rootFileExp = new TFile(rootFileNameExp, "UPDATE");
  pipeTrans->Write();
  rootFileExp->Close();
  // Geometry Export - end
