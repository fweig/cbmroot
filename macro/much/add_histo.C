...
{ 
 ...
 TH1D *h1 = ( TH1D *)f->Get("h1"); h1->SetLineColor(kBlack);
 TH1D *h2 = ( TH1D *)f->Get("h2"); h2->SetLineColor(kRed);
 TH1D *h3 = ( TH1D *)f->Get("h3"); h3->SetLineColor(kBlue);
 TH1D *h4 = ( TH1D *)f->Get("h4"); h4->SetLineColor(kGreen);
 TH1D *h5 = ( TH1D *)f->Get("h5"); h5->SetLineColor(kMagenta);

 TH1D *h1a = ( TH1D *)h1->Clone("BgSup_Vtx");             h1a->Reset();
 TH1D *h2a = ( TH1D *)h2->Clone("BgSup_VtxSts");          h2a->Reset();
 TH1D *h3a = ( TH1D *)h3->Clone("BgSup_VtxStsMuch");      h3a->Reset();
 TH1D *h4a = ( TH1D *)h4->Clone("BgSup_VtxStsMuchTrd");   h4a->Reset();
 TH1D *h5a = ( TH1D *)h5->Clone("BgSup_VtxStsMuchTrdTof");h5a->Reset();
 ...
 for(int k=1; k<NofFiles+1; k++){ 
  ...
  if(k%100 == 0)
   ...
   if (!f1 || f1->IsZombie() || f1->GetNkeys() < 1 || f1->TestBit(TFile::kRecovered)){f1->Close(); continue;}
