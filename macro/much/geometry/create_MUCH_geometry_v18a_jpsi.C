...
TGeoVolume* CreateLayers(int istn, int ily) {
  ...
  Double_t sideDz =
    fSupportLz[istn] / 2. + fDriftDz + fG10Dz
    + fActiveLzSector / 2.;  // distance between side's and layer's centers
  ...
  Double_t DriftZ_in =
    moduleZ - dz
    - dz_sD;  //distance of the inner copper Drift center from layer layer center cs
  Double_t G10Z_in =
    DriftZ_in - dz_sD
    - dz_sG;  //distance of the innerG10 center from layer layer center cs
  Double_t DriftZ_out =
    moduleZ + dz
    + dz_sD;  //distance of the outer copper Drift center from layer layer center cs
  Double_t G10Z_out =
    DriftZ_out + dz_sD
    + dz_sG;  //distance of the outerG10 center from layer layer center cs
  Double_t coolZ =
    G10Z_out + dz_sG
    + dz_s;  //distance of the cooling plate center from layer center cs
  ...
  TGeoTube* shSupportHole =
    new TGeoTube(supportHoleName, 0., rmin, supportDz + 0.001);
  TGeoBBox* shSupportBox =
    new TGeoBBox(supportBoxName, supportDx, supportDy, supportDz);
  ...
  TString supportName1 =
    Form("muchstation%02ilayer%isupport", istn + 1, ily + 1);
  ...
  for (Int_t iSide = 0; iSide < 2; iSide++) {
    ...
    for (Int_t iModule = 0; iModule < fNSectorsPerLayer[istn]; iModule++) {
      ...
      Double_t phi =
        2 * phi0
        * (iModule
           + 0.2);  // add 0.5 to not overlap with y-axis for left-right layer separation
      ...
      pos[2] =
        (isBack ? 1 : -1) * moduleZ + layerGlobalZ0;  //z pos for ArgonActive
      posDin[2] = (isBack ? 1 : -1) * (DriftZ_in)
                  + layerGlobalZ0;  // Z pos for copper Drift innerside
      posGin[2] = (isBack ? 1 : -1) * (G10Z_in)
                  + layerGlobalZ0;  // Z pos for G10......................
      posDout[2] = (isBack ? 1 : -1) * (DriftZ_out)
                   + layerGlobalZ0;  // Z pos for copper Drift outerside
      posGout[2] = (isBack ? 1 : -1) * (G10Z_out)
                   + layerGlobalZ0;  // Z pos for G10.................
      posAl[2] = (isBack ? 1 : -1) * (coolZ)
                 + layerGlobalZ0;  // Z pos for Al Cooling Plates
      ...
      innerG10->SetName(Form(
        "shStation%02iLayer%i%cModule%03iinner", istn, ily, cside, iModule));
      TString fG10Name   = Form("muchstation%02ilayer%i%cside%03iinnerG10",
                              istn + 1,
                              ily + 1,
                              cside,
                              iMod + 1);
      ...
      innerDrift->SetName(Form(
        "shStation%02iLayer%i%cModule%03iinner", istn, ily, cside, iModule));
      TString fDriftName =
        Form("muchstation%02ilayer%i%cside%03iinnercopperDrift",
             istn + 1,
             ily + 1,
             cside,
             iMod + 1);
      TGeoVolume* vofDrift =
        new TGeoVolume(fDriftName, innerDrift, copperplate);
      vofDrift->SetLineColor(kRed);

      // Define and place Additional material  G10 on outer
      TGeoTrap* outerG10 =
        new TGeoTrap(dz_sG, 0, phi, sdy, sdx1, sdx2, 0, sdy, sdx1, sdx2, 0);
      outerG10->SetName(Form(
        "shStation%02iLayer%i%cModule%03iouter", istn, ily, cside, iModule));
      TString bG10Name   = Form("muchstation%02ilayer%i%cside%03iouterG10",
                              istn + 1,
                              ily + 1,
                              cside,
                              iMod + 1);
      ...
      TGeoTrap* outerDrift =
        new TGeoTrap(dz_sD, 0, phi, sdy, sdx1, sdx2, 0, sdy, sdx1, sdx2, 0);
      outerDrift->SetName(Form(
        "shStation%02iLayer%i%cModule%03iouter", istn, ily, cside, iModule));
      TString bDriftName =
        Form("muchstation%02ilayer%i%cside%03ioutercopperDrift",
             istn + 1,
             ily + 1,
             cside,
             iMod + 1);
      TGeoVolume* vobDrift =
        new TGeoVolume(bDriftName, outerDrift, copperplate);
      vobDrift->SetLineColor(kRed);


      // Define and place Additional material  Al Cooling Plate
      TGeoTrap* cool =
        new TGeoTrap(dz_s, 0, phi, sdy, sdx1, sdx2, 0, sdy, sdx1, sdx2, 0);
      cool->SetName(Form(
        "shStation%02iLayer%i%cModule%03icool", istn, ily, cside, iModule));
      TString CoolName   = Form("muchstation%02ilayer%i%cside%03iAluminum",
                              istn + 1,
                              ily + 1,
                              cside,
                              iMod + 1);
      ...
      TGeoTrap* shape =
        new TGeoTrap(dz, 0, phi, dy, dx1, dx2, 0, dy, dx1, dx2, 0);
      shape->SetName(Form("shStation%02iLayer%i%cModule%03iActiveNoHole",
                          istn,
                          ily,
                          cside,
                          iModule));
      ...
      TGeoTrap* shapeFrame =
        new TGeoTrap(sdz, 0, phi, sdy, sdx1, sdx2, 0, sdy, sdx1, sdx2, 0);
      shapeFrame->SetName(
      ...
      TGeoCompositeShape* shFrame = new TGeoCompositeShape(
        Form("shStation%02iLayer%i%cModule%03iFrameNoHole",
             istn,
             ily,
             cside,
             iModule),
      ...
      TGeoTranslation* trans2Din =
        new TGeoTranslation("", pos[0], pos[1], posDin[2]);
      TGeoTranslation* trans2Gin =
        new TGeoTranslation("", pos[0], pos[1], posGin[2]);
      TGeoTranslation* trans2Dout =
        new TGeoTranslation("", pos[0], pos[1], posDout[2]);
      TGeoTranslation* trans2Gout =
        new TGeoTranslation("", pos[0], pos[1], posGout[2]);
      TGeoTranslation* transAl =
        new TGeoTranslation("", pos[0], pos[1], posAl[2]);


      cout << "layerGlobalZ0: " << layerGlobalZ0
           << " Active Modules: " << pos[2] << " DriftIn: " << posDin[2]
           << " G10In: " << posGin[2] << " Driftout: " << posDout[2]
           << " G10out: " << posGout[2] << endl;
      ...
      TGeoHMatrix* incline_modGin =
        new TGeoHMatrix("");  //Inner copper Drift and G10
      (*incline_modGin)           = (*trans2Gin) * (*r2);
      TGeoHMatrix* incline_modDin = new TGeoHMatrix("");
      (*incline_modDin)           = (*trans2Din) * (*r2);

      TGeoHMatrix* incline_modGout =
        new TGeoHMatrix("");  //Outer copper Drift and G10
      ...
      volayer->AddNode(
        vobDrift, iMod, incline_modDout);  // copper Drift volume Outer Side
      volayer->AddNode(
        vobG10, iMod, incline_modGout);  // G10 volume ------------

      volayer->AddNode(voActive, iMod, incline_mod);  // add active volume
      volayer->AddNode(voFrame, iMod, incline_mod);   // add spacer ------

      volayer->AddNode(
        vofDrift, iMod, incline_modDin);  //copper  Drift volume Inner Side
      volayer->AddNode(
        vofG10, iMod, incline_modGin);  //  G10 volume ------------
