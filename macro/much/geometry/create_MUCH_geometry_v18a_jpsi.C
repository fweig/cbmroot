...
TGeoVolume* CreateLayers(int istn, int ily)
{
  ...
  Double_t sideDz =
    fSupportLz[istn] / 2. + fDriftDz + fG10Dz + fActiveLzSector / 2.;  // distance between side's and layer's centers
  ...
  for (Int_t iSide = 0; iSide < 2; iSide++) {
    ...
    for (Int_t iModule = 0; iModule < fNSectorsPerLayer[istn]; iModule++) {
      ...
      cout << "layerGlobalZ0: " << layerGlobalZ0 << " Active Modules: " << pos[2] << " DriftIn: " << posDin[2]
           << " G10In: " << posGin[2] << " Driftout: " << posDout[2] << " G10out: " << posGout[2] << endl;
