...
Double_t fAbsorberZ1[6] = {0, 24, 90, 140, 190, 250};
...
Int_t fNSectorsPerLayer[5] = {14, 18, 24, 26, 36};
...
void create_MUCH_geometry_v17a() {
  ...
  for (Int_t iabs = 0; iabs < 6; iabs++) {  // 6 pieces of absorbers
    ...
    much->AddNode(gModules[iabs], iabs);
  }
  ...
  for (Int_t ishi = 0; ishi < 4; ishi++) {
    ...
    much->AddNode(gModules_shield[ishi], ishi);
  }


  for (Int_t istn = 0; istn < 5; istn++) {  // 5 Stations
    // for (Int_t istn = 0; istn < 1; istn++) { // 1 Stations

    gModules_station[istn] = CreateStations(istn);

    much->AddNode(gModules_station[istn], istn);
  }
...
TGeoVolume* CreateShields(int ish) {
  ...
  cout << " Shields:  rmin1 " << rmin1 << " rmax 1  " << rmax1 << "  rmin2 "
       << rmin2 << "  rmax2  " << rmax2 << endl;
...
TGeoVolume* CreateLayers(int istn, int ily) {
  ...
  Double_t phi0 =
    TMath::Pi()
    / fNSectorsPerLayer[istn];  // azimuthal half widh of ecah module
  ...
  TGeoBBox* shSupportBox =
    new TGeoBBox(supportBoxName, supportDx / 2., supportDy, supportDz);
  TGeoTranslation* translation =
    new TGeoTranslation(translationName, -supportDx / 2., 0., 0.);
  translation->RegisterYourself();

  TString expression =
    supportBoxName + "-" + supportHoleName + ":" + translationName;
  TGeoCompositeShape* shSupport =
    new TGeoCompositeShape(supportShapeName, expression);

  TString supportName1 =
    Form("muchstation%02ilayer%isupport1", istn + 1, ily + 1);
  TString supportName2 =
    Form("muchstation%02ilayer%isupport2", istn + 1, ily + 1);
  TGeoMedium* coolMat = gGeoMan->GetMedium(supportmedium);

  TGeoVolume* voSupport1 = new TGeoVolume(supportName1, shSupport, coolMat);
  TGeoVolume* voSupport2 = new TGeoVolume(supportName2, shSupport, coolMat);
  voSupport1->SetLineColor(kCyan);
  voSupport2->SetLineColor(kCyan);

  TGeoRotation* rZ = new TGeoRotation("rZ");  //
  rZ->RotateZ(180.0);

  TGeoTranslation* support_trans1 =
    new TGeoTranslation("supportName1", supportDx / 2, 0, layerGlobalZ0);
  TGeoTranslation* support_trans2 =
    new TGeoTranslation("supportName2", -supportDx / 2, 0, layerGlobalZ0);

  TGeoHMatrix* support_rot2 = new TGeoHMatrix("");  // Matrix
  *support_rot2 = (*support_trans2) * (*rZ);  //Product of translation+rotation

  volayer->AddNode(voSupport1, 0, support_trans1);
  volayer->AddNode(voSupport2, 0, support_rot2);

  // Now start adding the GEM modules
  for (Int_t iModule = 0; iModule < fNSectorsPerLayer[istn]; iModule++) {

    TString moduleHoleName = Form("shStation%02iModuleHole", istn);
    TGeoTube* shModuleHole =
      new TGeoTube(moduleHoleName, 0., rmin, size[2] / 2. + 0.001);
    ...
    TString activeName = Form("muchstation%02ilayer%i%cactive%03i",
                              istn + 1,
                              ily + 1,
                              cside,
                              iModule + 1);
    ...
    TGeoVolume* voFrame = new TGeoVolume("", shFrame, noryl);
    voFrame->SetLineColor(kMagenta);


    // Calculate the phi angle of the sector where it has to be placed
    Double_t phi_cent = 180. / fNSectorsPerLayer[istn];
    Double_t angle    = phi_cent * (2 * iModule);
    ...
    cout << "DE i: " << iModule << " x: " << pos[0] << " y: " << pos[1]
         << " z: " << pos[2] << " angle: " << angle << " phi_cent: " << phi_cent
         << endl;
