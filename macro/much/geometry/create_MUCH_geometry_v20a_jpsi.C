...
TString AbsMaterial[7] =
  {"LD Graphite", "LD Graphite", "Concrete", "Iron", "Iron", "Iron", "Iron"};
...
void create_MUCH_geometry_v20a_jpsi() {
  ...
  infoFile << "MUCH geometry created with create_MUCH_geometry_v20a_jpsi.C"
           << endl
           << endl;
...
TGeoVolume* CreateShields(int ish) {
  ...
  if (ish == 5) {
    ...
    TGeoCompositeShape* shSupport_sh =
      new TGeoCompositeShape(ShapeName_sh, expression_sh);
...
TGeoVolume* CreateAbsorbers(int i) {
  ...
  if (i == 6) {
    ...
    TGeoBBox* box1 =
      new TGeoBBox(BoxName1, sh_x + 0.00001, sh_y + 0.00001, sh_z);
