...
fstream infoFile;
void create_MUCH_geometry_v20a_jpsi() {
 ...
 // -------   Open info file   -----------------------------------------------
  ...
  infoFile << "MUCH geometry created with create_MUCH_geometry_v20a_jpsi.C" << endl << endl;
...
TGeoVolume* CreateShields(int ish) {
  ...
  Double_t rmax2 = globalZ2 * fShield_AcceptanceTanMax[ish]-0.0001;
    ...
    if (ish<5) {infoFile<<"   "<<ish<<"\t\t"<<globalZ1<<"\t  "<<globalZ2<<"\t"<<rmin1<<"\t   "<<rmax1<<"\t  "<<ShMaterial[ish]<<endl;}
  else 
{infoFile<<"   "<<ish<<"\t\t"<<globalZ1<<"\t  "<<globalZ2<<"\t"<<"25.000"<<"\t   "<<"25.000"<<"\t  "<<ShMaterial[ish]<<endl;}//For the 5th absorber, a cylinder of diameter 50 cm is placed for the beam pipe.
