
{
  ...
  TCanvas* cFeeRes =
    new TCanvas("cFeeRes", "Fee Resolution after fit (for single channel)");
  cFeeRes->cd();
  ...
  TCanvas* cTimeResPairs =
    new TCanvas("cTimeResPairs", "Time Res. of pairs of selected chan");
  cTimeResPairs->Divide(4, 4);
  Int_t iPadIndexPairs = 1;
  //   TPaveStats *st;
  gStyle->SetOptStat(1110);
  gStyle->SetOptFit(2);
  for (UInt_t uChanA = 0; uChanA < kuNbChanMax && iPadIndexPairs < kuNbChanTest;
       uChanA++)
    for (UInt_t uChanB = 0;
         uChanB < kuNbChanMax && iPadIndexPairs < kuNbChanTest;
         uChanB++) {
      cTimeResPairs->cd(iPadIndexPairs);
      TH1* hHistoPtr = (TH1*) (gDirectory->FindObjectAny(
        Form("hTimeResPairs_%03u_%03u", uChanA, uChanB)));
      if (NULL != hHistoPtr) {
        gPad->SetLogy();
        hHistoPtr->Draw();
      ...
      if (32 * 4 - 1 == uChanA) uChanA += 32 * 4;
      if (32 * 4 - 1 == uChanB) uChanB += 32 * 4;
    }  // Loop on all possible channel combinations

  TCanvas* cTimeResComb = new TCanvas(
    "cTimeResComb",
    "Time Res. of all Comb. for 4 first selected chan (for single channel)");
  cTimeResComb->Divide(3, 2);
  Int_t iPadIndexComb = 1;
  for (UInt_t uChanA = 0; uChanA < kuNbChanMax && iPadIndexComb < 7; uChanA++)
    for (UInt_t uChanB = 0; uChanB < kuNbChanMax && iPadIndexComb < 7;
         uChanB++) {
      cTimeResComb->cd(iPadIndexComb);
      TH1* hHistoPtr = (TH1*) (gDirectory->FindObjectAny(
        Form("hTimeResCombi_%03u_%03u", uChanA, uChanB)));
      if (NULL != hHistoPtr) {
        gPad->SetLogy();
        hHistoPtr->Draw();
        //         st = (TPaveStats*)hHistoPtr->FindObject("stats");
  ...
  TCanvas* cChosenRmsRes =
    new TCanvas("cChosenRmsRes", "RMS and time res for chosen pairs and combi");
  ...
  if (0 < hTimeResAllFeeB->GetEntries()) {
    TCanvas* cFeeResB = new TCanvas(
      "cFeeResB", "Fee Resolution for FEE B after fit (for single channel)");
    cFeeResB->cd();
  ...
  if (0 < hTimeResAllFeeAB->GetEntries()) {
    TCanvas* cFeeResAB = new TCanvas(
      "cFeeResAB", "Fee A vs Fee B Resolution after fit (for single channel)");
    cFeeResAB->cd();
