...

{
    ...
    for (UInt_t uFee = 0; uFee < uNbFeeRoc; uFee++)
      for (UInt_t uGet4 = 0; uGet4 < uNbChipFee; uGet4++) {
        TH2* hHistoPtrD = (TH2*) (gDirectory->FindObjectAny(
          Form("hGet4DistDoubleHits_%03u", uNbChipFee * uNbTotFeeRoc * uRoc + uNbChipFee * uFee + uGet4)));
        if (NULL != hHistoPtrD) profDistDouble[uTdc] = hHistoPtrD->ProfileX(Form("ProfDouble_%03u", uTdc));
        else
          profDistDouble[uTdc] = NULL;

        TH2* hHistoPtrT = (TH2*) (gDirectory->FindObjectAny(
          Form("hGet4DistTripleHits_%03u", uNbChipFee * uNbTotFeeRoc * uRoc + uNbChipFee * uFee + uGet4)));
        if (NULL != hHistoPtrT) profDistTriple[uTdc] = hHistoPtrT->ProfileX(Form("ProfTriple_%03u", uTdc));
        else
  ...
  uTdc = 0;
  for (UInt_t uRoc = 0; uRoc < uNbRocs; uRoc++) {
    cRocS[uRoc] = new TCanvas(Form("cRocS_%03u", uRoc),
                              Form("Roc %03u summary mean double dist (blue) and mean triple dist (red)", uRoc));
