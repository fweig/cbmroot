...

{
  ...
  Int_t iNbTdcFound = 0;
  for (Int_t iTdcIdx = 0; iTdcIdx < iNbGet4; iTdcIdx++)
    if (NULL != pGet4Histos[iTdcIdx]) iNbTdcFound++;
      ...
      if (NULL != pGet4Histos[iTdcIdx]) {
        ...
        for (Int_t iCh = 0; iCh < 4; iCh++) {
          ...
          if (0 < dRate) {
            dMeanRate += dRate;
            dNbNonZeroChannels++;
            if (dRate < dMinRate || dMinRate < 0) dMinRate = dRate;
            if (dMaxRate < dRate || dMaxRate < 0) dMaxRate = dRate;
          }  // if( 0 < dRate )
  ...
  TLegend* leg = new TLegend(0.3, 0.7, 0.78, 0.9);
  gPad->Update();
  gPad->SetLogy();
  gPad->SetGridx();
  gPad->SetGridy();
