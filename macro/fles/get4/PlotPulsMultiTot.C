...

{
  ...
  for (UInt_t uRoc = 0; uRoc < uNbRocs; uRoc++) {
    cRocS[uRoc] =
      new TCanvas(Form("cRocS_%03u", uRoc), Form("Roc %03u summary Multiple hist ratio as function of Tot", uRoc));
    cRocS[uRoc]->Divide(2);
    ...
    hDoubleHitsRatioTot[uRoc] =
      new TH2D(Form("hDoubleHitsRatioTot_%03u", uRoc),
               Form("Ratio of double hits per chip Vs Tot for Roc %03u; Chip; "
                    "ToT [ns]; Ratio [\%]",
                    uRoc),
               uNbChipFee * uNbFeeRoc, -0.5, uNbChipFee * uNbFeeRoc - 0.5, iNbTotBins, 0.0, iNbTotBins);
    ...
    for (UInt_t uFee = 0; uFee < uNbFeeRoc; uFee++) {
      ...
      for (UInt_t uGet4 = 0; uGet4 < uNbChipFee; uGet4++) {
        TH2* hHistoPtr = (TH2*) (gDirectory->FindObjectAny(
          Form("hGet4MultipleHitsVsTot_%03u", uNbChipFee * uNbTotFeeRoc * uRoc + uNbChipFee * uFee + uGet4)));
        if (NULL != hHistoPtr)
